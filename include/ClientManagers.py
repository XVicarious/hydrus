...
def BuildServiceKeysToChildrenToParents(
    ...
    def AddParentsAndGrandparents(simple_children_to_parents,
                                  this_childs_parents, parents):
        ...
        for parent in parents:
            ...
            if parent in simple_children_to_parents:
                ...
                AddParentsAndGrandparents(simple_children_to_parents,
                                          this_childs_parents, grandparents)

    service_keys_to_children_to_parents = collections.defaultdict(
        HydrusData.default_dict_list)

    for (service_key, simple_children_to_parents
         ) in service_keys_to_simple_children_to_parents.items():
...
def BuildServiceKeysToSimpleChildrenToParents(service_keys_to_pairs_flat):
    ...
    service_keys_to_simple_children_to_parents = collections.defaultdict(
        HydrusData.default_dict_set)

    for (service_key, pairs) in service_keys_to_pairs_flat.items():

        service_keys_to_simple_children_to_parents[
            service_key] = BuildSimpleChildrenToParents(pairs)
...
def BuildSimpleChildrenToParents(pairs):
    ...
    for (child, parent) in pairs:
        ...
        if parent in simple_children_to_parents and LoopInSimpleChildrenToParents(
                simple_children_to_parents, child, parent):
    ...
    while True:
        ...
        for potential_loop_path in potential_loop_paths:
            ...
            if potential_loop_path in simple_children_to_parents:
                ...
                new_potential_loop_paths.update(
                    simple_children_to_parents[potential_loop_path])
...
class BitmapManager(object):
    ...
    def GetQtImageFromBuffer(self, width, height, depth, data):
        ...
        # no copy here
        qt_image = QG.QImage(data, width, height, bytes_per_line,
                             qt_image_format)
    ...
    def GetMediaBackgroundPixmap(self):
        ...
        pixmap_path = self._controller.new_options.GetNoneableString(
        ...
        if pixmap_path != self._media_background_pixmap_path:
            ...
            try:
                ...
                self._media_background_pixmap = QG.QPixmap(
                    self._media_background_pixmap_path)
...
class FileViewingStatsManager(object):
    def __init__(self, controller):
        ...
        self._my_flush_job = self._controller.CallRepeating(
            5, 60, self.REPEATINGFlush)
    ...
    def _GenerateViewsRow(self, viewtype, viewtime_delta):
        ...
        if viewtype == 'preview':
            ...
            preview_min = new_options.GetNoneableInteger(
                'file_viewing_statistics_preview_min_time')
            preview_max = new_options.GetNoneableInteger(
            ...
            if viewtime_delta == 'media_duplicates_filter' and not new_options.GetBoolean(
            ...
            if do_it:
                ...
                media_min = new_options.GetNoneableInteger(
                    'file_viewing_statistics_media_min_time')
                media_max = new_options.GetNoneableInteger(
        ...
        return (preview_views_delta, preview_viewtime_delta, media_views_delta,
                media_viewtime_delta)

    def _PubSubRow(self, hash, row):

        (preview_views_delta, preview_viewtime_delta, media_views_delta,
         media_viewtime_delta) = row

        pubsub_row = (hash, preview_views_delta, preview_viewtime_delta,
                      media_views_delta, media_viewtime_delta)
    ...
    def Flush(self):
        ...
        with self._lock:
            ...
            if len(self._pending_updates) > 0:
                ...
                for (hash,
                     (preview_views_delta, preview_viewtime_delta,
                      media_views_delta,
                      media_viewtime_delta)) in self._pending_updates.items():

                    row = (hash, preview_views_delta, preview_viewtime_delta,
                           media_views_delta, media_viewtime_delta)

                    content_update = HydrusData.ContentUpdate(
                        HC.CONTENT_TYPE_FILE_VIEWING_STATS,
                        HC.CONTENT_UPDATE_ADD, row)

                    content_updates.append(content_update)

                service_keys_to_content_updates = {
                    CC.COMBINED_LOCAL_FILE_SERVICE_KEY: content_updates
                }

                # non-synchronous
                self._controller.Write('content_updates',
                                       service_keys_to_content_updates,
                                       do_pubsubs=False)
    ...
    def FinishViewing(self, viewtype, hash, viewtime_delta):
        ...
        with self._lock:
            ...
            if hash not in self._pending_updates:
                ...
                (existing_preview_views_delta, existing_preview_viewtime_delta,
                 existing_media_views_delta,
                 existing_media_viewtime_delta) = self._pending_updates[hash]

                self._pending_updates[hash] = (
                    existing_preview_views_delta + preview_views_delta,
                    existing_preview_viewtime_delta + preview_viewtime_delta,
                    existing_media_views_delta + media_views_delta,
                    existing_media_viewtime_delta + media_viewtime_delta)
...
class ServicesManager(object):
    ...
    def _SetServices(self, services):
        ...
        self._keys_to_services = {
            service.GetServiceKey(): service
            for service in services
        }

        self._keys_to_services[
            CC.TEST_SERVICE_KEY] = ClientServices.GenerateService(
                CC.TEST_SERVICE_KEY, HC.TEST_SERVICE, 'test service')
    ...
    def Filter(self, service_keys, desired_types):
        ...
        with self._lock:
            ...
            filtered_service_keys = [
                service_key for service_key in service_keys
                if service_key in self._keys_to_services
                and self._keys_to_services[service_key].GetServiceType() in
                desired_types
            ]
    ...
    def GetServiceKeys(self, desired_types=HC.ALL_SERVICES):
        ...
        with self._lock:
            ...
            filtered_service_keys = [
                service_key
                for (service_key, service) in self._keys_to_services.items()
                if service.GetServiceType() in desired_types
            ]
    ...
    def GetServices(self, desired_types=HC.ALL_SERVICES, randomised=True):
        ...
        with self._lock:
            ...
            services = [
                service for service in self._services_sorted
                if service.GetServiceType() in desired_types
            ]
...
class TagParentsManager(object):
    def __init__(self, controller):
        ...
        self._service_keys_to_children_to_parents = collections.defaultdict(
            HydrusData.default_dict_list)
    ...
    def _RefreshParents(self):
        ...
        service_keys_to_statuses_to_pairs = self._controller.Read(
            'tag_parents')
        ...
        for (service_key, statuses_to_pairs
            ...
            pairs_flat = statuses_to_pairs[HC.CONTENT_STATUS_CURRENT].union(
        ...
        service_keys_to_pairs_flat[
            CC.COMBINED_TAG_SERVICE_KEY] = combined_pairs_flat
    ...
    def ExpandTags(self, service_key, tags, service_strict=False):
        ...
        with self._lock:
            ...
            for tag in tags:
                ...
                tags_results.update(
                    self._service_keys_to_children_to_parents[service_key]
                    [tag])
    ...
    def NotifyNewParents(self):
        ...
        with self._lock:
            ...
            self._refresh_job = self._controller.CallLater(
                8.0, self.RefreshParentsIfDirty)
...
class TagSiblingsManager(object):
    def __init__(self, controller):
        ...
        self._controller.sub(self, 'NotifyNewSiblings',
                             'notify_new_siblings_data')
    ...
    def _RefreshSiblings(self):
        ...
        for (service_key,
            ...
            all_pairs = statuses_to_pairs[HC.CONTENT_STATUS_CURRENT].union(
                statuses_to_pairs[HC.CONTENT_STATUS_PENDING])
        ...
        combined_siblings = CollapseTagSiblingPairs(
            [local_tags_pairs, tag_repo_pairs])

        self._service_keys_to_siblings[
            CC.COMBINED_TAG_SERVICE_KEY] = combined_siblings
        ...
        self._service_keys_to_reverse_lookup[
            CC.COMBINED_TAG_SERVICE_KEY] = combined_reverse_lookup

    def CollapsePredicates(self,
                           service_key,
                           predicates,
                           service_strict=False):
            ...
            for tag in tags:
                ...
                if tag in siblings:
                    ...
                    if new_tag not in tags_to_predicates:
                        ...
                        new_predicate = ClientSearch.Predicate(
                            old_pred_type, new_tag, old_inclusive)
    ...
    def CollapseStatusesToTags(self,
                               service_key,
                               statuses_to_tags,
                               service_strict=False):
        ...
        with self._lock:
            ...
            for (status, tags) in statuses_to_tags.items():
                ...
                new_statuses_to_tags[status] = self._CollapseTags(
                    service_key, tags)
    ...
    def NotifyNewSiblings(self):
        ...
        with self._lock:
            ...
            self._refresh_job = self._controller.CallLater(
                8.0, self.RefreshSiblingsIfDirty)
...
class UndoManager(object):
    ...
    def _FilterServiceKeysToContentUpdates(self,
                                           service_keys_to_content_updates):
        ...
        for (service_key,
            ...
            for content_update in content_updates:
                ...
                if data_type == HC.CONTENT_TYPE_FILES:
                    ...
                    if action in (HC.CONTENT_UPDATE_ADD,
                    ...
                    if action in (HC.CONTENT_UPDATE_RESCIND_PETITION,
                                  HC.CONTENT_UPDATE_ADVANCED):
                ...
                filtered_content_update = HydrusData.ContentUpdate(
                    data_type, action, row)
            ...
            if len(filtered_content_updates) > 0:
                ...
                filtered_service_keys_to_content_updates[
                    service_key] = filtered_content_updates

        return filtered_service_keys_to_content_updates

    def _InvertServiceKeysToContentUpdates(self,
                                           service_keys_to_content_updates):
            ...
            for content_update in content_updates:
                ...
                if data_type == HC.CONTENT_TYPE_FILES:
                    ...
                    if action == HC.CONTENT_UPDATE_ARCHIVE:
                        inverted_action = HC.CONTENT_UPDATE_INBOX
                    elif action == HC.CONTENT_UPDATE_INBOX:
                        inverted_action = HC.CONTENT_UPDATE_ARCHIVE
                    elif action == HC.CONTENT_UPDATE_PEND:
                        inverted_action = HC.CONTENT_UPDATE_RESCIND_PEND
                    elif action == HC.CONTENT_UPDATE_RESCIND_PEND:
                        inverted_action = HC.CONTENT_UPDATE_PEND
                    ...
                    if action == HC.CONTENT_UPDATE_ADD:
                        inverted_action = HC.CONTENT_UPDATE_DELETE
                    elif action == HC.CONTENT_UPDATE_DELETE:
                        inverted_action = HC.CONTENT_UPDATE_ADD
                    elif action == HC.CONTENT_UPDATE_PEND:
                        inverted_action = HC.CONTENT_UPDATE_RESCIND_PEND
                    elif action == HC.CONTENT_UPDATE_RESCIND_PEND:
                        inverted_action = HC.CONTENT_UPDATE_PEND
                ...
                inverted_content_update = HydrusData.ContentUpdate(
                    data_type, inverted_action, inverted_row)

                inverted_content_updates.append(inverted_content_update)

            inverted_service_keys_to_content_updates[
                service_key] = inverted_content_updates
    ...
    def AddCommand(self, action, *args, **kwargs):
        ...
        with self._lock:
            ...
            if action == 'content_updates':
                ...
                service_keys_to_content_updates = self._FilterServiceKeysToContentUpdates(
                    service_keys_to_content_updates)

                if len(service_keys_to_content_updates) == 0: return

                inverted_service_keys_to_content_updates = self._InvertServiceKeysToContentUpdates(
                    service_keys_to_content_updates)
            ...
            self._inverted_commands = self._inverted_commands[:self.
                                                              _current_index]

            self._commands.append((action, args, kwargs))

            self._inverted_commands.append(
                (inverted_action, inverted_args, inverted_kwargs))
    ...
    def Undo(self):
        ...
        with self._lock:
            ...
            if self._current_index > 0:
                ...
                (action, args,
                 kwargs) = self._inverted_commands[self._current_index]
