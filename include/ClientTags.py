...
def SortTags(sort_by, tags_list, tags_to_count=None):
    ...
    if sort_by in (CC.SORT_BY_INCIDENCE_ASC, CC.SORT_BY_INCIDENCE_DESC,
        ...
        if sort_by in (CC.SORT_BY_INCIDENCE_ASC,
                       CC.SORT_BY_INCIDENCE_NAMESPACE_ASC):
        ...
        elif sort_by in (CC.SORT_BY_INCIDENCE_DESC,
                         CC.SORT_BY_INCIDENCE_NAMESPACE_DESC):
        ...
        if sort_by in (CC.SORT_BY_INCIDENCE_NAMESPACE_ASC,
                       CC.SORT_BY_INCIDENCE_NAMESPACE_DESC):
        ...
        elif sort_by in (CC.SORT_BY_LEXICOGRAPHIC_IGNORE_NAMESPACE_ASC,
                         CC.SORT_BY_LEXICOGRAPHIC_IGNORE_NAMESPACE_DESC):
...
class ServiceKeysToTags(HydrusSerialisable.SerialisableBase,
    ...
    def _GetSerialisableInfo(self):
        ...
        return [(service_key.hex(), list(tags))
                for (service_key, tags) in self.items()]
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_SERVICE_KEYS_TO_TAGS] = ServiceKeysToTags
...
class TagDisplayManager(HydrusSerialisable.SerialisableBase):
    ...
    def __init__(self):
        ...
        service_keys_to_tag_filters_defaultdict = lambda: collections.defaultdict(
            TagFilter)

        self._tag_display_types_to_service_keys_to_tag_filters = collections.defaultdict(
            service_keys_to_tag_filters_defaultdict)
    ...
    def FilterTags(self, tag_display_type, service_key, tags):
        ...
        with self._lock:
            ...
            if service_key in self._tag_display_types_to_service_keys_to_tag_filters[
                    tag_display_type]:

                tag_filter = self._tag_display_types_to_service_keys_to_tag_filters[
                    tag_display_type][service_key]

                tags = tag_filter.Filter(tags)

            if service_key != CC.COMBINED_TAG_SERVICE_KEY and CC.COMBINED_TAG_SERVICE_KEY in self._tag_display_types_to_service_keys_to_tag_filters[
                    tag_display_type]:

                tag_filter = self._tag_display_types_to_service_keys_to_tag_filters[
                    tag_display_type][CC.COMBINED_TAG_SERVICE_KEY]
    ...
    def GetTagFilter(self, tag_display_type, service_key):
        ...
        with self._lock:
            ...
            return self._tag_display_types_to_service_keys_to_tag_filters[
                tag_display_type][service_key].Duplicate()
        ...
        with self._lock:
            ...
            if tag_filter.AllowsEverything():
                ...
                if service_key in self._tag_display_types_to_service_keys_to_tag_filters[
                    ...
                    del self._tag_display_types_to_service_keys_to_tag_filters[
                        tag_display_type][service_key]
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_TAG_DISPLAY_MANAGER] = TagDisplayManager
        ...
        with self._lock:
            ...
            if len(blacklist) == 0:
                ...
                if set(blacklist) == {'', ':'}:
                    ...
                    text = 'blacklisting on ' + ', '.join(
                        (ConvertTagSliceToString(tag_slice)
                         for tag_slice in blacklist))

                if len(whitelist) > 0:

                    text += ' except ' + ', '.join(
                        (ConvertTagSliceToString(tag_slice)
                         for tag_slice in whitelist))
        ...
        with self._lock:
            ...
            if len(blacklist) == 0:
                ...
                if set(blacklist) == {'', ':'}:
                    ...
                    text = 'all but ' + ', '.join(
                        (ConvertTagSliceToString(tag_slice)
                         for tag_slice in blacklist)) + ' allowed'
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_TAG_FILTER] = TagFilter
