...
def GenerateExportFilename(destination_directory,
                           media,
                           terms,
                           append_number=None):
    ...
    for (term_type, term) in terms:
        ...
        if term_type == 'string':
            ...
            tags = tags_manager.GetNamespaceSlice(
                (term, ), ClientTags.TAG_DISPLAY_SIBLINGS_AND_PARENTS)
            ...
            if tags_manager.HasTag(
                    subtag, ClientTags.TAG_DISPLAY_SIBLINGS_AND_PARENTS):
...
class ExportFolder(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def __init__(self,
        ...
        if phrase is None:
            ...
            phrase = HG.client_controller.new_options.GetString(
                'export_phrase')
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._path, self._export_type,
                self._delete_from_client_after_export,
                serialisable_file_search_context, self._run_regularly,
                self._period, self._phrase, self._last_checked, self._paused,
                self._run_now)
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (self._path, self._export_type, self._delete_from_client_after_export,
         ...
         self._phrase, self._last_checked, self._paused,
         self._run_now) = serialisable_info
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (export_type, serialisable_file_search_context, period, phrase,
             last_checked) = old_serialisable_info

            path = self._name

            new_serialisable_info = (path, export_type,
                                     serialisable_file_search_context, period,
                                     phrase, last_checked)
        ...
        if version == 2:
            ...
            (path, export_type, serialisable_file_search_context, period,
             phrase, last_checked) = old_serialisable_info
        ...
        if version == 3:
            ...
            (path, export_type, delete_from_client_after_export,
             serialisable_file_search_context, period, phrase,
             last_checked) = old_serialisable_info
            ...
            new_serialisable_info = (path, export_type,
                                     delete_from_client_after_export,
                                     serialisable_file_search_context,
                                     run_regularly, period, phrase,
                                     last_checked, paused, run_now)
    ...
    def DoWork(self):
        ...
        regular_run_due = self._run_regularly and HydrusData.TimeHasPassed(
        ...
        try:
            ...
            if not os.path.exists(self._path):
                ...
                raise Exception('The path, "{}", does not exist!'.format(
                    self._path))

            if not os.path.isdir(self._path):

                raise Exception('The path, "{}", is not a directory!'.format(
                    self._path))
            ...
            HydrusData.ShowText(
                'The export folder "' + self._name +
                '" encountered an error! The error will follow! It has now been paused. Please check the folder\'s settings and maybe report to hydrus dev if the error is complicated!'
            )
    ...
    def ToTuple(self):
        ...
        return (self._name, self._path, self._export_type,
                self._delete_from_client_after_export,
                self._file_search_context, self._run_regularly, self._period,
                self._phrase, self._last_checked, self._paused, self._run_now)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_EXPORT_FOLDER] = ExportFolder
