        ...
        if mime in HC.DECOMPRESSION_BOMB_IMAGES and not decompression_bombs_ok:
            ...
            if HydrusImageHandling.IsDecompressionBomb(path):
                ...
                raise HydrusExceptions.InsufficientCredentialsException(
                    'File seemed to be a Decompression Bomb!')
...
hydrus_favicon = FileResource(os.path.join(HC.STATIC_DIR, 'hydrus.ico'),
...
class HydrusResource(Resource):
    ...
    def _checkUserAgent(self, request):
        ...
        if request.requestHeaders.hasHeader('User-Agent'):
            ...
            user_agent_texts = request.requestHeaders.getRawHeaders(
                'User-Agent')
                    ...
                    if client == 'hydrus':
                        ...
                        if network_version == HC.NETWORK_VERSION:
                            ...
                            if network_version < HC.NETWORK_VERSION:
                                ...
                                message = 'This server is out of date; please ask its admin to update to the latest release.'

                            raise HydrusExceptions.NetworkVersionException(
                                'Network version mismatch! This server\'s network version is '
                                + str(HC.NETWORK_VERSION) +
                                ', whereas your client\'s is ' +
                                str(network_version) + '! ' + message)
    ...
    def _errbackHandleProcessingError(self, failure, request):
        ...
        if failure.type == HydrusExceptions.BadRequestException:
            ...
            response_context = ResponseContext(400,
                                               mime=default_mime,
                                               body=default_encoding(
                                                   failure.value))

        elif failure.type in (HydrusExceptions.MissingCredentialsException,
                              HydrusExceptions.DoesNotSupportCORSException):

            response_context = ResponseContext(401,
                                               mime=default_mime,
                                               body=default_encoding(
                                                   failure.value))
            ...
            response_context = ResponseContext(403,
                                               mime=default_mime,
                                               body=default_encoding(
                                                   failure.value))

        elif failure.type in (HydrusExceptions.NotFoundException,
                              HydrusExceptions.DataMissing,
                              HydrusExceptions.FileMissingException):

            response_context = ResponseContext(404,
                                               mime=default_mime,
                                               body=default_encoding(
                                                   failure.value))
            ...
            response_context = ResponseContext(419,
                                               mime=default_mime,
                                               body=default_encoding(
                                                   failure.value))
            ...
            response_context = ResponseContext(426,
                                               mime=default_mime,
                                               body=default_encoding(
                                                   failure.value))
            ...
            response_context = ResponseContext(503,
                                               mime=default_mime,
                                               body=default_encoding(
                                                   failure.value))
            ...
            response_context = ResponseContext(509,
                                               mime=default_mime,
                                               body=default_encoding(
                                                   failure.value))
            ...
            response_context = ResponseContext(500,
                                               mime=default_mime,
                                               body=default_encoding(
                                                   failure.value))
    ...
    def _parseHydrusNetworkAccessKey(self, request, key_required=True):
        ...
        try:
            ...
            raise HydrusExceptions.InsufficientCredentialsException(
                'Could not parse the hydrus key!')
    ...
    def _threadDoOPTIONSJob(self, request):
        ...
        if request.requestHeaders.hasHeader('Origin'):
            ...
            if self._service.SupportsCORS():
                ...
                # 401
                raise HydrusExceptions.DoesNotSupportCORSException(
                    'This service does not support CORS.')
...
class ResponseContext(object):
    def __init__(self,
        ...
        if body is None:
            ...
            raise Exception(
                'Was given an incompatible object to respond with: ' +
                repr(body))
