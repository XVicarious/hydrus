...
from . import (ClientGUICommon, ClientGUIDialogs, ClientGUIDialogsQuick,
               ...
               ClientGUITopLevelWindows, ClientImportGallerySeeds, ClientPaths,
               ClientSerialisable)
...
class EditGallerySeedLogPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, controller, read_only, can_generate_more_pages,
        ...
        self._list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self, 'gallery_seed_log', 30, 30, columns,
    ...
    def _CopySelectedGalleryURLs(self):
        ...
        if len(gallery_seeds) > 0:
            ...
            text = separator.join(
                (gallery_seed.url for gallery_seed in gallery_seeds))
    ...
    def _GetListCtrlMenu(self):
        ...
        ClientGUIMenus.AppendMenuItem(
            menu, 'copy urls', 'Copy all the selected urls to clipboard.',
            self._CopySelectedGalleryURLs)
        ClientGUIMenus.AppendMenuItem(
        ...
        ClientGUIMenus.AppendMenuItem(
            menu, 'open urls',
            'Open all the selected urls in your web browser.',
            self._OpenSelectedGalleryURLs)
        ...
        if not self._read_only:
            ...
            ClientGUIMenus.AppendMenuItem(
                menu, 'try again (just this one page)',
                'Schedule this url to occur again.',
                HydrusData.Call(self._TrySelectedAgain, False))

            if self._can_generate_more_pages:
                ClientGUIMenus.AppendMenuItem(
                    menu, 'try again (and allow search to continue)',
                    'Schedule this url to occur again and continue.',
                    HydrusData.Call(self._TrySelectedAgain, True))

        ClientGUIMenus.AppendMenuItem(
            menu, 'skip', 'Skip all the selected urls.',
    ...
    def _TrySelectedAgain(self, can_generate_more_pages):
        ...
        for gallery_seed in gallery_seeds:
            ...
            new_gallery_seeds.append(
                gallery_seed.GenerateRestartedDuplicate(
...
class GallerySeedLogButton(ClientGUICommon.BetterBitmapButton):
    def __init__(self,
        ...
        ClientGUICommon.BetterBitmapButton.__init__(
            self, parent, CC.GlobalPixmaps.listctrl,
            self._ShowGallerySeedLogFrame)
        ...
        self.setToolTip(
            'open detailed gallery log--right-click for quick actions, if applicable'
        )
    ...
    def _ClearGallerySeeds(self, statuses_to_remove):
        ...
        message = 'Are you sure you want to delete all the ' + '/'.join(
            (CC.status_string_lookup[status] for status in statuses_to_remove)
        ) + ' gallery log entries? This is useful for cleaning up and de-laggifying a very large list, but be careful you aren\'t removing something you would want to revisit.'
    ...
    def _ImportURLs(self, urls):
        ...
        if len(filtered_urls) < len(urls):
            ...
            message = 'Of the ' + HydrusData.ToHumanInt(
                ...
                num_removed
            ) + ' are already in the gallery log. Would you like to only add new URLs or add everything (which will force a re-check of the duplicates)?'
    ...
    def ShowMenu(self):
        ...
        if len(gallery_seed_log) > 0:
            ...
            ClientGUIMenus.AppendMenuItem(
                submenu, 'to png',
                'Export all the urls in this list to a png file.',
                self._ExportToPng)
        ...
        if not self._read_only:
            ...
            ClientGUIMenus.AppendMenuItem(
                submenu, 'from png',
                'Import new urls to this list from a png file.',
                self._ImportFromPng)
...
class GallerySeedLogStatusControl(QW.QFrame):
    def __init__(self,
        ...
        QP.AddToLayout(hbox, self._log_summary_st,
    ...
    def _Update(self):
        ...
        if self._gallery_seed_log is None:
            ...
            (import_summary, (num_done,
                              num_to_do)) = self._gallery_seed_log.GetStatus()
