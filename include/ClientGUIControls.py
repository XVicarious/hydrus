...
class EditStringConverterPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, string_converter, example_string_override=None):
        ...
        self._transformations = ClientGUIListCtrl.BetterListCtrl(
            transformations_panel,
            'string_converter_transformations',
            7,
            35,
            columns,
            self._ConvertTransformationToListCtrlTuples,
            delete_key_callback=self._DeleteTransformation,
            activation_callback=self._EditTransformation)

        transformations_panel.SetListCtrl(self._transformations)

        transformations_panel.AddButton('add', self._AddTransformation)
        transformations_panel.AddButton('edit',
                                        self._EditTransformation,
                                        enabled_only_on_selection=True)
        transformations_panel.AddDeleteButton()

        transformations_panel.AddSeparator()

        transformations_panel.AddButton('move up',
                                        self._MoveUp,
                                        enabled_check_func=self._CanMoveUp)
        transformations_panel.AddButton('move down',
                                        self._MoveDown,
                                        enabled_check_func=self._CanMoveDown)

        self._example_string = QW.QLineEdit(self)

        #

        self._transformations.AddDatas([
            (i + 1, transformation_type, data)
            for (i, (transformation_type,
        ...
        )  # to refresh, now they are all in the list
    ...
    def _AddTransformation(self):
        ...
        try:
            ...
            example_string_at_this_point = string_converter.Convert(
                self._example_string.text())
        ...
        self._transformations.UpdateDatas(
        )  # need to refresh string after the insertion, so the new row can be included in the parsing calcs
    ...
    def _ConvertTransformationToListCtrlTuples(self, transformation):
        ...
        pretty_transformation = ClientParsing.StringConverter.TransformationToString(
        ...
        try:
            ...
            pretty_result = ClientParsing.MakeParsedTextPretty(
                string_converter.Convert(self._example_string.text(), number))
    ...
    def _EditTransformation(self):
        ...
        for enumerated_transformation in selected_data:
            ...
            try:
                ...
                example_string_at_this_point = string_converter.Convert(
                    self._example_string.text(), number - 1)
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                ...
                panel = self._TransformationPanel(
                    dlg, transformation_type, data,
                    example_string_at_this_point)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    self._transformations.DeleteDatas(
                        (enumerated_transformation, ))

                    (transformation_type, data) = panel.GetValue()

                    enumerated_transformation = (number, transformation_type,
                                                 data)
    ...
    def _GetValue(self):
        ...
        transformations = [(transformation_type, data)
                           for (number, transformation_type,
                                data) in enumerated_transformations]

        example_string = self._example_string.text()

        string_converter = ClientParsing.StringConverter(
            transformations, example_string)

        return string_converter

    def _MoveDown(self):

        selected_transformation = self._transformations.GetData(
            only_selected=True)[0]
    ...
    def GetValue(self):
        ...
        try:
            ...
            raise HydrusExceptions.VetoException(
                'Please enter an example text that can be converted!')
    ...
    class _TransformationPanel(ClientGUIScrolledPanels.EditPanel):
        def __init__(self, parent, transformation_type, data, example_text):
            ...
            for t_type in (
                    ...
                    ClientParsing.STRING_TRANSFORMATION_REMOVE_TEXT_FROM_END,
                    ClientParsing.
                    STRING_TRANSFORMATION_CLIP_TEXT_FROM_BEGINNING,
                    ClientParsing.STRING_TRANSFORMATION_CLIP_TEXT_FROM_END,
                    ClientParsing.STRING_TRANSFORMATION_PREPEND_TEXT,
                    ClientParsing.STRING_TRANSFORMATION_APPEND_TEXT,
                    ClientParsing.STRING_TRANSFORMATION_ENCODE,
                    ClientParsing.STRING_TRANSFORMATION_DECODE,
                    ClientParsing.STRING_TRANSFORMATION_REVERSE,
                    ClientParsing.STRING_TRANSFORMATION_REGEX_SUB,
                    ClientParsing.STRING_TRANSFORMATION_DATE_DECODE,
                    ClientParsing.STRING_TRANSFORMATION_DATE_ENCODE,
                    ClientParsing.STRING_TRANSFORMATION_INTEGER_ADDITION):

                self._transformation_type.addItem(
                    ClientParsing.transformation_type_str_lookup[t_type],
                    t_type)

            self._example_string = QW.QLineEdit(self)

            min_width = ClientGUIFunctions.ConvertTextToPixelWidth(
                self._example_string, 96)
            ...
            self._data_date_link = ClientGUICommon.BetterHyperLink(
                self, 'link to date info',
            ...
            self._data_timezone_offset = QP.MakeQSpinBox(self,
                                                         min=-86400,
                                                         max=86400)
            ...
            self._data_text_label = ClientGUICommon.BetterStaticText(
                self, 'string data: ')
            self._data_number_label = ClientGUICommon.BetterStaticText(
                self, 'number data: ')
            self._data_encoding_label = ClientGUICommon.BetterStaticText(
                self, 'encoding data: ')
            self._data_regex_repl_label = ClientGUICommon.BetterStaticText(
                self, 'regex replacement: ')
            self._data_date_link_label = ClientGUICommon.BetterStaticText(
                self, 'date info: ')
            self._data_timezone_decode_label = ClientGUICommon.BetterStaticText(
                self, 'date decode timezone: ')
            self._data_timezone_offset_label = ClientGUICommon.BetterStaticText(
                self, 'timezone offset: ')
            self._data_timezone_encode_label = ClientGUICommon.BetterStaticText(
                self, 'date encode timezone: ')
            ...
            rows.append(
                (self._data_timezone_decode_label, self._data_timezone_decode))
            rows.append(
                (self._data_timezone_offset_label, self._data_timezone_offset))
            rows.append(
                (self._data_timezone_encode_label, self._data_timezone_encode))

            self._control_gridbox = ClientGUICommon.WrapInGrid(self, rows)

            vbox = QP.VBoxLayout()

            QP.AddToLayout(vbox, self._control_gridbox,
            ...
            self._transformation_type.currentIndexChanged.connect(
                self._UpdateDataControls)
            self._transformation_type.currentIndexChanged.connect(
            ...
            self._data_encoding.currentIndexChanged.connect(
                self._UpdateExampleText)
            self._data_regex_repl.textEdited.connect(self._UpdateExampleText)
            self._data_timezone_decode.currentIndexChanged.connect(
                self._UpdateExampleText)
            self._data_timezone_offset.valueChanged.connect(
                self._UpdateExampleText)
            self._data_timezone_encode.currentIndexChanged.connect(
                self._UpdateExampleText)
            ...
            if transformation_type in (
                ...
                if transformation_type == ClientParsing.STRING_TRANSFORMATION_PREPEND_TEXT:
                    ...
                    if transformation_type == ClientParsing.STRING_TRANSFORMATION_DATE_DECODE:
                        ...
                        if self._data_timezone_decode.GetValue(
                        ) == HC.TIMEZONE_OFFSET:
        ...
        def _UpdateExampleText(self):
            ...
            try:
                ...
                example_transformation = string_converter.Convert(
                    example_string)

                try:

                    self._example_transformation.setText(
                        str(example_transformation))

                except:

                    self._example_transformation.setText(
                        repr(example_transformation))
...
class EditStringMatchPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, string_match=None):
        ...
        self._match_type.addItem('any characters',
                                 ClientParsing.STRING_MATCH_ANY)
        self._match_type.addItem('fixed characters',
                                 ClientParsing.STRING_MATCH_FIXED)
        self._match_type.addItem('character set',
                                 ClientParsing.STRING_MATCH_FLEXIBLE)
        ...
        self._match_value_flexible_input.addItem(
            'alphabetic characters (a-zA-Z)', ClientParsing.ALPHA)
        self._match_value_flexible_input.addItem(
            'alphanumeric characters (a-zA-Z0-9)', ClientParsing.ALPHANUMERIC)
        self._match_value_flexible_input.addItem('numeric characters (0-9)',
                                                 ClientParsing.NUMERIC)

        self._min_chars = ClientGUICommon.NoneableSpinCtrl(
            self, min=1, max=65535, unit='characters', none_phrase='no limit')
        self._max_chars = ClientGUICommon.NoneableSpinCtrl(
            self, min=1, max=65535, unit='characters', none_phrase='no limit')
        ...
        rows.append(('match value (character set): ',
                     self._match_value_flexible_input))
        rows.append(
            ('minumum allowed number of characters: ', self._min_chars))
        rows.append(
            ('maximum allowed number of characters: ', self._max_chars))
        rows.append(('example string: ', self._example_string))
        ...
        QP.AddToLayout(vbox, gridbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, self._example_string_matches,
        ...
        self._match_value_flexible_input.currentIndexChanged.connect(
    ...
    def _GetValue(self):
        ...
        string_match = ClientParsing.StringMatch(match_type=match_type,
    ...
    def _UpdateControls(self):
        ...
        if match_type == ClientParsing.STRING_MATCH_FIXED:
            ...
            self._example_string.blockSignals(
                True
            )  # Temporarily block the text changed signal here so we won't end up in infinite recursion
            ...
            try:
                ...
                QP.SetForegroundColour(self._example_string_matches,
                                       (0, 128, 0))
    ...
    def SetValue(self, string_match):
        ...
        (match_type, match_value, min_chars, max_chars,
         example_string) = string_match.ToTuple()
...
class NetworkJobControl(QW.QFrame):
    def __init__(self, parent):
        ...
        self._cog_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.cog, self._ShowCogMenu)
        self._cancel_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.stop, self.Cancel)
    ...
    def _ShowCogMenu(self):
        ...
        if self._network_job is not None:
            ...
            if self._network_job.CurrentlyWaitingOnConnectionError():
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'reattempt connection now',
                    'Stop waiting on a connection error and reattempt the job now.',
                    self._network_job.OverrideConnectionErrorWait)

            if self._network_job.CurrentlyWaitingOnServersideBandwidth():

                ClientGUIMenus.AppendMenuItem(
                    menu,
                    'reattempt request now (server reports low bandwidth)',
                    'Stop waiting on a serverside bandwidth delay and reattempt the job now.',
                    self._network_job.OverrideServersideBandwidthWait)

            if self._network_job.ObeysBandwidth():

                ClientGUIMenus.AppendMenuItem(
                    menu, 'override bandwidth rules for this job',
            ...
            if not self._network_job.TokensOK():
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'override gallery slot requirements for this job',
                    'Force-allow this download to proceed, ignoring the normal gallery wait times.',
                    self._network_job.OverrideToken)
        ...
        ClientGUIMenus.AppendMenuCheckItem(
            ...
            self._auto_override_bandwidth_rules,
            self.FlipAutoOverrideBandwidth)
    ...
    def _OverrideBandwidthIfAppropriate(self):
        ...
        if self._network_job is None or self._network_job.NoEngineYet():
            ...
            if self._auto_override_bandwidth_rules and HydrusData.TimeHasPassed(
                    self._network_job.GetCreationTime() + 5):
        ...
        if self._network_job is None or self._network_job.NoEngineYet():
            ...
            if self._download_started and not self._network_job.HasError():
                ...
                if current_speed != bytes_to_read:  # if it is a real quick download, just say its size
                    ...
                    speed_text += ' ' + HydrusData.ToHumanBytes(
                        current_speed) + '/s'

            self._right_text.setText(speed_text)

            right_width = ClientGUIFunctions.ConvertTextToPixelWidth(
                self._right_text, len(speed_text))
...
class StringConverterButton(ClientGUICommon.BetterButton):
    ...
    def _Edit(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit string converter',
                frame_key='deeply_nested_dialog') as dlg:

            panel = EditStringConverterPanel(
                dlg,
                self._string_converter,
                example_string_override=self._example_string_override)
    ...
    def _UpdateLabel(self):
        ...
        if num_rules == 0:
            ...
            label = HydrusData.ToHumanInt(
                num_rules) + ' string transformations'
...
class StringMatchToStringMatchDictControl(QW.QWidget):
    ...
    def _Add(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, 'edit ' +
                                                 self._key_name) as dlg:
...
class StringToStringDictButton(ClientGUICommon.BetterButton):
    ...
    def _Edit(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit string dictionary') as dlg:
...
class StringToStringDictControl(QW.QWidget):
    ...
    def __init__(self,
        ...
        self._listctrl = ClientGUIListCtrl.BetterListCtrl(
            ...
            columns,
            self._ConvertDataToListCtrlTuples,
            use_simple_delete=use_simple_delete,
            activation_callback=self._Edit)
