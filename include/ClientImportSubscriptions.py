...
class Subscription(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def __init__(self, name, gug_key_and_name=None):
        ...
        if HC.options['gallery_file_limit'] is None:
            ...
            self._initial_file_limit = min(100,
                                           HC.options['gallery_file_limit'])
    ...
    def _CanDoWorkNow(self):
        ...
        p1 = not (self._paused
                  or HG.client_controller.options['pause_subs_sync']
                  or HG.client_controller.new_options.GetBoolean(
        ...
        if HG.subscription_report_mode:
            ...
            message += os.linesep
            message += 'Paused/Global/Network Pause: {}/{}/{}'.format(
                self._paused, HG.client_controller.options['pause_subs_sync'],
                HG.client_controller.new_options.GetBoolean(
                    'pause_all_new_network_traffic'))
            message += os.linesep
            message += 'View/Thread shutdown: {}/{}'.format(
                HG.view_shutdown, HydrusThreading.IsThreadShuttingDown())
            message += os.linesep
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_queries = [
            query.GetSerialisableTuple() for query in self._queries
        ]
        ...
        return (serialisable_gug_key_and_name, serialisable_queries,
                serialisable_checker_options, self._initial_file_limit,
                self._periodic_file_limit, self._paused,
                serialisable_file_import_options,
                serialisable_tag_import_options, self._no_work_until,
                self._no_work_until_reason, self._show_a_popup_while_working,
                self._publish_files_to_popup_button,
                self._publish_files_to_page, self._publish_label_override,
                self._merge_query_publish_events)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_gug_key_and_name, serialisable_queries,
         serialisable_checker_options, self._initial_file_limit,
         self._periodic_file_limit, self._paused,
         serialisable_file_import_options, serialisable_tag_import_options,
         self._no_work_until, self._no_work_until_reason,
         self._show_a_popup_while_working, self._publish_files_to_popup_button,
         self._publish_files_to_page, self._publish_label_override,
         self._merge_query_publish_events) = serialisable_info

        (serialisable_gug_key, gug_name) = serialisable_gug_key_and_name

        self._gug_key_and_name = (bytes.fromhex(serialisable_gug_key),
                                  gug_name)
        self._queries = [
            HydrusSerialisable.CreateFromSerialisableTuple(serialisable_query)
            for serialisable_query in serialisable_queries
        ]
        self._checker_options = HydrusSerialisable.CreateFromSerialisableTuple(
    ...
    def _GenerateNetworkJobFactory(self, query):
        ...
        def network_job_factory(*args, **kwargs):
            ...
            network_job = ClientNetworkingJobs.NetworkJobSubscription(
                subscription_key, *args, **kwargs)
    ...
    def _QueryFileLoginIsOK(self, query):
        ...
        if file_seed is None:
            ...
            nj = file_seed.GetExampleNetworkJob(
                self._GenerateNetworkJobFactory(query))
            ...
            if nj.NeedsLogin():
                ...
                try:
                    ...
                    if not self._paused:
                        ...
                        message = 'Query "' + query.GetHumanName(
                        ) + '" for subscription "' + self._name + '" seemed to have an invalid login for one of its file imports. The reason was:'
        ...
        if HG.subscription_report_mode:
            ...
            HydrusData.ShowText('Query "' + query.GetHumanName() +
                                '" pre-work file login test. Login ok: ' +
                                str(result) + '.')
    ...
    def _QuerySyncLoginIsOK(self, query):
        ...
        if gallery_seed is None:
            ...
            nj = gallery_seed.GetExampleNetworkJob(
                self._GenerateNetworkJobFactory(query))
            ...
            if nj.NeedsLogin():
                ...
                try:
                    ...
                    if not self._paused:
                        ...
                        message = 'Query "' + query.GetHumanName(
                        ) + '" for subscription "' + self._name + '" seemed to have an invalid login. The reason was:'
                        message += os.linesep * 2
                        message += login_fail_reason
                        message += os.linesep * 2
                        message += 'The subscription has paused. Please see if you can fix the problem and then unpause. Hydrus dev would like feedback on this process.'
        ...
        if HG.subscription_report_mode:
            ...
            HydrusData.ShowText('Query "' + query.GetHumanName() +
                                '" pre-work sync login test. Login ok: ' +
                                str(result) + '.')
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (serialisable_gallery_identifier,
             serialisable_gallery_stream_identifiers, query, period,
             get_tags_if_url_recognised_and_file_redundant, initial_file_limit,
             periodic_file_limit, paused, serialisable_file_import_options,
             serialisable_tag_import_options, last_checked, last_error,
             serialisable_file_seed_cache) = old_serialisable_info
            ...
            new_serialisable_info = (
                serialisable_gallery_identifier,
                serialisable_gallery_stream_identifiers, query, period,
                get_tags_if_url_recognised_and_file_redundant,
                initial_file_limit, periodic_file_limit, paused,
                serialisable_file_import_options,
                serialisable_tag_import_options, last_checked, check_now,
                last_error, serialisable_file_seed_cache)
        ...
        if version == 2:
            ...
            (serialisable_gallery_identifier,
             ...
             periodic_file_limit, paused, serialisable_file_import_options,
             serialisable_tag_import_options, last_checked, check_now,
             last_error, serialisable_file_seed_cache) = old_serialisable_info
            ...
            new_serialisable_info = (
                ...
                serialisable_file_import_options,
                serialisable_tag_import_options, last_checked, check_now,
                last_error, no_work_until, no_work_until_reason,
        ...
        if version == 3:
            ...
            checker_options = ClientImportOptions.CheckerOptions(
                5, period // 5, period * 10, (1, period * 10))
            ...
            serialisable_queries = [
                query.GetSerialisableTuple() for query in queries
            ]
            ...
            new_serialisable_info = (
                ...
                initial_file_limit, periodic_file_limit, paused,
                serialisable_file_import_options,
                serialisable_tag_import_options, no_work_until,
                no_work_until_reason)
        ...
        if version == 4:
            ...
            new_serialisable_info = (serialisable_gallery_identifier,
                                     serialisable_gallery_stream_identifiers,
                                     serialisable_queries,
                                     serialisable_checker_options,
                                     initial_file_limit, periodic_file_limit,
                                     paused, serialisable_file_import_options,
                                     serialisable_tag_import_options,
        ...
        if version == 5:
            ...
            (serialisable_gallery_identifier,
             serialisable_gallery_stream_identifiers, serialisable_queries,
             serialisable_checker_options, initial_file_limit,
             periodic_file_limit, paused, serialisable_file_import_options,
             serialisable_tag_import_options, no_work_until,
             no_work_until_reason) = old_serialisable_info
        ...
        if version == 6:
            ...
            (serialisable_gallery_identifier,
             ...
             periodic_file_limit, paused, serialisable_file_import_options,
             serialisable_tag_import_options, no_work_until,
             no_work_until_reason, publish_files_to_popup_button,
             publish_files_to_page,
        ...
        if version == 7:
            ...
            gallery_identifier = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_gallery_identifier)

            (gug_key, gug_name
             ) = ClientDownloading.ConvertGalleryIdentifierToGUGKeyAndName(
                 gallery_identifier)

            serialisable_gug_key_and_name = (gug_key.hex(), gug_name)

            new_serialisable_info = (serialisable_gug_key_and_name,
                                     serialisable_queries,
                                     serialisable_checker_options,
                                     initial_file_limit, periodic_file_limit,
                                     paused, serialisable_file_import_options,
        ...
        if version == 9:
            ...
            (serialisable_gug_key_and_name, serialisable_queries,
             ...
             periodic_file_limit, paused, serialisable_file_import_options,
             serialisable_tag_import_options, no_work_until,
             no_work_until_reason, show_a_popup_while_working,
             publish_files_to_popup_button, publish_files_to_page,
             merge_query_publish_events) = old_serialisable_info
    ...
    def _WorkOnFiles(self, job_key):
        ...
        for (i, query) in enumerate(queries):
            ...
            if num_queries > 1:
                ...
                text_1 += ' (' + HydrusData.ConvertValueRangeToPrettyString(
                    i + 1, num_queries) + ')'
            ...
            starting_num_unknown = file_seed_cache.GetFileSeedCount(
                CC.STATUS_UNKNOWN)
            ...
            try:
                ...
                while True:
                    ...
                    file_seed = file_seed_cache.GetNextFileSeed(
                        CC.STATUS_UNKNOWN)
                    ...
                    if file_seed is None:
                        ...
                        if HG.subscription_report_mode:
                            ...
                            HydrusData.ShowText(
                                'Query "' + query_name +
                                '" can do no more file work due to running out of unknown urls.'
                            )
                    ...
                    try:
                        ...
                        num_unknown = file_seed_cache.GetFileSeedCount(
                            CC.STATUS_UNKNOWN)
                        ...
                        x_out_of_y = 'file ' + HydrusData.ConvertValueRangeToPrettyString(
                            human_num_done + 1, human_num_urls) + ': '

                        job_key.SetVariable('popup_gauge_2',
                                            (human_num_done, human_num_urls))
                        ...
                        file_seed.WorkOnURL(
                            file_seed_cache, status_hook,
                            self._GenerateNetworkJobFactory(query),
                            ClientImporting.
                            GenerateMultiplePopupNetworkJobPresentationContextFactory(
                                job_key), self._file_import_options,
                            self._tag_import_options)

                        query_tag_import_options = query.GetTagImportOptions()

                        if query_tag_import_options.HasAdditionalTags(
                        ) and file_seed.status in CC.SUCCESSFUL_IMPORT_STATES:

                            if file_seed.HasHash():

                                hash = file_seed.GetHash()

                                in_inbox = HG.client_controller.Read(
                                    'in_inbox', hash)

                                downloaded_tags = []

                                service_keys_to_content_updates = query_tag_import_options.GetServiceKeysToContentUpdates(
                                    file_seed.status, in_inbox, hash,
                        ...
                        error_count_threshold = HG.client_controller.new_options.GetNoneableInteger(
                            'subscription_file_error_cancel_threshold')

                        if error_count_threshold is not None and error_count >= error_count_threshold:

                            raise Exception(
                                'The subscription ' + self._name +
                                ' encountered several errors when downloading files, so it abandoned its sync.'
                            )
                    ...
                    if len(presentation_hashes) > 0:
                        ...
                        job_key.SetVariable(
                            'popup_files',
                            (list(presentation_hashes), query_summary_name))
                ...
                if len(presentation_hashes) > 0:
                    ...
                    ClientImporting.PublishPresentationHashes(
                        publishing_label, presentation_hashes,
                        self._publish_files_to_popup_button,
    ...
    def _WorkOnFilesCanDoWork(self):
            ...
            if query.HasFileWorkToDo():
                ...
                if query.BandwidthIsOK(self._name):
                    ...
                    if HG.subscription_report_mode:
                        ...
                        HydrusData.ShowText(
                            'Subscription "{}" checking if any file work due: True'
                            .format(self._name))
        ...
        if HG.subscription_report_mode:
            ...
            HydrusData.ShowText(
                'Subscription "{}" checking if any file work due: False'.
                format(self._name))
    ...
    def _SyncQuery(self, job_key):
        ...
        if gug is None:
            ...
            HydrusData.ShowText(
                'The subscription "' + self._name +
                '" could not find a Gallery URL Generator for "' +
                self._gug_key_and_name[1] + '"! The sub has paused!')
        ...
        if not gug.IsFunctional():
            ...
            HydrusData.ShowText(
                'The subscription "' + self._name +
                '"\'s Gallery URL Generator, "' + self._gug_key_and_name[1] +
                '" seems not to be functional! Maybe it needs a gallery url class or a gallery parser? The sub has paused!'
            )

            return

        self._gug_key_and_name = gug.GetGUGKeyAndName(
        )  # just a refresher, to keep up with any changes
        ...
        for (i, query) in enumerate(queries):
            ...
            if num_queries > 1:
                ...
                prefix += ' (' + HydrusData.ConvertValueRangeToPrettyString(
                    i + 1, num_queries) + ')'
            ...
            if len(initial_search_urls) == 0:
                ...
                HydrusData.ShowText(
                    'The subscription "' + self._name +
                    '"\'s Gallery URL Generator, "' +
                    self._gug_key_and_name[1] +
                    '" did not generate any URLs! The sub has paused!')

                return

            gallery_seeds = [
                ClientImportGallerySeeds.GallerySeed(
                    url, can_generate_more_pages=True)
            ...
            try:
                ...
                while gallery_seed_log.WorkToDo():
                    ...
                    gallery_seed = gallery_seed_log.GetNextGallerySeed(
                        CC.STATUS_UNKNOWN)
                    ...
                    def file_seeds_callable(file_seeds):
                            ...
                            if file_limit_for_this_sync is not None and total_new_urls_for_this_sync + num_urls_added >= file_limit_for_this_sync:
                                ...
                                if this_is_initial_sync:
                                    ...
                                    if total_already_in_urls_for_this_sync + num_urls_already_in_file_seed_cache > 0:
                                        ...
                                        self._ShowHitPeriodicFileLimitMessage(
                                            query_name)
                        ...
                        if current_contiguous_num_urls_already_in_file_seed_cache >= WE_HIT_OLD_GROUND_THRESHOLD:
                            ...
                            stop_reason = 'saw ' + HydrusData.ToHumanInt(
                                current_contiguous_num_urls_already_in_file_seed_cache
                            ) + ' previously seen urls, so assuming we caught up'
                        ...
                        return (num_urls_added,
                                num_urls_already_in_file_seed_cache,
                                can_search_for_more_files, stop_reason)

                    job_key.SetVariable(
                        'popup_text_1', prefix + ': found ' +
                        HydrusData.ToHumanInt(total_new_urls_for_this_sync) +
                        ' new urls, checking next page')
                    ...
                    try:
                        ...
                        (
                            num_urls_added,
                            num_urls_already_in_file_seed_cache,
                            num_urls_total, result_404,
                            added_new_gallery_pages, stop_reason
                        ) = gallery_seed.WorkOnURL(
                            'subscription',
                            gallery_seed_log,
                            file_seeds_callable,
                            status_hook,
                            ...
                            gallery_urls_seen_before=gallery_urls_seen_this_sync
                        )
            ...
            if query.IsDead():
                ...
                if this_is_initial_sync:
                    ...
                    HydrusData.ShowText(
                        ...
                        self._name +
                        '" did not find any files on its first sync! Could the query text have a typo, like a missing underscore?'
                    )

                else:

                    HydrusData.ShowText('The query "' + query_name +
                                        '" for subscription "' + self._name +
                                        '" appears to be dead!')
                ...
                if this_is_initial_sync:
                    ...
                    if not query.BandwidthIsOK(
                            self._name
                    ) and not have_made_an_initial_sync_bandwidth_notification:

                        HydrusData.ShowText(
                            'FYI: The query "' + query_name +
                            '" for subscription "' + self._name +
                            '" performed its initial sync ok, but that domain is short on bandwidth right now, so no files will be downloaded yet. The subscription will catch up in future as bandwidth becomes available. You can review the estimated time until bandwidth is available under the manage subscriptions dialog. If more queries are performing initial syncs in this run, they may be the same.'
                        )
    ...
    def _SyncQueryCanDoWork(self):
        ...
        if HG.subscription_report_mode:
            ...
            HydrusData.ShowText(
                'Subscription "{}" checking if any sync work due: {}'.format(
                    self._name, result))
    ...
    def GetBestEarliestNextWorkTime(self):
        ...
        latest_nearby_next_work_time = max(
            (work_time for work_time in next_work_times
             if work_time < earliest_next_work_time + LAUNCH_WINDOW))
        ...
        if not HydrusData.TimeHasPassed(self._no_work_until):
            ...
            best_next_work_time = max(
                (best_next_work_time, self._no_work_until))
    ...
    def GetPresentationOptions(self):
        ...
        return (self._show_a_popup_while_working,
                self._publish_files_to_popup_button,
                self._publish_files_to_page, self._publish_label_override,
                self._merge_query_publish_events)
    ...
    def Merge(self, mergees):
        ...
        for subscription in mergees:
            ...
            if subscription._gug_key_and_name[1] == self._gug_key_and_name[1]:
                ...
                my_new_queries = [
                    query.Duplicate() for query in subscription._queries
                ]
                ...
                raise Exception(
                    self._name +
                    ' was told to merge an unmergeable subscription, ' +
    ...
    def Separate(self, base_name, only_these_queries=None):
        ...
        self._queries = [
            query for query in my_queries if query not in only_these_queries
        ]
    ...
    def SetPresentationOptions(self, show_a_popup_while_working,
                               publish_files_to_popup_button,
    ...
    def SetTuple(self, gug_key_and_name, queries, checker_options,
                 initial_file_limit, periodic_file_limit, paused,
                 file_import_options, tag_import_options, no_work_until):
    ...
    def Sync(self):
        ...
        if self._CanDoWorkNow() and (sync_ok or files_ok):
            ...
            try:
                ...
                HydrusData.Print(
                    'The subscription ' + self._name +
                    ' encountered an exception when trying to sync:')

                HydrusData.Print(e)

                job_key.SetVariable(
                    'popup_text_1',
                    'Encountered a network error, will retry again later')
    ...
    def ToTuple(self):
        ...
        return (self._name, self._gug_key_and_name, self._queries,
                self._checker_options, self._initial_file_limit,
                self._periodic_file_limit, self._paused,
                self._file_import_options, self._tag_import_options,
                self._no_work_until, self._no_work_until_reason)
...
class SubscriptionJob(object):
    ...
    def _DoWork(self):
        ...
        if HG.subscription_report_mode:
            ...
            HydrusData.ShowText('Subscription "{}" about to start.'.format(
                self._subscription.GetName()))
...
class SubscriptionsManager(object):
    ...
    def _ClearFinishedSubscriptions(self):
        ...
        for (name,
             (thread, job,
              subscription)) in list(self._running_subscriptions.items()):
    ...
    def _GetNameReadyToGo(self):
        ...
        p2 = HG.client_controller.new_options.GetBoolean(
            'pause_all_new_network_traffic')
        p3 = HG.view_shutdown
        ...
        max_simultaneous_subscriptions = HG.client_controller.new_options.GetInteger(
        ...
        possible_names.difference_update(
            set(self._running_subscriptions.keys()))
        ...
        names_not_due = {
            name
            for (name,
                 next_work_time) in self._names_to_next_work_time.items()
            if not HydrusData.TimeHasPassed(next_work_time +
        ...
        if HG.subscription_report_mode:
            ...
            HydrusData.ShowText(
                'Subscription manager selected "{}" to start.'.format(
                    subscription_name))
        ...
        if subscription.AllPaused():
            ...
            if next_work_time is None:
                ...
                if just_finished_work:
                    ...
                    next_work_time = max(next_work_time,
                                         HydrusData.GetNow() + BUFFER_TIME)
    ...
    def MainLoop(self):
        ...
        try:
                ...
                if subscription_name is not None:
                    ...
                    try:
                        ...
                        try:
                            ...
                            subscription = self._controller.Read(
                                'serialisable_named', HydrusSerialisable.
                                SERIALISABLE_TYPE_SUBSCRIPTION,
                                subscription_name)

                        except Exception as e:

                            HydrusData.ShowText(
                                'Subscription "{}" failed to load! Error information should follow. No more subscriptions will run this boot.'
                        ...
                        thread = threading.Thread(target=job.Work,
                        ...
                        with self._lock:
                            ...
                            self._running_subscriptions[subscription_name] = (
                                thread, job, subscription)
            ...
            with self._lock:
                ...
                for (name,
                     (thread, job,
                      subscription)) in self._running_subscriptions.items():
    ...
    def NewSubscriptions(self, subscriptions):
        ...
        with self._lock:
            ...
            self._current_subscription_names = {
                subscription.GetName()
    ...
    def ShowSnapshot(self):
        ...
        with self._lock:
            ...
            message = '{} subs: {}'.format(
                HydrusData.ToHumanInt(len(self._current_subscription_names)),
                ', '.join(subs))
            message += os.linesep * 2
            message += '{} running: {}'.format(
                HydrusData.ToHumanInt(len(self._running_subscriptions)),
                ', '.join(running))
            message += os.linesep * 2
            message += '{} not runnable: {}'.format(
                HydrusData.ToHumanInt(len(self._names_that_cannot_run)),
                ', '.join(cannot_run))
            message += os.linesep * 2
            message += '{} next times: {}'.format(
                HydrusData.ToHumanInt(len(self._names_to_next_work_time)),
                ', '.join(('{}: {}'.format(
                    name,
                    HydrusData.TimestampToPrettyTimeDelta(next_work_time))
                           for (name, next_work_time) in next_times)))
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_SUBSCRIPTION] = Subscription
...
class SubscriptionQuery(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._query, self._display_name, self._check_now,
                self._last_check_time, self._next_check_time, self._paused,
                self._status, serialisable_gallery_seed_log,
                serialisable_file_seed_cache, serialisable_tag_import_options)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._query, self._display_name, self._check_now,
         self._last_check_time, self._next_check_time, self._paused,
         self._status, serialisable_gallery_seed_log,
         serialisable_file_seed_cache,
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (query, check_now, last_check_time, next_check_time, paused,
             status, serialisable_file_seed_cache) = old_serialisable_info
            ...
            new_serialisable_info = (query, check_now, last_check_time,
                                     next_check_time, paused, status,
                                     serialisable_gallery_seed_log,
                                     serialisable_file_seed_cache)
        ...
        if version == 2:
            ...
            (query, check_now, last_check_time, next_check_time, paused,
             status, serialisable_gallery_seed_log,
             serialisable_file_seed_cache) = old_serialisable_info
            ...
            new_serialisable_info = (query, display_name, check_now,
                                     last_check_time, next_check_time, paused,
                                     status, serialisable_gallery_seed_log,
                                     serialisable_file_seed_cache,
                                     serialisable_tag_import_options)
    ...
    def BandwidthIsOK(self, subscription_name):
        ...
        example_network_contexts = self._GetExampleNetworkContexts(
            subscription_name)

        threshold = 90

        result = HG.client_controller.network_engine.bandwidth_manager.CanDoWork(
            example_network_contexts, threshold=threshold)

        if HG.subscription_report_mode:

            HydrusData.ShowText('Query "' + self.GetHumanName() +
                                '" bandwidth test. Bandwidth ok: ' +
                                str(result) + '.')
    ...
    def GetBandwidthWaitingEstimate(self, subscription_name):
        ...
        ) = HG.client_controller.network_engine.bandwidth_manager.GetWaitingEstimateAndContext(
            example_network_contexts)
    ...
    def GetNextCheckStatusString(self):
        ...
        if self._check_now:
            ...
            if HydrusData.TimeHasPassed(self._next_check_time):
                ...
                s = HydrusData.TimestampToPrettyTimeDelta(
    ...
    def GetNextWorkTime(self, subscription_name):
        ...
        if self.HasFileWorkToDo():
            ...
            file_bandwidth_estimate = self.GetBandwidthWaitingEstimate(
                subscription_name)
    ...
    def GetNumURLsAndFailed(self):
        ...
        return (self._file_seed_cache.GetFileSeedCount(CC.STATUS_UNKNOWN),
                len(self._file_seed_cache),
                self._file_seed_cache.GetFileSeedCount(CC.STATUS_ERROR))
    ...
    def HasFileWorkToDo(self):
        ...
        if HG.subscription_report_mode:
            ...
            HydrusData.ShowText('Query "' + self._query +
                                '" HasFileWorkToDo test. Next import is ' +
                                repr(file_seed) + '.')
    ...
    def IsSyncDue(self):
        ...
        if HG.subscription_report_mode:
            ...
            HydrusData.ShowText(
                'Query "' + self._query +
                '" IsSyncDue test. Paused/dead status is {}/{}, check time due is {}, and check_now is {}.'
                .format(self._paused, self.IsDead(),
                        HydrusData.TimeHasPassed(self._next_check_time),
                        self._check_now))
    ...
    def UpdateNextCheckTime(self, checker_options):
        ...
        if self._check_now:
            ...
            if checker_options.IsDead(self._file_seed_cache,
            ...
            self._next_check_time = checker_options.GetNextCheckTime(
                self._file_seed_cache, self._last_check_time,
                last_next_check_time)

    def ToTuple(self):

        return (self._query, self._check_now, self._last_check_time,
                self._next_check_time, self._paused, self._status)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_SUBSCRIPTION_QUERY] = SubscriptionQuery
