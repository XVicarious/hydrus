...
def next_token(src):
    def check_tag_end(src):
        if re.match(r"\s(and|or|implies|xor|nor|nand|xnor|iff)", src):
            return True
...
class Node:
    ...
    def __str__(self):  #pretty string form, for debug purposes
        if self.op == "not":
            return "not ({})".format(
                str(self.children[0]
                    ) if type(self.children[0]) != str else self.children[0])
        else:
            child_strs = [
                "(" + (str(x) if type(x) != str else x) + ")"
...
def convert_to_and_or_not(node):
    ...
    if node.op == "implies":  #convert to !a || b
        return Node("or", [
            ...
            convert_to_and_or_not(node.children[1])
        ])
    elif node.op == "xor":  #convert to (a && !b) || (!a && b)
        return Node("or", [
            Node("and", [
                convert_to_and_or_not(node.children[0]),
                negate(node.children[1])
            ]),
            Node("and", [
                negate(node.children[0]),
                convert_to_and_or_not(node.children[1])
        ...
        return Node("or", [
            ...
            Node("and", [negate(node.children[0]),
                         negate(node.children[1])])
        ])
    else:
    ...
    if hasattr(node, 'op'):
        if node.op == "not" and hasattr(node.children[0], 'op'):
            if node.children[0].op == "not":  #eliminate double negation
                ...
                return Node("or", [
                    move_not_inwards(
                        Node("not", [node.children[0].children[0]])),
                    move_not_inwards(
                        Node("not", [node.children[0].children[1]]))
                ])
                ...
                return Node(node.op, list(map(move_not_inwards,
                                              node.children)))
        else:
...
def distribute_and_over_or(node):
    if hasattr(node, 'op'):
        ...
        if node.op == 'or' and hasattr(
                node.children[0], 'op'
        ) and node.children[
                0].op == 'and':  #apply (A && B) || C -> (A || C) && (B || C)
            a = node.children[0].children[0]
            b = node.children[0].children[1]
            c = node.children[1]
            return Node("and", [
                distribute_and_over_or(Node("or", [a, c])),
                distribute_and_over_or(Node("or", [b, c]))
            ])
        elif node.op == 'or' and hasattr(
                node.children[1], 'op'
        ) and node.children[
                1].op == 'and':  #apply C || (A && B) -> (A || C) && (B || C)
...
def convert_to_list_and_simplify(node):
                    ...
                    res.append(
                        set(
                            map(
                                ...
                                if hasattr(x, "op") else x, chld.children)))
        elif node.op == 'or':
            res.append(
                set(
                    map(
                        lambda x: "-" + x.children[0]
                        if hasattr(x, "op") else x, node.children)))
        elif node.op == 'not':
    ...
    for i in range(len(filtered_res)):
        for j in range(len(filtered_res)):
            if i != j and filtered_res[i] == filtered_res[j]:
                filtered_res[i] = None
...
def parse_logic_expression_query(input_str):
    return convert_to_list_and_simplify(
        flatten_tree(
            distribute_and_over_or(
                move_not_inwards(convert_to_and_or_not(parse(input_str))))))
