    ...
    def EventOK(self):
        ...
        try:
            ...
            for panel in self._panels:
                ...
                service_keys_to_content_updates.update(
                    sub_service_keys_to_content_updates)
    ...
    class _LikePanel(QW.QWidget):
        ...
        def GetContentUpdates(self):
            ...
            for (service_key,
                ...
                original_rating_state = self._service_keys_to_original_ratings_states[
                    service_key]
...
class DialogManageUPnP(ClientGUIDialogs.Dialog):
    def __init__(self, parent):
        ...
        columns = [('description', -1), ('internal ip', 17),
                   ('internal port', 7), ('external port', 7),
                   ('prototcol', 5), ('lease', 12)]

        self._mappings_list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            listctrl_panel,
            'manage_upnp_mappings',
            12,
            36,
            columns,
    ...
    def _ConvertDataToListCtrlTuples(self, mapping):
        ...
        (description, internal_ip, internal_port, external_port, protocol,
         duration) = mapping
        ...
        display_tuple = (description, internal_ip, str(internal_port),
                         str(external_port), protocol, pretty_duration)
        sort_tuple = mapping
    ...
    def _Edit(self):
        ...
        selected_mappings = self._mappings_list_ctrl.GetData(
            only_selected=True)

        for selected_mapping in selected_mappings:

            (description, internal_ip, internal_port, external_port, protocol,
             duration) = selected_mapping
