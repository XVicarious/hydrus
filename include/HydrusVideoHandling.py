...
from . import (HydrusData, HydrusExceptions, HydrusImageHandling, HydrusPaths,
               HydrusText, HydrusThreading)
...
def GetFFMPEGVideoProperties(path, force_count_frames_manually=False):
    ...
    first_second_lines = GetFFMPEGInfoLines(path,
                                            count_frames_manually=True,
                                            only_first_second=True)
    ...
    (file_duration_in_s,
     stream_duration_in_s) = ParseFFMPEGDuration(first_second_lines)
...
def GetMime(path):
    ...
    if 'matroska' in mime_text or 'webm' in mime_text:
        ...
        if has_video:
            ...
            has_webm_video = True in (
                webm_video_format in video_format
                for webm_video_format in webm_video_formats)

        if has_audio:

            webm_audio_formats = ('vorbis', 'opus')

            has_webm_audio = True in (
                webm_audio_format in audio_format
                for webm_audio_format in webm_audio_formats)
...
def ParseFFMPEGFPS(first_second_lines):
    ...
    try:
        ...
        if match is not None:
            ...
            tbr_fps_is_likely_garbage = match is None or tbr.endswith(
                'k') or float(tbr) > 144
        ...
        if match is not None:
            ...
            fps_is_likely_garbage = match is None or fps.endswith(
                'k') or float(fps) > 144
        ...
        num_frames_in_first_second = ParseFFMPEGNumFramesManually(
            first_second_lines)
...
def ParseFFMPEGNumFramesManually(lines):
    ...
    final_line = frame_lines[
        -1]  # there will be many progress rows, counting up as the file renders. we hence want the final one
    ...
    try:
        ...
        raise HydrusExceptions.MimeException(
            'Video was unable to render correctly--could not parse ffmpeg output line: "{}"'
            .format(final_line))
...
def ParseFFMPEGVideoLine(lines):
    ...
    lines_video = [
        l for l in lines if re.search(r'^\s*Stream', l) is not None
        and 'Video: ' in l and not ('Video: png' in l or 'Video: jpg' in l)
    ...
    def read_frame(self):
        ...
        if self.process is None:
            ...
            if len(s) != nbytes:
                ...
                result = numpy.fromstring(s, dtype='uint8').reshape(
                    (h, w, len(s) // (w * h)))
