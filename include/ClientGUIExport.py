...
class EditExportFoldersPanel(ClientGUIScrolledPanels.EditPanel):
    ...
    def _GetExistingNames(self):
        ...
        existing_names = {
            export_folder.GetName()
            for export_folder in self._export_folders.GetData()
        }
    ...
    def EventDeleteFilesAfterExport(self):
        ...
        if self._delete_from_client_after_export.isChecked():
            ...
            QW.QMessageBox.warning(
                self, 'Warning',
                'This will delete the exported files from your client after the export! If you do not know what this means, uncheck it!'
            )
...
class ReviewExportFilesPanel(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, flat_media, do_export_and_then_quit=False):
        ...
        t = ClientGUIListBoxes.ListBoxTagsSelection(
            self._tags_box,
        ...
        QP.SetForegroundColour(self._delete_files_after_export,
                               QG.QColor(127, 0, 0))
    ...
    def _GetPath(self, media):
        ...
        filename = ClientExporting.GenerateExportFilename(
            directory, media, terms)

        i = 1

        while filename in self._existing_filenames:

            filename = ClientExporting.GenerateExportFilename(directory,
                                                              media,
                                                              terms,
                                                              append_number=i)
    ...
    def _SetTxtServices(self):
        ...
        choice_tuples = [(
            ...
            service.GetServiceKey() in self._neighbouring_txt_tag_service_keys)
                         for service in tag_services]

        with ClientGUITopLevelWindows.DialogEdit(self,
                                                 'select tag services') as dlg:
    ...
    def _UpdateTxtButton(self):
        ...
        if len(self._neighbouring_txt_tag_service_keys) == 0:
            ...
            names = [
                HG.client_controller.services_manager.GetName(service_key)
    ...
    def EventDeleteFilesChanged(self):
        ...
        HG.client_controller.new_options.SetBoolean(
            'delete_files_after_export', value)
