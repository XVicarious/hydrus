...
def ConvertParseResultToPrettyString(result):
    ...
    if content_type == HC.CONTENT_TYPE_URLS:
        ...
        if url_type == HC.URL_TYPE_DESIRED:
            ...
            return 'downloadable/pursuable url (priority ' + str(
                priority) + '): ' + parsed_text

        elif url_type == HC.URL_TYPE_SOURCE:

            return 'associable/source url (priority ' + str(
                priority) + '): ' + parsed_text

        elif url_type == HC.URL_TYPE_NEXT:

            return 'next page url (priority ' + str(
                priority) + '): ' + parsed_text
        ...
        try:
            ...
            tag = HydrusTags.CleanTag(
                HydrusTags.CombineTag(additional_info, parsed_text))
        ...
        try:
            ...
            timestamp_string = HydrusData.ConvertTimestampToPrettyTime(
                timestamp)
        ...
        return 'watcher page title (priority ' + str(
            priority) + '): ' + parsed_text
...
def ConvertParsableContentToPrettyString(parsable_content, include_veto=False):
    ...
    content_type_to_additional_infos = HydrusData.BuildKeyToSetDict(
        (((content_type, name), additional_infos)
    ...
    for ((content_type, name), additional_infos) in data:
        ...
        if content_type == HC.CONTENT_TYPE_URLS:
            ...
            pretty_strings.append('temp variables: ' +
    ...
    for child in children:
        ...
        try:
            ...
            if isinstance(child, ParseNodeContentLink):
                ...
                child_content = child.Parse(job_key, parsing_text,
                                            referral_url)
...
def GetNamespacesFromParsableContent(parsable_content):
    ...
    namespaces = content_type_to_additional_infos[
        HC.CONTENT_TYPE_MAPPINGS]  # additional_infos is a set of namespaces
...
def GetURLsFromParseResults(results,
                            desired_url_types,
                            only_get_top_priority=False):
...
class ParseFormula(HydrusSerialisable.SerialisableBase):
    def __init__(self, string_match=None, string_converter=None):
        ...
        if string_converter is None:
            ...
            string_converter = StringConverter(
                example_string='parsed information')
    ...
    def ParsePretty(self, parsing_context, parsing_text):
        ...
        pretty_texts = [
            '*** ' + HydrusData.ToHumanInt(len(pretty_texts)) +
            ' RESULTS BEGIN ***'
        ] + pretty_texts + ['*** RESULTS END ***']
...
class ParseFormulaCompound(ParseFormula):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_formulae = HydrusSerialisable.SerialisableList(
            self._formulae).GetSerialisableTuple()
        ...
        return (serialisable_formulae, self._sub_phrase,
                serialisable_string_match, serialisable_string_converter)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_formulae, self._sub_phrase, serialisable_string_match,
         serialisable_string_converter) = serialisable_info

        self._formulae = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_formulae)
        self._string_match = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_string_match)
        self._string_converter = HydrusSerialisable.CreateFromSerialisableTuple(
    ...
    def _ParseRawTexts(self, parsing_context, parsing_text):
        ...
        for formula in self._formulae:
            ...
            if len(
                    stream
            ) == 0:  # no contents were found for one of the /1 replace components, so no valid strings can be made.
    ...
    def ToPrettyString(self):
        ...
        return 'COMPOUND with ' + HydrusData.ToHumanInt(len(
            self._formulae)) + ' formulae.'
    ...
    def ToTuple(self):
        ...
        return (self._formulae, self._sub_phrase, self._string_match,
                self._string_converter)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_PARSE_FORMULA_COMPOUND] = ParseFormulaCompound
...
class ParseFormulaContextVariable(ParseFormula):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._variable_name, serialisable_string_match,
                serialisable_string_converter)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._variable_name, serialisable_string_match,
         serialisable_string_converter) = serialisable_info
    ...
    def ToPrettyMultilineString(self):
        ...
        s.append('fetch the "' + self._variable_name +
                 '" variable from the parsing context')
    ...
    def ToTuple(self):
        ...
        return (self._variable_name, self._string_match,
                self._string_converter)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_PARSE_FORMULA_CONTEXT_VARIABLE] = ParseFormulaContextVariable
...
class ParseFormulaHTML(ParseFormula):
    ...
    def _GetRawTextFromTag(self, tag):
        ...
        if tag is None:
            ...
            if tag.has_attr(self._attribute_to_fetch):
                ...
                if isinstance(unknown_attr_result, list):
                    ...
                    if len(unknown_attr_result) == 0:
                        ...
                        raise HydrusExceptions.ParseException(
                            'Attribute ' + self._attribute_to_fetch +
                            ' not found!')
                ...
                raise HydrusExceptions.ParseException(
                    'Attribute ' + self._attribute_to_fetch + ' not found!')
    ...
    def _GetSerialisableInfo(self):
        ...
        return (serialisable_tag_rules, self._content_to_fetch,
                self._attribute_to_fetch, serialisable_string_match,
                serialisable_string_converter)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_tag_rules, self._content_to_fetch,
         self._attribute_to_fetch, serialisable_string_match,
         serialisable_string_converter) = serialisable_info

        self._tag_rules = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_tag_rules)
    ...
    def _ParseRawTexts(self, parsing_context, parsing_text):
        ...
        try:
            ...
            raise HydrusExceptions.ParseException(
                'Unable to parse that HTML: {}. HTML Sample: {}'.format(
                    str(e), parsing_text[:1024]))
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            new_serialisable_info = (tag_rules, attribute_to_fetch,
                                     culling_and_adding)
        ...
        if version == 2:
            ...
            (tag_rules, attribute_to_fetch,
             culling_and_adding) = old_serialisable_info
            ...
            if cull_front > 0:
                ...
                transformations.append(
                    (STRING_TRANSFORMATION_CLIP_TEXT_FROM_BEGINNING,
                     cull_front))

            elif cull_front < 0:

                transformations.append(
                    (STRING_TRANSFORMATION_REMOVE_TEXT_FROM_END, cull_front))

            if cull_back > 0:

                transformations.append(
                    (STRING_TRANSFORMATION_CLIP_TEXT_FROM_END, cull_back))

            elif cull_back < 0:

                transformations.append(
                    (STRING_TRANSFORMATION_REMOVE_TEXT_FROM_BEGINNING,
                     cull_back))

            if prepend != '':

                transformations.append(
                    (STRING_TRANSFORMATION_PREPEND_TEXT, prepend))

            if append != '':

                transformations.append(
                    (STRING_TRANSFORMATION_APPEND_TEXT, append))

            string_converter = StringConverter(transformations,
                                               'parsed information')
        ...
        if version == 3:
            ...
            (tag_rules, attribute_to_fetch,
             serialisable_string_converter) = old_serialisable_info
        ...
        if version == 4:
            ...
            (tag_rules, attribute_to_fetch, serialisable_string_match,
             serialisable_string_converter) = old_serialisable_info
            ...
            new_serialisable_info = (tag_rules, content_to_fetch,
                                     attribute_to_fetch,
                                     serialisable_string_match,
                                     serialisable_string_converter)
        ...
        if version == 5:
            ...
            (tag_rules, content_to_fetch, attribute_to_fetch,
             serialisable_string_match,
             serialisable_string_converter) = old_serialisable_info
            ...
            for (name, attrs, index) in tag_rules:
                ...
                tag_rule = ParseRuleHTML(rule_type=HTML_RULE_TYPE_DESCENDING,
                                         tag_name=name,
            ...
            new_serialisable_info = (serialisable_new_tag_rules,
                                     content_to_fetch, attribute_to_fetch,
                                     serialisable_string_match,
                                     serialisable_string_converter)
    ...
    def ToPrettyString(self):
        ...
        return 'HTML with ' + HydrusData.ToHumanInt(len(
            self._tag_rules)) + ' tag rules.'
    ...
    def ToPrettyMultilineString(self):
        ...
        if self._content_to_fetch == HTML_CONTENT_ATTRIBUTE:
            ...
            pretty_strings.append('get the ' + self._attribute_to_fetch +
        ...
        pretty_strings.extend(
            self._string_converter.GetTransformationStrings())

        separator = os.linesep + 'and then '

        pretty_multiline_string = '--HTML--' + os.linesep + separator.join(
            pretty_strings)
    ...
    def ToTuple(self):
        ...
        return (self._tag_rules, self._content_to_fetch,
                self._attribute_to_fetch, self._string_match,
                self._string_converter)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_PARSE_FORMULA_HTML] = ParseFormulaHTML
...
class ParseRuleHTML(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._rule_type, self._tag_name, self._tag_attributes,
                self._tag_index, self._tag_depth, self._should_test_tag_string,
                serialisable_tag_string_string_match)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._rule_type, self._tag_name, self._tag_attributes,
         self._tag_index, self._tag_depth, self._should_test_tag_string,
         serialisable_tag_string_string_match) = serialisable_info

        self._tag_string_string_match = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_tag_string_string_match)
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (rule_type, tag_name, tag_attributes, tag_index,
             tag_depth) = old_serialisable_info
            ...
            new_serialisable_info = (rule_type, tag_name, tag_attributes,
                                     tag_index, tag_depth,
    ...
    def GetNodes(self, nodes):
        ...
        for node in nodes:
            ...
            if self._rule_type == HTML_RULE_TYPE_DESCENDING:
                ...
                still_in_tree = lambda node: isinstance(
                    node, bs4.element.Tag
                )  # if we go one above html, we get the BS document itself
        ...
        if self._rule_type == HTML_RULE_TYPE_DESCENDING:
            ...
            if len(self._tag_attributes) > 0:
                ...
                s += ' with attributes ' + ', '.join(
                    key + '=' + value
                    for (key, value) in list(self._tag_attributes.items()))
            ...
            if self._tag_name is None:
                ...
                s += ' ' + HydrusData.ToHumanInt(
                    self._tag_depth) + ' tag levels'

            else:

                s += ' to the ' + HydrusData.ConvertIntToPrettyOrdinalString(
                    self._tag_depth) + ' <' + self._tag_name + '> tag'
    ...
    def ToTuple(self):
        ...
        return (self._rule_type, self._tag_name, self._tag_attributes,
                self._tag_index, self._tag_depth, self._should_test_tag_string,
                self._tag_string_string_match)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_PARSE_RULE_HTML] = ParseRuleHTML
...
class ParseFormulaJSON(ParseFormula):
    ...
    def __init__(self,
        ...
        if parse_rules is None:
            ...
            parse_rules = [(JSON_PARSE_RULE_TYPE_DICT_KEY,
                            StringMatch(match_type=STRING_MATCH_FIXED,
                                        match_value='posts',
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_parse_rules = [
            (parse_rule_type, parse_rule.GetSerialisableTuple())
            if parse_rule_type == JSON_PARSE_RULE_TYPE_DICT_KEY else
            (parse_rule_type, parse_rule)
            for (parse_rule_type, parse_rule) in self._parse_rules
        ]
        serialisable_string_match = self._string_match.GetSerialisableTuple()
        serialisable_string_converter = self._string_converter.GetSerialisableTuple(
        ...
        return (serialisable_parse_rules, self._content_to_fetch,
                serialisable_string_match, serialisable_string_converter)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_parse_rules, self._content_to_fetch,
         serialisable_string_match,
         serialisable_string_converter) = serialisable_info
        ...
        self._parse_rules = [
            (parse_rule_type,
             HydrusSerialisable.CreateFromSerialisableTuple(
                 serialisable_parse_rule))
            if parse_rule_type == JSON_PARSE_RULE_TYPE_DICT_KEY else
            (parse_rule_type, serialisable_parse_rule)
            for (parse_rule_type,
                 serialisable_parse_rule) in serialisable_parse_rules
        ]
        self._string_match = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_string_match)
        self._string_converter = HydrusSerialisable.CreateFromSerialisableTuple(
    ...
    def _ParseRawTexts(self, parsing_context, parsing_text):
        ...
        try:
            ...
            message = 'Unable to parse that JSON: {}. JSON sample: {}'.format(
                str(e), parsing_text[:1024])
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (parse_rules, content_to_fetch, serialisable_string_match,
             serialisable_string_converter) = old_serialisable_info
            ...
            for parse_rule in parse_rules:
                ...
                if parse_rule is None:
                    ...
                    new_parse_rules.append(
                        (JSON_PARSE_RULE_TYPE_ALL_ITEMS, None))

                elif isinstance(parse_rule, int):

                    new_parse_rules.append(
                        (JSON_PARSE_RULE_TYPE_INDEXED_ITEM, parse_rule))

                else:

                    sm = StringMatch(match_type=STRING_MATCH_FIXED,
                                     match_value=parse_rule,
                                     example_string=parse_rule)
            ...
            serialisable_parse_rules = [
                ...
                if parse_rule_type == JSON_PARSE_RULE_TYPE_DICT_KEY else
                (parse_rule_type, parse_rule)
                for (parse_rule_type, parse_rule) in new_parse_rules
            ]

            new_serialisable_info = (serialisable_parse_rules,
                                     content_to_fetch,
                                     serialisable_string_match,
                                     serialisable_string_converter)
    ...
    def ToPrettyString(self):
        ...
        return 'JSON with ' + HydrusData.ToHumanInt(len(
            self._parse_rules)) + ' parse rules.'

    def ToPrettyMultilineString(self):

        pretty_strings = [
            RenderJSONParseRule(p_r) for p_r in self._parse_rules
        ]

        if self._content_to_fetch == JSON_CONTENT_STRING:

            pretty_strings.append(
                'get final data content, converting to strings as needed')
        ...
        pretty_multiline_string = '--JSON--' + os.linesep + separator.join(
            pretty_strings)
    ...
    def ToTuple(self):
        ...
        return (self._parse_rules, self._content_to_fetch, self._string_match,
                self._string_converter)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_PARSE_FORMULA_JSON] = ParseFormulaJSON
...
class SimpleDownloaderParsingFormula(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        self._formula = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_formula)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_SIMPLE_DOWNLOADER_PARSE_FORMULA] = SimpleDownloaderParsingFormula
...
class ContentParser(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        if self._content_type == HC.CONTENT_TYPE_VETO:
            ...
            serialisable_additional_info = (
                veto_if_matches_found, string_match.GetSerialisableTuple())

        else:

            serialisable_additional_info = self._additional_info

        return (self._name, self._content_type, serialisable_formula,
                self._sort_type, self._sort_asc, serialisable_additional_info)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._name, self._content_type, serialisable_formula, self._sort_type,
         self._sort_asc, serialisable_additional_info) = serialisable_info

        if self._content_type == HC.CONTENT_TYPE_VETO:

            (veto_if_matches_found,
             serialisable_string_match) = serialisable_additional_info

            string_match = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_string_match)
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (name, content_type, serialisable_formula,
             additional_info) = old_serialisable_info

            if content_type == HC.CONTENT_TYPE_VETO:

                (veto_if_matches_found, match_if_text_present,
                 search_text) = additional_info

                if match_if_text_present:

                    string_match = StringMatch(match_type=STRING_MATCH_REGEX,
                                               match_value=search_text,
                                               example_string=search_text)
                ...
                additional_info = (veto_if_matches_found,
                                   serialisable_string_match)

            new_serialisable_info = (name, content_type, serialisable_formula,
                                     additional_info)
        ...
        if version == 3:
            ...
            new_serialisable_info = (name, content_type, serialisable_formula,
                                     sort_type, sort_asc, additional_info)
    ...
    def Parse(self, parsing_context, parsing_text):
        ...
        try:
            ...
            parsed_texts = list(
                self._formula.Parse(parsing_context, parsing_text))
        ...
        parsed_texts = [
            parsed_text for parsed_text in parsed_texts
            if isinstance(parsed_text, we_want)
        ]
        ...
        if self._content_type == HC.CONTENT_TYPE_URLS:
            ...
            if 'url' in parsing_context:
                ...
                parsed_texts = [
                    urllib.parse.urljoin(base_url, parsed_text)
        ...
        if self._content_type == HC.CONTENT_TYPE_VETO:
            ...
            match_found = True in (string_match.Matches(parsed_text)
                                   for parsed_text in parsed_texts)

            veto_if_missing = not veto_if_matches_found

            do_veto = (veto_if_matches_found
                       and match_found) or (veto_if_missing
                                            and not match_found)
            ...
            content_description = (self._name, self._content_type,
                                   self._additional_info)

            return [(content_description, parsed_text)
                    for parsed_text in parsed_texts]
    ...
    def ParsePretty(self, parsing_context, parsing_text):
        ...
        try:
            ...
            results = [
                ConvertParseResultToPrettyString(parse_result)
        ...
        result_lines = [
            '*** ' + HydrusData.ToHumanInt(len(results)) + ' RESULTS BEGIN ***'
        ]
    ...
    def ToPrettyStrings(self):
        ...
        return (self._name, 'content',
                ConvertParsableContentToPrettyString(self.GetParsableContent(),
                                                     include_veto=True))

    def ToTuple(self):

        return (self._name, self._content_type, self._formula, self._sort_type,
                self._sort_asc, self._additional_info)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_CONTENT_PARSER] = ContentParser
...
class PageParser(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def __init__(self,
                 ...
                 parser_key=None,
                 string_converter=None,
                 sub_page_parsers=None,
                 content_parsers=None,
                 example_urls=None,
                 example_parsing_context=None):
        ...
        if example_parsing_context is None:
            ...
            example_parsing_context[
                'url'] = 'http://example.com/posts/index.php?id=123456'
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_sub_page_parsers = [
            (formula.GetSerialisableTuple(),
             page_parser.GetSerialisableTuple())
            for (formula, page_parser) in self._sub_page_parsers
        ]

        serialisable_content_parsers = HydrusSerialisable.SerialisableList(
            self._content_parsers).GetSerialisableTuple()

        return (self._name, serialisable_parser_key,
                serialisable_string_converter, serialisable_sub_page_parsers,
                serialisable_content_parsers, self._example_urls,
                self._example_parsing_context)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._name, serialisable_parser_key, serialisable_string_converter,
         serialisable_sub_page_parsers, serialisable_content_parsers,
         self._example_urls, self._example_parsing_context) = serialisable_info

        self._parser_key = bytes.fromhex(serialisable_parser_key)
        self._string_converter = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_string_converter)
        self._sub_page_parsers = [
            (HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_formula),
             HydrusSerialisable.CreateFromSerialisableTuple(
                 serialisable_page_parser))
            for (serialisable_formula,
                 serialisable_page_parser) in serialisable_sub_page_parsers
        ]
        self._content_parsers = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_content_parsers)

    def _UpdateSerialisableInfo(self, version, old_serialisable_info):

        if version == 1:

            (name, serialisable_parser_key, serialisable_string_converter,
             serialisable_sub_page_parsers, serialisable_content_parsers,
             example_urls) = old_serialisable_info
            ...
            new_serialisable_info = (name, serialisable_parser_key,
                                     serialisable_string_converter,
                                     serialisable_sub_page_parsers,
                                     serialisable_content_parsers,
                                     example_urls, example_parsing_context)
    ...
    def GetSafeSummary(self):
        ...
        domains = list({
            ClientNetworkingDomain.ConvertURLIntoDomain(url)
            for url in self._example_urls
        })
    ...
    def Parse(self, parsing_context, parsing_text):
        ...
        try:
            ...
            converted_parsing_text = self._string_converter.Convert(
                parsing_text)
        ...
        try:
            ...
            for content_parser in self._content_parsers:
                ...
                whole_page_parse_results.extend(
                    content_parser.Parse(parsing_context,
                                         converted_parsing_text))
        ...
        if len(self._sub_page_parsers) == 0:
            ...
            try:
                ...
                for (formula, page_parser) in self._sub_page_parsers:
                    ...
                    posts = formula.Parse(parsing_context,
                                          converted_parsing_text)
                    ...
                    for post in posts:
                        ...
                        try:
                            ...
                            page_parser_all_parse_results = page_parser.Parse(
                                parsing_context, post)
                        ...
                        for page_parser_parse_results in page_parser_all_parse_results:
                            ...
                            page_parser_parse_results.extend(
                                whole_page_parse_results)
    ...
    def ParsePretty(self, parsing_context, parsing_text):
        ...
        try:
            ...
            pretty_parse_result_text = group_separator.join(
                pretty_groups_of_parse_results)
        ...
        result_lines.append('*** ' +
                            HydrusData.ToHumanInt(len(all_parse_results)) +
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_PAGE_PARSER] = PageParser
...
class ParseNodeContentLink(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_children = [
            child.GetSerialisableTuple() for child in self._children
        ]
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (self._name, serialisable_formula,
         serialisable_children) = serialisable_info

        self._formula = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_formula)
        self._children = [
            HydrusSerialisable.CreateFromSerialisableTuple(serialisable_child)
            for serialisable_child in serialisable_children
        ]
    ...
    def Parse(self, job_key, parsing_text, referral_url):
        ...
        for search_url in search_urls:
            ...
            job_key.SetVariable(
                'script_status',
                'fetching ' + HydrusText.ElideText(search_url, 32))

            network_job = ClientNetworkingJobs.NetworkJob(
                'GET', search_url, referral_url=referral_url)
            ...
            try:
                ...
                if isinstance(e, HydrusExceptions.NotFoundException):
                    ...
                    HydrusData.Print('Problem fetching ' +
                                     HydrusText.ElideText(search_url, 32) +
                                     ':')
    ...
    def ParseURLs(self, job_key, parsing_text, referral_url):
        ...
        absolute_urls = [
            urllib.parse.urljoin(referral_url, basic_url)
    ...
    def ToPrettyStrings(self):
        ...
        return (self._name, 'link',
                ConvertParsableContentToPrettyString(
                    self.GetParsableContent()))
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_PARSE_NODE_CONTENT_LINK] = ParseNodeContentLink
...
file_identifier_string_lookup[
    FILE_IDENTIFIER_TYPE_FILE] = 'the actual file (POST only)'
...
file_identifier_string_lookup[
    FILE_IDENTIFIER_TYPE_USER_INPUT] = 'custom user input'
...
class ParseRootFileLookup(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def __init__(self,
                 ...
                 url=None,
                 query_type=None,
                 file_identifier_type=None,
                 file_identifier_string_converter=None,
                 file_identifier_arg_name=None,
                 static_args=None,
                 children=None):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._url, self._query_type, self._file_identifier_type,
                serialisable_file_identifier_string_converter,
                self._file_identifier_arg_name, self._static_args,
                serialisable_children)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._url, self._query_type, self._file_identifier_type,
         serialisable_file_identifier_string_converter,
         self._file_identifier_arg_name, self._static_args,
         serialisable_children) = serialisable_info
        ...
        self._file_identifier_string_converter = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_file_identifier_string_converter)
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (url, query_type, file_identifier_type, file_identifier_encoding,
             file_identifier_arg_name, static_args,
             serialisable_children) = old_serialisable_info
            ...
            if file_identifier_encoding == HC.ENCODING_RAW:
                ...
                transformations.append(
                    (STRING_TRANSFORMATION_ENCODE, 'base64'))
            ...
            new_serialisable_info = (
                url, query_type, file_identifier_type,
                serialisable_file_identifier_string_converter,
    ...
    def ConvertMediaToFileIdentifier(self, media):
        ...
        if self._file_identifier_type == FILE_IDENTIFIER_TYPE_USER_INPUT:
            ...
            raise Exception(
                'Cannot convert media to file identifier--this script takes user input!'
            )
        ...
        elif self._file_identifier_type in (FILE_IDENTIFIER_TYPE_MD5,
                                            FILE_IDENTIFIER_TYPE_SHA1,
                                            FILE_IDENTIFIER_TYPE_SHA512):
            ...
            try:
                ...
                raise Exception('I do not know that file\'s ' + hash_type +
                                ' hash, so I cannot look it up!')
            ...
            try:
                ...
                raise Exception(
                    'That file is not in the database\'s local files, so I cannot look it up!'
                )
    ...
    def FetchParsingText(self, job_key, file_identifier):
        ...
        if self._file_identifier_type != FILE_IDENTIFIER_TYPE_FILE:
            ...
            request_args[
                self.
                _file_identifier_arg_name] = self._file_identifier_string_converter.Convert(
                    file_identifier)
        ...
        if self._query_type == HC.GET:
            ...
            full_request_url = self._url + '?' + ClientNetworkingDomain.ConvertQueryDictToText(
                request_args)

            job_key.SetVariable(
                'script_status',
                'fetching ' + HydrusText.ElideText(full_request_url, 32))

            job_key.AddURL(full_request_url)

            network_job = ClientNetworkingJobs.NetworkJob(
                'GET', full_request_url)
            ...
            if self._file_identifier_type == FILE_IDENTIFIER_TYPE_FILE:
                ...
                if self._file_identifier_string_converter.MakesChanges():
                    ...
                    f_altered = self._file_identifier_string_converter.Convert(
                        file_bytes)
            ...
            network_job = ClientNetworkingJobs.NetworkJob('POST',
                                                          self._url,
                                                          body=request_args)
    ...
    def Parse(self, job_key, parsing_text):
        ...
        parse_results = GetChildrenContent(job_key, self._children,
                                           parsing_text, self._url)
        ...
        if len(parse_results) == 0:
            ...
            job_key.SetVariable(
                'script_status', 'Found ' +
                HydrusData.ToHumanInt(len(parse_results)) + ' rows.')
    ...
    def ToPrettyStrings(self):
        ...
        return (self._name, HC.query_type_string_lookup[self._query_type],
                'File Lookup',
                ConvertParsableContentToPrettyString(
                    self.GetParsableContent()))
    ...
    def ToTuple(self):
        ...
        return (self._name, self._url, self._query_type,
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_PARSE_ROOT_FILE_LOOKUP] = ParseRootFileLookup
...
transformation_type_str_lookup[
    STRING_TRANSFORMATION_REMOVE_TEXT_FROM_BEGINNING] = 'remove text from beginning of string'
transformation_type_str_lookup[
    STRING_TRANSFORMATION_REMOVE_TEXT_FROM_END] = 'remove text from end of string'
transformation_type_str_lookup[
    STRING_TRANSFORMATION_PREPEND_TEXT] = 'prepend text'
transformation_type_str_lookup[
    STRING_TRANSFORMATION_APPEND_TEXT] = 'append text'
...
transformation_type_str_lookup[
    STRING_TRANSFORMATION_CLIP_TEXT_FROM_BEGINNING] = 'take the start of the string'
transformation_type_str_lookup[
    STRING_TRANSFORMATION_CLIP_TEXT_FROM_END] = 'take the end of the string'
transformation_type_str_lookup[STRING_TRANSFORMATION_REVERSE] = 'reverse text'
transformation_type_str_lookup[
    STRING_TRANSFORMATION_REGEX_SUB] = 'regex substitution'
transformation_type_str_lookup[
    STRING_TRANSFORMATION_DATE_DECODE] = 'datestring to timestamp'
transformation_type_str_lookup[
    STRING_TRANSFORMATION_INTEGER_ADDITION] = 'integer addition'
transformation_type_str_lookup[
    STRING_TRANSFORMATION_DATE_ENCODE] = 'timestamp to datestring'
...
class StringConverter(HydrusSerialisable.SerialisableBase):
            ...
            try:
                ...
                if transformation_type == STRING_TRANSFORMATION_REMOVE_TEXT_FROM_BEGINNING:
                    ...
                    if timezone == HC.TIMEZONE_GMT:
                        ...
                        timestamp = int(
                            calendar.timegm(struct_time)) - timezone_offset
                ...
                raise HydrusExceptions.StringConvertException(
                    'ERROR: Could not apply "' +
                    self.TransformationToString(transformation) +
                    '" to string "' + repr(s) + '":' + str(e))
    ...
    def TransformationToString(transformation):
        ...
        if transformation_type == STRING_TRANSFORMATION_REMOVE_TEXT_FROM_BEGINNING:
            ...
            return 'remove the first ' + HydrusData.ToHumanInt(
                data) + ' characters'

        elif transformation_type == STRING_TRANSFORMATION_REMOVE_TEXT_FROM_END:

            return 'remove the last ' + HydrusData.ToHumanInt(
                data) + ' characters'

        elif transformation_type == STRING_TRANSFORMATION_CLIP_TEXT_FROM_BEGINNING:

            return 'take the first ' + HydrusData.ToHumanInt(
                data) + ' characters'

        elif transformation_type == STRING_TRANSFORMATION_CLIP_TEXT_FROM_END:

            return 'take the last ' + HydrusData.ToHumanInt(
                data) + ' characters'
            ...
            return transformation_type_str_lookup[
                STRING_TRANSFORMATION_REVERSE]
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_STRING_CONVERTER] = StringConverter
...
class StringMatch(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._match_type, self._match_value, self._min_chars,
                self._max_chars, self._example_string)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._match_type, self._match_value, self._min_chars, self._max_chars,
         self._example_string) = serialisable_info
    ...
    def Test(self, text):
        ...
        if self._min_chars is not None and text_len < self._min_chars:
            ...
            raise HydrusExceptions.StringMatchException(
                ...
                HydrusData.ToHumanInt(self._min_chars) + ' characters')

        if self._max_chars is not None and text_len > self._max_chars:

            raise HydrusExceptions.StringMatchException(
                presentation_text + ' had more than ' +
                HydrusData.ToHumanInt(self._max_chars) + ' characters')
    ...
    def ToString(self):
        ...
        if self._min_chars is None:
            ...
            if self._max_chars is None:
                ...
                result += 'between ' + str(self._min_chars) + ' and ' + str(
                    self._max_chars) + ' '
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_STRING_MATCH] = StringMatch
