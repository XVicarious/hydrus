...
class NetworkBandwidthManager(HydrusSerialisable.SerialisableBase):
    ...
    def _CanStartRequest(self, network_contexts):
        ...
        for network_context in network_contexts:
            ...
            bandwidth_tracker = self._network_contexts_to_bandwidth_trackers[
                network_context]
    ...
    def _GetSerialisableInfo(self):
        ...
        all_serialisable_trackers = [
            (network_context.GetSerialisableTuple(),
             tracker.GetSerialisableTuple())
            for (network_context, tracker) in list(
                self._network_contexts_to_bandwidth_trackers.items())
            if not network_context.IsEphemeral()
        ]
        all_serialisable_rules = [
            (network_context.GetSerialisableTuple(),
             rules.GetSerialisableTuple())
            for (network_context, rules
                 ) in list(self._network_contexts_to_bandwidth_rules.items())
        ]
    ...
    def AlreadyHaveExactlyTheseBandwidthRules(self, network_context,
                                              bandwidth_rules):
        ...
        with self._lock:
            ...
            if network_context in self._network_contexts_to_bandwidth_rules:
                ...
                if self._network_contexts_to_bandwidth_rules[
                        network_context].GetSerialisableTuple(
                        ) == bandwidth_rules.GetSerialisableTuple():
    ...
    def CanDoWork(self,
        ...
        with self._lock:
            ...
            for network_context in network_contexts:
                ...
                if not bandwidth_rules.CanDoWork(
                        bandwidth_tracker,
        ...
        with self._lock:
            ...
            if network_context.context_data is None:
                ...
                if network_context in self._network_contexts_to_bandwidth_rules:
                    ...
                    del self._network_contexts_to_bandwidth_rules[
                        network_context]
    ...
    def GetDefaultRules(self):
        ...
        with self._lock:
            ...
            for (network_context, bandwidth_rules) in list(
                    self._network_contexts_to_bandwidth_rules.items()):
    ...
    def GetBandwidthStringsAndGaugeTuples(self, network_context):
        ...
        with self._lock:
            ...
            return bandwidth_rules.GetBandwidthStringsAndGaugeTuples(
                bandwidth_tracker)
    ...
    def GetNetworkContextsForUser(self, history_time_delta_threshold=None):
        ...
        with self._lock:
            ...
            for (network_context, bandwidth_tracker) in list(
                    self._network_contexts_to_bandwidth_trackers.items()):
                ...
                if network_context != ClientNetworkingContexts.GLOBAL_NETWORK_CONTEXT and history_time_delta_threshold is not None:
                    ...
                    if bandwidth_tracker.GetUsage(
                            HC.BANDWIDTH_TYPE_REQUESTS,
                            history_time_delta_threshold) == 0:
    ...
    def GetWaitingEstimateAndContext(self, network_contexts):
        ...
        with self._lock:
            ...
            for network_context in network_contexts:
                ...
                estimates.append(
                    (bandwidth_rules.GetWaitingEstimate(bandwidth_tracker),
                     network_context))
    ...
    def ReportDataUsed(self, network_contexts, num_bytes):
        ...
        with self._lock:
            ...
            for network_context in network_contexts:
                ...
                self._network_contexts_to_bandwidth_trackers[
                    network_context].ReportDataUsed(num_bytes)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_NETWORK_BANDWIDTH_MANAGER] = NetworkBandwidthManager
