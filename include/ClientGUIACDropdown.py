...
def AppendLoadingPredicate(predicates):
    ...
    predicates.append(
        ClientSearch.Predicate(predicate_type=HC.PREDICATE_TYPE_LABEL,
...
def CacheCanBeUsedForInput(search_text_for_cache, new_search_text):
    ...
    if (namespace_cache and namespace_search) or (not namespace_cache
                                                  and not namespace_search):
...
def InsertStaticPredicatesForRead(predicates, parsed_search_text,
                                  include_unusual_predicate_types,
                                  under_construction_or_predicate):

    (raw_entry, inclusive, wildcard_text, search_text, explicit_wildcard,
     cache_text, entry_predicate) = parsed_search_text
...
def InsertStaticPredicatesForWrite(predicates, parsed_search_text,
                                   tag_service_key, expand_parents):

    (raw_entry, search_text, cache_text, entry_predicate,
     sibling_predicate) = parsed_search_text
    ...
    if search_text in ('', ':', '*') or subtag == '':
        ...
        if expand_parents:
            ...
            predicates = HG.client_controller.tag_parents_manager.ExpandPredicates(
                tag_service_key, predicates)

    return predicates


def ReadFetch(win, job_key, results_callable, parsed_search_text,
              qt_media_callable, file_search_context, synchronised,
              include_unusual_predicate_types, initial_matches_fetched,
              search_text_for_current_cache, cached_results,
              under_construction_or_predicate):
    ...
    if search_text in ('', ':', '*'):
        ...
        if raw_entry == '':
            ...
            if definitely_do_it or db_not_going_to_hang_if_we_hit_it:
                ...
                cached_results = HG.client_controller.Read(
                    'file_system_predicates', search_service_key)

                matches = cached_results

            elif (
                    search_text_for_current_cache is None
                    or search_text_for_current_cache == ''
            ) and cached_results is not None:  # if repeating query but busy, use same
        ...
        if half_complete_subtag == '':
            ...
            if synchronised and qt_media_callable is not None:
                ...
                try:
                    ...
                    media = HG.client_controller.CallBlockingToQt(
                        win, qt_media_callable)
            ...
            if fetch_from_db:
                ...
                if small_exact_match_search:
                    ...
                    cache_valid = CacheCanBeUsedForInput(
                ...
                matches = ClientSearch.FilterPredicatesBySearchText(
                    tag_service_key, search_text, predicates)
                ...
                for group_of_tags_managers in HydrusData.SplitListIntoChunks(
                        tags_managers, 1000):
                    ...
                    if include_current:
                        ...
                        current_tags_to_count.update(
                            itertools.chain.from_iterable(
                                tags_manager.GetCurrent(
                                    tag_service_key, ClientTags.
                                    TAG_DISPLAY_SIBLINGS_AND_PARENTS)
                                for tags_manager in group_of_tags_managers))

                    if include_pending:

                        pending_tags_to_count.update(
                            itertools.chain.from_iterable([
                                tags_manager.GetPending(
                                    tag_service_key, ClientTags.
                                    TAG_DISPLAY_SIBLINGS_AND_PARENTS)
                                for tags_manager in group_of_tags_managers
                            ]))
                ...
                tags_to_do = ClientSearch.FilterTagsBySearchText(
                    tag_service_key, search_text, tags_to_do)
    ...
    matches = InsertStaticPredicatesForRead(matches, parsed_search_text,
                                            include_unusual_predicate_types,
                                            under_construction_or_predicate)
    ...
    HG.client_controller.CallLaterQtSafe(win, 0.0, results_callable, job_key,
                                         search_text,
                                         search_text_for_current_cache,
                                         cached_results, matches,
                                         next_search_is_probably_fast)
...
def ShouldDoExactSearch(cache_text):
    ...
    autocomplete_exact_match_threshold = HG.client_controller.new_options.GetNoneableInteger(
...
def WriteFetch(win, job_key, results_callable, parsed_search_text,
               file_service_key, tag_service_key, expand_parents,
               search_text_for_current_cache, cached_results):
    ...
    matches = InsertStaticPredicatesForWrite(matches, parsed_search_text,
                                             tag_service_key, expand_parents)
...
class WindowActivationACDropdownEventFilter(QC.QObject):
    ...
    def eventFilter(self, watched, event):
        ...
        if event.type() in (QC.QEvent.WindowActivate,
                            QC.QEvent.WindowDeactivate):
...
class AutoCompleteDropdown(QW.QWidget):
    ...
    def __init__(self, parent):
        ...
        if self.window() == HG.client_controller.gui:
            ...
            use_float_mode = HG.client_controller.new_options.GetBoolean(
        ...
        self._text_ctrl_widget_event_filter = QP.WidgetEventFilter(
            self._text_ctrl)

        if self._float_mode:

            self._text_ctrl_widget_event_filter.EVT_SET_FOCUS(
                self.EventSetFocus)
            self._text_ctrl_widget_event_filter.EVT_KILL_FOCUS(
                self.EventKillFocus)
        ...
        self._text_ctrl_widget_event_filter.EVT_MOUSEWHEEL(
            self.EventMouseWheel)

        vbox = QP.VBoxLayout(margin=0)

        self._text_input_hbox = QP.HBoxLayout()

        QP.AddToLayout(self._text_input_hbox, self._text_ctrl,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)

        QP.AddToLayout(vbox, self._text_input_hbox,
        ...
        if self._float_mode:
            ...
            self._dropdown_window.setWindowFlags(QC.Qt.Tool
                                                 | QC.Qt.FramelessWindowHint)

            self._dropdown_window.setAttribute(QC.Qt.WA_ShowWithoutActivating)

            self._dropdown_window.setFrameStyle(QW.QFrame.Panel
                                                | QW.QFrame.Raised)
            self._dropdown_window.setLineWidth(2)

            self._dropdown_window.move(
                ClientGUIFunctions.ClientToScreen(self._text_ctrl, (0, 0)))

            self._dropdown_window_widget_event_filter = QP.WidgetEventFilter(
                self._dropdown_window)
            self._dropdown_window_widget_event_filter.EVT_CLOSE(
                self.EventCloseDropdown)

            self._dropdown_hidden = True

            self._window_activation_ac_dropdown_event_filter = WindowActivationACDropdownEventFilter(
                self)

            self._dropdown_window.installEventFilter(
                self._window_activation_ac_dropdown_event_filter)
        ...
        self._dropdown_notebook.setCurrentIndex(
            self._dropdown_notebook.addTab(self._search_results_list,
                                           'results'))
        ...
        if not self._float_mode:
            ...
            QP.AddToLayout(vbox, self._dropdown_window,
        ...
        if self._float_mode:
            ...
            HG.client_controller.sub(self, '_ParentMovedOrResized',
                                     'top_level_window_move_event')
        ...
        HG.client_controller.sub(self, 'DoDropdownHideShow',
                                 'notify_page_change')
            ...
            if time_to_delay_these_calls:
                ...
                if self._ShouldShow():
                    ...
                    if self._move_hide_job is None:
                        ...
                        self._move_hide_job = HG.client_controller.CallRepeatingQtSafe(
                            self._dropdown_window, 0.0, 0.25,
    ...
    def _ScheduleListRefresh(self, delay):
        ...
        if self._refresh_list_job is not None and delay == 0.0:
            ...
            self._refresh_list_job = HG.client_controller.CallLaterQtSafe(
                self, delay, self._UpdateSearchResultsList)
    ...
    def _ShouldShow(self):
        ...
        i_am_active_and_focused = self.window().isActiveWindow(
        ) and self._text_ctrl.hasFocus()
    ...
    def _ShowDropdown(self):
        ...
        if self._text_ctrl.isVisible():
            ...
            desired_dropdown_position = ClientGUIFunctions.ClientToScreen(
                self._text_ctrl, (0, text_height))
    ...
    def keyPressFilter(self, event):
        ...
        if key in (QC.Qt.Key_Insert, ):
        ...
        elif key == QC.Qt.Key_Space and event.modifiers(
        ) & raw_control_modifier:
            ...
            if key in (ord('A'),
                       ord('a')) and modifier == QC.Qt.ControlModifier:
                ...
                if key in (QC.Qt.Key_Up,
                           QC.Qt.Key_Down) and current_list_is_empty:
                    ...
                    if key in (QC.Qt.Key_Up, ):
                        ...
                        self.selectDown.emit()
                             ...
                             QC.Qt.Key_PageUp) and current_list_is_empty:
            ...
            if send_input_to_current_list:
                ...
                current_results_list.keyPressEvent(
                    event
                )  # ultimately, this typically ignores the event, letting the text ctrl take it
    ...
    def EventText(self, new_text):
        ...
        if num_chars == 0:
            ...
            if self._dropdown_notebook.currentWidget(
        ...
        if index is not None:
            ...
            if current_index is not None and current_index != -1:
                ...
                new_index = (current_index +
                             direction) % number_of_pages  # does wraparound
    ...
    def SetFetchedResults(self, job_key, search_text, search_text_for_cache,
                          cached_results, results):

        if self._current_fetch_job_key is not None and self._current_fetch_job_key.GetKey(
        ) == job_key.GetKey():
...
class AutoCompleteDropdownTags(AutoCompleteDropdown):
    def __init__(self, parent, file_service_key, tag_service_key):
        ...
        HG.client_controller.sub(self, 'RefreshFavouriteTags',
                                 'notify_new_favourite_tags')
    ...
    def _ChangeFileService(self, file_service_key):
        ...
        if file_service_key == CC.COMBINED_FILE_SERVICE_KEY and self._tag_service_key == CC.COMBINED_TAG_SERVICE_KEY:
            ...
            local_tag_services = HG.client_controller.services_manager.GetServices(
                (HC.LOCAL_TAG, ))
    ...
    def _ChangeTagService(self, tag_service_key):
        ...
        tag_service = tag_service = HG.client_controller.services_manager.GetService(
            self._tag_service_key)
    ...
    def FileButtonHit(self):
        ...
        if advanced_mode and self._allow_all_known_files:
            ...
            services.append(
                services_manager.GetService(CC.COMBINED_FILE_SERVICE_KEY))
        ...
        for service in services:
            ...
            ClientGUIMenus.AppendMenuItem(
                ...
                'Change the current file domain to ' + service.GetName() + '.',
                self._ChangeFileService, service.GetServiceKey())
    ...
    def RefreshFavouriteTags(self):
        ...
        favourite_tags = list(
            HG.client_controller.new_options.GetStringList('favourite_tags'))
    ...
    def TagButtonHit(self):
        ...
        services.append(
            services_manager.GetService(CC.COMBINED_TAG_SERVICE_KEY))
        ...
        for service in services:
            ...
            ClientGUIMenus.AppendMenuItem(
                menu, service.GetName(),
                'Change the current tag domain to ' + service.GetName() + '.',
                self._ChangeTagService, service.GetServiceKey())
...
class AutoCompleteDropdownTagsRead(AutoCompleteDropdownTags):
    def __init__(self,
        ...
        AutoCompleteDropdownTags.__init__(self, parent, file_service_key,
                                          tag_service_key)
        ...
        self._include_current_tags = ClientGUICommon.OnOffButton(
            self._dropdown_window,
            self._page_key,
            'notify_include_current',
            on_label='include current tags',
            off_label='exclude current tags',
            start_on=file_search_context.IncludeCurrentTags())
        self._include_current_tags.setToolTip(
            'select whether to include current tags in the search')
        self._include_pending_tags = ClientGUICommon.OnOffButton(
            self._dropdown_window,
            self._page_key,
            'notify_include_pending',
            on_label='include pending tags',
            off_label='exclude pending tags',
            start_on=file_search_context.IncludePendingTags())
        self._include_pending_tags.setToolTip(
            'select whether to include pending tags in the search')

        self._synchronised = ClientGUICommon.OnOffButton(
            self._dropdown_window,
            self._page_key,
            'notify_search_immediately',
            on_label='searching immediately',
            off_label='waiting -- tag counts may be inaccurate',
            start_on=synchronised)
        self._synchronised.setToolTip(
            'select whether to renew the search as soon as a new predicate is entered'
        )

        self._or_advanced = ClientGUICommon.BetterButton(
            self._dropdown_window, 'OR', self._AdvancedORInput)
        ...
        self._or_cancel = ClientGUICommon.BetterBitmapButton(
            self._dropdown_window, CC.GlobalPixmaps.delete,
            self._CancelORConstruction)
        self._or_cancel.setToolTip('Cancel OR Predicate construction.')
        self._or_cancel.hide()

        self._or_rewind = ClientGUICommon.BetterBitmapButton(
            self._dropdown_window, CC.GlobalPixmaps.previous,
        ...
        QP.AddToLayout(button_hbox_1, self._include_current_tags,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(button_hbox_1, self._include_pending_tags,
                       CC.FLAGS_EXPAND_BOTH_WAYS)

        sync_button_hbox = QP.HBoxLayout()

        QP.AddToLayout(sync_button_hbox, self._synchronised,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(button_hbox_2, self._file_repo_button,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(button_hbox_2, self._tag_repo_button,
                       CC.FLAGS_EXPAND_BOTH_WAYS)

        vbox = QP.VBoxLayout()

        QP.AddToLayout(vbox, button_hbox_1,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, sync_button_hbox,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, button_hbox_2,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, self._dropdown_notebook,
                       CC.FLAGS_EXPAND_BOTH_WAYS)

        self._dropdown_window.setLayout(vbox)

        HG.client_controller.sub(self, 'SetSynchronisedWait',
                                 'synchronised_wait_switch')

        HG.client_controller.sub(self, 'IncludeCurrent',
                                 'notify_include_current')
        HG.client_controller.sub(self, 'IncludePending',
                                 'notify_include_pending')
    ...
    def _BroadcastChoices(self, predicates, shift_down):
        ...
        if shift_down:
            ...
            if self._under_construction_or_predicate is None:
                ...
                self._under_construction_or_predicate = ClientSearch.Predicate(
                    HC.PREDICATE_TYPE_OR_CONTAINER, predicates)
                ...
                or_preds = list(
                    self._under_construction_or_predicate.GetValue())

                or_preds.extend([
                    predicate for predicate in predicates
                    if predicate not in or_preds
                ])

                self._under_construction_or_predicate = ClientSearch.Predicate(
                    HC.PREDICATE_TYPE_OR_CONTAINER, or_preds)
            ...
            if self._under_construction_or_predicate is not None and not or_pred_in_broadcast:
                ...
                predicates = {
                    ClientSearch.Predicate(HC.PREDICATE_TYPE_OR_CONTAINER,
                                           or_preds)
                }
    ...
    def _BroadcastCurrentText(self, shift_down):
        ...
        (raw_entry, inclusive, wildcard_text, search_text, explicit_wildcard,
         cache_text, entry_predicate) = self._ParseSearchText()
        ...
        if namespace != '' and subtag in ('', '*'):
            ...
            entry_predicate = ClientSearch.Predicate(
                HC.PREDICATE_TYPE_NAMESPACE, namespace, inclusive)
    ...
    def _ChangeFileService(self, file_service_key):
        ...
        HG.client_controller.pub('change_file_service', self._page_key,
                                 file_service_key)
    ...
    def _ChangeTagService(self, tag_service_key):
        ...
        HG.client_controller.pub('change_tag_service', self._page_key,
                                 tag_service_key)
    ...
    def _InitFavouritesList(self):
        ...
        favs_list = ClientGUIListBoxes.ListBoxTagsACRead(
            self._dropdown_notebook,
            self.BroadcastChoices,
            self._tag_service_key,
            height_num_chars=self._list_height_num_chars)
    ...
    def _InitSearchResultsList(self):
        ...
        return ClientGUIListBoxes.ListBoxTagsACRead(
            self._dropdown_notebook,
            self.BroadcastChoices,
            self._tag_service_key,
            height_num_chars=self._list_height_num_chars)
    ...
    def _ParseSearchText(self):
        ...
        return (raw_entry, inclusive, wildcard_text, search_text,
                explicit_wildcard, cache_text, entry_predicate)
    ...
    def _StartResultsFetchJob(self, job_key):
        ...
        stub_predicates = InsertStaticPredicatesForRead(
            stub_predicates, parsed_search_text,
            self._include_unusual_predicate_types,
            self._under_construction_or_predicate)

        AppendLoadingPredicate(stub_predicates)

        HG.client_controller.CallLaterQtSafe(self, 0.2, self.SetStubPredicates,
                                             job_key, stub_predicates)

        HG.client_controller.CallToThread(
            ReadFetch, self, job_key, self.SetFetchedResults,
            parsed_search_text,
            self._media_callable, self._file_search_context,
            self._synchronised.IsOn(), self._include_unusual_predicate_types,
            self._initial_matches_fetched, self._search_text_for_current_cache,
    ...
    def _ShouldTakeResponsibilityForEnter(self):
        ...
        looking_at_search_results = self._dropdown_notebook.currentWidget(
        ) == self._search_results_list
    ...
    def SetFetchedResults(self, job_key, search_text, search_text_for_cache,
        ...
        if self._current_fetch_job_key is not None and self._current_fetch_job_key.GetKey(
            ...
            AutoCompleteDropdownTags.SetFetchedResults(self, job_key,
                                                       search_text,
                                                       search_text_for_cache,
                                                       cached_results, results)
...
class AutoCompleteDropdownTagsWrite(AutoCompleteDropdownTags):
    def __init__(self,
                 ...
                 file_service_key,
                 tag_service_key,
                 null_entry_callable=None,
                 tag_service_key_changed_callable=None,
        ...
        elif tag_service_key != CC.COMBINED_TAG_SERVICE_KEY and HC.options[
                'show_all_tags_in_autocomplete']:
        ...
        self._paste_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.paste, self._Paste)
        self._paste_button.setToolTip(
            'Paste from the clipboard and quick-enter as if you had typed. This can take multiple newline-separated tags.'
        )
        ...
        QP.AddToLayout(self._text_input_hbox, self._paste_button,
                       CC.FLAGS_VCENTER)
    ...
    def _BroadcastCurrentText(self, shift_down):
        ...
        (raw_entry, search_text, cache_text, entry_predicate,
         sibling_predicate) = self._ParseSearchText()
    ...
    def _InitFavouritesList(self):
        ...
        favs_list = ClientGUIListBoxes.ListBoxTagsACWrite(
            self._dropdown_notebook,
            self.BroadcastChoices,
            self._tag_service_key,
            height_num_chars=self._list_height_num_chars)
    ...
    def _InitSearchResultsList(self):
        ...
        return ClientGUIListBoxes.ListBoxTagsACWrite(
            self._dropdown_notebook,
            self.BroadcastChoices,
            self._tag_service_key,
            height_num_chars=self._list_height_num_chars)
    ...
    def _ParseSearchText(self):
        ...
        return (raw_entry, search_text, cache_text, entry_predicate,
                sibling_predicate)
    ...
    def _StartResultsFetchJob(self, job_key):
        ...
        stub_predicates = InsertStaticPredicatesForWrite(
            stub_predicates, parsed_search_text, self._tag_service_key,
        ...
        HG.client_controller.CallToThread(
            WriteFetch, self, job_key, self.SetFetchedResults,
            parsed_search_text, self._file_service_key, self._tag_service_key,
            self._expand_parents, self._search_text_for_current_cache,
            self._cached_results)

    def _TakeResponsibilityForEnter(self, shift_down):

        if self._text_ctrl.text(
        ) == '' and self._dropdown_notebook.currentWidget(
        ) == self._search_results_list:
    ...
    def RefreshFavouriteTags(self):
        ...
        predicates = parents_manager.ExpandPredicates(
            CC.COMBINED_TAG_SERVICE_KEY, predicates)
    ...
    def _UpdateText(self):
        ...
        if len(text) > 0:
            ...
            try:
                ...
                result = LogicExpressionQueryParser.parse_logic_expression_query(
                    text)
                ...
                for s in result:
                    ...
                    for tag_string in s:
                        ...
                        if '*' in tag_string:
                            ...
                            (namespace,
                             subtag) = HydrusTags.SplitTag(tag_string)

                            if '*' not in namespace and subtag == '*':

                                row_pred = ClientSearch.Predicate(
                                    HC.PREDICATE_TYPE_NAMESPACE, namespace,
                                    inclusive)

                            else:

                                row_pred = ClientSearch.Predicate(
                                    HC.PREDICATE_TYPE_WILDCARD, tag_string,
                                    inclusive)

                        else:

                            row_pred = ClientSearch.Predicate(
                                HC.PREDICATE_TYPE_TAG, tag_string, inclusive)
                    ...
                    if len(row_preds) == 1:
                        ...
                        self._current_predicates.append(
                            ClientSearch.Predicate(
                                HC.PREDICATE_TYPE_OR_CONTAINER, row_preds))

                output = os.linesep.join(
                    (pred.ToString() for pred in self._current_predicates))
                colour = (0, 128, 0)
