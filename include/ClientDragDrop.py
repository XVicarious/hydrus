...
def DoFileExportDragDrop(window, page_key, media, alt_down):
    ...
    do_secret_discord_dnd_fix = new_options.GetBoolean(
        'secret_discord_dnd_fix') and alt_down
    ...
    discord_dnd_fix_possible = new_options.GetBoolean(
        'discord_dnd_fix'
    ) and len(original_paths) <= 50 and total_size < 200 * 1048576
...
class FileDropTarget(QC.QObject):
    ...
    def eventFilter(self, object, event):
        ...
        if event.type() == QC.QEvent.Drop:
            ...
            if self.OnDrop(event.pos().x(), event.pos().y()):
                ...
                event.setDropAction(
                    self.OnData(event.mimeData(), event.proposedAction()))
        ...
        if media_dnd and self._media_callable is not None:
            ...
            if len(urls) > 0:
                ...
                for url in urls:
                    ...
                    QP.CallAfter(self._url_callable,
            ...
            QP.CallAfter(self._url_callable,
                         text)  # callafter to terminate dnd event now
    ...
    def OnDrop(self, x, y):
        ...
        screen_position = ClientGUIFunctions.ClientToScreen(
            self._parent, (x, y))
