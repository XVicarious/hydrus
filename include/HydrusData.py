...
def CalculateScoreFromRating(count, rating):
    ...
    score = ((positive + 1.9208) / count - 1.96 *
             (((positive * negative) / count + 0.9604)**0.5) / count) / (
                 1 + 3.8416 / count)
...
def ConvertNumericalRatingToPrettyString(lower,
                                         upper,
                                         rating,
    ...
    if seconds >= 60:
        ...
        for (time_string, duration) in lines:
            ...
            if time_quantity > 0:
                ...
                if len(
                        result_components
                ) > 0:  # something like '1 year' -- in which case we do not care about the days and hours
...
def TimestampToPrettyTimeDelta(timestamp,
    ...
    try:
        ...
        time_delta_string = TimeDeltaToPrettyTimeDelta(
            time_delta, show_seconds=show_seconds)
...
def ConvertUglyNamespacesToPrettyStrings(namespaces):
    ...
    result = [
        ConvertUglyNamespaceToPrettyString(namespace)
...
def ConvertValueRangeToScanbarTimestampsMS(value_ms, range_ms):
    ...
    if range_hours > 0:
        ...
        args = (value_hours, value_minutes, value_seconds, value_ms,
                range_hours, range_minutes, range_seconds, range_ms)
        ...
        args = (value_minutes, value_seconds, value_ms, range_minutes,
                range_seconds, range_ms)
...
def Get64BitHammingDistance(phash1, phash2):
    ...
    n = (n & 0x5555555555555555) + (
        (n & 0xAAAAAAAAAAAAAAAA) >> 1)  # 10101010, 01010101
    n = (n & 0x3333333333333333) + (
        (n & 0xCCCCCCCCCCCCCCCC) >> 2)  # 11001100, 00110011
    n = (n & 0x0F0F0F0F0F0F0F0F) + (
        (n & 0xF0F0F0F0F0F0F0F0) >> 4)  # 11110000, 00001111
    ...
    if os.path.exists(path):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            ...
            try:
                ...
                (pid,
                 create_time) = HydrusText.DeserialiseNewlinedTexts(file_text)
...
def GenerateHumanTextSortKey():
    ...
    split_alphanum = lambda t: tuple(
        (int_convert(sub_t) for sub_t in re.split('([0-9]+)', t.lower())))
...
def MergeKeyToListDicts(key_to_list_dicts):
    ...
    for key_to_list_dict in key_to_list_dicts:
        ...
        for (key, value) in list(key_to_list_dict.items()):
...
def PrintException(e, do_wait=True):
    ...
    message = str(etype.__name__) + ': ' + str(
        value) + os.linesep + trace + os.linesep + stack
...
def SplitIteratorIntoAutothrottledChunks(iterator, starting_n,
                                         precise_time_to_stop):
    ...
    while len(chunk) > 0:
        ...
        if TimeHasPassedPrecise(precise_time_to_stop):
            ...
            expected_items_in_remaining_time = max(
                1, int(time_remaining * items_per_second))
...
def SplitMappingIteratorIntoAutothrottledChunks(iterator, starting_n,
                                                precise_time_to_stop):
...
def WaitForProcessToFinish(p, timeout):
    ...
    while p.poll() is None:
        ...
        if TimeHasPassed(started + timeout):
            ...
            raise Exception('Process did not finish within ' +
                            ToHumanInt(timeout) + ' seconds!')
...
class AccountIdentifier(HydrusSerialisable.SerialisableBase):
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        if self._type == self.TYPE_ACCOUNT_KEY:
            ...
            self._data = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_data)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_ACCOUNT_IDENTIFIER] = AccountIdentifier
...
class AccountType(HydrusYAMLBase):
    ...
    def ConvertToString(self):
        ...
        if self._permissions == [HC.UNKNOWN_PERMISSION]:
            result_string += 'no permissions'
        else:
            result_string += ', '.join([
                HC.permissions_string_lookup[permission]
                for permission in self._permissions
            ]) + ' permissions'

        return result_string

    def IsUnknownAccountType(self):
        return self._permissions == [HC.UNKNOWN_PERMISSION]
...
class ContentUpdate(object):
    ...
    def __repr__(self):
        ...
        return 'Content Update: ' + str(
            (self._data_type, self._action, self._row, self._reason))
    ...
    def GetHashes(self):
        ...
        if self._data_type == HC.CONTENT_TYPE_FILES:
        ...
        elif self._data_type in (HC.CONTENT_TYPE_TAG_PARENTS,
            ...
            (hash, preview_views_delta, preview_viewtime_delta,
             media_views_delta, media_viewtime_delta) = self._row
    ...
    def IsInboxRelated(self):
        ...
        return self._action in (HC.CONTENT_UPDATE_ARCHIVE,
