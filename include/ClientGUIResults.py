...
class MediaPanel(ClientMedia.ListeningMediaList, QW.QScrollArea):
    def __init__(self, parent, page_key, file_service_key, media_results):
        ...
        ClientMedia.ListeningMediaList.__init__(self, file_service_key,
                                                media_results)
        ...
        HG.client_controller.sub(self, 'SelectByTags',
                                 'select_files_with_tags')
        HG.client_controller.sub(self, 'LaunchMediaViewerOnFocus',
                                 'launch_media_viewer')
        ...
        if len(hashes) > 0:
            ...
            if HC.options['confirm_archive']:
                ...
                if len(hashes) > 1:
                    ...
                    message = 'Archive ' + HydrusData.ToHumanInt(
                        len(hashes)) + ' files?'
    ...
    def _ArchiveDeleteFilter(self):
        ...
        if len(self._selected_media) == 0:
            ...
            media_results = self.GenerateMediaResults(
                discriminant=CC.DISCRIMINANT_LOCAL_BUT_NOT_IN_TRASH,
                selected_media=set(self._sorted_media),
                for_media_viewer=True)

        else:

            media_results = self.GenerateMediaResults(
                discriminant=CC.DISCRIMINANT_LOCAL_BUT_NOT_IN_TRASH,
                selected_media=set(self._selected_media),
                for_media_viewer=True)
        ...
        if len(media_results) > 0:
            ...
            canvas_window = ClientGUICanvas.CanvasMediaListFilterArchiveDelete(
                canvas_frame, self._page_key, media_results)
        ...
        if self._focused_media is not None:
            ...
            if media.GetMime() in HC.IMAGES:
                ...
                QW.QMessageBox.critical(
                    self, 'Error',
                    'Sorry, cannot take bmps of anything but static images right now!'
                )
    ...
    def _CopyHashToClipboard(self, hash_type):
        ...
        if hash_type == 'sha256':
            ...
            if display_media.GetLocationsManager().IsLocal():
                ...
                (other_hash, ) = HG.client_controller.Read(
                    'file_hashes', (sha256_hash, ), 'sha256', hash_type)
                ...
                QW.QMessageBox.critical(
                    self, 'Error',
                    'Unfortunately, you do not have that file in your database, so its non-sha256 hashes are unknown.'
                )
    ...
    def _CopyHashesToClipboard(self, hash_type):
        ...
        if hash_type == 'sha256':
            ...
            hex_hashes = os.linesep.join(
                [hash.hex() for hash in self._GetSelectedHashes(ordered=True)])

        else:

            sha256_hashes = self._GetSelectedHashes(
                discriminant=CC.DISCRIMINANT_LOCAL, ordered=True)

            if len(sha256_hashes) > 0:

                other_hashes = HG.client_controller.Read(
                    'file_hashes', sha256_hashes, 'sha256', hash_type)

                hex_hashes = os.linesep.join(
                    [other_hash.hex() for other_hash in other_hashes])

            else:

                QW.QMessageBox.critical(
                    self, 'Error',
                    'Unfortunately, none of those files are in your database, so their non-sha256 hashes are unknown.'
                )
    ...
    def _CopyPathToClipboard(self):
        ...
        path = client_files_manager.GetFilePath(display_media.GetHash(),
                                                display_media.GetMime())
    ...
    def _CopyPathsToClipboard(self):
        ...
        media_results = self.GenerateMediaResults(
            discriminant=CC.DISCRIMINANT_LOCAL,
            selected_media=set(self._selected_media))
        ...
        for media_result in media_results:
            ...
            paths.append(
                client_files_manager.GetFilePath(media_result.GetHash(),
                                                 media_result.GetMime(),
                                                 check_file_exists=False))
    ...
    def _CopyServiceFilenameToClipboard(self, service_key):
        ...
        (filename, ) = HG.client_controller.Read('service_filenames',
                                                 service_key, {hash})
    ...
    def _CopyServiceFilenamesToClipboard(self, service_key):
        ...
        if len(hashes) > 0:
            ...
            filenames = [
                prefix + filename for filename in HG.client_controller.Read(
                    'service_filenames', service_key, hashes)
            ]
            ...
            if len(filenames) > 0:
                ...
                HydrusData.ShowText(
                    'Could not find any service filenames for that selection!')

        else:

            HydrusData.ShowText(
                'Could not find any files with the requested service!')

    def _Delete(self, file_service_key=None):

        if file_service_key is None or file_service_key in (
                CC.LOCAL_FILE_SERVICE_KEY, CC.TRASH_SERVICE_KEY):
        ...
        try:
            ...
            (involves_physical_delete,
             jobs) = ClientGUIDialogsQuick.GetDeleteFilesJobs(
                 self,
                 self._selected_media,
                 default_reason,
                 suggested_file_service_key=file_service_key)
    ...
    def _DownloadSelected(self):
        ...
        hashes = self._GetSelectedHashes(
            discriminant=CC.DISCRIMINANT_NOT_LOCAL)
    ...
    def _EditDuplicateActionOptions(self, duplicate_type):
        ...
        duplicate_action_options = new_options.GetDuplicateActionOptions(
            duplicate_type)

        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit duplicate merge options') as dlg_2:

            panel = ClientGUIScrolledPanelsEdit.EditDuplicateActionOptionsPanel(
                dlg_2, duplicate_type, duplicate_action_options)
            ...
            if dlg_2.exec() == QW.QDialog.Accepted:
                ...
                new_options.SetDuplicateActionOptions(
                    duplicate_type, duplicate_action_options)
    ...
    def _ExportFiles(self, do_export_and_then_quit=False):
        ...
        if len(self._selected_media) > 0:
            ...
            panel = ClientGUIExport.ReviewExportFilesPanel(
                frame,
                flat_media,
                do_export_and_then_quit=do_export_and_then_quit)
    ...
    def _FullScreen(self, first_media=None):
        ...
        if self._focused_media is not None:
            ...
            (media_show_action, media_start_paused,
             media_start_with_embed) = new_options.GetMediaShowAction(
                 display_media.GetMime())
        ...
        media_results = self.GenerateMediaResults(
            discriminant=CC.DISCRIMINANT_LOCAL, for_media_viewer=True)
        ...
        if len(media_results) > 0:
            ...
            if first_media is not None and first_media.GetLocationsManager(
            ).IsLocal():
                first_hash = first_media.GetDisplayMedia().GetHash()
            else:
                first_hash = None

            canvas_frame = ClientGUICanvas.CanvasFrame(self.window())

            canvas_window = ClientGUICanvas.CanvasMediaListBrowser(
                canvas_frame, self._page_key, media_results, first_hash)
    ...
    def _GetPrettyStatus(self):
        ...
        (num_files_descriptor,
         selected_files_descriptor) = self._GetSortedSelectedMimeDescriptors()
        ...
        if num_files == 1:
            ...
            num_files_string = HydrusData.ToHumanInt(
                num_files) + ' ' + num_files_descriptor + 's'
        ...
        if num_selected == 0:
            ...
            if num_selected == 1 or selected_files_descriptor == num_files_descriptor:
                ...
                selected_files_string = HydrusData.ToHumanInt(
                    num_selected) + ' ' + selected_files_descriptor + 's'

            if num_selected == 1:  # 23 files - 1 video selected, file_info

                (selected_media, ) = self._selected_media

                s += selected_files_string + ' selected, ' + ', '.join(
                    selected_media.GetPrettyInfoLines())

            else:  # 23 files - 5 selected, selection_info

                num_inbox = sum(
                    (media.GetNumInbox() for media in self._selected_media))
                ...
                if num_inbox == num_selected:
                    ...
                    inbox_phrase = HydrusData.ToHumanInt(
                        num_inbox) + ' in inbox and ' + HydrusData.ToHumanInt(
                            num_selected - num_inbox) + ' archived, '
    ...
    def _GetPrettyTotalSize(self, only_selected=False):
        ...
        unknown_size = False in (media.IsSizeDefinite()
        ...
        if total_size == 0:
            ...
            if unknown_size:
                ...
                return HydrusData.ToHumanBytes(
                    total_size) + ' + some unknown size'

            else:

                return HydrusData.ToHumanBytes(total_size)

    def _GetSelectedHashes(self,
                           has_location=None,
                           discriminant=None,
                           not_uploaded_to=None,
                           ordered=False):
        ...
        if ordered:
            ...
            for media in self._sorted_media:
                ...
                if media in self._selected_media:
                    ...
                    result.extend(
                        media.GetHashes(has_location, discriminant,
                                        not_uploaded_to, ordered))
    ...
    def _GetSelectedCollections(self):
        ...
        sorted_selected_collections = [
            media for media in self._sorted_media
            if media.IsCollection() and media in self._selected_media
        ]
    ...
    def _GetSelectedFlatMedia(self,
        ...
        flat_media = [
            media for media in flat_media
            if media.MatchesDiscriminant(has_location=has_location,
                                         discriminant=discriminant,
                                         not_uploaded_to=not_uploaded_to)
        ]
    ...
    def _GetSimilarTo(self, max_hamming):
        ...
        if len(hashes) > 0:
            ...
            initial_predicates = [
                ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_SIMILAR_TO,
                                       (tuple(hashes), max_hamming))
            ]
    ...
    def _HitMedia(self, media, ctrl, shift):
        ...
        if media is None:
            ...
            if not ctrl and not shift:
                ...
                self._Select(
                    ClientMedia.FileFilter(ClientMedia.FILE_FILTER_NONE))
                self._SetFocusedMedia(None)
                self._shift_focused_media = None
            ...
            if ctrl:
                ...
                start_index = self._sorted_media.index(
                    self._shift_focused_media)

                end_index = self._sorted_media.index(media)

                if start_index < end_index:
                    media_to_select = set(
                        self._sorted_media[start_index:end_index + 1])
                else:
                    media_to_select = set(
                        self._sorted_media[end_index:start_index + 1])
        ...
        if len(hashes) > 0:
            ...
            if HC.options['confirm_archive']:
                ...
                if len(hashes) > 1:
                    ...
                    message = 'Send {} files to inbox?'.format(
    ...
    def _ManageNotes(self):
        def qt_do_it(media, notes):
            ...
            with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
                ...
                panel = ClientGUIScrolledPanels.EditSingleCtrlPanel(
                    dlg, ['manage_file_notes'])
        ...
        HG.client_controller.CallToThread(
            thread_wait, self._focused_media.GetDisplayMedia())
        ...
        if len(self._selected_media) > 0:
            ...
            if len(
                    HG.client_controller.services_manager.GetServices(
                        HC.RATINGS_SERVICES)) > 0:

                flat_media = self._GetSelectedFlatMedia()

                with ClientGUIDialogsManage.DialogManageRatings(
                        self, flat_media) as dlg:
    ...
    def _ManageTags(self):
        ...
        if len(self._selected_media) > 0:
            ...
            title = 'manage tags for ' + HydrusData.ToHumanInt(
                num_files) + ' files'
            ...
            with ClientGUITopLevelWindows.DialogManage(self, title,
                ...
                panel = ClientGUITags.ManageTagsPanel(dlg,
    ...
    def _ManageURLs(self):
        ...
        if len(self._selected_media) > 0:
            ...
            with ClientGUITopLevelWindows.DialogManage(self, title) as dlg:
                ...
                panel = ClientGUIScrolledPanelsManagement.ManageURLsPanel(
                    dlg, self._selected_media)
    ...
    def _ModifyUploaders(self, file_service_key):
        ...
        if hashes is not None and len(hashes) > 0:
            ...
            contents = [
                HydrusNetwork.Content(HC.CONTENT_TYPE_FILES, [hash])
                for hash in hashes
            ]
            ...
            with ClientGUIDialogs.DialogModifyAccounts(
                    self, file_service_key, subject_accounts) as dlg:
                dlg.exec()
        ...
        if self._focused_media is not None:
            ...
            if open_externally_media.GetLocationsManager().IsLocal():
                ...
                HG.client_controller.pub(
                    'media_focus_went_to_external_program', self._page_key)
    ...
    def _OpenKnownURL(self):
        ...
        if self._focused_media is not None:
            ...
            ClientGUIMedia.DoOpenKnownURLFromShortcut(self,
                                                      self._focused_media)
    ...
    def _PetitionFiles(self, remote_service_key):
        ...
        if hashes is not None and len(hashes) > 0:
            ...
            remote_service = HG.client_controller.services_manager.GetService(
                remote_service_key)
            ...
            if service_type == HC.FILE_REPOSITORY:
                ...
                if len(hashes) == 1:
                    ...
                    message = 'Enter a reason for these ' + HydrusData.ToHumanInt(
                        len(hashes)
                    ) + ' files to be removed from ' + remote_service.GetName(
                    ) + '.'
                ...
                with ClientGUIDialogs.DialogTextEntry(self, message) as dlg:
                    ...
                    if dlg.exec() == QW.QDialog.Accepted:
                        ...
                        service_keys_to_content_updates = {
                            remote_service_key: (content_update, )
                        }
    ...
    def _PublishSelectionChange(self, force_reload=False):
        ...
        if HG.client_controller.gui.IsCurrentPage(self._page_key):
            ...
            HG.client_controller.pub('new_tags_selection',
                                     self._page_key,
                                     tags_media,
                                     force_reload=force_reload)
            HG.client_controller.pub('new_page_status', self._page_key,
                                     self._GetPrettyStatus())
    ...
    def _PublishSelectionIncrement(self, medias):
        ...
        if HG.client_controller.gui.IsCurrentPage(self._page_key):
            ...
            HG.client_controller.pub('increment_tags_selection',
                                     self._page_key, medias)
            HG.client_controller.pub('new_page_status', self._page_key,
                                     self._GetPrettyStatus())
    ...
    def _RegenerateFileData(self, job_type):
        ...
        if num_files > 0:
            ...
            if job_type == ClientFiles.REGENERATE_FILE_DATA_JOB_FILE_METADATA:
                ...
                text = 'This will reparse the {} selected files\' metadata.'.format(
                    HydrusData.ToHumanInt(num_files))
                text += os.linesep * 2
                ...
                text = 'This will force-regenerate the {} selected files\' thumbnails.'.format(
                    HydrusData.ToHumanInt(num_files))

            elif job_type == ClientFiles.REGENERATE_FILE_DATA_JOB_REFIT_THUMBNAIL:

                text = 'This will regenerate the {} selected files\' thumbnails, but only if they are the wrong size.'.format(
                    HydrusData.ToHumanInt(num_files))
            ...
            if num_files > 50:
                ...
                text += os.linesep * 2
                text += 'You have selected {} files, so this job may take some time. You can run it all now or schedule it to the overall file maintenance queue for later spread-out processing.'.format(
                    HydrusData.ToHumanInt(num_files))
                ...
                with ClientGUIDialogs.DialogYesYesNo(
                        self, text, yes_tuples=yes_tuples,
            ...
            if do_it_now:
                ...
                HG.client_controller.CallToThread(
                    HG.client_controller.files_maintenance_manager.
                    RunJobImmediately, flat_media, job_type)
                ...
                HG.client_controller.CallToThread(
                    HG.client_controller.files_maintenance_manager.ScheduleJob,
                    hashes, job_type)
    ...
    def _RescindDownloadSelected(self):
        ...
        HG.client_controller.Write(
            'content_updates', {
                CC.COMBINED_LOCAL_FILE_SERVICE_KEY: [
                    HydrusData.ContentUpdate(HC.CONTENT_TYPE_FILES,
                                             HC.CONTENT_UPDATE_RESCIND_PEND,
                                             hashes)
                ]
            })
                ...
                'content_updates', {
                    file_service_key: [
                        HydrusData.ContentUpdate(
                            HC.CONTENT_TYPE_FILES,
                            HC.CONTENT_UPDATE_RESCIND_PETITION, hashes)
                    ]
                })
            ...
            HG.client_controller.Write(
                'content_updates', {
                    file_service_key: [
                        HydrusData.ContentUpdate(
                            HC.CONTENT_TYPE_FILES,
                            HC.CONTENT_UPDATE_RESCIND_PEND, hashes)
                    ]
                })
    ...
    def _Select(self, file_filter):
        ...
        if move_focus:
            ...
            if len(self._selected_media) == 0:
                ...
                media_visible = True in (self._MediaIsVisible(media)
                                         for media in self._selected_media)
    ...
    def _SetDuplicates(self,
        ...
        if duplicate_type == HC.DUPLICATE_POTENTIAL:
            ...
            yes_no_text = 'apply "{}"'.format(
                HC.duplicate_type_string_lookup[duplicate_type])

            if duplicate_type in [
                    HC.DUPLICATE_BETTER, HC.DUPLICATE_SAME_QUALITY
            ] or (HG.client_controller.new_options.GetBoolean('advanced_mode')
                  and duplicate_type == HC.DUPLICATE_ALTERNATE):
            ...
            yes_no_text = 'apply "{}" (with custom duplicate metadata merge options)'.format(
                HC.duplicate_type_string_lookup[duplicate_type])

        file_deletion_reason = 'Deleted from duplicate action on Media Page ({}).'.format(
            yes_no_text)
        ...
        if media_pairs is None:
            ...
            if duplicate_type in (HC.DUPLICATE_FALSE_POSITIVE,
                ...
                media_pairs = [(first_media, other_media)
                               for other_media in flat_media
                               if other_media != first_media]
        ...
        if not silent:
            ...
            if len(media_pairs) > 100 and duplicate_type in (
                    HC.DUPLICATE_FALSE_POSITIVE, HC.DUPLICATE_ALTERNATE):
        ...
        for (first_media, second_media) in media_pairs:
            ...
            if duplicate_action_options is None:
                ...
                service_keys_to_content_updates = duplicate_action_options.ProcessPairIntoContentUpdates(
                    first_media,
                    second_media,
                    file_deletion_reason=file_deletion_reason)

            pair_info.append((duplicate_type, first_hash, second_hash,
                              service_keys_to_content_updates))

        if len(pair_info) > 0:

            HG.client_controller.WriteSynchronous('duplicate_pair_status',
                                                  pair_info)
    ...
    def _SetDuplicatesCustom(self):
        ...
        choice_tuples = [(HC.duplicate_type_string_lookup[duplicate_type],
                          duplicate_type)
                         for duplicate_type in duplicate_types]

        try:

            duplicate_type = ClientGUIDialogsQuick.SelectFromList(
                self, 'select duplicate type', choice_tuples)
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit duplicate merge options') as dlg:

            panel = ClientGUIScrolledPanelsEdit.EditDuplicateActionOptionsPanel(
                dlg,
                duplicate_type,
                duplicate_action_options,
                for_custom_action=True)
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                self._SetDuplicates(
                    duplicate_type,
                    duplicate_action_options=duplicate_action_options)
    ...
    def _SetDuplicatesFocusedBetter(self, duplicate_action_options=None):
        ...
        if self._focused_media is None:
            ...
            QW.QMessageBox.warning(
                self, 'Warning',
                'No file is focused, so cannot set the focused file as better!'
            )
        ...
        (better_media, ) = [
            media for media in flat_media if media.GetHash() == focused_hash
        ]
        ...
        media_pairs = [(better_media, worse_media)
                       for worse_media in worse_flat_media]
    ...
    def _SetDuplicatesFocusedKing(self):
        ...
        if self._focused_media is None:
            ...
            QW.QMessageBox.warning(
                self, 'Warning',
                'No file is focused, so cannot set the focused file as king!')

            return

        focused_hash = self._focused_media.GetDisplayMedia().GetHash()

        HG.client_controller.WriteSynchronous('duplicate_set_king',
                                              focused_hash)
    ...
    def _SetFocusedMedia(self, media):
        ...
        HG.client_controller.pub('preview_changed', self._page_key,
                                 publish_media)
    ...
    def _ShareOnLocalBooru(self):
        ...
        if len(self._selected_media) > 0:
            ...
            with ClientGUIDialogs.DialogInputLocalBooruShare(
                    self, share_key, name, text, timeout, hashes,
                    new_share=True) as dlg:
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HG.client_controller.Write('local_booru_share', share_key,
                                               info)
    ...
    def _ShowDuplicatesInNewPage(self, hash, duplicate_type):
        ...
        hashes = HG.client_controller.Read('file_duplicate_hashes',
                                           self._file_service_key, hash,
                                           duplicate_type)
        ...
        if num_to_undelete > 0:
            ...
            if not HC.options['confirm_trash']:
                ...
                if num_to_undelete == 1:
                    ...
                    message = 'Are you sure you want to undelete these ' + HydrusData.ToHumanInt(
                        num_to_undelete) + ' files?'
                ...
                HG.client_controller.Write(
                    'content_updates', {
                        CC.TRASH_SERVICE_KEY: [
                            HydrusData.ContentUpdate(
                                HC.CONTENT_TYPE_FILES,
                                HC.CONTENT_UPDATE_UNDELETE, hashes)
                        ]
                    })
    ...
    def _UploadDirectory(self, file_service_key):
        ...
        if hashes is not None and len(hashes) > 0:
            ...
            ipfs_service = HG.client_controller.services_manager.GetService(
                file_service_key)

        with ClientGUIDialogs.DialogTextEntry(
                self, 'Enter a note to describe this directory.') as dlg:

            if dlg.exec() == QW.QDialog.Accepted:

                note = dlg.GetValue()

                HG.client_controller.CallToThread(ipfs_service.PinDirectory,
                                                  hashes, note)
    ...
    def AddMediaResults(self, page_key, media_results):
        ...
        if page_key == self._page_key:
            ...
            return ClientMedia.ListeningMediaList.AddMediaResults(
                self, media_results)
        ...
        if command_type == CC.APPLICATION_COMMAND_TYPE_SIMPLE:
            ...
            if action == 'copy_bmp':
                ...
                if self._focused_media is not None:
                    ...
                    ClientGUIDuplicates.RemoveFromAlternateGroup(
                        self, (hash, ))
                ...
                if self._focused_media is not None:
                    ...
                    ClientGUIDuplicates.RemoveFromDuplicateGroup(
                        self, (hash, ))
                ...
                self._Remove(
                    ClientMedia.FileFilter(ClientMedia.FILE_FILTER_SELECTED))
            ...
            command_processed = ClientGUIFunctions.ApplyContentApplicationCommandToMedia(
                self, command, self._GetSelectedFlatMedia())
    ...
    def ProcessContentUpdates(self, service_keys_to_content_updates):
        ...
        ClientMedia.ListeningMediaList.ProcessContentUpdates(
            self, service_keys_to_content_updates)

        we_were_file_or_tag_affected = False

        for (service_key,
             content_updates) in list(service_keys_to_content_updates.items()):
            ...
            for content_update in content_updates:
                ...
                if self._HasHashes(hashes):
                    ...
                    if content_update.GetDataType() in (
                            HC.CONTENT_TYPE_FILES, HC.CONTENT_TYPE_MAPPINGS):
    ...
    def ProcessServiceUpdates(self, service_keys_to_service_updates):
        ...
        ClientMedia.ListeningMediaList.ProcessServiceUpdates(
            self, service_keys_to_service_updates)

        for (service_key,
             service_updates) in list(service_keys_to_service_updates.items()):

            for service_update in service_updates:

                (action, row) = service_update.ToTuple()

                if action in (HC.SERVICE_UPDATE_DELETE_PENDING,
                              HC.SERVICE_UPDATE_RESET):
    ...
    def SelectByTags(self, page_key, and_or_or, tags):
        ...
        if page_key == self._page_key:
            ...
            self._Select(
                ClientMedia.FileFilter(ClientMedia.FILE_FILTER_TAGS,
                                       (and_or_or, tags)))
...
class MediaPanelLoading(MediaPanel):
    def __init__(self, parent, page_key, file_service_key):
        ...
        HG.client_controller.sub(self, 'SetNumQueryResults',
                                 'set_num_query_results')
...
class MediaPanelThumbnails(MediaPanel):
    def __init__(self, parent, page_key, file_service_key, media_results):
        ...
        (thumbnail_span_width,
         thumbnail_span_height) = self._GetThumbnailSpanDimensions()

        thumbnail_scroll_rate = float(
            HG.client_controller.new_options.GetString(
        ...
        self.verticalScrollBar().setSingleStep(
            int(round(thumbnail_span_height * thumbnail_scroll_rate)))
        ...
        HG.client_controller.sub(self, 'MaintainPageCache',
                                 'memory_maintenance_pulse')
        HG.client_controller.sub(self, 'NotifyNewFileInfo', 'new_file_info')
        HG.client_controller.sub(self, 'NewThumbnails', 'new_thumbnails')
        HG.client_controller.sub(self, 'ThumbnailsReset',
                                 'notify_complete_thumbnail_reset')
        HG.client_controller.sub(self, 'RedrawAllThumbnails',
                                 'refresh_all_tag_presentation_gui')
        HG.client_controller.sub(self, 'RefreshAcceleratorTable',
                                 'notify_new_options')
        HG.client_controller.sub(self, 'WaterfallThumbnails',
    ...
    def _CalculateVisiblePageIndices(self):
        ...
        (client_width,
         client_height) = QP.ScrollAreaVisibleRect(self).size().toTuple()
        ...
        page_indices = list(
            range(first_visible_page_index, last_visible_page_index + 1))
    ...
    def _CreateNewDirtyPage(self):
        ...
        self._dirty_canvas_pages.append(
            HG.client_controller.bitmap_manager.GetQtImage(
                client_width,
                self._num_rows_per_canvas_page * thumbnail_span_height, 32))
    ...
    def _DirtyPage(self, clean_index):
        ...
        thumbnails = [
            thumbnail
            for (thumbnail_index,
                 thumbnail) in self._GetThumbnailsFromPageIndex(clean_index)
        ]

        if len(thumbnails) > 0:

            HG.client_controller.GetCache('thumbnail').CancelWaterfall(
                self._page_key, thumbnails)
    ...
    def _DrawCanvasPage(self, page_index, canvas_page):
        ...
        bg_colour = HG.client_controller.new_options.GetColour(
        ...
        if new_options.GetNoneableString(
                'media_background_bmp_path') is not None:
        ...
        thumbnail_margin = HG.client_controller.new_options.GetInteger(
            'thumbnail_margin')
        ...
        for (thumbnail_index, thumbnail) in page_thumbnails:
            ...
            if hash in self._hashes_faded and thumbnail_cache.HasThumbnailCached(
                    thumbnail):
                ...
                y = (thumbnail_row -
                     (page_index * self._num_rows_per_canvas_page)
                     ) * thumbnail_span_height + thumbnail_margin
        ...
        if len(thumbnails_to_render_later) > 0:
            ...
            HG.client_controller.GetCache('thumbnail').Waterfall(
                self._page_key, thumbnails_to_render_later)
    ...
    def _FadeThumbnails(self, thumbnails):
        ...
        self._hashes_faded.update((thumbnail.GetDisplayMedia().GetHash()
        ...
        for thumbnail in thumbnails:
            ...
            if self._GetPageIndexFromThumbnailIndex(
                    thumbnail_index) not in self._clean_canvas_pages:
            ...
            self._thumbnails_being_faded_in[hash] = (bmp, alpha_bmp,
                                                     thumbnail_index,
                                                     thumbnail, now_precise, 0)
    ...
    def _GetMediaCoordinates(self, media):
        ...
        (x, y) = (column * thumbnail_span_width + thumbnail_margin,
                  row * thumbnail_span_height + thumbnail_margin)
    ...
    def _GetThumbnailSpanDimensions(self):
        ...
        thumbnail_border = HG.client_controller.new_options.GetInteger(
            'thumbnail_border')
        thumbnail_margin = HG.client_controller.new_options.GetInteger(
            'thumbnail_margin')

        return ClientData.AddPaddingToDimensions(
            HC.options['thumbnail_dimensions'],
            (thumbnail_border + thumbnail_margin) * 2)
    ...
    def _GetThumbnailsFromPageIndex(self, page_index):
        ...
        if start_index <= len(self._sorted_media):
            ...
            end_index = min(len(self._sorted_media),
                            start_index + num_thumbnails_per_page)

            thumbnails = [(index, self._sorted_media[index])
    ...
    def _MediaIsInCleanPage(self, thumbnail):
        ...
        if self._GetPageIndexFromThumbnailIndex(
                index) in self._clean_canvas_pages:
    ...
    def _MoveFocusedThumbnail(self, rows, columns, shift):
        ...
        if media_to_use is not None:
            ...
            new_position = current_position + columns + (self._num_columns *
                                                         rows)
    ...
    def _RecalculateVirtualSize(self, called_from_resize_event=False):
        ...
        if client_width > 0 and client_height > 0:
            ...
            if (virtual_width,
                    virtual_height) != self.widget().size().toTuple():
                ...
                if not called_from_resize_event:
                    ...
                    self._UpdateScrollBars(
                    )  # would lead to infinite recursion if called from a resize event
    ...
    def _RedrawMedia(self, thumbnails):
        ...
        visible_thumbnails = [
            ...
            if self._MediaIsInCleanPage(thumbnail)
        ]
    ...
    def _ScrollToMedia(self, media):
        ...
        if media is not None:
            ...
            percent_visible = new_options.GetInteger(
                'thumbnail_visibility_scroll_percent') / 100

            if y < visible_rect_y:

                self.ensureVisible(0, y, 0, 0)

            elif y > visible_rect_y + visible_rect_height - (
                    thumbnail_span_height * percent_visible):
    ...
    def _StopFading(self, hash):
        ...
        if hash in self._thumbnails_being_faded_in:
            ...
            (bmp, alpha_bmp, thumbnail_index, thumbnail, animation_started,
             num_frames) = self._thumbnails_being_faded_in[hash]
    ...
    def AddMediaResults(self, page_key, media_results):
        ...
        if page_key == self._page_key:
            ...
            thumbnails = MediaPanel.AddMediaResults(self, page_key,
                                                    media_results)
    ...
    def mouseMoveEvent(self, event):
        ...
        if event.buttons() & QC.Qt.LeftButton:
            ...
            if we_started_dragging_on_this_panel:
                ...
                (delta_x, delta_y) = (global_mouse_pos.x() - old_x,
                                      global_mouse_pos.y() - old_y)
                ...
                moved_a_decent_bit_from_start = abs(delta_x) > 15 or abs(
                    delta_y) > 15
                ...
                if clean_drag_started and moved_a_decent_bit_from_start:
                    ...
                    if len(media) > 0:
                        ...
                        result = ClientDragDrop.DoFileExportDragDrop(
                            self, self._page_key, media, alt_down)

                        if result not in (QC.Qt.IgnoreAction, ):

                            HG.client_controller.pub(
                                'media_focus_went_to_external_program',
                                self._page_key)
    ...
    def EventKeyDown(self, event):
        ...
        if event.key() == QC.Qt.Key_Escape:
            ...
            self._MoveFocusedThumbnail(
                self._num_rows_per_actual_page * direction, 0, shift)
    ...
    class _InnerWidget(QW.QWidget):
        ...
        def mousePressEvent(self, event):
            ...
            right_on_whitespace = event.button(
            ) == QC.Qt.RightButton and thumb is None

            if not right_on_whitespace:

                self._parent._HitMedia(
                    thumb,
        ...
        def paintEvent(self, event):
            ...
            (thumbnail_span_width, thumbnail_span_height
             ) = self._parent._GetThumbnailSpanDimensions()
            ...
            potential_clean_indices_to_steal = [
                page_index
                for page_index in list(self._parent._clean_canvas_pages.keys())
                if page_index not in page_indices_to_draw
            ]
            ...
            if background_pixmap is not None:
                ...
                (client_x, client_y) = QP.ScrollAreaVisibleRect(
                    self._parent).size().toTuple()

                (background_bmp_width,
                 background_bmp_height) = background_pixmap.size().toTuple()

                painter.drawPixmap(client_x - background_bmp_width,
                                   client_y - background_bmp_height,
                                   background_pixmap)
    ...
    def EventResize(self, event):
        ...
        self._last_client_size = QP.ScrollAreaVisibleRect(
            self).size().toTuple()
    ...
    def ShowMenu(self):
        ...
        all_locations_managers = [
            media.GetLocationsManager()
            for media in ClientMedia.FlattenMedia(self._sorted_media)
        ]
        selected_locations_managers = [
            media.GetLocationsManager()
            for media in ClientMedia.FlattenMedia(self._selected_media)
        ]

        selection_has_local = True in (
            locations_manager.IsLocal()
            for locations_manager in selected_locations_managers)
        selection_has_local_file_domain = True in (
            CC.LOCAL_FILE_SERVICE_KEY in locations_manager.GetCurrent()
            for locations_manager in selected_locations_managers)
        selection_has_trash = True in (
            CC.TRASH_SERVICE_KEY in locations_manager.GetCurrent()
            for locations_manager in selected_locations_managers)
        selection_has_inbox = True in (media.HasInbox()
                                       for media in self._selected_media)
        selection_has_archive = True in (media.HasArchive()
                                         for media in self._selected_media)

        all_file_domains = HydrusData.MassUnion(
            locations_manager.GetCurrent()
            for locations_manager in all_locations_managers)
        all_specific_file_domains = all_file_domains.difference(
            {CC.COMBINED_FILE_SERVICE_KEY, CC.COMBINED_LOCAL_FILE_SERVICE_KEY})
        all_local_file_domains = services_manager.Filter(
            all_specific_file_domains, (HC.LOCAL_FILE_DOMAIN, ))
        all_file_repos = services_manager.Filter(all_specific_file_domains,
                                                 (HC.FILE_REPOSITORY, ))

        has_local = True in (locations_manager.IsLocal()
                             for locations_manager in all_locations_managers)
        has_remote = True in (locations_manager.IsRemote()
                              for locations_manager in all_locations_managers)
        ...
        if self._focused_media is not None:
            ...
            collections_selected = True in (media.IsCollection()
                                            for media in self._selected_media)
            ...
            service_keys_to_names = {
                service.GetServiceKey(): service.GetName()
            ...
            local_ratings_services = [
                service for service in services
                if service.GetServiceType() in (HC.LOCAL_RATING_LIKE,
            ...
            focused_is_local = CC.COMBINED_LOCAL_FILE_SERVICE_KEY in self._focused_media.GetLocationsManager(
            ).GetCurrent()

            file_service_keys = {
                repository.GetServiceKey()
                for repository in file_repositories
            }
            upload_permission_file_service_keys = {
                repository.GetServiceKey()
                for repository in file_repositories
                if repository.HasPermission(HC.CONTENT_TYPE_FILES,
            ...
            petition_permission_file_service_keys = {
                repository.GetServiceKey()
                for repository in file_repositories
                if repository.HasPermission(HC.CONTENT_TYPE_FILES,
            ...
            user_manage_permission_file_service_keys = {
                repository.GetServiceKey()
                for repository in file_repositories
                if repository.HasPermission(HC.CONTENT_TYPE_ACCOUNTS,
                                            HC.PERMISSION_ACTION_OVERRULE)
            }
            ipfs_service_keys = {
            ...
            focused_is_ipfs = True in (
                service_key in ipfs_service_keys for service_key in
                self._focused_media.GetLocationsManager().GetCurrentRemote())
            ...
            groups_of_current_remote_service_keys = [
                locations_manager.GetCurrentRemote()
                for locations_manager in selected_locations_managers
            ]
            groups_of_pending_remote_service_keys = [
                locations_manager.GetPendingRemote()
                for locations_manager in selected_locations_managers
            ]
            groups_of_petitioned_remote_service_keys = [
                locations_manager.GetPetitionedRemote()
                for locations_manager in selected_locations_managers
            ]
            groups_of_deleted_remote_service_keys = [
                locations_manager.GetDeletedRemote()
                for locations_manager in selected_locations_managers
            ]

            current_remote_service_keys = HydrusData.MassUnion(
                groups_of_current_remote_service_keys)
            pending_remote_service_keys = HydrusData.MassUnion(
                groups_of_pending_remote_service_keys)
            petitioned_remote_service_keys = HydrusData.MassUnion(
                groups_of_petitioned_remote_service_keys)
            deleted_remote_service_keys = HydrusData.MassUnion(
                groups_of_deleted_remote_service_keys)

            common_current_remote_service_keys = HydrusData.IntelligentMassIntersect(
                groups_of_current_remote_service_keys)
            common_pending_remote_service_keys = HydrusData.IntelligentMassIntersect(
                groups_of_pending_remote_service_keys)
            common_petitioned_remote_service_keys = HydrusData.IntelligentMassIntersect(
                groups_of_petitioned_remote_service_keys)
            common_deleted_remote_service_keys = HydrusData.IntelligentMassIntersect(
                groups_of_deleted_remote_service_keys)
            ...
            some_downloading = True in (
                locations_manager.IsDownloading()
                for locations_manager in selected_locations_managers)

            pending_file_service_keys = pending_remote_service_keys.intersection(
                file_service_keys)
            petitioned_file_service_keys = petitioned_remote_service_keys.intersection(
                file_service_keys)

            common_current_file_service_keys = common_current_remote_service_keys.intersection(
                file_service_keys)
            common_pending_file_service_keys = common_pending_remote_service_keys.intersection(
                file_service_keys)
            common_petitioned_file_service_keys = common_petitioned_remote_service_keys.intersection(
                file_service_keys)
            common_deleted_file_service_keys = common_deleted_remote_service_keys.intersection(
                file_service_keys)

            disparate_current_file_service_keys = disparate_current_remote_service_keys.intersection(
                file_service_keys)
            disparate_pending_file_service_keys = disparate_pending_remote_service_keys.intersection(
                file_service_keys)
            disparate_petitioned_file_service_keys = disparate_petitioned_remote_service_keys.intersection(
                file_service_keys)
            disparate_deleted_file_service_keys = disparate_deleted_remote_service_keys.intersection(
                file_service_keys)

            pending_ipfs_service_keys = pending_remote_service_keys.intersection(
                ipfs_service_keys)
            petitioned_ipfs_service_keys = petitioned_remote_service_keys.intersection(
                ipfs_service_keys)

            common_current_ipfs_service_keys = common_current_remote_service_keys.intersection(
                ipfs_service_keys)
            common_pending_ipfs_service_keys = common_pending_file_service_keys.intersection(
                ipfs_service_keys)
            common_petitioned_ipfs_service_keys = common_petitioned_remote_service_keys.intersection(
                ipfs_service_keys)

            disparate_current_ipfs_service_keys = disparate_current_remote_service_keys.intersection(
                ipfs_service_keys)
            disparate_pending_ipfs_service_keys = disparate_pending_remote_service_keys.intersection(
                ipfs_service_keys)
            disparate_petitioned_ipfs_service_keys = disparate_petitioned_remote_service_keys.intersection(
                ipfs_service_keys)
            ...
            for locations_manager in selected_locations_managers:
                ...
                if locations_manager.IsLocal():
                    ...
                    uploadable_file_service_keys.update(
                        upload_permission_file_service_keys -
                        locations_manager.GetCurrentRemote() -
                        locations_manager.GetPendingRemote() -
                        (locations_manager.GetDeletedRemote() -
                ...
                ) and not locations_manager.IsDownloading():

                    downloadable_file_service_keys.update(
                        ipfs_service_keys.union(file_service_keys)
                        & locations_manager.GetCurrentRemote())

                # we can petition when we have permission and a file is current and it is not already petitioned

                petitionable_file_service_keys.update(
                    (petition_permission_file_service_keys
                     & locations_manager.GetCurrentRemote()) -
                    locations_manager.GetPetitionedRemote())
                ...
                deletable_file_service_keys.update(
                    (petition_resolve_permission_file_service_keys
                     & locations_manager.GetCurrentRemote()) -
                    locations_manager.GetPetitionedRemote())
                ...
                modifyable_file_service_keys.update(
                    user_manage_permission_file_service_keys
                    & (locations_manager.GetCurrentRemote()
                       | locations_manager.GetDeletedRemote()))
                ...
                if locations_manager.IsLocal():
                    ...
                    pinnable_ipfs_service_keys.update(
                        ipfs_service_keys -
                        locations_manager.GetCurrentRemote() -
                        locations_manager.GetPendingRemote())
                ...
                unpinnable_ipfs_service_keys.update(
                    (ipfs_service_keys
                     & locations_manager.GetCurrentRemote()) -
                    locations_manager.GetPetitionedRemote())
            ...
            if multiple_selected:
                ...
                selection_info_menu_label = '{} files, {}'.format(
                    HydrusData.ToHumanInt(num_selected),
                    self._GetPrettyTotalSize(only_selected=True))
                ...
                for line in pretty_info_lines:
                    ...
                    ClientGUIMenus.AppendMenuLabel(selection_info_menu, line,
                                                   line)

            if len(self._selected_media) == 1:

                ClientGUIMedia.AddFileViewingStatsMenu(selection_info_menu,
                                                       self._focused_media)

            if len(disparate_current_file_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, disparate_current_file_service_keys,
                    'some uploaded to')

            if multiple_selected and len(common_current_file_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, common_current_file_service_keys,
                    'selected uploaded to')

            if len(disparate_pending_file_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, disparate_pending_file_service_keys,
                    'some pending to')

            if len(common_pending_file_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, common_pending_file_service_keys,
                    'pending to')
            ...
            if len(disparate_petitioned_file_service_keys) > 0:
                ...
                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu,
                    disparate_petitioned_file_service_keys,
                    'some petitioned from')

            if len(common_petitioned_file_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, common_petitioned_file_service_keys,
                    'petitioned from')

            if len(disparate_deleted_file_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, disparate_deleted_file_service_keys,
                    'some deleted from')

            if len(common_deleted_file_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, common_deleted_file_service_keys,
                    'deleted from')

            if len(disparate_current_ipfs_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, disparate_current_ipfs_service_keys,
                    'some pinned to')

            if multiple_selected and len(common_current_ipfs_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, common_current_ipfs_service_keys,
                    'selected pinned to')

            if len(disparate_pending_ipfs_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, disparate_pending_ipfs_service_keys,
                    'some to be pinned to')

            if len(common_pending_ipfs_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, common_pending_ipfs_service_keys,
                    'to be pinned to')
            ...
            if len(disparate_petitioned_ipfs_service_keys) > 0:
                ...
                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu,
                    disparate_petitioned_ipfs_service_keys,
                    'some to be unpinned from')

            if len(common_petitioned_ipfs_service_keys) > 0:

                ClientGUIMedia.AddServiceKeyLabelsToMenu(
                    selection_info_menu, common_petitioned_ipfs_service_keys,
                    unpin_phrase)
            ...
            if len(selection_info_menu.actions()) == 0:
                ...
                ClientGUIMenus.AppendMenu(menu, selection_info_menu,
                                          selection_info_menu_label)

        ClientGUIMenus.AppendSeparator(menu)

        ClientGUIMenus.AppendMenuItem(menu, 'refresh',
                                      'Refresh the current search.',
                                      HG.client_controller.pub,
                                      'refresh_query', self._page_key)
        ...
        if len(self._sorted_media) > 0:
            ...
            filter_counts[ClientMedia.FileFilter(
                ClientMedia.FILE_FILTER_ALL)] = num_files
            filter_counts[ClientMedia.FileFilter(
                ClientMedia.FILE_FILTER_INBOX)] = num_inbox
            filter_counts[ClientMedia.FileFilter(
                ClientMedia.FILE_FILTER_ARCHIVE)] = num_archive
            filter_counts[ClientMedia.FileFilter(
                ClientMedia.FILE_FILTER_SELECTED)] = num_selected

            has_local_and_remote = has_local and has_remote

            AddSelectMenu(self, menu, filter_counts, all_specific_file_domains,
                          has_local_and_remote)
            AddRemoveMenu(self, menu, filter_counts, all_specific_file_domains,
                          has_local_and_remote)
            ...
            if has_local:
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'archive/delete filter',
                    'Launch a special media viewer that will quickly archive (left-click) and delete (right-click) the selected media.',
                    self._ArchiveDeleteFilter)
        ...
        if self._focused_media is not None:
            ...
            if selection_has_inbox:
                ...
                ClientGUIMenus.AppendMenuItem(menu, archive_phrase,
            ...
            if selection_has_archive:
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, inbox_phrase,
                    'Put the selected files back in the inbox.', self._Inbox)
        ...
        if self._focused_media is not None:
            ...
            if selection_has_local_file_domain:
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, local_delete_phrase,
                    'Delete the selected files from \'my files\'.',
                    self._Delete)

            if selection_has_trash:

                ClientGUIMenus.AppendMenuItem(
                    menu, trash_delete_phrase,
                    'Delete the selected files from the trash, forcing an immediate physical delete from your hard drive.',
                    self._Delete, CC.TRASH_SERVICE_KEY)
                ClientGUIMenus.AppendMenuItem(
                    menu, undelete_phrase,
                    'Restore the selected files back to \'my files\'.',
                    self._Undelete)
            ...
            ClientGUIMenus.AppendMenuItem(
                manage_menu, 'tags', 'Manage tags for the selected files.',
                self._ManageTags)
            ...
            if i_can_post_ratings:
                ...
                ClientGUIMenus.AppendMenuItem(
                    manage_menu, 'ratings',
                    'Manage ratings for the selected files.',
                    self._ManageRatings)

            ClientGUIMenus.AppendMenuItem(
                manage_menu, 'urls', 'Manage urls for the selected files.',
                self._ManageURLs)
            ClientGUIMenus.AppendMenuItem(
                manage_menu, 'notes', 'Manage notes for the focused file.',
                self._ManageNotes)

            len_interesting_remote_service_keys = 0

            len_interesting_remote_service_keys += len(
                downloadable_file_service_keys)
            len_interesting_remote_service_keys += len(
                uploadable_file_service_keys)
            len_interesting_remote_service_keys += len(
                pending_file_service_keys)
            len_interesting_remote_service_keys += len(
                petitionable_file_service_keys)
            len_interesting_remote_service_keys += len(
                petitioned_file_service_keys)
            len_interesting_remote_service_keys += len(
                deletable_file_service_keys)
            len_interesting_remote_service_keys += len(
                modifyable_file_service_keys)
            len_interesting_remote_service_keys += len(
                pinnable_ipfs_service_keys)
            len_interesting_remote_service_keys += len(
                pending_ipfs_service_keys)
            len_interesting_remote_service_keys += len(
                unpinnable_ipfs_service_keys)
            len_interesting_remote_service_keys += len(
                petitioned_ipfs_service_keys)
            ...
            if len_interesting_remote_service_keys > 0:
                ...
                if len(downloadable_file_service_keys) > 0:
                    ClientGUIMenus.AppendMenuItem(
                        remote_action_menu, download_phrase,
                        'Download all possible selected files.',
                        self._DownloadSelected)

                if some_downloading:
                    ClientGUIMenus.AppendMenuItem(
                        remote_action_menu, rescind_download_phrase,
                        'Stop downloading any of the selected files.',
                        self._RescindDownloadSelected)

                if len(uploadable_file_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, uploadable_file_service_keys,
                        upload_phrase,
                        'Upload all selected files to the file repository.',
                        self._UploadFiles)

                if len(pending_file_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, pending_file_service_keys,
                        rescind_upload_phrase,
                        'Rescind the pending upload to the file repository.',
                        self._RescindUploadFiles)

                if len(petitionable_file_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu,
                        petitionable_file_service_keys, petition_phrase,
                        'Petition these files for deletion from the file repository.',
                        self._PetitionFiles)

                if len(petitioned_file_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, petitioned_file_service_keys,
                        rescind_petition_phrase,
                        'Rescind the petition to delete these files from the file repository.',
                        self._RescindPetitionFiles)

                if len(deletable_file_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, deletable_file_service_keys,
                        remote_delete_phrase,
                        'Delete these files from the file repository.',
                        self._Delete)

                if len(modifyable_file_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, modifyable_file_service_keys,
                        modify_account_phrase,
                        'Modify the account(s) that uploaded these files to the file repository.',
                        self._ModifyUploaders)

                if len(pinnable_ipfs_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, pinnable_ipfs_service_keys,
                        pin_phrase, 'Pin these files to the ipfs service.',
                        self._UploadFiles)

                if len(pending_ipfs_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, pending_ipfs_service_keys,
                        rescind_pin_phrase,
                        'Rescind the pending pin to the ipfs service.',
                        self._RescindUploadFiles)

                if len(unpinnable_ipfs_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, unpinnable_ipfs_service_keys,
                        unpin_phrase,
                        'Unpin these files from the ipfs service.',
                        self._PetitionFiles)

                if len(petitioned_ipfs_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, petitioned_ipfs_service_keys,
                        rescind_unpin_phrase,
                        'Rescind the pending unpin from the ipfs service.',
                        self._RescindPetitionFiles)

                if multiple_selected and len(ipfs_service_keys) > 0:

                    ClientGUIMedia.AddServiceKeysToMenu(
                        self, remote_action_menu, ipfs_service_keys,
                        'pin new directory to',
                        'Pin these files as a directory to the ipfs service.',
                        self._UploadDirectory)

                ClientGUIMenus.AppendMenu(manage_menu, remote_action_menu,
                                          'remote services')
            ...
            if HG.client_controller.DBCurrentlyDoingJob():
                ...
                file_duplicate_info = HG.client_controller.Read(
                    'file_duplicate_info', self._file_service_key,
                    focused_hash)
            ...
            focus_can_be_searched = self._focused_media.GetDisplayMedia(
            ).GetMime() in HC.MIMES_WE_CAN_PHASH

            if file_duplicate_info is None:

                ClientGUIMenus.AppendMenuLabel(
                    duplicates_menu,
                    'could not fetch file\'s duplicates (db currently locked)')
                ...
                if len(file_duplicate_types_to_counts) > 0:
                    ...
                    if HC.DUPLICATE_MEMBER in file_duplicate_types_to_counts:
                        ...
                        if file_duplicate_info['is_king']:
                            ...
                            ClientGUIMenus.AppendMenuItem(
                                duplicates_view_menu,
                                'show the best quality file of this file\'s group',
                                'Load up a new search with this file\'s best quality duplicate.',
                                self._ShowDuplicatesInNewPage, focused_hash,
                                HC.DUPLICATE_KING)

                        ClientGUIMenus.AppendSeparator(duplicates_view_menu)

                    for duplicate_type in (HC.DUPLICATE_MEMBER,
                                           HC.DUPLICATE_ALTERNATE,
                        ...
                        if duplicate_type in file_duplicate_types_to_counts:
                            ...
                            count = file_duplicate_types_to_counts[
                                duplicate_type]

                            if count > 0:

                                label = HydrusData.ToHumanInt(
                                    count
                                ) + ' ' + HC.duplicate_type_string_lookup[
                                    duplicate_type]

                                ClientGUIMenus.AppendMenuItem(
                                    duplicates_view_menu, label,
                                    'Show these duplicates in a new page.',
                                    self._ShowDuplicatesInNewPage,
                                    focused_hash, duplicate_type)
                    ...
                    ClientGUIMenus.AppendMenu(duplicates_menu,
                                              duplicates_view_menu,
                                              'view this file\'s relations')

            focus_is_definitely_king = file_duplicate_info is not None and file_duplicate_info[
                'is_king']
            ...
            if multiple_selected or single_action_available:
                ...
                if file_duplicate_info is None:
                    ...
                    ClientGUIMenus.AppendMenuLabel(
                        duplicates_action_submenu,
                        'could not fetch info to check for available file actions (db currently locked)'
                    )
                    ...
                    if not focus_is_definitely_king:
                        ClientGUIMenus.AppendMenuItem(
                            duplicates_action_submenu,
                            'set this file as the best quality of its group',
                            'Set the focused media to be the King of its group.',
                            self.ProcessApplicationCommand,
                            ClientData.ApplicationCommand(
                                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                'duplicate_media_set_focused_king'))

                    if dissolution_actions_available:

                        duplicates_single_dissolution_menu = QW.QMenu(
                            duplicates_action_submenu)

                        if focus_can_be_searched:
                            ClientGUIMenus.AppendMenuItem(
                                duplicates_single_dissolution_menu,
                                'schedule this file to be searched for potentials again',
                                'Queue this file for another potentials search. Will not remove any existing potentials.',
                                self.ProcessApplicationCommand,
                                ClientData.ApplicationCommand(
                                    CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                    'duplicate_media_reset_focused_potential_search'
                                ))

                        if focus_has_potentials:
                            ClientGUIMenus.AppendMenuItem(
                                duplicates_single_dissolution_menu,
                                'remove this file\'s potential relationships',
                                'Clear out this file\'s potential relationships.',
                                self.ProcessApplicationCommand,
                                ClientData.ApplicationCommand(
                                    CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                    'duplicate_media_remove_focused_potentials'
                                ))
                        ...
                        if focus_is_in_duplicate_group:
                            ...
                            if not focus_is_definitely_king:
                                ClientGUIMenus.AppendMenuItem(
                                    duplicates_single_dissolution_menu,
                                    'remove this file from its duplicate group',
                                    'Extract this file from its duplicate group and reset its search status.',
                                    self.ProcessApplicationCommand,
                                    ClientData.ApplicationCommand(
                                        CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                        'duplicate_media_remove_focused_from_duplicate_group'
                                    ))

                            ClientGUIMenus.AppendMenuItem(
                                duplicates_single_dissolution_menu,
                                'dissolve this file\'s duplicate group completely',
                                'Completely eliminate this file\'s duplicate group and reset all files\' search status.',
                                self.ProcessApplicationCommand,
                                ClientData.ApplicationCommand(
                                    CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                    'duplicate_media_dissolve_focused_duplicate_group'
                                ))
                        ...
                        if focus_is_in_alternate_group:
                            ...
                            ClientGUIMenus.AppendMenuItem(
                                duplicates_single_dissolution_menu,
                                'remove this file from its alternate group',
                                'Extract this file\'s duplicate group from its alternate group and reset the duplicate group\'s search status.',
                                self.ProcessApplicationCommand,
                                ClientData.ApplicationCommand(
                                    CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                    'duplicate_media_remove_focused_from_alternate_group'
                                ))

                            ClientGUIMenus.AppendMenuItem(
                                duplicates_single_dissolution_menu,
                                'dissolve this file\'s alternate group completely',
                                'Completely eliminate this file\'s alternate group and all duplicate group members. This resets search status for all involved files.',
                                self.ProcessApplicationCommand,
                                ClientData.ApplicationCommand(
                                    CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                    'duplicate_media_dissolve_focused_alternate_group'
                                ))
                        ...
                        if focus_has_fps:
                            ClientGUIMenus.AppendMenuItem(
                                ...
                                'Clear out all false-positive relationships this file\'s alternates group has with other groups and resets search status.',
                                self.ProcessApplicationCommand,
                                ClientData.ApplicationCommand(
                                    CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                    'duplicate_media_clear_focused_false_positives'
                                ))

                        ClientGUIMenus.AppendMenu(
                            duplicates_action_submenu,
                            duplicates_single_dissolution_menu,
                            'remove/reset for this file')

                if multiple_selected:

                    ClientGUIMenus.AppendSeparator(duplicates_action_submenu)

                    label = 'set this file as better than the ' + HydrusData.ToHumanInt(
                        num_selected - 1) + ' other selected'

                    ClientGUIMenus.AppendMenuItem(
                        duplicates_action_submenu, label,
                        'Set the focused media to be better than the other selected files.',
                        self.ProcessApplicationCommand,
                        ClientData.ApplicationCommand(
                            CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                            'duplicate_media_set_focused_better'))

                    num_pairs = num_selected * (
                        num_selected - 1) / 2  # com // ations -- n!/2(n-2)!

                    num_pairs_text = HydrusData.ToHumanInt(
                        num_pairs) + ' pairs'

                    ClientGUIMenus.AppendMenuItem(
                        duplicates_action_submenu,
                        'set all selected as same quality duplicates',
                        'Set all the selected files as same quality duplicates.',
                        self.ProcessApplicationCommand,
                        ClientData.ApplicationCommand(
                            CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                            'duplicate_media_set_same_quality'))

                    ClientGUIMenus.AppendSeparator(duplicates_action_submenu)

                    ClientGUIMenus.AppendMenuItem(
                        duplicates_action_submenu,
                        'set all selected as alternates',
                        'Set all the selected files as alternates.',
                        self.ProcessApplicationCommand,
                        ClientData.ApplicationCommand(
                            CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                            'duplicate_media_set_alternate'))

                    ClientGUIMenus.AppendMenuItem(
                        duplicates_action_submenu,
                        'set a relationship with custom metadata merge options',
                        'Choose which duplicates status to set to this selection and customise non-default duplicate metadata merge options.',
                        self.ProcessApplicationCommand,
                        ClientData.ApplicationCommand(
                            CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                            'duplicate_media_set_custom'))
                    ...
                    if collections_selected:
                        ...
                        ClientGUIMenus.AppendMenuItem(
                            duplicates_action_submenu,
                            'set selected collections as groups of alternates',
                            'Set files in the selection which are collected together as alternates.',
                            self.ProcessApplicationCommand,
                            ClientData.ApplicationCommand(
                                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                'duplicate_media_set_alternate_collections'))
                    ...
                    ClientGUIMenus.AppendMenuItem(
                        ...
                        'Queue all these files up in the duplicates filter.',
                        self.ProcessApplicationCommand,
                        ClientData.ApplicationCommand(
                            CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                            'duplicate_media_set_potential'))
                    ...
                    if advanced_mode:
                        ...
                        duplicates_multiple_dissolution_menu = QW.QMenu(
                            duplicates_action_submenu)

                        ClientGUIMenus.AppendMenuItem(
                            duplicates_multiple_dissolution_menu,
                            'schedule these files to be searched for potentials again',
                            'Queue these files for another potentials search. Will not remove any existing potentials.',
                            self.ProcessApplicationCommand,
                            ClientData.ApplicationCommand(
                                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                'duplicate_media_reset_potential_search'))
                        ClientGUIMenus.AppendMenuItem(
                            duplicates_multiple_dissolution_menu,
                            'remove these files\' potential relationships',
                            'Clear out these files\' potential relationships.',
                            self.ProcessApplicationCommand,
                            ClientData.ApplicationCommand(
                                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                'duplicate_media_remove_potentials'))
                        ClientGUIMenus.AppendMenuItem(
                            duplicates_multiple_dissolution_menu,
                            'dissolve these files\' duplicate groups completely',
                            'Completely eliminate these files\' duplicate groups and reset all files\' search status.',
                            self.ProcessApplicationCommand,
                            ClientData.ApplicationCommand(
                                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                'duplicate_media_dissolve_duplicate_group'))
                        ClientGUIMenus.AppendMenuItem(
                            duplicates_multiple_dissolution_menu,
                            'dissolve these files\' alternate groups completely',
                            'Completely eliminate these files\' alternate groups and all duplicate group members. This resets search status for all involved files.',
                            self.ProcessApplicationCommand,
                            ClientData.ApplicationCommand(
                                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                'duplicate_media_dissolve_alternate_group'))
                        ClientGUIMenus.AppendMenuItem(
                            duplicates_multiple_dissolution_menu,
                            'delete all false-positive relationships these files\' alternate groups have with other groups',
                            'Clear out all false-positive relationships these files\' alternates groups has with other groups and resets search status.',
                            self.ProcessApplicationCommand,
                            ClientData.ApplicationCommand(
                                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                'duplicate_media_clear_false_positives'))

                        ClientGUIMenus.AppendMenu(
                            duplicates_action_submenu,
                            duplicates_multiple_dissolution_menu,
                            'remove/reset for all selected')

                    duplicates_edit_action_submenu = QW.QMenu(
                        duplicates_action_submenu)

                    for duplicate_type in (HC.DUPLICATE_BETTER,
                                           HC.DUPLICATE_SAME_QUALITY):
                        ClientGUIMenus.AppendMenuItem(
                            duplicates_edit_action_submenu, 'for ' +
                            HC.duplicate_type_string_lookup[duplicate_type],
                            'Edit what happens when you set this status.',
                            self._EditDuplicateActionOptions, duplicate_type)

                    if HG.client_controller.new_options.GetBoolean(
                            'advanced_mode'):
                        ClientGUIMenus.AppendMenuItem(
                            duplicates_edit_action_submenu,
                            'for ' + HC.duplicate_type_string_lookup[
                                HC.DUPLICATE_ALTERNATE] + ' (advanced!)',
                            'Edit what happens when you set this status.',
                            self._EditDuplicateActionOptions,
                            HC.DUPLICATE_ALTERNATE)
            ...
            if self._focused_media.HasImages():
                ...
                ClientGUIMenus.AppendMenuItem(
                    similar_menu, 'exact match',
                    'Search the database for files that look precisely like those selected.',
                    self._GetSimilarTo, HC.HAMMING_EXACT_MATCH)
                ClientGUIMenus.AppendMenuItem(
                    similar_menu, 'very similar',
                    'Search the database for files that look just like those selected.',
                    self._GetSimilarTo, HC.HAMMING_VERY_SIMILAR)
                ClientGUIMenus.AppendMenuItem(
                    similar_menu, 'similar',
                    'Search the database for files that look generally like those selected.',
                    self._GetSimilarTo, HC.HAMMING_SIMILAR)
                ClientGUIMenus.AppendMenuItem(
                    similar_menu, 'speculative',
                    'Search the database for files that probably look like those selected. This is sometimes useful for symbols with sharp edges or lines.',
                    self._GetSimilarTo, HC.HAMMING_SPECULATIVE)

                ClientGUIMenus.AppendMenu(duplicates_menu, similar_menu,
                                          'find similar-looking files')

            if len(duplicates_menu.actions()) == 0:

                ClientGUIMenus.AppendMenuLabel(
                    duplicates_menu,
                    'no file relationships or actions available for this file at present'
                )

            ClientGUIMenus.AppendMenu(manage_menu, duplicates_menu,
                                      'file relationships')

            regen_menu = QW.QMenu(manage_menu)

            ClientGUIMenus.AppendMenuItem(
                regen_menu, 'thumbnails, but only if wrong size',
                'Regenerate the selected files\' thumbnails, but only if they are the wrong size.',
                self._RegenerateFileData,
                ClientFiles.REGENERATE_FILE_DATA_JOB_REFIT_THUMBNAIL)
            ClientGUIMenus.AppendMenuItem(
                regen_menu, 'thumbnails',
                'Regenerate the selected files\'s thumbnails.',
                self._RegenerateFileData,
                ClientFiles.REGENERATE_FILE_DATA_JOB_FORCE_THUMBNAIL)
            ClientGUIMenus.AppendMenuItem(
                regen_menu, 'file metadata',
                'Regenerated the selected files\' metadata and thumbnails.',
                self._RegenerateFileData,
            ...
            ClientGUIMedia.AddKnownURLsViewCopyMenu(
                self,
                menu,
                self._focused_media,
                selected_media=self._selected_media)
            ...
            ClientGUIMenus.AppendMenuItem(
                open_menu, 'in external program',
                'Launch this file with your OS\'s default program for it.',
                self._OpenExternally)
            ClientGUIMenus.AppendMenuItem(
                open_menu, 'in a new page',
                'Copy your current selection into a simple new page.',
                self._ShowSelectionInNewPage)
            ClientGUIMenus.AppendMenuItem(
                open_menu, 'in web browser',
                'Show this file in your OS\'s web browser.',
                self._OpenFileInWebBrowser)
            ...
            if focused_is_local:
                ...
                if show_open_in_explorer:
                    ...
                    ClientGUIMenus.AppendMenuItem(
                        open_menu, 'in file browser',
                        'Show this file in your OS\'s file browser.',
            ...
            if selection_has_local:
                ...
                if advanced_mode:
                    ...
                    ClientGUIMenus.AppendMenuItem(
                        copy_hash_menu, 'sha256 (hydrus default)',
                        'Copy the selected file\'s SHA256 hash to the clipboard.',
                        self._CopyHashToClipboard, 'sha256')
                    ClientGUIMenus.AppendMenuItem(
                        copy_hash_menu, 'md5',
                        'Copy the selected file\'s MD5 hash to the clipboard.',
                        self._CopyHashToClipboard, 'md5')
                    ClientGUIMenus.AppendMenuItem(
                        copy_hash_menu, 'sha1',
                        'Copy the selected file\'s SHA1 hash to the clipboard.',
                        self._CopyHashToClipboard, 'sha1')
                    ClientGUIMenus.AppendMenuItem(
                        copy_hash_menu, 'sha512',
                        'Copy the selected file\'s SHA512 hash to the clipboard.',
                        self._CopyHashToClipboard, 'sha512')

                    ClientGUIMenus.AppendMenu(copy_menu, copy_hash_menu,
                                              'hash')
                    ...
                    if multiple_selected:
                        ...
                        ClientGUIMenus.AppendMenuItem(
                            copy_hash_menu, 'sha256 (hydrus default)',
                            'Copy the selected files\' SHA256 hashes to the clipboard.',
                            self._CopyHashesToClipboard, 'sha256')
                        ClientGUIMenus.AppendMenuItem(
                            copy_hash_menu, 'md5',
                            'Copy the selected files\' MD5 hashes to the clipboard.',
                            self._CopyHashesToClipboard, 'md5')
                        ClientGUIMenus.AppendMenuItem(
                            copy_hash_menu, 'sha1',
                            'Copy the selected files\' SHA1 hashes to the clipboard.',
                            self._CopyHashesToClipboard, 'sha1')
                        ClientGUIMenus.AppendMenuItem(
                            copy_hash_menu, 'sha512',
                            'Copy the selected files\' SHA512 hashes to the clipboard.',
                ...
                if advanced_mode:
                    ...
                    if multiple_selected:
                        ...
                        ClientGUIMenus.AppendMenuItem(
                            copy_menu, 'sha256 hashes',
                            'Copy the selected files\' SHA256 hash to the clipboard.',
                            self._CopyHashesToClipboard, 'sha256')

            for ipfs_service_key in self._focused_media.GetLocationsManager(
            ).GetCurrentRemote().intersection(ipfs_service_keys):

                name = service_keys_to_names[ipfs_service_key]

                ClientGUIMenus.AppendMenuItem(
                    copy_menu, name + ' multihash',
                    'Copy the selected file\'s multihash to the clipboard.',
                    self._CopyServiceFilenameToClipboard, ipfs_service_key)

            if multiple_selected:

                for ipfs_service_key in disparate_current_ipfs_service_keys.union(
                        common_current_ipfs_service_keys):

                    name = service_keys_to_names[ipfs_service_key]

                    ClientGUIMenus.AppendMenuItem(
                        copy_menu, name + ' multihashes',
                        'Copy the selected files\' multihashes to the clipboard.',
                        self._CopyServiceFilenamesToClipboard,
                        ipfs_service_key)
            ...
            if focused_is_local:
                ...
                if self._focused_media.GetMime() in HC.IMAGES:
                    ...
                    ClientGUIMenus.AppendMenuItem(
                        copy_menu, 'image (bitmap)',
                        'Copy the selected file\'s image data to the clipboard (as a bmp).',
                        self._CopyBMPToClipboard)
            ...
            ClientGUIMenus.AppendMenuItem(
                export_menu, export_phrase,
                'Export the selected files to an external folder.',
                self._ExportFiles)
            ...
            if local_booru_is_running:
                ...
                ClientGUIMenus.AppendMenuItem(
                    share_menu, 'on local booru',
                    'Share the selected files on your client\'s local booru.',
                    self._ShareOnLocalBooru)
    ...
    def NotifyNewFileInfo(self, hashes):
        def qt_do_update(hashes_to_media_results):
            ...
            affected_media = self._GetMedia(set(
                hashes_to_media_results.keys()))
        ...
        def do_it(win, callable, affected_hashes):
            ...
            HG.client_controller.CallLaterQtSafe(win, 0, qt_do_update,
                                                 hashes_to_media_results)
    ...
    def RefreshAcceleratorTable(self):
        ...
        QP.AddShortcut(self, QC.Qt.NoModifier, QC.Qt.Key_Home,
                       self._ScrollHome, False)
        QP.AddShortcut(self, QC.Qt.KeypadModifier, QC.Qt.Key_Home,
                       self._ScrollHome, False)
        QP.AddShortcut(self, QC.Qt.NoModifier, QC.Qt.Key_End, self._ScrollEnd,
                       False)
        QP.AddShortcut(self, QC.Qt.KeypadModifier, QC.Qt.Key_End,
                       self._ScrollEnd, False)
        QP.AddShortcut(self, QC.Qt.NoModifier, QC.Qt.Key_Delete, self._Delete)
        QP.AddShortcut(self, QC.Qt.KeypadModifier, QC.Qt.Key_Delete,
                       self._Delete)
        QP.AddShortcut(self, QC.Qt.NoModifier, QC.Qt.Key_Return,
                       self._FullScreen)
        QP.AddShortcut(self, QC.Qt.KeypadModifier, QC.Qt.Key_Enter,
                       self._FullScreen)
        QP.AddShortcut(self, QC.Qt.NoModifier, QC.Qt.Key_Up,
                       self._MoveFocusedThumbnail, -1, 0, False)
        QP.AddShortcut(self, QC.Qt.KeypadModifier, QC.Qt.Key_Up,
                       self._MoveFocusedThumbnail, -1, 0, False)
        QP.AddShortcut(self, QC.Qt.NoModifier, QC.Qt.Key_Down,
                       self._MoveFocusedThumbnail, 1, 0, False)
        QP.AddShortcut(self, QC.Qt.KeypadModifier, QC.Qt.Key_Down,
                       self._MoveFocusedThumbnail, 1, 0, False)
        QP.AddShortcut(self, QC.Qt.NoModifier, QC.Qt.Key_Left,
                       self._MoveFocusedThumbnail, 0, -1, False)
        QP.AddShortcut(self, QC.Qt.KeypadModifier, QC.Qt.Key_Left,
                       self._MoveFocusedThumbnail, 0, -1, False)
        QP.AddShortcut(self, QC.Qt.NoModifier, QC.Qt.Key_Right,
                       self._MoveFocusedThumbnail, 0, 1, False)
        QP.AddShortcut(self, QC.Qt.KeypadModifier, QC.Qt.Key_Right,
                       self._MoveFocusedThumbnail, 0, 1, False)
        QP.AddShortcut(self, QC.Qt.ShiftModifier, QC.Qt.Key_Home,
                       self._ScrollHome, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier | QC.Qt.KeypadModifier,
                       QC.Qt.Key_Home, self._ScrollHome, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier, QC.Qt.Key_End,
                       self._ScrollEnd, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier | QC.Qt.KeypadModifier,
                       QC.Qt.Key_End, self._ScrollEnd, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier, QC.Qt.Key_Delete,
                       self._Undelete)
        QP.AddShortcut(self, QC.Qt.ShiftModifier | QC.Qt.KeypadModifier,
                       QC.Qt.Key_Delete, self._Undelete)
        QP.AddShortcut(self, QC.Qt.ShiftModifier, QC.Qt.Key_Up,
                       self._MoveFocusedThumbnail, -1, 0, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier | QC.Qt.KeypadModifier,
                       QC.Qt.Key_Up, self._MoveFocusedThumbnail, -1, 0, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier, QC.Qt.Key_Down,
                       self._MoveFocusedThumbnail, 1, 0, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier | QC.Qt.KeypadModifier,
                       QC.Qt.Key_Down, self._MoveFocusedThumbnail, 1, 0, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier, QC.Qt.Key_Left,
                       self._MoveFocusedThumbnail, 0, -1, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier | QC.Qt.KeypadModifier,
                       QC.Qt.Key_Left, self._MoveFocusedThumbnail, 0, -1, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier, QC.Qt.Key_Right,
                       self._MoveFocusedThumbnail, 0, 1, True)
        QP.AddShortcut(self, QC.Qt.ShiftModifier | QC.Qt.KeypadModifier,
                       QC.Qt.Key_Right, self._MoveFocusedThumbnail, 0, 1, True)
        QP.AddShortcut(self, QC.Qt.ControlModifier, QC.Qt.Key_A, self._Select,
                       ClientMedia.FileFilter(ClientMedia.FILE_FILTER_ALL))
        QP.AddShortcut(self, QC.Qt.ControlModifier, QC.Qt.Key_Space,
                       ctrl_space_callback, self)

        if HC.PLATFORM_MACOS:

            QP.AddShortcut(self, QC.Qt.NoModifier, QC.Qt.Key_Back,
                           self._Delete)
            QP.AddShortcut(self, QC.Qt.ShiftModifier, QC.Qt.Key_Back,
                           self._Undelete)
    ...
    def TIMERAnimationUpdate(self):
        ...
        for hash in hashes:
            ...
            (original_bmp, alpha_bmp, thumbnail_index, thumbnail,
             animation_started,
             num_frames_rendered) = self._thumbnails_being_faded_in[hash]

            num_frames_supposed_to_be_rendered = int(
                (loop_started - animation_started) / FRAME_DURATION)
            ...
            if expected_thumbnail != thumbnail:
                ...
                if num_frames_supposed_to_be_rendered >= NUM_FRAMES_TO_FILL_IN:
                    ...
                    self._thumbnails_being_faded_in[hash] = (
                        original_bmp, alpha_bmp, thumbnail_index, thumbnail,
                        animation_started, num_frames_rendered)
                ...
                self.widget().update(
                    QC.QRect(x, page_virtual_y + y,
                             thumbnail_span_width - thumbnail_margin,
                             thumbnail_span_height - thumbnail_margin))
...
def AddRemoveMenu(win: MediaPanel, menu, filter_counts,
                  all_specific_file_domains, has_local_and_remote):
    ...
    if file_filter_all.GetCount(win, filter_counts) > 0:
        ...
        file_filter_selected = ClientMedia.FileFilter(
            ClientMedia.FILE_FILTER_SELECTED)

        file_filter_inbox = ClientMedia.FileFilter(
            ClientMedia.FILE_FILTER_INBOX)

        file_filter_archive = ClientMedia.FileFilter(
            ClientMedia.FILE_FILTER_ARCHIVE)

        file_filter_not_selected = ClientMedia.FileFilter(
            ClientMedia.FILE_FILTER_NOT_SELECTED)

        #

        selected_count = file_filter_selected.GetCount(win, filter_counts)

        if selected_count > 0 and selected_count < file_filter_all.GetCount(
                win, filter_counts):

            ClientGUIMenus.AppendMenuItem(
                remove_menu, file_filter_selected.ToString(win, filter_counts),
                'Remove all the selected files from the current view.',
                win._Remove, file_filter_selected)
        ...
        if file_filter_all.GetCount(win, filter_counts) > 0:
            ...
            ClientGUIMenus.AppendMenuItem(
                remove_menu, file_filter_all.ToString(win, filter_counts),
                'Remove all the files from the current view.', win._Remove,
                file_filter_all)

        if file_filter_inbox.GetCount(
                win, filter_counts) > 0 and file_filter_archive.GetCount(
                    win, filter_counts) > 0:

            ClientGUIMenus.AppendSeparator(remove_menu)

            ClientGUIMenus.AppendMenuItem(
                remove_menu, file_filter_inbox.ToString(win, filter_counts),
                'Remove all the inbox files from the current view.',
                win._Remove, file_filter_inbox)

            ClientGUIMenus.AppendMenuItem(
                remove_menu, file_filter_archive.ToString(win, filter_counts),
                'Remove all the archived files from the current view.',
                win._Remove, file_filter_archive)
        ...
        if len(all_specific_file_domains) > 1:
            ...
            for file_service_key in all_specific_file_domains:
                ...
                file_filter = ClientMedia.FileFilter(
                    ClientMedia.FILE_FILTER_FILE_SERVICE, file_service_key)

                ClientGUIMenus.AppendMenuItem(
                    remove_menu, file_filter.ToString(win, filter_counts),
                    'Remove all the files that are in this file domain.',
                    win._Remove, file_filter)

        if has_local_and_remote:

            file_filter_local = ClientMedia.FileFilter(
                ClientMedia.FILE_FILTER_LOCAL)
            file_filter_remote = ClientMedia.FileFilter(
                ClientMedia.FILE_FILTER_REMOTE)

            ClientGUIMenus.AppendSeparator(remove_menu)

            ClientGUIMenus.AppendMenuItem(
                remove_menu, file_filter_local.ToString(win, filter_counts),
                'Remove all the files that are in this client.', win._Remove,
                file_filter_local)
            ClientGUIMenus.AppendMenuItem(
                remove_menu, file_filter_remote.ToString(win, filter_counts),
                'Remove all the files that are not in this client.',
                win._Remove, file_filter_remote)

        not_selected_count = file_filter_not_selected.GetCount(
            win, filter_counts)
        ...
        if not_selected_count > 0 and selected_count > 0:
            ...
            ClientGUIMenus.AppendMenuItem(
                remove_menu,
                file_filter_not_selected.ToString(win, filter_counts),
                'Remove all the not selected files from the current view.',
                win._Remove, file_filter_not_selected)

        ClientGUIMenus.AppendMenu(menu, remove_menu, 'remove')


def AddSelectMenu(win: MediaPanel, menu, filter_counts,
                  all_specific_file_domains, has_local_and_remote):
    ...
    if file_filter_all.GetCount(win, filter_counts) > 0:
        ...
        if file_filter_all.GetCount(win, filter_counts) > 0:
            ...
            ClientGUIMenus.AppendMenuItem(
                select_menu, file_filter_all.ToString(win, filter_counts),
                'Select all the files in the current view.', win._Select,
                file_filter_all)
        ...
        if file_filter_inbox.GetCount(
            ...
            ClientGUIMenus.AppendMenuItem(
                select_menu, file_filter_inbox.ToString(win, filter_counts),
                'Select all the inbox files in the current view.', win._Select,
                file_filter_inbox)

            ClientGUIMenus.AppendMenuItem(
                select_menu, file_filter_archive.ToString(win, filter_counts),
                'Select all the archived files in the current view.',
                win._Select, file_filter_archive)
        ...
        if len(all_specific_file_domains) > 1:
            ...
            for file_service_key in all_specific_file_domains:
                ...
                ClientGUIMenus.AppendMenuItem(
                    select_menu, file_filter.ToString(win, filter_counts),
                    'Select all the files in this file domain.', win._Select,
                    file_filter)
        ...
        if has_local_and_remote:
            ...
            ClientGUIMenus.AppendMenuItem(
                select_menu, file_filter_local.ToString(win, filter_counts),
                'Remove all the files that are in this client.', win._Select,
                file_filter_local)
            ClientGUIMenus.AppendMenuItem(
                select_menu, file_filter_remote.ToString(win, filter_counts),
                'Remove all the files that are not in this client.',
                win._Select, file_filter_remote)
        ...
        if selected_count > 0:
            ...
            if not_selected_count > 0:
                ...
                ClientGUIMenus.AppendMenuItem(
                    select_menu,
                    file_filter_not_selected.ToString(win, filter_counts),
                    'Swap what is and is not selected.', win._Select,
                    file_filter_not_selected)

            ClientGUIMenus.AppendSeparator(select_menu)

            ClientGUIMenus.AppendMenuItem(
                select_menu, file_filter_none.ToString(win, filter_counts),
                'Deselect everything selected.', win._Select, file_filter_none)
...
class Thumbnail(Selectable):
    ...
    def _ScaleUpThumbnailDimensions(self, thumbnail_dimensions,
                                    scale_up_dimensions):
        ...
        scale_factor = max(scale_up_width / thumb_width,
                           scale_up_height / thumb_height)
    ...
    def GetQtImage(self):
        ...
        thumbnail_hydrus_bmp = HG.client_controller.GetCache(
            'thumbnail').GetThumbnail(self)

        thumbnail_border = HG.client_controller.new_options.GetInteger(
            'thumbnail_border')

        (width, height) = ClientData.AddPaddingToDimensions(
            HC.options['thumbnail_dimensions'], thumbnail_border * 2)

        qt_image = HG.client_controller.bitmap_manager.GetQtImage(
            width, height, 24)
        ...
        painter.setBrush(
            QG.QBrush(new_options.GetColour(background_colour_type)))

        painter.drawRect(thumbnail_border, thumbnail_border,
                         width - (thumbnail_border * 2),
                         height - (thumbnail_border * 2))

        thumbnail_fill = HG.client_controller.new_options.GetBoolean(
            'thumbnail_fill')
        ...
        if thumbnail_fill:
            ...
            (offset_position,
             destination_dimensions) = self._ScaleUpThumbnailDimensions(
                 (thumb_width, thumb_height), scale_up_dimensions)

            (destination_width, destination_height) = destination_dimensions

            raw_thumbnail_qt_image = raw_thumbnail_qt_image_original.scaled(
                destination_width, destination_height, QC.Qt.IgnoreAspectRatio,
        ...
        tags = self.GetTagsManager().GetCurrentAndPending(
            CC.COMBINED_TAG_SERVICE_KEY, ClientTags.TAG_DISPLAY_SINGLE_MEDIA)

        if len(tags) > 0:

            upper_tag_summary_generator = new_options.GetTagSummaryGenerator(
                'thumbnail_top')
            lower_tag_summary_generator = new_options.GetTagSummaryGenerator(
            ...
            if self._last_tags is not None and self._last_tags == tags:
                ...
                upper_summary = upper_tag_summary_generator.GenerateSummary(
                    tags)

                lower_summary = lower_tag_summary_generator.GenerateSummary(
                    tags)
            ...
            if len(upper_summary) > 0 or len(lower_summary) > 0:
                ...
                if len(upper_summary) > 0:
                    ...
                    (text_width, text_height) = painter.fontMetrics().size(
                        QC.Qt.TextSingleLine, upper_summary).toTuple()
                ...
                if len(lower_summary) > 0:
                    ...
                    (text_width, text_height) = painter.fontMetrics().size(
                        QC.Qt.TextSingleLine, lower_summary).toTuple()
        ...
        if thumbnail_border > 0:
            ...
            painter.setBrush(
                QG.QBrush(new_options.GetColour(border_colour_type)))
            painter.setPen(QG.QPen(QC.Qt.NoPen))
            ...
            rectangles.append(
                QC.QRectF(0, height - thumbnail_border, width,
                          thumbnail_border))  # bottom
            rectangles.append(
                QC.QRectF(0, thumbnail_border, thumbnail_border,
                          side_height))  # left
            rectangles.append(
                QC.QRectF(width - thumbnail_border, thumbnail_border,
                          thumbnail_border, side_height))  # right
        ...
        ) or CC.COMBINED_LOCAL_FILE_SERVICE_KEY in locations_manager.GetDeleted(
        ):
        ...
        if self.IsCollection():
            ...
            (text_width, text_height) = painter.fontMetrics().size(
                QC.Qt.TextSingleLine, num_files_str).toTuple()

            painter.setBrush(QG.QBrush(CC.COLOUR_UNSELECTED))

            painter.setPen(QC.Qt.NoPen)

            painter.drawRect(17, height - text_height - 3, text_width + 2,
                             text_height + 2)
        ...
        service_types = [
            services_manager.GetService(service_key).GetServiceType()
            for service_key in current_to_display
        ]
        ...
        for icon_to_draw in icons_to_draw:
            ...
            painter.drawPixmap(top_left_x, thumbnail_border + ICON_MARGIN,
                               icon_to_draw)
...
class ThumbnailMediaCollection(Thumbnail, ClientMedia.MediaCollection):
    def __init__(self, file_service_key, media_results):
        ...
        ClientMedia.MediaCollection.__init__(self, file_service_key,
                                             media_results)
