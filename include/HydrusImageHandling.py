...
def Dequantize(pil_image):
    ...
    if pil_image.mode not in ('RGBA', 'RGB'):
        ...
        if pil_image.mode == 'LA' or (pil_image.mode == 'P'
...
def GenerateNumPyImageFromPILImage(pil_image):
    ...
    return numpy.fromstring(s, dtype='uint8').reshape(
        (h, w, len(s) // (w * h)))
...
def GenerateThumbnailBytesFromStaticImagePath(path, target_resolution, mime):
    ...
    if OPENCV_OK:
        ...
        thumbnail_numpy_image = ResizeNumPyImage(numpy_image,
                                                 target_resolution)

        try:

            thumbnail_bytes = GenerateThumbnailBytesNumPy(
                thumbnail_numpy_image, mime)
    ...
    thumbnail_pil_image = pil_image.resize(target_resolution,
                                           PILImage.ANTIALIAS)
...
def GetImageProperties(path, mime):
    ...
    if OPENCV_OK and mime not in PIL_ONLY_MIMETYPES:  # webp here too maybe eventually, or offload it all to ffmpeg
        ...
        ((width, height),
         num_frames) = GetResolutionAndNumFramesPIL(path, mime)
