...
def ExpandTLWIfPossible(tlw, frame_key, desired_size_delta):
    ...
    if not tlw.isMaximized() and not tlw.isFullScreen():
        ...
        (width, height) = GetSafeSize(tlw, (desired_width, desired_height),
                                      default_gravity)
...
def SaveTLWSizeAndPosition(tlw, frame_key):
    ...
    new_options.SetFrameLocation(frame_key, remember_size, remember_position,
...
def SetInitialTLWSizeAndPosition(tlw, frame_key):
    ...
    if remember_size and last_size is not None:
        ...
        (width, height) = GetSafeSize(tlw, (min_width, min_height),
                                      default_gravity)
    ...
    if remember_position and last_position is not None:
        ...
        if parent is not None:
            ...
            tlw.move(
                QP.TupleToQPoint((parent_x + CHILD_POSITION_PADDING,
                                  parent_y + CHILD_POSITION_PADDING)))

        else:

            safe_position = GetSafePosition(
                (0 + CHILD_POSITION_PADDING, 0 + CHILD_POSITION_PADDING))
...
class NewDialog(QP.Dialog):
    ...
    def keyPressEvent(self, event):
        ...
        event_from_us = current_focus is not None and ClientGUIFunctions.IsQtAncestor(
            current_focus, self)
...
class DialogThatTakesScrollablePanel(DialogThatResizes):
    def __init__(self,
        ...
        DialogThatResizes.__init__(self,
                                   parent,
                                   title,
                                   frame_key,
                                   do_not_activate=do_not_activate)
    ...
    def SetPanel(self, panel):
        ...
        if hasattr(self._panel, 'okSignal'):
            self._panel.okSignal.connect(self.EventOK)
...
class DialogNullipotent(DialogThatTakesScrollablePanel):
    ...
    def _InitialiseButtons(self):
        ...
        if self._hide_buttons:
            ...
            self._widget_event_filter.EVT_CLOSE(
                lambda ev: self.EventOK()
            )  # the close event no longer goes to the default button, since it is hidden, wew
...
class DialogEdit(DialogApplyCancel):
    def __init__(self,
        ...
        DialogApplyCancel.__init__(self,
                                   parent,
                                   title,
                                   frame_key=frame_key,
...
class DialogCustomButtonQuestion(DialogThatTakesScrollablePanel):
    def __init__(self, parent, title, frame_key='regular_center_dialog'):
        ...
        DialogThatTakesScrollablePanel.__init__(self,
                                                parent,
                                                title,
                                                frame_key=frame_key)
...
class FrameThatResizes(Frame):
    def __init__(self, parent, title, frame_key):
        ...
        self._widget_event_filter.EVT_MOVE_END(
            self.EventSizeAndPositionChanged)
        self._widget_event_filter.EVT_CLOSE(self.EventSizeAndPositionChanged)
        self._widget_event_filter.EVT_MAXIMIZE(
            self.EventSizeAndPositionChanged)
    ...
    def EventSizeAndPositionChanged(self, event):
        ...
        HG.client_controller.CallLaterQtSafe(self, 0.1, SaveTLWSizeAndPosition,
