...
class TestCheckerOptions(unittest.TestCase):
    def test_checker_options(self):
        ...
        self.assertEqual(
            ...
            'at last check, found 50 files in previous 1 day')
        self.assertEqual(
        ...
        self.assertEqual(
            callous_checker_options.GetPrettyCurrentVelocity(
                file_seed_cache, last_check_time),
            'at last check, found 0 files in previous 1 minute')
        ...
        self.assertEqual(
            ...
            'at last check, found 1 files in previous 1 day')
        ...
        self.assertEqual(
            ...
            'at last check, found 8,640 files in previous 1 day')
        self.assertEqual(
            callous_checker_options.GetPrettyCurrentVelocity(
                busy_file_seed_cache, last_check_time),
            'at last check, found 6 files in previous 1 minute')
        ...
        self.assertEqual(
            ...
            'at last check, found 10 files in previous 10 minutes')
        self.assertEqual(
            fast_checker_options.GetPrettyCurrentVelocity(
                new_thread_file_seed_cache, last_check_time),
            'at last check, found 10 files in previous 10 minutes')
...
class TestFileImportOptions(unittest.TestCase):
    def test_file_import_options(self):
        ...
        file_import_options.CheckFileIsValid(65536, HC.APPLICATION_7Z, None,
                                             None)

        self.assertTrue(
            file_import_options.ShouldPresent(CC.STATUS_SUCCESSFUL_AND_NEW,
                                              False))
        self.assertTrue(
            file_import_options.ShouldPresent(CC.STATUS_SUCCESSFUL_AND_NEW,
                                              True))
        self.assertTrue(
            file_import_options.ShouldPresent(
                CC.STATUS_SUCCESSFUL_BUT_REDUNDANT, False))
        self.assertTrue(
            file_import_options.ShouldPresent(
                CC.STATUS_SUCCESSFUL_BUT_REDUNDANT, True))

        self.assertFalse(
            file_import_options.ShouldPresent(CC.STATUS_DELETED, False))
        ...
        with self.assertRaises(HydrusExceptions.SizeException):
            ...
            file_import_options.CheckFileIsValid(65536, HC.IMAGE_JPEG, 180,
                                                 480)
        ...
        with self.assertRaises(HydrusExceptions.SizeException):
            ...
            file_import_options.CheckFileIsValid(65536, HC.IMAGE_JPEG, 3200,
                                                 480)

        with self.assertRaises(HydrusExceptions.SizeException):

            file_import_options.CheckFileIsValid(65536, HC.IMAGE_JPEG, 640,
                                                 4200)
...
class TestServiceTagImportOptions(unittest.TestCase):
    ...
    def test_application(self):
        ...
        service_tag_import_options = ClientImportOptions.ServiceTagImportOptions(
            get_tags=True,
            to_new_files=True,
            to_already_in_inbox=False,
            to_already_in_archive=False)
        ...
        self.assertEqual(
            ...
            some_tags)
        self.assertEqual(
            service_tag_import_options.GetTags(
                example_service_key, CC.STATUS_SUCCESSFUL_BUT_REDUNDANT, True,
                example_hash, some_tags), set())
        self.assertEqual(
            service_tag_import_options.GetTags(
                example_service_key, CC.STATUS_SUCCESSFUL_BUT_REDUNDANT, False,
                example_hash, some_tags), set())
        ...
        self.assertEqual(
            service_tag_import_options.GetTags(
                example_service_key, CC.STATUS_SUCCESSFUL_BUT_REDUNDANT, True,
                example_hash, some_tags), some_tags)
        self.assertEqual(
            service_tag_import_options.GetTags(
                example_service_key, CC.STATUS_SUCCESSFUL_BUT_REDUNDANT, False,
                example_hash, some_tags), set())
