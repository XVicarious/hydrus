...
def DoClick(click, panel, do_delayed_ok_afterwards=False):
    ...
    if do_delayed_ok_afterwards:
        ...
        HG.test_controller.CallLaterQtSafe(panel, 1, PressKeyOnFocusedWindow,
                                           QC.Qt.Key_Return)
...
def GenerateClick(window, pos, click_type, click_button, modifier):
    ...
    click = QG.QMouseEvent(click_type, pos, screen_pos, click_button,
...
def GetAllClickableIndices(panel):
    ...
    click = GenerateClick(panel, QC.QPointF(10, current_y),
                          QC.QEvent.MouseButtonPress, QC.Qt.LeftButton,
                          QC.Qt.NoModifier)
    ...
    def test_listbox_colour_options(self):
        def qt_code():
            ...
            try:
                ...
                panel = ClientGUIListBoxes.ListBoxTagsColourOptions(
                    frame, initial_namespace_colours)

                frame.SetPanel(panel)

                self.assertEqual(panel.GetNamespaceColours(),
                                 initial_namespace_colours)
                ...
                self.assertEqual(len(list(all_clickable_indices.keys())),
                                 len(terms))
                self.assertEqual(
                    set(all_clickable_indices.keys()),
                    set(range(len(list(all_clickable_indices.keys())))))

                #

                for (index, y) in list(all_clickable_indices.items()):

                    click = GenerateClick(panel, QC.QPointF(10, y),
                                          QC.QEvent.MouseButtonPress,
                                          QC.Qt.LeftButton, QC.Qt.NoModifier)

                    DoClick(click, panel)

                    self.assertEqual(panel.GetSelectedNamespaceColours(),
                                     dict([ordered_terms[index]]))

                #

                current_y = 5

                click = QG.QMouseEvent(QC.QEvent.MouseButtonPress,
                                       QC.QPointF(10,
                                                  current_y), QC.Qt.LeftButton,
                                       QC.Qt.LeftButton, QC.Qt.NoModifier)
                ...
                if len(list(all_clickable_indices.keys())) > 2:
                    ...
                    indices = random.sample(
                        list(all_clickable_indices.keys()),
                        len(list(all_clickable_indices.keys())) - 1)

                    for index in indices:

                        click = GenerateClick(
                            panel, QC.QPointF(10,
                                              all_clickable_indices[index]),
                            QC.QEvent.MouseButtonPress, QC.Qt.LeftButton,
                    ...
                    expected_selected_terms = [
                        ordered_terms[index] for index in indices
                    ]

                    self.assertEqual(panel.GetSelectedNamespaceColours(),
                                     dict(expected_selected_terms))

                #

                random_index = random.choice(list(
                    all_clickable_indices.keys()))
                ...
                click = GenerateClick(
                    panel, QC.QPointF(10, all_clickable_indices[random_index]),
                    QC.QEvent.MouseButtonPress, QC.Qt.LeftButton,
                    QC.Qt.NoModifier)
