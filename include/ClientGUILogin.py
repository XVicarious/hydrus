...
from . import (ClientData, ClientDefaults, ClientGUICommon, ClientGUIControls,
               ...
               ClientGUIParsing, ClientGUIScrolledPanels,
               ClientGUIScrolledPanelsEdit, ClientGUISerialisable,
               ClientGUITopLevelWindows, ClientImporting, ClientNetworking,
               ClientNetworkingBandwidth, ClientNetworkingContexts,
               ClientNetworkingDomain, ClientNetworkingJobs,
               ClientNetworkingLogin, ClientNetworkingSessions, ClientParsing,
               ClientPaths, ClientSerialisable, ClientThreading)
...
class EditLoginCredentialsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, credential_definitions, credentials):
        ...
        for credential_definition in credential_definitions:
            ...
            self._control_data.append(
                (credential_definition, control, control_st))
        ...
        gridbox = ClientGUICommon.WrapInGrid(self,
                                             rows,
                                             add_stretch_at_end=False)
...
class EditLoginCredentialDefinitionPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, credential_definition):
        ...
        for credential_type in [
                ClientNetworkingLogin.CREDENTIAL_TYPE_TEXT,
                ClientNetworkingLogin.CREDENTIAL_TYPE_PASS
        ]:

            self._credential_type.addItem(
                ClientNetworkingLogin.
                credential_type_str_lookup[credential_type], credential_type)

        string_match = credential_definition.GetStringMatch()

        self._string_match = ClientGUIControls.StringMatchButton(
            self, string_match)
    ...
    def GetValue(self):
        ...
        credential_definition = ClientNetworkingLogin.LoginCredentialDefinition(
            name=name,
...
class EditLoginsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, engine, login_scripts, domains_to_login_info):
        ...
        self._domains_and_login_info_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self)

        columns = [('domain', 20), ('login script', -1), ('access given', 36),
                   ('active?', 8), ('logged in now?', 28), ('validity', 28),
                   ('recent error/delay?', 21)]

        self._domains_and_login_info = ClientGUIListCtrl.BetterListCtrl(
            self._domains_and_login_info_panel,
            'domains_to_login_info',
            8,
            36,
            columns,
            self._ConvertDomainAndLoginInfoListCtrlTuples,
            use_simple_delete=True,
            activation_callback=self._EditCredentials)

        self._domains_and_login_info_panel.SetListCtrl(
            self._domains_and_login_info)
        ...
        self._domains_and_login_info_panel.AddButton(
            'edit credentials',
            self._EditCredentials,
            enabled_check_func=self._CanEditCreds)
        self._domains_and_login_info_panel.AddButton(
            'change login script',
            self._EditLoginScript,
            enabled_only_on_selection=True)
        self._domains_and_login_info_panel.AddButton(
            'flip active', self._FlipActive, enabled_only_on_selection=True)
        self._domains_and_login_info_panel.AddSeparator()
        self._domains_and_login_info_panel.AddButton(
            'scrub invalidity',
            self._ScrubInvalidity,
            enabled_check_func=self._CanScrubInvalidity)
        self._domains_and_login_info_panel.AddButton(
            'scrub delays',
            self._ScrubDelays,
        ...
        self._domains_and_login_info_panel.AddButton(
            'do login now', self._DoLogin, enabled_check_func=self._CanDoLogin)
        self._domains_and_login_info_panel.AddButton(
            'reset login (delete cookies)',
            self._ClearSessions,
            enabled_only_on_selection=True)
        ...
        for (login_domain,
             (login_script_key_and_name, credentials, login_access_type,
              login_access_text, active, validity, validity_error_text,
              no_work_until,
              no_work_until_reason)) in list(domains_to_login_info.items()):

            credentials_tuple = tuple(credentials.items())

            domain_and_login_info = (login_domain, login_script_key_and_name,
                                     credentials_tuple, login_access_type,
                                     login_access_text, active, validity,
                                     validity_error_text, no_work_until,
        ...
        QP.AddToLayout(vbox, self._domains_and_login_info_panel,
    ...
    def _Add(self):
        ...
        if len(self._login_scripts) == 0:
            ...
            QW.QMessageBox.critical(
                self, 'Error',
                'You have no login scripts, so you cannot add a new login!')

            return

        choice_tuples = [(login_script.GetName(), login_script)
                         for login_script in self._login_scripts]

        try:

            login_script = ClientGUIDialogsQuick.SelectFromList(
                self, 'select the login script to use', choice_tuples)
        ...
        domains_in_use = {
            login_domain
            for (login_domain, login_script_key_and_name, credentials_tuple,
                 login_access_type, login_access_text, active, validity,
                 validity_error_text, no_work_until, no_work_until_reason
                 ) in self._domains_and_login_info.GetData()
        }
        ...
        if len(available_examples) > 0:
            ...
            choice_tuples = [(login_domain, login_domain)
                             for login_domain in available_examples]

            choice_tuples.append(('use other domain', None))

            try:

                login_domain = ClientGUIDialogsQuick.SelectFromList(
                    self,
            ...
            if login_domain is not None:
                ...
                (login_access_type, login_access_text
                 ) = login_script.GetExampleDomainInfo(login_domain)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    if login_domain in domains_in_use:
                        ...
                        QW.QMessageBox.warning(
                            self, 'Warning', 'That domain is already in use!')

                        return

                    a_types = [
                        ClientNetworkingLogin.LOGIN_ACCESS_TYPE_EVERYTHING,
                        ClientNetworkingLogin.LOGIN_ACCESS_TYPE_NSFW,
                        ClientNetworkingLogin.LOGIN_ACCESS_TYPE_SPECIAL,
                        ClientNetworkingLogin.LOGIN_ACCESS_TYPE_USER_PREFS_ONLY
                    ]

                    choice_tuples = [
                        (ClientNetworkingLogin.
                         login_access_type_str_lookup[a_type], a_type)
                        for a_type in a_types
                    ]

                    try:

                        login_access_type = ClientGUIDialogsQuick.SelectFromList(
                            self,
                            'select what type of access the login gives to this domain',
                            choice_tuples,
                            sort_tuples=False)
                    ...
                    login_access_text = ClientNetworkingLogin.login_access_type_default_description_lookup[
                        login_access_type]

                    with ClientGUIDialogs.DialogTextEntry(
                            self,
                            'edit the access description, if needed',
                            default=login_access_text,
                            allow_blank=False) as dlg:
        ...
        if len(credential_definitions) > 0:
            ...
            with ClientGUITopLevelWindows.DialogEdit(self,
                ...
                panel = EditLoginCredentialsPanel(dlg, credential_definitions,
                                                  credentials)
        ...
        try:
            ...
            creds_are_good = len(credentials) == 0 or True in (
                value != '' for value in list(credentials.values()))
    ...
    def _CanDoLogin(self):
        ...
        domain_and_login_infos = self._domains_and_login_info.GetData(
            only_selected=True)

        for domain_and_login_info in domain_and_login_infos:

            (login_domain, login_script_key_and_name, credentials_tuple,
             login_access_type, login_access_text, active, validity,
             validity_error_text, no_work_until,
             no_work_until_reason) = domain_and_login_info
            ...
            try:
                ...
                network_context = ClientNetworkingContexts.NetworkContext(
                    ...
                    context_data=login_domain)

                logged_in = login_script.IsLoggedIn(self._engine,
                                                    network_context)
    ...
    def _CanScrubDelays(self):
        ...
        for domain_and_login_info in domain_and_login_infos:
            ...
            if not HydrusData.TimeHasPassed(
                    no_work_until) or no_work_until_reason != '':
    ...
    def _ConvertDomainAndLoginInfoListCtrlTuples(self, domain_and_login_info):
        ...
        try:
            ...
            if logged_in:
                ...
                login_expiry = login_script.GetLoginExpiry(
                    self._engine, network_context)
        ...
        access = ClientNetworkingLogin.login_access_type_str_lookup[
            login_access_type] + ' - ' + login_access_text
        ...
        if HydrusData.TimeHasPassed(no_work_until):
            ...
            pretty_no_work_until = HydrusData.ConvertTimestampToPrettyExpires(
        ...
        if logged_in:
            ...
            if login_expiry is None:
                ...
                pretty_login_expiry = HydrusData.ConvertTimestampToPrettyExpires(
                    login_expiry)
        ...
        display_tuple = (pretty_login_domain, pretty_login_script,
                         pretty_access, pretty_active, pretty_logged_in,
                         pretty_validity, pretty_no_work_until)
        sort_tuple = (login_domain, sort_login_script, access, sort_active,
                      sort_logged_in, sort_validity, no_work_until)
        ...
        for domain_and_login_info in domain_and_login_infos:
            ...
            try:
                ...
                QW.QMessageBox.information(
                    self, 'Information',
                    'Could not find a login script for "' + login_domain +
                    '"! Please re-add the login script in the other dialog or update the entry here to a new one!'
                )
            ...
            if len(credential_definitions) > 0:
                ...
                with ClientGUITopLevelWindows.DialogEdit(self,
                    ...
                    panel = EditLoginCredentialsPanel(dlg,
            ...
            edited_domain_and_login_info = (login_domain,
                                            login_script_key_and_name,
                                            credentials_tuple,
                                            login_access_type,
                                            login_access_text, active,
                                            validity, validity_error_text,
                                            no_work_until,
                                            no_work_until_reason)

            self._domains_and_login_info.DeleteDatas((domain_and_login_info, ))
            self._domains_and_login_info.AddDatas(
                (edited_domain_and_login_info, ))
    ...
    def _EditLoginScript(self):
        ...
        for domain_and_login_info in domain_and_login_infos:
            ...
            try:
                ...
                current_login_script = self._GetLoginScript(
                    login_script_key_and_name)
            ...
            matching_potential_login_scripts = [
                login_script for login_script in potential_login_scripts
                if login_domain in login_script.GetExampleDomains()
            ]
            unmatching_potential_login_scripts = [
                login_script for login_script in potential_login_scripts
                if login_domain not in login_script.GetExampleDomains()
            ]

            choice_tuples = [
                (login_script.GetName(), login_script)
                for login_script in matching_potential_login_scripts
            ]

            if len(matching_potential_login_scripts) > 0 and len(
                    unmatching_potential_login_scripts) > 0:

                choice_tuples.append(('------', None))

            choice_tuples.extend([
                (login_script.GetName(), login_script)
                for login_script in unmatching_potential_login_scripts
            ])
            ...
            try:
                ...
                login_script = ClientGUIDialogsQuick.SelectFromList(
                    self,
                    'select the login script to use',
                    choice_tuples,
                    value_to_select=current_login_script,
                    sort_tuples=False)
            ...
            try:
                ...
                choice_tuples = [(
                    ClientNetworkingLogin.login_access_type_str_lookup[a_type],
                    a_type) for a_type in a_types]
    ...
    def _FlipActive(self):
        ...
        for domain_and_login_info in domain_and_login_infos:
            ...
            flipped_domain_and_login_info = (login_domain,
                                             login_script_key_and_name,
                                             credentials_tuple,
                                             login_access_type,
                                             login_access_text, active,
    ...
    def _ScrubDelays(self):
        ...
        for domain_and_login_info in domain_and_login_infos:
            ...
            scrubbed_domain_and_login_info = (login_domain,
                                              login_script_key_and_name,
                                              credentials_tuple,
                                              login_access_type,
                                              login_access_text, active,
        ...
        for domain_and_login_info in domain_and_login_infos:
            ...
            try:
                ...
                try:
                    ...
                    login_script = self._GetLoginScript(
                        login_script_key_and_name)
    ...
    def GetValue(self):
        ...
        for (login_domain, login_script_key_and_name, credentials_tuple,
             login_access_type, login_access_text, active, validity,
             validity_error_text, no_work_until,
             no_work_until_reason) in self._domains_and_login_info.GetData():

            credentials = dict(credentials_tuple)

            domains_to_login_info[login_domain] = (login_script_key_and_name,
                                                   credentials,
                                                   login_access_type,
                                                   login_access_text, active,
                                                   validity,
...
def GenerateTestNetworkJobPresentationContextFactory(window,
                                                     network_job_control):
    def network_job_presentation_context_factory(network_job):
        ...
        return ClientImporting.NetworkJobPresentationContext(
            enter_call, exit_call)
...
class ReviewTestResultPanel(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, test_result):
        ...
        (name, url, body, self._downloaded_data, new_temp_strings,
         new_cookie_strings, result) = test_result
        ...
        min_size = ClientGUIFunctions.ConvertTextToPixels(
            self._data_preview, (64, 8))

        QP.SetMinClientSize(self._data_preview, min_size)

        self._data_copy_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.copy, self._CopyData)
        self._data_copy_button.setToolTip(
...
class EditLoginScriptPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, login_script):
        ...
        page_func = HydrusData.Call(
            ...
            os.path.join(HC.HELP_DIR, 'downloader_login.html'))

        menu_items.append(
            ('normal', 'open the login scripts help',
             'Open the help page for login scripts in your web browser.',
             page_func))
        ...
        credential_definitions_box_panel = ClientGUICommon.StaticBox(
            self, 'credential definitions')

        credential_definitions_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            credential_definitions_box_panel)

        columns = [('name', -1), ('type', 10), ('value', 16)]

        self._credential_definitions = ClientGUIListCtrl.BetterListCtrl(
            credential_definitions_panel,
            'credential_definitions',
            4,
            16,
            columns,
            self._ConvertCredentialDefinitionToListCtrlTuples,
            use_simple_delete=True,
            activation_callback=self._EditCredentialDefinitions)

        credential_definitions_panel.SetListCtrl(self._credential_definitions)

        credential_definitions_panel.AddButton('add',
                                               self._AddCredentialDefinition)
        credential_definitions_panel.AddButton('edit',
                                               self._EditCredentialDefinitions,
                                               enabled_only_on_selection=True)
        ...
        self._login_steps = ClientGUIListBoxes.QueueListBox(
            login_steps_box_panel,
            5,
            self._ConvertLoginStepToListBoxString,
            add_callable=self._AddLoginStep,
            edit_callable=self._EditLoginStep)

        #

        required_cookies_info_box_panel = ClientGUICommon.StaticBox(
            self, 'cookies required to consider session logged in')

        self._required_cookies_info = ClientGUIControls.StringMatchToStringMatchDictControl(
            required_cookies_info_box_panel,
            login_script.GetRequiredCookiesInfo(),
            min_height=4,
            key_name='cookie name')

        #

        example_domains_info_box_panel = ClientGUICommon.StaticBox(
            self, 'example domains')

        example_domains_info_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            example_domains_info_box_panel)

        columns = [('domain', -1), ('access type', 14), ('description', 40)]

        self._example_domains_info = ClientGUIListCtrl.BetterListCtrl(
            example_domains_info_panel,
            'example_domains_info',
            6,
            16,
            columns,
            self._ConvertExampleDomainInfoToListCtrlTuples,
            use_simple_delete=True,
            activation_callback=self._EditExampleDomainsInfo)

        example_domains_info_panel.SetListCtrl(self._example_domains_info)

        example_domains_info_panel.AddButton('add',
                                             self._AddExampleDomainsInfo)
        example_domains_info_panel.AddButton('edit',
                                             self._EditExampleDomainsInfo,
                                             enabled_only_on_selection=True)
        ...
        self._test_button = ClientGUICommon.BetterButton(
            test_panel, 'run test', self._DoTest)
        ...
        self._test_listctrl = ClientGUIListCtrl.BetterListCtrl(
            test_listctrl_panel,
            'test_login_script_results',
            6,
            20,
            columns,
            self._ConvertTestResultToListCtrlTuples,
            activation_callback=self._ReviewTestResult)

        test_listctrl_panel.SetListCtrl(self._test_listctrl)

        test_listctrl_panel.AddButton('review',
                                      self._ReviewTestResult,
                                      enabled_only_on_selection=True)
        ...
        self._credential_definitions.SetData(
            login_script.GetCredentialDefinitions())
        self._login_steps.AddDatas(login_script.GetLoginSteps())
        self._example_domains_info.SetData(
            login_script.GetExampleDomainsInfo())

        #

        credential_definitions_box_panel.Add(credential_definitions_panel,
                                             CC.FLAGS_EXPAND_BOTH_WAYS)
        login_steps_box_panel.Add(self._login_steps, CC.FLAGS_EXPAND_BOTH_WAYS)
        required_cookies_info_box_panel.Add(self._required_cookies_info,
                                            CC.FLAGS_EXPAND_BOTH_WAYS)
        example_domains_info_box_panel.Add(example_domains_info_panel,
                                           CC.FLAGS_EXPAND_BOTH_WAYS)

        test_panel.Add(self._test_button, CC.FLAGS_EXPAND_PERPENDICULAR)
        test_panel.Add(self._test_network_job_control,
        ...
        QP.AddToLayout(vbox, credential_definitions_box_panel,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(vbox, login_steps_box_panel, CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(vbox, required_cookies_info_box_panel,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(vbox, example_domains_info_box_panel,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
    ...
    def _AddCredentialDefinition(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
            ...
            panel = EditLoginCredentialDefinitionPanel(
                dlg_edit, new_credential_definition)
            ...
            if dlg_edit.exec() == QW.QDialog.Accepted:
                ...
                HydrusSerialisable.SetNonDupeName(
                    new_credential_definition,
                    self._GetExistingCredentialDefinitionNames())

                self._credential_definitions.AddDatas(
                    (new_credential_definition, ))
    ...
    def _AddExampleDomainsInfo(self):
        ...
        (domain, access_type, access_text) = (
            'example.com', ClientNetworkingLogin.LOGIN_ACCESS_TYPE_NSFW,
            ClientNetworkingLogin.login_access_type_default_description_lookup[
                ClientNetworkingLogin.LOGIN_ACCESS_TYPE_NSFW])
        ...
        try:
            ...
            new_access_type = ClientGUIDialogsQuick.SelectFromList(
                self,
                'select what type of access the login gives to this domain',
                choice_tuples,
                value_to_select=access_type,
                sort_tuples=False)
        ...
        if new_access_type != access_type:
            ...
            access_text = ClientNetworkingLogin.login_access_type_default_description_lookup[
                access_type]
    ...
    def _ConvertCredentialDefinitionToListCtrlTuples(self,
                                                     credential_definition):

        name = credential_definition.GetName()
        credential_type = credential_definition.GetType()

        type_string = ClientNetworkingLogin.credential_type_str_lookup[
            credential_type]
    ...
    def _ConvertExampleDomainInfoToListCtrlTuples(self, example_domain_info):
        ...
        pretty_access_type = ClientNetworkingLogin.login_access_type_str_lookup[
            access_type]
    ...
    def _ConvertTestResultToListCtrlTuples(self, test_result):
        ...
        (name, url, body, downloaded_data, new_temp_strings,
         new_cookie_strings, result) = test_result
    ...
    def _EditCredentialDefinitions(self):
        ...
        for credential_definition in credential_definitions:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                    self, 'edit login script',
                    frame_key='deeply_nested_dialog') as dlg:

                panel = EditLoginCredentialDefinitionPanel(
                    dlg, credential_definition)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    self._credential_definitions.DeleteDatas(
                        (credential_definition, ))

                    HydrusSerialisable.SetNonDupeName(
                        edited_credential_definition,
                        self._GetExistingCredentialDefinitionNames())
    ...
    def _DoTest(self):
        ...
        def do_it(login_script, domain, credentials,
            ...
            try:
                ...
                domain_manager = HG.client_controller.network_engine.domain_manager.Duplicate(
                )  # keep custom headers from current domain stuff
                login_manager = ClientNetworkingLogin.NetworkLoginManager()

                network_engine = ClientNetworking.NetworkEngine(
                    HG.client_controller, bandwidth_manager, session_manager,
                    domain_manager, login_manager)

                HG.client_controller.CallToThreadLongRunning(
                    network_engine.MainLoop)

                network_context = ClientNetworkingContexts.NetworkContext.STATICGenerateForDomain(
                    domain)

                login_result = login_script.Start(
                    network_engine,
                    network_context,
                    credentials,
                    network_job_presentation_context_factory=
                    network_job_presentation_context_factory,
                    test_result_callable=qt_add_result)
        ...
        if self._currently_testing:
            ...
            QW.QMessageBox.warning(
                self, 'Warning',
                'Currently testing already! Please cancel current job!')
        ...
        network_job_presentation_context_factory = GenerateTestNetworkJobPresentationContextFactory(
            self, self._test_network_job_control)

        self._currently_testing = True

        HG.client_controller.CallToThread(
            do_it, login_script, self._test_domain, self._test_credentials,
            network_job_presentation_context_factory)

    def _EditExampleDomainsInfo(self):

        selected_example_domains_info = self._example_domains_info.GetData(
            only_selected=True)
    ...
    def _GetExistingCredentialDefinitionNames(self):
        ...
        return {
            credential_definition.GetName()
            for credential_definition in
            self._credential_definitions.GetData()
        }
    ...
    def _GetExistingDomains(self):
        ...
        return {
            domain
            for (domain, access_type,
                 access_text) in self._example_domains_info.GetData()
        }
    ...
    def GetValue(self):
        ...
        login_script = ClientNetworkingLogin.LoginScriptDomain(
            name=name,
            login_script_key=login_script_key,
            required_cookies_info=required_cookies_info,
            credential_definitions=credential_definitions,
            login_steps=login_steps,
            example_domains_info=example_domains_info)
        ...
        try:
            ...
            result = ClientGUIDialogsQuick.GetYesNo(self,
                                                    message,
                                                    yes_label='ok as invalid',
                                                    no_label='go back')

            if result != QW.QDialog.Accepted:

                raise HydrusExceptions.VetoException(
                    'The ok event has been cancelled!')
...
class EditLoginScriptsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, login_scripts):
        ...
        self._login_scripts = ClientGUIListCtrl.BetterListCtrl(
            login_scripts_panel,
            'login_scripts',
            20,
            24,
            columns,
            self._ConvertLoginScriptToListCtrlTuples,
            use_simple_delete=True,
        ...
        login_scripts_panel.AddButton('edit',
                                      self._Edit,
                                      enabled_only_on_selection=True)
        login_scripts_panel.AddDeleteButton()
        login_scripts_panel.AddSeparator()
        login_scripts_panel.AddImportExportButtons(
            (ClientNetworkingLogin.LoginScriptDomain, ), self._AddLoginScript)
        login_scripts_panel.AddSeparator()
        login_scripts_panel.AddDefaultsButton(
            ClientDefaults.GetDefaultLoginScripts, self._AddLoginScript)
    ...
    def _AddLoginScript(self, login_script):
        ...
        HydrusSerialisable.SetNonDupeName(login_script,
        ...
        for login_script in login_scripts:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HydrusSerialisable.SetNonDupeName(edited_login_script,
                                                      self._GetExistingNames())
    ...
    def _GetExistingNames(self):
        ...
        names = {
            login_script.GetName()
            for login_script in self._login_scripts.GetData()
        }
...
class EditLoginStepPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, login_step):
        ...
        (scheme, method, subdomain, path, required_credentials, static_args,
         temp_args, required_cookies_info,
         content_parsers) = login_step.ToTuple()
        ...
        self._subdomain = ClientGUICommon.NoneableTextCtrl(self,
                                                           none_phrase='none')

        self._path = QW.QLineEdit(self)

        required_credentials_panel = ClientGUICommon.StaticBox(
            self, 'credentials to send')

        self._required_credentials = ClientGUIControls.StringToStringDictControl(
            required_credentials_panel,
        ...
        static_args_panel = ClientGUICommon.StaticBox(
            self, 'static variables to send')
        ...
        temp_args_panel = ClientGUICommon.StaticBox(
            self, 'temporary variables to send')

        self._temp_args = ClientGUIControls.StringToStringDictControl(
            temp_args_panel,
            temp_args,
            min_height=4,
            key_name='temp variable name',
            value_name='parameter name')

        #

        required_cookies_info_box_panel = ClientGUICommon.StaticBox(
            self, 'cookies required to consider step successful')
        ...
        content_parsers_panel = ClientGUICommon.StaticBox(
            self, 'content parsers')

        test_context_callable = lambda: ({}, '')

        permitted_content_types = [
            HC.CONTENT_TYPE_VARIABLE, HC.CONTENT_TYPE_VETO
        ]

        self._content_parsers = ClientGUIParsing.EditContentParsersPanel(
            content_parsers_panel, test_context_callable,
            permitted_content_types)
        ...
        required_credentials_panel.Add(self._required_credentials,
                                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        required_cookies_info_box_panel.Add(self._required_cookies_info,
                                            CC.FLAGS_EXPAND_BOTH_WAYS)
        content_parsers_panel.Add(self._content_parsers,
                                  CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        rows.append(
            ('subdomain (replaces www, if present): ', self._subdomain))
        ...
        QP.AddToLayout(vbox, gridbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, required_credentials_panel,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
    ...
    def GetValue(self):
        ...
        login_step = ClientNetworkingLogin.LoginStep(name=name,
                                                     scheme=scheme,
                                                     method=method,
                                                     subdomain=subdomain,
                                                     path=path)

        login_step.SetComplicatedVariables(required_credentials, static_args,
                                           temp_args, required_cookies_info,
                                           content_parsers)
