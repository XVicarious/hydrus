...
def ConvertDomainIntoAllApplicableDomains(domain, discard_www=True):
    ...
    while domain.count('.') > 0:
        ...
        domain = '.'.join(
            domain.split('.')[1:]
        )  # i.e. strip off the leftmost subdomain maps.google.com -> google.com
...
def ConvertQueryDictToText(query_dict, param_order=None):
    ...
    if param_order is None:
        ...
        param_pairs = [(key, query_dict[key]) for key in param_order
                       if key in query_dict]

    query_text = '&'.join(
        (str(key) + '=' + str(value) for (key, value) in param_pairs))
    ...
    for pair in pairs:
        ...
        if len(result) == 2:
            ...
            try:
                ...
                if True not in (bad_char in unquoted_key
            ...
            try:
                ...
                if True not in (bad_char in unquoted_value
                                for bad_char in bad_chars):
...
def CookieDomainMatches(cookie, search_domain):
    ...
    valid_subdomain = cookie_domain.startswith('.') and search_domain.endswith(
        cookie_domain)
...
def GetSearchURLs(url):
    ...
    for url in list(search_urls):
        ...
        r = urllib.parse.ParseResult(scheme, netloc, path, params, query,
                                     fragment)
...
class NetworkDomainManager(HydrusSerialisable.SerialisableBase):
    ...
    def __init__(self):
        ...
        self._network_contexts_to_custom_header_dicts = collections.defaultdict(
            dict)
        ...
        self._second_level_domains_to_url_classes = collections.defaultdict(
            list)
    ...
    def _GetDefaultTagImportOptionsForURL(self, url):
        ...
        if url_class is None or url_class.GetURLType() not in (
                HC.URL_TYPE_POST, HC.URL_TYPE_WATCHABLE):
        ...
        if url_class_key in self._url_class_keys_to_default_tag_import_options:
            ...
            return self._url_class_keys_to_default_tag_import_options[
                url_class_key]
    ...
    def _GetNormalisedAPIURLClassAndURL(self, url):
        ...
        while api_url_class.UsesAPIURL():
            ...
            if api_url_class is None:
                ...
                raise HydrusExceptions.URLClassException(
                    'Could not find an API URL Class for ' + api_url +
                    ' URL, which originally came from ' + url + '!')
            ...
            if api_url_class in seen_url_classes:
                ...
                if loop_size == 1:
                    ...
                    message = 'Could not find an API URL Class for ' + url + ' as it and its API url classes linked in a loop of size ' + HydrusData.ToHumanInt(
                        loop_size) + '!'
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_gug_keys_to_display = [
            gug_key.hex() for gug_key in self._gug_keys_to_display
        ]

        serialisable_url_classes = self._url_classes.GetSerialisableTuple()
        serialisable_url_class_keys_to_display = [
            url_class_key.hex()
            for url_class_key in self._url_class_keys_to_display
        ]
        ...
        serialisable_url_class_keys_to_default_tag_import_options = [
            (url_class_key.hex(), tag_import_options.GetSerialisableTuple())
            for (url_class_key, tag_import_options) in list(
                self._url_class_keys_to_default_tag_import_options.items())
        ]

        serialisable_default_tag_import_options_tuple = (
            serialisable_file_post_default_tag_import_options,
            serialisable_watchable_default_tag_import_options,
            serialisable_url_class_keys_to_default_tag_import_options)

        serialisable_parsers = self._parsers.GetSerialisableTuple()
        serialisable_network_contexts_to_custom_header_dicts = [
            (network_context.GetSerialisableTuple(),
             list(custom_header_dict.items()))
            for (network_context, custom_header_dict) in list(
                self._network_contexts_to_custom_header_dicts.items())
        ]

        return (serialisable_gugs, serialisable_gug_keys_to_display,
                serialisable_url_classes,
                serialisable_url_class_keys_to_display,
                serialisable_url_class_keys_to_parser_keys,
                serialisable_default_tag_import_options_tuple,
                serialisable_parsers,
                serialisable_network_contexts_to_custom_header_dicts)
    ...
    def _GetURLToFetchAndParser(self, url):
        ...
        try:
            ...
            (parser_url_class,
             parser_url) = self._GetNormalisedAPIURLClassAndURL(url)

        except HydrusExceptions.URLClassException as e:

            raise HydrusExceptions.URLClassException(
                'Could not find a parser for ' + url + '!' + os.linesep * 2 +
                str(e))
        ...
        raise HydrusExceptions.URLClassException(
            'Could not find a parser for ' + parser_url_class.GetName() +
            ' URL Class!')

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_gugs, serialisable_gug_keys_to_display,
         serialisable_url_classes, serialisable_url_class_keys_to_display,
         serialisable_url_class_keys_to_parser_keys,
         serialisable_default_tag_import_options_tuple, serialisable_parsers,
         serialisable_network_contexts_to_custom_header_dicts
         ) = serialisable_info

        self._gugs = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_gugs)

        self._gug_keys_to_display = {
            bytes.fromhex(serialisable_gug_key)
            for serialisable_gug_key in serialisable_gug_keys_to_display
        }

        self._url_classes = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_url_classes)

        self._url_class_keys_to_display = {
            bytes.fromhex(serialisable_url_class_key)
            for serialisable_url_class_key in
            serialisable_url_class_keys_to_display
        }
        self._url_class_keys_to_parser_keys = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_url_class_keys_to_parser_keys)
        ...
        self._file_post_default_tag_import_options = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_file_post_default_tag_import_options)
        self._watchable_default_tag_import_options = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_watchable_default_tag_import_options)
        ...
        self._url_class_keys_to_default_tag_import_options = {
            ...
            for (serialisable_url_class_key, serialisable_tag_import_options
                 ) in serialisable_url_class_keys_to_default_tag_import_options
        }

        self._parsers = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_parsers)

        self._network_contexts_to_custom_header_dicts = collections.defaultdict(
            dict)

        for (serialisable_network_context, custom_header_dict_items
            ...
            self._network_contexts_to_custom_header_dicts[
                network_context] = custom_header_dict
    ...
    def _RecalcCache(self):
        ...
        for url_classes in self._second_level_domains_to_url_classes.values():
            ...
            NetworkDomainManager.STATICSortURLClassesDescendingComplexity(
                url_classes)
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            url_classes = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_url_classes)
            ...
            serialisable_url_class_names_to_display = list(
                url_class_names_to_display.items())
            ...
            new_serialisable_info = (
                serialisable_url_classes,
                serialisable_url_class_names_to_display,
                serialisable_url_class_names_to_page_parser_keys,
                serialisable_url_class_names_to_gallery_parser_keys,
                serialisable_network_contexts_to_custom_header_dicts)
        ...
        if version == 2:
            ...
            (serialisable_url_classes, serialisable_url_class_names_to_display,
             serialisable_url_class_names_to_page_parser_keys,
             serialisable_url_class_names_to_gallery_parser_keys,
             serialisable_network_contexts_to_custom_header_dicts
             ) = old_serialisable_info
            ...
            url_class_names_to_display = dict(
                serialisable_url_class_names_to_display)

            url_class_keys_to_display = []

            url_class_names_to_gallery_parser_keys = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_url_class_names_to_gallery_parser_keys)
            url_class_names_to_page_parser_keys = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_url_class_names_to_page_parser_keys)
            ...
            for url_class in url_classes:
                ...
                if name in url_class_names_to_display and url_class_names_to_display[
                        name]:

                    url_class_keys_to_display.append(url_class_key)

            serialisable_url_classes = url_classes.GetSerialisableTuple(
            )  # added random key this week, so save these changes back again!

            serialisable_url_class_keys_to_display = [
                url_class_key.hex()
                for url_class_key in url_class_keys_to_display
            ]
        ...
        if version == 3:
            ...
            serialisable_url_class_keys_to_default_tag_import_options = [
                (url_class_key.hex(),
                 tag_import_options.GetSerialisableTuple())
                for (url_class_key, tag_import_options) in list(
                    self._url_class_keys_to_default_tag_import_options.items())
            ]
        ...
        if version == 4:
            ...
            new_serialisable_info = (
                serialisable_gugs, serialisable_url_classes,
                serialisable_url_class_keys_to_display,
                serialisable_url_class_keys_to_parser_keys,
                serialisable_default_tag_import_options_tuple,
                serialisable_parsing_parsers,
        ...
        if version == 5:
            ...
            gugs = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_gugs)

            gug_keys_to_display = [
                gug.GetGUGKey() for gug in gugs
                if 'ugoira' not in gug.GetName()
            ]

            serialisable_gug_keys_to_display = [
                gug_key.hex() for gug_key in gug_keys_to_display
            ]
    ...
    def AlreadyHaveExactlyTheseHeaders(self, network_context, headers_list):
        ...
        with self._lock:
            ...
            if network_context in self._network_contexts_to_custom_header_dicts:
                ...
                custom_headers_dict = self._network_contexts_to_custom_header_dicts[
                    network_context]
                ...
                for (key, value, reason) in headers_list:
                    ...
                    (existing_value, existing_approved,
                     existing_reason) = custom_headers_dict[key]
    ...
    def AlreadyHaveExactlyThisParser(self, new_parser):
        ...
        with self._lock:
            ...
            dupe_parsers = [(parser.Duplicate(), parser)
                            for parser in self._parsers]
            ...
            for (dupe_parser, parser) in dupe_parsers:
                ...
                dupe_parser.SetExampleParsingContext(
                    new_example_parsing_context)
    ...
    def AlreadyHaveExactlyThisURLClass(self, new_url_class):
        ...
        with self._lock:
            ...
            dupe_url_classes = [
                url_class.Duplicate() for url_class in self._url_classes
            ]
    ...
    def AutoAddDomainMetadatas(self, domain_metadatas, approved=False):
        ...
        for domain_metadata in domain_metadatas:
            ...
            with self._lock:
                ...
                custom_headers_dict = {
                    key: (value, approved, reason)
                    for (key, value, reason) in headers_list
                }

                self._network_contexts_to_custom_header_dicts[
                    network_context] = custom_headers_dict

    def AutoAddURLClassesAndParsers(self, new_url_classes, dupe_url_classes,
                                    new_parsers):
        ...
        with self._lock:
            ...
            for existing_url_class in self._url_classes:
                ...
                for new_url_class in new_url_classes:
                    ...
                    if new_url_class.Matches(existing_url_class.GetExampleURL(
                    )) and existing_url_class.Matches(
                            new_url_class.GetExampleURL()):
            ...
            for renamee in renamees:
                ...
                existing_names = [
                    url_class.GetName() for url_class in self._url_classes
                    if url_class != renamee
                ]
        ...
        with self._lock:
            ...
            for dupe_url_class in dupe_url_classes:
                ...
                actual_existing_dupe_url_class = self._GetURLClass(
                    dupe_url_class.GetExampleURL())

                if actual_existing_dupe_url_class is not None:

                    url_classes_to_link.append(actual_existing_dupe_url_class)

            new_url_class_keys_to_parser_keys = NetworkDomainManager.STATICLinkURLClassesAndParsers(
                url_classes_to_link, new_parsers, {})

            self._url_class_keys_to_parser_keys.update(
    ...
    def DeleteGUGs(self, deletee_names):
        ...
        with self._lock:
            ...
            gugs = [
                gug for gug in self._gugs if gug.GetName() not in deletee_names
            ]
    ...
    def DeleteURLClasses(self, deletee_names):
        ...
        with self._lock:
            ...
            url_classes = [
                url_class for url_class in self._url_classes
                if url_class.GetName() not in deletee_names
            ]
        ...
        with self._lock:
            ...
            for network_context in network_contexts:
                ...
                if network_context in self._network_contexts_to_custom_header_dicts:
                    ...
                    custom_header_dict = self._network_contexts_to_custom_header_dicts[
                        network_context]
                    ...
                    for (key, (value, approved,
                        ...
                        if approved == VALID_UNKNOWN:
                            ...
                            header_tuples.append(
                                (network_context, key, value, reason))
    ...
    def GetDefaultGUGKeyAndName(self):
        ...
        with self._lock:
            ...
            gug_key = HG.client_controller.new_options.GetKey(
                'default_gug_key')
            gug_name = HG.client_controller.new_options.GetString(
                'default_gug_name')
    ...
    def GetDefaultTagImportOptions(self):
        ...
        with self._lock:
            ...
            return (self._file_post_default_tag_import_options,
                    self._watchable_default_tag_import_options,
                    self._url_class_keys_to_default_tag_import_options)
    ...
    def GetURLToFetchAndParser(self, url):
        ...
        with self._lock:
            ...
            if HG.network_report_mode:
                ...
                HydrusData.ShowText(
                    'request for URL to fetch and parser: {} ({}) -> {} ({}): {}'
                    .format(url, url_name, url_to_fetch, url_to_fetch_name,
                            parser.GetName()))
    ...
    def HasCustomHeaders(self, network_context):
        ...
        with self._lock:
            ...
            return network_context in self._network_contexts_to_custom_header_dicts and len(
    ...
    def IsValid(self, network_contexts):
        ...
        for network_context in network_contexts:
            ...
            if network_context in self._network_contexts_to_custom_header_dicts:
                ...
                for (value, approved,
                     reason) in list(custom_header_dict.values()):
    ...
    def NormaliseURL(self, url):
        ...
        with self._lock:
            ...
            if url_class is None:
                ...
                r = urllib.parse.ParseResult(scheme, netloc, path, params,
                                             query, fragment)
    ...
    def OverwriteDefaultGUGs(self, gug_names):
        ...
        with self._lock:
            ...
            new_gugs = [
                gug for gug in existing_gugs if gug.GetName() not in gug_names
            ]
            new_gugs.extend(
                [gug for gug in default_gugs if gug.GetName() in gug_names])
    ...
    def OverwriteDefaultParsers(self, parser_names):
        ...
        with self._lock:
            ...
            new_parsers = [
                parser for parser in existing_parsers
                if parser.GetName() not in parser_names
            ]
            new_parsers.extend([
                parser for parser in default_parsers
                if parser.GetName() in parser_names
            ])
    ...
    def OverwriteDefaultURLClasses(self, url_class_names):
        ...
        with self._lock:
            ...
            new_url_classes = [
                url_class for url_class in existing_url_classes
                if url_class.GetName() not in url_class_names
            ]
            new_url_classes.extend([
                url_class for url_class in default_url_classes
                if url_class.GetName() in url_class_names
            ])
    ...
    def SetDefaultGUGKeyAndName(self, gug_key_and_name):
        ...
        with self._lock:
            ...
            HG.client_controller.new_options.SetString('default_gug_name',
                                                       gug_name)

    def SetDefaultTagImportOptions(self, file_post_default_tag_import_options,
    ...
    def SetNetworkContextsToCustomHeaderDicts(
        self, network_contexts_to_custom_header_dicts):
    ...
    def SetParsers(self, parsers):
        ...
        with self._lock:
            ...
            for (url_class_key, parser_key) in list(
                    self._url_class_keys_to_parser_keys.items()):
    ...
    def SetURLClasses(self, url_classes):
        ...
        with self._lock:
            ...
            old_post_url_class_keys = {
                url_class.GetMatchKey()
                for url_class in self._url_classes if url_class.IsPostURL()
            }
            post_url_class_keys = {
                url_class.GetMatchKey()
                for url_class in url_classes if url_class.IsPostURL()
            }

            added_post_url_class_keys = post_url_class_keys.difference(
                old_post_url_class_keys)
            ...
            for deletee_key in set(
                    self._url_class_keys_to_parser_keys.keys()).difference(
                        url_class_keys):
            ...
            url_class_api_pairs = ConvertURLClassesIntoAPIPairs(
                self._url_classes)
    ...
    def TryToLinkURLClassesAndParsers(self):
        ...
        with self._lock:
            ...
            new_url_class_keys_to_parser_keys = NetworkDomainManager.STATICLinkURLClassesAndParsers(
                ...
                self._url_class_keys_to_parser_keys)
    ...
    @staticmethod
    def STATICLinkURLClassesAndParsers(url_classes, parsers,
                ...
                for url_class in url_classes:
                    ...
                    if url_class.Matches(example_url):
                        ...
                        if parsable and linkable:
                            ...
                            new_url_class_keys_to_parser_keys[
                                url_class_key] = parser.GetParserKey()
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_NETWORK_DOMAIN_MANAGER] = NetworkDomainManager
...
class DomainMetadataPackage(HydrusSerialisable.SerialisableBase):
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (self._domain, self._headers_list,
         serialisable_bandwidth_rules) = serialisable_info
        ...
        if serialisable_bandwidth_rules is None:
            ...
            self._bandwidth_rules = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_bandwidth_rules)
    ...
    def GetDetailedSafeSummary(self):
        ...
        if self.HasBandwidthRules():
            ...
            m += os.linesep
            m += os.linesep.join([
                HydrusNetworking.ConvertBandwidthRuleToString(rule)
                for rule in self._bandwidth_rules.GetRules()
            ])
        ...
        if self.HasHeaders():
            ...
            m += os.linesep.join([
                key + ' : ' + value + ' - ' + reason
                for (key, value, reason) in self._headers_list
            ])
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_DOMAIN_METADATA_PACKAGE] = DomainMetadataPackage
    ...
    def Start(self):
        ...
        try:
            ...
            for (network_context, key, value, reason) in self._header_tuples:
                ...
                question = 'For the network context ' + network_context.ToString(
...
class GalleryURLGenerator(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def __init__(self,
                 ...
                 search_terms_separator=None,
                 initial_search_text=None,
                 example_search_text=None):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (serialisable_gallery_url_generator_key, self._url_template,
                self._replacement_phrase, self._search_terms_separator,
                self._initial_search_text, self._example_search_text)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_gallery_url_generator_key, self._url_template,
         self._replacement_phrase, self._search_terms_separator,
         self._initial_search_text,
         self._example_search_text) = serialisable_info

        self._gallery_url_generator_key = bytes.fromhex(
            serialisable_gallery_url_generator_key)
    ...
    def GenerateGalleryURL(self, query_text):
        ...
        if self._replacement_phrase not in self._url_template:
            ...
            raise HydrusExceptions.GUGException(
                'Replacement phrase not in URL template!')

        (first_part,
         second_part) = self._url_template.split(self._replacement_phrase, 1)
        ...
        try:
            ...
            search_phrase = self._search_terms_separator.join(
                encoded_search_terms)

            gallery_url = self._url_template.replace(self._replacement_phrase,
                                                     search_phrase)
    ...
    def GetSafeSummary(self):
        ...
        return 'Downloader "' + self._name + '" - ' + ConvertURLIntoDomain(
            self.GetExampleURL())

    def GetURLTemplateVariables(self):

        return (self._url_template, self._replacement_phrase,
                self._search_terms_separator, self._example_search_text)
    ...
    def IsFunctional(self):
        ...
        try:
            ...
            ) = HG.client_controller.network_engine.domain_manager.GetURLParseCapability(
                example_url)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_GALLERY_URL_GENERATOR] = GalleryURLGenerator
...
class NestedGalleryURLGenerator(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_gug_keys_and_names = [
            (gug_key.hex(), gug_name)
            for (gug_key, gug_name) in self._gug_keys_and_names
        ]

        return (serialisable_gug_key, self._initial_search_text,
                serialisable_gug_keys_and_names)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_gug_key, self._initial_search_text,
         serialisable_gug_keys_and_names) = serialisable_info

        self._gallery_url_generator_key = bytes.fromhex(serialisable_gug_key)
        self._gug_keys_and_names = [
            (bytes.fromhex(gug_key), gug_name)
            for (gug_key, gug_name) in serialisable_gug_keys_and_names
        ]
    ...
    def GetSafeSummary(self):
        ...
        return 'Nested downloader "' + self._name + '" - ' + ', '.join(
            (name for (gug_key, name) in self._gug_keys_and_names))
    ...
    def RepairGUGs(self, available_gugs):
        ...
        available_keys_to_gugs = {
            gug.GetGUGKey(): gug
            for gug in available_gugs
        }
        available_names_to_gugs = {
            gug.GetName(): gug
            for gug in available_gugs
        }
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_NESTED_GALLERY_URL_GENERATOR] = NestedGalleryURLGenerator
...
SEND_REFERRAL_URL_TYPES = [
    SEND_REFERRAL_URL_ONLY_IF_PROVIDED, SEND_REFERRAL_URL_NEVER,
...
send_referral_url_string_lookup[
    SEND_REFERRAL_URL_ONLY_IF_PROVIDED] = 'send a referral url if available'
send_referral_url_string_lookup[
    SEND_REFERRAL_URL_NEVER] = 'never send a referral url'
send_referral_url_string_lookup[
    SEND_REFERRAL_URL_CONVERTER_IF_NONE_PROVIDED] = 'use the converter if no referral is available'
send_referral_url_string_lookup[
    SEND_REFERRAL_URL_ONLY_CONVERTER] = 'always use the converter referral url'
...
class URLClass(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def _ClipAndFleshOutPath(self, path, allow_clip=True):
        ...
        if allow_clip or len(path_components) < len(self._path_components):
            ...
            for (index, (string_match,
                         default)) in enumerate(self._path_components):

                if len(path_components
                       ) > index:  # the given path has the value
                    ...
                    raise HydrusExceptions.URLClassException(
                        'Could not clip path--given url appeared to be too short!'
                    )
        ...
        for (key, (string_match, default)) in self._parameters.items():
            ...
            if key not in query_dict:
                ...
                if default is None:
                    ...
                    raise HydrusExceptions.URLClassException(
                        'Could not flesh out query--no default for ' + key +
                        ' defined!')
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_path_components = [
            (string_match.GetSerialisableTuple(), default)
            for (string_match, default) in self._path_components
        ]
        serialisable_parameters = [
            (key, (string_match.GetSerialisableTuple(), default))
            for (key, (string_match,
                       default)) in list(self._parameters.items())
        ]
        ...
        return (serialisable_url_class_key, self._url_type,
                self._preferred_scheme, self._netloc, self._match_subdomains,
                self._keep_matched_subdomains,
                self._alphabetise_get_parameters, serialisable_path_components,
                serialisable_parameters, serialisable_api_lookup_converter,
                self._send_referral_url, serialisable_referral_url_converter,
                self._can_produce_multiple_files,
                self._should_be_associated_with_files,
                self._gallery_index_type, self._gallery_index_identifier,
                self._gallery_index_delta, self._example_url)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_url_class_key, self._url_type, self._preferred_scheme,
         self._netloc, self._match_subdomains, self._keep_matched_subdomains,
         self._alphabetise_get_parameters, serialisable_path_components,
         serialisable_parameters, serialisable_api_lookup_converter,
         self._send_referral_url, serialisable_referral_url_converter,
         self._can_produce_multiple_files,
         self._should_be_associated_with_files, self._gallery_index_type,
         self._gallery_index_identifier, self._gallery_index_delta,
         self._example_url) = serialisable_info

        self._url_class_key = bytes.fromhex(serialisable_url_class_key)
        self._path_components = [
            (HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_string_match), default)
            for (serialisable_string_match,
                 default) in serialisable_path_components
        ]
        self._parameters = {
            key: (HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_string_match), default)
            for (key, (serialisable_string_match,
                       default)) in serialisable_parameters
        }
        self._api_lookup_converter = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_api_lookup_converter)
        self._referral_url_converter = HydrusSerialisable.CreateFromSerialisableTuple(
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (url_type, preferred_scheme, netloc, match_subdomains,
             keep_matched_subdomains, serialisable_path_components,
            ...
            new_serialisable_info = (serialisable_url_class_key, url_type,
                                     preferred_scheme, netloc,
                                     match_subdomains, keep_matched_subdomains,
                                     serialisable_path_components,
                                     serialisable_parameters,
                                     serialisable_api_lookup_converter,
                                     example_url)
        ...
        if version == 2:
            ...
            (serialisable_url_class_key, url_type, preferred_scheme, netloc,
             match_subdomains, keep_matched_subdomains,
             serialisable_path_components, serialisable_parameters,
             serialisable_api_lookup_converter,
             example_url) = old_serialisable_info
        ...
        if version == 4:
            ...
            (serialisable_url_class_key, url_type, preferred_scheme, netloc,
             match_subdomains, keep_matched_subdomains,
             serialisable_path_components, serialisable_parameters,
             serialisable_api_lookup_converter, can_produce_multiple_files,
             should_be_associated_with_files,
             example_url) = old_serialisable_info
            ...
            new_serialisable_info = (
                ...
                serialisable_path_components, serialisable_parameters,
                serialisable_api_lookup_converter, can_produce_multiple_files,
                should_be_associated_with_files, gallery_index_type,
                gallery_index_identifier, gallery_index_delta, example_url)
        ...
        if version == 5:
            ...
            (serialisable_url_class_key, url_type, preferred_scheme, netloc,
             ...
             serialisable_api_lookup_converter, can_produce_multiple_files,
             should_be_associated_with_files, gallery_index_type,
             gallery_index_identifier, gallery_index_delta,
             example_url) = old_serialisable_info

            path_components = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_path_components)
            parameters = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_parameters)
            ...
            serialisable_parameters = [
                (key, (string_match.GetSerialisableTuple(), default))
                for (key, (string_match, default)) in list(parameters.items())
            ]
        ...
        if version == 6:
            ...
            new_serialisable_info = (
                ...
                match_subdomains, keep_matched_subdomains,
                serialisable_path_components, serialisable_parameters,
                serialisable_api_lookup_converter, send_referral_url,
                serialisable_referrel_url_converter,
                can_produce_multiple_files, should_be_associated_with_files,
                gallery_index_type, gallery_index_identifier,
                gallery_index_delta, example_url)
        ...
        if version == 7:
            ...
            (serialisable_url_class_key, url_type, preferred_scheme, netloc,
             ...
             serialisable_path_components, serialisable_parameters,
             serialisable_api_lookup_converter, send_referral_url,
             serialisable_referrel_url_converter, can_produce_multiple_files,
             should_be_associated_with_files, gallery_index_type,
             gallery_index_identifier, gallery_index_delta,
             example_url) = old_serialisable_info
            ...
            new_serialisable_info = (
                serialisable_url_class_key, url_type, preferred_scheme, netloc,
                match_subdomains, keep_matched_subdomains,
                alphabetise_get_parameters, serialisable_path_components,
                serialisable_parameters, serialisable_api_lookup_converter,
                send_referral_url, serialisable_referrel_url_converter,
                can_produce_multiple_files, should_be_associated_with_files,
                gallery_index_type, gallery_index_identifier,
                gallery_index_delta, example_url)
    ...
    def CanReferToMultipleFiles(self):
        ...
        is_a_gallery_page = self._url_type in (HC.URL_TYPE_GALLERY,
    ...
    def GetGalleryIndexValues(self):
        ...
        return (self._gallery_index_type, self._gallery_index_identifier,
    ...
    def GetNextGalleryPage(self, url):
        ...
        if self._gallery_index_type == GALLERY_INDEX_TYPE_PATH_COMPONENT:
            ...
            try:
                ...
                raise HydrusExceptions.URLClassException(
                    'Could not generate next gallery page--index component was not an integer!'
                )

            path_components[page_index_path_component_index] = str(
                page_index + self._gallery_index_delta)
            ...
            if page_index_name not in query_dict:
                ...
                raise HydrusExceptions.URLClassException(
                    'Could not generate next gallery page--did not find ' +
                    str(self._gallery_index_identifier) + ' in parameters!')
            ...
            query_dict[
                page_index_name] = page_index + self._gallery_index_delta
            ...
            raise NotImplementedError(
                'Did not understand the next gallery page rules!')
    ...
    def GetReferralURL(self, url, referral_url):
        ...
        elif self._send_referral_url in (
                SEND_REFERRAL_URL_CONVERTER_IF_NONE_PROVIDED,
                SEND_REFERRAL_URL_ONLY_CONVERTER):

            try:

                converted_referral_url = self._referral_url_converter.Convert(
                    url)
    ...
    def GetSafeSummary(self):
        ...
        return 'URL Class "' + self._name + '" - ' + ConvertURLIntoDomain(
            self.GetExampleURL())

    def GetURLBooleans(self):

        return (self._match_subdomains, self._keep_matched_subdomains,
    ...
    def IsParsable(self):
        ...
        return self._url_type in (HC.URL_TYPE_POST, HC.URL_TYPE_GALLERY,
                                  HC.URL_TYPE_WATCHABLE)
    ...
    def SetURLBooleans(self, match_subdomains, keep_matched_subdomains,
                       alphabetise_get_parameters, can_produce_multiple_files,
                       should_be_associated_with_files):
    ...
    def Test(self, url):
        ...
        if self._match_subdomains:
            ...
            if p.netloc != self._netloc and not p.netloc.endswith(
                    '.' + self._netloc):

                raise HydrusExceptions.URLClassException(
                    p.netloc +
                    ' (potentially excluding subdomains) did not match ' +
                    self._netloc)
            ...
            if not DomainEqualsAnotherForgivingWWW(p.netloc, self._netloc):
                ...
                raise HydrusExceptions.URLClassException(p.netloc +
                                                         ' did not match ' +
                                                         self._netloc)
        ...
        for (index, (string_match,
            ...
            if len(url_path_components) > index:
                ...
                raise HydrusExceptions.URLClassException(
                    url_path +
                    ' did not have enough of the required path components!')
        ...
        for (key, (string_match, default)) in list(self._parameters.items()):
            ...
            if key not in url_parameters:
                ...
                if default is None:
                    ...
                    raise HydrusExceptions.URLClassException(key +
                                                             ' not found in ' +
                                                             p.query)
    ...
    def ToTuple(self):
        ...
        return (self._url_type, self._preferred_scheme, self._netloc,
                self._path_components, self._parameters,
                self._api_lookup_converter, self._send_referral_url,
                self._referral_url_converter, self._example_url)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_URL_CLASS] = URLClass
