...
from . import (ClientData, ClientDefaults, ClientFiles, ClientGUIShortcuts,
               ...
               ClientRatings, ClientSearch, ClientServices, ClientTags,
               ClientThreading)
...
from . import (HydrusImageHandling, HydrusNetwork, HydrusNetworking,
               HydrusPaths, HydrusSerialisable, HydrusTags,
               HydrusVideoHandling)
...
def DealWithBrokenJSONDump(db_dir, dump, dump_descriptor):
    ...
    message += os.linesep * 2
    message += 'The database has attempted to delete the broken object, errors have been written to the log, and the object\'s dump written to {}. Depending on the object, your client may no longer be able to boot, or it may have lost something like a session or a subscription.'.format(
        path)
    message += os.linesep * 2
...
def GenerateSpecificMappingsCacheTableNames(file_service_id, tag_service_id):
    ...
    return (cache_files_table_name, cache_current_mappings_table_name,
            cache_deleted_mappings_table_name,
            cache_pending_mappings_table_name, ac_cache_table_name)


def report_content_speed_to_job_key(job_key, rows_done, total_rows,
                                    precise_timestamp, num_rows, row_name):

    it_took = HydrusData.GetNowPrecise() - precise_timestamp

    rows_s = HydrusData.ToHumanInt(int(num_rows / it_took))

    popup_message = 'content row ' + HydrusData.ConvertValueRangeToPrettyString(
        rows_done,
        total_rows) + ': processing ' + row_name + ' at ' + rows_s + ' rows/s'
...
def report_speed_to_log(precise_timestamp, num_rows, row_name):
    ...
    summary = 'processed ' + HydrusData.ToHumanInt(
        num_rows) + ' ' + row_name + ' at ' + rows_s + ' rows/s'
...
class DB(HydrusDB.HydrusDB):
    ...
    READ_WRITE_ACTIONS = [
        'service_info', 'system_predicates', 'missing_thumbnail_hashes'
    ]
    ...
    def _AddFilesInfo(self, rows, overwrite=False):
        ...
        self._c.executemany(
            insert_phrase +
            ' files_info VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ? );', rows)

    def _AddFiles(self, service_id, rows):

        hash_ids = {row[0] for row in rows}

        existing_hash_ids = self._STS(
            self._ExecuteManySelect(
                'SELECT hash_id FROM current_files WHERE service_id = ? AND hash_id = ?;',
                ((service_id, hash_id) for hash_id in hash_ids)))

        valid_hash_ids = hash_ids.difference(existing_hash_ids)

        if len(valid_hash_ids) > 0:

            valid_rows = [(hash_id, timestamp) for (hash_id, timestamp) in rows
                          if hash_id in valid_hash_ids]

            # insert the files

            self._c.executemany(
                'INSERT OR IGNORE INTO current_files VALUES ( ?, ?, ? );',
                ((service_id, hash_id, timestamp)
                 for (hash_id, timestamp) in valid_rows))

            self._c.executemany(
                'DELETE FROM file_transfers WHERE service_id = ? AND hash_id = ?;',
                ((service_id, hash_id) for hash_id in valid_hash_ids))

            info = list(
                self._ExecuteManySelectSingleParam(
                    'SELECT hash_id, size, mime FROM files_info WHERE hash_id = ?;',
                    valid_hash_ids))
            ...
            service_info_updates.append(
                (delta_size, service_id, HC.SERVICE_INFO_TOTAL_SIZE))
            service_info_updates.append(
                (num_files, service_id, HC.SERVICE_INFO_NUM_FILES))
            service_info_updates.append(
                (num_inbox, service_id, HC.SERVICE_INFO_NUM_INBOX))

            select_statement = 'SELECT 1 FROM files_info WHERE mime IN ' + HydrusData.SplayListForDB(
                HC.SEARCHABLE_MIMES) + ' AND hash_id = ?;'

            num_viewable_files = sum(
                self._STI(
                    self._ExecuteManySelectSingleParam(select_statement,
                                                       valid_hash_ids)))

            service_info_updates.append((num_viewable_files, service_id,
                                         HC.SERVICE_INFO_NUM_VIEWABLE_FILES))
            ...
            if service_id == self._combined_local_file_service_id or service_type == HC.FILE_REPOSITORY:
                ...
                self._c.executemany(
                    'DELETE FROM deleted_files WHERE service_id = ? AND hash_id = ?;',
                    ((service_id, hash_id) for hash_id in valid_hash_ids))

                num_deleted = self._GetRowCount()

                service_info_updates.append(
                    (-num_deleted, service_id,
            ...
            if service_type == HC.LOCAL_FILE_DOMAIN:
                ...
                self._AddFiles(self._combined_local_file_service_id,
                               valid_rows)
            ...
            if service_type in HC.AUTOCOMPLETE_CACHE_SPECIFIC_FILE_SERVICES:
                ...
                for tag_service_id in tag_service_ids:
                    ...
                    self._CacheSpecificMappingsAddFiles(
                        service_id, tag_service_id, valid_hash_ids)
    ...
    def _AddService(self, service_key, service_type, name, dictionary):
        ...
        result = self._c.execute('SELECT 1 FROM services WHERE name = ?;',
                                 (name, )).fetchone()
        ...
        if service_type in HC.REPOSITORIES:
            ...
            repository_updates_table_name = GenerateRepositoryRepositoryUpdatesTableName(
                service_id)

            self._c.execute(
                'CREATE TABLE ' + repository_updates_table_name +
                ' ( update_index INTEGER, hash_id INTEGER, processed INTEGER_BOOLEAN, PRIMARY KEY ( update_index, hash_id ) );'
            )
            self._CreateIndex(repository_updates_table_name, ['hash_id'])

            (hash_id_map_table_name, tag_id_map_table_name
             ) = GenerateRepositoryMasterCacheTableNames(service_id)

            self._c.execute(
                'CREATE TABLE ' + hash_id_map_table_name +
                ' ( service_hash_id INTEGER PRIMARY KEY, hash_id INTEGER );')
            self._c.execute(
                'CREATE TABLE ' + tag_id_map_table_name +
                ' ( service_tag_id INTEGER PRIMARY KEY, tag_id INTEGER );')
        ...
        if service_type in HC.TAG_SERVICES:
            ...
            file_service_ids = self._GetServiceIds(
                HC.AUTOCOMPLETE_CACHE_SPECIFIC_FILE_SERVICES)

            for file_service_id in file_service_ids:

                self._CacheSpecificMappingsGenerate(file_service_id,
                                                    service_id)
    ...
    def _AddTagParents(self, service_id, pairs, make_content_updates=False):
        ...
        self._c.executemany(
            'DELETE FROM tag_parents WHERE service_id = ? AND child_tag_id = ? AND parent_tag_id = ?;',
            ((service_id, child_tag_id, parent_tag_id)
             for (child_tag_id, parent_tag_id) in pairs))
        self._c.executemany(
            'DELETE FROM tag_parent_petitions WHERE service_id = ? AND child_tag_id = ? AND parent_tag_id = ? AND status = ?;',
            ((service_id, child_tag_id, parent_tag_id,
              HC.CONTENT_STATUS_PENDING)
             for (child_tag_id, parent_tag_id) in pairs))

        self._c.executemany(
            'INSERT OR IGNORE INTO tag_parents ( service_id, child_tag_id, parent_tag_id, status ) VALUES ( ?, ?, ?, ? );',
            ((service_id, child_tag_id, parent_tag_id,
        ...
        for tag_id in tag_ids:
            ...
            self._FillInParents(service_id,
                                tag_id,
                                make_content_updates=make_content_updates)

    def _AddTagSiblings(self, service_id, pairs, make_content_updates=False):

        self._c.executemany(
            'DELETE FROM tag_siblings WHERE service_id = ? AND bad_tag_id = ?;',
            ((service_id, bad_tag_id) for (bad_tag_id, good_tag_id) in pairs))
        self._c.executemany(
            'DELETE FROM tag_sibling_petitions WHERE service_id = ? AND bad_tag_id = ? AND status = ?;',
            ((service_id, bad_tag_id, HC.CONTENT_STATUS_PENDING)
             for (bad_tag_id, good_tag_id) in pairs))

        self._c.executemany(
            'INSERT OR IGNORE INTO tag_siblings ( service_id, bad_tag_id, good_tag_id, status ) VALUES ( ?, ?, ?, ? );',
            ((service_id, bad_tag_id, good_tag_id, HC.CONTENT_STATUS_CURRENT)
             for (bad_tag_id, good_tag_id) in pairs))
        ...
        if len(names_to_analyze) > 0:
            ...
            try:
                ...
                job_key.SetVariable('popup_title',
                                    'database maintenance - analyzing')
    ...
    def _AnalyzeTable(self, name):
        ...
        result = self._c.execute(
            'SELECT num_rows FROM analyze_timestamps WHERE name = ?;',
            (name, )).fetchone()
        ...
        if do_it:
            ...
            (num_rows, ) = self._c.execute('SELECT COUNT( * ) FROM ' + name +
                                           ';').fetchone()

        self._c.execute('DELETE FROM analyze_timestamps WHERE name = ?;',
                        (name, ))

        self._c.execute(
            'INSERT OR IGNORE INTO analyze_timestamps ( name, num_rows, timestamp ) VALUES ( ?, ?, ? );',
            (name, num_rows, HydrusData.GetNow()))
    ...
    def _ArchiveFiles(self, hash_ids):
        ...
        if len(valid_hash_ids) > 0:
            ...
            self._c.executemany('DELETE FROM file_inbox WHERE hash_id = ?;',
                                ((hash_id, ) for hash_id in valid_hash_ids))

            with HydrusDB.TemporaryIntegerTable(self._c, valid_hash_ids,
                                                'hash_id') as temp_table_name:

                self._AnalyzeTempTable(temp_table_name)

                updates = self._c.execute(
                    'SELECT service_id, COUNT( * ) FROM current_files NATURAL JOIN {} GROUP BY service_id;'
                    .format(temp_table_name)).fetchall()

                self._c.executemany(
                    'UPDATE service_info SET info = info - ? WHERE service_id = ? AND info_type = ?;',
                    [(count, service_id, HC.SERVICE_INFO_NUM_INBOX)
                     for (service_id, count) in updates])
    ...
    def _AssociateRepositoryUpdateHashes(self, service_key, metadata_slice):
        ...
        for (update_index,
             update_hashes) in metadata_slice.GetUpdateIndicesAndHashes():
        ...
        self._c.executemany(
            'INSERT OR IGNORE INTO ' + repository_updates_table_name +
            ' ( update_index, hash_id, processed ) VALUES ( ?, ?, ? );',
            inserts)
    ...
    def _Backup(self, path):
        ...
        try:
            ...
            if os.path.exists(client_files_default):
                ...
                HydrusPaths.MirrorTree(client_files_default,
                                       os.path.join(path, 'client_files'),
                                       text_update_hook=text_update_hook,
                                       is_cancelled_hook=is_cancelled_hook)
    ...
    def _CacheCombinedFilesMappingsDrop(self, service_id):
        ...
        ac_cache_table_name = GenerateCombinedFilesMappingsCacheTableName(
            service_id)
    ...
    def _CacheCombinedFilesMappingsGenerate(self, service_id):
        ...
        self._c.execute(
            'CREATE TABLE ' + ac_cache_table_name +
            ' ( tag_id INTEGER PRIMARY KEY, current_count INTEGER, pending_count INTEGER );'
        )
        ...
        (current_mappings_table_name, deleted_mappings_table_name,
         pending_mappings_table_name, petitioned_mappings_table_name
         ) = GenerateMappingsTableNames(service_id)

        current_mappings_exist = self._c.execute(
            'SELECT 1 FROM ' + current_mappings_table_name +
            ' LIMIT 1;').fetchone() is not None
        pending_mappings_exist = self._c.execute(
            'SELECT 1 FROM ' + pending_mappings_table_name +
            ' LIMIT 1;').fetchone() is not None

        if current_mappings_exist or pending_mappings_exist:

            for group_of_ids in HydrusDB.ReadLargeIdQueryInSeparateChunks(
                    self._c, 'SELECT tag_id FROM tags;', 10000):
                ...
                if current_mappings_exist:
                    ...
                    for (tag_id, count) in self._ExecuteManySelectSingleParam(
                            select_statement, group_of_ids):
                ...
                count_ids = [(tag_id, current_counter[tag_id],
                              pending_counter[tag_id])
                             for tag_id in all_ids_seen]

                if len(count_ids) > 0:

                    self._CacheCombinedFilesMappingsUpdate(
                        service_id, count_ids)

    def _CacheCombinedFilesMappingsGetAutocompleteCounts(
        self, service_id, tag_ids):
        ...
        return list(
            self._ExecuteManySelectSingleParam(select_statement, tag_ids))
    ...
    def _CacheCombinedFilesMappingsUpdate(self, service_id, count_ids):
        ...
        self._c.executemany(
            'INSERT OR IGNORE INTO ' + ac_cache_table_name +
            ' ( tag_id, current_count, pending_count ) VALUES ( ?, ?, ? );',
            ((tag_id, 0, 0)
             for (tag_id, current_delta, pending_delta) in count_ids))

        self._c.executemany(
            'UPDATE ' + ac_cache_table_name +
            ' SET current_count = current_count + ?, pending_count = pending_count + ? WHERE tag_id = ?;',
            ((current_delta, pending_delta, tag_id)
             for (tag_id, current_delta, pending_delta) in count_ids))

        self._c.executemany(
            'DELETE FROM ' + ac_cache_table_name +
            ' WHERE tag_id = ? AND current_count = ? AND pending_count = ?;',
            ((tag_id, 0, 0)
             for (tag_id, current_delta, pending_delta) in count_ids))
    ...
    def _CacheLocalTagIdsGenerate(self):
        ...
        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_caches.local_tags_cache ( tag_id INTEGER PRIMARY KEY, tag TEXT UNIQUE );'
        )
        ...
        for tag_service_id in tag_service_ids:
            ...
            (cache_files_table_name, cache_current_mappings_table_name,
             cache_deleted_mappings_table_name,
             cache_pending_mappings_table_name,
             ac_cache_table_name) = GenerateSpecificMappingsCacheTableNames(
                 self._combined_local_file_service_id, tag_service_id)

            service_tag_ids = self._STL(
                self._c.execute('SELECT tag_id FROM ' + ac_cache_table_name +
    ...
    def _CacheLocalTagIdsPotentialAdd(self, tag_ids):
        ...
        self._c.executemany(
            'INSERT OR IGNORE INTO local_tags_cache ( tag_id, tag ) VALUES ( ?, ? );',
            ((tag_id, self._tag_ids_to_tags_cache[tag_id])
             for tag_id in tag_ids))
    ...
    def _CacheLocalTagIdsPotentialDelete(self, tag_ids):
        ...
        ids_to_count = self._GetAutocompleteCounts(
            self._combined_tag_service_id,
            self._combined_local_file_service_id, tag_ids, include_current,
            include_pending)

        useful_tag_ids = [
            tag_id for (tag_id, (current_min, current_max, pending_min,
                                 pending_max)) in ids_to_count.items()
            if current_min > 0
        ]

        bad_tag_ids = set(tag_ids).difference(useful_tag_ids)

        self._c.executemany('DELETE FROM local_tags_cache WHERE tag_id = ?;',
                            ((tag_id, ) for tag_id in bad_tag_ids))

    def _CacheRepositoryNormaliseServiceHashId(self, service_id,
                                               service_hash_id):
        ...
        result = self._c.execute(
            'SELECT hash_id FROM ' + hash_id_map_table_name +
    ...
    def _CacheRepositoryNormaliseServiceHashIds(self, service_id,
                                                service_hash_ids):
    ...
    def _CacheRepositoryNormaliseServiceTagId(self, service_id,
                                              service_tag_id):
        ...
        result = self._c.execute(
            'SELECT tag_id FROM ' + tag_id_map_table_name +
    ...
    def _CacheSpecificMappingsAddFiles(self, file_service_id, tag_service_id,
                                       hash_ids):
        ...
        self._c.executemany(
            'INSERT OR IGNORE INTO ' + cache_files_table_name +
            ' VALUES ( ? );', ((hash_id, ) for hash_id in hash_ids))
        ...
        for group_of_hash_ids in HydrusData.SplitListIntoChunks(hash_ids, 100):
            ...
            splayed_group_of_hash_ids = HydrusData.SplayListForDB(
                group_of_hash_ids)

            current_mapping_ids_raw = self._c.execute(
                'SELECT tag_id, hash_id FROM ' + current_mappings_table_name +
                ' WHERE hash_id IN ' + splayed_group_of_hash_ids +
                ';').fetchall()

            current_mapping_ids_dict = HydrusData.BuildKeyToSetDict(
                current_mapping_ids_raw)

            deleted_mapping_ids_raw = self._c.execute(
                'SELECT tag_id, hash_id FROM ' + deleted_mappings_table_name +
                ' WHERE hash_id IN ' + splayed_group_of_hash_ids +
                ';').fetchall()

            deleted_mapping_ids_dict = HydrusData.BuildKeyToSetDict(
                deleted_mapping_ids_raw)

            pending_mapping_ids_raw = self._c.execute(
                'SELECT tag_id, hash_id FROM ' + pending_mappings_table_name +
                ' WHERE hash_id IN ' + splayed_group_of_hash_ids +
                ';').fetchall()
            ...
            for tag_id in all_ids_seen:
                ...
                if num_current > 0:
                    ...
                    self._c.executemany(
                        ...
                        ((hash_id, tag_id) for hash_id in current_hash_ids))
                ...
                if num_deleted > 0:
                    ...
                    self._c.executemany(
                        ...
                        cache_deleted_mappings_table_name +
                        ' ( hash_id, tag_id ) VALUES ( ?, ? );',
                        ((hash_id, tag_id) for hash_id in deleted_hash_ids))
                ...
                if num_pending > 0:
                    ...
                    self._c.executemany(
                        ...
                        cache_pending_mappings_table_name +
                        ' ( hash_id, tag_id ) VALUES ( ?, ? );',
                        ((hash_id, tag_id) for hash_id in pending_hash_ids))
        ...
        if len(ac_cache_changes) > 0:
            ...
            self._c.executemany(
                ...
                ' ( tag_id, current_count, pending_count ) VALUES ( ?, ?, ? );',
                ((tag_id, 0, 0)
                 for (tag_id, num_current, num_pending) in ac_cache_changes))

            self._c.executemany(
                'UPDATE ' + ac_cache_table_name +
                ' SET current_count = current_count + ?, pending_count = pending_count + ? WHERE tag_id = ?;',
                ((num_current, num_pending, tag_id)
                 for (tag_id, num_current, num_pending) in ac_cache_changes))
    ...
    def _CacheSpecificMappingsAddMappings(self, file_service_id,
                                          tag_service_id, mappings_ids):
        ...
        for (tag_id, hash_ids) in mappings_ids:
            ...
            hash_ids = self._CacheSpecificMappingsFilterHashIds(
                file_service_id, tag_service_id, hash_ids)

            if len(hash_ids) > 0:

                self._c.executemany(
                    'DELETE FROM ' + cache_pending_mappings_table_name +
                    ' WHERE hash_id = ? AND tag_id = ?;',
                    ((hash_id, tag_id) for hash_id in hash_ids))
                ...
                if num_pending_rescinded > 0:
                    ...
                    self._c.execute(
                        'UPDATE ' + ac_cache_table_name +
                        ' SET current_count = current_count + ?, pending_count = pending_count - ? WHERE tag_id = ?;',
                        (num_added, num_pending_rescinded, tag_id))
                    ...
                    self._c.execute(
                        'UPDATE ' + ac_cache_table_name +
                        ' SET current_count = current_count + ? WHERE tag_id = ?;',
                        (num_added, tag_id))

                #

                self._c.executemany(
                    'DELETE FROM ' + cache_deleted_mappings_table_name +
                    ' WHERE hash_id = ? AND tag_id = ?;',
                    ((hash_id, tag_id) for hash_id in hash_ids))

        if file_service_id == self._combined_local_file_service_id and len(
                potential_new_tag_ids) > 0:
    ...
    def _CacheSpecificMappingsDrop(self, file_service_id, tag_service_id):
        ...
        self._c.execute('DROP TABLE IF EXISTS ' +
                        cache_current_mappings_table_name + ';')
    ...
    def _CacheSpecificMappingsDeleteFiles(self, file_service_id,
                                          tag_service_id, hash_ids):
        ...
        self._c.executemany(
            'DELETE FROM ' + cache_files_table_name + ' WHERE hash_id = ?;',
        ...
        for group_of_hash_ids in HydrusData.SplitListIntoChunks(hash_ids, 100):
            ...
            current_mapping_ids_raw = self._c.execute(
                'SELECT tag_id, hash_id FROM ' +
                cache_current_mappings_table_name + ' WHERE hash_id IN ' +
                splayed_group_of_hash_ids + ';').fetchall()
            ...
            deleted_mapping_ids_raw = self._c.execute(
                'SELECT tag_id, hash_id FROM ' +
                cache_deleted_mappings_table_name + ' WHERE hash_id IN ' +
                splayed_group_of_hash_ids + ';').fetchall()
            ...
            pending_mapping_ids_raw = self._c.execute(
                'SELECT tag_id, hash_id FROM ' +
                cache_pending_mappings_table_name + ' WHERE hash_id IN ' +
                splayed_group_of_hash_ids + ';').fetchall()
            ...
            for tag_id in all_ids_seen:
                ...
                if num_current > 0:
                    ...
                    self._c.executemany(
                        'DELETE FROM ' + cache_current_mappings_table_name +
                        ' WHERE tag_id = ? AND hash_id = ?;',
                        ((tag_id, hash_id) for hash_id in current_hash_ids))
                ...
                if num_deleted > 0:
                    ...
                    self._c.executemany(
                        'DELETE FROM ' + cache_deleted_mappings_table_name +
                        ' WHERE tag_id = ? AND hash_id = ?;',
                        ((tag_id, hash_id) for hash_id in deleted_hash_ids))
                ...
                if num_pending > 0:
                    ...
                    self._c.executemany(
                        'DELETE FROM ' + cache_pending_mappings_table_name +
                        ' WHERE tag_id = ? AND hash_id = ?;',
                        ((tag_id, hash_id) for hash_id in pending_hash_ids))
        ...
        if len(ac_cache_changes) > 0:
            ...
            if file_service_id == self._combined_local_file_service_id:
                ...
                potential_deleted_tag_ids = [
                    tag_id
                    for (tag_id, num_current, num_pending) in ac_cache_changes
                    if num_current > 0
                ]

                self._CacheLocalTagIdsPotentialDelete(
                    potential_deleted_tag_ids)

    def _CacheSpecificMappingsDeleteMappings(self, file_service_id,
                                             tag_service_id, mappings_ids):
    ...
    def _CacheSpecificMappingsFilterHashIds(self, file_service_id,
                                            tag_service_id, hash_ids):
        ...
        return self._STL(
            self._ExecuteManySelectSingleParam(select_statement, hash_ids))
    ...
    def _CacheSpecificMappingsGenerate(self, file_service_id, tag_service_id):
        ...
        self._c.execute('CREATE TABLE ' + cache_files_table_name +
                        ' ( hash_id INTEGER PRIMARY KEY );')

        self._c.execute(
            'CREATE TABLE ' + cache_current_mappings_table_name +
            ' ( hash_id INTEGER, tag_id INTEGER, PRIMARY KEY ( hash_id, tag_id ) ) WITHOUT ROWID;'
        )

        self._c.execute(
            'CREATE TABLE ' + cache_deleted_mappings_table_name +
            ' ( hash_id INTEGER, tag_id INTEGER, PRIMARY KEY ( hash_id, tag_id ) ) WITHOUT ROWID;'
        )

        self._c.execute(
            'CREATE TABLE ' + cache_pending_mappings_table_name +
            ' ( hash_id INTEGER, tag_id INTEGER, PRIMARY KEY ( hash_id, tag_id ) ) WITHOUT ROWID;'
        )
        ...
        select_statement = 'SELECT hash_id FROM current_files WHERE service_id = {};'.format(
            file_service_id)

        for group_of_hash_ids in HydrusDB.ReadLargeIdQueryInSeparateChunks(
                self._c, select_statement, 10000):

            self._CacheSpecificMappingsAddFiles(file_service_id,
                                                tag_service_id,
                                                group_of_hash_ids)

        self._CreateIndex(cache_current_mappings_table_name,
                          ['tag_id', 'hash_id'],
                          unique=True)
        self._CreateIndex(cache_deleted_mappings_table_name,
                          ['tag_id', 'hash_id'],
                          unique=True)
        self._CreateIndex(cache_pending_mappings_table_name,
                          ['tag_id', 'hash_id'],
                          unique=True)

    def _CacheSpecificMappingsGetAutocompleteCounts(self, file_service_id,
    ...
    def _CacheSpecificMappingsPendMappings(self, file_service_id,
                                           tag_service_id, mappings_ids):
        ...
        for (tag_id, hash_ids) in mappings_ids:
            ...
            if len(hash_ids) > 0:
                ...
                if num_added > 0:
                    ...
                    self._c.execute(
                        'UPDATE ' + ac_cache_table_name +
                        ' SET pending_count = pending_count + ? WHERE tag_id = ?;',
                        (num_added, tag_id))

    def _CacheSpecificMappingsRescindPendingMappings(self, file_service_id,
                                                     tag_service_id,
                                                     mappings_ids):
    ...
    def _CheckDBIntegrity(self):
        ...
        try:
            ...
            job_key.SetVariable(
                'popup_text_1',
                'errors found so far: ' + HydrusData.ToHumanInt(num_errors))
            ...
            for db_name in db_names:
                ...
                for (text, ) in self._c.execute('PRAGMA ' + db_name +
                                                '.integrity_check;'):
                    ...
                    if should_quit:
                        ...
                        job_key.SetVariable('popup_title',
                                            prefix_string + 'cancelled')
                        job_key.SetVariable(
                            'popup_text_1', 'errors found: ' +
                    ...
                    if text != 'ok':
                        ...
                        if num_errors == 0:
                            ...
                            HydrusData.Print(
                                'During a db integrity check, these errors were discovered:'
                            )
                    ...
                    job_key.SetVariable(
                        'popup_text_1', 'errors found so far: ' +
                        HydrusData.ToHumanInt(num_errors))
            ...
            job_key.SetVariable(
                'popup_text_1',
                'errors found: ' + HydrusData.ToHumanInt(num_errors))
    ...
    def _ClearOrphanFileRecords(self):
        ...
        try:
            ...
            local_file_service_ids = self._GetServiceIds(
                (HC.LOCAL_FILE_DOMAIN, HC.LOCAL_FILE_TRASH_DOMAIN))

            local_hash_ids = self._STS(
                self._c.execute(
                    'SELECT hash_id FROM current_files WHERE service_id IN ' +
                    HydrusData.SplayListForDB(local_file_service_ids) + ';'))

            combined_local_file_service_id = self._GetServiceId(
                CC.COMBINED_LOCAL_FILE_SERVICE_KEY)

            combined_local_hash_ids = self._STS(
                self._c.execute(
                    'SELECT hash_id FROM current_files WHERE service_id = ?;',
                    (combined_local_file_service_id, )))

            in_local_not_in_combined = local_hash_ids.difference(
                combined_local_hash_ids)
            in_combined_not_in_local = combined_local_hash_ids.difference(
                local_hash_ids)
            ...
            if len(in_local_not_in_combined) > 0:
                ...
                self._DeleteFiles(self._combined_local_file_service_id,
                                  in_local_not_in_combined)
                ...
                HydrusData.ShowText(
                    'Found and deleted ' +
                    HydrusData.ToHumanInt(len(in_local_not_in_combined)) +
                    ' local domain orphan file records.')
            ...
            if len(in_combined_not_in_local) > 0:
                ...
                HydrusData.ShowText(
                    'Found and deleted ' +
                    HydrusData.ToHumanInt(len(in_combined_not_in_local)) +
                    ' combined domain orphan file records.')

            if len(in_local_not_in_combined) == 0 and len(
                    in_combined_not_in_local) == 0:
    ...
    def _ClearOrphanTables(self):
        ...
        service_ids = self._STL(
            self._c.execute('SELECT service_id FROM services;'))
        ...
        existing_table_names.update(
            self._STS(
                self._c.execute(
                    'SELECT name FROM external_master.sqlite_master WHERE type = ?;',
                    ('table', ))))

        existing_table_names = {
            name
            for name in existing_table_names
            if True in (name.startswith(table_prefix)
                        for table_prefix in table_prefixes)
        }
    ...
    def _CreateDB(self):
        ...
        self._c.execute(
            'CREATE TABLE IF NOT EXISTS alternate_file_groups ( alternates_group_id INTEGER PRIMARY KEY );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS alternate_file_group_members ( alternates_group_id INTEGER, media_id INTEGER UNIQUE, PRIMARY KEY ( alternates_group_id, media_id ) );'
        )
        ...
        self._c.execute(
            'CREATE TABLE IF NOT EXISTS ideal_client_files_locations ( location TEXT, weight INTEGER );'
        )
        self._c.execute(
            'CREATE TABLE IF NOT EXISTS ideal_thumbnail_override_location ( location TEXT );'
        )

        self._c.execute(
            'CREATE TABLE current_files ( service_id INTEGER REFERENCES services ON DELETE CASCADE, hash_id INTEGER, timestamp INTEGER, PRIMARY KEY ( service_id, hash_id ) );'
        )
        self._CreateIndex('current_files', ['timestamp'])

        self._c.execute(
            'CREATE TABLE deleted_files ( service_id INTEGER REFERENCES services ON DELETE CASCADE, hash_id INTEGER, PRIMARY KEY ( service_id, hash_id ) );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS duplicate_files ( media_id INTEGER PRIMARY KEY, king_hash_id INTEGER UNIQUE );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS duplicate_file_members ( media_id INTEGER, hash_id INTEGER UNIQUE, PRIMARY KEY ( media_id, hash_id ) );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS duplicate_false_positives ( smaller_alternates_group_id INTEGER, larger_alternates_group_id INTEGER, PRIMARY KEY ( smaller_alternates_group_id, larger_alternates_group_id ) );'
        )
        self._CreateIndex(
            'duplicate_false_positives',
            ['larger_alternates_group_id', 'smaller_alternates_group_id'],
            unique=True)

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS potential_duplicate_pairs ( smaller_media_id INTEGER, larger_media_id INTEGER, distance INTEGER, PRIMARY KEY ( smaller_media_id, larger_media_id ) );'
        )
        self._CreateIndex('potential_duplicate_pairs',
                          ['larger_media_id', 'smaller_media_id'],
                          unique=True)

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS confirmed_alternate_pairs ( smaller_media_id INTEGER, larger_media_id INTEGER, PRIMARY KEY ( smaller_media_id, larger_media_id ) );'
        )
        self._CreateIndex('confirmed_alternate_pairs',
                          ['larger_media_id', 'smaller_media_id'],
                          unique=True)
        ...
        self._c.execute(
            'CREATE TABLE file_notes ( hash_id INTEGER PRIMARY KEY, notes TEXT );'
        )

        self._c.execute(
            'CREATE TABLE file_transfers ( service_id INTEGER REFERENCES services ON DELETE CASCADE, hash_id INTEGER, PRIMARY KEY ( service_id, hash_id ) );'
        )
        self._CreateIndex('file_transfers', ['hash_id'])

        self._c.execute(
            'CREATE TABLE file_petitions ( service_id INTEGER REFERENCES services ON DELETE CASCADE, hash_id INTEGER, reason_id INTEGER, PRIMARY KEY ( service_id, hash_id, reason_id ) );'
        )
        ...
        self._c.execute(
            'CREATE TABLE local_ratings ( service_id INTEGER REFERENCES services ON DELETE CASCADE, hash_id INTEGER, rating REAL, PRIMARY KEY ( service_id, hash_id ) );'
        )
        ...
        self._CreateIndex('file_modified_timestamps',
                          ['file_modified_timestamp'])

        self._c.execute('CREATE TABLE options ( options TEXT_YAML );', )

        self._c.execute(
            'CREATE TABLE recent_tags ( service_id INTEGER REFERENCES services ON DELETE CASCADE, tag_id INTEGER, timestamp INTEGER, PRIMARY KEY ( service_id, tag_id ) );'
        )

        self._c.execute(
            'CREATE TABLE remote_ratings ( service_id INTEGER REFERENCES services ON DELETE CASCADE, hash_id INTEGER, count INTEGER, rating REAL, score REAL, PRIMARY KEY ( service_id, hash_id ) );'
        )
        ...
        self._c.execute(
            'CREATE TABLE service_filenames ( service_id INTEGER REFERENCES services ON DELETE CASCADE, hash_id INTEGER, filename TEXT, PRIMARY KEY ( service_id, hash_id ) );'
        )
        self._c.execute(
            'CREATE TABLE service_directories ( service_id INTEGER REFERENCES services ON DELETE CASCADE, directory_id INTEGER, num_files INTEGER, total_size INTEGER, note TEXT, PRIMARY KEY ( service_id, directory_id ) );'
        )
        self._c.execute(
            'CREATE TABLE service_directory_file_map ( service_id INTEGER REFERENCES services ON DELETE CASCADE, directory_id INTEGER, hash_id INTEGER, PRIMARY KEY ( service_id, directory_id, hash_id ) );'
        )

        self._c.execute(
            'CREATE TABLE service_info ( service_id INTEGER REFERENCES services ON DELETE CASCADE, info_type INTEGER, info INTEGER, PRIMARY KEY ( service_id, info_type ) );'
        )
        ...
        self._c.execute(
            'CREATE TABLE tag_parents ( service_id INTEGER REFERENCES services ON DELETE CASCADE, child_tag_id INTEGER, parent_tag_id INTEGER, status INTEGER, PRIMARY KEY ( service_id, child_tag_id, parent_tag_id, status ) );'
        )

        self._c.execute(
            'CREATE TABLE tag_parent_petitions ( service_id INTEGER REFERENCES services ON DELETE CASCADE, child_tag_id INTEGER, parent_tag_id INTEGER, status INTEGER, reason_id INTEGER, PRIMARY KEY ( service_id, child_tag_id, parent_tag_id, status ) );'
        )

        self._c.execute(
            'CREATE TABLE tag_siblings ( service_id INTEGER REFERENCES services ON DELETE CASCADE, bad_tag_id INTEGER, good_tag_id INTEGER, status INTEGER, PRIMARY KEY ( service_id, bad_tag_id, status ) );'
        )

        self._c.execute(
            'CREATE TABLE tag_sibling_petitions ( service_id INTEGER REFERENCES services ON DELETE CASCADE, bad_tag_id INTEGER, good_tag_id INTEGER, status INTEGER, reason_id INTEGER, PRIMARY KEY ( service_id, bad_tag_id, status ) );'
        )
        ...
        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_master.hashes ( hash_id INTEGER PRIMARY KEY, hash BLOB_BYTES UNIQUE );'
        )

        self._c.execute(
            'CREATE TABLE external_master.local_hashes ( hash_id INTEGER PRIMARY KEY, md5 BLOB_BYTES, sha1 BLOB_BYTES, sha512 BLOB_BYTES );'
        )
        ...
        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_master.namespaces ( namespace_id INTEGER PRIMARY KEY, namespace TEXT UNIQUE );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_master.subtags ( subtag_id INTEGER PRIMARY KEY, subtag TEXT UNIQUE );'
        )

        self._c.execute(
            'CREATE VIRTUAL TABLE IF NOT EXISTS external_master.subtags_fts4 USING fts4( subtag );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_master.tags ( tag_id INTEGER PRIMARY KEY, namespace_id INTEGER, subtag_id INTEGER );'
        )
        self._CreateIndex('external_master.tags',
                          ['subtag_id', 'namespace_id'])

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_master.texts ( text_id INTEGER PRIMARY KEY, text TEXT UNIQUE );'
        )
        ...
        location = HydrusPaths.ConvertAbsPathToPortablePath(
            client_files_default)

        for prefix in HydrusData.IterateHexPrefixes():

            self._c.execute(
                'INSERT INTO client_files_locations ( prefix, location ) VALUES ( ?, ? );',
                ('f' + prefix, location))
            self._c.execute(
                'INSERT INTO client_files_locations ( prefix, location ) VALUES ( ?, ? );',
                ('t' + prefix, location))
        ...
        self._c.execute(
            'INSERT INTO ideal_client_files_locations ( location, weight ) VALUES ( ?, ? );',
            (location, 1))

        init_service_info = []

        init_service_info.append((CC.COMBINED_LOCAL_FILE_SERVICE_KEY,
                                  HC.COMBINED_LOCAL_FILE, 'all local files'))
        init_service_info.append(
            (CC.LOCAL_FILE_SERVICE_KEY, HC.LOCAL_FILE_DOMAIN, 'my files'))
        init_service_info.append(
            (CC.TRASH_SERVICE_KEY, HC.LOCAL_FILE_TRASH_DOMAIN, 'trash'))
        init_service_info.append((CC.LOCAL_UPDATE_SERVICE_KEY,
                                  HC.LOCAL_FILE_DOMAIN, 'repository updates'))
        init_service_info.append(
            (CC.DEFAULT_LOCAL_TAG_SERVICE_KEY, HC.LOCAL_TAG, 'my tags'))
        init_service_info.append((CC.COMBINED_FILE_SERVICE_KEY,
                                  HC.COMBINED_FILE, 'all known files'))
        init_service_info.append(
        ...
        for (service_key, service_type, name) in init_service_info:
            ...
            dictionary = ClientServices.GenerateDefaultServiceDictionary(
                service_type)

            self._AddService(service_key, service_type, name, dictionary)

        self._c.executemany(
            'INSERT INTO yaml_dumps VALUES ( ?, ?, ? );',
            ((YAML_DUMP_ID_IMAGEBOARD, name, imageboards)
             for (name,
                  imageboards) in ClientDefaults.GetDefaultImageboards()))

        new_options = ClientOptions.ClientOptions()

        new_options.SetSimpleDownloaderFormulae(
        ...
        self._c.execute('INSERT INTO version ( version ) VALUES ( ? );',
                        (HC.SOFTWARE_VERSION, ))

        self._c.executemany(
            'INSERT INTO json_dumps_named VALUES ( ?, ?, ?, ?, ? );',
            ClientDefaults.GetDefaultScriptRows())

    def _CreateDBCaches(self):

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_caches.file_maintenance_jobs ( hash_id INTEGER, job_type INTEGER, time_can_start INTEGER, PRIMARY KEY ( hash_id, job_type ) );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_caches.shape_perceptual_hashes ( phash_id INTEGER PRIMARY KEY, phash BLOB_BYTES UNIQUE );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_caches.shape_perceptual_hash_map ( phash_id INTEGER, hash_id INTEGER, PRIMARY KEY ( phash_id, hash_id ) );'
        )
        self._CreateIndex('external_caches.shape_perceptual_hash_map',
                          ['hash_id'])

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_caches.shape_vptree ( phash_id INTEGER PRIMARY KEY, parent_id INTEGER, radius INTEGER, inner_id INTEGER, inner_population INTEGER, outer_id INTEGER, outer_population INTEGER );'
        )
        self._CreateIndex('external_caches.shape_vptree', ['parent_id'])

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_caches.shape_maintenance_branch_regen ( phash_id INTEGER PRIMARY KEY );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_caches.shape_search_cache ( hash_id INTEGER PRIMARY KEY, searched_distance INTEGER );'
        )

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS external_caches.integer_subtags ( subtag_id INTEGER PRIMARY KEY, integer_subtag INTEGER );'
        )
        self._CreateIndex('external_caches.integer_subtags',
                          ['integer_subtag'])
    ...
    def _CullFileViewingStatistics(self):
        ...
        media_min = self._controller.new_options.GetNoneableInteger(
            'file_viewing_statistics_media_min_time')
        media_max = self._controller.new_options.GetNoneableInteger(
            'file_viewing_statistics_media_max_time')
        preview_min = self._controller.new_options.GetNoneableInteger(
            'file_viewing_statistics_preview_min_time')
        preview_max = self._controller.new_options.GetNoneableInteger(
            'file_viewing_statistics_preview_max_time')

        if media_min is not None and media_max is not None and media_min > media_max:

            raise Exception(
                'Media min was greater than media max! Abandoning cull now!')

        if preview_min is not None and preview_max is not None and preview_min > preview_max:

            raise Exception(
                'Preview min was greater than preview max! Abandoning cull now!'
            )

        if media_min is not None:

            self._c.execute(
                'UPDATE file_viewing_stats SET media_views = CAST( media_viewtime / ? AS INTEGER ) WHERE media_views * ? > media_viewtime;',
                (media_min, media_min))

        if media_max is not None:

            self._c.execute(
                'UPDATE file_viewing_stats SET media_viewtime = media_views * ? WHERE media_viewtime > media_views * ?;',
                (media_max, media_max))

        if preview_min is not None:

            self._c.execute(
                'UPDATE file_viewing_stats SET preview_views = CAST( preview_viewtime / ? AS INTEGER ) WHERE preview_views * ? > preview_viewtime;',
                (preview_min, preview_min))

        if preview_max is not None:

            self._c.execute(
                'UPDATE file_viewing_stats SET preview_viewtime = preview_views * ? WHERE preview_viewtime > preview_views * ?;',
    ...
    def _DeleteFiles(self, service_id, hash_ids):
        ...
        if len(existing_hash_ids) > 0:
            ...
            splayed_existing_hash_ids = HydrusData.SplayListForDB(
                existing_hash_ids)

            # remove them from the service

            self._c.executemany(
                'DELETE FROM current_files WHERE service_id = ? AND hash_id = ?;',
                ((service_id, hash_id) for hash_id in existing_hash_ids))

            self._c.executemany(
                'DELETE FROM file_petitions WHERE service_id = ? AND hash_id = ?;',
                ((service_id, hash_id) for hash_id in existing_hash_ids))

            info = list(
                self._ExecuteManySelectSingleParam(
                    'SELECT size, mime FROM files_info WHERE hash_id = ?;',
                    existing_hash_ids))

            num_existing_files_removed = len(existing_hash_ids)
            delta_size = sum((size for (size, mime) in info))
            num_inbox = len(
                existing_hash_ids.intersection(self._inbox_hash_ids))

            service_info_updates.append(
                (-delta_size, service_id, HC.SERVICE_INFO_TOTAL_SIZE))
            service_info_updates.append(
                (-num_existing_files_removed, service_id,
            ...
            if service_type in HC.AUTOCOMPLETE_CACHE_SPECIFIC_FILE_SERVICES:
                ...
                for tag_service_id in tag_service_ids:
                    ...
                    self._CacheSpecificMappingsDeleteFiles(
                        service_id, tag_service_id, existing_hash_ids)
            ...
            if service_id in local_file_service_ids:
                ...
                splayed_local_file_service_ids = HydrusData.SplayListForDB(
                    local_file_service_ids)

                non_orphan_hash_ids = self._STS(
                    self._c.execute(
                        'SELECT hash_id FROM current_files WHERE hash_id IN ' +
                        splayed_existing_hash_ids + ' AND service_id IN ' +
                        splayed_local_file_service_ids + ';'))

                orphan_hash_ids = existing_hash_ids.difference(
                    non_orphan_hash_ids)
                ...
                if len(orphan_hash_ids) > 0:
                    ...
                    delete_rows = [(hash_id, now)
                                   for hash_id in orphan_hash_ids]
        ...
        if service_id == self._combined_local_file_service_id or service_type == HC.FILE_REPOSITORY:
            ...
            self._c.executemany(
                'INSERT OR IGNORE INTO deleted_files ( service_id, hash_id ) VALUES ( ?, ? );',
                [(service_id, hash_id) for hash_id in hash_ids])

            num_new_deleted_files = self._GetRowCount()

            service_info_updates.append((num_new_deleted_files, service_id,
                                         HC.SERVICE_INFO_NUM_DELETED_FILES))
    ...
    def _DeleteJSONDumpNamed(self, dump_type, dump_name=None, timestamp=None):
        ...
        if dump_name is None:
            ...
            self._c.execute(
                'DELETE FROM json_dumps_named WHERE dump_type = ?;',
                (dump_type, ))
    ...
    def _DeletePending(self, service_key):
        ...
        if service.GetServiceType() == HC.TAG_REPOSITORY:
            ...
            pending_rescinded_mappings_ids = list(
                HydrusData.BuildKeyToListDict(
                    self._c.execute('SELECT tag_id, hash_id FROM ' +
                                    pending_mappings_table_name +
                                    ';')).items())

            petitioned_rescinded_mappings_ids = list(
                HydrusData.BuildKeyToListDict(
                    self._c.execute('SELECT tag_id, hash_id FROM ' +
            ...
            self._UpdateMappings(
                service_id,
                pending_rescinded_mappings_ids=pending_rescinded_mappings_ids,
                petitioned_rescinded_mappings_ids=
                petitioned_rescinded_mappings_ids)

            self._c.execute(
                'DELETE FROM tag_sibling_petitions WHERE service_id = ?;',
                (service_id, ))
            self._c.execute(
                'DELETE FROM tag_parent_petitions WHERE service_id = ?;',
                (service_id, ))

        elif service.GetServiceType() in (HC.FILE_REPOSITORY, HC.IPFS):

            self._c.execute('DELETE FROM file_transfers WHERE service_id = ?;',
                            (service_id, ))
            self._c.execute('DELETE FROM file_petitions WHERE service_id = ?;',
                            (service_id, ))
        ...
        self.pub_service_updates_after_commit({
            service_key:
            [HydrusData.ServiceUpdate(HC.SERVICE_UPDATE_DELETE_PENDING)]
        })
    ...
    def _DeletePhysicalFiles(self, hash_ids):
        ...
        self._c.executemany(
            'DELETE FROM file_maintenance_jobs WHERE hash_id = ?;',
            ((hash_id, ) for hash_id in hash_ids))

        potentially_pending_upload_hash_ids = self._STS(
            self._c.execute('SELECT hash_id FROM file_transfers;', ))

        deletable_file_hash_ids = hash_ids.difference(
            potentially_pending_upload_hash_ids)
        ...
        if len(deletable_file_hash_ids) > 0:
            ...
            self._controller.CallToThread(
                client_files_manager.DelayedDeleteFiles, file_hashes)

        useful_thumbnail_hash_ids = self._STS(
            self._c.execute(
                'SELECT hash_id FROM current_files WHERE hash_id IN ' +
                HydrusData.SplayListForDB(hash_ids) + ';'))

        deletable_thumbnail_hash_ids = hash_ids.difference(
            useful_thumbnail_hash_ids)
        ...
        if len(deletable_thumbnail_hash_ids) > 0:
            ...
            self._controller.CallToThread(
                client_files_manager.DelayedDeleteThumbnails, thumbnail_hashes)
    ...
    def _DeleteService(self, service_id):
        ...
        self._c.execute('DELETE FROM remote_thumbnails WHERE service_id = ?;',
                        (service_id, ))
    ...
    def _DeleteServiceDirectory(self, service_id, dirname):
        ...
        self._c.execute(
            'DELETE FROM service_directories WHERE service_id = ? AND directory_id = ?;',
            (service_id, directory_id))
        self._c.execute(
            'DELETE FROM service_directory_file_map WHERE service_id = ? AND directory_id = ?;',
            (service_id, directory_id))
    ...
    def _DeleteTagSiblings(self, service_id, pairs):
        ...
        self._c.executemany(
            'DELETE FROM tag_sibling_petitions WHERE service_id = ? AND bad_tag_id = ? AND status = ?;',
            ((service_id, bad_tag_id, HC.CONTENT_STATUS_PETITIONED)
             for (bad_tag_id, good_tag_id) in pairs))

        self._c.executemany(
            'INSERT OR IGNORE INTO tag_siblings ( service_id, bad_tag_id, good_tag_id, status ) VALUES ( ?, ?, ?, ? );',
            ((service_id, bad_tag_id, good_tag_id, HC.CONTENT_STATUS_DELETED)
             for (bad_tag_id, good_tag_id) in pairs))
    ...
    def _DeleteYAMLDump(self, dump_type, dump_name=None):
        ...
        if dump_name is None:
            ...
            self._c.execute('DELETE FROM yaml_dumps WHERE dump_type = ?;',
                            (dump_type, ))
            ...
            self._c.execute(
                'DELETE FROM yaml_dumps WHERE dump_type = ? AND dump_name = ?;',
                (dump_type, dump_name))
        ...
        if dump_type == YAML_DUMP_ID_LOCAL_BOORU:
            ...
            self._c.execute(
                'DELETE FROM service_info WHERE service_id = ? AND info_type = ?;',
                (service_id, HC.SERVICE_INFO_NUM_SHARES))
    ...
    def _DuplicatesAddPotentialDuplicates(
        self, media_id, potential_duplicate_media_ids_and_distances):

        inserts = []

        for (potential_duplicate_media_id,
             distance) in potential_duplicate_media_ids_and_distances:

            if potential_duplicate_media_id == media_id:  # already duplicates!

                continue

            if self._DuplicatesMediasAreFalsePositive(
                    media_id, potential_duplicate_media_id):

                continue

            if self._DuplicatesMediasAreConfirmedAlternates(
                    media_id, potential_duplicate_media_id):
        ...
        if len(inserts) > 0:
            ...
            self._c.executemany(
                'INSERT OR IGNORE INTO potential_duplicate_pairs ( smaller_media_id, larger_media_id, distance ) VALUES ( ?, ?, ? );',
                inserts)

    def _DuplicatesAlternatesGroupsAreFalsePositive(self,
                                                    alternates_group_id_a,
                                                    alternates_group_id_b):

        if alternates_group_id_a == alternates_group_id_b:

            return False

        smaller_alternates_group_id = min(alternates_group_id_a,
                                          alternates_group_id_b)
        larger_alternates_group_id = max(alternates_group_id_a,
                                         alternates_group_id_b)

        result = self._c.execute(
            'SELECT 1 FROM duplicate_false_positives WHERE smaller_alternates_group_id = ? AND larger_alternates_group_id = ?;',
            (alternates_group_id_a, larger_alternates_group_id)).fetchone()
    ...
    def _DuplicatesClearAllFalsePositiveRelations(self, alternates_group_id):
        ...
        self._c.execute(
            'DELETE FROM duplicate_false_positives WHERE smaller_alternates_group_id = ? OR larger_alternates_group_id = ?;',
            (alternates_group_id, alternates_group_id))
    ...
    def _DuplicatesClearAllFalsePositiveRelationsFromHashes(self, hashes):
        ...
        for hash_id in hash_ids:
            ...
            if media_id is not None:
                ...
                alternates_group_id = self._DuplicatesGetAlternatesGroupId(
                    media_id, do_not_create=True)

                if alternates_group_id is not None:

                    self._DuplicatesClearAllFalsePositiveRelations(
                        alternates_group_id)

    def _DuplicatesClearFalsePositiveRelationsBetweenGroups(
        self, alternates_group_ids):
        ...
        for (alternates_group_id_a, alternates_group_id_b) in pairs:
            ...
            self._c.execute(
                'DELETE FROM duplicate_false_positives WHERE smaller_alternates_group_id = ? AND larger_alternates_group_id = ?;',
                (smaller_alternates_group_id, larger_alternates_group_id))

        for alternates_group_id in alternates_group_ids:

            media_ids = self._DuplicatesGetAlternateMediaIds(
                alternates_group_id)
    ...
    def _DuplicatesClearFalsePositiveRelationsBetweenGroupsFromHashes(
        self, hashes):
        ...
        if len(alternates_group_ids) > 1:
            ...
            self._DuplicatesClearFalsePositiveRelationsBetweenGroups(
                alternates_group_ids)

    def _DuplicatesClearPotentialsBetweenMedias(self, media_ids_a,
                                                media_ids_b):
        ...
        potential_duplicate_pairs.update(
            self._ExecuteManySelectSingleParam(
                'SELECT smaller_media_id, larger_media_id FROM potential_duplicate_pairs WHERE smaller_media_id = ?;',
                all_media_ids))
        potential_duplicate_pairs.update(
            self._ExecuteManySelectSingleParam(
                'SELECT smaller_media_id, larger_media_id FROM potential_duplicate_pairs WHERE larger_media_id = ?;',
                all_media_ids))
        ...
        for (smaller_media_id, larger_media_id) in potential_duplicate_pairs:
            ...
            if (smaller_media_id in media_ids_a
                    and larger_media_id in media_ids_b) or (
                        smaller_media_id in media_ids_b
                        and larger_media_id in media_ids_a):
        ...
        if len(deletees) > 0:
            ...
            self._c.executemany(
                'DELETE FROM potential_duplicate_pairs WHERE smaller_media_id = ? AND larger_media_id = ?;',
                deletees)

    def _DuplicatesClearPotentialsBetweenAlternatesGroups(
        self, alternates_group_id_a, alternates_group_id_b):

        # these groups are being set as false positive. therefore, any potential between them no longer applies

        media_ids_a = self._DuplicatesGetAlternateMediaIds(
            alternates_group_id_a)
        media_ids_b = self._DuplicatesGetAlternateMediaIds(
            alternates_group_id_b)
    ...
    def _DuplicatesDeleteAllPotentialDuplicatePairs(self):
        ...
        for (smaller_media_id, larger_media_id) in self._c.execute(
                'SELECT smaller_media_id, larger_media_id FROM potential_duplicate_pairs;'
        ):
        ...
        for hash_id in hash_ids:
            ...
            if media_id is not None:
                ...
                if alternates_group_id is not None:
                    ...
                    self._DuplicatesDissolveAlternatesGroupId(
                        alternates_group_id)
    ...
    def _DuplicatesDissolveMediaId(self, media_id):
        ...
        self._c.execute(
            'DELETE FROM potential_duplicate_pairs WHERE smaller_media_id = ? OR larger_media_id = ?;',
            (media_id, media_id))

        hash_ids = self._DuplicatesGetDuplicateHashIds(media_id)

        self._c.execute(
            'DELETE FROM duplicate_file_members WHERE media_id = ?;',
            (media_id, ))
        self._c.execute('DELETE FROM duplicate_files WHERE media_id = ?;',
                        (media_id, ))
    ...
    def _DuplicatesFilterKingHashIds(self, allowed_hash_ids):
        ...
        explicit_king_hash_ids = self._STS(
            self._ExecuteManySelectSingleParam(query, allowed_hash_ids))

        query = 'SELECT hash_id FROM duplicate_file_members WHERE hash_id = ?;'

        all_duplicate_member_hash_ids = self._STS(
            self._ExecuteManySelectSingleParam(query, allowed_hash_ids))

        all_non_king_hash_ids = all_duplicate_member_hash_ids.difference(
            explicit_king_hash_ids)
    ...
    def _DuplicatesGetAlternatesGroupId(self, media_id, do_not_create=False):
        ...
        result = self._c.execute(
            'SELECT alternates_group_id FROM alternate_file_group_members WHERE media_id = ?;',
            (media_id, )).fetchone()
        ...
        if result is None:
            ...
            self._c.execute(
                'INSERT INTO alternate_file_groups DEFAULT VALUES;')

            alternates_group_id = self._c.lastrowid

            self._c.execute(
                'INSERT INTO alternate_file_group_members ( alternates_group_id, media_id ) VALUES ( ?, ? );',
                (alternates_group_id, media_id))
    ...
    def _DuplicatesGetAlternateMediaIds(self, alternates_group_id):
        ...
        media_ids = self._STS(
            self._c.execute(
                'SELECT media_id FROM alternate_file_group_members WHERE alternates_group_id = ?;',
                (alternates_group_id, )))

        return media_ids

    def _DuplicatesGetBestKingId(self,
                                 media_id,
                                 file_service_id,
                                 allowed_hash_ids=None,
                                 preferred_hash_ids=None):

        media_hash_ids = self._DuplicatesGetDuplicateHashIds(
            media_id, file_service_id=file_service_id)
        ...
        if len(media_hash_ids) > 0:
            ...
            if preferred_hash_ids is not None:
                ...
                preferred_hash_ids = media_hash_ids.intersection(
                    preferred_hash_ids)
    ...
    def _DuplicatesGetDuplicateHashIds(self, media_id, file_service_id=None):
        ...
        if file_service_id is None or file_service_id == self._combined_file_service_id:
            ...
            hash_ids = self._STS(
                self._c.execute(
                    'SELECT hash_id FROM duplicate_file_members WHERE media_id = ?;',
                    (media_id, )))
            ...
            hash_ids = self._STS(
                self._c.execute(
                    'SELECT hash_id FROM duplicate_file_members NATURAL JOIN current_files WHERE media_id = ? AND service_id = ?;',
                    (media_id, file_service_id)))

        return hash_ids

    def _DuplicatesGetFalsePositiveAlternatesGroupIds(self,
                                                      alternates_group_id):

        false_positive_alternates_group_ids = set()

        results = self._c.execute(
            'SELECT smaller_alternates_group_id, larger_alternates_group_id FROM duplicate_false_positives WHERE smaller_alternates_group_id = ? OR larger_alternates_group_id = ?;',
            (alternates_group_id, alternates_group_id)).fetchall()

        for (smaller_alternates_group_id,
             larger_alternates_group_id) in results:

            false_positive_alternates_group_ids.add(
    ...
    def _DuplicatesGetFileDuplicateInfo(self, file_service_key, hash):
        ...
        if media_id is not None:
            ...
            (
                table_join, predicate_string
            ) = self._DuplicatesGetPotentialDuplicatePairsTableJoinInfoOnFileService(
                file_service_key)

            (num_potentials, ) = self._c.execute(
                'SELECT COUNT( * ) FROM ( SELECT DISTINCT smaller_media_id, larger_media_id FROM '
                + table_join + ' WHERE ' + predicate_string +
                ' AND ( smaller_media_id = ? OR larger_media_id = ? ) );', (
                    media_id,
                    media_id,
                )).fetchone()
            ...
            if alternates_group_id is not None:
                ...
                alt_media_ids = self._DuplicatesGetAlternateMediaIds(
                    alternates_group_id)
                ...
                for alt_media_id in alt_media_ids:
                    ...
                    alt_hash_ids = self._DuplicatesGetDuplicateHashIds(
                        alt_media_id, file_service_id=file_service_id)
                    ...
                    if len(alt_hash_ids) > 0:
                        ...
                        result = self._c.execute(
                            'SELECT 1 FROM confirmed_alternate_pairs WHERE smaller_media_id = ? AND larger_media_id = ?;',
                            (smaller_media_id, larger_media_id)).fetchone()

                        if result is not None:

                            counter[HC.DUPLICATE_CONFIRMED_ALTERNATE] += 1

                false_positive_alternates_group_ids = self._DuplicatesGetFalsePositiveAlternatesGroupIds(
                    alternates_group_id)

                false_positive_alternates_group_ids.discard(
                    alternates_group_id)

                for false_positive_alternates_group_id in false_positive_alternates_group_ids:

                    fp_media_ids = self._DuplicatesGetAlternateMediaIds(
                        false_positive_alternates_group_id)

                    for fp_media_id in fp_media_ids:

                        fp_hash_ids = self._DuplicatesGetDuplicateHashIds(
                            fp_media_id, file_service_id=file_service_id)
    ...
    def _DuplicatesGetFileHashesByDuplicateType(self,
                                                file_service_key,
                                                hash,
                                                duplicate_type,
                                                allowed_hash_ids=None,
        ...
        if duplicate_type == HC.DUPLICATE_FALSE_POSITIVE:
            ...
            if media_id is not None:
                ...
                if alternates_group_id is not None:
                    ...
                    for false_positive_alternates_group_id in false_positive_alternates_group_ids:
                        ...
                        false_positive_media_ids.update(
                            self._DuplicatesGetAlternateMediaIds(
                                false_positive_alternates_group_id))

                    for false_positive_media_id in false_positive_media_ids:

                        best_king_hash_id = self._DuplicatesGetBestKingId(
                            false_positive_media_id,
                            file_service_id,
                            allowed_hash_ids=allowed_hash_ids,
                            preferred_hash_ids=preferred_hash_ids)
            ...
            if media_id is not None:
                ...
                if alternates_group_id is not None:
                    ...
                    alternates_media_ids = self._STS(
                        self._c.execute(
                            'SELECT media_id FROM alternate_file_group_members WHERE alternates_group_id = ?;',
                            (alternates_group_id, )))
            ...
            if media_id is not None:
                ...
                for (smaller_media_id, larger_media_id) in self._c.execute(
                        'SELECT smaller_media_id, larger_media_id FROM ' +
                        table_join + ' WHERE ' + predicate_string +
                        ' AND ( smaller_media_id = ? OR larger_media_id = ? );',
    ...
    def _DuplicatesGetHashIdsFromDuplicateCountPredicate(
        self, file_service_key, operator, num_relationships, dupe_type):
        ...
        if dupe_type == HC.DUPLICATE_FALSE_POSITIVE:
            ...
            alternates_group_ids_to_false_positives = collections.defaultdict(
                list)

            query = 'SELECT smaller_alternates_group_id, larger_alternates_group_id FROM duplicate_false_positives;'

            for (alternates_group_id_a,
                 alternates_group_id_b) in self._c.execute(query):

                alternates_group_ids_to_false_positives[
                    alternates_group_id_a].append(alternates_group_id_b)
                alternates_group_ids_to_false_positives[
                    alternates_group_id_b].append(alternates_group_id_a)

            for (alternates_group_id, false_positive_alternates_group_ids
                 ) in alternates_group_ids_to_false_positives.items():
                ...
                for false_positive_alternates_group_id in false_positive_alternates_group_ids:
                    ...
                    if false_positive_alternates_group_id not in alternates_group_ids_to_valid_for_file_domain:
                        ...
                        alternates_group_ids_to_valid_for_file_domain[
                            false_positive_alternates_group_id] = valid

                    if alternates_group_ids_to_valid_for_file_domain[
                            false_positive_alternates_group_id]:
                ...
                if filter_func(count):
                    ...
                    for media_id in media_ids:
                        ...
                        hash_ids.update(
                            self._DuplicatesGetDuplicateHashIds(
                                media_id, file_service_id=file_service_id))
            ...
            for (alternates_group_id, count) in results:
                ...
                for media_id in media_ids:
                    ...
                    media_id_hash_ids = self._DuplicatesGetDuplicateHashIds(
                        media_id, file_service_id=file_service_id)
            ...
            query = 'SELECT media_id, COUNT( * ) FROM {} WHERE {} GROUP BY media_id;'.format(
                table_join, predicate_string)
            ...
            media_ids = [
                media_id for (media_id, count) in media_ids_to_counts.items()
                if filter_func(count)
            ]
    ...
    def _DuplicatesGetKingHashId(self, media_id):
        ...
        (king_hash_id, ) = self._c.execute(
            'SELECT king_hash_id FROM duplicate_files WHERE media_id = ?;',
            (media_id, )).fetchone()
    ...
    def _DuplicatesGetMediaId(self, hash_id, do_not_create=False):
        ...
        result = self._c.execute(
            'SELECT media_id FROM duplicate_file_members WHERE hash_id = ?;',
            (hash_id, )).fetchone()
        ...
        if result is None:
            ...
            self._c.execute(
                'INSERT INTO duplicate_files ( king_hash_id ) VALUES ( ? );',
                (hash_id, ))

            media_id = self._c.lastrowid

            self._c.execute(
                'INSERT INTO duplicate_file_members ( media_id, hash_id ) VALUES ( ?, ? );',
                (media_id, hash_id))
    ...
    def _DuplicatesGetPotentialDuplicatePairsTableJoinInfoOnFileService(
        ...
        if file_service_key == CC.COMBINED_FILE_SERVICE_KEY:
            ...
            predicate_string = 'current_files_smaller.service_id = {} AND current_files_larger.service_id = {}'.format(
                service_id, service_id)

        return (table_join, predicate_string)

    def _DuplicatesGetPotentialDuplicatePairsTableJoinInfoOnSearchResults(
        self, file_service_key, results_table_name, both_files_match):

        if both_files_match:

            table_join = 'potential_duplicate_pairs, duplicate_file_members AS duplicate_file_members_smaller, {} AS results_smaller, duplicate_file_members AS duplicate_file_members_larger, {} AS results_larger ON ( smaller_media_id = duplicate_file_members_smaller.media_id AND duplicate_file_members_smaller.hash_id = results_smaller.hash_id AND larger_media_id = duplicate_file_members_larger.media_id AND duplicate_file_members_larger.hash_id = results_larger.hash_id )'.format(
    ...
    def _DuplicatesGetRandomPotentialDuplicateHashes(self, search_context,
        ...
        with HydrusDB.TemporaryIntegerTable(self._c, [],
                                            'hash_id') as temp_table_name:
            ...
            if search_context.IsJustSystemEverything(
            ) or search_context.HasNoPredicates():
                ...
                query_hash_ids = self._GetHashIdsFromQuery(
                    search_context, apply_implicit_limit=False)
                ...
                self._c.executemany(
                    'INSERT OR IGNORE INTO {} ( hash_id ) VALUES ( ? );'.
                    format(temp_table_name),
                    ((hash_id, ) for hash_id in query_hash_ids))

                self._AnalyzeTempTable(temp_table_name)

                (
                    table_join, predicate_string
                ) = self._DuplicatesGetPotentialDuplicatePairsTableJoinInfoOnSearchResults(
                    file_service_key, temp_table_name, both_files_match)

            potential_media_ids = set()

            # distinct important here for the search results table join
            for (smaller_media_id, larger_media_id) in self._c.execute(
                    'SELECT DISTINCT smaller_media_id, larger_media_id FROM ' +
                    table_join + ' WHERE ' + predicate_string + ';'):
        ...
        return self._DuplicatesGetFileHashesByDuplicateType(
            file_service_key,
            hash,
    ...
    def _DuplicatesGetPotentialDuplicatePairsForFiltering(
        self, search_context, both_files_match):
        ...
        with HydrusDB.TemporaryIntegerTable(self._c, [],
            ...
            # distinct important here for the search results table join
            result = self._c.execute(
                'SELECT DISTINCT smaller_media_id, larger_media_id, distance FROM '
                + table_join + ' WHERE ' + predicate_string +
                ' LIMIT 2500;').fetchall()

        MAX_BATCH_SIZE = HG.client_controller.new_options.GetInteger(
            'duplicate_filter_max_batch_size')

        batch_of_pairs_of_media_ids = []
        seen_media_ids = set()

        distances_to_pairs = HydrusData.BuildKeyToListDict(
            ((distance, (smaller_media_id, larger_media_id))
             for (smaller_media_id, larger_media_id, distance) in result))
        ...
        for distance in distances:
            ...
            for (master_media_id,
                 count) in master_hash_ids_to_values.most_common():
                ...
                for pair in master_media_ids_to_groups[master_media_id]:
                    ...
                    seen_media_ids_for_this_master_media_id.add(
                        smaller_media_id)
                    seen_media_ids_for_this_master_media_id.add(
                        larger_media_id)
        ...
        batch_of_pairs_of_hash_ids = [
            (media_ids_to_best_king_ids[smaller_media_id],
             media_ids_to_best_king_ids[larger_media_id])
            for (smaller_media_id,
                 larger_media_id) in batch_of_pairs_of_media_ids
            if smaller_media_id in media_ids_to_best_king_ids
            and larger_media_id in media_ids_to_best_king_ids
        ]

        self._PopulateHashIdsToHashesCache(seen_hash_ids)

        batch_of_pairs_of_hashes = [
            (self._hash_ids_to_hashes_cache[hash_id_a],
             self._hash_ids_to_hashes_cache[hash_id_b])
            for (hash_id_a, hash_id_b) in batch_of_pairs_of_hash_ids
        ]

        return batch_of_pairs_of_hashes

    def _DuplicatesGetPotentialDuplicatesCount(self, search_context,
                                               both_files_match):
        ...
        with HydrusDB.TemporaryIntegerTable(self._c, [],
            ...
            # distinct important here for the search results table join
            (potential_duplicates_count, ) = self._c.execute(
                'SELECT COUNT( * ) FROM ( SELECT DISTINCT smaller_media_id, larger_media_id FROM '
                + table_join + ' WHERE ' + predicate_string +
                ' );', ).fetchone()
    ...
    def _DuplicatesMediasAreAlternates(self, media_id_a, media_id_b):
        ...
        alternates_group_id_a = self._DuplicatesGetAlternatesGroupId(
            media_id_a, do_not_create=True)

        if alternates_group_id_a is None:

            return False

        alternates_group_id_b = self._DuplicatesGetAlternatesGroupId(
            media_id_b, do_not_create=True)
    ...
    def _DuplicatesMediasAreFalsePositive(self, media_id_a, media_id_b):
        ...
        return self._DuplicatesAlternatesGroupsAreFalsePositive(
            alternates_group_id_a, alternates_group_id_b)
    ...
    def _DuplicatesMergeMedias(self, superior_media_id, mergee_media_id):
        ...
        self._DuplicatesClearPotentialsBetweenMedias((superior_media_id, ),
                                                     (mergee_media_id, ))

        alternates_group_id = self._DuplicatesGetAlternatesGroupId(
            superior_media_id)
        mergee_alternates_group_id = self._DuplicatesGetAlternatesGroupId(
            mergee_media_id)

        if alternates_group_id != mergee_alternates_group_id:

            if self._DuplicatesAlternatesGroupsAreFalsePositive(
                    alternates_group_id, mergee_alternates_group_id):

                smaller_alternates_group_id = min(alternates_group_id,
                                                  mergee_alternates_group_id)
                larger_alternates_group_id = max(alternates_group_id,
                                                 mergee_alternates_group_id)
        ...
        self._c.execute(
            'UPDATE duplicate_file_members SET media_id = ? WHERE media_id = ?;',
            (superior_media_id, mergee_media_id))
        ...
        existing_potential_info_of_mergee_media_id = self._c.execute(
            'SELECT smaller_media_id, larger_media_id, distance FROM potential_duplicate_pairs WHERE smaller_media_id = ? OR larger_media_id = ?;',
            (mergee_media_id, mergee_media_id)).fetchall()

        self._c.execute(
            'DELETE FROM potential_duplicate_pairs WHERE smaller_media_id = ? OR larger_media_id = ?;',
            (mergee_media_id, mergee_media_id))

        for (smaller_media_id, larger_media_id,
             distance) in existing_potential_info_of_mergee_media_id:
            ...
            potential_duplicate_media_ids_and_distances = [(media_id_b,
                                                            distance)]

            self._DuplicatesAddPotentialDuplicates(
                media_id_a, potential_duplicate_media_ids_and_distances)

        # ensure any previous confirmed alt pair is gone

        self._c.execute(
            'DELETE FROM confirmed_alternate_pairs WHERE smaller_media_id = ? AND larger_media_id = ?;',
            (smaller_media_id, larger_media_id))

        # now merge confirmed alts from the old to the new

        self._c.execute(
            'UPDATE OR IGNORE confirmed_alternate_pairs SET smaller_media_id = ? WHERE smaller_media_id = ?;',
            (superior_media_id, mergee_media_id))
        self._c.execute(
            'UPDATE OR IGNORE confirmed_alternate_pairs SET larger_media_id = ? WHERE larger_media_id = ?;',
            (superior_media_id, mergee_media_id))

        # and clear out potentials that are now invalid

        confirmed_alternate_pairs = self._c.execute(
            'SELECT smaller_media_id, larger_media_id FROM confirmed_alternate_pairs WHERE smaller_media_id = ? OR larger_media_id = ?;',
            (superior_media_id, superior_media_id)).fetchall()
        ...
        self._c.execute(
            'DELETE FROM alternate_file_group_members WHERE media_id = ?;',
            (mergee_media_id, ))
    ...
    def _DuplicatesRemoveAlternateMember(self, media_id):
        ...
        if alternates_group_id is not None:
            ...
            alternates_media_ids = self._DuplicatesGetAlternateMediaIds(
                alternates_group_id)
            ...
            self._c.execute(
                'DELETE FROM confirmed_alternate_pairs WHERE smaller_media_id = ? OR larger_media_id = ?;',
                (media_id, media_id))

            if len(
                    alternates_media_ids
            ) == 1:  # i.e. what we just removed was the last of the group

                self._c.execute(
                    'DELETE FROM alternate_file_groups WHERE alternates_group_id = ?;',
                    (alternates_group_id, ))
    ...
    def _DuplicatesRemoveMediaIdMember(self, hash_id):
        ...
        if media_id is not None:
            ...
            if hash_id == king_hash_id:
                ...
                self._c.execute(
                    'DELETE FROM duplicate_file_members WHERE hash_id = ?;',
                    (hash_id, ))
    ...
    def _DuplicatesSetAlternates(self, media_id_a, media_id_b):
        ...
        self._DuplicatesClearPotentialsBetweenMedias((media_id_a, ),
                                                     (media_id_b, ))
        ...
        self._c.execute(
            'INSERT OR IGNORE INTO confirmed_alternate_pairs ( smaller_media_id, larger_media_id ) VALUES ( ?, ? );',
            (smaller_media_id, larger_media_id))
        ...
        false_positive_pairs = self._c.execute(
            'SELECT smaller_alternates_group_id, larger_alternates_group_id FROM duplicate_false_positives WHERE smaller_alternates_group_id = ? OR larger_alternates_group_id = ?;',
            (alternates_group_id_a, alternates_group_id_a))

        for (smaller_false_positive_alternates_group_id,
             larger_false_positive_alternates_group_id
             ) in false_positive_pairs:

            if smaller_false_positive_alternates_group_id == alternates_group_id_a:

                self._DuplicatesClearPotentialsBetweenAlternatesGroups(
                    alternates_group_id_b,
                    larger_false_positive_alternates_group_id)
                ...
                self._DuplicatesClearPotentialsBetweenAlternatesGroups(
                    smaller_false_positive_alternates_group_id,
                    alternates_group_id_b)

        # first, update all B to A

        self._c.execute(
            'UPDATE alternate_file_group_members SET alternates_group_id = ? WHERE alternates_group_id = ?;',
            (alternates_group_id_a, alternates_group_id_b))
        ...
        false_positive_pairs = self._c.execute(
            'SELECT smaller_alternates_group_id, larger_alternates_group_id FROM duplicate_false_positives WHERE smaller_alternates_group_id = ? OR larger_alternates_group_id = ?;',
            (alternates_group_id_b, alternates_group_id_b))
        ...
        for (smaller_false_positive_alternates_group_id,
            ...
            if smaller_false_positive_alternates_group_id == alternates_group_id_b:
                ...
                self._DuplicatesSetFalsePositive(
                    alternates_group_id_a,
                    larger_false_positive_alternates_group_id)
    ...
    def _DuplicatesSetDuplicatePairStatus(self, pair_info):
        ...
        for (duplicate_type, hash_a, hash_b,
             service_keys_to_content_updates) in pair_info:
            ...
            if duplicate_type in (HC.DUPLICATE_FALSE_POSITIVE,
                ...
                if duplicate_type == HC.DUPLICATE_FALSE_POSITIVE:
                    ...
                    self._DuplicatesSetFalsePositive(alternates_group_id_a,
                                                     alternates_group_id_b)
                ...
                if duplicate_type == HC.DUPLICATE_BETTER:
                    ...
                    if media_id_a != media_id_b:
                        ...
                        self._DuplicatesMergeMedias(superior_media_id,
                                                    mergee_media_id)
    ...
    def _DuplicatesSetFalsePositive(self, alternates_group_id_a,
                                    alternates_group_id_b):
        ...
        self._c.execute(
            'INSERT OR IGNORE INTO duplicate_false_positives ( smaller_alternates_group_id, larger_alternates_group_id ) VALUES ( ?, ? );',
            (smaller_alternates_group_id, larger_alternates_group_id))

    def _DuplicatesSetKing(self, king_hash_id, media_id):

        self._c.execute(
            'UPDATE duplicate_files SET king_hash_id = ? WHERE media_id = ?;',
            (king_hash_id, media_id))
    ...
    def _FileMaintenanceAddJobs(self, hash_ids, job_type, time_can_start=0):
        ...
        deletee_job_types = ClientFiles.regen_file_enum_to_overruled_jobs[
            job_type]

        for deletee_job_type in deletee_job_types:

            self._c.executemany(
                'DELETE FROM file_maintenance_jobs WHERE hash_id = ? AND job_type = ?;',
                ((hash_id, deletee_job_type) for hash_id in hash_ids))

        #

        self._c.executemany(
            'REPLACE INTO file_maintenance_jobs ( hash_id, job_type, time_can_start ) VALUES ( ?, ?, ? );',
            ((hash_id, job_type, time_can_start) for hash_id in hash_ids))

    def _FileMaintenanceAddJobsHashes(self,
                                      hashes,
                                      job_type,
                                      time_can_start=0):
        ...
        self._FileMaintenanceAddJobs(hash_ids,
                                     job_type,
                                     time_can_start=time_can_start)

    def _FileMaintenanceCancelJobs(self, job_type):

        self._c.execute(
            'DELETE FROM file_maintenance_jobs WHERE job_type = ?;',
            (job_type, ))
    ...
    def _FileMaintenanceClearJobs(self, cleared_job_tuples):
        ...
        for (hash, job_type, additional_data) in cleared_job_tuples:
            ...
            if additional_data is not None:
                ...
                if job_type == ClientFiles.REGENERATE_FILE_DATA_JOB_FILE_METADATA:
                    ...
                    (size, mime, width, height, duration, num_frames,
                     has_audio, num_words) = additional_data

                    self._c.execute(
                        'UPDATE files_info SET size = ?, mime = ?, width = ?, height = ?, duration = ?, num_frames = ?, has_audio = ?, num_words = ? WHERE hash_id = ?;',
                        (size, mime, width, height, duration, num_frames,
                         has_audio, num_words, hash_id))
                    ...
                    if mime not in HC.HYDRUS_UPDATE_FILES:
                        ...
                        result = self._c.execute(
                            'SELECT 1 FROM local_hashes WHERE hash_id = ?;',
                            (hash_id, )).fetchone()

                        if result is None:

                            self._FileMaintenanceAddJobs(
                                {hash_id}, ClientFiles.
                                REGENERATE_FILE_DATA_JOB_OTHER_HASHES)

                        result = self._c.execute(
                            'SELECT 1 FROM file_modified_timestamps WHERE hash_id = ?;',
                            (hash_id, )).fetchone()
                    ...
                    self._c.execute(
                        'INSERT OR IGNORE INTO local_hashes ( hash_id, md5, sha1, sha512 ) VALUES ( ?, ?, ?, ? );',
                        (hash_id, sqlite3.Binary(md5), sqlite3.Binary(sha1),
                         sqlite3.Binary(sha512)))
                    ...
                    self._c.execute(
                        'REPLACE INTO file_modified_timestamps ( hash_id, file_modified_timestamp ) VALUES ( ?, ? );',
                        (hash_id, file_modified_timestamp))
            ...
            job_types_to_delete.extend(
                ClientFiles.regen_file_enum_to_overruled_jobs[job_type])

            self._c.executemany(
                'DELETE FROM file_maintenance_jobs WHERE hash_id = ? AND job_type = ?;',
                ((hash_id, job_type_to_delete)
                 for job_type_to_delete in job_types_to_delete))
        ...
        if len(new_file_info) > 0:
            ...
            for (hash_id, hash) in new_file_info:
                ...
                if self._weakref_media_result_cache.HasFile(hash_id):
                    ...
                    self._weakref_media_result_cache.DropMediaResult(
                        hash_id, hash)
    ...
    def _FileMaintenanceGetJob(self, job_types=None):
        ...
        for job_type in possible_job_types:
            ...
            hash_ids = self._STL(
                self._c.execute(
                    'SELECT hash_id FROM file_maintenance_jobs WHERE job_type = ? AND time_can_start < ? LIMIT ?;',
                    (job_type, HydrusData.GetNow(), 256)))
    ...
    def _FileMaintenanceGetJobCounts(self):
        ...
        result = self._c.execute(
            'SELECT job_type, COUNT( * ) FROM file_maintenance_jobs WHERE time_can_start < ? GROUP BY job_type;',
            (HydrusData.GetNow(), )).fetchall()

        job_types_to_count = dict(result)

        return job_types_to_count

    def _FillInParents(self,
                       service_id,
                       fill_in_tag_id,
                       make_content_updates=False):
        ...
        while len(search_tag_ids) > 0:
            ...
            for (tag_id_a, tag_id_b) in self._c.execute(
                    'SELECT bad_tag_id, good_tag_id FROM tag_siblings WHERE ( bad_tag_id = ? OR good_tag_id = ? ) AND status = ?;',
        ...
        while len(search_tag_ids) > 0:
            ...
            for parent_tag_id in self._STI(
                    self._c.execute(
                        'SELECT parent_tag_id FROM tag_parents WHERE child_tag_id = ? AND status = ?;',
                        (tag_id, HC.CONTENT_STATUS_CURRENT))):
        ...
        child_hash_ids = self._STS(
            self._ExecuteManySelectSingleParam(
                'SELECT hash_id FROM ' + current_mappings_table_name +
                ' WHERE tag_id = ?;', sibling_tag_ids))

        for parent_tag_id in parent_tag_ids:

            parent_hash_ids = self._STS(
                self._c.execute(
                    'SELECT hash_id FROM ' + current_mappings_table_name +
        ...
        if len(content_updates) > 0:
            ...
            self.pub_content_updates_after_commit(
    ...
    def _FilterExistingTags(self, service_key, tags):
        ...
        counts = self._CacheCombinedFilesMappingsGetAutocompleteCounts(
            service_id, list(tag_ids_to_tags.keys()))

        existing_tag_ids = [
            tag_id for (tag_id, current_count, pending_count) in counts
            if current_count > 0
        ]

        filtered_tags = {
            tag_ids_to_tags[tag_id]
    ...
    def _FilterHashes(self, hashes, file_service_key):
        ...
        for hash in hashes:
            ...
            result = self._c.execute(
                'SELECT 1 FROM current_files WHERE hash_id = ? AND service_id = ?;',
                (hash_id, service_id)).fetchone()
    ...
    def _GenerateMappingsTables(self, service_id):
        ...
        self._c.execute(
            'CREATE TABLE IF NOT EXISTS ' + current_mappings_table_name +
            ' ( tag_id INTEGER, hash_id INTEGER, PRIMARY KEY ( tag_id, hash_id ) ) WITHOUT ROWID;'
        )
        self._CreateIndex(current_mappings_table_name, ['hash_id', 'tag_id'],
                          unique=True)

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS ' + deleted_mappings_table_name +
            ' ( tag_id INTEGER, hash_id INTEGER, PRIMARY KEY ( tag_id, hash_id ) ) WITHOUT ROWID;'
        )
        self._CreateIndex(deleted_mappings_table_name, ['hash_id', 'tag_id'],
                          unique=True)

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS ' + pending_mappings_table_name +
            ' ( tag_id INTEGER, hash_id INTEGER, PRIMARY KEY ( tag_id, hash_id ) ) WITHOUT ROWID;'
        )
        self._CreateIndex(pending_mappings_table_name, ['hash_id', 'tag_id'],
                          unique=True)

        self._c.execute(
            'CREATE TABLE IF NOT EXISTS ' + petitioned_mappings_table_name +
            ' ( tag_id INTEGER, hash_id INTEGER, reason_id INTEGER, PRIMARY KEY ( tag_id, hash_id ) ) WITHOUT ROWID;'
        )
    ...
    def _GetAutocompleteCounts(self, tag_service_id, file_service_id, tag_ids,
                               include_current, include_pending):
        ...
        if file_service_id == self._combined_file_service_id:
            ...
            cache_results = self._CacheCombinedFilesMappingsGetAutocompleteCounts(
                tag_service_id, tag_ids)
            ...
            for search_tag_service_id in search_tag_service_ids:
                ...
                cache_results.extend(
                    self._CacheSpecificMappingsGetAutocompleteCounts(
                        file_service_id, search_tag_service_id, tag_ids))
        ...
        for (tag_id, current_count, pending_count) in cache_results:
            ...
            if tag_id in ids_to_count:
                ...
                (current_min, current_max, pending_min,
                 pending_max) = ids_to_count[tag_id]

                (current_min,
                 current_max) = ClientData.MergeCounts(current_min,
                                                       current_max,
                                                       current_count, None)
                (pending_min,
                 pending_max) = ClientData.MergeCounts(pending_min,
                                                       pending_max,
                                                       pending_count, None)
                ...
                (current_min, current_max, pending_min,
                 pending_max) = (current_count, None, pending_count, None)
    ...
    def _GetAutocompleteTagIds(self,
                               service_key,
                               search_text,
                               exact_match,
                               job_key=None):
        ...
        if exact_match:
            ...
            if namespace != '':
                ...
                if half_complete_subtag not in ('*', ''):
                    ...
                    (t_j, pred,
                     param) = GetSubTagSearchInfo(half_complete_subtag)
                ...
                if ClientSearch.ConvertTagToSearchable(
                        half_complete_subtag) in ('', '*'):
        ...
        query = 'SELECT DISTINCT tag_id FROM {} WHERE {};'.format(
            table_join, predicates_phrase)
        ...
        for group_of_tag_ids in HydrusData.SplitIteratorIntoChunks(
                tag_ids, 1024):
            ...
            group_of_sibling_tag_ids = self._GetTagSiblingIds(
                sibling_service_id, group_of_tag_ids)
    ...
    def _GetAutocompletePredicates(
        ...
        tag_ids = self._GetAutocompleteTagIds(tag_service_key,
                                              search_text,
                                              exact_match,
                                              job_key=job_key)
        ...
        for search_tag_service_id in search_tag_service_ids:
            ...
            for group_of_tag_ids in HydrusData.SplitIteratorIntoChunks(
                ...
                search_tag_service_key = self._GetService(
                    search_tag_service_id).GetServiceKey()

                ids_to_count = self._GetAutocompleteCounts(
                    search_tag_service_id, file_service_id, group_of_tag_ids,
                    include_current, include_pending)
                ...
                tags_and_counts_generator = ((self._tag_ids_to_tags_cache[id],
                                              ids_to_count[id])
                                             for id in ids_to_count.keys())
                ...
                predicates = [
                    ...
                    for (tag, (min_current_count, max_current_count,
                               min_pending_count,
                ...
                if collapse_siblings:
                    ...
                    predicates = siblings_manager.CollapsePredicates(
                        search_tag_service_key, predicates)
        ...
        predicates = ClientData.MergePredicates(
            all_predicates, add_namespaceless=add_namespaceless)
    ...
    def _GetTableNamesDueAnalysis(self, force_reanalyze=False):
        ...
        if force_reanalyze:
            ...
            existing_names_to_info = {
                name: (num_rows, timestamp)
                for (name, num_rows, timestamp) in self._c.execute(
                    'SELECT name, num_rows, timestamp FROM analyze_timestamps;'
                )
            }
            ...
            for name in all_names:
                ...
                if name in existing_names_to_info:
                    ...
                    for (row_limit_for_this_boundary, can_analyze_now,
                        ...
                        if HydrusData.TimeHasPassed(timestamp + period):
                            ...
                            if can_analyze_now:
                                ...
                                if self._TableHasAtLeastRowCount(
                                        name, row_limit_for_this_boundary):
    ...
    def _GetBonedStats(self):
        ...
        (num_total, size_total) = self._c.execute(
            'SELECT COUNT( hash_id ), SUM( size ) FROM files_info NATURAL JOIN current_files WHERE service_id = ?;',
            (self._local_file_service_id, )).fetchone()
        (num_inbox, size_inbox) = self._c.execute(
            'SELECT COUNT( hash_id ), SUM( size ) FROM files_info NATURAL JOIN current_files NATURAL JOIN file_inbox WHERE service_id = ?;',
            (self._local_file_service_id, )).fetchone()
        ...
        total_viewtime = self._c.execute(
            'SELECT SUM( media_views ), SUM( media_viewtime ), SUM( preview_views ), SUM( preview_viewtime ) FROM file_viewing_stats;'
        ).fetchone()
        ...
        total_alternate_files = sum((count for (
            alternates_group_id, count
        ) in self._c.execute(
            'SELECT alternates_group_id, COUNT( * ) FROM alternate_file_group_members GROUP BY alternates_group_id;'
        ) if count > 1))
        total_duplicate_files = sum((count for (
            media_id, count
        ) in self._c.execute(
            'SELECT media_id, COUNT( * ) FROM duplicate_file_members GROUP BY media_id;'
        ) if count > 1))

        (
            table_join, predicates_string
        ) = self._DuplicatesGetPotentialDuplicatePairsTableJoinInfoOnFileService(
            CC.LOCAL_FILE_SERVICE_KEY)

        (total_potential_pairs, ) = self._c.execute(
            'SELECT COUNT( * ) FROM ( SELECT DISTINCT smaller_media_id, larger_media_id FROM {} WHERE {} );'
            .format(table_join, predicates_string)).fetchone()
    ...
    def _GetClientFilesLocations(self):
        ...
        result = {
            prefix: HydrusPaths.ConvertPortablePathToAbsPath(location)
            for (prefix, location) in self._c.execute(
                'SELECT prefix, location FROM client_files_locations;')
        }
        ...
        if len(result) < 512:
            ...
            for prefix in HydrusData.IterateHexPrefixes():
                ...
                self._c.execute(
                    'INSERT OR IGNORE INTO client_files_locations ( prefix, location ) VALUES ( ?, ? );',
                    ('f' + prefix, location))
                self._c.execute(
                    'INSERT OR IGNORE INTO client_files_locations ( prefix, location ) VALUES ( ?, ? );',
                    ('t' + prefix, location))
    ...
    def _GetFileHashes(self, given_hashes, given_hash_type, desired_hash_type):
        ...
        if given_hash_type == 'sha256':
            ...
            for given_hash in given_hashes:
                ...
                result = self._c.execute(
                    'SELECT hash_id FROM local_hashes WHERE ' +
                    given_hash_type + ' = ?;',
        ...
        if desired_hash_type == 'sha256':
            ...
            desired_hashes = [
                desired_hash for (desired_hash, ) in
                self._c.execute('SELECT ' + desired_hash_type +
                                ' FROM local_hashes WHERE hash_id IN ' +
                                HydrusData.SplayListForDB(hash_ids) + ';')
            ]
    ...
    def _GetFileNotes(self, hash):
        ...
        result = self._c.execute(
            'SELECT notes FROM file_notes WHERE hash_id = ?;',
            (hash_id, )).fetchone()
    ...
    def _GetFileSystemPredicates(self, service_key):
        ...
        if service_type in (HC.COMBINED_FILE, HC.COMBINED_TAG):
            ...
            predicates.extend([
                ClientSearch.Predicate(predicate_type, None)
            ...
            service_info = self._GetServiceInfoSpecific(
                service_id, service_type, {HC.SERVICE_INFO_NUM_FILES})

            num_everything = service_info[HC.SERVICE_INFO_NUM_FILES]

            if num_everything <= system_everything_limit or self._controller.new_options.GetBoolean(
                    'always_show_system_everything'):

                predicates.append(
                    ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_EVERYTHING,
                                           min_current_count=num_everything))
            ...
            if have_ratings:
                ...
                predicates.append(
                    ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_RATING))
            ...
            if service_type == HC.FILE_REPOSITORY:
                ...
                (num_local, ) = self._c.execute(
                    'SELECT COUNT( * ) FROM current_files AS remote_current_files, current_files ON ( remote_current_files.hash_id = current_files.hash_id ) WHERE remote_current_files.service_id = ? AND current_files.service_id = ?;',
                    (service_id,
                     self._combined_local_file_service_id)).fetchone()
            ...
            if self._controller.new_options.GetBoolean(
                    'filter_inbox_and_archive_predicates') and (
                        num_inbox == 0 or num_archive == 0):
            ...
            if show_inbox_and_archive:
                ...
                predicates.append(
                    ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_INBOX,
                                           min_current_count=num_inbox))
                predicates.append(
                    ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_ARCHIVE,
                                           min_current_count=num_archive))

            if service_type == HC.FILE_REPOSITORY:

                predicates.append(
                    ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_LOCAL,
                                           min_current_count=num_local))
                predicates.append(
                    ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_NOT_LOCAL,
                                           min_current_count=num_not_local))

            predicates.extend([
                ClientSearch.Predicate(predicate_type) for predicate_type in [
                    HC.PREDICATE_TYPE_SYSTEM_NUM_TAGS, HC.
                    PREDICATE_TYPE_SYSTEM_LIMIT, HC.PREDICATE_TYPE_SYSTEM_SIZE,
    ...
    def _GetForceRefreshTagsManagers(self,
                                     hash_ids,
                                     hash_ids_to_current_file_service_ids=None
                                     ):

        if hash_ids_to_current_file_service_ids is None:

            hash_ids_to_current_file_service_ids = HydrusData.BuildKeyToListDict(
                self._ExecuteManySelectSingleParam(
                    'SELECT hash_id, service_id FROM current_files WHERE hash_id = ?;',
                    hash_ids))
        ...
        for (file_service_id, count) in file_service_id_counter.items():
            ...
            if count == len(hash_ids):  # i.e. every hash has this file service
                ...
                (file_service_type, ) = self._c.execute(
                    'SELECT service_type FROM services WHERE service_id = ?;',
                    (file_service_id, )).fetchone()
        ...
        for tag_service_id in tag_service_ids:
            ...
            if common_file_service_id is None:
                ...
                tag_data.extend(
                    (hash_id, (tag_service_id, HC.CONTENT_STATUS_CURRENT,
                               tag_id))
                    for (hash_id,
                         tag_id) in self._ExecuteManySelectSingleParam(
                             'SELECT hash_id, tag_id FROM ' +
                             current_mappings_table_name +
                             ' WHERE hash_id = ?;', hash_ids))
                tag_data.extend(
                    (hash_id, (tag_service_id, HC.CONTENT_STATUS_DELETED,
                               tag_id))
                    for (hash_id,
                         tag_id) in self._ExecuteManySelectSingleParam(
                             'SELECT hash_id, tag_id FROM ' +
                             ...
                             ' WHERE hash_id = ?;', hash_ids))
                tag_data.extend(
                    (hash_id, (tag_service_id, HC.CONTENT_STATUS_PENDING,
                               tag_id))
                    for (hash_id,
                         tag_id) in self._ExecuteManySelectSingleParam(
                             'SELECT hash_id, tag_id FROM ' +
                ...
                (cache_files_table_name, cache_current_mappings_table_name,
                 cache_deleted_mappings_table_name,
                 cache_pending_mappings_table_name, ac_cache_table_name
                 ) = GenerateSpecificMappingsCacheTableNames(
                     common_file_service_id, tag_service_id)
            ...
            tag_data.extend(
                (hash_id, (tag_service_id, HC.CONTENT_STATUS_PETITIONED,
                           tag_id))
                for (hash_id, tag_id) in self._ExecuteManySelectSingleParam(
                    'SELECT hash_id, tag_id FROM ' +
                    petitioned_mappings_table_name +
                    ' WHERE hash_id = ?;', hash_ids))
        ...
        seen_tag_ids = {
            tag_id
            for (hash_id, (tag_service_id, status, tag_id)) in tag_data
        }
        ...
        service_ids_to_service_keys = {
            ...
            for (service_id, service_key) in self._c.execute(
                'SELECT service_id, service_key FROM services;')
        }
        ...
        for hash_id in hash_ids:
            ...
            service_ids_to_tag_data = HydrusData.BuildKeyToListDict(
                ((tag_service_id, (status,
                                   self._tag_ids_to_tags_cache[tag_id]))
                 for (tag_service_id, status, tag_id) in raw_tag_data))

            service_keys_to_statuses_to_tags = collections.defaultdict(
                HydrusData.default_dict_set)

            service_keys_to_statuses_to_tags.update({
                service_ids_to_service_keys[service_id]:
                HydrusData.BuildKeyToSetDict(tag_data)
                for (service_id,
                     tag_data) in list(service_ids_to_tag_data.items())
            })
    ...
    def _GetHashes(self, hash_ids):
        ...
        return [
            self._hash_ids_to_hashes_cache[hash_id] for hash_id in hash_ids
        ]
    ...
    def _GetHashIdsFromFileViewingStatistics(self, view_type,
                                             viewing_locations, operator,
                                             viewing_value):
        ...
        if operator == '\u2248':
            ...
            test_phrase = content_phrase + ' BETWEEN ' + str(
                lower_bound) + ' AND ' + str(upper_bound)
    ...
    def _GetHashIdsFromNamespace(self,
                                 file_service_key,
                                 tag_service_key,
                                 namespace,
        ...
        tables = self._GetMappingTables(file_service_id,
        ...
        for table in tables:
            ...
            select = 'SELECT hash_id FROM {} WHERE {};'.format(
                table, predicate_string)
    ...
    def _GetHashIdsFromNamespaceIdsSubtagIds(self,
                                             file_service_key,
                                             tag_service_key,
                                             namespace_ids,
                                             subtag_ids,
        ...
        with HydrusDB.TemporaryIntegerTable(
                self._c, namespace_ids,
                'namespace_id') as namespace_temp_table_name:

            self._AnalyzeTempTable(namespace_temp_table_name)

            with HydrusDB.TemporaryIntegerTable(
                    self._c, subtag_ids,
                    'subtag_id') as subtag_temp_table_name:

                self._AnalyzeTempTable(subtag_temp_table_name)

                tables = [
                    table +
                    ' NATURAL JOIN tags NATURAL JOIN {} NATURAL JOIN {}'.
                    format(namespace_temp_table_name, subtag_temp_table_name)
                    for table in tables
                ]
    ...
    def _GetHashIdsFromQuery(self,
        ...
        there_are_tags_to_search = len(tags_to_include) > 0 or len(
            namespaces_to_include) > 0 or len(wildcards_to_include) > 0
        ...
        if 'mimes' in simple_preds:
            ...
            if len(mimes) == 1:
                ...
                files_info_predicates.append('mime IN ' +
                                             HydrusData.SplayListForDB(mimes))

        if 'has_audio' in simple_preds:

            has_audio = simple_preds['has_audio']

            files_info_predicates.append('has_audio = {}'.format(
                int(has_audio)))

        if file_service_key != CC.COMBINED_FILE_SERVICE_KEY:

            if 'min_import_timestamp' in simple_preds:
                files_info_predicates.append(
                    'timestamp >= ' +
                    str(simple_preds['min_import_timestamp']))
            if 'max_import_timestamp' in simple_preds:
                files_info_predicates.append(
                    'timestamp <= ' +
                    str(simple_preds['max_import_timestamp']))

        if 'min_width' in simple_preds:
            files_info_predicates.append('width > ' +
                                         str(simple_preds['min_width']))
        if 'width' in simple_preds:
            files_info_predicates.append('width = ' +
                                         str(simple_preds['width']))
        if 'max_width' in simple_preds:
            files_info_predicates.append('width < ' +
                                         str(simple_preds['max_width']))

        if 'min_height' in simple_preds:
            files_info_predicates.append('height > ' +
                                         str(simple_preds['min_height']))
        if 'height' in simple_preds:
            files_info_predicates.append('height = ' +
                                         str(simple_preds['height']))
        if 'max_height' in simple_preds:
            files_info_predicates.append('height < ' +
                                         str(simple_preds['max_height']))

        if 'min_num_pixels' in simple_preds:
            files_info_predicates.append('width * height > ' +
                                         str(simple_preds['min_num_pixels']))
        if 'num_pixels' in simple_preds:
            files_info_predicates.append('width * height = ' +
                                         str(simple_preds['num_pixels']))
        if 'max_num_pixels' in simple_preds:
            files_info_predicates.append('width * height < ' +
                                         str(simple_preds['max_num_pixels']))

        if 'min_ratio' in simple_preds:

            (ratio_width, ratio_height) = simple_preds['min_ratio']

            files_info_predicates.append('( width * 1.0 ) / height > ' +
                                         str(float(ratio_width)) + ' / ' +
                                         str(ratio_height))
        ...
        if 'min_num_words' in simple_preds:
            files_info_predicates.append('num_words > ' +
                                         str(simple_preds['min_num_words']))
        if 'num_words' in simple_preds:
            ...
            if num_words == 0:
                files_info_predicates.append(
                    '( num_words IS NULL OR num_words = 0 )')
            else:
                files_info_predicates.append('num_words = ' + str(num_words))
        ...
        if 'max_num_words' in simple_preds:
            ...
            if max_num_words == 0:
                ...
                files_info_predicates.append('( num_words < ' +
                                             str(max_num_words) +
                                             ' OR num_words IS NULL )')

        if 'min_duration' in simple_preds:
            files_info_predicates.append('duration > ' +
                                         str(simple_preds['min_duration']))
        ...
        if 'max_duration' in simple_preds:
            ...
            if max_duration == 0:
                files_info_predicates.append('duration < ' + str(max_duration))
            else:
                files_info_predicates.append('( duration < ' +
                                             str(max_duration) +
                                             ' OR duration IS NULL )')

        there_are_simple_files_info_preds_to_search_for = len(
            files_info_predicates) > 0

        # start with some quick ways to populate query_hash_ids

        def intersection_update_qhi(query_hash_ids,
                                    some_hash_ids,
                                    force_create_new_set=False):
        ...
        def do_or_preds(or_predicates, query_hash_ids):
            ...
            for or_predicate in or_predicates:
                ...
                for or_subpredicate in or_predicate.GetValue():
                    ...
                    or_query_hash_ids.update(
                        self._GetHashIdsFromQuery(
                            or_search_context,
                            job_key,
                            query_hash_ids=query_hash_ids))
                ...
                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, or_query_hash_ids)
        ...
        if not (there_are_tags_to_search
                or there_are_simple_files_info_preds_to_search_for):
        ...
        if 'hash' in simple_preds:
            ...
            if search_hash_type == 'sha256':
                ...
                matching_sha256_hashes = [
                    search_hash for search_hash in search_hashes
                    if self._HashExists(search_hash)
                ]

            else:

                matching_sha256_hashes = self._GetFileHashes(
                    search_hashes, search_hash_type, 'sha256')

            specific_hash_ids = self._GetHashIds(matching_sha256_hashes)

            query_hash_ids = intersection_update_qhi(query_hash_ids,
                                                     specific_hash_ids)
        ...
        if 'min_modified_timestamp' in simple_preds:
            modified_timestamp_predicates.append(
                'file_modified_timestamp >= ' +
                str(simple_preds['min_modified_timestamp']))
        if 'max_modified_timestamp' in simple_preds:
            modified_timestamp_predicates.append(
                'file_modified_timestamp <= ' +
                str(simple_preds['max_modified_timestamp']))

        if len(modified_timestamp_predicates) > 0:

            pred_string = ' AND '.join(modified_timestamp_predicates)

            modified_timestamp_hash_ids = self._STS(
                self._c.execute(
                    'SELECT hash_id FROM file_modified_timestamps WHERE {};'.
                    format(pred_string)))

            query_hash_ids = intersection_update_qhi(
                query_hash_ids, modified_timestamp_hash_ids)
        ...
        if system_predicates.HasSimilarTo():
            ...
            for similar_to_hash in similar_to_hashes:
                ...
                similar_hash_ids_and_distances = self._PHashesSearch(
                    hash_id, max_hamming)

                similar_hash_ids = [
                    similar_hash_id
                    for (similar_hash_id,
                         distance) in similar_hash_ids_and_distances
                ]

                all_similar_hash_ids.update(similar_hash_ids)

            query_hash_ids = intersection_update_qhi(query_hash_ids,
                                                     all_similar_hash_ids)

        for (operator, value,
             rating_service_key) in system_predicates.GetRatingsPredicates():
            ...
            if value == 'rated':
                ...
                rating_hash_ids = self._STI(
                    self._c.execute(
                        'SELECT hash_id FROM local_ratings WHERE service_id = ?;',
                        (service_id, )))

                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, rating_hash_ids)
                ...
                if operator == '\u2248':
                    ...
                    predicate = str((value - half_a_star_value) *
                                    0.8) + ' < rating AND rating < ' + str(
                                        (value + half_a_star_value) * 1.2)
                    ...
                    predicate = str(value - half_a_star_value
                                    ) + ' < rating AND rating <= ' + str(
                                        value + half_a_star_value)

                rating_hash_ids = self._STI(
                    self._c.execute(
                        'SELECT hash_id FROM local_ratings WHERE service_id = ? AND '
                        + predicate + ';', (service_id, )))
        ...
        if is_inbox:
            ...
            query_hash_ids = intersection_update_qhi(query_hash_ids,
                                                     self._inbox_hash_ids,
                                                     force_create_new_set=True)

        for (operator, num_relationships, dupe_type
             ) in system_predicates.GetDuplicateRelationshipCountPredicates():

            only_do_zero = (operator in ('=', '\u2248')
                            and num_relationships == 0) or (
                                operator == '<' and num_relationships == 1)
            ...
            if only_do_zero:
                ...
                dupe_hash_ids = self._DuplicatesGetHashIdsFromDuplicateCountPredicate(
                    file_service_key, operator, num_relationships, dupe_type)

                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, dupe_hash_ids)

        for (view_type, viewing_locations, operator, viewing_value
             ) in system_predicates.GetFileViewingStatsPredicates():

            only_do_zero = (operator in ('=', '\u2248')
                            and viewing_value == 0) or (operator == '<'
                                                        and viewing_value == 1)
            include_zero = operator == '<'
            ...
            if only_do_zero:
                ...
                viewing_hash_ids = self._GetHashIdsFromFileViewingStatistics(
                    view_type, viewing_locations, operator, viewing_value)

                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, viewing_hash_ids)
        ...
        if there_are_tags_to_search:
            ...
            for tag in tags_to_include:
                ...
                if query_hash_ids is None:
                    ...
                    tag_query_hash_ids = self._GetHashIdsFromTag(
                        file_service_key, tag_service_key, tag,
                        include_current_tags, include_pending_tags)

                elif is_inbox and len(query_hash_ids) == len(
                        self._inbox_hash_ids):
                    ...
                    tag_query_hash_ids = self._GetHashIdsFromTag(
                        ...
                        include_current_tags,
                        include_pending_tags,
                        hash_ids_table_name='file_inbox')

                else:

                    with HydrusDB.TemporaryIntegerTable(
                            self._c, query_hash_ids,
                            'hash_id') as temp_table_name:
                ...
                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, tag_query_hash_ids)
            ...
            for namespace in namespaces_to_include:
                ...
                if query_hash_ids is None:
                    ...
                    namespace_query_hash_ids = self._GetHashIdsFromNamespace(
                        file_service_key,
                        tag_service_key,
                        namespace,
                        include_current_tags,
                ...
                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, namespace_query_hash_ids)
            ...
            for wildcard in wildcards_to_include:
                ...
                if query_hash_ids is None:
                    ...
                    wildcard_query_hash_ids = self._GetHashIdsFromWildcard(
                        file_service_key, tag_service_key, wildcard,
                        include_current_tags, include_pending_tags)
                ...
                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, wildcard_query_hash_ids)
        ...
        if not (there_are_simple_files_info_preds_to_search_for
                or done_or_predicates):
        ...
        if query_hash_ids is None or (is_inbox and len(query_hash_ids) == len(
                self._inbox_hash_ids)):
            ...
            if file_service_key == CC.COMBINED_FILE_SERVICE_KEY:
                ...
                query_hash_ids = intersection_update_qhi(
                    query_hash_ids,
                    self._GetHashIdsThatHaveTags(tag_service_key,
                                                 include_current_tags,
                                                 include_pending_tags))

            else:

                files_info_predicates.insert(
                    0, 'service_id = ' + str(file_service_id))

                query_hash_ids = intersection_update_qhi(
                    query_hash_ids,
                    self._STS(
                        self._c.execute(
                            'SELECT hash_id FROM current_files NATURAL JOIN files_info WHERE '
                            + ' AND '.join(files_info_predicates) + ';')))
        ...
        if not done_files_info_predicates and (
                need_file_domain_cross_reference
                or there_are_simple_files_info_preds_to_search_for):

            with HydrusDB.TemporaryIntegerTable(self._c, query_hash_ids,
                                                'hash_id') as temp_table_name:
                ...
                select = 'SELECT hash_id FROM {}{};'.format(
                    table, predicate_string)

                files_info_hash_ids = self._STI(self._c.execute(select))

                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, files_info_hash_ids)
        ...
        if file_service_key == CC.COMBINED_LOCAL_FILE_SERVICE_KEY:
            ...
            repo_update_hash_ids = self._STS(
                self._c.execute(
                    'SELECT hash_id FROM current_files NATURAL JOIN files_info WHERE service_id = ?;',
                    (self._local_update_service_id, )))
        ...
        for tag in tags_to_exclude:
            ...
            with HydrusDB.TemporaryIntegerTable(self._c, query_hash_ids,
                ...
                unwanted_hash_ids = self._GetHashIdsFromTag(
                    file_service_key,
                    tag_service_key,
                    tag,
                    include_current_tags,
        ...
        for namespace in namespaces_to_exclude:
            ...
            with HydrusDB.TemporaryIntegerTable(self._c, query_hash_ids,
                ...
                unwanted_hash_ids = self._GetHashIdsFromNamespace(
                    file_service_key,
                    tag_service_key,
                    namespace,
                    include_current_tags,
        ...
        for wildcard in wildcards_to_exclude:
            ...
            with HydrusDB.TemporaryIntegerTable(self._c, query_hash_ids,
                ...
                unwanted_hash_ids = self._GetHashIdsFromWildcard(
                    file_service_key,
                    tag_service_key,
                    wildcard,
                    include_current_tags,
        ...
        (file_services_to_include_current, file_services_to_include_pending,
         file_services_to_exclude_current, file_services_to_exclude_pending
         ) = system_predicates.GetFileServiceInfo()
        ...
        for service_key in file_services_to_include_pending:
            ...
            query_hash_ids = intersection_update_qhi(
                ...
                self._STI(
                    self._c.execute(
                        'SELECT hash_id FROM file_transfers WHERE service_id = ?;',
                        (service_id, ))))
        ...
        for (operator, value,
             service_key) in system_predicates.GetRatingsPredicates():
        ...
        for (operator, num_relationships, dupe_type
            ...
            if only_do_zero:
                ...
                nonzero_hash_ids = self._DuplicatesGetHashIdsFromDuplicateCountPredicate(
                    file_service_key, '>', 0, dupe_type)
                ...
                accurate_except_zero_hash_ids = self._DuplicatesGetHashIdsFromDuplicateCountPredicate(
                    file_service_key, operator, num_relationships, dupe_type)
        ...
        for (view_type, viewing_locations, operator, viewing_value
            ...
            if only_do_zero:
                ...
                nonzero_hash_ids = self._GetHashIdsFromFileViewingStatistics(
                    view_type, viewing_locations, '>', 0)
                ...
                accurate_except_zero_hash_ids = self._GetHashIdsFromFileViewingStatistics(
                    view_type, viewing_locations, operator, viewing_value)
        ...
        must_be_local = system_predicates.MustBeLocal(
        ) or system_predicates.MustBeArchive()
        ...
        if file_service_type in HC.LOCAL_FILE_SERVICES:
            ...
            local_hash_ids = self._STL(
                self._c.execute(
                    'SELECT hash_id FROM current_files WHERE service_id = ?;',
                    (self._combined_local_file_service_id, )))

            if must_be_local:

                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, local_hash_ids)
            ...
            for (operator, rule_type, rule) in simple_preds['known_url_rules']:
                ...
                if rule_type == 'exact_match':
                    ...
                    with HydrusDB.TemporaryIntegerTable(
                        ...
                        url_hash_ids = self._GetHashIdsFromURLRule(
                            rule_type,
                            rule,
                            hash_ids_table_name=temp_table_name)
        ...
        tag_predicates_care_about_zero_counts = len(
            tag_predicates) > 0 and False not in (pred(0)
                                                  for pred in tag_predicates)

        if num_tags_zero or num_tags_nonzero or tag_predicates_care_about_zero_counts:

            nonzero_tag_query_hash_ids = self._GetHashIdsThatHaveTags(
                tag_service_key, include_current_tags, include_pending_tags,
                query_hash_ids)
            ...
            if num_tags_zero:
                ...
                query_hash_ids = intersection_update_qhi(
                    query_hash_ids, nonzero_tag_query_hash_ids)

        if len(tag_predicates) > 0:

            hash_id_tag_counts = self._GetHashIdsTagCounts(
                tag_service_key, include_current_tags, include_pending_tags,
                query_hash_ids)

            good_tag_count_hash_ids = {
                id
                for (id, count) in hash_id_tag_counts
                if False not in (pred(count) for pred in tag_predicates)
            }

            if tag_predicates_care_about_zero_counts:

                zero_hash_ids = query_hash_ids.difference(
                    nonzero_tag_query_hash_ids)
        ...
        if 'min_tag_as_number' in simple_preds:
            ...
            with HydrusDB.TemporaryIntegerTable(self._c, query_hash_ids,
                ...
                good_hash_ids = self._GetHashIdsThatHaveTagAsNum(
                    file_service_key,
                    tag_service_key,
                    namespace,
                    num,
        ...
        limit = system_predicates.GetLimit(
            apply_implicit_limit=apply_implicit_limit)

        we_are_applying_limit = limit is not None and limit < len(
            query_hash_ids)
        ...
        if sort_by is not None and file_service_id != self._combined_file_service_id:
            ...
            (did_sort,
             query_hash_ids) = self._TryToSortHashIds(file_service_id,
                                                      query_hash_ids, sort_by)
    ...
    def _GetHashIdsFromSubtagIds(self,
                                 file_service_key,
                                 tag_service_key,
                                 subtag_ids,
                                 include_current_tags,
        ...
        with HydrusDB.TemporaryIntegerTable(
                self._c, subtag_ids, 'subtag_id') as subtag_temp_table_name:

            self._AnalyzeTempTable(subtag_temp_table_name)

            tables = [
                table + ' NATURAL JOIN tags NATURAL JOIN {}'.format(
                    subtag_temp_table_name) for table in tables
            ]
    ...
    def _GetHashIdsFromTag(self,
        ...
        for tag in tags:
            ...
            if namespace != '':
                ...
                predicate_strings.append(
                    'namespace_id = {} AND subtag_id = {}'.format(
                        namespace_id, subtag_id))
        ...
        for (table,
             predicate_string) in itertools.product(tables, predicate_strings):
    ...
    def _GetHashIdsFromURLRule(self,
                               rule_type,
                               rule,
                               hash_ids_table_name=None):
        ...
        if rule_type == 'exact_match':
            ...
            with HydrusDB.TemporaryIntegerTable(
                    self._c, domain_ids,
                    'domain_id') as domain_temp_table_name:
    ...
    def _GetHashIdsFromWildcard(self,
                                file_service_key,
                                tag_service_key,
                                wildcard,
                                include_current_tags,
        ...
        def GetNamespaceIdsFromWildcard(w):
            ...
            if '*' in w:
                ...
                return self._STL(
                    self._c.execute(
                        'SELECT namespace_id FROM namespaces WHERE namespace LIKE ?;',
                        (like_param, )))
        ...
        def GetSubtagIdsFromWildcard(w):
            ...
            if '*' in w:
                ...
                return self._STL(
                    self._c.execute(
                        'SELECT subtag_id FROM subtags WHERE subtag LIKE ?;',
                        (like_param, )))
        ...
        if namespace_wildcard != '':
            ...
            possible_namespace_ids = GetNamespaceIdsFromWildcard(
                namespace_wildcard)

            return self._GetHashIdsFromNamespaceIdsSubtagIds(
                file_service_key,
                tag_service_key,
                ...
                include_current_tags,
                include_pending_tags,
                hash_ids_table_name=hash_ids_table_name)

        else:

            return self._GetHashIdsFromSubtagIds(
                file_service_key,
                tag_service_key,
                possible_subtag_ids,
                include_current_tags,
    ...
    def _GetHashIdsTagCounts(self, tag_service_key, include_current,
                             include_pending, hash_ids):
        ...
        for search_tag_service_id in search_tag_service_ids:
            ...
            (current_mappings_table_name, deleted_mappings_table_name,
             pending_mappings_table_name, petitioned_mappings_table_name
             ) = GenerateMappingsTableNames(search_tag_service_id)
        ...
        if hash_ids is None or len(hash_ids) > 20000:
            ...
            for search_tag_service_id in search_tag_service_ids:
                ...
                if include_current:
                    ...
                    nonzero_tag_hash_ids.update(
                        self._STI(
                            self._c.execute('SELECT DISTINCT hash_id FROM ' +
                                            current_mappings_table_name +
                                            ';')))
            ...
            with HydrusDB.TemporaryIntegerTable(self._c, hash_ids,
                                                'hash_id') as temp_table_name:
                ...
                for search_tag_service_id in search_tag_service_ids:
                    ...
                    if include_current and include_pending:
                        ...
                        nonzero_tag_hash_ids.update(
                            self._STI(
                                self._c.execute(
                                    'SELECT hash_id as h FROM ' +
                                    temp_table_name +
                                    ' WHERE EXISTS ( SELECT 1 FROM ' +
                                    current_mappings_table_name +
                                    ' WHERE hash_id = h ) OR EXISTS ( SELECT 1 FROM '
                                    + pending_mappings_table_name +
                                    ' WHERE hash_id = h );')))
    ...
    def _GetHashIdsThatHaveTagAsNum(self,
                                    file_service_key,
                                    tag_service_key,
                                    namespace,
                                    num,
        ...
        possible_subtag_ids = self._STS(
            self._c.execute(
                'SELECT subtag_id FROM integer_subtags WHERE integer_subtag ' +
                operator + ' ' + str(num) + ';'))
    ...
    def _GetHashIdsToHashes(self, hash_ids=None, hashes=None):
        ...
        if hash_ids is not None:
            ...
            hash_ids_to_hashes = {
                hash_id: self._hash_ids_to_hashes_cache[hash_id]
                for hash_id in hash_ids
            }
    ...
    def _GetHashIdStatus(self, hash_id, prefix=''):
        ...
        result = self._c.execute(
            'SELECT reason_id FROM local_file_deletion_reasons WHERE hash_id = ?;',
            (hash_id, )).fetchone()
        ...
        result = self._c.execute(
            'SELECT 1 FROM deleted_files WHERE service_id = ? AND hash_id = ?;',
            (self._combined_local_file_service_id, hash_id)).fetchone()
        ...
        result = self._c.execute(
            'SELECT timestamp FROM current_files WHERE service_id = ? AND hash_id = ?;',
            (self._trash_service_id, hash_id)).fetchone()
        ...
        if result is not None:
            ...
            if result is not None:
                ...
                try:
                    ...
                    self._controller.client_files_manager.LocklessGetFilePath(
                        hash, mime)
    ...
    def _GetIdealClientFilesLocations(self):
        ...
        for (portable_location, weight) in self._c.execute(
                'SELECT location, weight FROM ideal_client_files_locations;'):

            abs_location = HydrusPaths.ConvertPortablePathToAbsPath(
                portable_location)

            locations_to_ideal_weights[abs_location] = weight

        result = self._c.execute(
            'SELECT location FROM ideal_thumbnail_override_location;'
        ).fetchone()
        ...
        if result is None:
            ...
            abs_ideal_thumbnail_override_location = HydrusPaths.ConvertPortablePathToAbsPath(
                portable_ideal_thumbnail_override_location)

        return (locations_to_ideal_weights,
                abs_ideal_thumbnail_override_location)
    ...
    def _GetMaintenanceDue(self, stop_time):
        ...
        maintenance_vacuum_period_days = self._controller.new_options.GetNoneableInteger(
        ...
        if maintenance_vacuum_period_days is not None:
            ...
            existing_names_to_timestamps = dict(
                self._c.execute(
                    'SELECT name, timestamp FROM vacuum_timestamps;').fetchall(
                    ))
            ...
            due_names = {
                ...
                if name not in existing_names_to_timestamps or
                HydrusData.TimeHasPassed(existing_names_to_timestamps[name] +
                                         stale_time_delta)
            }
            ...
            if len(due_names) > 0:
                ...
                try:
                    ...
                    if len(possible_due_names) > 0:
                        ...
                        jobs_to_do.append('vacuum ' +
                                          ', '.join(possible_due_names))
        ...
        if len(names_to_analyze) > 0:
            ...
            jobs_to_do.append('analyze ' +
                              HydrusData.ToHumanInt(len(names_to_analyze)) +
                              ' tables')
    ...
    def _GetJSONDump(self, dump_type):
        ...
        result = self._c.execute(
            'SELECT version, dump FROM json_dumps WHERE dump_type = ?;',
            (dump_type, )).fetchone()
        ...
        if result is None:
            ...
            try:
                ...
                DealWithBrokenJSONDump(self._db_dir, dump,
    ...
    def _GetJSONDumpNamed(self, dump_type, dump_name=None, timestamp=None):
        ...
        if dump_name is None:
            ...
            if timestamp is None:
                ...
                (version, dump, object_timestamp) = self._c.execute(
    ...
    def _GetJSONDumpNames(self, dump_type):
        ...
        names = [
            name for (name, ) in self._c.execute(
                'SELECT DISTINCT dump_name FROM json_dumps_named WHERE dump_type = ?;',
                (dump_type, ))
        ]
    ...
    def _GetJSONDumpNamesToBackupTimestamps(self, dump_type):
        ...
        names_to_backup_timestamps = HydrusData.BuildKeyToListDict(
            self._c.execute(
                'SELECT dump_name, timestamp FROM json_dumps_named WHERE dump_type = ? ORDER BY timestamp ASC;',
                (dump_type, )))
    ...
    def _GetJSONSimple(self, name):
        ...
        result = self._c.execute('SELECT dump FROM json_dict WHERE name = ?;',
                                 (name, )).fetchone()
    ...
    def _GetLastShutdownWorkTime(self):
        ...
        result = self._c.execute(
            'SELECT last_shutdown_work_time FROM last_shutdown_work_time;'
        ).fetchone()
    ...
    def _GetMappingTables(self, file_service_id, tag_service_ids,
                          include_current, include_pending):
    ...
    def _GetMediaResults(self, hash_ids):
        ...
        (cached_media_results, missing_hash_ids
         ) = self._weakref_media_result_cache.GetMediaResultsAndMissing(
             hash_ids)
        ...
        if len(missing_hash_ids) > 0:
            ...
            with HydrusDB.TemporaryIntegerTable(self._c, hash_ids,
                ...
                hash_ids_to_info = {
                    hash_id: ClientMedia.FileInfoManager(
                        hash_id, self._hash_ids_to_hashes_cache[hash_id], size,
                        mime, width, height, duration, num_frames, has_audio,
                        num_words)
                    for (hash_id, size, mime, width, height, duration,
                         num_frames, has_audio, num_words) in self._c.
                    execute('SELECT * FROM files_info NATURAL JOIN {};'.format(
                        temp_table_name))
                }

                hash_ids_to_current_file_service_ids_and_timestamps = HydrusData.BuildKeyToListDict(
                    ((hash_id, (service_id, timestamp)) for (
                        hash_id, service_id, timestamp
                    ) in self._c.execute(
                        'SELECT hash_id, service_id, timestamp FROM current_files NATURAL JOIN {};'
                        .format(temp_table_name))))

                hash_ids_to_deleted_file_service_ids = HydrusData.BuildKeyToListDict(
                    self._c.execute(
                        'SELECT hash_id, service_id FROM deleted_files NATURAL JOIN {};'
                        .format(temp_table_name)))

                hash_ids_to_pending_file_service_ids = HydrusData.BuildKeyToListDict(
                    self._c.execute(
                        'SELECT hash_id, service_id FROM file_transfers NATURAL JOIN {};'
                        .format(temp_table_name)))

                hash_ids_to_petitioned_file_service_ids = HydrusData.BuildKeyToListDict(
                    self._c.execute(
                        'SELECT hash_id, service_id FROM file_petitions NATURAL JOIN {};'
                        .format(temp_table_name)))

                hash_ids_to_urls = HydrusData.BuildKeyToSetDict(
                    self._c.execute(
                        'SELECT hash_id, url FROM url_map NATURAL JOIN urls NATURAL JOIN {};'
                        .format(temp_table_name)))

                hash_ids_to_service_ids_and_filenames = HydrusData.BuildKeyToListDict(
                    ((hash_id, (service_id, filename)) for (
                        hash_id, service_id, filename
                    ) in self._c.execute(
                        'SELECT hash_id, service_id, filename FROM service_filenames NATURAL JOIN {};'
                        .format(temp_table_name))))

                hash_ids_to_local_ratings = HydrusData.BuildKeyToListDict(((
                    hash_id, (service_id, rating)
                ) for (service_id, hash_id, rating) in self._c.execute(
                    'SELECT service_id, hash_id, rating FROM local_ratings NATURAL JOIN {};'
                    .format(temp_table_name))))

                hash_ids_to_file_viewing_stats_managers = {
                    hash_id: ClientMedia.FileViewingStatsManager(
                        preview_views, preview_viewtime, media_views,
                        media_viewtime)
                    for (hash_id, preview_views, preview_viewtime, media_views,
                         media_viewtime) in
                    self._c.execute(
                        'SELECT hash_id, preview_views, preview_viewtime, media_views, media_viewtime FROM file_viewing_stats NATURAL JOIN {};'
                        .format(temp_table_name))
                }

                hash_ids_to_file_modified_timestamps = dict(
                    self._c.execute(
                        'SELECT hash_id, file_modified_timestamp FROM file_modified_timestamps NATURAL JOIN {};'
                        .format(temp_table_name)))

            hash_ids_to_current_file_service_ids = {
                hash_id: [
                    file_service_id
                    for (file_service_id,
                         timestamp) in file_service_ids_and_timestamps
                ]
                for (hash_id, file_service_ids_and_timestamps) in list(
                    hash_ids_to_current_file_service_ids_and_timestamps.items(
                    ))
            }

            hash_ids_to_tags_managers = self._GetForceRefreshTagsManagers(
                hash_ids,
                hash_ids_to_current_file_service_ids=
                hash_ids_to_current_file_service_ids)
            ...
            for hash_id in hash_ids:
                ...
                current_file_service_keys = {
                    service_ids_to_service_keys[service_id]
                    for (service_id, timestamp) in
                    hash_ids_to_current_file_service_ids_and_timestamps[hash_id]
                }

                deleted_file_service_keys = {
                    service_ids_to_service_keys[service_id]
                    for service_id in
                    hash_ids_to_deleted_file_service_ids[hash_id]
                }

                pending_file_service_keys = {
                    service_ids_to_service_keys[service_id]
                    for service_id in
                    hash_ids_to_pending_file_service_ids[hash_id]
                }

                petitioned_file_service_keys = {
                    service_ids_to_service_keys[service_id]
                    for service_id in
                    hash_ids_to_petitioned_file_service_ids[hash_id]
                }
                ...
                service_ids_to_filenames = HydrusData.BuildKeyToListDict(
                    hash_ids_to_service_ids_and_filenames[hash_id])

                service_keys_to_filenames = {
                    service_ids_to_service_keys[service_id]: filenames
                    for (service_id,
                         filenames) in list(service_ids_to_filenames.items())
                }

                current_file_service_keys_to_timestamps = {
                    service_ids_to_service_keys[service_id]: timestamp
                    for (service_id, timestamp) in
                    hash_ids_to_current_file_service_ids_and_timestamps[hash_id]
                }
                ...
                if hash_id in hash_ids_to_file_modified_timestamps:
                    ...
                    file_modified_timestamp = hash_ids_to_file_modified_timestamps[
                        hash_id]
                ...
                local_ratings = {
                    service_ids_to_service_keys[service_id]: rating
                    for (service_id,
                         rating) in hash_ids_to_local_ratings[hash_id]
                }
                ...
                if hash_id in hash_ids_to_file_viewing_stats_managers:
                    ...
                    file_viewing_stats_manager = hash_ids_to_file_viewing_stats_managers[
                        hash_id]
                ...
                missing_media_results.append(
                    ClientMedia.MediaResult(file_info_manager, tags_manager,
                                            locations_manager, ratings_manager,
                                            file_viewing_stats_manager))

            self._weakref_media_result_cache.AddMediaResults(
                missing_media_results)
    ...
    def _GetNumsPending(self):
        ...
        services = self._GetServices(
            (HC.TAG_REPOSITORY, HC.FILE_REPOSITORY, HC.IPFS))
        ...
        for service in services:
            ...
            pendings[service_key] = self._GetServiceInfoSpecific(
                service_id, service_type, info_types)
    ...
    def _GetOptions(self):
        ...
        if result is None:
            ...
            self._c.execute('INSERT INTO options ( options ) VALUES ( ? );',
                            (options, ))
    ...
    def _GetPending(self, service_key):
        ...
        if service_type in HC.REPOSITORIES:
            ...
            if service_type == HC.TAG_REPOSITORY:
                ...
                pending_dict = HydrusData.BuildKeyToListDict(
                    self._c.execute('SELECT tag_id, hash_id FROM ' +
                                    pending_mappings_table_name +
                                    ' ORDER BY tag_id LIMIT 100;'))
                ...
                petitioned_dict = HydrusData.BuildKeyToListDict([
                    ((tag_id, reason_id), hash_id)
                    for (tag_id, hash_id, reason_id) in
                    self._c.execute('SELECT tag_id, hash_id, reason_id FROM ' +
                                    petitioned_mappings_table_name +
                                    ' ORDER BY reason_id LIMIT 100;')
                ])

                for ((tag_id, reason_id),
                     hash_ids) in list(petitioned_dict.items()):
                    ...
                    client_to_server_update.AddContent(
                        HC.CONTENT_UPDATE_PETITION, content, reason)

                # tag parents

                pending = self._c.execute(
                    'SELECT child_tag_id, parent_tag_id, reason_id FROM tag_parent_petitions WHERE service_id = ? AND status = ? ORDER BY reason_id LIMIT 1;',
                    (service_id, HC.CONTENT_STATUS_PENDING)).fetchall()
                ...
                for (child_tag_id, parent_tag_id, reason_id) in pending:
                    ...
                    content = HydrusNetwork.Content(
                        HC.CONTENT_TYPE_TAG_PARENTS, (child_tag, parent_tag))

                    client_to_server_update.AddContent(HC.CONTENT_UPDATE_PEND,
                                                       content, reason)

                petitioned = self._c.execute(
                    'SELECT child_tag_id, parent_tag_id, reason_id FROM tag_parent_petitions WHERE service_id = ? AND status = ? ORDER BY reason_id LIMIT 100;',
                    (service_id, HC.CONTENT_STATUS_PETITIONED)).fetchall()
                ...
                pending = self._c.execute(
                    'SELECT bad_tag_id, good_tag_id, reason_id FROM tag_sibling_petitions WHERE service_id = ? AND status = ? ORDER BY reason_id LIMIT 100;',
                    (service_id, HC.CONTENT_STATUS_PENDING)).fetchall()
                ...
                for (bad_tag_id, good_tag_id, reason_id) in pending:
                    ...
                    content = HydrusNetwork.Content(
                        HC.CONTENT_TYPE_TAG_SIBLINGS, (bad_tag, good_tag))
                ...
                petitioned = list(
                    HydrusData.BuildKeyToListDict(
                        self._c.execute(
                            'SELECT reason_id, hash_id FROM file_petitions WHERE service_id = ? ORDER BY reason_id LIMIT 100;',
                            (service_id, ))).items())
                ...
                for (reason_id, hash_ids) in petitioned:
                    ...
                    content = HydrusNetwork.Content(HC.CONTENT_TYPE_FILES,
                                                    hashes)
            ...
            result = self._c.execute(
                'SELECT hash_id FROM file_petitions WHERE service_id = ?;',
                (service_id, )).fetchone()
    ...
    def _GetRecentTags(self, service_key):
        ...
        tag_ids_to_timestamp = {
            tag_id: timestamp
            for (tag_id, timestamp) in self._c.execute(
                'SELECT tag_id, timestamp FROM recent_tags WHERE service_id = ?;',
                (service_id, ))
        }
        ...
        num_we_want = HG.client_controller.new_options.GetNoneableInteger(
            'num_recent_tags')
        ...
        if len(decayed) > 0:
            ...
            self._c.executemany(
                ...
                ((service_id, tag_id) for tag_id in decayed))
        ...
        sorted_recent_tags = [
            self._tag_ids_to_tags_cache[tag_id]
            for tag_id in sorted_recent_tag_ids
        ]

        return sorted_recent_tags

    def _GetRelatedTags(self, service_key, skip_hash, search_tags, max_results,
                        max_time_to_take):

        siblings_manager = HG.client_controller.tag_siblings_manager

        stop_time_for_finding_files = HydrusData.GetNowPrecise() + (
            max_time_to_take / 2)
        stop_time_for_finding_tags = HydrusData.GetNowPrecise() + (
            max_time_to_take / 2)

        search_tags = siblings_manager.CollapseTags(service_key,
                                                    search_tags,
                                                    service_strict=True)
        ...
        query = self._c.execute('SELECT hash_id FROM ' +
                                current_mappings_table_name +
                                ' WHERE tag_id IN ' +
                                HydrusData.SplayListForDB(tag_ids) + ';')
        ...
        hash_ids = [
            hash_id for (hash_id, count) in list(hash_ids_counter.items())
            if count > largest_count * 0.8
        ]
        ...
        for hash_id in hash_ids:
            ...
            for tag_id in self._STI(
                    self._c.execute(
                        'SELECT tag_id FROM ' + current_mappings_table_name +
        ...
        tags_to_counts = {
            self._GetTag(tag_id): count
            for (tag_id, count) in results
        }

        tags_to_counts = siblings_manager.CollapseTagsToCount(
            service_key, tags_to_counts)

        inclusive = True
        pending_count = 0

        predicates = [
            ClientSearch.Predicate(HC.PREDICATE_TYPE_TAG, tag, inclusive,
                                   current_count, pending_count)
            for (tag, current_count) in tags_to_counts.items()
    ...
    def _GetRepositoryProgress(self, service_key):
        ...
        (num_updates, ) = self._c.execute('SELECT COUNT( * ) FROM ' +
                                          repository_updates_table_name +
                                          ';').fetchone()

        (num_processed_updates, ) = self._c.execute(
            'SELECT COUNT( * ) FROM ' + repository_updates_table_name +
            ' WHERE processed = ?;', (True, )).fetchone()

        (num_local_updates, ) = self._c.execute(
            'SELECT COUNT( * ) FROM current_files NATURAL JOIN ' +
            repository_updates_table_name + ' WHERE service_id = ?;',
            (self._local_update_service_id, )).fetchone()
    ...
    def _GetRepositoryThumbnailHashesIDoNotHave(self, service_key):
        ...
        needed_hash_ids = self._STL(
            self._c.execute(
                'SELECT hash_id FROM current_files NATURAL JOIN files_info WHERE mime IN '
                + HydrusData.SplayListForDB(HC.MIMES_WITH_THUMBNAILS) +
                ' and service_id = ? EXCEPT SELECT hash_id FROM remote_thumbnails WHERE service_id = ?;',
                (service_id, service_id)))
        ...
        for hash_id in needed_hash_ids:
            ...
            if client_files_manager.LocklessHasThumbnail(hash):
                ...
                self._c.execute(
                    'INSERT OR IGNORE INTO remote_thumbnails ( service_id, hash_id ) VALUES ( ?, ? );',
                    (service_id, hash_id))
    ...
    def _GetRepositoryUpdateHashesICanProcess(self, service_key):
        ...
        result = self._c.execute(
            'SELECT 1 FROM {} NATURAL JOIN files_info WHERE mime = ? AND processed = ?;'
            .format(repository_updates_table_name),
            (HC.APPLICATION_HYDRUS_UPDATE_DEFINITIONS, True)).fetchone()
        ...
        result = self._c.execute(
            'SELECT 1 FROM {} NATURAL JOIN files_info WHERE mime = ? AND processed = ?;'
            .format(repository_updates_table_name),
            (HC.APPLICATION_HYDRUS_UPDATE_CONTENT, True)).fetchone()

        this_is_first_content_work = result is None

        update_indices_to_unprocessed_hash_ids = HydrusData.BuildKeyToSetDict(
            self._c.execute(
                'SELECT update_index, hash_id FROM {} WHERE processed = ?;'.
                format(repository_updates_table_name), (False, )))
        ...
        for update_index in update_indices:
            ...
            select_args_iterator = ((self._local_update_service_id, hash_id)
                                    for hash_id in unprocessed_hash_ids)

            local_hash_ids = self._STS(
                self._ExecuteManySelect(select_statement,
                                        select_args_iterator))
        ...
        for (hash, mime) in self._ExecuteManySelectSingleParam(
                select_statement, hash_ids_i_can_process):
        ...
        return (this_is_first_definitions_work, definition_hashes,
                this_is_first_content_work, content_hashes)
    ...
    def _GetRepositoryUpdateHashesIDoNotHave(self, service_key):
        ...
        desired_hash_ids = self._STL(
            self._c.execute(
                'SELECT hash_id FROM {} ORDER BY update_index ASC;'.format(
                    repository_updates_table_name)))
        ...
        needed_hash_ids = [
            hash_id for hash_id in desired_hash_ids
    ...
    def _GetRepositoryUpdateHashesUnprocessed(self, service_key):
        ...
        unprocessed_hash_ids = self._STL(
            self._c.execute(
                'SELECT hash_id FROM {} WHERE processed = ?;'.format(
                    repository_updates_table_name), (False, )))
    ...
    def _GetService(self, service_id):
        ...
        if service_id in self._service_cache:
            ...
            result = self._c.execute(
                'SELECT service_key, service_type, name, dictionary_string FROM services WHERE service_id = ?;',
                (service_id, )).fetchone()
    ...
    def _GetServiceDirectoryHashes(self, service_key, dirname):
        ...
        hash_ids = self._STL(
            self._c.execute(
                'SELECT hash_id FROM service_directory_file_map WHERE service_id = ? AND directory_id = ?;',
                (service_id, directory_id)))
    ...
    def _GetServiceDirectoriesInfo(self, service_key):
        ...
        incomplete_info = self._c.execute(
            'SELECT directory_id, num_files, total_size, note FROM service_directories WHERE service_id = ?;',
            (service_id, )).fetchall()

        info = [(self._GetText(directory_id), num_files, total_size, note)
                for (directory_id, num_files, total_size,
                     note) in incomplete_info]
    ...
    def _GetServiceFilename(self, service_id, hash_id):
        ...
        result = self._c.execute(
            'SELECT filename FROM service_filenames WHERE service_id = ? AND hash_id = ?;',
            (service_id, hash_id)).fetchone()
    ...
    def _GetServiceFilenames(self, service_key, hashes):
        ...
        result = [
            filename for (filename, ) in self._c.execute(
                'SELECT filename FROM service_filenames WHERE service_id = ? AND hash_id IN '
                + HydrusData.SplayListForDB(hash_ids) + ';', (service_id, ))
        ]
    ...
    def _GetServiceIds(self, service_types):
        ...
        return self._STL(
            self._c.execute(
                'SELECT service_id FROM services WHERE service_type IN ' +
                HydrusData.SplayListForDB(service_types) + ';'))
    ...
    def _GetServiceInfo(self, service_key):
        ...
        if service_type == HC.COMBINED_LOCAL_FILE:
            ...
            info_types = {
                HC.SERVICE_INFO_NUM_FILES, HC.SERVICE_INFO_NUM_VIEWABLE_FILES,
                HC.SERVICE_INFO_TOTAL_SIZE, HC.SERVICE_INFO_NUM_DELETED_FILES
            }

        elif service_type in (HC.LOCAL_FILE_DOMAIN,
            ...
            info_types = {
                HC.SERVICE_INFO_NUM_FILES, HC.SERVICE_INFO_NUM_TAGS,
                HC.SERVICE_INFO_NUM_MAPPINGS
            }
        ...
        service_info = self._GetServiceInfoSpecific(service_id, service_type,
                                                    info_types)
    ...
    def _GetServiceInfoSpecific(self, service_id, service_type, info_types):
        ...
        results = {
            info_type: info
            for (info_type, info) in self._c.execute(
                'SELECT info_type, info FROM service_info WHERE service_id = ? AND info_type IN '
                + HydrusData.SplayListForDB(info_types) + ';', (service_id, ))
        }
        ...
        if len(results) != len(info_types):
            ...
            if service_type in HC.TAG_SERVICES:
                ...
                if common_tag_info_types <= info_types_missed:
                    ...
                    (num_files, num_tags) = self._c.execute(
                        'SELECT COUNT( DISTINCT hash_id ), COUNT( DISTINCT tag_id ) FROM '
                        + current_mappings_table_name + ';').fetchone()

                    results[HC.SERVICE_INFO_NUM_FILES] = num_files
                    results[HC.SERVICE_INFO_NUM_TAGS] = num_tags

                    self._c.execute(
                        'INSERT INTO service_info ( service_id, info_type, info ) VALUES ( ?, ?, ? );',
                        (service_id, HC.SERVICE_INFO_NUM_FILES, num_files))
                    self._c.execute(
                        'INSERT INTO service_info ( service_id, info_type, info ) VALUES ( ?, ?, ? );',
                        (service_id, HC.SERVICE_INFO_NUM_TAGS, num_tags))
            ...
            for info_type in info_types_missed:
                ...
                if service_type in HC.FILE_SERVICES:
                    ...
                    if info_type in (HC.SERVICE_INFO_NUM_PENDING_FILES,
                                     HC.SERVICE_INFO_NUM_PETITIONED_FILES):
                    ...
                    if info_type == HC.SERVICE_INFO_NUM_FILES:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM current_files WHERE service_id = ?;',
                            (service_id, )).fetchone()
                    elif info_type == HC.SERVICE_INFO_NUM_VIEWABLE_FILES:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM current_files NATURAL JOIN files_info WHERE service_id = ? AND mime IN '
                            + HydrusData.SplayListForDB(HC.SEARCHABLE_MIMES) +
                            ';', (service_id, )).fetchone()
                    elif info_type == HC.SERVICE_INFO_TOTAL_SIZE:
                        result = self._c.execute(
                            'SELECT SUM( size ) FROM current_files NATURAL JOIN files_info WHERE service_id = ?;',
                            (service_id, )).fetchone()
                    elif info_type == HC.SERVICE_INFO_NUM_DELETED_FILES:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM deleted_files WHERE service_id = ?;',
                            (service_id, )).fetchone()
                    elif info_type == HC.SERVICE_INFO_NUM_PENDING_FILES:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM file_transfers WHERE service_id = ?;',
                            (service_id, )).fetchone()
                    elif info_type == HC.SERVICE_INFO_NUM_PETITIONED_FILES:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM file_petitions where service_id = ?;',
                            (service_id, )).fetchone()
                    elif info_type == HC.SERVICE_INFO_NUM_INBOX:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM file_inbox NATURAL JOIN current_files WHERE service_id = ?;',
                            (service_id, )).fetchone()
                    ...
                    if info_type == HC.SERVICE_INFO_NUM_FILES:
                        result = self._c.execute(
                            'SELECT COUNT( DISTINCT hash_id ) FROM ' +
                            current_mappings_table_name + ';').fetchone()
                        ...
                        result = self._c.execute('SELECT COUNT( * ) FROM ' +
                                                 current_mappings_table_name +
                                                 ';').fetchone()
                        ...
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM ' +
                            petitioned_mappings_table_name + ';').fetchone()
                    elif info_type == HC.SERVICE_INFO_NUM_PENDING_TAG_SIBLINGS:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM tag_sibling_petitions WHERE service_id = ? AND status = ?;',
                            (service_id,
                             HC.CONTENT_STATUS_PENDING)).fetchone()
                    elif info_type == HC.SERVICE_INFO_NUM_PETITIONED_TAG_SIBLINGS:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM tag_sibling_petitions WHERE service_id = ? AND status = ?;',
                            (service_id,
                             HC.CONTENT_STATUS_PETITIONED)).fetchone()
                    elif info_type == HC.SERVICE_INFO_NUM_PENDING_TAG_PARENTS:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM tag_parent_petitions WHERE service_id = ? AND status = ?;',
                            (service_id,
                             HC.CONTENT_STATUS_PENDING)).fetchone()
                ...
                elif service_type in (HC.LOCAL_RATING_LIKE,
                                      HC.LOCAL_RATING_NUMERICAL):

                    if info_type == HC.SERVICE_INFO_NUM_FILES:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM local_ratings WHERE service_id = ?;',
                            (service_id, )).fetchone()
                    ...
                    if info_type == HC.SERVICE_INFO_NUM_SHARES:
                        result = self._c.execute(
                            'SELECT COUNT( * ) FROM yaml_dumps WHERE dump_type = ?;',
                            (YAML_DUMP_ID_LOCAL_BOORU, )).fetchone()
    ...
    def _GetSiteId(self, name):
        ...
        result = self._c.execute(
            'SELECT site_id FROM imageboard_sites WHERE name = ?;',
            (name, )).fetchone()

        if result is None:

            self._c.execute(
                'INSERT INTO imageboard_sites ( name ) VALUES ( ? );',
                (name, ))
    ...
    def _GetSubtagId(self, subtag):
        ...
        result = self._c.execute(
            'SELECT subtag_id FROM subtags WHERE subtag = ?;',
            (subtag, )).fetchone()

        if result is None:

            self._c.execute('INSERT INTO subtags ( subtag ) VALUES ( ? );',
                            (subtag, ))
            ...
            self._c.execute(
                'REPLACE INTO subtags_fts4 ( docid, subtag ) VALUES ( ?, ? );',
                (subtag_id, subtag_searchable))
            ...
            try:
                ...
                if CanCacheInteger(integer_subtag):
                    ...
                    self._c.execute(
                        'INSERT OR IGNORE INTO integer_subtags ( subtag_id, integer_subtag ) VALUES ( ?, ? );',
    ...
    def _GetTagId(self, tag):
        ...
        result = self._c.execute(
            'SELECT tag_id FROM tags NATURAL JOIN namespaces NATURAL JOIN subtags WHERE namespace = ? AND subtag = ?;',
            (namespace, subtag)).fetchone()
        ...
        if result is None:
            ...
            self._c.execute(
                'INSERT INTO tags ( namespace_id, subtag_id ) VALUES ( ?, ? );',
                (namespace_id, subtag_id))
    ...
    def _GetTagParents(self, service_key=None):
        def convert_statuses_and_pair_ids_to_statuses_to_pairs(
            statuses_and_pair_ids):
            ...
            statuses_to_pairs = HydrusData.BuildKeyToSetDict(
                ((status, (self._tag_ids_to_tags_cache[child_tag_id],
                           self._tag_ids_to_tags_cache[parent_tag_id]))
                 for (status, child_tag_id,
                      parent_tag_id) in statuses_and_pair_ids))

            return statuses_to_pairs

        if service_key is None:

            service_ids_to_statuses_and_pair_ids = HydrusData.BuildKeyToListDict(
                ((service_id, (status, child_tag_id, parent_tag_id)) for (
                    service_id, status, child_tag_id, parent_tag_id
                ) in self._c.execute(
                    'SELECT service_id, status, child_tag_id, parent_tag_id FROM tag_parents UNION SELECT service_id, status, child_tag_id, parent_tag_id FROM tag_parent_petitions;'
                )))

            service_keys_to_statuses_to_pairs = collections.defaultdict(
                HydrusData.default_dict_set)

            for (service_id, statuses_and_pair_ids) in list(
                    service_ids_to_statuses_and_pair_ids.items()):
                ...
                try:
                    ...
                    self._c.execute(
                        'DELETE FROM tag_parents WHERE service_id = ?;',
                        (service_id, ))
                    self._c.execute(
                        'DELETE FROM tag_parent_petitions WHERE service_id = ?;',
                        (service_id, ))
                ...
                statuses_to_pairs = convert_statuses_and_pair_ids_to_statuses_to_pairs(
                    statuses_and_pair_ids)

                service_keys_to_statuses_to_pairs[
                    service_key] = statuses_to_pairs
            ...
            statuses_and_pair_ids = self._c.execute(
    ...
    def _GetTagSiblings(self, service_key=None):
        def convert_statuses_and_pair_ids_to_statuses_to_pairs(
            ...
            statuses_to_pairs = HydrusData.BuildKeyToSetDict(
                ((status, (self._tag_ids_to_tags_cache[bad_tag_id],
                           self._tag_ids_to_tags_cache[good_tag_id]))
                 for (status, bad_tag_id,
                      good_tag_id) in statuses_and_pair_ids))
        ...
        if service_key is None:
            ...
            service_ids_to_statuses_and_pair_ids = HydrusData.BuildKeyToListDict(
                ((service_id, (status, bad_tag_id, good_tag_id)) for (
                    service_id, status, bad_tag_id, good_tag_id
                ) in self._c.execute(
                    'SELECT service_id, status, bad_tag_id, good_tag_id FROM tag_siblings UNION SELECT service_id, status, bad_tag_id, good_tag_id FROM tag_sibling_petitions;'
                )))
            ...
            for (service_id, statuses_and_pair_ids) in list(
                ...
                try:
                    ...
                    self._c.execute(
                        'DELETE FROM tag_siblings WHERE service_id = ?;',
                        (service_id, ))
                    self._c.execute(
                        'DELETE FROM tag_sibling_petitions WHERE service_id = ?;',
                        (service_id, ))
            ...
            statuses_and_pair_ids = self._c.execute(
                'SELECT status, bad_tag_id, good_tag_id FROM tag_siblings WHERE service_id = ? UNION SELECT status, bad_tag_id, good_tag_id FROM tag_sibling_petitions WHERE service_id = ?;',
                (service_id, service_id)).fetchall()
    ...
    def _GetTagSiblingIds(self, service_id, tag_ids):
        ...
        while len(search_tag_ids) > 0:
            ...
            with HydrusDB.TemporaryIntegerTable(self._c, search_tag_ids,
                                                'tag_id') as temp_table_name:

                self._AnalyzeTempTable(temp_table_name)

                good_select = 'SELECT good_tag_id FROM tag_siblings, {} ON ( bad_tag_id = tag_id );'.format(
                    temp_table_name)
                bad_select = 'SELECT bad_tag_id FROM tag_siblings, {} ON ( good_tag_id = tag_id );'.format(
                    temp_table_name)
            ...
            new_sibling_tag_ids = goods.union(bads).difference(
                searched_tag_ids)
    ...
    def _GetText(self, text_id):
        ...
        result = self._c.execute('SELECT text FROM texts WHERE text_id = ?;',
                                 (text_id, )).fetchone()
    ...
    def _GetTextId(self, text):
        ...
        result = self._c.execute('SELECT text_id FROM texts WHERE text = ?;',
                                 (text, )).fetchone()

        if result is None:

            self._c.execute('INSERT INTO texts ( text ) VALUES ( ? );',
                            (text, ))
    ...
    def _GetTrashHashes(self, limit=None, minimum_age=None):
        ...
        hash_ids = self._STS(
            self._c.execute(
                'SELECT hash_id FROM current_files WHERE service_id = ?' +
                age_phrase + limit_phrase + ';', (self._trash_service_id, )))
        ...
        if HG.db_report_mode:
            ...
            if minimum_age is not None:
                ...
                message += ' with minimum age ' + HydrusData.TimestampToPrettyTimeDelta(
                    timestamp_cutoff, just_now_threshold=0) + ','
    ...
    def _GetURLDomainId(self, domain):
        ...
        result = self._c.execute(
            'SELECT domain_id FROM url_domains WHERE domain = ?;',
            (domain, )).fetchone()

        if result is None:

            self._c.execute('INSERT INTO url_domains ( domain ) VALUES ( ? );',
                            (domain, ))
    ...
    def _GetURLDomainAndSubdomainIds(self, domain):
        ...
        for (domain_id, ) in self._c.execute(
    ...
    def _GetURLId(self, url):
        ...
        result = self._c.execute('SELECT url_id FROM urls WHERE url = ?;',
                                 (url, )).fetchone()
    ...
    def _GetURLStatuses(self, url):
        ...
        results = [
            self._GetHashIdStatus(hash_id, prefix='url recognised')
            for hash_id in hash_ids
        ]
    ...
    def _GetYAMLDump(self, dump_type, dump_name=None):
        ...
        if dump_name is None:
            ...
            result = {
                dump_name: data
                for (dump_name, data) in self._c.execute(
                    'SELECT dump_name, dump FROM yaml_dumps WHERE dump_type = ?;',
                    (dump_type, ))
            }
            ...
            if dump_type == YAML_DUMP_ID_LOCAL_BOORU:
                ...
                result = {
                    bytes.fromhex(dump_name): data
                    for (dump_name, data) in list(result.items())
                }
            ...
            result = self._c.execute(
                'SELECT dump FROM yaml_dumps WHERE dump_type = ? AND dump_name = ?;',
                (dump_type, dump_name)).fetchone()
            ...
            if result is None:
                ...
                if result is None:
                    ...
                    raise HydrusExceptions.DataMissing(dump_name +
                                                       ' was not found!')
    ...
    def _GetYAMLDumpNames(self, dump_type):
        ...
        names = [
            name for (name, ) in self._c.execute(
                'SELECT dump_name FROM yaml_dumps WHERE dump_type = ?;', (
                    dump_type, ))
        ]
    ...
    def _HandleCriticalRepositoryDefinitionError(self, service_id):
        ...
        self._ReprocessRepositoryFromServiceId(
            service_id, (HC.APPLICATION_HYDRUS_UPDATE_DEFINITIONS, ))

        self._ScheduleRepositoryUpdateFileMaintenance(
            service_id,
            ClientFiles.REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA)
        self._ScheduleRepositoryUpdateFileMaintenance(
            service_id, ClientFiles.REGENERATE_FILE_DATA_JOB_FILE_METADATA)
    ...
    def _ImportFile(self, file_import_job):
        ...
        (status, status_hash,
         note) = self._GetHashIdStatus(hash_id, prefix='file recognised')
        ...
        if status != CC.STATUS_SUCCESSFUL_BUT_REDUNDANT:
            ...
            (size, mime, width, height, duration, num_frames, has_audio,
             num_words) = file_import_job.GetFileInfo()
            ...
            self._AddFilesInfo([(hash_id, size, mime, width, height, duration,
            ...
            if HG.file_import_report_mode:
                ...
                HydrusData.ShowText(
                    'File import job mapping file to local file service')
        ...
        if HG.file_import_report_mode:
            ...
            HydrusData.ShowText(
                'File import job done at db level, final status: {}, {}'.
                format(CC.status_string_lookup[status], note))
    ...
    def _ImportUpdate(self, update_network_bytes, update_hash, mime):
        ...
        client_files_manager.LocklessAddFileFromBytes(update_hash, mime,
                                                      update_network_bytes)
    ...
    def _InboxFiles(self, hash_ids):
        ...
        self._c.executemany('INSERT OR IGNORE INTO file_inbox VALUES ( ? );',
                            ((hash_id, ) for hash_id in hash_ids))
        ...
        if num_added > 0:
            ...
            updates = self._c.execute(
                'SELECT service_id, COUNT( * ) FROM current_files WHERE hash_id IN '
                + splayed_hash_ids + ' GROUP BY service_id;').fetchall()
    ...
    def _InitCaches(self):
        ...
        self._local_file_service_id = self._GetServiceId(
            CC.LOCAL_FILE_SERVICE_KEY)
        self._trash_service_id = self._GetServiceId(CC.TRASH_SERVICE_KEY)
        self._local_update_service_id = self._GetServiceId(
            CC.LOCAL_UPDATE_SERVICE_KEY)
        self._combined_local_file_service_id = self._GetServiceId(
            CC.COMBINED_LOCAL_FILE_SERVICE_KEY)
        self._combined_file_service_id = self._GetServiceId(
            CC.COMBINED_FILE_SERVICE_KEY)
        self._combined_tag_service_id = self._GetServiceId(
        ...
        (self._null_namespace_id, ) = self._c.execute(
            'SELECT namespace_id FROM namespaces WHERE namespace = ?;',
            ('', )).fetchone()

        HG.client_controller.pub('splash_set_status_subtext', 'inbox')

        self._inbox_hash_ids = self._STS(
            self._c.execute('SELECT hash_id FROM file_inbox;'))

    def _InitDiskCache(self):

        new_options = self._GetJSONDump(
            HydrusSerialisable.SERIALISABLE_TYPE_CLIENT_OPTIONS)

        disk_cache_init_period = new_options.GetNoneableInteger(
    ...
    def _IsAnOrphan(self, test_type, possible_hash):
        ...
        if self._HashExists(possible_hash):
            ...
            if test_type == 'file':
                ...
                result = self._c.execute(
                    'SELECT 1 FROM current_files WHERE service_id = ? AND hash_id = ?;',
                    (self._combined_local_file_service_id,
                     hash_id)).fetchone()
                ...
                result = self._c.execute(
                    'SELECT 1 FROM current_files WHERE hash_id = ?;',
                    (hash_id, )).fetchone()
        ...
        try:
            ...
            if not for_processing:
                ...
                filenames = [
                    ...
                    if 'mappings' not in filename and 'master' not in filename
                ]

            paths = [
                os.path.join(self._db_dir, filename) for filename in filenames
            ]
                        ...
                        if stop_time is not None:
                            ...
                            if HydrusData.TimeHasPassed(
                                ...
                                HG.client_controller.pub(
                                    'splash_set_status_subtext', 'cached ' +
                                    HydrusData.TimestampToPrettyTimeDelta(
                                        stop_time,
                                        just_now_string='ok',
                                        just_now_threshold=1))

                                next_stop_time_presentation = HydrusData.GetNow(
                                ) + 1
    ...
    def _ManageDBError(self, job, e):
        ...
        if isinstance(e, MemoryError):
            ...
            HydrusData.ShowText(
                'The client is running out of memory! Restart it ASAP!')
        ...
        if 'malformed' in tb:
            ...
            HydrusData.ShowText(
                'A database exception looked like it could be a very serious \'database image is malformed\' error! Unless you know otherwise, please shut down the client immediately and check the \'help my db is broke.txt\' under install_dir/db.'
            )
    ...
    def _MigrationGetMappings(self, database_temp_job_name, file_service_key,
                              tag_service_key, hash_type, tag_filter,
                              content_statuses):
        ...
        if file_service_key == CC.COMBINED_FILE_SERVICE_KEY:
            ...
            statuses_to_tables[
                HC.CONTENT_STATUS_CURRENT] = current_mappings_table_name
            statuses_to_tables[
                HC.CONTENT_STATUS_DELETED] = deleted_mappings_table_name
            statuses_to_tables[
                HC.CONTENT_STATUS_PENDING] = pending_mappings_table_name
            ...
            statuses_to_tables[
                HC.CONTENT_STATUS_CURRENT] = cache_current_mappings_table_name
            statuses_to_tables[
                HC.CONTENT_STATUS_DELETED] = cache_deleted_mappings_table_name
            statuses_to_tables[
                HC.CONTENT_STATUS_PENDING] = cache_pending_mappings_table_name
        ...
        for content_status in content_statuses:
            ...
            select_query = 'SELECT tag_id FROM {} WHERE hash_id = ?;'.format(
                table_name)
        ...
        while not we_should_stop:
            ...
            result = self._c.execute('SELECT hash_id FROM {};'.format(
                database_temp_job_name)).fetchone()
            ...
            self._c.execute(
                'DELETE FROM {} WHERE hash_id = ?;'.format(
                    database_temp_job_name), (hash_id, ))
            ...
            if hash_type == 'sha256':
                ...
                result = self._c.execute(
                    'SELECT ' + hash_type +
                    ' FROM local_hashes WHERE hash_id = ?;',
                    (hash_id, )).fetchone()
            ...
            for select_query in select_queries:
                ...
                tags.update((self._tag_ids_to_tags_cache[tag_id]
                             for tag_id in tag_ids))
            ...
            we_should_stop = len(data) >= 256 or (
                len(data) > 0 and
                HydrusData.TimeHasPassedPrecise(time_started_precise + 1.0))

        return data

    def _MigrationGetPairs(self, database_temp_job_name, left_tag_filter,
                           right_tag_filter):
        ...
        while not we_should_stop:
            ...
            result = self._c.execute(
                'SELECT left_tag_id, right_tag_id FROM {};'.format(
                    database_temp_job_name)).fetchone()
            ...
            self._c.execute(
                'DELETE FROM {} WHERE left_tag_id = ? AND right_tag_id = ?;'.
                format(database_temp_job_name), (left_tag_id, right_tag_id))
    ...
    def _MigrationStartMappingsJob(self, database_temp_job_name,
                                   file_service_key, tag_service_key, hashes,
                                   content_statuses):

        self._c.execute(
            'CREATE TABLE durable_temp.{} ( hash_id INTEGER PRIMARY KEY );'.
            format(database_temp_job_name))
        ...
        if hashes is not None:
            ...
            self._c.executemany(
                'INSERT INTO {} ( hash_id ) VALUES ( ? );'.format(
            ...
            if file_service_key == CC.COMBINED_FILE_SERVICE_KEY:
                ...
                other_services = [
                    service for service in self._GetServices(HC.TAG_SERVICES)
                    if service.GetServiceKey() != tag_service_key
                ]
                ...
                for other_service in other_services:
                    ...
                    other_results = self._GetServiceInfo(
                        other_service.GetServiceKey())

                    other_num_files.append(
                        other_results[HC.SERVICE_INFO_NUM_FILES])
                ...
                if we_are_big:
                    ...
                    local_files_results = self._GetServiceInfo(
                        CC.COMBINED_LOCAL_FILE_SERVICE_KEY)

                    local_files_num_files = local_files_results[
                        HC.SERVICE_INFO_NUM_FILES]
            ...
            if use_hashes_table:
                ...
                if file_service_key == CC.COMBINED_FILE_SERVICE_KEY:
                    ...
                    statuses_to_tables[
                        HC.
                        CONTENT_STATUS_CURRENT] = current_mappings_table_name
                    statuses_to_tables[
                        HC.
                        CONTENT_STATUS_DELETED] = deleted_mappings_table_name
                    statuses_to_tables[
                        HC.
                        CONTENT_STATUS_PENDING] = pending_mappings_table_name
                    ...
                    statuses_to_tables[
                        HC.
                        CONTENT_STATUS_CURRENT] = cache_current_mappings_table_name
                    statuses_to_tables[
                        HC.
                        CONTENT_STATUS_DELETED] = cache_deleted_mappings_table_name
                    statuses_to_tables[
                        HC.
                        CONTENT_STATUS_PENDING] = cache_pending_mappings_table_name
                ...
                for content_status in content_statuses:
                    ...
                    select_subquery = 'SELECT DISTINCT hash_id FROM {}'.format(
                        table_name)
            ...
            for select_subquery in select_subqueries:
                ...
                self._c.execute(
                    'INSERT OR IGNORE INTO {} ( hash_id ) {};'.format(
                        database_temp_job_name, select_subquery))

    def _MigrationStartPairsJob(self, database_temp_job_name, tag_service_key,
                                content_type, content_statuses):

        self._c.execute(
            'CREATE TABLE durable_temp.{} ( left_tag_id INTEGER, right_tag_id INTEGER, PRIMARY KEY ( left_tag_id, right_tag_id ) );'
            .format(database_temp_job_name))
        ...
        for source_table_name in source_table_names:
            ...
            self._c.execute(
                'INSERT OR IGNORE INTO {} ( left_tag_id, right_tag_id ) SELECT {}, {} FROM {} WHERE service_id = ? AND status IN {};'
                .format(database_temp_job_name, left_column_name,
                        right_column_name, source_table_name,
                        HydrusData.SplayListForDB(content_statuses)),
                (tag_service_id, ))
    ...
    def _PHashesAssociatePHashes(self, hash_id, phashes):
        ...
        self._c.executemany(
            'INSERT OR IGNORE INTO shape_perceptual_hash_map ( phash_id, hash_id ) VALUES ( ?, ? );',
            ((phash_id, hash_id) for phash_id in phash_ids))

        if self._GetRowCount() > 0:

            self._c.execute(
                'REPLACE INTO shape_search_cache ( hash_id, searched_distance ) VALUES ( ?, ? );',
                (hash_id, None))
    ...
    def _PHashesEnsureFileInSystem(self, hash_id):
        ...
        result = self._c.execute(
            'SELECT 1 FROM shape_search_cache WHERE hash_id = ?;',
            (hash_id, )).fetchone()
        ...
        if result is None:
            ...
            self._FileMaintenanceAddJobs(
                (hash_id, ),
                ClientFiles.REGENERATE_FILE_DATA_JOB_SIMILAR_FILES_METADATA)

    def _PHashesEnsureFileOutOfSystem(self, hash_id):

        self._DuplicatesRemoveMediaIdMember(hash_id)

        current_phash_ids = self._STS(
            self._c.execute(
                'SELECT phash_id FROM shape_perceptual_hash_map WHERE hash_id = ?;',
                (hash_id, )))
        ...
        self._c.execute('DELETE FROM shape_search_cache WHERE hash_id = ?;',
                        (hash_id, ))
    ...
    def _PHashesGenerateBranch(self, job_key, parent_id, phash_id, phash,
                               children):
        ...
        while len(process_queue) > 0:
            ...
            if len(children) == 0:
                ...
                (inner_id, inner_phash) = self._PHashesPopBestRootNode(
                ...
                if len(outer_children) == 0:
                    ...
                    (outer_id, outer_phash) = self._PHashesPopBestRootNode(
                        outer_children
    ...
    def _PHashesGetMaintenanceStatus(self):
        ...
        searched_distances_to_count = collections.Counter(
            dict(
                self._c.execute(
                    'SELECT searched_distance, COUNT( * ) FROM shape_search_cache GROUP BY searched_distance;'
                )))
    ...
    def _PHashesGetPHashId(self, phash):
        ...
        result = self._c.execute(
            ...
            (sqlite3.Binary(phash), )).fetchone()

        if result is None:

            self._c.execute(
                'INSERT INTO shape_perceptual_hashes ( phash ) VALUES ( ? );',
                (sqlite3.Binary(phash), ))
    ...
    def _PHashesMaintainTree(self, job_key=None, stop_time=None):
        ...
        try:
            ...
            while len(rebalance_phash_ids) > 0:
                ...
                should_stop = stop_time is not None and HydrusData.TimeHasPassed(
                    stop_time)
                ...
                with HydrusDB.TemporaryIntegerTable(
                    ...
                    (biggest_phash_id, ) = self._c.execute(
                        'SELECT phash_id FROM shape_vptree NATURAL JOIN ' +
                        temp_table_name +
                        ' ORDER BY inner_population + outer_population DESC;'
                    ).fetchone()
    ...
    def _PHashesMaintenanceDue(self):
        ...
        if new_options.GetBoolean(
            ...
            search_distance = new_options.GetInteger(
                'similar_files_duplicate_pairs_search_distance')

            (count, ) = self._c.execute(
                'SELECT COUNT( * ) FROM ( SELECT 1 FROM shape_search_cache WHERE searched_distance IS NULL or searched_distance < ? LIMIT 100 );',
                (search_distance, )).fetchone()
    ...
    def _PHashesRegenerateBranch(self, job_key, phash_id):
        ...
        useful_phash_ids = self._STS(
            self._ExecuteManySelectSingleParam(
                'SELECT phash_id FROM shape_perceptual_hash_map WHERE phash_id = ?;',
                unbalanced_phash_ids))
        ...
        if useful_population > 0:
            ...
            (new_phash_id, new_phash) = self._PHashesPopBestRootNode(
        ...
        if useful_population > 0:
            ...
            self._PHashesGenerateBranch(job_key, parent_id, new_phash_id,
                                        new_phash, useful_nodes)
    ...
    def _PHashesRegenerateTree(self):
        ...
        try:
            ...
            (root_id, root_phash) = self._PHashesPopBestRootNode(
                all_nodes)  #HydrusData.RandomPop( all_nodes )

            self._PHashesGenerateBranch(job_key, None, root_id, root_phash,
                                        all_nodes)
    ...
    def _PHashesResetSearch(self, hash_ids):
        ...
        self._c.executemany(
            'UPDATE shape_search_cache SET searched_distance = NULL WHERE hash_id = ?;',
            ((hash_id, ) for hash_id in hash_ids))
    ...
    def _PHashesSearchForPotentialDuplicates(self,
                                             search_distance,
                                             job_key=None,
                                             stop_time=None):
        ...
        try:
            ...
            (total_num_hash_ids_in_cache, ) = self._c.execute(
                'SELECT COUNT( * ) FROM shape_search_cache;').fetchone()

            hash_ids = self._STL(
                self._c.execute(
                    'SELECT hash_id FROM shape_search_cache WHERE searched_distance IS NULL or searched_distance < ?;',
                    (search_distance, )))
            ...
            for (i, hash_id) in enumerate(hash_ids):
                ...
                job_key.SetVariable('popup_title',
                                    'similar files duplicate pair discovery')
                ...
                if i % 25 == 0:
                    ...
                    text = 'searched ' + HydrusData.ConvertValueRangeToPrettyString(
                        total_done_previously + i,
                        total_num_hash_ids_in_cache) + ' files'

                    job_key.SetVariable('popup_text_1', text)
                    job_key.SetVariable('popup_gauge_1',
                                        (total_done_previously + i,
                                         total_num_hash_ids_in_cache))

                    HG.client_controller.pub('splash_set_status_subtext', text)

                media_id = self._DuplicatesGetMediaId(hash_id)

                potential_duplicate_media_ids_and_distances = [
                    (self._DuplicatesGetMediaId(duplicate_hash_id), distance)
                    for (duplicate_hash_id, distance
                         ) in self._PHashesSearch(hash_id, search_distance)
                    if duplicate_hash_id != hash_id
                ]

                self._DuplicatesAddPotentialDuplicates(
                    media_id, potential_duplicate_media_ids_and_distances)

                self._c.execute(
                    'UPDATE shape_search_cache SET searched_distance = ? WHERE hash_id = ?;',
                    (search_distance, hash_id))
    ...
    def _PHashesSearch(self, hash_id, max_hamming_distance):
        ...
        if max_hamming_distance == 0:
            ...
            for search_phash in search_phashes:
                ...
                while len(next_potentials) > 0:
                    ...
                    for group_of_current_potentials in HydrusData.SplitListIntoChunks(
                        ...
                        results = list(
                            self._ExecuteManySelectSingleParam(
                                select_statement, group_of_current_potentials))
            ...
            if HG.db_report_mode:
                ...
                HydrusData.ShowText('Similar file search completed in ' +
            ...
            similar_phash_ids_to_hash_ids = HydrusData.BuildKeyToListDict(
                self._ExecuteManySelectSingleParam(
                    'SELECT phash_id, hash_id FROM shape_perceptual_hash_map WHERE phash_id = ?;',
                    similar_phash_ids))
    ...
    def _PopulateHashIdsToHashesCache(self,
        ...
        if len(uncached_hash_ids) > 0:
            ...
            uncached_hash_ids_to_hashes = dict(
                self._ExecuteManySelectSingleParam(
                ...
                for hash_id in uncached_hash_ids:
                    ...
                    if hash_id not in uncached_hash_ids_to_hashes:
                        ...
                        if not pubbed_error:
                            ...
                            HydrusData.ShowText(
                                'A file identifier was missing! This is a serious error that likely needs a repository reset to fix! Think about contacting hydrus dev!'
                            )
    ...
    def _PopulateTagIdsToTagsCache(self, tag_ids):
        ...
        uncached_tag_ids = [
            ...
            if tag_id not in self._tag_ids_to_tags_cache
        ]
        ...
        if len(uncached_tag_ids) > 0:
            ...
            local_uncached_tag_ids_to_tags = {
                tag_id: tag
                for (tag_id, tag) in self._ExecuteManySelectSingleParam(
                    'SELECT tag_id, tag FROM local_tags_cache WHERE tag_id = ?;',
                    uncached_tag_ids)
            }
        ...
        if len(uncached_tag_ids) > 0:
            ...
            uncached_tag_ids_to_tags = {
                tag_id: HydrusTags.CombineTag(namespace, subtag)
                for (tag_id, namespace,
                     subtag) in self._ExecuteManySelectSingleParam(
                         select_statement, uncached_tag_ids)
            }
            ...
            if len(uncached_tag_ids_to_tags) < len(uncached_tag_ids):
                ...
                for tag_id in uncached_tag_ids:
                    ...
                    if tag_id not in uncached_tag_ids_to_tags:
                        ...
                        self._c.execute(
                            'REPLACE INTO tags ( tag_id, namespace_id, subtag_id ) VALUES ( ?, ?, ? );',
                            (tag_id, namespace_id, subtag_id))

                        uncached_tag_ids_to_tags[tag_id] = tag

            self._tag_ids_to_tags_cache.update(uncached_tag_ids_to_tags)

    def _ProcessContentUpdates(self,
                ...
                if service_type in HC.FILE_SERVICES:
                    ...
                    if data_type == HC.CONTENT_TYPE_FILES:
                        ...
                        if action == HC.CONTENT_UPDATE_ADVANCED:
                            ...
                            if sub_action == 'delete_deleted':
                                ...
                                if hashes is None:
                                    ...
                                    self._c.execute(
                                        'DELETE FROM deleted_files WHERE service_id = ?;',
                                        (service_id, ))

                                    self._c.execute(
                                        'DELETE FROM local_file_deletion_reasons;'
                                    )
                            ...
                            self._c.execute(
                                'DELETE FROM service_info WHERE service_id = ?;',
                                (service_id, ))
                            ...
                            if service_type in HC.LOCAL_FILE_SERVICES or service_type == HC.FILE_REPOSITORY:
                                ...
                                (hash_id, hash, size, mime, width, height,
                                 duration, num_frames, has_audio,
                                 num_words) = file_info_manager.ToTuple()

                                self._AddFilesInfo([
                                    (hash_id, size, mime, width, height,
                                     duration, num_frames, has_audio,
                                     num_words)
                                ])
                                ...
                                self._SetServiceFilename(
                                    service_id, hash_id, multihash)
                            ...
                            self._c.executemany(
                                'INSERT OR IGNORE INTO file_transfers ( service_id, hash_id ) VALUES ( ?, ? );',
                                ((service_id, hash_id)
                                 for hash_id in hash_ids))
                            ...
                            self._c.executemany(
                                'INSERT OR IGNORE INTO file_petitions ( service_id, hash_id, reason_id ) VALUES ( ?, ?, ? );',
                                ((service_id, hash_id, reason_id)
                                 for hash_id in hash_ids))
                            ...
                            if action == HC.CONTENT_UPDATE_ARCHIVE:
                                ...
                                if service_id == self._local_file_service_id:
                                    ...
                                    self._SetLocalFileDeletionReason(
                                        hash_ids, reason)
                                ...
                                splayed_hash_ids = HydrusData.SplayListForDB(
                                    hash_ids)

                                rows = self._c.execute(
                                    'SELECT hash_id, timestamp FROM current_files WHERE service_id = ? AND hash_id IN '
                                    + splayed_hash_ids + ';',
                                    (self._combined_local_file_service_id,
                                     )).fetchall()

                                self._AddFiles(self._local_file_service_id,
                                               rows)
                        ...
                        if action == HC.CONTENT_UPDATE_ADD:
                            ...
                            self._SetServiceDirectory(service_id, hash_ids,
                                                      dirname, note)
                        ...
                        if action == HC.CONTENT_UPDATE_ADD:
                            ...
                            self._c.executemany(
                                'INSERT OR IGNORE INTO url_map ( hash_id, url_id ) VALUES ( ?, ? );',
                                itertools.product(hash_ids, url_ids))
                            ...
                            self._c.executemany(
                                'DELETE FROM url_map WHERE hash_id = ? AND url_id = ?;',
                                itertools.product(hash_ids, url_ids))
                        ...
                        if action == HC.CONTENT_UPDATE_ADVANCED:
                            ...
                            if action == 'clear':
                                ...
                                self._c.execute(
                                    'DELETE FROM file_viewing_stats;')
                            ...
                            self._c.execute(
                                'INSERT OR IGNORE INTO file_viewing_stats ( hash_id, preview_views, preview_viewtime, media_views, media_viewtime ) VALUES ( ?, ?, ?, ?, ? );',
                                (hash_id, 0, 0, 0, 0))

                            self._c.execute(
                                'UPDATE file_viewing_stats SET preview_views = preview_views + ?, preview_viewtime = preview_viewtime + ?, media_views = media_views + ?, media_viewtime = media_viewtime + ? WHERE hash_id = ?;',
                    ...
                    if data_type == HC.CONTENT_TYPE_MAPPINGS:
                        ...
                        if action == HC.CONTENT_UPDATE_ADD:
                            ...
                            if not HG.client_controller.tag_display_manager.TagOK(
                            ...
                            ultimate_deleted_mappings_ids.append(
                                (tag_id, hash_ids))
                            ...
                            ultimate_pending_mappings_ids.append(
                                (tag_id, hash_ids))

                        elif action == HC.CONTENT_UPDATE_RESCIND_PEND:

                            ultimate_pending_rescinded_mappings_ids.append(
                                (tag_id, hash_ids))
                            ...
                            ultimate_petitioned_mappings_ids.append(
                                (tag_id, hash_ids, reason_id))

                        elif action == HC.CONTENT_UPDATE_RESCIND_PETITION:

                            ultimate_petitioned_rescinded_mappings_ids.append(
                                (tag_id, hash_ids))
                            ...
                            self._c.executemany(
                                'DELETE FROM {} WHERE tag_id = ? AND hash_id = ?;'
                                .format(deleted_mappings_table_name),
                                ((tag_id, hash_id) for hash_id in hash_ids))
                            ...
                            cache_file_service_ids = self._GetServiceIds(
                                HC.AUTOCOMPLETE_CACHE_SPECIFIC_FILE_SERVICES)
                            ...
                            for cache_file_service_id in cache_file_service_ids:
                                ...
                                (cache_files_table_name,
                                 ...
                                 ac_cache_table_name
                                     ...
                                     cache_file_service_id, service_id)
                        ...
                        if action in (HC.CONTENT_UPDATE_ADD,
                            ...
                            if action == HC.CONTENT_UPDATE_ADD:
                                ...
                                self._AddTagParents(service_id,
                                                    pairs,
                                                    make_content_updates=True)

                            elif action == HC.CONTENT_UPDATE_DELETE:

                                self._DeleteTagParents(service_id, pairs)

                        elif action in (HC.CONTENT_UPDATE_PEND,
                            ...
                            self._c.execute(
                                'DELETE FROM tag_parent_petitions WHERE service_id = ? AND child_tag_id = ? AND parent_tag_id = ?;',
                                (service_id, child_tag_id, parent_tag_id))

                            self._c.execute(
                                'INSERT OR IGNORE INTO tag_parent_petitions ( service_id, child_tag_id, parent_tag_id, reason_id, status ) VALUES ( ?, ?, ?, ?, ? );',
                                (service_id, child_tag_id, parent_tag_id,
                                 reason_id, new_status))

                            notify_new_pending = True

                        elif action in (HC.CONTENT_UPDATE_RESCIND_PEND,
                        ...
                        if action in (HC.CONTENT_UPDATE_ADD,
                            ...
                            if action == HC.CONTENT_UPDATE_ADD:
                                ...
                                self._AddTagSiblings(service_id,
                                                     pairs,
                                                     make_content_updates=True)
                            ...
                            self._c.execute(
                                'DELETE FROM tag_sibling_petitions WHERE service_id = ? AND bad_tag_id = ? AND good_tag_id = ?;',
                                (service_id, bad_tag_id, good_tag_id))

                            self._c.execute(
                                'INSERT OR IGNORE INTO tag_sibling_petitions ( service_id, bad_tag_id, good_tag_id, reason_id, status ) VALUES ( ?, ?, ?, ?, ? );',
                                (service_id, bad_tag_id, good_tag_id,
                                 reason_id, new_status))
                            ...
                            self._c.execute(
                                'DELETE FROM tag_sibling_petitions WHERE service_id = ? AND bad_tag_id = ? AND status = ?;',
                                (service_id, bad_tag_id, deletee_status))
                    ...
                    if action == HC.CONTENT_UPDATE_ADD:
                        ...
                        if service_type in (HC.LOCAL_RATING_LIKE,
                            ...
                            self._c.executemany(
                                'DELETE FROM local_ratings WHERE service_id = ? AND hash_id = ?;',
                                ((service_id, hash_id)
                                 for hash_id in hash_ids))
                        ...
                        if action == 'delete_for_deleted_files':
                            ...
                            self._c.execute(
                                'DELETE FROM local_ratings WHERE local_ratings.service_id = ? and hash_id IN ( SELECT hash_id FROM deleted_files WHERE deleted_files.service_id = ? );',
                                (service_id,
                                 self._combined_local_file_service_id))
                            ...
                            self._c.execute(
                                'UPDATE service_info SET info = info - ? WHERE service_id = ? AND info_type = ?;',
                                (ratings_deleted, service_id,
                                 HC.SERVICE_INFO_NUM_FILES))

                        elif action == 'delete_for_non_local_files':

                            self._c.execute(
                                'DELETE FROM local_ratings WHERE local_ratings.service_id = ? and hash_id NOT IN ( SELECT hash_id FROM current_files WHERE current_files.service_id = ? );',
                                (service_id,
                                 self._combined_local_file_service_id))
                            ...
                            self._c.execute(
                                'DELETE FROM local_ratings WHERE service_id = ?;',
                                (service_id, ))

                            self._c.execute(
                                'UPDATE service_info SET info = ? WHERE service_id = ? AND info_type = ?;',
                                (0, service_id, HC.SERVICE_INFO_NUM_FILES))
                    ...
                    if action == HC.CONTENT_UPDATE_SET:
                        ...
                        self._c.execute(
                            'DELETE FROM file_notes WHERE hash_id = ?;',
                            (hash_id, ))

                        if len(notes) > 0:

                            self._c.execute(
                                'INSERT OR IGNORE INTO file_notes ( hash_id, notes ) VALUES ( ?, ? );',
                                (hash_id, notes))

            if len(ultimate_mappings_ids) + len(
                    ultimate_deleted_mappings_ids
            ) + len(ultimate_pending_mappings_ids) + len(
                    ultimate_pending_rescinded_mappings_ids) + len(
                        ultimate_petitioned_mappings_ids) + len(
                            ultimate_petitioned_rescinded_mappings_ids) > 0:

                self._UpdateMappings(
                    service_id,
                    mappings_ids=ultimate_mappings_ids,
                    deleted_mappings_ids=ultimate_deleted_mappings_ids,
                    pending_mappings_ids=ultimate_pending_mappings_ids,
                    pending_rescinded_mappings_ids=
                    ultimate_pending_rescinded_mappings_ids,
                    petitioned_mappings_ids=ultimate_petitioned_mappings_ids,
                    petitioned_rescinded_mappings_ids=
                    ultimate_petitioned_rescinded_mappings_ids)
    ...
    def _ProcessRepositoryContent(self, service_key, content_hash,
                                  content_iterator_dict, job_key, work_time):
        ...
        if 'new_files' in content_iterator_dict:
            ...
            for chunk in HydrusData.SplitIteratorIntoAutothrottledChunks(
                    i, FILES_INITIAL_CHUNK_SIZE, precise_time_to_stop):

                files_info_rows = []
                files_rows = []

                for (service_hash_id, size, mime, timestamp, width, height,
                     duration, num_frames, num_words) in chunk:

                    hash_id = self._CacheRepositoryNormaliseServiceHashId(
                        service_id, service_hash_id)

                    files_info_rows.append(
                        (hash_id, size, mime, width, height, duration,
                         num_frames, has_audio, num_words))
        ...
        if 'deleted_files' in content_iterator_dict:
            ...
            for chunk in HydrusData.SplitIteratorIntoAutothrottledChunks(
                ...
                hash_ids = self._CacheRepositoryNormaliseServiceHashIds(
        ...
        if 'new_mappings' in content_iterator_dict:
            ...
            for chunk in HydrusData.SplitMappingIteratorIntoAutothrottledChunks(
                    i, MAPPINGS_INITIAL_CHUNK_SIZE, precise_time_to_stop):
                ...
                for (service_tag_id, service_hash_ids) in chunk:
                    ...
                    tag_id = self._CacheRepositoryNormaliseServiceTagId(
        ...
        if 'deleted_mappings' in content_iterator_dict:
            ...
            for chunk in HydrusData.SplitMappingIteratorIntoAutothrottledChunks(
                ...
                self._UpdateMappings(service_id,
                                     deleted_mappings_ids=deleted_mappings_ids)
        ...
        if 'new_parents' in content_iterator_dict:
            ...
            for chunk in HydrusData.SplitIteratorIntoAutothrottledChunks(
                    i, NEW_TAG_PARENTS_INITIAL_CHUNK_SIZE,
                ...
                for (service_child_tag_id, service_parent_tag_id) in chunk:
                    ...
                    child_tag_id = self._CacheRepositoryNormaliseServiceTagId(
                        service_id, service_child_tag_id)
                    parent_tag_id = self._CacheRepositoryNormaliseServiceTagId(
                        service_id, service_parent_tag_id)
        ...
        if 'deleted_parents' in content_iterator_dict:
            ...
            for chunk in HydrusData.SplitIteratorIntoAutothrottledChunks(
                    i, PAIR_ROWS_INITIAL_CHUNK_SIZE, precise_time_to_stop):
        ...
        if 'new_siblings' in content_iterator_dict:
            ...
            for chunk in HydrusData.SplitIteratorIntoAutothrottledChunks(
                ...
                for (service_bad_tag_id, service_good_tag_id) in chunk:
                    ...
                    bad_tag_id = self._CacheRepositoryNormaliseServiceTagId(
                        service_id, service_bad_tag_id)
                    good_tag_id = self._CacheRepositoryNormaliseServiceTagId(
                        service_id, service_good_tag_id)
        ...
        self._c.execute(
            'UPDATE {} SET processed = ? WHERE hash_id = ?;'.format(
                repository_updates_table_name), (True, update_hash_id))

        return num_rows_processed

    def _ProcessRepositoryDefinitions(self, service_key, definition_hash,
                                      definition_iterator_dict, job_key,
                                      work_time):
        ...
        if 'service_hash_ids_to_hashes' in definition_iterator_dict:
            ...
            for chunk in HydrusData.SplitIteratorIntoAutothrottledChunks(
                ...
                self._c.executemany(
                    'INSERT OR IGNORE INTO {} ( service_hash_id, hash_id ) VALUES ( ?, ? );'
        ...
        if 'service_tag_ids_to_tags' in definition_iterator_dict:
            ...
            for chunk in HydrusData.SplitIteratorIntoAutothrottledChunks(
                ...
                self._c.executemany(
                    'INSERT OR IGNORE INTO {} ( service_tag_id, tag_id ) VALUES ( ?, ? );'
    ...
    def _PushRecentTags(self, service_key, tags):
        ...
        if tags is None:
            ...
            self._c.execute('DELETE FROM recent_tags WHERE service_id = ?;',
                            (service_id, ))
            ...
            self._c.executemany(
                'REPLACE INTO recent_tags ( service_id, tag_id, timestamp ) VALUES ( ?, ?, ? );',
                ((service_id, tag_id, now) for tag_id in tag_ids))

    def _Read(self, action, *args, **kwargs):

        if action == 'autocomplete_predicates':
            result = self._GetAutocompletePredicates(*args, **kwargs)
        elif action == 'boned_stats':
            result = self._GetBonedStats(*args, **kwargs)
        elif action == 'client_files_locations':
            result = self._GetClientFilesLocations(*args, **kwargs)
        elif action == 'duplicate_pairs_for_filtering':
            result = self._DuplicatesGetPotentialDuplicatePairsForFiltering(
                *args, **kwargs)
        elif action == 'file_duplicate_hashes':
            result = self._DuplicatesGetFileHashesByDuplicateType(
                *args, **kwargs)
        elif action == 'file_duplicate_info':
            result = self._DuplicatesGetFileDuplicateInfo(*args, **kwargs)
        elif action == 'file_hashes':
            result = self._GetFileHashes(*args, **kwargs)
        elif action == 'file_maintenance_get_job':
            result = self._FileMaintenanceGetJob(*args, **kwargs)
        elif action == 'file_maintenance_get_job_counts':
            result = self._FileMaintenanceGetJobCounts(*args, **kwargs)
        elif action == 'file_notes':
            result = self._GetFileNotes(*args, **kwargs)
        elif action == 'file_query_ids':
            result = self._GetHashIdsFromQuery(*args, **kwargs)
        elif action == 'file_system_predicates':
            result = self._GetFileSystemPredicates(*args, **kwargs)
        elif action == 'filter_existing_tags':
            result = self._FilterExistingTags(*args, **kwargs)
        elif action == 'filter_hashes':
            result = self._FilterHashes(*args, **kwargs)
        elif action == 'force_refresh_tags_managers':
            result = self._GetForceRefreshTagsManagers(*args, **kwargs)
        elif action == 'hash_ids_to_hashes':
            result = self._GetHashIdsToHashes(*args, **kwargs)
        elif action == 'hash_status':
            result = self._GetHashStatus(*args, **kwargs)
        elif action == 'ideal_client_files_locations':
            result = self._GetIdealClientFilesLocations(*args, **kwargs)
        elif action == 'imageboards':
            result = self._GetYAMLDump(YAML_DUMP_ID_IMAGEBOARD, *args,
                                       **kwargs)
        elif action == 'in_inbox':
            result = self._InInbox(*args, **kwargs)
        elif action == 'is_an_orphan':
            result = self._IsAnOrphan(*args, **kwargs)
        elif action == 'last_shutdown_work_time':
            result = self._GetLastShutdownWorkTime(*args, **kwargs)
        elif action == 'load_into_disk_cache':
            result = self._LoadIntoDiskCache(*args, **kwargs)
        elif action == 'local_booru_share_keys':
            result = self._GetYAMLDumpNames(YAML_DUMP_ID_LOCAL_BOORU)
        elif action == 'local_booru_share':
            result = self._GetYAMLDump(YAML_DUMP_ID_LOCAL_BOORU, *args,
                                       **kwargs)
        elif action == 'local_booru_shares':
            result = self._GetYAMLDump(YAML_DUMP_ID_LOCAL_BOORU)
        elif action == 'maintenance_due':
            result = self._GetMaintenanceDue(*args, **kwargs)
        elif action == 'media_results':
            result = self._GetMediaResultsFromHashes(*args, **kwargs)
        elif action == 'media_results_from_ids':
            result = self._GetMediaResults(*args, **kwargs)
        elif action == 'migration_get_mappings':
            result = self._MigrationGetMappings(*args, **kwargs)
        elif action == 'migration_get_pairs':
            result = self._MigrationGetPairs(*args, **kwargs)
        elif action == 'missing_repository_update_hashes':
            result = self._GetRepositoryUpdateHashesIDoNotHave(*args, **kwargs)
        elif action == 'missing_thumbnail_hashes':
            result = self._GetRepositoryThumbnailHashesIDoNotHave(
                *args, **kwargs)
        elif action == 'nums_pending':
            result = self._GetNumsPending(*args, **kwargs)
        elif action == 'trash_hashes':
            result = self._GetTrashHashes(*args, **kwargs)
        elif action == 'options':
            result = self._GetOptions(*args, **kwargs)
        elif action == 'pending':
            result = self._GetPending(*args, **kwargs)
        elif action == 'random_potential_duplicate_hashes':
            result = self._DuplicatesGetRandomPotentialDuplicateHashes(
                *args, **kwargs)
        elif action == 'recent_tags':
            result = self._GetRecentTags(*args, **kwargs)
        elif action == 'repository_progress':
            result = self._GetRepositoryProgress(*args, **kwargs)
        elif action == 'repository_unprocessed_hashes':
            result = self._GetRepositoryUpdateHashesUnprocessed(
                *args, **kwargs)
        elif action == 'repository_update_hashes_to_process':
            result = self._GetRepositoryUpdateHashesICanProcess(
                *args, **kwargs)
        elif action == 'serialisable':
            result = self._GetJSONDump(*args, **kwargs)
        elif action == 'serialisable_simple':
            result = self._GetJSONSimple(*args, **kwargs)
        elif action == 'serialisable_named':
            result = self._GetJSONDumpNamed(*args, **kwargs)
        elif action == 'serialisable_names':
            result = self._GetJSONDumpNames(*args, **kwargs)
        elif action == 'serialisable_names_to_backup_timestamps':
            result = self._GetJSONDumpNamesToBackupTimestamps(*args, **kwargs)
        elif action == 'service_directory':
            result = self._GetServiceDirectoryHashes(*args, **kwargs)
        elif action == 'service_directories':
            result = self._GetServiceDirectoriesInfo(*args, **kwargs)
        elif action == 'service_filenames':
            result = self._GetServiceFilenames(*args, **kwargs)
        elif action == 'service_info':
            result = self._GetServiceInfo(*args, **kwargs)
        elif action == 'services':
            result = self._GetServices(*args, **kwargs)
        elif action == 'similar_files_maintenance_status':
            result = self._PHashesGetMaintenanceStatus(*args, **kwargs)
        elif action == 'related_tags':
            result = self._GetRelatedTags(*args, **kwargs)
        elif action == 'tag_parents':
            result = self._GetTagParents(*args, **kwargs)
        elif action == 'tag_siblings':
            result = self._GetTagSiblings(*args, **kwargs)
        elif action == 'potential_duplicates_count':
            result = self._DuplicatesGetPotentialDuplicatesCount(
                *args, **kwargs)
        elif action == 'url_statuses':
            result = self._GetURLStatuses(*args, **kwargs)
        else:
    ...
    def _RegenerateACCache(self):
        ...
        try:
            ...
            job_key.SetVariable('popup_title',
                                'regenerating autocomplete cache')
            ...
            for (file_service_id,
                 tag_service_id) in itertools.product(file_service_ids,
                                                      tag_service_ids):
                ...
                message = 'generating specific ac_cache {}_{}'.format(
                    file_service_id, tag_service_id)
                ...
                self._CacheSpecificMappingsDrop(file_service_id,
                                                tag_service_id)
            ...
            for tag_service_id in tag_service_ids:
                ...
                message = 'generating combined files ac_cache {}'.format(
                    tag_service_id)
    ...
    def _RelocateClientFiles(self, prefix, source, dest):
        ...
        self._c.execute(
            'UPDATE client_files_locations SET location = ? WHERE prefix = ?;',
            (portable_dest, prefix))

        if os.path.exists(full_source):

            try:
                HydrusPaths.RecyclePath(full_source)
            except:
                pass
    ...
    def _RepairClientFiles(self, correct_rows):
        ...
        for (incorrect_location, prefix, correct_location) in correct_rows:
            ...
            portable_incorrect_location = HydrusPaths.ConvertAbsPathToPortablePath(
                incorrect_location)
            portable_correct_location = HydrusPaths.ConvertAbsPathToPortablePath(
                correct_location)
            ...
            if portable_correct_location != portable_incorrect_location:
                ...
                self._c.execute(
                    'UPDATE client_files_locations SET location = ? WHERE location = ? AND prefix = ?;',
                    (portable_correct_location, portable_incorrect_location,
                     prefix))
    ...
    def _RepairDB(self):
        ...
        existing_master_tables = self._STS(
            self._c.execute(
                'SELECT name FROM external_master.sqlite_master WHERE type = ?;',
                ('table', )))
        ...
        missing_main_tables = main_master_tables.difference(
            existing_master_tables)
        ...
        if 'local_hashes' not in existing_master_tables:
            ...
            self._controller.CallBlockingToQt(self._controller.app,
                                              QW.QMessageBox.warning, None,
                                              'Warning', message)
        ...
        existing_mapping_tables = self._STS(
            self._c.execute(
                'SELECT name FROM external_mappings.sqlite_master WHERE type = ?;',
                ('table', )))
        ...
        for service_id in tag_service_ids:
            ...
            main_mappings_tables.update(
                (name.split('.')[1]
                 for name in GenerateMappingsTableNames(service_id)))

        missing_main_tables = main_mappings_tables.difference(
            existing_mapping_tables)
        ...
        existing_cache_tables = self._STS(
            self._c.execute(
                'SELECT name FROM external_caches.sqlite_master WHERE type = ?;',
                ('table', )))
        ...
        missing_main_tables = main_cache_tables.difference(
            existing_cache_tables)
        ...
        for (file_service_id,
            ...
            mappings_cache_tables.update(
                (name.split('.')[1]
                 for name in GenerateSpecificMappingsCacheTableNames(
                     file_service_id, tag_service_id)))

        for tag_service_id in tag_service_ids:

            mappings_cache_tables.add(
                GenerateCombinedFilesMappingsCacheTableName(
                    tag_service_id).split('.')[1])

        mappings_cache_tables.add('local_tags_cache')

        missing_main_tables = mappings_cache_tables.difference(
            existing_cache_tables)
    ...
    def _ReportOverupdatedDB(self, version):
        def qt_code():
            ...
            QW.QMessageBox.warning(
                None, 'Warning', 'This client\'s database is version ' +
                HydrusData.ToHumanInt(version) +
                ', but the software is version ' +
                HydrusData.ToHumanInt(HC.SOFTWARE_VERSION) +
                '! This situation only sometimes works, and when it does not, it can break things! If you are not sure what is going on, or if you accidentally installed an older version of the software to a newer database, force-kill this client in Task Manager right now. Otherwise, ok this dialog box to continue.'
            )
    ...
    def _ReprocessRepositoryFromServiceId(self, service_id, update_mime_types):
        ...
        for update_mime_type in update_mime_types:
            ...
            hash_ids = self._STL(
                self._c.execute(
                    'SELECT hash_id FROM {} NATURAL JOIN files_info WHERE mime = ? AND processed = ?;'
                    .format(repository_updates_table_name),
                    (update_mime_type, True)))
        ...
        self._c.executemany(
            'UPDATE {} SET processed = ? WHERE hash_id = ?;'.format(
                repository_updates_table_name),
            ((False, hash_id) for hash_id in update_hash_ids))
    ...
    def _ScheduleRepositoryUpdateFileMaintenance(self, service_id, job_type):
        ...
        update_hash_ids = self._STL(
            self._c.execute('SELECT hash_id FROM {};'.format(
                repository_updates_table_name)))

        self._FileMaintenanceAddJobs(update_hash_ids, job_type)

    def _ScheduleRepositoryUpdateFileMaintenanceFromServiceKey(
        self, service_key, job_type):

        service_id = self._GetServiceId(service_key)

        self._ScheduleRepositoryUpdateFileMaintenance(service_id, job_type)

    def _SetIdealClientFilesLocations(self, locations_to_ideal_weights,
        ...
        for (abs_location, weight) in locations_to_ideal_weights.items():
            ...
            self._c.execute(
                'INSERT INTO ideal_client_files_locations ( location, weight ) VALUES ( ?, ? );',
                (portable_location, weight))

        self._c.execute('DELETE FROM ideal_thumbnail_override_location;')

        if ideal_thumbnail_override_location is not None:

            portable_ideal_thumbnail_override_location = HydrusPaths.ConvertAbsPathToPortablePath(
                ideal_thumbnail_override_location)

            self._c.execute(
                'INSERT INTO ideal_thumbnail_override_location ( location ) VALUES ( ? );',
                (portable_ideal_thumbnail_override_location, ))
    ...
    def _SetJSONSimple(self, name, value):
        ...
        if value is None:
            ...
            try:
                ...
                self._c.execute(
                    'REPLACE INTO json_dict ( name, dump ) VALUES ( ?, ? );',
                    (name, dump_buffer))
    ...
    def _SetLastShutdownWorkTime(self, timestamp):
        ...
        self._c.execute(
            'INSERT INTO last_shutdown_work_time ( last_shutdown_work_time ) VALUES ( ? );',
            (timestamp, ))
    ...
    def _SetLocalFileDeletionReason(self, hash_ids, reason):
        ...
        if reason is not None:
            ...
            self._c.executemany(
                'REPLACE INTO local_file_deletion_reasons ( hash_id, reason_id ) VALUES ( ?, ? );',
                ((hash_id, reason_id) for hash_id in hash_ids))
    ...
    def _SetServiceFilename(self, service_id, hash_id, filename):
        ...
        self._c.execute(
            'REPLACE INTO service_filenames ( service_id, hash_id, filename ) VALUES ( ?, ?, ? );',
            (service_id, hash_id, filename))
    ...
    def _SetServiceDirectory(self, service_id, hash_ids, dirname, note):
        ...
        result = self._c.execute(
            'SELECT SUM( size ) FROM files_info WHERE hash_id IN ' +
            HydrusData.SplayListForDB(hash_ids) + ';').fetchone()
        ...
        self._c.execute(
            'INSERT INTO service_directories ( service_id, directory_id, num_files, total_size, note ) VALUES ( ?, ?, ?, ?, ? );',
            (service_id, directory_id, num_files, total_size, note))
        self._c.executemany(
            'INSERT INTO service_directory_file_map ( service_id, directory_id, hash_id ) VALUES ( ?, ?, ? );',
            ((service_id, directory_id, hash_id) for hash_id in hash_ids))
    ...
    def _SetYAMLDump(self, dump_type, dump_name, data):
        ...
        try:
            self._c.execute(
                'INSERT INTO yaml_dumps ( dump_type, dump_name, dump ) VALUES ( ?, ?, ? );',
                (dump_type, dump_name, data))
        except:
    ...
    def _SubtagExists(self, subtag):
        ...
        result = self._c.execute('SELECT 1 FROM subtags WHERE subtag = ?;',
                                 (subtag, )).fetchone()
    ...
    def _TagExists(self, tag):
        ...
        if self._SubtagExists(subtag):
            ...
            result = self._c.execute(
                'SELECT 1 FROM tags WHERE namespace_id = ? AND subtag_id = ?;',
                (namespace_id, subtag_id)).fetchone()
    ...
    def _TryToSortHashIds(self, file_service_id, hash_ids, sort_by):
        ...
        if sort_metadata == 'system':
            ...
            if sort_data in simple_sorts:
                ...
                if sort_data == CC.SORT_FILES_BY_IMPORT_TIME:
                    ...
                    select_args_iterator = ((hash_id, file_service_id)
                                            for hash_id in hash_ids)
        ...
        if query is not None:
            ...
            hash_ids_and_other_data = list(
                self._ExecuteManySelect(query, select_args_iterator))
    ...
    def _UpdateDB(self, version):
        ...
        self._controller.pub('splash_set_status_text',
                             'updating db to v' + str(version + 1))

        if version == 330:

            try:

                domain_manager = self._GetJSONDump(
                ...
                login_manager = self._GetJSONDump(
                ...
                login_manager.OverwriteDefaultLoginScripts(
                    ['e-hentai login 2018.11.12'])

                #

                login_manager.TryToLinkMissingLoginScripts(
                    ['e-hentai.org'])  # remapping new login script
        ...
        if version == 331:
            ...
            self._controller.pub('splash_set_status_subtext',
                                 'updating some storage')

            self._c.execute(
                'ALTER TABLE json_dumps_named RENAME TO json_dumps_named_old;')
            ...
            self._c.execute(
                'INSERT INTO json_dumps_named ( dump_type, dump_name, version, timestamp, dump ) SELECT dump_type, dump_name, version, ?, dump FROM json_dumps_named_old;',
                (HydrusData.GetNow(), ))
        ...
        if version == 332:
            ...
            self._c.execute(
                'CREATE TABLE IF NOT EXISTS file_viewing_stats ( hash_id INTEGER PRIMARY KEY, preview_views INTEGER, preview_viewtime INTEGER, media_views INTEGER, media_viewtime INTEGER );'
            )
            self._CreateIndex('file_viewing_stats', ['preview_views'])
            self._CreateIndex('file_viewing_stats', ['preview_viewtime'])
            self._CreateIndex('file_viewing_stats', ['media_views'])
        ...
        if version == 337:
            ...
            try:
                ...
                domain_manager.OverwriteDefaultURLClasses([
                    'gelbooru gallery favorites page',
                    'gelbooru gallery pool page'
                ])

                #

                domain_manager.OverwriteDefaultGUGs([
                    'gelbooru pools (folders) by id',
                    'gelbooru favorites by user id'
                ])
                ...
                domain_manager.OverwriteDefaultParsers([
                    ...
                    'gelbooru 0.2.x gallery page parser'
                ])
            ...
            self._AddService(CC.CLIENT_API_SERVICE_KEY, HC.CLIENT_API_SERVICE,
                             'client api', dictionary)
        ...
        if version == 339:
            ...
            try:
                ...
                domain_manager.OverwriteDefaultURLClasses(
                    ['nijie artist page', 'nijie view', 'nijie view popup'])
                ...
                domain_manager.OverwriteDefaultParsers([
                    ...
                    'nijie artist gallery parser', 'nijie view parser',
        ...
        if version == 344:
            ...
            for (location, weight) in new_options._dictionary[
                    'client_files_locations_ideal_weights']:

                self._c.execute(
                    'INSERT INTO ideal_client_files_locations ( location, weight ) VALUES ( ?, ? );',
                    (location, weight))

            thumbnail_override_location = new_options._dictionary[
            ...
            for (i, prefix) in enumerate(HydrusData.IterateHexPrefixes()):
                ...
                self._controller.pub(
                    'splash_set_status_subtext',
                    'deleting resized thumbnails {}'.format(
                        HydrusData.ConvertValueRangeToPrettyString(i + 1,
                                                                   256)))
                ...
                try:
                    ...
                    (location, ) = self._c.execute(
                        'SELECT location FROM client_files_locations WHERE prefix = ?;',
                        (resized_prefix, )).fetchone()
                ...
                self._c.execute(
                    'DELETE FROM client_files_locations WHERE prefix = ?;',
                    (resized_prefix, ))
        ...
        if version == 345:
            ...
            if not HC.PLATFORM_WINDOWS:
                ...
                locations = self._STL(
                    self._c.execute(
                        'SELECT DISTINCT location FROM client_files_locations;'
                    ))
                ...
                if num_possible_resized_paths > 0:
                    ...
                    def qt_thumb_delete_warning_reattempt():
                        ...
                        message += os.linesep * 2
                        message += 'I have found {} spare \'rxx\' directories (this number should be less than or equal to 256) in these current locations:'.format(
                            num_possible_resized_paths)
                        message += os.linesep * 2
                        message += os.linesep.join([
                            HydrusPaths.ConvertPortablePathToAbsPath(location)
                            for location in
                            locations_where_r_folders_were_found
                        ])
                        message += os.linesep * 2
                    ...
                    self._controller.CallBlockingToQt(
                        None, qt_thumb_delete_warning_reattempt)

                    for (i, full_path) in enumerate(possible_resized_paths):

                        self._controller.pub(
                            'splash_set_status_subtext',
                            'deleting resized thumbnails 2: electric boogaloo {}'
                            .format(
                                HydrusData.ConvertValueRangeToPrettyString(
                                    i + 1, num_possible_resized_paths)))
        ...
        if version == 346:
            ...
            self._c.execute(
                'CREATE TABLE IF NOT EXISTS local_file_deletion_reasons ( hash_id INTEGER PRIMARY KEY, reason_id INTEGER );'
            )
        ...
        if version == 347:
            ...
            try:
                ...
                domain_manager.OverwriteDefaultURLClasses(
                    ['yiff.party file attachment long'])
        ...
        if version == 349:
            ...
            try:
                ...
                if default_gug == 'artstation artist lookup':
                    ...
                    new_options.SetString('default_gug_name',
                                          'safebooru tag search')
        ...
        if version == 350:
            ...
            self._controller.pub('splash_set_status_subtext',
                                 'generating new local tag cache')
            ...
            try:
                ...
                for tag_service_id in tag_service_ids:
                    ...
                    (cache_files_table_name, cache_current_mappings_table_name,
                     ...
                     cache_pending_mappings_table_name, ac_cache_table_name
                         ...
                         combined_local_file_service_id, tag_service_id)

                    service_tag_ids = self._STL(
                        self._c.execute('SELECT tag_id FROM ' +
                                        ac_cache_table_name +
                                        ' WHERE current_count > 0;'))
                ...
                for block_of_tag_ids in HydrusData.SplitListIntoChunks(
                        tag_ids, 1000):

                    self._controller.pub(
                        'splash_set_status_subtext',
                        'generating new local tag cache: {}'.format(
                            HydrusData.ConvertValueRangeToPrettyString(
                                i, num_to_do)))
                    ...
                    self._c.executemany(
                        'INSERT OR IGNORE INTO local_tags_cache ( tag_id, tag ) VALUES ( ?, ? );',
                        ((tag_id, self._tag_ids_to_tags_cache[tag_id])
                         for tag_id in block_of_tag_ids))
                ...
                self._controller.SafeShowCriticalMessage(
                    'problem updating!',
                    'When trying to update, I could not create a new local tag cache! Error information will follow. Please let hydrus dev know!'
                )
        ...
        if version == 353:
            ...
            alternate_pairs = self._c.execute(
                'SELECT smaller_hash_id, larger_hash_id FROM duplicate_pairs WHERE duplicate_type = ?;',
                (HC.DUPLICATE_ALTERNATE, )).fetchall()
            ...
            self._c.execute(
                'DELETE FROM duplicate_pairs WHERE duplicate_type = ?;',
                (HC.DUPLICATE_ALTERNATE, ))

            false_positive_pairs = self._c.execute(
                'SELECT smaller_hash_id, larger_hash_id FROM duplicate_pairs WHERE duplicate_type = ?;',
                (HC.DUPLICATE_FALSE_POSITIVE, )).fetchall()
        ...
        if version == 355:
            ...
            better_worse_pairs.extend(
                self._c.execute(
                    'SELECT smaller_hash_id, larger_hash_id FROM duplicate_pairs WHERE duplicate_type = ?;',
                    (HC.DUPLICATE_SMALLER_BETTER, )))
            better_worse_pairs.extend(
                self._c.execute(
                    'SELECT larger_hash_id, smaller_hash_id FROM duplicate_pairs WHERE duplicate_type = ?;',
                    (HC.DUPLICATE_LARGER_BETTER, )))

            same_pairs = self._c.execute(
                'SELECT smaller_hash_id, larger_hash_id FROM duplicate_pairs WHERE duplicate_type = ?;',
                (HC.DUPLICATE_SAME_QUALITY, )).fetchall()
            ...
            self._c.execute(
                'DELETE FROM duplicate_pairs WHERE duplicate_type IN ( ?, ?, ? );',
                (HC.DUPLICATE_SMALLER_BETTER, HC.DUPLICATE_LARGER_BETTER,
                 HC.DUPLICATE_SAME_QUALITY))
        ...
        if version == 356:
            ...
            try:
                ...
                new_options.SetInteger('duplicate_comparison_score_more_tags',
                                       8)
        ...
        if version == 357:
            ...
            result = self._c.execute(
                'SELECT 1 FROM external_caches.sqlite_master WHERE name = ?;',
                ('duplicate_pairs', )).fetchone()

            if result is not None:

                potential_pairs = self._c.execute(
                    'SELECT smaller_hash_id, larger_hash_id FROM duplicate_pairs WHERE duplicate_type = ?;',
                    (HC.DUPLICATE_POTENTIAL, )).fetchall()

                self._controller.pub('splash_set_status_subtext',
                                     'updating potential duplicate storage')

                smaller_hash_ids_to_larger_hash_ids = HydrusData.BuildKeyToListDict(
                    potential_pairs)

                for (smaller_hash_id, larger_hash_ids
                     ) in smaller_hash_ids_to_larger_hash_ids.items():

                    # we can't figure out searched distance quickly, so let's fudge an approx solution

                    result = self._c.execute(
                        'SELECT searched_distance FROM shape_search_cache WHERE hash_id = ?;',
                        (smaller_hash_id, )).fetchone()
                    ...
                    potential_duplicate_media_ids_and_distances = [
                        (self._DuplicatesGetMediaId(larger_hash_id),
                         searched_distance)
                        for larger_hash_id in larger_hash_ids
                    ]
            ...
            try:
                ...
                domain_manager.OverwriteDefaultParsers(
                    ('shimmie file page parser',
                     'deviant gallery page api parser',
                     'deviant art file page parser'))

                domain_manager.OverwriteDefaultGUGs(
                    ('deviant art tag search', 'deviant art artist lookup'))

                domain_manager.OverwriteDefaultURLClasses(
                    ('deviant art artist gallery page api',
                     'deviant art tag gallery page api'))
                ...
                login_scripts = [
                    login_script for login_script in login_scripts
                    if login_script.GetName() !=
                    'deviant art login (only works on a client that has already done some downloading)'
                ]
        ...
        if version == 359:
            ...
            result = self._c.execute(
                'SELECT 1 FROM external_caches.sqlite_master WHERE name = ?;',
                ('shape_maintenance_phash_regen', )).fetchone()
            ...
            if result is not None:
                ...
                try:
                    ...
                    self._c.execute(
                        'INSERT OR IGNORE INTO file_maintenance_jobs ( hash_id, job_type, time_can_start ) SELECT hash_id, ?, ? FROM shape_maintenance_phash_regen;',
                        (ClientFiles.
                         REGENERATE_FILE_DATA_JOB_SIMILAR_FILES_METADATA, 0))

                    self._c.execute(
                        'DROP TABLE shape_maintenance_phash_regen;')
        ...
        if version == 362:
            ...
            self._c.execute(
                'INSERT OR IGNORE INTO file_maintenance_jobs ( hash_id, job_type, time_can_start ) SELECT hash_id, ?, time_can_start FROM file_maintenance_jobs WHERE job_type = ?;',
            ...
            if one_row is None or len(
                    one_row) == 8:  # doesn't have has_audio yet

                self._controller.pub('splash_set_status_subtext',
                                     'adding \'has audio\' metadata column')

                existing_files_info = self._c.execute(
                    'SELECT * FROM files_info;').fetchall()
                ...
                insert_iterator = (
                    (hash_id, size, mime, width, height, duration, num_frames,
                     mime in HC.MIMES_THAT_DEFINITELY_HAVE_AUDIO, num_words)
                    for (hash_id, size, mime, width, height, duration,
                         num_frames, num_words) in existing_files_info)

                self._c.executemany(
                    'INSERT INTO files_info VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ? );',
                    insert_iterator)
                ...
                try:
                    ...
                    service_id = self._GetServiceId(
                        CC.COMBINED_LOCAL_FILE_SERVICE_KEY)

                    self._c.execute(
                        'INSERT OR IGNORE INTO file_maintenance_jobs ( hash_id, job_type, time_can_start ) SELECT hash_id, ?, ? FROM files_info NATURAL JOIN current_files WHERE service_id = ? AND mime IN '
                        + HydrusData.SplayListForDB(
                            HC.MIMES_THAT_MAY_HAVE_AUDIO) + ';',
                        (ClientFiles.REGENERATE_FILE_DATA_JOB_FILE_METADATA, 0,
                         service_id))
        ...
        if version == 364:
            ...
            try:
                ...
                (options, ) = self._c.execute(
                    'SELECT options FROM options;').fetchone()
                ...
                namespaces = [
                    n for (t, n) in default_collect if t == 'namespace'
                ]
                rating_service_keys = [
                    bytes.fromhex(r) for (t, r) in default_collect
                    if t == 'rating'
                ]

                default_media_collect = ClientMedia.MediaCollect(
        ...
        if version == 367:
            ...
            try:
                ...
                result = self._c.execute(
                    'SELECT name FROM services WHERE service_key = ?;',
                    (sqlite3.Binary(
                        CC.DEFAULT_LOCAL_TAG_SERVICE_KEY), )).fetchone()
                ...
                if result is not None:
                    ...
                    if service_name == 'local tags':
                        ...
                        self._c.execute(
                            'UPDATE services SET name = ? WHERE service_key = ?;',
                            (
                                'my tags',
                                sqlite3.Binary(
            ...
            try:
                ...
                bandwidth_manager = self._GetJSONDump(
                    HydrusSerialisable.
                    SERIALISABLE_TYPE_NETWORK_BANDWIDTH_MANAGER)

                #

                hydrus_default_nc = ClientNetworkingContexts.NetworkContext(
                ...
                old_defaults.AddRule(HC.BANDWIDTH_TYPE_DATA, 86400,
                                     64 * 1024 * 1024)

                if current_rules.GetSerialisableTuple(
                ) == old_defaults.GetSerialisableTuple():

                    new_rules = HydrusNetworking.BandwidthRules()

                    new_rules.AddRule(HC.BANDWIDTH_TYPE_DATA, 86400,
                                      512 * 1024 * 1024)
            ...
            try:
                ...
                session_manager = self._GetJSONDump(
                    HydrusSerialisable.
                    SERIALISABLE_TYPE_NETWORK_SESSION_MANAGER)
                ...
                for service in services:
                    ...
                    nc = ClientNetworkingContexts.NetworkContext(
                        CC.NETWORK_CONTEXT_HYDRUS, service_key)
                    ...
                    if host == 'hydrus.no-ip.org':
                        ...
                        if port == 45872:
                            ...
                            def ask_what_to_do():
                                ...
                                message += os.linesep * 2
                                message += 'The PTR is at a new address, "https://ptr.hydrus.network:45871". Would you like to automatically redirect your client\'s PTR service, "{}", to that new location and keep using it?'.format(
                                    name)
                                ...
                                result = ClientGUIDialogsQuick.GetYesNo(
                                    ...
                                    title='PTR has moved!',
                                    yes_label=
                                    'yes, move me to the new location',
                                    no_label='no, pause my ptr as it is')

                                return result

                            result = self._controller.CallBlockingToQt(
                                None, ask_what_to_do)
        ...
        if version == 368:
            ...
            self._c.execute(
                'CREATE TABLE IF NOT EXISTS file_modified_timestamps ( hash_id INTEGER PRIMARY KEY, file_modified_timestamp INTEGER );'
            )
            self._CreateIndex('file_modified_timestamps',
                              ['file_modified_timestamp'])
            ...
            try:
                ...
                self._controller.pub('splash_set_status_subtext',
                                     'queueing up modified timestamp jobs')

                service_id = self._GetServiceId(
                    CC.COMBINED_LOCAL_FILE_SERVICE_KEY)

                self._c.execute(
                    'INSERT OR IGNORE INTO file_maintenance_jobs ( hash_id, job_type, time_can_start ) SELECT hash_id, ?, ? FROM files_info NATURAL JOIN current_files WHERE service_id = ?;',
                    (ClientFiles.
                     REGENERATE_FILE_DATA_JOB_FILE_MODIFIED_TIMESTAMP, 0,
                     service_id))
        ...
        if version == 369:
            ...
            try:
                ...
                for service_id in service_ids:
                    ...
                    for update_index in update_indices_to_reset:
                        ...
                        content_hash_ids = self._STS(
                            self._c.execute(
                                'SELECT hash_id from {} NATURAL JOIN files_info WHERE update_index = ? AND mime = ?;'
                                .format(repository_updates_table_name),
                                (update_index,
                    ...
                    self._c.executemany(
                        'UPDATE {} SET processed = ? WHERE hash_id = ?;'.
                        format(repository_updates_table_name),
                        ((False, hash_id)
                         for hash_id in content_hash_ids_to_reset))
            ...
            try:
                ...
                if result is not None:
                    ...
                    old_tag_censorship = self._c.execute(
                        'SELECT service_id, blacklist, tags FROM tag_censorship;'
                    ).fetchall()
                    ...
                    for (service_id, blacklist, tags) in old_tag_censorship:
                        ...
                        tag_display_manager.SetTagFilter(
            ...
            try:
                ...
                domain_manager.OverwriteDefaultURLClasses(
                    ('pixiv file page new format (without language)',
                     'pixiv file page new format (with language)'))
        ...
        if version == 370:
            ...
            try:
                ...
                names_to_tag_filters[
                    'exclude long/spammy namespaces'] = tag_filter
        ...
        if version == 375:
            ...
            try:
                ...
                domain_manager.OverwriteDefaultGUGs(
                    ('pixiv tag search', 'twitter username lookup'))
                domain_manager.OverwriteDefaultURLClasses(
                    ('pixiv search api', 'twitter tweets api - media only'))
                domain_manager.OverwriteDefaultParsers(
                    ('pixiv tag search api parser',
                     'twitter tweet parser (video from koto.reisen)',
                     'twitter media tweets api parser'))
        ...
        if version == 376:
            ...
            result = self._c.execute(
                'SELECT 1 FROM external_master.sqlite_master WHERE name = ?;',
                ('url_domains', )).fetchone()
            ...
            try:
                ...
                if result is None:
                    ...
                    self._controller.pub('splash_set_status_subtext',
                                         'compressing url storage--creating')
                    ...
                    self._controller.pub(
                        'splash_set_status_subtext',
                        'compressing url storage--populating domains')

                    self._c.execute(
                        'INSERT INTO url_domains ( domain ) SELECT DISTINCT domain FROM urls_old;'
                    )

                    self._controller.pub(
                        'splash_set_status_subtext',
                        'compressing url storage--populating urls')

                    self._c.execute(
                        'INSERT INTO urls ( url_id, domain_id, url ) SELECT url_id, domain_id, url FROM urls_old NATURAL JOIN url_domains;'
                    )

                    self._controller.pub('splash_set_status_subtext',
                                         'compressing url storage--indexing')
                    ...
                    self._controller.pub(
                        'splash_set_status_subtext',
                        'compressing url storage--optimising')
        ...
        if version == 378:
            ...
            try:
                ...
                for (login_domain,
                     (login_script_key_and_name, credentials,
                      ...
                      no_work_until_reason)) in list(
                          domains_to_login_info.items()):

                    (login_script_key,
                     login_script_name) = login_script_key_and_name

                    if login_domain == 'www.pixiv.net' and login_script_name == 'pixiv login' and active:

                        active = False

                        domains_to_login_info[login_domain] = (
                            login_script_key_and_name, credentials,
                            login_access_type, login_access_text, active,
                            validity, validity_error_text, no_work_until,
                            no_work_until_reason)

                        login_manager.SetDomainsToLoginInfo(
                            domains_to_login_info)

                        self._SetJSONDump(login_manager)

                        self.pub_initial_message(
                            'The default Pixiv login script no longer works. It appeared to be active for you, so it has been deactivated. Please use the Hydrus Companion web browser addon to log in to Pixiv.'
                        )
            ...
            try:
                ...
                domain_manager.OverwriteDefaultGUGs([
                    'derpibooru tag search',
                    'derpibooru tag search - no filter'
                ])

                #

                domain_manager.OverwriteDefaultURLClasses(
                    ['derpibooru gallery page', 'derpibooru file page'])

                #

                domain_manager.OverwriteDefaultParsers([
                    'derpibooru.org file page parser',
                    'derpibooru gallery page parser'
                ])
        ...
        if version == 379:
            ...
            try:
                ...
                domain_manager.OverwriteDefaultURLClasses([
                    'pixiv artist page', '8kun thread', '8kun thread json api',
                    'vch.moe thread', 'vch.moe thread json api'
                ])
                ...
                domain_manager.OverwriteDefaultParsers([
                    '4chan-style thread api parser', '8kun thread api parser'
                ])
        ...
        if version == 382:
            ...
            existing_shortcut_names = self._GetJSONDumpNames(
        ...
        if version == 383:
            ...
            for new_name in ('media_viewer_media_window',
            ...
            if 'media_viewer_browser' in existing_shortcut_names:
                ...
                try:
                    ...
                    media_viewer_browser_shortcuts = self._GetJSONDumpNamed(
                        HydrusSerialisable.SERIALISABLE_TYPE_SHORTCUT_SET,
                        dump_name='media_viewer_browser')

                    right_up = ClientGUIShortcuts.Shortcut(
                        CC.SHORTCUT_TYPE_MOUSE, CC.SHORTCUT_MOUSE_RIGHT,
                        CC.SHORTCUT_PRESS_TYPE_RELEASE, [])

                    if media_viewer_browser_shortcuts.GetCommand(
                            right_up) is None:

                        media_viewer_browser_shortcuts.SetCommand(
                            right_up,
        ...
        self._controller.pub('splash_set_title_text',
                             'updated db to v' + str(version + 1))
    ...
    def _UpdateMappings(self,
                        tag_service_id,
                        mappings_ids=None,
                        deleted_mappings_ids=None,
                        pending_mappings_ids=None,
                        pending_rescinded_mappings_ids=None,
                        petitioned_mappings_ids=None,
                        petitioned_rescinded_mappings_ids=None):
        ...
        if pending_rescinded_mappings_ids is None:
            pending_rescinded_mappings_ids = []
        if petitioned_mappings_ids is None: petitioned_mappings_ids = []
        if petitioned_rescinded_mappings_ids is None:
            petitioned_rescinded_mappings_ids = []
        ...
        hash_ids_being_added = {
            hash_id
            for hash_id in itertools.chain.from_iterable(hash_ids_lists)
        }
        ...
        hash_ids_being_removed = {
            hash_id
            for hash_id in itertools.chain.from_iterable(hash_ids_lists)
        }

        tag_ids_to_search_for = tag_ids_being_added.union(
            tag_ids_being_removed)
        hash_ids_to_search_for = hash_ids_being_added.union(
            hash_ids_being_removed)
        ...
        self._c.executemany(
            'INSERT INTO temp_tag_ids ( tag_id ) VALUES ( ? );',
            ((tag_id, ) for tag_id in tag_ids_to_search_for))
        self._c.executemany(
            'INSERT INTO temp_hash_ids ( hash_id ) VALUES ( ? );',
            ((hash_id, ) for hash_id in hash_ids_to_search_for))

        pre_existing_tag_ids = self._STS(
            self._c.execute(
                'SELECT tag_id as t FROM temp_tag_ids WHERE EXISTS ( SELECT 1 FROM '
                + current_mappings_table_name + ' WHERE tag_id = t );'))
        pre_existing_hash_ids = self._STS(
            self._c.execute(
                'SELECT hash_id as h FROM temp_hash_ids WHERE EXISTS ( SELECT 1 FROM '
                + current_mappings_table_name + ' WHERE hash_id = h );'))

        num_tags_added = len(
            tag_ids_being_added.difference(pre_existing_tag_ids))
        num_files_added = len(
            hash_ids_being_added.difference(pre_existing_hash_ids))
        ...
        if len(mappings_ids) > 0:
            ...
            for file_service_id in file_service_ids:
                ...
                self._CacheSpecificMappingsAddMappings(file_service_id,
                                                       tag_service_id,
                                                       mappings_ids)
        ...
        if len(deleted_mappings_ids) > 0:
            ...
            for file_service_id in file_service_ids:
                ...
                self._CacheSpecificMappingsDeleteMappings(
                    file_service_id, tag_service_id, deleted_mappings_ids)
        ...
        if len(pending_mappings_ids) > 0:
            ...
            for (tag_id, hash_ids) in pending_mappings_ids:
                ...
                select_args_iterator = ((tag_id, hash_id)
                                        for hash_id in hash_ids)

                existing_current_hash_ids = self._STS(
                    self._ExecuteManySelect(select_statement,
                                            select_args_iterator))

                valid_hash_ids = set(hash_ids).difference(
                    existing_current_hash_ids)
            ...
            for (tag_id, hash_ids) in pending_mappings_ids:
                ...
                self._c.executemany(
                    'INSERT OR IGNORE INTO ' + pending_mappings_table_name +
                    ' VALUES ( ?, ? );',
                    ((tag_id, hash_id) for hash_id in hash_ids))
            ...
            for file_service_id in file_service_ids:
                ...
                self._CacheSpecificMappingsPendMappings(
                    file_service_id, tag_service_id, pending_mappings_ids)
        ...
        if len(pending_rescinded_mappings_ids) > 0:
            ...
            for file_service_id in file_service_ids:
                ...
                self._CacheSpecificMappingsRescindPendingMappings(
                    file_service_id, tag_service_id,
                    pending_rescinded_mappings_ids)

        combined_files_seen_ids = set(
            (key
             for (key, value) in list(combined_files_current_counter.items())
             if value != 0))
        combined_files_seen_ids.update(
            (key
             for (key, value) in list(combined_files_pending_counter.items())
             if value != 0))

        combined_files_counts = [
            (tag_id, combined_files_current_counter[tag_id],
             combined_files_pending_counter[tag_id])
            for tag_id in combined_files_seen_ids
        ]

        self._CacheCombinedFilesMappingsUpdate(tag_service_id,
                                               combined_files_counts)

        #

        post_existing_tag_ids = self._STS(
            self._c.execute(
                'SELECT tag_id as t FROM temp_tag_ids WHERE EXISTS ( SELECT 1 FROM '
                + current_mappings_table_name + ' WHERE tag_id = t );'))
        post_existing_hash_ids = self._STS(
            self._c.execute(
                'SELECT hash_id as h FROM temp_hash_ids WHERE EXISTS ( SELECT 1 FROM '
                + current_mappings_table_name + ' WHERE hash_id = h );'))
        ...
        num_tags_removed = len(
            pre_existing_tag_ids.intersection(
                tag_ids_being_removed).difference(post_existing_tag_ids))
        num_files_removed = len(
            pre_existing_hash_ids.intersection(
                hash_ids_being_removed).difference(post_existing_hash_ids))
        ...
        for (tag_id, hash_ids, reason_id) in petitioned_mappings_ids:
            ...
            self._c.executemany(
                'INSERT OR IGNORE INTO ' + petitioned_mappings_table_name +
                ' VALUES ( ?, ?, ? );',
                [(tag_id, hash_id, reason_id) for hash_id in hash_ids])
        ...
        if change_in_num_mappings != 0:
            service_info_updates.append(
                (change_in_num_mappings, tag_service_id,
                 HC.SERVICE_INFO_NUM_MAPPINGS))
        if change_in_num_deleted_mappings != 0:
            service_info_updates.append(
                (change_in_num_deleted_mappings, tag_service_id,
                 HC.SERVICE_INFO_NUM_DELETED_MAPPINGS))
        if change_in_num_pending_mappings != 0:
            service_info_updates.append(
                (change_in_num_pending_mappings, tag_service_id,
                 HC.SERVICE_INFO_NUM_PENDING_MAPPINGS))
        if change_in_num_petitioned_mappings != 0:
            service_info_updates.append(
                (change_in_num_petitioned_mappings, tag_service_id,
                 HC.SERVICE_INFO_NUM_PETITIONED_MAPPINGS))
        if change_in_num_tags != 0:
            service_info_updates.append(
                (change_in_num_tags, tag_service_id, HC.SERVICE_INFO_NUM_TAGS))
        if change_in_num_files != 0:
            service_info_updates.append((change_in_num_files, tag_service_id,
                                         HC.SERVICE_INFO_NUM_FILES))
    ...
    def _UpdateServerServices(self, admin_service_key, serverside_services,
        ...
        for serverside_service in serverside_services:
            ...
            if service_key in current_service_keys:
                ...
                if service_key in service_keys_to_access_keys:
                    ...
                    self._AddService(service_key, service_type, name,
                                     dictionary)
    ...
    def _UpdateService(self, service):
        ...
        (old_dictionary_string, ) = self._c.execute(
            'SELECT dictionary_string FROM services WHERE service_id = ?;',
            (service_id, )).fetchone()

        old_dictionary = HydrusSerialisable.CreateFromString(
            old_dictionary_string)

        dictionary_string = dictionary.DumpToString()

        self._c.execute(
            'UPDATE services SET name = ?, dictionary_string = ? WHERE service_id = ?;',
            (name, dictionary_string, service_id))
    ...
    def _Vacuum(self,
        ...
        maintenance_vacuum_period_days = new_options.GetNoneableInteger(
            'maintenance_vacuum_period_days')
        ...
        if len(due_names) > 0:
            ...
            try:
                ...
                for name in due_names:
                    ...
                    if self._controller.ShouldStopThisWork(
                            maintenance_mode, stop_time=stop_time):
                    ...
                    try:
                        ...
                        new_options.SetNoneableInteger(
                            'maintenance_vacuum_period_days', None)
                ...
                self._c.executemany(
                    'DELETE FROM vacuum_timestamps WHERE name = ?;',
                    ((name, ) for name in names_done))

                self._c.executemany(
                    'INSERT OR IGNORE INTO vacuum_timestamps ( name, timestamp ) VALUES ( ?, ? );',
                    ((name, HydrusData.GetNow()) for name in names_done))
    ...
    def _Write(self, action, *args, **kwargs):
        ...
        elif action == 'associate_repository_update_hashes':
            self._AssociateRepositoryUpdateHashes(*args, **kwargs)
        elif action == 'backup':
            self._Backup(*args, **kwargs)
        elif action == 'clear_false_positive_relations':
            self._DuplicatesClearAllFalsePositiveRelationsFromHashes(
                *args, **kwargs)
        elif action == 'clear_false_positive_relations_between_groups':
            self._DuplicatesClearFalsePositiveRelationsBetweenGroupsFromHashes(
                *args, **kwargs)
        elif action == 'clear_orphan_file_records':
            self._ClearOrphanFileRecords(*args, **kwargs)
        elif action == 'clear_orphan_tables':
            self._ClearOrphanTables(*args, **kwargs)
        elif action == 'content_updates':
            self._ProcessContentUpdates(*args, **kwargs)
        elif action == 'cull_file_viewing_statistics':
            self._CullFileViewingStatistics(*args, **kwargs)
        elif action == 'db_integrity':
            self._CheckDBIntegrity(*args, **kwargs)
        elif action == 'delete_imageboard':
            self._DeleteYAMLDump(YAML_DUMP_ID_IMAGEBOARD, *args, **kwargs)
        elif action == 'delete_local_booru_share':
            self._DeleteYAMLDump(YAML_DUMP_ID_LOCAL_BOORU, *args, **kwargs)
        elif action == 'delete_pending':
            self._DeletePending(*args, **kwargs)
        elif action == 'delete_serialisable_named':
            self._DeleteJSONDumpNamed(*args, **kwargs)
        elif action == 'delete_service_info':
            self._DeleteServiceInfo(*args, **kwargs)
        elif action == 'delete_potential_duplicate_pairs':
            self._DuplicatesDeleteAllPotentialDuplicatePairs(*args, **kwargs)
        elif action == 'dirty_services':
            self._SaveDirtyServices(*args, **kwargs)
        elif action == 'dissolve_alternates_group':
            self._DuplicatesDissolveAlternatesGroupIdFromHashes(
                *args, **kwargs)
        elif action == 'dissolve_duplicates_group':
            self._DuplicatesDissolveMediaIdFromHashes(*args, **kwargs)
        elif action == 'duplicate_pair_status':
            self._DuplicatesSetDuplicatePairStatus(*args, **kwargs)
        elif action == 'duplicate_set_king':
            self._DuplicatesSetKingFromHash(*args, **kwargs)
        elif action == 'file_maintenance_add_jobs':
            self._FileMaintenanceAddJobs(*args, **kwargs)
        elif action == 'file_maintenance_add_jobs_hashes':
            self._FileMaintenanceAddJobsHashes(*args, **kwargs)
        elif action == 'file_maintenance_cancel_jobs':
            self._FileMaintenanceCancelJobs(*args, **kwargs)
        elif action == 'file_maintenance_clear_jobs':
            self._FileMaintenanceClearJobs(*args, **kwargs)
        elif action == 'imageboard':
            self._SetYAMLDump(YAML_DUMP_ID_IMAGEBOARD, *args, **kwargs)
        elif action == 'ideal_client_files_locations':
            self._SetIdealClientFilesLocations(*args, **kwargs)
        elif action == 'import_file':
            result = self._ImportFile(*args, **kwargs)
        elif action == 'import_update':
            self._ImportUpdate(*args, **kwargs)
        elif action == 'last_shutdown_work_time':
            self._SetLastShutdownWorkTime(*args, **kwargs)
        elif action == 'local_booru_share':
            self._SetYAMLDump(YAML_DUMP_ID_LOCAL_BOORU, *args, **kwargs)
        elif action == 'maintain_similar_files_search_for_potential_duplicates':
            self._PHashesSearchForPotentialDuplicates(*args, **kwargs)
        elif action == 'maintain_similar_files_tree':
            self._PHashesMaintainTree(*args, **kwargs)
        elif action == 'migration_clear_job':
            self._MigrationClearJob(*args, **kwargs)
        elif action == 'migration_start_mappings_job':
            self._MigrationStartMappingsJob(*args, **kwargs)
        elif action == 'migration_start_pairs_job':
            self._MigrationStartPairsJob(*args, **kwargs)
        elif action == 'process_repository_content':
            result = self._ProcessRepositoryContent(*args, **kwargs)
        elif action == 'process_repository_definitions':
            result = self._ProcessRepositoryDefinitions(*args, **kwargs)
        elif action == 'push_recent_tags':
            self._PushRecentTags(*args, **kwargs)
        elif action == 'regenerate_ac_cache':
            self._RegenerateACCache(*args, **kwargs)
        elif action == 'regenerate_similar_files':
            self._PHashesRegenerateTree(*args, **kwargs)
        elif action == 'relocate_client_files':
            self._RelocateClientFiles(*args, **kwargs)
        elif action == 'remove_alternates_member':
            self._DuplicatesRemoveAlternateMemberFromHashes(*args, **kwargs)
        elif action == 'remove_duplicates_member':
            self._DuplicatesRemoveMediaIdMemberFromHashes(*args, **kwargs)
        elif action == 'remove_potential_pairs':
            self._DuplicatesRemovePotentialPairsFromHashes(*args, **kwargs)
        elif action == 'repair_client_files':
            self._RepairClientFiles(*args, **kwargs)
        elif action == 'reprocess_repository':
            self._ReprocessRepository(*args, **kwargs)
        elif action == 'reset_repository':
            self._ResetRepository(*args, **kwargs)
        elif action == 'reset_potential_search_status':
            self._PHashesResetSearchFromHashes(*args, **kwargs)
        elif action == 'save_options':
            self._SaveOptions(*args, **kwargs)
        elif action == 'serialisable_simple':
            self._SetJSONSimple(*args, **kwargs)
        elif action == 'serialisable':
            self._SetJSONDump(*args, **kwargs)
        elif action == 'serialisables_overwrite':
            self._OverwriteJSONDumps(*args, **kwargs)
        elif action == 'set_password':
            self._SetPassword(*args, **kwargs)
        elif action == 'schedule_repository_update_file_maintenance':
            self._ScheduleRepositoryUpdateFileMaintenanceFromServiceKey(
                *args, **kwargs)
        elif action == 'update_server_services':
            self._UpdateServerServices(*args, **kwargs)
        elif action == 'update_services':
            self._UpdateServices(*args, **kwargs)
        elif action == 'vacuum':
    ...
    def pub_content_updates_after_commit(self,
                                         service_keys_to_content_updates):

        self.pub_after_job('content_updates_data',
                           service_keys_to_content_updates)
        self.pub_after_job('content_updates_gui',
                           service_keys_to_content_updates)
    ...
    def pub_service_updates_after_commit(self,
                                         service_keys_to_service_updates):

        self.pub_after_job('service_updates_data',
                           service_keys_to_service_updates)
        self.pub_after_job('service_updates_gui',
                           service_keys_to_service_updates)
