...
class EditAccountTypePanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, service_type, account_type):
        ...
        (self._account_type_key, title, permissions,
         bandwidth_rules) = account_type.ToTuple()
        ...
        self._permissions_panel = ClientGUICommon.StaticBox(
            self, 'permissions')
        ...
        for (content_type, action_rows) in permission_choices:
            ...
            choice_control = ClientGUICommon.BetterChoice(
                self._permissions_panel)
        ...
        gridbox = ClientGUICommon.WrapInGrid(self._permissions_panel,
                                             gridbox_rows)

        self._bandwidth_rules_control = ClientGUIControls.BandwidthRulesCtrl(
            self, bandwidth_rules)
        ...
        self._permissions_panel.Add(gridbox,
                                    CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)

        vbox = QP.VBoxLayout()

        QP.AddToLayout(vbox, t_hbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, self._permissions_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._bandwidth_rules_control,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _GeneratePermissionChoices(self, service_type):
        ...
        possible_permissions = HydrusNetwork.GetPossiblePermissions(
            service_type)
        ...
        for (content_type, possible_actions) in possible_permissions:
            ...
            for action in possible_actions:
                ...
                choices.append(
                    (HC.permission_pair_string_lookup[(content_type, action)],
                     action))
    ...
    def GetValue(self):
        ...
        return HydrusNetwork.AccountType.GenerateAccountTypeFromParameters(
            self._account_type_key, title, permissions, bandwidth_rules)
...
class EditBandwidthRulesPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, bandwidth_rules, summary=''):
        ...
        self._bandwidth_rules_ctrl = ClientGUIControls.BandwidthRulesCtrl(
            self, bandwidth_rules)
        ...
        QP.AddToLayout(vbox, self._bandwidth_rules_ctrl,
...
class EditCookiePanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, name, value, domain, path, expires):
        ...
        self._expires_time_delta = ClientGUITime.TimeDeltaButton(expires_panel,
                                                                 min=1200,
                                                                 days=True,
                                                                 hours=True,
                                                                 minutes=True)
        ...
        rows.append(
            ('Actual expires as UTC Timestamp: ', self._expires_st_utc))
        rows.append(
            ('Set expires as a delta from now: ', self._expires_time_delta))
    ...
    def _UpdateExpiresText(self):
        ...
        self._expires_st.setText(
            HydrusData.ConvertTimestampToPrettyExpires(self._expires))
...
class EditDefaultTagImportOptionsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, url_classes, parsers,
        ...
        self._parser_keys_to_parsers = {
            parser.GetParserKey(): parser
            for parser in self._parsers
        }

        self._url_class_keys_to_tag_import_options = dict(
            url_class_keys_to_tag_import_options)
        ...
        self._file_post_default_tag_import_options_button = ClientGUIImport.TagImportOptionsButton(
            self, file_post_default_tag_import_options,
            show_downloader_options)
        self._watchable_default_tag_import_options_button = ClientGUIImport.TagImportOptionsButton(
            self, watchable_default_tag_import_options,
            show_downloader_options)
        ...
        self._list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._list_ctrl_panel,
            'default_tag_import_options',
            15,
            36,
            columns,
            self._ConvertDataToListCtrlTuples,
            delete_key_callback=self._Clear,
            activation_callback=self._Edit)

        self._list_ctrl_panel.SetListCtrl(self._list_ctrl)

        self._list_ctrl_panel.AddButton(
            'copy', self._Copy, enabled_check_func=self._OnlyOneTIOSelected)
        self._list_ctrl_panel.AddButton('paste',
                                        self._Paste,
                                        enabled_only_on_selection=True)
        self._list_ctrl_panel.AddButton('edit',
                                        self._Edit,
                                        enabled_only_on_selection=True)
        self._list_ctrl_panel.AddButton('clear',
                                        self._Clear,
                                        enabled_only_on_selection=True)

        #

        eligible_url_classes = [
            url_class for url_class in url_classes
            if url_class.GetURLType() in (HC.URL_TYPE_POST,
                                          HC.URL_TYPE_WATCHABLE)
            and url_class.GetMatchKey() in self._url_class_keys_to_parser_keys
        ]
        ...
        rows.append(('default for file posts: ',
                     self._file_post_default_tag_import_options_button))
        rows.append(('default for watchable urls: ',
                     self._watchable_default_tag_import_options_button))
    ...
    def _Clear(self):
        ...
        result = ClientGUIDialogsQuick.GetYesNo(
            self, 'Clear default tag import options for all selected?')
        ...
        if result == QW.QDialog.Accepted:
            ...
            for url_class in url_classes_to_clear:
                ...
                if url_class_key in self._url_class_keys_to_tag_import_options:
                    ...
                    del self._url_class_keys_to_tag_import_options[
                        url_class_key]
    ...
    def _Copy(self):
        ...
        if len(selected) == 1:
            ...
            if url_class_key in self._url_class_keys_to_tag_import_options:
                ...
                tag_import_options = self._url_class_keys_to_tag_import_options[
                    url_class_key]
    ...
    def _Edit(self):
        ...
        for url_class in url_classes_to_edit:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                    self, 'edit tag import options') as dlg:

                tag_import_options = self._GetDefaultTagImportOptions(
                    url_class)
                show_downloader_options = True
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    self._url_class_keys_to_tag_import_options[
                        url_class_key] = tag_import_options
        ...
        if url_class_key in self._url_class_keys_to_tag_import_options:
            ...
            if url_type == HC.URL_TYPE_POST:
                ...
                raise HydrusExceptions.URLClassException(
                    'Could not find tag import options for that kind of URL Class!'
                )
    ...
    def _Paste(self):
        ...
        try:
            ...
            if not isinstance(tag_import_options,
                              ClientImportOptions.TagImportOptions):
            ...
            for url_class in self._list_ctrl.GetData(only_selected=True):
                ...
                self._url_class_keys_to_tag_import_options[
                    url_class_key] = tag_import_options.Duplicate()
    ...
    def GetValue(self):
        ...
        return (file_post_default_tag_import_options,
                watchable_default_tag_import_options,
                self._url_class_keys_to_tag_import_options)
...
class EditDeleteFilesPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self,
        ...
        self._simple_description = ClientGUICommon.BetterStaticText(
            self, label='init')
        ...
        self._action_radio = ClientGUICommon.BetterRadioBox(
            self, choices=self._permitted_action_choices, vertical=True)
        ...
        for s in HG.client_controller.new_options.GetStringList(
        ...
        self._reason_radio = ClientGUICommon.BetterRadioBox(
            self._reason_panel,
            choices=permitted_reason_choices,
            vertical=True)
        ...
        self._reason_panel.Add(self._reason_radio,
                               CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        QP.AddToLayout(vbox, self._simple_description,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _InitialisePermittedActionChoices(self,
        ...
        if suggested_file_service_key == CC.LOCAL_FILE_SERVICE_KEY:
            ...
            possible_file_service_keys.append(
        ...
        keys_to_hashes = {
            possible_file_service_key: [
                m.GetHash() for m in self._media if possible_file_service_key
                in m.GetLocationsManager().GetCurrent()
            ]
            ...
            if len(hashes) > 0:
                ...
                if possible_file_service_key == CC.LOCAL_FILE_SERVICE_KEY:
                    ...
                    if num_to_delete == 1:
                        ...
                        text = 'Send these ' + HydrusData.ToHumanInt(
                            num_to_delete) + ' files to the trash?'
                    ...
                    if num_to_delete == 1:
                        ...
                        text = 'Permanently delete these ' + HydrusData.ToHumanInt(
                            num_to_delete) + ' files?'
                    ...
                    if CC.LOCAL_FILE_SERVICE_KEY in keys_to_hashes and len(
                            keys_to_hashes[CC.LOCAL_FILE_SERVICE_KEY]) > 0:
                    ...
                    if num_to_delete == 1: text = 'Admin-delete this file?'
                        ...
                        text = 'Admin-delete these ' + HydrusData.ToHumanInt(
                            num_to_delete) + ' files?'

                self._permitted_action_choices.append(
                    (text, (possible_file_service_key, hashes, text)))

        if HG.client_controller.new_options.GetBoolean(
                'use_advanced_file_deletion_dialog'):

            hashes = [
                m.GetHash() for m in self._media
                if CC.COMBINED_LOCAL_FILE_SERVICE_KEY in
                m.GetLocationsManager().GetCurrent()
            ]
            ...
            if len(hashes) > 0:
                ...
                if num_to_delete == 1:
                    ...
                    text = 'Permanently delete these ' + HydrusData.ToHumanInt(
                        num_to_delete
                    ) + ' files and do not save a deletion record?'

                self._permitted_action_choices.append(
                    (text, ('clear_delete', hashes, text)))

        if len(self._permitted_action_choices) == 0:

            raise HydrusExceptions.CancelledException(
                'No valid delete choices!')

    def _UpdateControls(self):

        (file_service_key, hashes, description) = self._action_radio.GetValue()

        reason_permitted = file_service_key in (CC.LOCAL_FILE_SERVICE_KEY,
                                                'physical_delete')
    ...
    def GetValue(self):
        ...
        if file_service_key in local_file_services:
            ...
            jobs = [{
                file_service_key: [content_update]
            ...
            chunks_of_hashes = list(HydrusData.SplitListIntoChunks(
                hashes,
                64))  # iterator, so list it to use it more than once, jej
            ...
            content_updates = [
                HydrusData.ContentUpdate(HC.CONTENT_TYPE_FILES,
                                         HC.CONTENT_UPDATE_ADVANCED,
                                         ('delete_deleted', chunk_of_hashes))
                for chunk_of_hashes in chunks_of_hashes
            ]
...
class EditDomainManagerInfoPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, url_classes,
        ...
        self._url_classes_panel = EditURLClassesPanel(self._notebook,
                                                      url_classes)
        self._network_contexts_to_custom_header_dicts_panel = EditNetworkContextCustomHeadersPanel(
            self._notebook, network_contexts_to_custom_header_dicts)

        self._notebook.addTab(self._url_classes_panel, 'url classes')
        self._notebook.setCurrentWidget(self._url_classes_panel)
        self._notebook.addTab(
            self._network_contexts_to_custom_header_dicts_panel,
            'custom headers')
...
class EditDownloaderDisplayPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, network_engine, gugs, gug_keys_to_display,
                 url_classes, url_class_keys_to_display):
        ...
        self._url_class_keys_to_url_classes = {
            url_class.GetMatchKey(): url_class
        ...
        self._gug_display_list_ctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self._notebook)

        columns = [('downloader', -1), ('show in main selector list?', 29)]

        self._gug_display_list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._gug_display_list_ctrl_panel,
            'gug_keys_to_display',
            15,
            36,
            columns,
            self._ConvertGUGDisplayDataToListCtrlTuples,
            activation_callback=self._EditGUGDisplay)

        self._gug_display_list_ctrl_panel.SetListCtrl(
            self._gug_display_list_ctrl)

        self._gug_display_list_ctrl_panel.AddButton(
            'edit', self._EditGUGDisplay, enabled_only_on_selection=True)

        #

        self._url_display_list_ctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self._notebook)

        columns = [('url class', -1), ('url type', 20),
                   ('display on media viewer?', 36)]

        self._url_display_list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._url_display_list_ctrl_panel,
            'url_class_keys_to_display',
            15,
            36,
            columns,
            self._ConvertURLDisplayDataToListCtrlTuples,
            activation_callback=self._EditURLDisplay)

        self._url_display_list_ctrl_panel.SetListCtrl(
            self._url_display_list_ctrl)

        self._url_display_list_ctrl_panel.AddButton(
            'edit', self._EditURLDisplay, enabled_only_on_selection=True)
        ...
        for (url_class_key,
             url_class) in list(self._url_class_keys_to_url_classes.items()):
        ...
        self._notebook.addTab(self._gug_display_list_ctrl_panel,
                              'downloaders selector')
        self._notebook.setCurrentWidget(self._gug_display_list_ctrl_panel)
        self._notebook.addTab(self._url_display_list_ctrl_panel,
                              'media viewer urls')
    ...
    def _EditGUGDisplay(self):
        ...
        for data in self._gug_display_list_ctrl.GetData(only_selected=True):
            ...
            result, closed_by_user = ClientGUIDialogsQuick.GetYesNo(
                self,
                message,
                title='Show in the first list?',
    ...
    def _EditURLDisplay(self):
        ...
        for data in self._url_display_list_ctrl.GetData(only_selected=True):
            ...
            url_class_name = self._url_class_keys_to_url_classes[
                url_class_key].GetName()
            ...
            result, closed_by_user = ClientGUIDialogsQuick.GetYesNo(
                self,
                message,
                title='Show in the media viewer?',
                check_for_cancelled=True)
    ...
    def GetValue(self):
        ...
        gug_keys_to_display = {
            gug_key
            for (gug_key, display) in self._gug_display_list_ctrl.GetData()
            if display
        }
        url_class_keys_to_display = {
            url_class_key
            for (url_class_key,
                 display) in self._url_display_list_ctrl.GetData() if display
        }
...
class EditDuplicateActionOptionsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self,
        ...
        tag_services_listctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            tag_services_panel)

        columns = [('service name', 24), ('action', 36), ('tags merged', -1)]

        self._tag_service_actions = ClientGUIListCtrl.BetterListCtrl(
            tag_services_listctrl_panel,
            'duplicate_action_options_tag_services',
            5,
            36,
            columns,
            self._ConvertTagDataToListCtrlTuple,
            delete_key_callback=self._DeleteTag,
            activation_callback=self._EditTag)

        tag_services_listctrl_panel.SetListCtrl(self._tag_service_actions)

        tag_services_listctrl_panel.AddButton('add', self._AddTag)
        tag_services_listctrl_panel.AddButton('edit',
                                              self._EditTag,
                                              enabled_only_on_selection=True)
        tag_services_listctrl_panel.AddButton('delete',
                                              self._DeleteTag,
                                              enabled_only_on_selection=True)

        #

        rating_services_panel = ClientGUICommon.StaticBox(
            self, 'rating services')

        rating_services_listctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            rating_services_panel)

        columns = [('service name', -1), ('action', 36)]

        self._rating_service_actions = ClientGUIListCtrl.BetterListCtrl(
            rating_services_listctrl_panel,
            'duplicate_action_options_rating_services',
            5,
            24,
            columns,
            self._ConvertRatingDataToListCtrlTuple,
            delete_key_callback=self._DeleteRating,
            activation_callback=self._EditRating)

        rating_services_listctrl_panel.SetListCtrl(
        ...
        if self._duplicate_action == HC.DUPLICATE_BETTER:  # because there is only one valid action otherwise
            ...
            rating_services_listctrl_panel.AddButton(
                'edit', self._EditRating, enabled_only_on_selection=True)

        rating_services_listctrl_panel.AddButton(
            'delete', self._DeleteRating, enabled_only_on_selection=True)
        ...
        if self._duplicate_action == HC.DUPLICATE_BETTER:
            ...
            self._sync_urls_action.addItem(
                HC.content_merge_string_lookup[HC.CONTENT_MERGE_ACTION_COPY],
                HC.CONTENT_MERGE_ACTION_COPY)

        self._sync_urls_action.addItem(
            HC.content_merge_string_lookup[
                HC.CONTENT_MERGE_ACTION_TWO_WAY_MERGE],
            HC.CONTENT_MERGE_ACTION_TWO_WAY_MERGE)

        #

        (tag_service_options, rating_service_options, sync_archive,
         sync_urls_action) = duplicate_action_options.ToTuple()

        services_manager = HG.client_controller.services_manager

        self._service_keys_to_tag_options = {
            service_key: (action, tag_filter)
            for (service_key, action, tag_filter) in tag_service_options
        }

        self._tag_service_actions.SetData(
            list(self._service_keys_to_tag_options.keys()))

        self._tag_service_actions.Sort()

        self._service_keys_to_rating_options = {
            service_key: action
            for (service_key, action) in rating_service_options
        }

        self._rating_service_actions.SetData(
            list(self._service_keys_to_rating_options.keys()))
        ...
        if self._duplicate_action in (
                HC.DUPLICATE_ALTERNATE,
                HC.DUPLICATE_FALSE_POSITIVE) and not for_custom_action:
        ...
        tag_services_panel.Add(tag_services_listctrl_panel,
                               CC.FLAGS_EXPAND_BOTH_WAYS)

        #

        rating_services_panel.Add(rating_services_listctrl_panel,
                                  CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        rows.append(('if one file is archived, archive the other as well: ',
                     self._sync_archive))
        ...
        if len(choice_tuples) == 0:
            ...
            try:
                ...
                service_key = ClientGUIDialogsQuick.SelectFromList(
                    self, 'select service', choice_tuples)
            ...
            if self._duplicate_action == HC.DUPLICATE_BETTER:
                ...
                choice_tuples = [(HC.content_merge_string_lookup[action],
                                  action) for action in possible_actions]

                try:

                    action = ClientGUIDialogsQuick.SelectFromList(
                        self, 'select action', choice_tuples)
    ...
    def _AddTag(self):
        ...
        if len(choice_tuples) == 0:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                    self, 'edit which tags will be merged') as dlg_3:

                namespaces = HG.client_controller.network_engine.domain_manager.GetParserNamespaces(
                )

                panel = ClientGUITags.EditTagFilterPanel(dlg_3,
                                                         tag_filter,
                ...
                if dlg_3.exec() == QW.QDialog.Accepted:
                    ...
                    self._service_keys_to_tag_options[service_key] = (
                        action, tag_filter)
    ...
    def _ConvertRatingDataToListCtrlTuple(self, service_key):
        ...
        service_name = HG.client_controller.services_manager.GetName(
            service_key)
    ...
    def _DeleteRating(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            for service_key in self._rating_service_actions.GetData(
                    only_selected=True):
    ...
    def _DeleteTag(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            for service_key in self._tag_service_actions.GetData(
                    only_selected=True):
    ...
    def _EditRating(self):
        ...
        for service_key in service_keys:
            ...
            if self._duplicate_action == HC.DUPLICATE_BETTER:
                ...
                possible_actions = [
                    HC.CONTENT_MERGE_ACTION_COPY, HC.CONTENT_MERGE_ACTION_MOVE,
                    HC.CONTENT_MERGE_ACTION_TWO_WAY_MERGE
                ]
    ...
    def _EditTag(self):
        ...
        for service_key in service_keys:
            ...
            (action,
             tag_filter) = self._service_keys_to_tag_options[service_key]
    ...
    def GetValue(self):
        ...
        tag_service_actions = [
            ...
            for (service_key,
                 (action,
                  tag_filter)) in self._service_keys_to_tag_options.items()
        ]
        rating_service_actions = [
            (service_key, action)
            for (service_key,
                 action) in self._service_keys_to_rating_options.items()
        ]
        ...
        duplicate_action_options = ClientDuplicates.DuplicateActionOptions(
            tag_service_actions, rating_service_actions, sync_archive,
            sync_urls_action)
...
class EditFileImportOptions(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, file_import_options, show_downloader_options):
        ...
        self._do_not_check_known_urls_before_importing = QW.QCheckBox(
            pre_import_panel)
        self._do_not_check_hashes_before_importing = QW.QCheckBox(
            pre_import_panel)
        ...
        self._min_size = ClientGUIControls.NoneableBytesControl(
            pre_import_panel)
        self._min_size.SetValue(5 * 1024)

        self._max_size = ClientGUIControls.NoneableBytesControl(
            pre_import_panel)
        self._max_size.SetValue(100 * 1024 * 1024)

        self._max_gif_size = ClientGUIControls.NoneableBytesControl(
            pre_import_panel)
        self._max_gif_size.SetValue(32 * 1024 * 1024)

        self._min_resolution = ClientGUICommon.NoneableSpinCtrl(
            pre_import_panel, num_dimensions=2)
        self._min_resolution.SetValue((50, 50))

        self._max_resolution = ClientGUICommon.NoneableSpinCtrl(
            pre_import_panel, num_dimensions=2)
        self._max_resolution.SetValue((8192, 8192))

        #

        post_import_panel = ClientGUICommon.StaticBox(self,
                                                      'post-import actions')
        ...
        presentation_panel = ClientGUICommon.StaticBox(self,
        ...
        self._present_already_in_archive_files = QW.QCheckBox(
            presentation_panel)

        #

        (exclude_deleted, do_not_check_known_urls_before_importing,
         do_not_check_hashes_before_importing, allow_decompression_bombs,
         min_size, max_size, max_gif_size, min_resolution,
         max_resolution) = file_import_options.GetPreImportOptions()

        self._exclude_deleted.setChecked(exclude_deleted)
        self._do_not_check_known_urls_before_importing.setChecked(
            do_not_check_known_urls_before_importing)
        self._do_not_check_hashes_before_importing.setChecked(
        ...
        (automatic_archive,
         associate_source_urls) = file_import_options.GetPostImportOptions()
        ...
        (present_new_files, present_already_in_inbox_files,
         present_already_in_archive_files
         ) = file_import_options.GetPresentationOptions()

        self._present_new_files.setChecked(present_new_files)
        self._present_already_in_inbox_files.setChecked(
            present_already_in_inbox_files)
        self._present_already_in_archive_files.setChecked(
            present_already_in_archive_files)
        ...
        rows.append(
            ('exclude previously deleted files: ', self._exclude_deleted))

        if show_downloader_options and HG.client_controller.new_options.GetBoolean(
                'advanced_mode'):

            rows.append(('do not skip downloading because of known urls: ',
                         self._do_not_check_known_urls_before_importing))
            rows.append(('do not skip downloading because of hashes: ',
                         self._do_not_check_hashes_before_importing))
        ...
        rows.append(
            ('allow decompression bombs: ', self._allow_decompression_bombs))
        ...
        if show_downloader_options and HG.client_controller.new_options.GetBoolean(
            ...
            rows.append(('associate (and trust) additional source urls: ',
        ...
        rows.append(('present \'already in db\' files in inbox',
                     self._present_already_in_inbox_files))
        rows.append(('present \'already in db\' files in archive',
                     self._present_already_in_archive_files))
...
If you have a very large (10k+ files) file import page, consider hiding some or all of its thumbs to reduce ui lag and increase overall import speed.'''
    ...
    def GetValue(self):
        ...
        file_import_options.SetPreImportOptions(
            exclude_deleted, do_not_check_known_urls_before_importing,
            do_not_check_hashes_before_importing, allow_decompression_bombs,
            min_size, max_size, max_gif_size, min_resolution, max_resolution)
        file_import_options.SetPostImportOptions(automatic_archive,
                                                 associate_source_urls)
        file_import_options.SetPresentationOptions(
            present_new_files, present_already_in_inbox_files,
            present_already_in_archive_files)
...
class EditFrameLocationPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, info):
        ...
        self._last_size = ClientGUICommon.NoneableSpinCtrl(
            self,
            'last size',
            none_phrase='none set',
            min=100,
            max=1000000,
            unit=None,
            num_dimensions=2)
        self._last_position = ClientGUICommon.NoneableSpinCtrl(
            self,
            'last position',
            none_phrase='none set',
            min=-1000000,
        ...
        self._default_gravity_x.addItem(
            'by default, expand to width of parent', 1)
        self._default_gravity_x.addItem(
            'by default, expand width as much as needed', -1)

        self._default_gravity_y = ClientGUICommon.BetterChoice(self)

        self._default_gravity_y.addItem(
            'by default, expand to height of parent', 1)
        self._default_gravity_y.addItem(
            'by default, expand height as much as needed', -1)

        self._default_position = ClientGUICommon.BetterChoice(self)

        self._default_position.addItem(
            'by default, position off the top-left corner of parent',
            'topleft')
        self._default_position.addItem(
            'by default, position centered on the parent', 'center')
        ...
        (name, remember_size, remember_position, last_size, last_position,
         default_gravity, default_position, maximised,
         fullscreen) = self._original_info
        ...
        QP.AddToLayout(vbox, ClientGUICommon.BetterStaticText(self, text),
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._remember_size,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._remember_position,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._last_size, CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._last_position,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._default_gravity_x,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._default_gravity_y,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._default_position,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def GetValue(self):
        ...
        return (name, remember_size, remember_position, last_size,
...
class EditGUGPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, gug):
        ...
        min_width = ClientGUIFunctions.ConvertTextToPixelWidth(
            self._url_template, 74)
        ...
        (url_template, replacement_phrase, search_terms_separator,
         example_search_text) = gug.GetURLTemplateVariables()
        ...
        rows.append(('initial search text (to prompt user): ',
                     self._initial_search_text))
        ...
        self._search_terms_separator.textChanged.connect(
            self._UpdateExampleURL)
    ...
    def _GetValue(self):
        ...
        gug = ClientNetworkingDomain.GalleryURLGenerator(
            ...
            search_terms_separator=search_terms_separator,
            initial_search_text=initial_search_text,
            example_search_text=example_search_text)
    ...
    def _UpdateExampleURL(self):
        ...
        try:
            ...
            example_url = HG.client_controller.network_engine.domain_manager.NormaliseURL(
                example_url)

            self._example_url.setText(example_url)

        except (HydrusExceptions.GUGException,
                HydrusExceptions.URLClassException) as e:
        ...
        if example_url is None:
            ...
            if url_class is None:
                ...
                url_class_text = 'Matched ' + url_class.GetName(
                ) + ' url class.'
...
class EditNGUGPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, ngug, available_gugs):
        ...
        self._gug_list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._gug_list_ctrl_panel,
            'ngug_gugs',
            30,
            74,
            columns,
            self._ConvertGUGDataToListCtrlTuples,
        ...
        self._add_button = ClientGUICommon.BetterButton(
            self._gug_list_ctrl_panel, 'add', self._AddGUGButtonClick)
        ...
        QP.AddToLayout(vbox, gridbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, self._gug_list_ctrl_panel,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
    ...
    def _AddGUGButtonClick(self):
        ...
        existing_gug_keys = {
            gug_key
            for (gug_key, gug_name) in self._gug_list_ctrl.GetData()
        }
        existing_gug_names = {
            gug_name
            for (gug_key, gug_name) in self._gug_list_ctrl.GetData()
        ...
        choice_tuples = [(gug.GetName(), gug, False)
                         for gug in self._available_gugs
                         if gug.GetName() not in existing_gug_names
                         and gug.GetGUGKey() not in existing_gug_keys]

        if len(choice_tuples) == 0:

            QW.QMessageBox.critical(self, 'Error',
                                    'No remaining gugs available!')
    ...
    def _ConvertGUGDataToListCtrlTuples(self, gug_key_and_name):
        ...
        available = gug_key in (
            gug.GetGUGKey() for gug in self._available_gugs) or gug_name in (
                gug.GetName() for gug in self._available_gugs)
    ...
    def GetValue(self):
        ...
        ngug = ClientNetworkingDomain.NestedGalleryURLGenerator(
            name,
            gug_key=gug_key,
            initial_search_text=initial_search_text,
            gug_keys_and_names=gug_keys_and_names)
...
class EditGUGsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, gugs):
        ...
        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR, 'downloader_gugs.html'))

        menu_items.append(
            ('normal', 'open the gugs help',
             'Open the help page for gugs in your web browser.', page_func))

        help_button = ClientGUICommon.MenuBitmapButton(self,
                                                       CC.GlobalPixmaps.help,
                                                       menu_items)
        ...
        self._gug_list_ctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self._notebook)

        columns = [('name', 24), ('example url', -1),
                   ('gallery url class?', 20)]
        ...
        self._gug_list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            ...
            74,
            columns,
            self._ConvertGUGToListCtrlTuples,
            delete_key_callback=self._DeleteGUG,
            activation_callback=self._EditGUG)

        self._gug_list_ctrl_panel.SetListCtrl(self._gug_list_ctrl)

        self._gug_list_ctrl_panel.AddButton('add', self._AddNewGUG)
        self._gug_list_ctrl_panel.AddButton('edit',
                                            self._EditGUG,
                                            enabled_only_on_selection=True)
        self._gug_list_ctrl_panel.AddDeleteButton()
        self._gug_list_ctrl_panel.AddSeparator()
        self._gug_list_ctrl_panel.AddImportExportButtons(
            (ClientNetworkingDomain.GalleryURLGenerator, ), self._AddGUG)
        self._gug_list_ctrl_panel.AddSeparator()
        self._gug_list_ctrl_panel.AddDefaultsButton(
            ClientDefaults.GetDefaultSingleGUGs, self._AddGUG)

        #

        self._ngug_list_ctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self._notebook)

        columns = [('name', 24), ('gugs', -1), ('missing gugs', 14)]

        self._ngug_list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._ngug_list_ctrl_panel,
            'ngugs',
            20,
            64,
            columns,
            self._ConvertNGUGToListCtrlTuples,
            use_simple_delete=True,
            activation_callback=self._EditNGUG)

        self._ngug_list_ctrl_panel.SetListCtrl(self._ngug_list_ctrl)

        self._ngug_list_ctrl_panel.AddButton('add', self._AddNewNGUG)
        self._ngug_list_ctrl_panel.AddButton('edit',
                                             self._EditNGUG,
                                             enabled_only_on_selection=True)
        self._ngug_list_ctrl_panel.AddDeleteButton()
        self._ngug_list_ctrl_panel.AddSeparator()
        self._ngug_list_ctrl_panel.AddImportExportButtons(
            (ClientNetworkingDomain.NestedGalleryURLGenerator, ),
            self._AddNGUG)
        self._ngug_list_ctrl_panel.AddSeparator()
        self._ngug_list_ctrl_panel.AddDefaultsButton(
            ClientDefaults.GetDefaultNGUGs, self._AddNGUG)
        ...
        ngugs = [
            gug for gug in gugs if isinstance(
                gug, ClientNetworkingDomain.NestedGalleryURLGenerator)
        ]
        ...
        self._notebook.addTab(self._gug_list_ctrl_panel,
                              'gallery url generators')
        self._notebook.setCurrentWidget(self._gug_list_ctrl_panel)
        self._notebook.addTab(self._ngug_list_ctrl_panel,
                              'nested gallery url generators')
    ...
    def _AddNewGUG(self):
        ...
        gug = ClientNetworkingDomain.GalleryURLGenerator(
            'new gallery url generator')

        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit gallery url generator') as dlg:
    ...
    def _AddNewNGUG(self):
        ...
        ngug = ClientNetworkingDomain.NestedGalleryURLGenerator(
            'new nested gallery url generator')

        available_gugs = self._gug_list_ctrl.GetData()

        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit nested gallery url generator') as dlg:
    ...
    def _ConvertNGUGToListCtrlTuples(self, ngug):
        ...
        existing_names = {
            gug.GetName()
            for gug in self._gug_list_ctrl.GetData()
        }
        ...
        if result == QW.QDialog.Accepted:
            ...
            for deletee in deletees:
                ...
                if len(affected_ngug_names) > 0:
                    ...
                    message = 'The GUG "' + deletee.GetName(
                    ) + '" is in the NGUGs:'
        ...
        for gug in self._gug_list_ctrl.GetData(only_selected=True):
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HydrusSerialisable.SetNonDupeName(gug,
        ...
        for ngug in self._ngug_list_ctrl.GetData(only_selected=True):
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HydrusSerialisable.SetNonDupeName(ngug,
                                                      self._GetExistingNames())
...
class EditMediaViewOptionsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, info):
        ...
        (self._mime, media_show_action, media_start_paused,
         media_start_with_embed, preview_show_action, preview_start_paused,
         preview_start_with_embed, (media_scale_up, media_scale_down,
                                    preview_scale_up, preview_scale_down,
                                    exact_zooms_only, scale_up_quality,
                                    scale_down_quality)) = self._original_info

        (possible_show_actions, can_start_paused,
         can_start_with_embed) = CC.media_viewer_capabilities[self._mime]
        ...
        for action in possible_show_actions:
            ...
            self._media_show_action.addItem(
                CC.media_viewer_action_string_lookup[action], action)

            if action != CC.MEDIA_VIEWER_ACTION_DO_NOT_SHOW_ON_ACTIVATION_OPEN_EXTERNALLY:

                self._preview_show_action.addItem(
                    CC.media_viewer_action_string_lookup[action], action)

        self._media_show_action.currentIndexChanged.connect(
            self.EventActionChange)
        self._preview_show_action.currentIndexChanged.connect(
            self.EventActionChange)
        ...
        for scale_action in (CC.MEDIA_VIEWER_SCALE_100,
                             CC.MEDIA_VIEWER_SCALE_MAX_REGULAR,
                             CC.MEDIA_VIEWER_SCALE_TO_CANVAS):
        ...
        self._exact_zooms_only = QW.QCheckBox(
            'only permit half and double zooms', self)
        self._exact_zooms_only.setToolTip(
            'This limits zooms to 25%, 50%, 100%, 200%, 400%, and so on. It makes for fast resize and is useful for files that often have flat colours and hard edges, which often scale badly otherwise. The \'canvas fit\' zoom will still be inserted.'
        )

        self._scale_up_quality = ClientGUICommon.BetterChoice(self)

        for zoom in (CC.ZOOM_NEAREST, CC.ZOOM_LINEAR, CC.ZOOM_CUBIC,
                     CC.ZOOM_LANCZOS4):
        ...
        text = 'Setting media view options for ' + HC.mime_string_lookup[
            self._mime] + '.'
        ...
        rows.append(('media starts covered with an embed button: ',
                     self._media_start_with_embed))
        rows.append(
            ('preview viewer show action: ', self._preview_show_action))
        rows.append(('preview starts paused: ', self._preview_start_paused))
        rows.append(('preview starts covered with an embed button: ',
                     self._preview_start_with_embed))
        ...
        if len(
                set(possible_show_actions).intersection({
            ...
            rows.append(
                ('if the media is smaller than the media viewer canvas: ',
                 self._media_scale_up))
            rows.append(
                ('if the media is larger than the media viewer canvas: ',
                 self._media_scale_down))
            rows.append(('if the media is smaller than the preview canvas: ',
                         self._preview_scale_up))
            rows.append(('if the media is larger than the preview canvas: ',
                         self._preview_scale_down))

            gridbox = ClientGUICommon.WrapInGrid(self, rows)

            QP.AddToLayout(vbox, gridbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)

            QP.AddToLayout(vbox, self._exact_zooms_only,
                           CC.FLAGS_EXPAND_PERPENDICULAR)
            ...
            QP.AddToLayout(
                ...
                ClientGUICommon.BetterStaticText(
                    self,
                    'Nearest neighbour is fast and ugly, 8x8 lanczos and area resampling are slower but beautiful.'
                ), CC.FLAGS_VCENTER)

            QP.AddToLayout(
                vbox,
                ClientGUICommon.WrapInText(self._scale_up_quality, self,
                                           '>100% (interpolation) quality:'),
                CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
            QP.AddToLayout(
                vbox,
                ClientGUICommon.WrapInText(self._scale_down_quality, self,
                                           '<100% (decimation) quality:'),
                CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
    ...
    def _UpdateControls(self):
        ...
        media_ok = self._media_show_action.GetValue(
        ) not in CC.unsupported_media_actions
        preview_ok = self._preview_show_action.GetValue(
        ) not in CC.unsupported_media_actions
    ...
    def GetValue(self):
        ...
        zoom_info = (media_scale_up, media_scale_down, preview_scale_up,
                     preview_scale_down, exact_zooms_only, scale_up_quality,
                     scale_down_quality)

        return (self._mime, media_show_action, media_start_paused,
                media_start_with_embed, preview_show_action,
                preview_start_paused, preview_start_with_embed, zoom_info)
...
class EditNetworkContextPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self,
        ...
        if limited_types is None:
            ...
            limited_types = (CC.NETWORK_CONTEXT_GLOBAL,
                             CC.NETWORK_CONTEXT_DOMAIN,
                             CC.NETWORK_CONTEXT_HYDRUS,
                             CC.NETWORK_CONTEXT_DOWNLOADER_PAGE,
        ...
        for ct in limited_types:
            ...
            self._context_type.addItem(
                CC.network_context_type_string_lookup[ct], ct)
        ...
        for service in HG.client_controller.services_manager.GetServices(
                HC.REPOSITORIES):

            self._context_data_services.addItem(service.GetName(),
                                                service.GetServiceKey())

        self._context_data_subscriptions = ClientGUICommon.BetterChoice(self)

        self._context_data_none = QW.QCheckBox(
            'No specific data--acts as default.', self)

        if not allow_default:

            self._context_data_none.setVisible(False)

        names = HG.client_controller.Read(
            'serialisable_names',
        ...
        if network_context.context_data is None:
            ...
            if context_type == CC.NETWORK_CONTEXT_DOMAIN:
                ...
                self._context_data_services.SetValue(
                    network_context.context_data)

            elif context_type == CC.NETWORK_CONTEXT_SUBSCRIPTION:

                self._context_data_subscriptions.SetValue(
                    network_context.context_data)
        ...
        QP.AddToLayout(vbox, self._context_type_info,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._context_data_text,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._context_data_services,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._context_data_subscriptions,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._context_data_none,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _Update(self):
        ...
        self._context_type_info.setText(
            CC.network_context_type_description_lookup[
                self._context_type.GetValue()])
        ...
        if context_type in (CC.NETWORK_CONTEXT_GLOBAL,
                            CC.NETWORK_CONTEXT_DOWNLOADER_PAGE,
                            CC.NETWORK_CONTEXT_WATCHER_PAGE):
    ...
    def GetValue(self):
        ...
        return ClientNetworkingContexts.NetworkContext(context_type,
                                                       context_data)
...
class EditNetworkContextCustomHeadersPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, network_contexts_to_custom_header_dicts):
        ...
        columns = [('context', 24), ('header', 30), ('approved?', 12),
                   ('reason', -1)]

        self._list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._list_ctrl_panel,
            'network_contexts_custom_headers',
            15,
            40,
            columns,
            self._ConvertDataToListCtrlTuples,
        ...
        for (network_context, custom_header_dict) in list(
                network_contexts_to_custom_header_dicts.items()):

            for (key, (value, approved,
                       reason)) in list(custom_header_dict.items()):
    ...
    def _Add(self):
        ...
        network_context = ClientNetworkingContexts.NetworkContext(
            CC.NETWORK_CONTEXT_DOMAIN, 'hostname.com')
        key = 'Authorization'
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, 'edit header') as dlg:
            ...
            panel = self._EditPanel(dlg, network_context, key, value, approved,
                                    reason)
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                (network_context, key, value, approved,
                 reason) = panel.GetValue()
    ...
    def _ConvertDataToListCtrlTuples(self, data):
        ...
        display_tuple = (pretty_network_context, pretty_key_value,
                         pretty_approved, pretty_reason)

        sort_tuple = (pretty_network_context, (key, value), pretty_approved,
                      reason)
    ...
    def _Edit(self):
        ...
        for data in self._list_ctrl.GetData(only_selected=True):
            ...
            with ClientGUITopLevelWindows.DialogEdit(self,
                ...
                panel = self._EditPanel(dlg, network_context, key, value,
                                        approved, reason)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    new_data = (network_context, (key, value), approved,
                                reason)
    ...
    def GetValue(self):
        ...
        for (network_context, (key, value), approved,
             reason) in self._list_ctrl.GetData():

            network_contexts_to_custom_header_dicts[network_context][key] = (
                value, approved, reason)

        return network_contexts_to_custom_header_dicts

    class _EditPanel(ClientGUIScrolledPanels.EditPanel):
        def __init__(self, parent, network_context, key, value, approved,
                     reason):

            ClientGUIScrolledPanels.EditPanel.__init__(self, parent)

            self._network_context = ClientGUICommon.NetworkContextButton(
                self,
                network_context,
                limited_types=(CC.NETWORK_CONTEXT_GLOBAL,
                               CC.NETWORK_CONTEXT_DOMAIN),
            ...
            for a in (ClientNetworkingDomain.VALID_APPROVED,
                      ClientNetworkingDomain.VALID_DENIED,
                      ClientNetworkingDomain.VALID_UNKNOWN):

                self._approved.addItem(
                    ClientNetworkingDomain.valid_str_lookup[a], a)
            ...
            QP.AddToLayout(vbox, self._network_context,
                           CC.FLAGS_EXPAND_PERPENDICULAR)
...
class EditNoneableIntegerPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self,
        ...
        self._value = ClientGUICommon.NoneableSpinCtrl(
            self,
            message=message,
...
class EditRegexFavourites(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, regex_favourites):
        ...
        self._regexes = ClientGUIListCtrl.BetterListCtrl(
            regex_listctrl_panel,
            'regex_favourites',
            8,
            48,
            columns,
        ...
        regex_listctrl_panel.AddButton('edit',
                                       self._Edit,
                                       enabled_only_on_selection=True)
        ...
        with ClientGUIDialogs.DialogTextEntry(self, 'Enter regex.') as dlg:
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                with ClientGUIDialogs.DialogTextEntry(
                        self, 'Enter description.') as dlg_2:
...
class EditServersideService(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, serverside_service):
        ...
        (self._service_key, self._service_type, name, port,
         self._dictionary) = duplicate_serverside_service.ToTuple()

        self._service_panel = self._ServicePanel(self, name, port,
                                                 self._dictionary)
        ...
        if self._service_type in HC.RESTRICTED_SERVICES:
            ...
            self._panels.append(
                self._ServiceRestrictedPanel(self, self._dictionary))

            if self._service_type == HC.FILE_REPOSITORY:

                self._panels.append(
                    self._ServiceFileRepositoryPanel(self, self._dictionary))

            if self._service_type == HC.SERVER_ADMIN:

                self._panels.append(
                    self._ServiceServerAdminPanel(self, self._dictionary))
    ...
    def GetValue(self):
        ...
        return HydrusNetwork.GenerateService(self._service_key,
                                             self._service_type, name, port,
                                             dictionary)
    ...
    class _ServicePanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, name, port, dictionary):
            ...
            self._upnp_port = ClientGUICommon.NoneableSpinCtrl(
                self,
                'external upnp port',
    ...
    class _ServiceRestrictedPanel(QW.QWidget):
        def __init__(self, parent, dictionary):
            ...
            QP.AddToLayout(vbox, self._bandwidth_rules,
                           CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
...
class EditSubscriptionPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, subscription):
        ...
        (name, gug_key_and_name, queries, checker_options, initial_file_limit,
         periodic_file_limit, paused, file_import_options, tag_import_options,
         self._no_work_until,
         self._no_work_until_reason) = subscription.ToTuple()

        self._query_panel = ClientGUICommon.StaticBox(self, 'site and queries')

        self._gug_key_and_name = ClientGUIImport.GUGKeyAndNameSelector(
            self._query_panel, gug_key_and_name)

        queries_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self._query_panel)

        columns = [('name/query', 20), ('paused', 8), ('status', 8),
                   ('last new file time', 20), ('last check time', 20),
                   ('next check time', 20), ('file velocity', 20),
                   ('recent delays', 20), ('items', 13)]

        self._queries = ClientGUIListCtrl.BetterListCtrl(
            queries_panel,
            'subscription_queries',
            10,
            20,
            columns,
            self._ConvertQueryToListCtrlTuples,
            use_simple_delete=True,
            activation_callback=self._EditQuery)

        queries_panel.SetListCtrl(self._queries)

        queries_panel.AddButton('add', self._AddQuery)
        queries_panel.AddButton('copy queries',
                                self._CopyQueries,
                                enabled_only_on_selection=True)
        queries_panel.AddButton('paste queries', self._PasteQueries)
        queries_panel.AddButton('edit',
                                self._EditQuery,
                                enabled_only_on_selection=True)
        queries_panel.AddDeleteButton()
        queries_panel.AddSeparator()
        queries_panel.AddButton('pause/play',
                                self._PausePlay,
                                enabled_only_on_selection=True)
        queries_panel.AddButton(
            'retry failed',
            self._RetryFailed,
            enabled_check_func=self._ListCtrlCanRetryFailed)
        queries_panel.AddButton(
            'retry ignored',
            self._RetryIgnored,
            enabled_check_func=self._ListCtrlCanRetryIgnored)
        queries_panel.AddButton('check now',
                                self._CheckNow,
                                enabled_check_func=self._ListCtrlCanCheckNow)
        queries_panel.AddButton('reset cache',
                                self._ResetCache,
                                enabled_check_func=self._ListCtrlCanResetCache)
        ...
        if HG.client_controller.new_options.GetBoolean('advanced_mode'):
            ...
            menu_items.append(('normal', 'show', 'Show quality info.',
                               self._ShowQualityInfo))
            menu_items.append(
                ('normal', 'copy csv data to clipboard',
                 'Copy quality info to clipboard.', self._CopyQualityInfo))

            queries_panel.AddMenuButton('quality info',
                                        menu_items,
                                        enabled_only_on_selection=True)

        self._checker_options = ClientGUIImport.CheckerOptionsButton(
            self._query_panel,
            checker_options,
            update_callable=self._CheckerOptionsUpdated)

        #

        self._file_limits_panel = ClientGUICommon.StaticBox(
            self, 'file limits')
...
If you want to get all of an artist's files from a site, use the manual gallery download page first. A good routine is to check that you have the right search text and it all works correctly and that you know what tags you want, and then once that big queue is fully downloaded synced, start a new sub with the same settings to continue checking for anything posted in future.'''
        ...
        help_button = ClientGUICommon.BetterBitmapButton(
            self._file_limits_panel, CC.GlobalPixmaps.help,
            QW.QMessageBox.information, None, 'Information', message)

        help_hbox_1 = ClientGUICommon.WrapInText(help_button,
                                                 self._file_limits_panel,
                                                 'help about file limits -->',
                                                 QG.QColor(0, 0, 255))
...
But if 2 is--and is also perhaps accompanied by many 'could not parse' errors--the maintainer for the site's download parser (hydrus dev or whoever), would be interested in knowing what has happened so they can roll out a fix.'.'''
        ...
        help_hbox_2 = ClientGUICommon.WrapInText(
            help_button, self._file_limits_panel,
            'help about hitting the normal file limit -->',
            QG.QColor(0, 0, 255))
        ...
        self._initial_file_limit = QP.MakeQSpinBox(self._file_limits_panel,
                                                   min=1,
                                                   max=limits_max)
        self._initial_file_limit.setToolTip(
            'The first sync will add no more than this many URLs.')

        self._periodic_file_limit = QP.MakeQSpinBox(self._file_limits_panel,
                                                    min=1,
                                                    max=limits_max)
        self._periodic_file_limit.setToolTip(
            'Normal syncs will add no more than this many URLs, stopping early if they find several URLs the query has seen before.'
        )

        self._file_presentation_panel = ClientGUICommon.StaticBox(
            self, 'presentation')

        self._show_a_popup_while_working = QW.QCheckBox(
            self._file_presentation_panel)
        self._show_a_popup_while_working.setToolTip(
            'Careful with this! Leave it on to begin with, just in case it goes wrong!'
        )

        self._publish_files_to_popup_button = QW.QCheckBox(
            self._file_presentation_panel)
        self._publish_files_to_page = QW.QCheckBox(
            self._file_presentation_panel)
        self._publish_label_override = ClientGUICommon.NoneableTextCtrl(
            self._file_presentation_panel,
            none_phrase='no, use subscription name')
        self._merge_query_publish_events = QW.QCheckBox(
            self._file_presentation_panel)
        ...
        self._file_import_options = ClientGUIImport.FileImportOptionsButton(
            self, file_import_options, show_downloader_options)
        self._tag_import_options = ClientGUIImport.TagImportOptionsButton(
            self,
            tag_import_options,
            show_downloader_options,
        ...
        (show_a_popup_while_working, publish_files_to_popup_button,
         publish_files_to_page, publish_label_override,
         merge_query_publish_events) = subscription.GetPresentationOptions()

        self._show_a_popup_while_working.setChecked(show_a_popup_while_working)
        self._publish_files_to_popup_button.setChecked(
        ...
        self._query_panel.Add(self._gug_key_and_name,
                              CC.FLAGS_EXPAND_PERPENDICULAR)
        self._query_panel.Add(queries_panel, CC.FLAGS_EXPAND_BOTH_WAYS)
        self._query_panel.Add(self._checker_options,
                              CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        rows.append(('on first check, get at most this many files: ',
                     self._initial_file_limit))
        rows.append(('on normal checks, get at most this many newer files: ',
        ...
        self._file_limits_panel.Add(gridbox,
                                    CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        ...
        rows.append(
            ('show a popup while working: ', self._show_a_popup_while_working))
        rows.append(('publish new files to a popup button: ',
                     self._publish_files_to_popup_button))
        rows.append(
            ('publish new files to a page: ', self._publish_files_to_page))
        rows.append(
            ('publish to a specific label: ', self._publish_label_override))
        rows.append(('publish all queries to the same page/popup button: ',
                     self._merge_query_publish_events))

        gridbox = ClientGUICommon.WrapInGrid(self._file_presentation_panel,
                                             rows)

        self._file_presentation_panel.Add(gridbox,
                                          CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        ...
        QP.AddToLayout(vbox, self._control_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._file_limits_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._file_presentation_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._file_import_options,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._tag_import_options,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _AddQuery(self):
        ...
        initial_search_text = HG.client_controller.network_engine.domain_manager.GetInitialSearchText(
            gug_key_and_name)

        query = ClientImportSubscriptions.SubscriptionQuery(
            initial_search_text)

        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit subscription query') as dlg:
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                if query_text in self._GetCurrentQueryTexts():
                    ...
                    QW.QMessageBox.warning(
                        self, 'Warning', 'You already have a query for "' +
                        query_text + '", so nothing new has been added.')
    ...
    def _ConvertQueryToListCtrlTuples(self, query):
        ...
        (query_text, check_now, last_check_time, next_check_time, paused,
         status) = query.ToTuple()
        ...
        if last_new_file_time is None or last_new_file_time == 0:
            ...
            pretty_last_new_file_time = HydrusData.TimestampToPrettyTimeDelta(
                last_new_file_time)
        ...
        if last_check_time is None or last_check_time == 0:
            ...
            pretty_last_check_time = HydrusData.TimestampToPrettyTimeDelta(
                last_check_time)
        ...
        file_velocity = checker_options.GetRawCurrentVelocity(
            query.GetFileSeedCache(), last_check_time)
        pretty_file_velocity = checker_options.GetPrettyCurrentVelocity(
            query.GetFileSeedCache(), last_check_time, no_prefix=True)

        estimate = query.GetBandwidthWaitingEstimate(
            self._original_subscription.GetName())
        ...
        if estimate == 0:
            ...
            pretty_delay = 'bandwidth: ' + HydrusData.TimeDeltaToPrettyTimeDelta(
                estimate)
            delay = estimate

        (file_status, simple_status,
         (num_done, num_total)) = file_seed_cache.GetStatus()

        items = (num_total, num_done)

        pretty_items = simple_status

        sort_last_new_file_time = ClientGUIListCtrl.SafeNoneInt(
            last_new_file_time)
        ...
        display_tuple = (pretty_name, pretty_paused, pretty_status,
                         pretty_last_new_file_time, pretty_last_check_time,
                         pretty_next_check_time, pretty_file_velocity,
                         pretty_delay, pretty_items)
        sort_tuple = (name, paused, status, sort_last_new_file_time,
                      sort_last_check_time, sort_next_check_time,
                      file_velocity, delay, items)
        ...
        for old_query in selected_queries:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    if edited_query_text != old_query.GetQueryText(
                    ) and edited_query_text in self._GetCurrentQueryTexts():

                        QW.QMessageBox.warning(
                            self, 'Warning', 'You already have a query for "' +
                            edited_query_text +
                            '"! The edit you just made will not be saved.')
    ...
    def _CopyQualityInfo(self):
        ...
        for (name, num_inbox, num_archived, num_deleted) in data:
            ...
            if num_archived + num_deleted > 0:
                ...
                percent = HydrusData.ConvertFloatToPercentage(
                    num_archived / (num_archived + num_deleted))
            ...
            data_string = '{},{},{},{},{}'.format(
                name, HydrusData.ToHumanInt(num_inbox),
                HydrusData.ToHumanInt(num_archived),
                HydrusData.ToHumanInt(num_deleted), percent)
    ...
    def _GetQualityInfo(self):
        ...
        for query in self._queries.GetData(only_selected=True):
            ...
            data.append(
                (query.GetHumanName(), num_inbox, num_archived, num_deleted))
    ...
    def _ShowQualityInfo(self):
        ...
        for (name, num_inbox, num_archived, num_deleted) in data:
            ...
            data_string = '{}: inbox {} | archive {} | deleted {}'.format(
                name, HydrusData.ToHumanInt(num_inbox),
                HydrusData.ToHumanInt(num_archived),
                HydrusData.ToHumanInt(num_deleted))

            if num_archived + num_deleted > 0:

                data_string += ' | good {}'.format(
                    HydrusData.ConvertFloatToPercentage(
                        num_archived / (num_archived + num_deleted)))
    ...
    def _PasteQueries(self):
        ...
        try:
            ...
            already_existing_query_texts = list(
                current_query_texts.intersection(query_texts))
            new_query_texts = list(
                set(query_texts).difference(current_query_texts))
            ...
            if len(already_existing_query_texts) > 0:
                ...
                if len(already_existing_query_texts) > 50:
                    ...
                    message = '{} queries were already in the subscription, so they need not be added.'.format(
                        HydrusData.ToHumanInt(
                            len(already_existing_query_texts)))
                    ...
                    message = 'The queries:'
                    message += os.linesep * 2
                    message += aeqt_separator.join(
                        already_existing_query_texts)
                    message += os.linesep * 2
                ...
                if len(new_query_texts) > 0:
                    ...
                    if len(new_query_texts) > 50:
                        ...
                        message = '{} queries were new and will be added.'.format(
                            HydrusData.ToHumanInt(len(new_query_texts)))
            ...
            queries = [
                ClientImportSubscriptions.SubscriptionQuery(query_text)
    ...
    def _UpdateDelayText(self):
        ...
        if HydrusData.TimeHasPassed(self._no_work_until):
            ...
            status = 'delayed--retrying ' + HydrusData.TimestampToPrettyTimeDelta(
                self._no_work_until, just_now_threshold=0
            ) + ' because: ' + self._no_work_until_reason
    ...
    def GetValue(self):
        ...
        subscription.SetTuple(gug_key_and_name, queries, checker_options,
                              initial_file_limit, periodic_file_limit, paused,
                              file_import_options, tag_import_options,
                              self._no_work_until)
        ...
        subscription.SetPresentationOptions(show_a_popup_while_working,
                                            publish_files_to_popup_button,
...
class EditSubscriptionQueryPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, query):
        ...
        st_width = ClientGUIFunctions.ConvertTextToPixelWidth(
            self._status_st, 50)

        self._status_st.setMinimumWidth(st_width)

        self._display_name = ClientGUICommon.NoneableTextCtrl(
            self, none_phrase='show query text')
        self._query_text = QW.QLineEdit(self)
        self._check_now = QW.QCheckBox(self)
        self._paused = QW.QCheckBox(self)

        self._file_seed_cache_control = ClientGUIFileSeedCache.FileSeedCacheStatusControl(
            self, HG.client_controller)

        self._gallery_seed_log_control = ClientGUIGallerySeedLog.GallerySeedLogStatusControl(
            self, HG.client_controller, True, True)
        ...
        self._tag_import_options = ClientGUIImport.TagImportOptionsButton(
            self, tag_import_options, show_downloader_options)

        #

        (query_text, check_now, self._last_check_time, self._next_check_time,
         paused, self._status) = self._original_query.ToTuple()
        ...
        self._file_seed_cache = self._original_query.GetFileSeedCache(
        ).Duplicate()

        self._file_seed_cache_control.SetFileSeedCache(self._file_seed_cache)

        self._gallery_seed_log = self._original_query.GetGallerySeedLog(
        ).Duplicate()

        self._gallery_seed_log_control.SetGallerySeedLog(
            self._gallery_seed_log)
        ...
        QP.AddToLayout(vbox, self._file_seed_cache_control,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._gallery_seed_log_control,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, gridbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, self._tag_import_options,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _GetValue(self):
        ...
        query.SetQueryAndSeeds(self._query_text.text(), self._file_seed_cache,
                               self._gallery_seed_log)
    ...
    def _UpdateStatus(self):
        ...
        self._status_st.setText('next check: ' +
                                query.GetNextCheckStatusString())
...
class EditSubscriptionsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, subscriptions, subs_are_globally_paused=False):
        ...
        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR, 'getting_started_subscriptions.html'))

        menu_items.append(
            ('normal', 'open the html subscriptions help',
             'Open the help page for subscriptions in your web browser.',
             page_func))
        ...
        columns = [('name', -1), ('source', 20), ('query status', 25),
                   ('last new file time', 20), ('last checked', 20),
                   ('recent error/delay?', 20), ('items', 13), ('paused', 8)]

        self._subscriptions = ClientGUIListCtrl.BetterListCtrl(
            subscriptions_panel,
            'subscriptions',
            12,
            20,
            columns,
            self._ConvertSubscriptionToListCtrlTuples,
            use_simple_delete=True,
            activation_callback=self.Edit)

        subscriptions_panel.SetListCtrl(self._subscriptions)

        subscriptions_panel.AddButton('add', self.Add)
        subscriptions_panel.AddButton('edit',
                                      self.Edit,
                                      enabled_only_on_selection=True)
        ...
        subscriptions_panel.AddImportExportButtons(
            (ClientImportSubscriptions.Subscription, ), self._AddSubscription)

        subscriptions_panel.NewButtonRow()

        subscriptions_panel.AddButton('merge',
                                      self.Merge,
                                      enabled_check_func=self._CanMerge)
        subscriptions_panel.AddButton('separate',
                                      self.Separate,
                                      enabled_check_func=self._CanSeparate)

        subscriptions_panel.AddSeparator()

        subscriptions_panel.AddButton('pause/resume',
                                      self.PauseResume,
                                      enabled_only_on_selection=True)
        subscriptions_panel.AddButton(
            'retry failures',
            self.RetryFailures,
            enabled_check_func=self._CanRetryFailures)
        subscriptions_panel.AddButton('retry ignored',
                                      self.RetryIgnored,
                                      enabled_check_func=self._CanRetryIgnored)
        subscriptions_panel.AddButton('scrub delays',
                                      self.ScrubDelays,
                                      enabled_check_func=self._CanScrubDelays)
        subscriptions_panel.AddButton('check queries now',
                                      self.CheckNow,
                                      enabled_check_func=self._CanCheckNow)

        subscriptions_panel.AddButton('reset',
                                      self.Reset,
                                      enabled_check_func=self._CanReset)

        subscriptions_panel.NewButtonRow()

        subscriptions_panel.AddButton('select subscriptions',
                                      self.SelectSubscriptions)
        subscriptions_panel.AddButton('overwrite checker timings',
                                      self.SetCheckerOptions,
                                      enabled_only_on_selection=True)
        subscriptions_panel.AddButton('overwrite tag import options',
                                      self.SetTagImportOptions,
                                      enabled_only_on_selection=True)
    ...
    def _CanCheckNow(self):
        ...
        return True in (subscription.CanCheckNow()
    ...
    def _CanMerge(self):
        ...
        mergeable_subscriptions = [
            subscription for subscription in subscriptions
            if len(subscription.GetQueries()) > 0
        ]

        unique_gug_names = {
            subscription.GetGUGKeyAndName()[1]
            for subscription in mergeable_subscriptions
        }
    ...
    def _CanReset(self):
        ...
        return True in (subscription.CanReset()
                        for subscription in subscriptions)
    ...
    def _CanRetryFailures(self):
        ...
        return True in (subscription.CanRetryFailures()
                        for subscription in subscriptions)
    ...
    def _CanRetryIgnored(self):
        ...
        return True in (subscription.CanRetryIgnored()
                        for subscription in subscriptions)
    ...
    def _CanScrubDelays(self):
        ...
        return True in (subscription.CanScrubDelay()
                        for subscription in subscriptions)
    ...
    def _ConvertSubscriptionToListCtrlTuples(self, subscription):
        ...
        (name, gug_key_and_name, queries, checker_options, initial_file_limit,
         periodic_file_limit, paused, file_import_options, tag_import_options,
         no_work_until, no_work_until_reason) = subscription.ToTuple()
        ...
        if len(queries) > 0:
            ...
            last_new_file_time = max(
                (query.GetLatestAddedTime() for query in queries))
        ...
        if last_checked is None or last_checked == 0:
            ...
            pretty_last_checked = HydrusData.TimestampToPrettyTimeDelta(
                last_checked)
        ...
        if num_queries == 0:
            ...
            if num_paused > 0:
                ...
                status_components.append(
                    HydrusData.ToHumanInt(num_paused) + ' paused')

            if num_dead > 0:

                status_components.append(
                    HydrusData.ToHumanInt(num_dead) + ' dead')
        ...
        if HydrusData.TimeHasPassed(no_work_until):
            ...
            (min_estimate,
             max_estimate) = subscription.GetBandwidthWaitingEstimateMinMax()
            ...
            if max_estimate == 0:  # don't seem to be any delays of any kind
                ...
                pretty_delay = 'bandwidth: some ok, some up to ' + HydrusData.TimeDeltaToPrettyTimeDelta(
                    max_estimate)
                delay = max_estimate
                ...
                if min_estimate == max_estimate:  # probably just one query, and it is delayed
                    ...
                    pretty_delay = 'bandwidth: up to ' + HydrusData.TimeDeltaToPrettyTimeDelta(
                        max_estimate)
                    delay = max_estimate

                else:

                    pretty_delay = 'bandwidth: from ' + HydrusData.TimeDeltaToPrettyTimeDelta(
                        min_estimate
                    ) + ' to ' + HydrusData.TimeDeltaToPrettyTimeDelta(
                        max_estimate)
                    delay = max_estimate

        else:

            pretty_delay = 'delayed--retrying ' + HydrusData.TimestampToPrettyTimeDelta(
                no_work_until,
                just_now_threshold=0) + ' - because: ' + no_work_until_reason
            delay = HydrusData.GetTimeDeltaUntilTime(no_work_until)

        file_seed_caches = [query.GetFileSeedCache() for query in queries]

        (queries_status, queries_simple_status,
         (num_done,
          num_total)) = ClientImportFileSeeds.GenerateFileSeedCachesStatus(
              file_seed_caches)
        ...
        display_tuple = (name, pretty_site, pretty_status,
                         pretty_last_new_file_time, pretty_last_checked,
                         pretty_delay, pretty_items, pretty_paused)
        sort_tuple = (name, pretty_site, status, sort_last_new_file_time,
                      sort_last_checked, delay, items, paused)
    ...
    def _ImportObject(self, obj):
        ...
        if isinstance(obj, HydrusSerialisable.SerialisableList):
            ...
            if isinstance(obj, ClientImportSubscriptions.Subscription):
                ...
                QW.QMessageBox.warning(
                    self, 'Warning',
                    'That was not a subscription--it was a: ' +
                    type(obj).__name__)
    ...
    def Add(self):
        ...
        empty_subscription = ClientImportSubscriptions.Subscription(
            'new subscription', gug_key_and_name=gug_key_and_name)

        frame_key = 'edit_subscription_dialog'

        with ClientGUITopLevelWindows.DialogEdit(self, 'edit subscription',
                                                 frame_key) as dlg_edit:
        ...
        for subscription in subs_to_edit:
            ...
            with ClientGUITopLevelWindows.DialogEdit(self, 'edit subscription',
                ...
                if result == QW.QDialog.Accepted:
                    ...
                    edited_subscription.SetNonDupeName(
                        self._GetExistingNames())
    ...
    def Merge(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            while len(potential_mergees) > 0:
                ...
                (mergeables_with_our_primary, not_mergeable_with_our_primary
                 ) = potential_primary.GetMergeable(potential_mergees)
            ...
            for mergeable_group in mergeable_groups:
                ...
                choice_tuples = [(sub.GetName(), sub)
                                 for sub in mergeable_group]

                try:

                    primary_sub = ClientGUIDialogsQuick.SelectFromList(
                        self,
                ...
                message = primary_sub_name + ' was able to merge ' + HydrusData.ToHumanInt(
                    len(mergeable_group)
                ) + ' other subscriptions. If you wish to change its name, do so here.'

                with ClientGUIDialogs.DialogTextEntry(
                        self, message, default=primary_sub_name) as dlg:
    ...
    def Separate(self):
        ...
        if len(subscriptions) != 1:
            ...
            QW.QMessageBox.critical(
                self, 'Error',
                'Separate only works if one subscription is selected!')
        ...
        if num_queries <= 1:
            ...
            QW.QMessageBox.critical(
                self, 'Error',
                'Separate only works if the selected subscription has more than one query!'
            )
        ...
        if num_queries > 100:
            ...
            yes_tuples = [('break it in half', 'half'),
                          ('break it all into single-query subscriptions',
                           'whole'),
                          ('only extract some of the subscription', 'part')]

            with ClientGUIDialogs.DialogYesYesNo(self,
                                                 message,
                                                 yes_tuples=yes_tuples,
        ...
        if action == 'part':
            ...
            choice_tuples = [(query.GetHumanName(), query, False)
                             for query in queries]

            with ClientGUITopLevelWindows.DialogEdit(
                    self, 'select the queries to extract') as dlg:
            ...
            if len(queries_to_extract
                   ) == num_queries:  # the madman selected them all

                action = 'whole'

            elif len(queries_to_extract) > 1:

                yes_tuples = [('one new merged subscription', True),
                              ('many subscriptions with only one query', False)
                              ]

                message = 'Do you want the extracted queries to be a new merged subscription, or many subscriptions with only one query?'

                with ClientGUIDialogs.DialogYesYesNo(
                        self, message, yes_tuples=yes_tuples,
                        no_label='forget it') as dlg:
        ...
        if action != 'half':
            ...
            with ClientGUIDialogs.DialogTextEntry(
                    self, message, default=subscription.GetName()) as dlg:
        ...
        if action == 'whole':
            ...
            extracted_subscriptions = list(
                subscription.Separate(name, queries_to_extract))
    ...
    def SetCheckerOptions(self):
        ...
        checker_options = ClientDefaults.GetDefaultCheckerOptions(
            'artist subscription')

        with ClientGUITopLevelWindows.DialogEdit(self,
                                                 'edit check timings') as dlg:
    ...
    def SetTagImportOptions(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
            ...
            panel = EditTagImportOptionsPanel(dlg,
                                              tag_import_options,
                                              show_downloader_options,
                                              allow_default_selection=True)
...
class EditTagDisplayManagerPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, tag_display_manager):
        ...
        services = list(
            HG.client_controller.services_manager.GetServices(
                (HC.COMBINED_TAG, HC.TAG_REPOSITORY, HC.LOCAL_TAG)))
        ...
        for service in services:
            ...
            page = self._Panel(self._tag_services, tag_display_manager,
                               service_key)
    ...
    def GetValue(self):
        ...
        for page in self._tag_services.GetPages():
            ...
            for (tag_display_type,
                 tag_filter) in tag_display_types_to_tag_filters.items():

                tag_display_manager.SetTagFilter(tag_display_type, service_key,
                                                 tag_filter)
    ...
    class _Panel(QW.QWidget):
        def __init__(self, parent, tag_display_manager, service_key):
            ...
            single_tag_filter = tag_display_manager.GetTagFilter(
                ClientTags.TAG_DISPLAY_SINGLE_MEDIA, service_key)
            selection_tag_filter = tag_display_manager.GetTagFilter(
                ClientTags.TAG_DISPLAY_SELECTION_LIST, service_key)
            ...
            self._single_tag_filter_button = ClientGUITags.TagFilterButton(
                self, message, single_tag_filter, label_prefix='tags shown: ')
            ...
            self._selection_tag_filter_button = ClientGUITags.TagFilterButton(
                self,
                message,
                selection_tag_filter,
                label_prefix='tags shown: ')

            #

            rows = []

            rows.append(('Tag filter for single file views: ',
                         self._single_tag_filter_button))
            rows.append(('Tag filter for multiple file views: ',
                         self._selection_tag_filter_button))
        ...
        def GetValue(self):
            ...
            tag_display_types_to_tag_filters[
                ClientTags.
                TAG_DISPLAY_SINGLE_MEDIA] = self._single_tag_filter_button.GetValue(
                )
            tag_display_types_to_tag_filters[
                ClientTags.
                TAG_DISPLAY_SELECTION_LIST] = self._selection_tag_filter_button.GetValue(
                )
...
class EditTagImportOptionsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self,
        ...
        self._load_default_options = ClientGUICommon.BetterButton(
            default_panel, 'load one of the default options',
        ...
        downloader_options_panel = ClientGUICommon.StaticBox(
            self._specific_options_panel, 'fetch options')

        self._fetch_tags_even_if_url_recognised_and_file_already_in_db = QW.QCheckBox(
            downloader_options_panel)
        self._fetch_tags_even_if_hash_recognised_and_file_already_in_db = QW.QCheckBox(
            downloader_options_panel)
        ...
        self._tag_filter_button = ClientGUITags.TagFilterButton(
            downloader_options_panel,
            message,
            tag_blacklist,
            is_blacklist=True)
        ...
        self._fetch_tags_even_if_url_recognised_and_file_already_in_db.setChecked(
            tag_import_options.ShouldFetchTagsEvenIfURLKnownAndFileAlreadyInDB(
            ))
        self._fetch_tags_even_if_hash_recognised_and_file_already_in_db.setChecked(
            tag_import_options.
            ShouldFetchTagsEvenIfHashKnownAndFileAlreadyInDB())
        ...
        rows.append((
            'rely on the appropriate default tag import options at the time of import: ',
            self._is_default))

        gridbox = ClientGUICommon.WrapInGrid(default_panel, rows)

        default_panel.Add(gridbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        default_panel.Add(self._load_default_options,
                          CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        rows.append(
            ('fetch tags even if url recognised and file already in db: ',
             self._fetch_tags_even_if_url_recognised_and_file_already_in_db))
        rows.append(
            ('fetch tags even if hash recognised and file already in db: ',
             self._fetch_tags_even_if_hash_recognised_and_file_already_in_db))
        rows.append(('set file blacklist: ', self._tag_filter_button))

        gridbox = ClientGUICommon.WrapInGrid(downloader_options_panel, rows)

        downloader_options_panel.Add(gridbox,
                                     CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        ...
        QP.AddToLayout(vbox, downloader_options_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._services_vbox,
        ...
        QP.AddToLayout(vbox, self._specific_options_panel,
                       CC.FLAGS_EXPAND_SIZER_BOTH_WAYS)
    ...
    def _InitialiseServices(self, tag_import_options):
        ...
        services = HG.client_controller.services_manager.GetServices(
            HC.TAG_SERVICES, randomised=False)

        for service in services:

            service_key = service.GetServiceKey()

            service_tag_import_options = tag_import_options.GetServiceTagImportOptions(
                service_key)

            panel = EditServiceTagImportOptionsPanel(
                self._specific_options_panel,
                service_key,
                service_tag_import_options,
                show_downloader_options=self._show_downloader_options)

            self._service_keys_to_service_tag_import_options_panels[
                service_key] = panel

            QP.AddToLayout(self._services_vbox, panel,
                           CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _LoadDefaultOptions(self):
        ...
        (file_post_default_tag_import_options,
         ...
         ) = domain_manager.GetDefaultTagImportOptions()

        choice_tuples = []

        choice_tuples.append(
            ('file post default', file_post_default_tag_import_options))
        choice_tuples.append(
            ('watchable default', watchable_default_tag_import_options))
        ...
        if len(url_class_keys_to_default_tag_import_options) > 0:
            ...
            url_class_keys_to_url_classes = {
                url_class.GetMatchKey(): url_class
                for url_class in url_classes
            }

            url_class_names_and_default_tag_import_options = [
                (url_class_keys_to_url_classes[url_class_key].GetName(),
                 url_class_keys_to_default_tag_import_options[url_class_key])
                for url_class_key in list(
                    url_class_keys_to_default_tag_import_options.keys())
                if url_class_key in url_class_keys_to_url_classes
            ]
        ...
        try:
            ...
            default_tag_import_options = ClientGUIDialogsQuick.SelectFromList(
                self, 'Select which default', choice_tuples, sort_tuples=False)
    ...
    def _SetValue(self, tag_import_options):
        ...
        for (service_key, panel) in list(
                self._service_keys_to_service_tag_import_options_panels.items(
                )):
    ...
    def GetValue(self):
        ...
        if is_default:
            ...
            service_keys_to_service_tag_import_options = {
                service_key: panel.GetValue()
                for (service_key, panel) in
                list(self._service_keys_to_service_tag_import_options_panels.
                     items())
            }
            ...
            tag_import_options = ClientImportOptions.TagImportOptions(
                fetch_tags_even_if_url_recognised_and_file_already_in_db=
                fetch_tags_even_if_url_recognised_and_file_already_in_db,
                fetch_tags_even_if_hash_recognised_and_file_already_in_db=
                fetch_tags_even_if_hash_recognised_and_file_already_in_db,
...
class EditSelectFromListPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self,
        ...
        max_label_width_chars = max(
            (len(label) for (label, value) in choice_tuples))

        width_chars = min(64, max_label_width_chars + 2)
        height_chars = min(max(6, len(choice_tuples)), 36)

        (width_px, height_px) = ClientGUIFunctions.ConvertTextToPixels(
            self._list, (width_chars, height_chars))
...
class EditSelectFromListButtonsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, choices):
        ...
        for (text, data, tooltip) in choices:
            ...
            button = ClientGUICommon.BetterButton(self, text,
                                                  self._ButtonChoice, data)
...
class EditServiceTagImportOptionsPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self,
        ...
        (get_tags, get_tags_filter, self._additional_tags, self._to_new_files,
         self._to_already_in_inbox, self._to_already_in_archive,
         self._only_add_existing_tags, self._only_add_existing_tags_filter
        ...
        cog_button = ClientGUICommon.MenuBitmapButton(main_box,
                                                      CC.GlobalPixmaps.cog,
                                                      menu_items)

        #

        downloader_options_panel = ClientGUICommon.StaticBox(
            main_box, 'tag parsing')

        self._get_tags_checkbox = QW.QCheckBox('get tags',
                                               downloader_options_panel)
        ...
        self._get_tags_filter_button = ClientGUITags.TagFilterButton(
            downloader_options_panel,
            message,
            get_tags_filter,
            label_prefix='adding: ')
        ...
        QP.AddToLayout(hbox, self._get_tags_filter_button,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        self._additional_button = ClientGUICommon.BetterButton(
            main_box, 'additional tags', self._DoAdditionalTags)
    ...
    def _DoAdditionalTags(self):
        ...
        with ClientGUIDialogs.DialogInputTags(self,
                                              self._service_key,
                                              list(self._additional_tags),
                                              message=message) as dlg:
    ...
    def _EditOnlyAddExistingTagsFilter(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit already-exist filter') as dlg:
            ...
            panel = ClientGUITags.EditTagFilterPanel(
                dlg,
    ...
    def _GetCogIconMenuItems(self):
        ...
        check_manager = ClientGUICommon.CheckboxManagerBoolean(
            self, '_to_new_files')

        menu_items.append(('check', 'apply tags to new files',
                           'Apply tags to new files.', check_manager))

        check_manager = ClientGUICommon.CheckboxManagerBoolean(
            self, '_to_already_in_inbox')

        menu_items.append((
            'check', 'apply tags to files already in inbox',
            'Apply tags to files that are already in the db and in the inbox.',
            check_manager))
        ...
        menu_items.append(
            ('check', 'apply tags to files already in archive',
             'Apply tags to files that are already in the db and archived.',
             check_manager))
        ...
        menu_items.append(
            ('check', 'only add tags that already exist',
             'Only add tags to this service if they have non-zero count.',
             check_manager))

        menu_items.append(
            ('normal', 'set a filter for already-exist test',
             'Tell the already-exist test to only work on a subset of tags.',
             self._EditOnlyAddExistingTagsFilter))

        return menu_items

    def _UpdateAdditionalTagsButtonLabel(self):

        button_label = HydrusData.ToHumanInt(len(
            self._additional_tags)) + ' additional tags'
    ...
    def GetValue(self):
        ...
        service_tag_import_options = ClientImportOptions.ServiceTagImportOptions(
            get_tags=get_tags,
            get_tags_filter=get_tags_filter,
            additional_tags=self._additional_tags,
            to_new_files=self._to_new_files,
            to_already_in_inbox=self._to_already_in_inbox,
            to_already_in_archive=self._to_already_in_archive,
            only_add_existing_tags=self._only_add_existing_tags,
            only_add_existing_tags_filter=self._only_add_existing_tags_filter)
...
class EditTagSummaryGeneratorPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, tag_summary_generator):
        ...
        self._background_colour = ClientGUICommon.AlphaColourControl(
            edit_panel)
        self._text_colour = ClientGUICommon.AlphaColourControl(edit_panel)

        self._namespaces_listbox = ClientGUIListBoxes.QueueListBox(
            edit_panel, 8, self._ConvertNamespaceToListBoxString,
            self._AddNamespaceInfo, self._EditNamespaceInfo)
        ...
        (background_colour, text_colour, namespace_info, separator,
         example_tags, show) = tag_summary_generator.ToTuple()
        ...
        edit_panel.Add(
            ClientGUICommon.BetterStaticText(
                edit_panel,
                'The colours only work for the thumbnails right now!'),
            CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        edit_panel.Add(
            ClientGUICommon.WrapInText(self._separator, edit_panel,
                                       'separator'),
            CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        example_panel.Add(
            ClientGUICommon.BetterStaticText(
                example_panel,
                'Enter some newline-separated tags here to see what your current object would generate.'
            ), CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _UpdateTest(self):
        ...
        self._test_result.setText(
            tag_summary_generator.GenerateExampleSummary())
    ...
    def GetValue(self):
        ...
        example_tags = HydrusTags.CleanTags(
            HydrusText.DeserialiseNewlinedTexts(
                self._example_tags.toPlainText()))

        return ClientGUITags.TagSummaryGenerator(background_colour,
                                                 text_colour, namespace_info,
                                                 separator, example_tags, show)
...
class TagSummaryGeneratorButton(ClientGUICommon.BetterButton):
    ...
    def _Edit(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self,
                                                 'edit tag summary') as dlg:

            panel = EditTagSummaryGeneratorPanel(dlg,
                                                 self._tag_summary_generator)
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                self.setText(
                    self._tag_summary_generator.GenerateExampleSummary())
...
class EditURLClassPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, url_class: ClientNetworkingDomain.URLClass):
        ...
        for url_type in (HC.URL_TYPE_POST, HC.URL_TYPE_GALLERY,
                         HC.URL_TYPE_WATCHABLE, HC.URL_TYPE_FILE):

            self._url_type.addItem(HC.url_type_string_lookup[url_type],
                                   url_type)
        ...
        self._path_components = ClientGUIListBoxes.QueueListBox(
            path_components_panel, 6, self._ConvertPathComponentRowToString,
            self._AddPathComponent, self._EditPathComponent)

        #

        parameters_panel = ClientGUICommon.StaticBox(self, 'parameters')

        parameters_listctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            parameters_panel)

        columns = [('key', 14), ('value', -1)]

        self._parameters = ClientGUIListCtrl.BetterListCtrl(
            parameters_listctrl_panel,
            'url_class_path_components',
            5,
            45,
            columns,
            self._ConvertParameterToListCtrlTuples,
            delete_key_callback=self._DeleteParameters,
            activation_callback=self._EditParameters)

        parameters_listctrl_panel.SetListCtrl(self._parameters)

        parameters_listctrl_panel.AddButton('add', self._AddParameters)
        parameters_listctrl_panel.AddButton('edit',
        ...
        self._next_gallery_page_panel = ClientGUICommon.StaticBox(
            self, 'next gallery page')

        self._next_gallery_page_choice = ClientGUICommon.BetterChoice(
            self._next_gallery_page_panel)

        self._next_gallery_page_delta = QP.MakeQSpinBox(
            self._next_gallery_page_panel, min=1, max=65536)
        ...
        (url_type, preferred_scheme, netloc, path_components, parameters,
         api_lookup_converter, send_referral_url, referral_url_converter,
         example_url) = url_class.ToTuple()
        ...
        for send_referral_url_type in ClientNetworkingDomain.SEND_REFERRAL_URL_TYPES:
            ...
            self._send_referral_url.addItem(
                ClientNetworkingDomain.
                send_referral_url_string_lookup[send_referral_url_type],
                send_referral_url_type)

        tt = 'Do not change this unless you know you need to. It fixes complicated problems.'

        self._send_referral_url.setToolTip(tt)

        self._referral_url_converter = ClientGUIControls.StringConverterButton(
            self, referral_url_converter)
        ...
        self._api_lookup_converter = ClientGUIControls.StringConverterButton(
        ...
        (match_subdomains, keep_matched_subdomains, alphabetise_get_parameters,
         can_produce_multiple_files,
         should_be_associated_with_files) = url_class.GetURLBooleans()
        ...
        self._should_be_associated_with_files.setChecked(
        ...
        example_url_width = ClientGUIFunctions.ConvertTextToPixelWidth(
            self._example_url, 75)

        self._example_url.setMinimumWidth(example_url_width)

        self._send_referral_url.SetValue(send_referral_url)

        (gallery_index_type, gallery_index_identifier,
         gallery_index_delta) = url_class.GetGalleryIndexValues()

        # this preps it for the upcoming update
        self._next_gallery_page_choice.addItem(
            'initialisation', (gallery_index_type, gallery_index_identifier))
        ...
        path_components_panel.Add(self._path_components,
                                  CC.FLAGS_EXPAND_BOTH_WAYS)

        #

        parameters_panel.Add(parameters_listctrl_panel,
                             CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(hbox, self._next_gallery_page_choice,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(hbox, self._next_gallery_page_delta, CC.FLAGS_VCENTER)

        self._next_gallery_page_panel.Add(hbox,
                                          CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        ...
        rows.append(('alphabetise GET parameters?: ',
                     self._alphabetise_get_parameters))
        rows.append(('match subdomains?: ', self._match_subdomains))
        rows.append(
            ('keep matched subdomains?: ', self._keep_matched_subdomains))
        rows.append(
            ('can produce multiple files: ', self._can_produce_multiple_files))
        rows.append(('should associate a \'known url\' with resulting files: ',
        ...
        rows.append(('optional referral url converter: ',
                     self._referral_url_converter))
        rows.append(('referral url: ', self._referral_url))
        rows.append(
            ('optional api url converter: ', self._api_lookup_converter))
        ...
        QP.AddToLayout(vbox, self._next_gallery_page_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._example_url_classes,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        self._preferred_scheme.currentIndexChanged.connect(
            self._UpdateControls)
        ...
        self._next_gallery_page_choice.currentIndexChanged.connect(
            self._UpdateControls)
        self._next_gallery_page_delta.valueChanged.connect(
            self._UpdateControls)
        ...
        self._send_referral_url.currentIndexChanged.connect(
            self._UpdateControls)
        self._referral_url_converter.stringConverterUpdate.connect(
            self._UpdateControls)
        self._api_lookup_converter.stringConverterUpdate.connect(
            self._UpdateControls)

        self._should_be_associated_with_files.clicked.connect(
            self.EventAssociationUpdate)
    ...
    def _AddParameters(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, 'edit value') as dlg:
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                with ClientGUIDialogs.DialogTextEntry(
                        self,
                        'Enter optional \'default\' value for this parameter, which will be filled in if missing. Leave blank for none (recommended).',
                        allow_blank=True) as dlg_default:
                    ...
                    if dlg_default.exec() == QW.QDialog.Accepted:
                        ...
                        if default == '':
                            ...
                            QW.QMessageBox.warning(
                                self, 'Warning',
                                'That default does not match the given rule! Clearing it to none!'
                            )
    ...
    def _EditParameters(self):
        ...
        for parameter in selected_params:
            ...
            (original_key, (original_string_match,
                            original_default)) = parameter
            ...
            with ClientGUITopLevelWindows.DialogEdit(self,
                ...
                panel = ClientGUIControls.EditStringMatchPanel(
                    dlg, original_string_match)
    ...
    def _EditPathComponent(self, row):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self,
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                with ClientGUIDialogs.DialogTextEntry(
                        self,
                        'Enter optional \'default\' value for this path component, which will be filled in if missing. Leave blank for none (recommended).',
                        default=default,
                        allow_blank=True) as dlg_default:
                    ...
                    if dlg_default.exec() == QW.QDialog.Accepted:
                        ...
                        QP.CallAfter(
                            self._UpdateControls
                        )  # seems sometimes this doesn't kick in naturally
    ...
    def _GetValue(self):
        ...
        (gallery_index_type,
         gallery_index_identifier) = self._next_gallery_page_choice.GetValue()
    ...
    def _UpdateControls(self):
        ...
        if self._url_type.GetValue() == HC.URL_TYPE_GALLERY:
            ...
            for (index,
                 (string_match,
                  default)) in enumerate(self._path_components.GetData()):

                if True in (string_match.Matches(n)
                            for n in ('0', '1', '10', '100', '42')):

                    choices.append((
                        HydrusData.ConvertIntToPrettyOrdinalString(index + 1) +
                        ' path component', (ClientNetworkingDomain.
                                            GALLERY_INDEX_TYPE_PATH_COMPONENT,
                                            index)))

            for (index, (key,
                         (string_match,
                          default))) in enumerate(self._parameters.GetData()):
                ...
                if True in (string_match.Matches(n)
                    ...
                    choices.append(
                        (key + ' parameter',
                         (ClientNetworkingDomain.GALLERY_INDEX_TYPE_PARAMETER,
                          key)))
            ...
            self._next_gallery_page_choice.SetValue(
                existing_choice)  # this should fail to ( None, None )

            (gallery_index_type, gallery_index_identifier
             ) = self._next_gallery_page_choice.GetValue(
        ...
        try:
            ...
            if url_class.UsesAPIURL():
                ...
                self._referral_url.setText(
                    'Not used, as API converter will redirect.')
                ...
                if send_referral_url in (
                        ClientNetworkingDomain.
                        SEND_REFERRAL_URL_ONLY_IF_PROVIDED,
                        ClientNetworkingDomain.SEND_REFERRAL_URL_NEVER):
                ...
                if send_referral_url == ClientNetworkingDomain.SEND_REFERRAL_URL_CONVERTER_IF_NONE_PROVIDED:
                    ...
                    referral_url = 'normal referral url -or- {}'.format(
                        referral_url)

                else:

                    referral_url = url_class.GetReferralURL(
                        normalised, 'normal referral url')
            ...
            try:
                ...
                if url_class.CanGenerateNextGalleryPage():
                    ...
                    next_gallery_page_url = url_class.GetNextGalleryPage(
                        normalised)
                ...
                self._next_gallery_page_url.setText('Could not convert - ' +
                                                    reason)
            ...
            self._example_url_classes.setText('Example does not match - ' +
                                              reason)
    ...
    def EventAssociationUpdate(self):
        ...
        if self._should_be_associated_with_files.isChecked():
            ...
            if self._url_type.GetValue() in (HC.URL_TYPE_GALLERY,
                                             HC.URL_TYPE_WATCHABLE):
            ...
            if self._url_type.GetValue() in (HC.URL_TYPE_FILE,
                                             HC.URL_TYPE_POST):
...
class EditURLClassesPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, url_classes):
        ...
        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR, 'downloader_url_classes.html'))

        menu_items.append(
            ('normal', 'open the url classes help',
             'Open the help page for url classes in your web browser.',
             page_func))
        ...
        self._url_class_checker.textChanged.connect(
            self.EventURLClassCheckerText)

        self._url_class_checker_st = ClientGUICommon.BetterStaticText(self)

        self._list_ctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(self)

        columns = [('name', 36), ('type', 20),
                   ('example (normalised) url', -1)]
        ...
        self._list_ctrl_panel.AddImportExportButtons(
            (ClientNetworkingDomain.URLClass, ), self._AddURLClass)
        self._list_ctrl_panel.AddSeparator()
        self._list_ctrl_panel.AddDefaultsButton(
            ClientDefaults.GetDefaultURLClasses, self._AddURLClass)
        ...
        QP.AddToLayout(url_hbox, self._url_class_checker,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(url_hbox, self._url_class_checker_st,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        for url_class in self._list_ctrl.GetData(only_selected=True):
            ...
            with ClientGUITopLevelWindows.DialogEdit(self,
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HydrusSerialisable.SetNonDupeName(url_class,
                                                      self._GetExistingNames())
...
class EditURLClassLinksPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, network_engine, url_classes, parsers,
                 url_class_keys_to_parser_keys):
        ...
        self._api_pairs_list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._notebook, 'url_class_api_pairs', 10, 36, columns,
            self._ConvertAPIPairDataToListCtrlTuples)

        #

        self._parser_list_ctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self._notebook)

        columns = [('url class', -1), ('url type', 20), ('parser', 36)]

        self._parser_list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._parser_list_ctrl_panel,
            'url_class_keys_to_parser_keys',
            24,
            36,
            columns,
            self._ConvertParserDataToListCtrlTuples,
            activation_callback=self._EditParser)

        self._parser_list_ctrl_panel.SetListCtrl(self._parser_list_ctrl)

        self._parser_list_ctrl_panel.AddButton('edit',
                                               self._EditParser,
                                               enabled_only_on_selection=True)
        self._parser_list_ctrl_panel.AddButton(
            'clear',
            self._ClearParser,
            enabled_check_func=self._LinksOnCurrentSelection)
        self._parser_list_ctrl_panel.AddButton(
            'try to fill in gaps based on example urls',
            self._TryToLinkURLClassesAndParsers,
            enabled_check_func=self._GapsExist)

        #

        api_pairs = ClientNetworkingDomain.ConvertURLClassesIntoAPIPairs(
            url_classes)
        ...
        for url_class in url_classes:
            ...
            if not url_class.IsParsable(
            ) or url_class in api_pair_unparsable_url_classes:
    ...
    def _ClearParser(self):
        ...
        result = ClientGUIDialogsQuick.GetYesNo(
            self, 'Clear all the selected linked parsers?')
    ...
    def _ConvertParserDataToListCtrlTuples(self, data):
        ...
        display_tuple = (pretty_url_class_name, pretty_url_type,
                         pretty_parser_name)
    ...
    def _EditParser(self):
        ...
        if len(self._parsers) == 0:
            ...
            QW.QMessageBox.information(
                self, 'Information',
                'Unfortunately, you do not have any parsers, so none can be linked to your url classes. Please create some!'
            )
        ...
        for data in self._parser_list_ctrl.GetData(only_selected=True):
            ...
            choice_tuples = [(parser.GetName(), parser)
                             for parser in self._parsers]

            try:

                parser = ClientGUIDialogsQuick.SelectFromList(
                    self, 'select parser for ' + url_class.GetName(),
                    choice_tuples)
    ...
    def _GapsExist(self):
        ...
        return None in (parser_key
                        for (url_class_key,
                             parser_key) in self._parser_list_ctrl.GetData())

    def _LinksOnCurrentSelection(self):

        non_none_parser_keys = [
            parser_key
            for (url_class_key, parser_key) in self._parser_list_ctrl.GetData(
                only_selected=True) if parser_key is not None
        ]
    ...
    def _TryToLinkURLClassesAndParsers(self):
        ...
        existing_url_class_keys_to_parser_keys = {
            url_class_key: parser_key
            for (url_class_key,
                 parser_key) in self._parser_list_ctrl.GetData()
            if parser_key is not None
        }

        new_url_class_keys_to_parser_keys = ClientNetworkingDomain.NetworkDomainManager.STATICLinkURLClassesAndParsers(
            self._url_classes, self._parsers,
            existing_url_class_keys_to_parser_keys)
        ...
        if len(new_url_class_keys_to_parser_keys) > 0:
            ...
            self._parser_list_ctrl.AddDatas(
                list(new_url_class_keys_to_parser_keys.items()))
