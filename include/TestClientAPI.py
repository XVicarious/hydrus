...
from . import (ClientLocalServer, ClientLocalServerResources, ClientManagers,
...
class TestClientAPI(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._client_api = ClientServices.GenerateService(
            CC.CLIENT_API_SERVICE_KEY, HC.CLIENT_API_SERVICE, 'client api')
        cls._client_api_cors = ClientServices.GenerateService(
            CC.CLIENT_API_SERVICE_KEY, HC.CLIENT_API_SERVICE, 'client api')
        ...
        def TWISTEDSetup():
            ...
            reactor.listenTCP(
                45869,
                ClientLocalServer.HydrusServiceClientAPI(
                    cls._client_api, allow_non_local_connections=False))
            reactor.listenTCP(
                45899,
                ClientLocalServer.HydrusServiceClientAPI(
                    cls._client_api_cors, allow_non_local_connections=False))
    ...
    def _test_client_api_basics(self, connection):
        ...
        def format_request_new_permissions_query(name, basic_permissions):
            ...
            return '/request_new_permissions?name={}&basic_permissions={}'.format(
                urllib.parse.quote(name),
                urllib.parse.quote(json.dumps(basic_permissions)))
        ...
        connection.request(
            'GET',
            format_request_new_permissions_query(
                'test', [ClientAPI.CLIENT_API_PERMISSION_ADD_FILES]))
        ...
        permissions_to_set_up.append(
            ('everything', list(ClientAPI.ALLOWED_PERMISSIONS)))
        permissions_to_set_up.append(
            ('add_files', [ClientAPI.CLIENT_API_PERMISSION_ADD_FILES]))
        permissions_to_set_up.append(
            ('add_tags', [ClientAPI.CLIENT_API_PERMISSION_ADD_TAGS]))
        permissions_to_set_up.append(
            ('add_urls', [ClientAPI.CLIENT_API_PERMISSION_ADD_URLS]))
        permissions_to_set_up.append(
            ('manage_pages', [ClientAPI.CLIENT_API_PERMISSION_MANAGE_PAGES]))
        permissions_to_set_up.append(
            ('manage_cookies',
        ...
        for (name, basic_permissions) in permissions_to_set_up:
            ...
            connection.request(
                'GET',
                format_request_new_permissions_query(name, basic_permissions))
            ...
            self.assertEqual(bytes.fromhex(access_key_hex),
        ...
        connection.request(
            'GET', '/verify_access_key?Hydrus-Client-API-Access-Key=abcd')
        ...
        def do_good_verify_test(api_permissions, key_hex, key_name):
            ...
            for request_type in ('header', 'get'):
                ...
                if request_type == 'header':
                    ...
                    connection.request(
                        'GET',
                        '/verify_access_key?{}={}'.format(key_name, key_hex))
                ...
                self.assertEqual(set(body_dict['basic_permissions']),
                                 set(api_permissions.GetBasicPermissions()))
                self.assertEqual(body_dict['human_description'],
                                 api_permissions.ToHumanString())

        for api_permissions in set_up_permissions.values():

            access_key_hex = api_permissions.GetAccessKey().hex()

            do_good_verify_test(api_permissions, access_key_hex,
        ...
        connection.request(
            'GET', '/verify_access_key?Hydrus-Client-API-Session-Key=abcd')
        ...
        for api_permissions in set_up_permissions.values():
            ...
            do_good_verify_test(api_permissions, session_key_hex,
        ...
        headers = {
            'Content-Type': HC.mime_mimetype_string_lookup[HC.APPLICATION_JSON]
        }
        ...
        body_dict = {
            'Hydrus-Client-API-Access-Key': 'abcd',
            'hash': hash_hex,
        ...
        body_dict = {
            'Hydrus-Client-API-Session-Key': 'abcd',
            'hash': hash_hex,
            'service_names_to_tags': {
                'my tags': ['test', 'test2']
            }
        ...
        body_dict = {
            'Hydrus-Client-API-Session-Key': session_key_hex,
            'hash': hash_hex,
            'service_names_to_tags': {
                'my tags': ['test', 'test2']
            }
    ...
    def _test_cors_succeeds(self, connection):
        ...
        self.assertEqual(response.getheader('Access-Control-Allow-Methods'),
                         'GET')
    ...
    def _test_add_files(self, connection, set_up_permissions):
        ...
        headers = {
            ...
            access_key_hex,
            'Content-Type':
            HC.mime_mimetype_string_lookup[HC.APPLICATION_OCTET_STREAM]
        }
        ...
        self.assertEqual(
            response_json['hash'],
            'a593942cb7ea9ffcd8ccf2f0fa23c338e23bfecd9a3e508dfc0bcf07501ead08')
    ...
    def _test_add_tags(self, connection, set_up_permissions):
        ...
        tags = [
            " bikini ", "blue    eyes", " character : samus aran ", ":)",
            "   ", "", "10", "11", "9", "system:wew", "-flower"
        ]

        json_tags = json.dumps(tags)

        path = '/add_tags/clean_tags?tags={}'.format(
            urllib.parse.quote(json_tags, safe=''))
        ...
        clean_tags = [
            "bikini", "blue eyes", "character:samus aran", "::)", "10", "11",
            "9", "wew", "flower"
        ]
        ...
        expected_service_keys_to_content_updates = collections.defaultdict(
            list)
        ...
        [((service_keys_to_content_updates, ), kwargs)
         ] = HG.test_controller.GetWrite('content_updates')

        self.assertEqual(len(service_keys_to_content_updates),
                         len(expected_service_keys_to_content_updates))

        for (service_key,
             content_updates) in service_keys_to_content_updates.items():

            expected_content_updates = expected_service_keys_to_content_updates[
                service_key]

            c_u_tuples = [c_u.ToTuple() for c_u in content_updates]
            e_c_u_tuples = [
                e_c_u.ToTuple() for e_c_u in expected_content_updates
            ]
        ...
        first_dict[HC.CONTENT_STATUS_CURRENT] = {('muh test',
                                                  'muh test parent')}
        ...
        HG.test_controller.tag_siblings_manager = ClientManagers.TagSiblingsManager(
            HG.test_controller)
        HG.test_controller.tag_parents_manager = ClientManagers.TagParentsManager(
            HG.test_controller)
        ...
        body_dict = {
            ...
            'add_siblings_and_parents': False
        }
    ...
    def _test_add_urls(self, connection, set_up_permissions):
        ...
        path = '/add_urls/get_url_files?url={}'.format(
        ...
        url_file_statuses = [(CC.STATUS_SUCCESSFUL_BUT_REDUNDANT, hash,
                              'muh import phrase')]
        json_url_file_statuses = [{
            'status': CC.STATUS_SUCCESSFUL_BUT_REDUNDANT,
            'hash': hash.hex(),
            'note': 'muh import phrase'
        }]
        ...
        path = '/add_urls/get_url_info?url={}'.format(
            urllib.parse.quote(url, safe=''))
        ...
        self.assertEqual(
            response_json['human_result_text'],
            '"https://8ch.net/tv/res/1846574.html" URL added successfully.')
        self.assertEqual(response_json['normalised_url'],
                         'https://8ch.net/tv/res/1846574.html')

        self.assertEqual(HG.test_controller.GetWrite('import_url_test'),
                         [((url, None, None, None, False), {})])
        ...
        self.assertEqual(
            HG.test_controller.GetWrite('import_url_test'),
            [((url, service_keys_to_tags, 'muh /tv/', None, True), {})])
    ...
    def _test_manage_cookies(self, connection, set_up_permissions):
        ...
        cookies.append(
            ['one', '1', '.somesite.com', '/',
             HydrusData.GetNow() + 86400])
        cookies.append(
            ['two', '2', 'somesite.com', '/',
             HydrusData.GetNow() + 86400])
        cookies.append([
            'three', '3', 'wew.somesite.com', '/',
            HydrusData.GetNow() + 86400
        ])
    ...
    def _test_search_files(self, connection, set_up_permissions):
        ...
        path = '/get_files/search_files?tags={}'.format(
        ...
        pretend_request.client_api_permissions = set_up_permissions[
            'everything']

        predicates = ClientLocalServerResources.ParseClientAPISearchPredicates(
            pretend_request)
        ...
        with self.assertRaises(
            ...
            ClientLocalServerResources.ParseClientAPISearchPredicates(
                pretend_request)
        ...
        file_ids_to_hashes = {
            1: bytes.fromhex('a' * 64),
            2: bytes.fromhex('b' * 64),
        ...
        for (file_id, hash) in file_ids_to_hashes.items():
            ...
            mime = random.choice(
                [HC.IMAGE_JPEG, HC.VIDEO_WEBM, HC.APPLICATION_PDF])
            ...
            service_keys_to_statuses_to_tags = {
                CC.DEFAULT_LOCAL_TAG_SERVICE_KEY: {
                    HC.CONTENT_STATUS_CURRENT: ['blue eyes', 'blonde hair'],
                    HC.CONTENT_STATUS_PENDING: ['bodysuit']
                }
            }

            tags_manager = ClientMedia.TagsManager(
                service_keys_to_statuses_to_tags)

            locations_manager = ClientMedia.LocationsManager(set(),
                                                             set(),
                                                             set(),
                                                             set(),
                                                             urls=urls)
            ratings_manager = ClientRatings.RatingsManager({})
            file_viewing_stats_manager = ClientMedia.FileViewingStatsManager(
                0, 0, 0, 0)
        ...
        for media_result in media_results:
            ...
            for (service_key,
                ...
                if service_key not in service_keys_to_names:
                    ...
                    service_keys_to_names[
                        service_key] = services_manager.GetName(service_key)
        ...
        path = '/get_files/file_metadata?file_ids={}&only_return_identifiers=true'.format(
            urllib.parse.quote(json.dumps([1, 2, 3, 7])))
        ...
        path = '/get_files/file_metadata?hashes={}&only_return_identifiers=true'.format(
            urllib.parse.quote(
                json.dumps(
                    [hash.hex() for hash in file_ids_to_hashes.values()])))
        ...
        path = '/get_files/file_metadata?file_ids={}&only_return_identifiers=true'.format(
            urllib.parse.quote(json.dumps([1, 2, 3])))
        ...
        path = '/get_files/file_metadata?file_ids={}'.format(
            urllib.parse.quote(json.dumps([1, 2, 3])))
        ...
        path = '/get_files/file_metadata?hashes={}'.format(
            urllib.parse.quote(
                json.dumps(
                    [hash.hex() for hash in file_ids_to_hashes.values()])))
        ...
        file_info_manager = ClientMedia.FileInfoManager(file_id,
                                                        hash,
                                                        size=size,
                                                        mime=mime,
                                                        width=width,
        ...
        locations_manager = ClientMedia.LocationsManager(
            set(), set(), set(), set())
        ratings_manager = ClientRatings.RatingsManager({})
        file_viewing_stats_manager = ClientMedia.FileViewingStatsManager(
            0, 0, 0, 0)

        media_result = ClientMedia.MediaResult(file_info_manager, tags_manager,
                                               locations_manager,
                                               ratings_manager,
                                               file_viewing_stats_manager)
        ...
        file_path = HG.test_controller.client_files_manager.GetFilePath(
            hash, HC.IMAGE_PNG, check_file_exists=False)
        ...
        thumb_path = HG.test_controller.client_files_manager._GenerateExpectedThumbnailPath(
            hash)
        ...
        file_info_manager = ClientMedia.FileInfoManager(123456,
                                                        hash_404,
                                                        size=size,
                                                        mime=mime,
                                                        width=width,
