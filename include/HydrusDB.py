...
def ReadLargeIdQueryInSeparateChunks(cursor, select_statement, chunk_size):
    ...
    cursor.execute(
        ...
        ' ( job_id INTEGER PRIMARY KEY AUTOINCREMENT, temp_id INTEGER );')

    cursor.execute('INSERT INTO ' + table_name + ' ( temp_id ) ' +
                   select_statement
                   )  # given statement should end in semicolon, so we are good
    ...
    while i < num_to_do:
        ...
        chunk = [
            temp_id for (temp_id, ) in cursor.execute(
                'SELECT temp_id FROM ' + table_name +
                ' WHERE job_id BETWEEN ? AND ?;', (i, i + chunk_size - 1))
        ]
...
def VacuumDB(db_path):
    ...
    fast_big_transaction_wal = not distutils.version.LooseVersion(
        sqlite3.sqlite_version) < distutils.version.LooseVersion('3.11.0')
...
class HydrusDB(object):
    ...
    def _AnalyzeTempTable(self, temp_table_name):
        ...
        self._c.execute(
            'ANALYZE mem.sqlite_master;'
        )  # this reloads the current stats into the query planner, may no longer be needed
    ...
    def _CreateIndex(self, table_name, columns, unique=False):
        ...
        on_phrase = ' ON ' + table_name_simple + ' (' + ', '.join(
            columns) + ');'
    ...
    def _InitDBCursor(self):
        ...
        for db_name in db_names:
            ...
            if HG.db_memory_journaling:
                ...
                self._c.execute(
                    'PRAGMA {}.journal_mode = MEMORY;'.format(db_name))

            elif HG.no_wal:

                self._c.execute(
                    'PRAGMA {}.journal_mode = TRUNCATE;'.format(db_name))

            else:

                self._c.execute(
                    'PRAGMA {}.journal_mode = WAL;'.format(db_name))
    ...
    def Read(self, action, *args, **kwargs):
        ...
        job = HydrusData.JobDatabase(job_type, synchronous, action, *args,
                                     **kwargs)
...
class TemporaryIntegerTable(object):
    ...
    def __enter__(self):
        ...
        self._cursor.execute('CREATE TABLE ' + self._table_name + ' ( ' +
                             self._column_name + ' INTEGER PRIMARY KEY );')

        self._cursor.executemany(
            'INSERT INTO ' + self._table_name + ' ( ' + self._column_name +
            ' ) VALUES ( ? );', ((i, ) for i in self._integer_iterable))
