...
class NetworkJob(object):
    ...
    def __init__(self,
        ...
        self._second_level_domain = ClientNetworkingDomain.ConvertURLIntoSecondLevelDomain(
            self._url)
    ...
    def _GenerateSpecificNetworkContexts(self):
        ...
        session_network_context = ClientNetworkingContexts.NetworkContext(
            CC.NETWORK_CONTEXT_DOMAIN, self._second_level_domain)
        login_network_context = ClientNetworkingContexts.NetworkContext(
            CC.NETWORK_CONTEXT_DOMAIN, self._domain)
    ...
    def _SendRequestAndGetResponse(self):
        ...
        with self._lock:
            ...
            if self.IS_HYDRUS_SERVICE:
                ...
                headers['User-Agent'] = 'hydrus client/' + str(
                    HC.NETWORK_VERSION)

            referral_url = self.engine.domain_manager.GetReferralURL(
                self._url, self._referral_url)

            if HG.network_report_mode:

                HydrusData.ShowText(
                    'Network Jobs Referral URLs for {}:{}Given: {}{}Used: {}'.
                    format(self._url, os.linesep, self._referral_url,
                           os.linesep, referral_url))
            ...
            if referral_url is not None:
                ...
                try:
                    ...
                    if HG.network_report_mode:
                        ...
                        HydrusData.ShowText(
                            'Network Jobs Quoted Referral URL for {}:{}{}'.
                            format(self._url, os.linesep, referral_url))
        ...
        connect_timeout = HG.client_controller.new_options.GetInteger(
            'network_timeout')
    ...
    def _ObeysBandwidth(self):
        ...
        if self._bandwidth_manual_override_delayed_timestamp is not None and HydrusData.TimeHasPassed(
    ...
    def _OngoingBandwidthOK(self):
        ...
        if now == self._last_time_ongoing_bandwidth_failed:  # it won't have changed, so no point spending any cpu checking
            ...
            result = self.engine.bandwidth_manager.CanContinueDownload(
    ...
    def _ReadResponse(self, response, stream_dest, max_allowed=None):
            ...
            if 'content-length' in response.headers:
                ...
                if max_allowed is not None and self._num_bytes_to_read > max_allowed:
                    ...
                    raise HydrusExceptions.NetworkException(
                        ...
                        HydrusData.ToHumanBytes(self._num_bytes_to_read) +
                        ' but the max network size for this type of job is ' +
                        HydrusData.ToHumanBytes(max_allowed) + '!')
                ...
                if self._file_import_options is not None:
                    ...
                    self._file_import_options.CheckNetworkDownload(
                        mime, self._num_bytes_to_read, is_complete_file_size)
        ...
        for chunk in response.iter_content(chunk_size=65536):
            ...
            with self._lock:
                ...
                if self._num_bytes_to_read is not None and num_bytes_read_is_accurate and self._num_bytes_read > self._num_bytes_to_read:
                    ...
                    raise HydrusExceptions.NetworkException(
                        'Too much data: Was expecting {} but server continued responding!'
                        .format(
                            HydrusData.ToHumanBytes(self._num_bytes_to_read)))
                ...
                if self._file_import_options is not None:
                    ...
                    self._file_import_options.CheckNetworkDownload(
                        mime, self._num_bytes_read, is_complete_file_size)
        ...
        if self._num_bytes_to_read is not None and num_bytes_read_is_accurate and self._num_bytes_read < self._num_bytes_to_read:
            ...
            raise HydrusExceptions.ShouldReattemptNetworkException(
                'Incomplete response: Was expecting {} but actually got {} !'.
                format(HydrusData.ToHumanBytes(self._num_bytes_to_read),
                       HydrusData.ToHumanBytes(self._num_bytes_read)))

    def _ReportDataUsed(self, num_bytes):

        self._bandwidth_tracker.ReportDataUsed(num_bytes)

        self.engine.bandwidth_manager.ReportDataUsed(self._network_contexts,
                                                     num_bytes)
    ...
    def _WaitOnConnectionError(self, status_text):
        ...
        connection_error_wait_time = HG.client_controller.new_options.GetInteger(
            'connection_error_wait_time')

        self._connection_error_wake_time = HydrusData.GetNow() + (
            (self._current_connection_attempt_number - 1) *
            connection_error_wait_time)

        while not HydrusData.TimeHasPassed(
                self._connection_error_wake_time) and not self._IsCancelled():

            with self._lock:

                self._status_text = status_text + ' - retrying in {}'.format(
                    HydrusData.TimestampToPrettyTimeDelta(
                        self._connection_error_wake_time))
    ...
    def _WaitOnServersideBandwidth(self, status_text):
        ...
        serverside_bandwidth_wait_time = HG.client_controller.new_options.GetInteger(
            'serverside_bandwidth_wait_time')

        self._serverside_bandwidth_wake_time = HydrusData.GetNow() + (
            (self._current_connection_attempt_number - 1) *
            serverside_bandwidth_wait_time)

        while not HydrusData.TimeHasPassed(self._serverside_bandwidth_wake_time
                                           ) and not self._IsCancelled():
        ...
        with self._lock:
            ...
            if self._ObeysBandwidth():
                ...
                if result:
                    ...
                    (
                        bandwidth_waiting_duration, bandwidth_network_context
                    ) = self.engine.bandwidth_manager.GetWaitingEstimateAndContext(
                        self._network_contexts)
                    ...
                    if override_coming_first:
                        ...
                        waiting_str = 'overriding bandwidth ' + HydrusData.TimestampToPrettyTimeDelta(
                            self._bandwidth_manual_override_delayed_timestamp,
                            just_now_string='imminently',
                        ...
                        waiting_str = 'bandwidth free ' + HydrusData.TimestampToPrettyTimeDelta(
    ...
    def CanValidateInPopup(self):
        ...
        with self._lock:
            ...
            return self.engine.domain_manager.CanValidateInPopup(
                self._network_contexts)
        ...
        with self._lock:
            ...
            if self._for_login:
                ...
                raise HydrusExceptions.ValidationException(
                    'Login jobs should not be asked if they can login!')

            else:

                return self.engine.login_manager.CheckCanLogin(
                    self._login_network_context)

    def CurrentlyWaitingOnConnectionError(self):

        with self._lock:

            return not HydrusData.TimeHasPassed(
                self._connection_error_wake_time)
    ...
    def GenerateLoginProcess(self):
        ...
        with self._lock:
            ...
            if self._for_login:
                ...
                raise Exception(
                    'Login jobs should not be asked to generate login processes!'
                )

            else:

                return self.engine.login_manager.GenerateLoginProcess(
                    self._login_network_context)
    ...
    def GenerateValidationPopupProcess(self):
        ...
        with self._lock:
            ...
            return self.engine.domain_manager.GenerateValidationPopupProcess(
                self._network_contexts)
    ...
    def GetContentText(self):
        ...
        (text, self._encoding) = HydrusText.NonFailingUnicodeDecode(
            data, self._encoding)
    ...
    def GetStatus(self):
        ...
        with self._lock:
            ...
            return (self._status_text,
                    ...
                    self._num_bytes_to_read)

    def GetTotalDataUsed(self):

        with self._lock:

            return self._bandwidth_tracker.GetUsage(HC.BANDWIDTH_TYPE_DATA,
                                                    None)
    ...
    def NeedsLogin(self):
        ...
        with self._lock:
            ...
            if self._for_login:
                ...
                return self.engine.login_manager.NeedsLogin(
                    self._login_network_context)
    ...
    def OverrideBandwidth(self, delay=None):
        ...
        with self._lock:
            ...
            if delay is None:
                ...
                self._bandwidth_manual_override_delayed_timestamp = HydrusData.GetNow(
                ) + delay

                self._wake_time = min(
                    self._wake_time,
                    self._bandwidth_manual_override_delayed_timestamp + 1)
        ...
        try:
                ...
                try:
                    ...
                    if response.ok:
                        ...
                        if response.encoding is not None:
                            ...
                            we_got_lame_iso_default_from_requests = encoding == 'ISO-8859-1' and (
                                self._content_type is None
                                or encoding not in self._content_type)
                        ...
                        with self._lock:
                            ...
                            if isinstance(e,
                                          HydrusExceptions.BandwidthException):
                    ...
                    if not self._CanReattemptRequest():
                        ...
                        raise HydrusExceptions.NetworkException(
                            'Server reported very limited bandwidth: ' +
                            str(e))

                    self._WaitOnServersideBandwidth(
                        'server reported limited bandwidth')
                    ...
                    if not self._CanReattemptRequest():
                        ...
                        raise HydrusExceptions.NetworkException(
                            'Ran out of reattempts on this error: ' + str(e))

                    self._WaitOnConnectionError(
                        'server suddenly stopped delivering data')
            ...
            with self._lock:
                ...
                if not isinstance(e, (HydrusExceptions.ConnectionException,
                                      HydrusExceptions.SizeException)):
    ...
    def TokensOK(self):
        ...
        if need_token:
            ...
            (consumed, next_timestamp
             ) = self.engine.bandwidth_manager.TryToConsumeAGalleryToken(
                 sld, gtn)
            ...
            with self._lock:
                ...
                if consumed:
                    ...
                    self._status_text = 'waiting for a ' + self._gallery_token_name + ' slot: next ' + HydrusData.TimestampToPrettyTimeDelta(
                        next_timestamp, just_now_threshold=1)
    ...
    def _GenerateNetworkContexts(self):
        ...
        network_contexts.append(
            ClientNetworkingContexts.NetworkContext(
                CC.NETWORK_CONTEXT_DOWNLOADER_PAGE, self._downloader_page_key))
...
class NetworkJobSubscription(NetworkJob):
    ...
    def _GenerateNetworkContexts(self):
        ...
        network_contexts.append(
            ClientNetworkingContexts.NetworkContext(
                CC.NETWORK_CONTEXT_SUBSCRIPTION, self._subscription_key))
...
class NetworkJobHydrus(NetworkJob):
    ...
    def _ReportDataUsed(self, num_bytes):
        ...
        service = self.engine.controller.services_manager.GetService(
            self._service_key)
...
class NetworkJobWatcherPage(NetworkJob):
    ...
    def _GenerateNetworkContexts(self):
        ...
        network_contexts.append(
            ClientNetworkingContexts.NetworkContext(
                CC.NETWORK_CONTEXT_WATCHER_PAGE, self._watcher_key))
