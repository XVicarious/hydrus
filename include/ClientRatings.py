...
def GetStars(service_key, rating_state, rating):
    ...
    if rating_state in (NULL, MIXED):
        ...
        (pen_colour,
         brush_colour) = GetPenAndBrushColours(service_key, rating_state)
        ...
        (pen_colour,
         brush_colour) = GetPenAndBrushColours(service_key, DISLIKE)
...
class RatingsManager(object):
    ...
    def GetRatingSlice(self, service_keys):
        return frozenset({
            self._service_keys_to_ratings[service_key]
            for service_key in service_keys
            if service_key in self._service_keys_to_ratings
        })

    def GetServiceKeysToRatings(self):
        return self._service_keys_to_ratings
