...
from . import (ClientData, ClientGUICommon, ClientGUIFunctions, ClientGUIMenus,
               ClientGUIMPV, ClientGUITopLevelWindows)
...
volume_types_to_option_names[AUDIO_GLOBAL] = ('global_audio_mute',
                                              'global_audio_volume')
volume_types_to_option_names[AUDIO_MEDIA_VIEWER] = (
    'media_viewer_audio_mute', 'media_viewer_audio_volume')
volume_types_to_option_names[AUDIO_PREVIEW] = ('preview_audio_mute',
...
def ChangeVolume(volume_type, volume):
    ...
    (mute_option_name,
     volume_option_name) = volume_types_to_option_names[volume_type]
...
class AudioMuteButton(ClientGUICommon.BetterBitmapButton):
    def __init__(self, parent, volume_type):
        ...
        ClientGUICommon.BetterBitmapButton.__init__(self, parent, pixmap,
                                                    FlipMute,
                                                    self._volume_type)
    ...
    def _GetCorrectPixmap(self):
        ...
        (mute_option_name,
         volume_option_name) = volume_types_to_option_names[self._volume_type]
...
class VolumeControl(QW.QWidget):
    def __init__(self, parent, canvas_type, direction='down'):
        ...
        QP.AddToLayout(vbox, self._global_mute,
                       CC.FLAGS_EXPAND_SIZER_BOTH_WAYS)

        self.setLayout(vbox)

        self._popup_window = self._PopupWindow(self,
                                               canvas_type,
                                               direction=direction)
    ...
    class _PopupWindow(QW.QFrame):
        ...
        def DoShowHide(self):
            ...
            over_parent = parent.rect().contains(
                parent.mapFromGlobal(cursor_pos))
