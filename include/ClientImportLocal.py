...
class HDDImport(HydrusSerialisable.SerialisableBase):
    ...
    def __init__(self,
                 ...
                 paths_to_service_keys_to_tags=None,
    ...
    def _GetSerialisableInfo(self):
        ...
        return (serialisable_file_seed_cache, serialisable_options,
                self._delete_after_success, self._paused)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_HDD_IMPORT] = HDDImport
...
class ImportFolder(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def __init__(self,
        ...
        if actions is None:
            ...
            actions[
                CC.STATUS_SUCCESSFUL_BUT_REDUNDANT] = CC.IMPORT_FOLDER_IGNORE
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_tag_service_keys_to_filename_tagging_options = [
            ...
            for (service_key, filename_tagging_options) in list(
                self._tag_service_keys_to_filename_tagging_options.items())
        ]
        serialisable_file_seed_cache = self._file_seed_cache.GetSerialisableTuple(
        )
    ...
    def ToListBoxTuple(self):
        ...
        return (self._name, self._path, self._paused, self._check_regularly,
                self._period)

    def ToTuple(self):

        return (self._name, self._path, self._mimes, self._file_import_options,
                self._tag_import_options,
                self._tag_service_keys_to_filename_tagging_options,
                self._actions, self._action_locations, self._period,
                self._check_regularly, self._paused, self._check_now,
                self._show_working_popup, self._publish_files_to_popup_button,
                self._publish_files_to_page)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_IMPORT_FOLDER] = ImportFolder
