...
from . import (ClientImportFileSeeds, ClientMedia, ClientNetworkingContexts,
               ClientSearch, ClientTags)
from . import HydrusConstants as HC
from . import HydrusData, HydrusExceptions
from . import HydrusGlobals as HG
from . import HydrusNetworking, HydrusPaths, HydrusServerResources, HydrusTags

local_booru_css = FileResource(os.path.join(HC.STATIC_DIR,
                                            'local_booru_style.css'),
...
CLIENT_API_BYTE_PARAMS = {
    'hash', 'destination_page_key', 'page_key', 'Hydrus-Client-API-Access-Key',
    'Hydrus-Client-API-Session-Key'
}
CLIENT_API_STRING_PARAMS = {'name', 'url', 'domain'}
CLIENT_API_JSON_PARAMS = {
    'basic_permissions', 'system_inbox', 'system_archive', 'tags', 'file_ids',
    'only_return_identifiers', 'simple'
}
...
def ParseLocalBooruGETArgs(requests_args):
    ...
    args = HydrusNetworking.ParseTwistedRequestGETArgs(
        requests_args, LOCAL_BOORU_INT_PARAMS, LOCAL_BOORU_BYTE_PARAMS,
        LOCAL_BOORU_STRING_PARAMS, LOCAL_BOORU_JSON_PARAMS,
        LOCAL_BOORU_JSON_BYTE_LIST_PARAMS)
...
def ParseClientAPIGETArgs(requests_args):
    ...
    args = HydrusNetworking.ParseTwistedRequestGETArgs(
        requests_args, CLIENT_API_INT_PARAMS, CLIENT_API_BYTE_PARAMS,
        CLIENT_API_STRING_PARAMS, CLIENT_API_JSON_PARAMS,
    ...
    for var_name in CLIENT_API_BYTE_PARAMS:
        ...
        if var_name in parsed_request_args:
            ...
            try:
                ...
                raise HydrusExceptions.BadRequestException(
                    'I was expecting to parse \'{}\' as a hex string, but it failed.'
                    .format(var_name))
        ...
        if var_name in parsed_request_args:
            ...
            try:
                ...
                v_list = [
                    ...
                    for hash_hex in parsed_request_args[var_name]
                ]
                ...
                raise HydrusExceptions.BadRequestException(
                    'I was expecting to parse \'{}\' as a list of hex strings, but it failed.'
                    .format(var_name))
...
def ParseClientAPISearchPredicates(request):
    ...
    for tag in negated_tags:
        ...
        predicates.append(
            ClientSearch.Predicate(predicate_type=HC.PREDICATE_TYPE_TAG,
                                   value=tag,
                                   inclusive=False))
    ...
    if system_inbox:
        ...
        predicates.append(
            ClientSearch.Predicate(
                predicate_type=HC.PREDICATE_TYPE_SYSTEM_INBOX))

    elif system_archive:

        predicates.append(
            ClientSearch.Predicate(
                predicate_type=HC.PREDICATE_TYPE_SYSTEM_ARCHIVE))
...
class HydrusResourceBooruFile(HydrusResourceBooru):
    def _threadDoGETJob(self, request):
        ...
        HG.client_controller.local_booru_manager.CheckFileAuthorised(
            share_key, hash)

        media_result = HG.client_controller.local_booru_manager.GetMediaResult(
            share_key, hash)
        ...
        for media_result in media_results:
            ...
            <span class="thumbnail">
                <span class="thumbnail_container">
                    <a href="page?share_key=''' + share_key.hex(
            ) + '''&hash=''' + hash.hex() + '''">
                        <img src="thumbnail?share_key=''' + share_key.hex(
            ) + '''&hash=''' + hash.hex() + '''" />
                    </a>
                </span>
            </span>'''
    ...
    <body>'''
        ...
        if mime in HC.IMAGES or mime in HC.ANIMATIONS:
            ...
            <img width="''' + str(width) + '''" height="''' + str(
                height) + '''" src="file?share_key=''' + share_key.hex(
                ) + '''&hash=''' + hash.hex() + '''" />'''
            ...
            <video width="''' + str(width) + '''" height="''' + str(
                height
            ) + '''" controls="" loop="" autoplay="" src="file?share_key=''' + share_key.hex(
            ) + '''&hash=''' + hash.hex() + '''" />
            <p><a href="file?share_key=''' + share_key.hex(
            ) + '''&hash=''' + hash.hex(
            ) + '''">link to ''' + HC.mime_string_lookup[
                mime] + ''' file</a></p>'''
...
class HydrusResourceBooruThumbnail(HydrusResourceBooru):
    def _threadDoGETJob(self, request):
        ...
        response_context = HydrusServerResources.ResponseContext(
            200, mime=response_context_mime, path=path)
...
class HydrusResourceClientAPI(HydrusServerResources.HydrusResource):
    ...
    def _callbackParsePOSTArgs(self, request):
        ...
        (parsed_request_args,
         total_bytes_read) = ParseClientAPIPOSTArgs(request)
    ...
    def _threadDoGETJob(self, request):
        ...
        if not ClientAPI.api_request_dialog_open:
            ...
            raise HydrusExceptions.InsufficientCredentialsException(
                'The permission registration dialog is not open. Please open it under "review services" in the hydrus client.'
            )

        name = request.parsed_request_args.GetValue('name', str)

        basic_permissions = request.parsed_request_args.GetValue(
            'basic_permissions', list)

        basic_permissions = [int(value) for value in basic_permissions]

        api_permissions = ClientAPI.APIPermissions(
            name=name, basic_permissions=basic_permissions)
        ...
        response_context = HydrusServerResources.ResponseContext(
            200, mime=HC.APPLICATION_JSON, body=body)
...
class HydrusResourceClientAPIRestricted(HydrusResourceClientAPI):
    def _callbackCheckAccountRestrictions(self, request):
        ...
        HydrusResourceClientAPI._callbackCheckAccountRestrictions(
            self, request)
    ...
    def _EstablishAPIPermissions(self, request, access_key):
        ...
        try:
            ...
            api_permissions = HG.client_controller.client_api_manager.GetPermissions(
                access_key)
    ...
    def _ParseClientAPIAccessKey(self, request, source):
        ...
        access_key = self._ParseClientAPIKey(request, source,
                                             'Hydrus-Client-API-Access-Key')

        if access_key is None:

            session_key = self._ParseClientAPIKey(
                request, source, 'Hydrus-Client-API-Session-Key')
            ...
            try:
                ...
                access_key = HG.client_controller.client_api_manager.GetAccessKey(
                    session_key)
...
class HydrusResourceClientAPIRestrictedAccount(
...
class HydrusResourceClientAPIRestrictedAccountSessionKey(
        HydrusResourceClientAPIRestrictedAccount):
    def _threadDoGETJob(self, request):

        new_session_key = HG.client_controller.client_api_manager.GenerateSessionKey(
            request.client_api_permissions.GetAccessKey())
...
class HydrusResourceClientAPIRestrictedAccountVerify(
        HydrusResourceClientAPIRestrictedAccount):
    def _threadDoGETJob(self, request):
        ...
        body_dict['basic_permissions'] = list(
            basic_permissions)  # set->list for json
...
class HydrusResourceClientAPIRestrictedAddFile(
        HydrusResourceClientAPIRestricted):
    def _CheckAPIPermissions(self, request):

        request.client_api_permissions.CheckPermission(
...
class HydrusResourceClientAPIRestrictedAddTags(
        HydrusResourceClientAPIRestricted):
    def _CheckAPIPermissions(self, request):
...
class HydrusResourceClientAPIRestrictedAddTagsAddTags(
        HydrusResourceClientAPIRestrictedAddTags):
    def _threadDoPOSTJob(self, request):
        ...
        if len(hashes) == 0:
            ...
            raise HydrusExceptions.BadRequestException(
                'There were no hashes given!')

        #

        add_siblings_and_parents = request.parsed_request_args.GetValue(
            'add_siblings_and_parents', bool, default_value=True)

        service_keys_to_content_updates = collections.defaultdict(list)

        if 'service_names_to_tags' in request.parsed_request_args:

            service_names_to_tags = request.parsed_request_args.GetValue(
                'service_names_to_tags', dict)
            ...
            for (service_name, tags) in service_names_to_tags.items():
                ...
                try:
                    ...
                    service_key = HG.client_controller.services_manager.GetServiceKeyFromName(
                        HC.TAG_SERVICES, service_name)

                except:

                    raise HydrusExceptions.BadRequestException(
                        'Could not find the service "{}"!'.format(
                            service_name))
        ...
        if 'service_names_to_actions_to_tags' in request.parsed_request_args:
            ...
            service_names_to_actions_to_tags = request.parsed_request_args.GetValue(
                'service_names_to_actions_to_tags', dict)

            for (service_name,
                 actions_to_tags) in service_names_to_actions_to_tags.items():
                ...
                for (content_action, tags) in actions_to_tags.items():
                    ...
                    if service.GetServiceType() == HC.LOCAL_TAG:
                        ...
                        if content_action not in (HC.CONTENT_UPDATE_ADD,
                        ...
                        if content_action in (HC.CONTENT_UPDATE_ADD,
                                              HC.CONTENT_UPDATE_DELETE):
                    ...
                    if content_action == HC.CONTENT_UPDATE_PETITION:
                        ...
                        if isinstance(tags[0], str):
                            ...
                            tags_and_reasons = [(tag, 'Petitioned from API')
                                                for tag in tags]
                        ...
                        content_updates = [
                            HydrusData.ContentUpdate(HC.CONTENT_TYPE_MAPPINGS,
                                                     ...
                                                     (tag, hashes),
                                                     reason=reason)
                            for (tag, reason) in tags_and_reasons
                        ]
...
class HydrusResourceClientAPIRestrictedAddTagsGetTagServices(
        HydrusResourceClientAPIRestrictedAddTags):
    def _threadDoGETJob(self, request):

        local_tags = HG.client_controller.services_manager.GetServices(
            (HC.LOCAL_TAG, ))
        tag_repos = HG.client_controller.services_manager.GetServices(
            (HC.TAG_REPOSITORY, ))

        body_dict = {}

        body_dict['local_tags'] = [service.GetName() for service in local_tags]
        body_dict['tag_repositories'] = [
            service.GetName() for service in tag_repos
        ]
...
class HydrusResourceClientAPIRestrictedAddTagsCleanTags(
        HydrusResourceClientAPIRestrictedAddTags):
    def _threadDoGETJob(self, request):
...
class HydrusResourceClientAPIRestrictedAddURLs(
        HydrusResourceClientAPIRestricted):
    def _CheckAPIPermissions(self, request):
...
class HydrusResourceClientAPIRestrictedAddURLsAssociateURL(
        HydrusResourceClientAPIRestrictedAddURLs):
    def _threadDoPOSTJob(self, request):
        ...
        if len(urls_to_delete) > 0:
            ...
            content_update = HydrusData.ContentUpdate(
                ...
                (urls_to_delete, applicable_hashes))

            service_keys_to_content_updates[
                CC.COMBINED_LOCAL_FILE_SERVICE_KEY].append(content_update)

        HG.client_controller.WriteSynchronous('content_updates',
                                              service_keys_to_content_updates)
...
class HydrusResourceClientAPIRestrictedAddURLsGetURLFiles(
        HydrusResourceClientAPIRestrictedAddURLs):
    def _threadDoGETJob(self, request):
        ...
        url_statuses = HG.client_controller.Read('url_statuses',
                                                 normalised_url)
        ...
        body_dict = {
            ...
            'url_file_statuses': json_happy_url_statuses
        }
...
class HydrusResourceClientAPIRestrictedAddURLsGetURLInfo(
        HydrusResourceClientAPIRestrictedAddURLs):
    def _threadDoGETJob(self, request):
        ...
        body_dict = {
            'normalised_url': normalised_url,
            'url_type': url_type,
            'url_type_string': HC.url_type_string_lookup[url_type],
...
class HydrusResourceClientAPIRestrictedAddURLsImportURL(
        HydrusResourceClientAPIRestrictedAddURLs):
    def _threadDoPOSTJob(self, request):
        ...
        if 'destination_page_name' in request.parsed_request_args:
            ...
            destination_page_name = request.parsed_request_args.GetValue(
        ...
        if 'destination_page_key' in request.parsed_request_args:
            ...
            destination_page_key = request.parsed_request_args.GetValue(
                'destination_page_key', bytes)

        show_destination_page = request.parsed_request_args.GetValue(
            'show_destination_page', bool, default_value=False)

        def do_it():

            return HG.client_controller.gui.ImportURLFromAPI(
                url, service_keys_to_tags, destination_page_name,
                destination_page_key, show_destination_page)

        try:

            (normalised_url,
             result_text) = HG.client_controller.CallBlockingToQt(
                 HG.client_controller.gui, do_it)
        ...
        time.sleep(
            0.05
        )  # yield and give the ui time to catch up with new URL pubsubs in case this is being spammed

        body_dict = {
            'human_result_text': result_text,
            'normalised_url': normalised_url
        }
...
class HydrusResourceClientAPIRestrictedGetFiles(
        HydrusResourceClientAPIRestricted):
    def _CheckAPIPermissions(self, request):
...
class HydrusResourceClientAPIRestrictedGetFilesSearchFiles(
        HydrusResourceClientAPIRestrictedGetFiles):
    def _threadDoGETJob(self, request):
        ...
        # newest first
        sort_by = ClientMedia.MediaSort(
            sort_type=('system', CC.SORT_FILES_BY_IMPORT_TIME),
            sort_asc=CC.SORT_DESC)

        hash_ids = HG.client_controller.Read('file_query_ids',
                                             file_search_context,
                                             sort_by=sort_by)
...
class HydrusResourceClientAPIRestrictedGetFilesGetFile(
        HydrusResourceClientAPIRestrictedGetFiles):
    def _threadDoGETJob(self, request):
        ...
        try:
            ...
            if 'file_id' in request.parsed_request_args:
                ...
                request.client_api_permissions.CheckPermissionToSeeFiles(
                    (file_id, ))

                (media_result, ) = HG.client_controller.Read(
                    'media_results_from_ids', (file_id, ))
            ...
            raise HydrusExceptions.NotFoundException(
                'One or more of those file identifiers was missing!')
...
class HydrusResourceClientAPIRestrictedGetFilesFileMetadata(
        HydrusResourceClientAPIRestrictedGetFiles):
    def _threadDoGETJob(self, request):

        only_return_identifiers = request.parsed_request_args.GetValue(
            'only_return_identifiers', bool, default_value=False)

        try:

            if 'file_ids' in request.parsed_request_args:

                file_ids = request.parsed_request_args.GetValue(
                    'file_ids', list)
                ...
                if only_return_identifiers:
                    ...
                    file_ids_to_hashes = HG.client_controller.Read(
                        'hash_ids_to_hashes', hash_ids=file_ids)

                else:

                    media_results = HG.client_controller.Read(
                        'media_results_from_ids', file_ids)
                ...
                if only_return_identifiers:
                    ...
                    file_ids_to_hashes = HG.client_controller.Read(
                        'hash_ids_to_hashes', hashes=hashes)
                ...
                raise HydrusExceptions.BadRequestException(
                    'Please include a file_ids or hashes parameter!')
        ...
        if only_return_identifiers:
            ...
            for media_result in media_results:
                ...
                metadata_row['mime'] = HC.mime_mimetype_string_lookup[
                    file_info_manager.mime]
                ...
                for (service_key, statuses_to_tags
                    ...
                    if service_key not in service_keys_to_names:
                        ...
                        service_keys_to_names[
                            service_key] = services_manager.GetName(
                                service_key)

                    service_name = service_keys_to_names[service_key]

                    service_names_to_statuses_to_tags[service_name] = {
                        str(status): list(tags)
                        for (status, tags) in statuses_to_tags.items()
                    }

                metadata_row[
                    'service_names_to_statuses_to_tags'] = service_names_to_statuses_to_tags
...
class HydrusResourceClientAPIRestrictedGetFilesGetThumbnail(
        HydrusResourceClientAPIRestrictedGetFiles):
    def _threadDoGETJob(self, request):
        ...
        try:
            ...
            path = HG.client_controller.client_files_manager.GetThumbnailPath(
                media_result)
...
class HydrusResourceClientAPIRestrictedManageCookies(
        HydrusResourceClientAPIRestricted):
    def _CheckAPIPermissions(self, request):

        request.client_api_permissions.CheckPermission(
            ClientAPI.CLIENT_API_PERMISSION_MANAGE_COOKIES)


class HydrusResourceClientAPIRestrictedManageCookiesGetCookies(
        HydrusResourceClientAPIRestrictedManageCookies):
    def _threadDoGETJob(self, request):
        ...
        if '.' not in domain:
            ...
            raise HydrusExceptions.BadRequestException(
                'The value "{}" does not seem to be a domain!'.format(domain))

        network_context = ClientNetworkingContexts.NetworkContext(
            CC.NETWORK_CONTEXT_DOMAIN, domain)

        session = HG.client_controller.network_engine.session_manager.GetSession(
            network_context)
...
class HydrusResourceClientAPIRestrictedManageCookiesSetCookies(
        HydrusResourceClientAPIRestrictedManageCookies):
    def _threadDoPOSTJob(self, request):
        ...
        for cookie_row in cookie_rows:
            ...
            if len(cookie_row) != 5:
                ...
                raise HydrusExceptions.BadRequestException(
                    'The cookie "{}" did not come in the format [ name, value, domain, path, expires ]!'
                    .format(cookie_row))

            (name, value, domain, path, expires) = cookie_row

            ndp_bad = True in (not isinstance(var, str)
                               for var in (name, domain, path))
            ...
            if ndp_bad or v_bad or e_bad:
                ...
                raise HydrusExceptions.BadRequestException(
                    'In the row [ name, value, domain, path, expires ], which I received as "{}", name, domain, and path need to be strings, value needs to be null or a string, and expires needs to be null or an integer!'
                    .format(cookie_row))
...
class HydrusResourceClientAPIRestrictedManagePages(
        HydrusResourceClientAPIRestricted):
    def _CheckAPIPermissions(self, request):
...
class HydrusResourceClientAPIRestrictedManagePagesFocusPage(
        HydrusResourceClientAPIRestrictedManagePages):
    def _threadDoPOSTJob(self, request):
        def do_it(page_key):
        ...
        try:
            ...
            HG.client_controller.CallBlockingToQt(HG.client_controller.gui,
                                                  do_it, page_key)
...
class HydrusResourceClientAPIRestrictedManagePagesGetPages(
        HydrusResourceClientAPIRestrictedManagePages):
    def _threadDoGETJob(self, request):
        def do_it():
        ...
        page_info_dict = HG.client_controller.CallBlockingToQt(
            HG.client_controller.gui, do_it)
...
class HydrusResourceClientAPIRestrictedManagePagesGetPageInfo(
        HydrusResourceClientAPIRestrictedManagePages):
    def _threadDoGETJob(self, request):
        def do_it(page_key, simple):

            return HG.client_controller.gui.GetPageAPIInfoDict(
                page_key, simple)

        page_key = request.parsed_request_args.GetValue('page_key', bytes)

        simple = request.parsed_request_args.GetValue('simple',
                                                      bool,
                                                      default_value=True)

        page_info_dict = HG.client_controller.CallBlockingToQt(
            HG.client_controller.gui, do_it, page_key, simple)

        if page_info_dict is None:

            raise HydrusExceptions.NotFoundException(
                'Did not find a page for "{}"!'.format(page_key.hex()))
