...
def ConvertServiceKeysToContentUpdatesToPrettyString(
    ...
    s += ', '.join(actions) + extra_words + ' ' + HydrusData.ToHumanInt(
        num_files) + ' files'
...
def GetSortTypeChoices():
    ...
    service_keys = HG.client_controller.services_manager.GetServiceKeys(
        ...
        for predicate in list(master_predicate_dict.values()):
            ...
            if predicate.HasNonZeroCount():
                ...
                if unnamespaced_predicate in unnamespaced_predicate_dict:
                    ...
                    unnamespaced_predicate_dict[
                        unnamespaced_predicate].AddCounts(
                            unnamespaced_predicate)

                else:

                    unnamespaced_predicate_dict[
                        unnamespaced_predicate] = unnamespaced_predicate

        for (unnamespaced_predicate,
             count) in list(subtag_nonzero_instance_counter.items()):
            ...
            if count > 1:
                ...
                master_predicate_dict[
                    unnamespaced_predicate] = unnamespaced_predicate_dict[
                        unnamespaced_predicate]
...
class ApplicationCommand(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        if self._command_type == CC.APPLICATION_COMMAND_TYPE_SIMPLE:
            ...
            serialisable_data = (service_key.hex(), content_type, action,
                                 value)
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        if self._command_type == CC.APPLICATION_COMMAND_TYPE_SIMPLE:
            ...
            (serialisable_service_key, content_type, action,
             value) = serialisable_data

            self._data = (bytes.fromhex(serialisable_service_key),
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_APPLICATION_COMMAND] = ApplicationCommand
...
class Booru(HydrusData.HydrusYAMLBase):
    ...
    def __init__(self, name, search_url, search_separator, advance_by_page_num,
                 thumb_classname, image_id, image_data,
    ...
    def GetData(self):
        return (self._search_url, self._search_separator,
...
class Credentials(HydrusData.HydrusYAMLBase):
    ...
    def __repr__(self):
        return 'Credentials: ' + str(
            (self._host, self._port, self._access_key.hex()))
    ...
    def HasAccessKey(self):
        return self._access_key is not None and self._access_key != ''
...
class Imageboard(HydrusData.HydrusYAMLBase):
    ...
    def GetBoardInfo(self):
        return (self._post_url, self._flood_time, self._form_fields,
                self._restrictions)
