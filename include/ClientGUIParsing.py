...
class DownloaderExportPanel(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, network_engine):
        ...
        page_func = HydrusData.Call(
            ...
            os.path.join(HC.HELP_DIR, 'downloader_sharing.html'))

        menu_items.append(
            ('normal', 'open the downloader sharing help',
             'Open the help page for sharing downloaders in your web browser.',
             page_func))
        ...
        self._listctrl = ClientGUIListCtrl.BetterListCtrl(
            ...
            36,
            columns,
            self._ConvertContentToListCtrlTuples,
        ...
        listctrl_panel.AddButton('add headers/bandwidth rules',
                                 self._AddDomainMetadata)
        listctrl_panel.AddDeleteButton()
        listctrl_panel.AddSeparator()
        listctrl_panel.AddButton('export to png',
                                 self._Export,
                                 enabled_check_func=self._CanExport)
    ...
    def _AddGUG(self):
        ...
        choosable_gugs = [
            gug for gug in self._network_engine.domain_manager.GetGUGs()
            if gug.IsFunctional() and gug not in existing_data
        ]
        ...
        domains = {
            ClientNetworkingDomain.ConvertURLIntoDomain(example_url)
            for example_url in itertools.chain.from_iterable((
                gug.GetExampleURLs() for gug in gugs_to_include))
        }

        domain_metadatas_to_include = self._GetDomainMetadatasToInclude(
            domains)

        url_classes_to_include = self._GetURLClassesToInclude(gugs_to_include)

        url_classes_to_include = self._FleshOutURLClassesWithAPILinks(
            url_classes_to_include)
    ...
    def _AddLoginScript(self):
        ...
        choosable_login_scripts = [
            ls for ls in self._network_engine.login_manager.GetLoginScripts()
            if ls not in existing_data
        ]

        choice_tuples = [(login_script.GetName(), login_script, False)
                         for login_script in choosable_login_scripts]

        with ClientGUITopLevelWindows.DialogEdit(
                self, 'select login scripts') as dlg:
    ...
    def _AddParser(self):
        ...
        choosable_parsers = [
            p for p in self._network_engine.domain_manager.GetParsers()
            if p not in existing_data
        ]

        choice_tuples = [(parser.GetName(), parser, False)
                         for parser in choosable_parsers]

        with ClientGUITopLevelWindows.DialogEdit(self,
                                                 'select parsers') as dlg:
    ...
    def _AddURLClass(self):
        ...
        choosable_url_classes = [
            u for u in self._network_engine.domain_manager.GetURLClasses()
            if u not in existing_data
        ]

        choice_tuples = [(url_class.GetName(), url_class, False)
                         for url_class in choosable_url_classes]

        with ClientGUITopLevelWindows.DialogEdit(self,
                                                 'select url classes') as dlg:
    ...
    def _Export(self):
        ...
        export_object = HydrusSerialisable.SerialisableList(
            self._listctrl.GetData())

        message = 'The end-user will see this sort of summary:'
        message += os.linesep * 2
        message += os.linesep.join(
            (obj.GetSafeSummary() for obj in export_object[:20]))

        if len(export_object) > 20:

            message += os.linesep
            message += '(and ' + HydrusData.ToHumanInt(
                len(export_object) - 20) + ' others)'
        ...
        with ClientGUITopLevelWindows.DialogNullipotent(
            ...
            if num_gugs == 0:
                ...
                title += ' - ' + HydrusData.ToHumanInt(
                    num_gugs) + ' downloaders'

                description = ', '.join(gug_names)

            panel = ClientGUISerialisable.PngExportPanel(
                dlg, export_object, title=title, description=description)
    ...
    def _FleshOutNGUGsWithGUGs(self, gugs):
        ...
        possible_new_gugs = [
            gug for gug in self._network_engine.domain_manager.GetGUGs()
            if gug.IsFunctional() and gug not in existing_data
            and gug not in gugs_to_include
        ]

        interesting_gug_keys_and_names = list(
            itertools.chain.from_iterable([
                gug.GetGUGKeysAndNames()
                for gug in gugs_to_include if isinstance(
                    gug, ClientNetworkingDomain.NestedGalleryURLGenerator)
            ]))

        interesting_gugs = [
            gug for gug in possible_new_gugs
            if gug.GetGUGKeyAndName() in interesting_gug_keys_and_names
        ]

        gugs_to_include.update(interesting_gugs)

        if True in (isinstance(
                gug, ClientNetworkingDomain.NestedGalleryURLGenerator)
                    for gug in interesting_gugs):
    ...
    def _FleshOutURLClassesWithAPILinks(self, url_classes):
        ...
        api_links_dict = dict(
            ClientNetworkingDomain.ConvertURLClassesIntoAPIPairs(
                self._network_engine.domain_manager.GetURLClasses()))
        ...
        url_classes_to_include = [
            u for u in url_classes_to_include if u not in existing_data
        ]
    ...
    def _GetDomainMetadatasToInclude(self, domains):
        ...
        domains = {
            ...
            for d in itertools.chain.from_iterable(
                ClientNetworkingDomain.ConvertDomainIntoAllApplicableDomains(
        ...
        existing_domains = {
            ...
            if isinstance(obj, ClientNetworkingDomain.DomainMetadataPackage)
        }
        ...
        for domain in domains:
            ...
            if self._network_engine.domain_manager.HasCustomHeaders(
                    network_context):

                headers_list = self._network_engine.domain_manager.GetShareableCustomHeaders(
                    network_context)
            ...
            if self._network_engine.bandwidth_manager.HasRules(
                    network_context):

                bandwidth_rules = self._network_engine.bandwidth_manager.GetRules(
                    network_context)
            ...
            if headers_list is not None or bandwidth_rules is not None:
                ...
                domain_metadata = ClientNetworkingDomain.DomainMetadataPackage(
                    domain=domain,
        ...
        for domain_metadata in domain_metadatas:
            ...
            QW.QMessageBox.information(
                self, 'Information', domain_metadata.GetDetailedSafeSummary())
    ...
    def _GetParsersToInclude(self, url_classes):
        ...
        for url_class in url_classes:
            ...
            (url_type, match_name, can_parse
             ) = self._network_engine.domain_manager.GetURLParseCapability(
                 example_url)
            ...
            if can_parse:
                ...
                try:
                    ...
                    (
                        url_to_fetch, parser
                    ) = self._network_engine.domain_manager.GetURLToFetchAndParser(
                        example_url)
    ...
    def _GetURLClassesToInclude(self, gugs):
        ...
        for gug in gugs:
            ...
            for example_url in example_urls:
                ...
                url_class = self._network_engine.domain_manager.GetURLClass(
                    example_url)
                ...
                if url_class is not None:
                    ...
                    domain = ClientNetworkingDomain.ConvertURLIntoSecondLevelDomain(
                        example_url)

                    for um in list(self._network_engine.domain_manager.
                                   GetURLClasses()):

                        if ClientNetworkingDomain.ConvertURLIntoSecondLevelDomain(
                                um.GetExampleURL()
                        ) == domain and um.GetURLType() in (HC.URL_TYPE_POST,
                                                            HC.URL_TYPE_FILE):
...
class EditCompoundFormulaPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, formula, test_context):
        ...
        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR,
                         'downloader_parsers_formulae.html#compound_formula'))

        menu_items.append(
            ('normal', 'open the compound formula help',
             'Open the help page for compound formulae in your web browser.',
             page_func))
        ...
        self._add_formula = ClientGUICommon.BetterButton(
            edit_panel, 'add', self.Add)

        self._edit_formula = ClientGUICommon.BetterButton(
            edit_panel, 'edit', self.Edit)

        self._move_formula_up = ClientGUICommon.BetterButton(
            edit_panel, '\u2191', self.MoveUp)

        self._delete_formula = ClientGUICommon.BetterButton(
            edit_panel, 'X', self.Delete)

        self._move_formula_down = ClientGUICommon.BetterButton(
            edit_panel, '\u2193', self.MoveDown)

        self._sub_phrase = QW.QLineEdit(edit_panel)

        (formulae, sub_phrase, string_match,
         string_converter) = formula.ToTuple()

        self._string_match_button = ClientGUIControls.StringMatchButton(
            edit_panel, string_match)

        self._string_converter_button = ClientGUIControls.StringConverterButton(
            edit_panel, string_converter)

        #

        test_panel = ClientGUICommon.StaticBox(self, 'test')

        self._test_panel = TestPanel(test_panel,
                                     self.GetValue,
        ...
        QP.AddToLayout(udd_button_vbox, (20, 20),
                       CC.FLAGS_EXPAND_SIZER_BOTH_WAYS)
        QP.AddToLayout(udd_button_vbox, self._move_formula_up,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(udd_button_vbox, self._delete_formula, CC.FLAGS_VCENTER)
        QP.AddToLayout(udd_button_vbox, self._move_formula_down,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(udd_button_vbox, (20, 20),
                       CC.FLAGS_EXPAND_SIZER_BOTH_WAYS)
        ...
        edit_panel.Add(
            ClientGUICommon.BetterStaticText(
                edit_panel,
                'Newlines are removed from parsed strings right after parsing, before this String Match is tested.',
                ellipsize_end=True), CC.FLAGS_EXPAND_PERPENDICULAR)
        edit_panel.Add(self._string_match_button,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        edit_panel.Add(self._string_converter_button,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def Add(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit formula', frame_key='deeply_nested_dialog') as dlg:

            panel = EditFormulaPanel(dlg, existing_formula,
                                     self._test_panel.GetTestContextForChild)
        ...
        if selection != -1:
            ...
            if self._formulae.count() == 1:
                ...
                QW.QMessageBox.critical(
                    self, 'Error',
                    'A compound formula needs at least one sub-formula!')
    ...
    def Edit(self):
        ...
        if selection != -1:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                    self, 'edit formula',
                    frame_key='deeply_nested_dialog') as dlg:

                panel = EditFormulaPanel(
                    dlg, old_formula, self._test_panel.GetTestContextForChild)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    self._formulae.item(selection).setData(
                        QC.Qt.UserRole, new_formula)

    def GetValue(self):

        formulae = [
            QP.GetClientData(self._formulae, i)
            for i in range(self._formulae.count())
        ]
        ...
        formula = ClientParsing.ParseFormulaCompound(formulae, sub_phrase,
                                                     string_match,
                                                     string_converter)
...
class EditContextVariableFormulaPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, formula, test_context):
        ...
        page_func = HydrusData.Call(
            ...
            os.path.join(
                HC.HELP_DIR,
                'downloader_parsers_formulae.html#context_variable_formula'))

        menu_items.append((
            'normal', 'open the context variable formula help',
            'Open the help page for context variable formulae in your web browser.',
            page_func))
    ...
    def GetValue(self):
        ...
        formula = ClientParsing.ParseFormulaContextVariable(
            variable_name, string_match, string_converter)
...
class EditFormulaPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, formula, test_context_callable):
        ...
        (width, height) = ClientGUIFunctions.ConvertTextToPixels(
            self._formula_description, (90, 8))
        ...
        self._edit_formula = ClientGUICommon.BetterButton(
            my_panel, 'edit formula', self._EditFormula)

        self._change_formula_type = ClientGUICommon.BetterButton(
            my_panel, 'change formula type', self._ChangeFormulaType)
        ...
        QP.AddToLayout(button_hbox, self._edit_formula,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(button_hbox, self._change_formula_type,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
    ...
    def _ChangeFormulaType(self):
        ...
        if self._current_formula.ParsesSeparatedContent():
            ...
            new_html = ClientParsing.ParseFormulaHTML(
                content_to_fetch=ClientParsing.HTML_CONTENT_HTML)
        ...
        elif isinstance(self._current_formula,
                        ClientParsing.ParseFormulaCompound):

            order = ('html', 'json', 'context_variable')

        elif isinstance(self._current_formula,
                        ClientParsing.ParseFormulaContextVariable):
        ...
        for formula_type in order:
            ...
            if formula_type == 'html':
                ...
                choice_tuples.append(
                    ('change to a new HTML formula', new_html))

            elif formula_type == 'json':

                choice_tuples.append(
                    ('change to a new JSON formula', new_json))

            elif formula_type == 'compound':

                choice_tuples.append(
                    ('change to a new COMPOUND formula', new_compound))

            elif formula_type == 'context_variable':

                choice_tuples.append(
                    ('change to a new CONTEXT VARIABLE formula',
                     new_context_variable))

        try:

            self._current_formula = ClientGUIDialogsQuick.SelectFromList(
                self, 'select formula type', choice_tuples)
    ...
    def _EditFormula(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, dlg_title, frame_key='deeply_nested_dialog') as dlg:
...
class EditHTMLTagRulePanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, tag_rule):
        ...
        (rule_type, tag_name, tag_attributes, tag_index, tag_depth,
         should_test_tag_string, tag_string_string_match) = tag_rule.ToTuple()
        ...
        self._rule_type.addItem('search descendents',
                                ClientParsing.HTML_RULE_TYPE_DESCENDING)
        self._rule_type.addItem('walk back up ancestors',
                                ClientParsing.HTML_RULE_TYPE_ASCENDING)

        self._tag_name = QW.QLineEdit(self)

        self._tag_attributes = ClientGUIControls.StringToStringDictControl(
            self, tag_attributes, min_height=4)

        self._tag_index = ClientGUICommon.NoneableSpinCtrl(
            self, 'index to fetch', none_phrase='get all', min=0, max=255)

        self._tag_depth = QP.MakeQSpinBox(self, min=1, max=255)

        self._should_test_tag_string = QW.QCheckBox(self)

        self._tag_string_string_match = ClientGUIControls.StringMatchButton(
            self, tag_string_string_match)
        ...
        QP.AddToLayout(vbox, self._current_description,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        self._should_test_tag_string.clicked.connect(
            self.EventShouldTestChanged)
    ...
    def GetValue(self):
        ...
        if rule_type == ClientParsing.HTML_RULE_TYPE_DESCENDING:
            ...
            tag_rule = ClientParsing.ParseRuleHTML(
                rule_type=rule_type,
                tag_name=tag_name,
                tag_attributes=tag_attributes,
                tag_index=tag_index,
                should_test_tag_string=should_test_tag_string,
                tag_string_string_match=tag_string_string_match)
...
class EditHTMLFormulaPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, formula, test_context):
        ...
        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR,
                         'downloader_parsers_formulae.html#html_formula'))

        menu_items.append(
            ('normal', 'open the html formula help',
             'Open the help page for html formulae in your web browser.',
             page_func))
        ...
        self._add_rule = ClientGUICommon.BetterButton(edit_panel, 'add',
                                                      self.Add)

        self._edit_rule = ClientGUICommon.BetterButton(edit_panel, 'edit',
                                                       self.Edit)

        self._move_rule_up = ClientGUICommon.BetterButton(
            edit_panel, '\u2191', self.MoveUp)

        self._delete_rule = ClientGUICommon.BetterButton(
            edit_panel, 'X', self.Delete)

        self._move_rule_down = ClientGUICommon.BetterButton(
            edit_panel, '\u2193', self.MoveDown)

        self._content_to_fetch = ClientGUICommon.BetterChoice(edit_panel)

        self._content_to_fetch.addItem('attribute',
                                       ClientParsing.HTML_CONTENT_ATTRIBUTE)
        self._content_to_fetch.addItem('string',
                                       ClientParsing.HTML_CONTENT_STRING)
        self._content_to_fetch.addItem('html', ClientParsing.HTML_CONTENT_HTML)

        self._content_to_fetch.currentIndexChanged.connect(
            self._UpdateControls)

        self._attribute_to_fetch = QW.QLineEdit(edit_panel)

        (tag_rules, content_to_fetch, attribute_to_fetch, string_match,
         string_converter) = formula.ToTuple()
        ...
        QP.AddToLayout(tag_rules_hbox, self._tag_rules,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
    ...
    def _UpdateControls(self):
        ...
        if self._content_to_fetch.GetValue(
        ) == ClientParsing.HTML_CONTENT_ATTRIBUTE:
        ...
        if selection != -1:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    self._tag_rules.item(selection).setData(
                        QC.Qt.UserRole, rule)

    def GetValue(self):

        tags_rules = [
            QP.GetClientData(self._tag_rules, i)
            for i in range(self._tag_rules.count())
        ]
        ...
        if content_to_fetch == ClientParsing.HTML_CONTENT_ATTRIBUTE and attribute_to_fetch == '':
            ...
            raise HydrusExceptions.VetoException(
                'Please enter an attribute to fetch!')

        string_match = self._string_match_button.GetValue()

        string_converter = self._string_converter_button.GetValue()

        formula = ClientParsing.ParseFormulaHTML(tags_rules, content_to_fetch,
                                                 attribute_to_fetch,
                                                 string_match,
                                                 string_converter)
...
class EditJSONParsingRulePanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, rule):
        ...
        self._parse_rule_type.addItem(
            'dictionary entry', ClientParsing.JSON_PARSE_RULE_TYPE_DICT_KEY)
        self._parse_rule_type.addItem(
            'all dictionary/list items',
            ClientParsing.JSON_PARSE_RULE_TYPE_ALL_ITEMS)
        self._parse_rule_type.addItem(
            'indexed list item',
            ClientParsing.JSON_PARSE_RULE_TYPE_INDEXED_ITEM)

        self._string_match = ClientGUIControls.EditStringMatchPanel(
            self,
            string_match=ClientParsing.StringMatch(
                match_type=ClientParsing.STRING_MATCH_FIXED,
                match_value='posts',
                example_string='posts'))
        ...
        QP.AddToLayout(vbox, self._parse_rule_type,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
...
class EditJSONFormulaPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, formula, test_context):
        ...
        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR,
                         'downloader_parsers_formulae.html#json_formula'))

        menu_items.append(
            ('normal', 'open the json formula help',
             'Open the help page for json formulae in your web browser.',
             page_func))
        ...
        self._parse_rules.setSelectionMode(
        ...
        self._content_to_fetch.addItem('string',
                                       ClientParsing.JSON_CONTENT_STRING)
        self._content_to_fetch.addItem('dictionary keys',
                                       ClientParsing.JSON_CONTENT_DICT_KEYS)
        self._content_to_fetch.addItem('json', ClientParsing.JSON_CONTENT_JSON)

        (parse_rules, content_to_fetch, string_match,
         string_converter) = formula.ToTuple()
        ...
        QP.AddToLayout(parse_rules_hbox, self._parse_rules,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
    ...
    def Add(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
            ...
            new_rule = (ClientParsing.JSON_PARSE_RULE_TYPE_DICT_KEY,
        ...
        if selection != -1:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    self._parse_rules.item(selection).setData(
                        QC.Qt.UserRole, rule)

    def GetValue(self):

        parse_rules = [
            QP.GetClientData(self._parse_rules, i)
            for i in range(self._parse_rules.count())
        ]
        ...
        formula = ClientParsing.ParseFormulaJSON(parse_rules, content_to_fetch,
                                                 string_match,
                                                 string_converter)
...
class EditContentParserPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, content_parser, test_context,
        ...
        page_func = HydrusData.Call(
            ...
            os.path.join(
                HC.HELP_DIR,
                'downloader_parsers_content_parsers.html#content_parsers'))

        menu_items.append(
            ('normal', 'open the content parsers help',
             'Open the help page for content parsers in your web browser.',
             page_func))
        ...
        self._content_panel = ClientGUICommon.StaticBox(
            self._edit_panel, 'content type')
        ...
        for permitted_content_type in permitted_content_types:
            ...
            self._content_type.addItem(types_to_str[permitted_content_type],
                                       permitted_content_type)

        self._content_type.currentIndexChanged.connect(
            self.EventContentTypeChange)

        self._urls_panel = QW.QWidget(self._content_panel)

        self._url_type = ClientGUICommon.BetterChoice(self._urls_panel)

        self._url_type.addItem('url to download/pursue (file/post url)',
                               HC.URL_TYPE_DESIRED)
        self._url_type.addItem('url to associate (source url)',
                               HC.URL_TYPE_SOURCE)
        ...
        self._timestamp_type = ClientGUICommon.BetterChoice(
            self._timestamp_panel)

        self._timestamp_type.addItem('source time', HC.TIMESTAMP_TYPE_SOURCE)

        self._title_panel = QW.QWidget(self._content_panel)

        self._title_priority = QP.MakeQSpinBox(self._title_panel,
                                               min=0,
                                               max=100)
        ...
        self._sort_type.addItem('do not sort formula text results',
                                ClientParsing.CONTENT_PARSER_SORT_TYPE_NONE)
        self._sort_type.addItem(
            'sort by human-friendly lexicographic',
            ClientParsing.CONTENT_PARSER_SORT_TYPE_HUMAN_SORT)
        self._sort_type.addItem(
            'sort by strict lexicographic',
        ...
        (name, content_type, formula, sort_type, sort_asc,
         additional_info) = content_parser.ToTuple()

        self._formula = EditFormulaPanel(
            self._edit_panel, formula, self._test_panel.GetTestContextForChild)
        ...
        rows.append(('file url quality precedence (higher is better): ',
                     self._file_priority))
        ...
        rows.append(
            ('title precedence (higher is better): ', self._title_priority))
        ...
        rows.append(
            ('veto if match found (OFF means \'veto if match not found\'): ',
             self._veto_if_matches_found))
        ...
        self._content_panel.Add(self._urls_panel,
                                CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        self._content_panel.Add(self._mappings_panel,
                                CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        self._content_panel.Add(self._hash_panel,
                                CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        self._content_panel.Add(self._timestamp_panel,
                                CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        self._content_panel.Add(self._title_panel,
                                CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        self._content_panel.Add(self._veto_panel,
                                CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        self._content_panel.Add(self._temp_variable_panel,
                                CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        ...
        self._edit_panel.Add(self._content_panel,
                             CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def GetValue(self):
        ...
        content_parser = ClientParsing.ContentParser(
            name=name,
...
class EditContentParsersPanel(ClientGUICommon.StaticBox):
    def __init__(self, parent, test_context_callable, permitted_content_types):
        ...
        self._content_parsers = ClientGUIListCtrl.BetterListCtrl(
            content_parsers_panel,
            'content_parsers',
            6,
            24,
            columns,
            self._ConvertContentParserToListCtrlTuples,
            use_simple_delete=True,
        ...
        content_parsers_panel.AddButton('edit',
                                        self._Edit,
                                        enabled_only_on_selection=True)
        content_parsers_panel.AddDeleteButton()
        content_parsers_panel.AddSeparator()
        content_parsers_panel.AddImportExportButtons(
            (ClientParsing.ContentParser, ), self._AddContentParser)
    ...
    def _Add(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit content parser',
                frame_key='deeply_nested_dialog') as dlg_edit:

            panel = EditContentParserPanel(dlg_edit, content_parser,
                                           test_context,
                                           self._permitted_content_types)
    ...
    def _AddContentParser(self, content_parser):
        ...
        HydrusSerialisable.SetNonDupeName(content_parser,
    ...
    def _ConvertContentParserToListCtrlTuples(self, content_parser):
        ...
        pretty_produces = ClientParsing.ConvertParsableContentToPrettyString(
            produces, include_veto=True)
    ...
    def _Edit(self):
        ...
        for content_parser in content_parsers:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                ...
                panel = EditContentParserPanel(dlg, content_parser,
                                               test_context,
                                               self._permitted_content_types)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HydrusSerialisable.SetNonDupeName(edited_content_parser,
                                                      self._GetExistingNames())
    ...
    def _GetExistingNames(self):
        ...
        names = {
            content_parser.GetName()
            for content_parser in self._content_parsers.GetData()
        }
...
class EditNodes(QW.QWidget):
    def __init__(self, parent, nodes, referral_url_callable,
                 example_data_callable):
        ...
        self._nodes = ClientGUIListCtrl.BetterListCtrl(
            self,
            'nodes',
            20,
            20, [('name', 16), ('node type', 11), ('produces', -1)],
            self._ConvertNodeToTuples,
            delete_key_callback=self.Delete,
            activation_callback=self.Edit)

        menu_items = []

        menu_items.append(('normal', 'content node',
                           'A node that parses the given data for content.',
                           self.AddContentNode))
        menu_items.append((
            'normal', 'link node',
            'A node that parses the given data for a link, which it then pursues.',
            self.AddLinkNode))
        ...
        self._duplicate_button = ClientGUICommon.BetterButton(
    ...
    def _ImportObject(self, obj):
        ...
        if isinstance(obj, HydrusSerialisable.SerialisableList):
            ...
            if isinstance(obj, (ClientParsing.ContentParser,
                                ClientParsing.ParseNodeContentLink)):
                ...
                QW.QMessageBox.warning(
                    self, 'Warning',
                    'That was not a script--it was a: ' + type(obj).__name__)
    ...
    def AddNode(self, dlg_title, empty_node, panel_class):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, dlg_title, frame_key='deeply_nested_dialog') as dlg_edit:
            ...
            if isinstance(empty_node, ClientParsing.ContentParser):
                ...
                panel = panel_class(
                    dlg_edit, empty_node, ({}, example_data),
                    [HC.CONTENT_TYPE_MAPPINGS, HC.CONTENT_TYPE_VETO])

            else:

                panel = panel_class(dlg_edit, empty_node, referral_url,
                                    example_data)
...
class EditParseNodeContentLinkPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, node, referral_url=None, example_data=None):
        ...
        self._formula = EditFormulaPanel(edit_panel, formula,
                                         self.GetTestContext)

        children_panel = ClientGUICommon.StaticBox(edit_panel,
                                                   'content parsing children')

        self._children = EditNodes(children_panel, children,
                                   self.GetExampleURL, self.GetExampleData)
        ...
        self._test_fetch_result = QW.QPushButton(
            'try fetching the first result', test_panel)
...
The formula should attempt to parse full or relative urls. If the url is relative (like href="/page/123"), it will be appended to the referral url given by this node's parent. It will then attempt to GET them all.'''
        ...
        QP.AddToLayout(vbox, self._test_fetch_result,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def EventTestFetchResult(self):
        ...
        network_job = ClientNetworkingJobs.NetworkJob(
            'GET', self._my_example_url, referral_url=self._referral_url)
    ...
    def EventTestParse(self):
        def qt_code(parsed_urls):
            ...
            result_lines = [
                '*** ' + HydrusData.ToHumanInt(len(parsed_urls)) +
                ' RESULTS BEGIN ***'
            ]
    ...
    def GetValue(self):
        ...
        node = ClientParsing.ParseNodeContentLink(name=name,
                                                  formula=formula,
                                                  children=children)
...
class EditPageParserPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, parser, formula=None, test_context=None):
        ...
        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR,
                         'downloader_parsers_page_parsers.html#page_parsers'))

        menu_items.append(
            ('normal', 'open the page parser help',
             'Open the help page for page parsers in your web browser.',
             page_func))
        ...
        conversion_panel = ClientGUICommon.StaticBox(main_panel,
                                                     'pre-parsing conversion')

        string_converter = parser.GetStringConverter()

        self._string_converter = ClientGUIControls.StringConverterButton(
            conversion_panel, string_converter)

        #

        example_urls_panel = ClientGUICommon.StaticBox(main_panel,
                                                       'example urls')

        self._example_urls = ClientGUIListBoxes.AddEditDeleteListBox(
            example_urls_panel, 6, str, self._AddExampleURL,
            self._EditExampleURL)
        ...
        self._formula = EditFormulaPanel(formula_panel, formula,
                                         formula_test_callable)
        ...
        sub_page_parsers_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            sub_page_parsers_notebook_panel)

        columns = [('name', 24), ('\'post\' separation formula', 24),
                   ('produces', -1)]

        self._sub_page_parsers = ClientGUIListCtrl.BetterListCtrl(
            sub_page_parsers_panel,
            'sub_page_parsers',
            4,
            36,
            columns,
            self._ConvertSubPageParserToListCtrlTuples,
            use_simple_delete=True,
            activation_callback=self._EditSubPageParser)

        sub_page_parsers_panel.SetListCtrl(self._sub_page_parsers)

        sub_page_parsers_panel.AddButton('add', self._AddSubPageParser)
        sub_page_parsers_panel.AddButton('edit',
                                         self._EditSubPageParser,
                                         enabled_only_on_selection=True)
        ...
        test_url_fetch_panel = ClientGUICommon.StaticBox(
            test_panel, 'fetch test data from url')

        self._test_url = QW.QLineEdit(test_url_fetch_panel)
        self._test_referral_url = QW.QLineEdit(test_url_fetch_panel)
        self._fetch_example_data = ClientGUICommon.BetterButton(
            test_url_fetch_panel, 'fetch test data from url',
            self._FetchExampleData)
        self._test_network_job_control = ClientGUIControls.NetworkJobControl(
            test_url_fetch_panel)

        if formula is None:

            self._test_panel = TestPanelPageParser(
                test_panel,
                self.GetValue,
                self._string_converter.GetValue,
                test_context=test_context)

        else:

            self._test_panel = TestPanelPageParserSubsidiary(
                test_panel,
                self.GetValue,
                self._string_converter.GetValue,
                self.GetFormula,
        ...
        permitted_content_types = [
            HC.CONTENT_TYPE_URLS, HC.CONTENT_TYPE_MAPPINGS,
            HC.CONTENT_TYPE_HASH, HC.CONTENT_TYPE_TIMESTAMP,
            HC.CONTENT_TYPE_TITLE, HC.CONTENT_TYPE_VETO
        ]

        self._content_parsers = EditContentParsersPanel(
            content_parsers_panel, self._test_panel.GetTestContextForChild,
            permitted_content_types)
        ...
        st = ClientGUICommon.BetterStaticText(
            conversion_panel,
            'If the data this parser gets is wrapped in some quote marks or is otherwise encoded,\nyou can convert it to neat HTML/JSON first with this.'
        )

        conversion_panel.Add(st, CC.FLAGS_EXPAND_PERPENDICULAR)
        conversion_panel.Add(self._string_converter,
                             CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        test_url_fetch_panel.Add(self._fetch_example_data,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        test_url_fetch_panel.Add(self._test_network_job_control,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        edit_notebook.addTab(sub_page_parsers_notebook_panel,
    ...
    def _AddSubPageParser(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit sub page parser',
                frame_key='deeply_nested_dialog') as dlg:

            panel = EditPageParserPanel(
                dlg,
                page_parser,
                formula=formula,
                test_context=self._test_panel.GetTestContextForChild())
    ...
    def _FetchExampleData(self):
        def wait_and_do_it(network_job):
            def qt_tidy_up(example_data):
                ...
                self._test_panel.SetExampleParsingContext(
        ...
        network_job = ClientNetworkingJobs.NetworkJob(
            'GET', url, referral_url=referral_url)
    ...
    def GetValue(self):
        ...
        parser = ClientParsing.PageParser(
            name,
            parser_key=parser_key,
            string_converter=string_converter,
            sub_page_parsers=sub_page_parsers,
...
class EditParsersPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, parsers):
        ...
        self._parsers = ClientGUIListCtrl.BetterListCtrl(
            parsers_panel,
            'parsers',
            20,
            24,
            columns,
            self._ConvertParserToListCtrlTuples,
            use_simple_delete=True,
            activation_callback=self._Edit)
        ...
        parsers_panel.AddButton('edit',
                                self._Edit,
                                enabled_only_on_selection=True)
        parsers_panel.AddDeleteButton()
        parsers_panel.AddSeparator()
        parsers_panel.AddImportExportButtons((ClientParsing.PageParser, ),
                                             self._AddParser)
        parsers_panel.AddSeparator()
        parsers_panel.AddDefaultsButton(ClientDefaults.GetDefaultParsers,
                                        self._AddParser)
        ...
        for parser in parsers:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HydrusSerialisable.SetNonDupeName(edited_parser,
                                                      self._GetExistingNames())
...
class EditParsingScriptFileLookupPanel(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, script):
        ...
        (name, url, query_type, file_identifier_type,
         file_identifier_string_converter, file_identifier_arg_name,
         static_args, children) = script.ToTuple()
        ...
        for t in [
                ClientParsing.FILE_IDENTIFIER_TYPE_FILE,
                ClientParsing.FILE_IDENTIFIER_TYPE_MD5,
                ClientParsing.FILE_IDENTIFIER_TYPE_SHA1,
                ClientParsing.FILE_IDENTIFIER_TYPE_SHA256,
                ClientParsing.FILE_IDENTIFIER_TYPE_SHA512,
                ClientParsing.FILE_IDENTIFIER_TYPE_USER_INPUT
        ]:

            self._file_identifier_type.addItem(
                ClientParsing.file_identifier_string_lookup[t], t)

        self._file_identifier_string_converter = ClientGUIControls.StringConverterButton(
            query_panel, file_identifier_string_converter)

        self._file_identifier_arg_name = QW.QLineEdit(query_panel)

        static_args_panel = ClientGUICommon.StaticBox(query_panel,
                                                      'static arguments')

        self._static_args = ClientGUIControls.StringToStringDictControl(
            static_args_panel, static_args, min_height=4)
        ...
        self._test_arg.setText(
            'enter example file path, hex hash, or raw user input here')
        ...
        self._test_parsing = QW.QPushButton(
            'test parse (note if you have \'link\' nodes, they will make their requests)',
            test_panel)
...
And pass that html to a number of 'parsing children' that will each look through it in turn and try to find tags.'''
        ...
        self._results.setPlainText(
            'Successfully parsed results will be printed here.')
        ...
        rows.append(('file identifier conversion (typically to hex): ',
                     self._file_identifier_string_converter))
        rows.append(('file identifier GET/POST argument name: ',
                     self._file_identifier_arg_name))
        ...
        query_panel.Add(QW.QLabel(query_message, query_panel),
                        CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        children_panel.Add(QW.QLabel(children_message, children_panel),
                           CC.FLAGS_EXPAND_PERPENDICULAR)
        children_panel.Add(self._children, CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(vbox, self._test_script_management,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def EventFetchData(self):
        ...
        try:
            ...
            try:
                ...
                self._example_data.setPlainText(
                    'The fetched data, which had length ' +
                    HydrusData.ToHumanBytes(len(parsing_text)) +
                    ', did not appear to be displayable text.')
    ...
    def EventTestParse(self):
        def qt_code(results):
            ...
            result_lines = [
                '*** ' + HydrusData.ToHumanInt(len(results)) +
                ' RESULTS BEGIN ***'
            ]

            result_lines.extend(
                (ClientParsing.ConvertParseResultToPrettyString(result)
    ...
    def GetValue(self):
        ...
        script = ClientParsing.ParseRootFileLookup(
            name,
            url=url,
            query_type=query_type,
            file_identifier_type=file_identifier_type,
            file_identifier_string_converter=file_identifier_string_converter,
            file_identifier_arg_name=file_identifier_arg_name,
...
class ManageParsingScriptsPanel(ClientGUIScrolledPanels.ManagePanel):
    ...
    def __init__(self, parent):
        ...
        self._scripts = ClientGUIListCtrl.BetterListCtrl(
            self,
            'parsing_scripts',
            20,
            20, [('name', 20), ('query type', 11), ('script type', 11),
                 ('produces', -1)],
            self._ConvertScriptToTuples,
            delete_key_callback=self.Delete,
            activation_callback=self.Edit)

        menu_items = []

        menu_items.append(('normal', 'file lookup script',
                           'A script that fetches content for a known file.',
                           self.AddFileLookupScript))
        ...
        menu_items.append(
            ('normal', 'to clipboard',
             'Serialise the script and put it on your clipboard.',
             self.ExportToClipboard))
        menu_items.append((
            'normal', 'to png',
            'Serialise the script and encode it to an image file you can easily share with other hydrus users.',
            self.ExportToPng))

        self._export_button = ClientGUICommon.MenuButton(
            self, 'export', menu_items)

        menu_items = []

        menu_items.append(('normal', 'from clipboard',
                           'Load a script from text in your clipboard.',
                           self.ImportFromClipboard))
        menu_items.append(
            ('normal', 'from png', 'Load a script from an encoded png.',
             self.ImportFromPng))

        self._import_button = ClientGUICommon.MenuButton(
            self, 'import', menu_items)
        ...
        for script_type in self.SCRIPT_TYPES:
            ...
            scripts.extend(
                HG.client_controller.Read('serialisable_named', script_type))
    ...
    def _ConvertScriptToTuples(self, script):
        ...
        return ((name, query_type, script_type, produces),
                (name, query_type, script_type, produces))
    ...
    def AddFileLookupScript(self):
        ...
        file_identifier_string_converter = ClientParsing.StringConverter(
            ((ClientParsing.STRING_TRANSFORMATION_ENCODE, 'hex'), ),
            'some hash bytes')
        ...
        empty_script = ClientParsing.ParseRootFileLookup(
            name,
            url=url,
            query_type=query_type,
            file_identifier_type=file_identifier_type,
    ...
    def CommitChanges(self):
        ...
        HG.client_controller.Write('serialisables_overwrite',
                                   self.SCRIPT_TYPES, scripts)
    ...
    def ImportFromPng(self):
        ...
        with QP.FileDialog(self,
                           'select the png with the encoded script',
...
class ScriptManagementControl(QW.QWidget):
    def __init__(self, parent):
        ...
        self._link_button = ClientGUICommon.BetterBitmapButton(
            main_panel, CC.GlobalPixmaps.link, self.LinkButton)
        self._link_button.setToolTip('urls found by the script')

        self._cancel_button = ClientGUICommon.BetterBitmapButton(
            main_panel, CC.GlobalPixmaps.stop, self.CancelButton)
    ...
    def LinkButton(self):
        ...
        for url in urls:
            ...
            ClientGUIMenus.AppendMenuItem(menu, url,
...
class TestPanel(QW.QWidget):
    def __init__(self, parent, object_callable, test_context=None):
        ...
        self._example_parsing_context = ClientGUIControls.StringToStringDictButton(
            self, 'edit example parsing context')

        self._data_preview_notebook = QW.QTabWidget(self)

        raw_data_panel = QW.QWidget(self._data_preview_notebook)

        self._example_data_raw_description = ClientGUICommon.BetterStaticText(
            raw_data_panel)

        self._copy_button = ClientGUICommon.BetterBitmapButton(
            raw_data_panel, CC.GlobalPixmaps.copy, self._Copy)
        self._copy_button.setToolTip(
            'Copy the current example data to the clipboard.')

        self._fetch_button = ClientGUICommon.BetterBitmapButton(
            raw_data_panel, CC.GlobalPixmaps.link, self._FetchFromURL)
        self._fetch_button.setToolTip('Fetch data from a URL.')

        self._paste_button = ClientGUICommon.BetterBitmapButton(
            raw_data_panel, CC.GlobalPixmaps.paste, self._Paste)
        self._paste_button.setToolTip(
            'Paste the current clipboard data into here.')

        self._example_data_raw_preview = QW.QPlainTextEdit(raw_data_panel)
        self._example_data_raw_preview.setReadOnly(True)

        (width, height) = ClientGUIFunctions.ConvertTextToPixels(
            self._example_data_raw_preview, (60, 9))

        self._example_data_raw_preview.setMinimumWidth(width)
        self._example_data_raw_preview.setMinimumHeight(height)

        self._test_parse = ClientGUICommon.BetterButton(
            self, 'test parse', self.TestParse)
        ...
        QP.AddToLayout(hbox, self._example_data_raw_description,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(vbox, self._example_data_raw_preview,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(vbox, self._example_parsing_context,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._data_preview_notebook,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(vbox, self._test_parse, CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._results, CC.FLAGS_EXPAND_BOTH_WAYS)
    ...
    def _SetExampleData(self, example_data):
        ...
        if len(example_data) > 0:
            ...
            description = HydrusData.ToHumanBytes(
                len(example_data)) + ' total, ' + parse_phrase
    ...
    def TestParse(self):
        ...
        try:
            ...
            results_text = obj.ParsePretty(example_parsing_context,
                                           example_data)
            ...
            message = 'Exception:' + os.linesep + str(
                etype.__name__) + ': ' + str(e) + os.linesep + trace
...
class TestPanelPageParser(TestPanel):
    def __init__(self,
                 parent,
                 object_callable,
                 pre_parsing_conversion_callable,
                 test_context=None):
        ...
        self._example_data_post_conversion_description = ClientGUICommon.BetterStaticText(
            post_conversion_panel)

        self._copy_button_post_conversion = ClientGUICommon.BetterBitmapButton(
            post_conversion_panel, CC.GlobalPixmaps.copy,
            self._CopyPostConversion)
        self._copy_button_post_conversion.setToolTip(
            'Copy the current post conversion data to the clipboard.')

        self._refresh_post_conversion_button = ClientGUICommon.BetterBitmapButton(
            post_conversion_panel, CC.GlobalPixmaps.refresh,
            self._RefreshDataPreviews)
        self._example_data_post_conversion_preview = QW.QPlainTextEdit(
            post_conversion_panel)
        ...
        QP.AddToLayout(hbox, self._example_data_post_conversion_description,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(hbox, self._copy_button_post_conversion,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._refresh_post_conversion_button,
                       CC.FLAGS_VCENTER)

        vbox = QP.VBoxLayout()

        QP.AddToLayout(vbox, hbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, self._example_data_post_conversion_preview,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        self._data_preview_notebook.addTab(post_conversion_panel,
                                           'post pre-parsing conversion')
    ...
    def _SetExampleData(self, example_data):
        ...
        if pre_parsing_conversion.MakesChanges():
            ...
            try:
                ...
                post_conversion_example_data = ClientParsing.MakeParsedTextPretty(
                    pre_parsing_conversion.Convert(self._example_data_raw))

                if len(post_conversion_example_data) > 1024:

                    preview = 'PREVIEW:' + os.linesep + str(
                        post_conversion_example_data[:1024])
                ...
                description = HydrusData.ToHumanBytes(
                    len(post_conversion_example_data)
                ) + ' total, ' + parse_phrase
...
class TestPanelPageParserSubsidiary(TestPanelPageParser):
    def __init__(self,
        ...
        TestPanelPageParser.__init__(self,
                                     parent,
                                     object_callable,
                                     pre_parsing_conversion_callable,
                                     test_context=test_context)
        ...
        self._example_data_post_separation_description = ClientGUICommon.BetterStaticText(
            post_separation_panel)

        self._copy_button_post_separation = ClientGUICommon.BetterBitmapButton(
            post_separation_panel, CC.GlobalPixmaps.copy,
            self._CopyPostSeparation)
        self._copy_button_post_separation.setToolTip(
            'Copy the current post separation data to the clipboard.')

        self._refresh_post_separation_button = ClientGUICommon.BetterBitmapButton(
            post_separation_panel, CC.GlobalPixmaps.refresh,
            self._RefreshDataPreviews)
        self._example_data_post_separation_preview = QW.QPlainTextEdit(
            post_separation_panel)
        ...
        QP.AddToLayout(hbox, self._example_data_post_separation_description,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(hbox, self._copy_button_post_separation,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._refresh_post_separation_button,
                       CC.FLAGS_VCENTER)

        vbox = QP.VBoxLayout()

        QP.AddToLayout(vbox, hbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, self._example_data_post_separation_preview,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        self._data_preview_notebook.addTab(post_separation_panel,
    ...
    def _CopyPostSeparation(self):
        ...
        HG.client_controller.pub(
            'clipboard', 'text',
            joiner.join(self._example_data_post_separation))
    ...
    def _SetExampleData(self, example_data):
        ...
        if formula is None:
            ...
            try:
                ...
                separation_example_data = formula.Parse(
                    example_parsing_context,
                    self._example_data_post_conversion)
                ...
                description = HydrusData.ToHumanInt(
                    len(separation_example_data)) + ' subsidiary posts parsed'
    ...
    def TestParse(self):
        ...
        try:
            ...
            for post in posts:
                ...
                pretty_text = page_parser.ParsePretty(example_parsing_context,
                                                      post)
