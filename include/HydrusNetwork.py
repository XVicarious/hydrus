...
BYTE_PARAMS = {
    'access_key', 'account_type_key', 'subject_account_key', 'hash',
    'registration_key', 'subject_hash', 'update_hash'
}
...
def GenerateDefaultServiceDictionary(service_type):
    ...
    if service_type in HC.RESTRICTED_SERVICES:
        ...
        if service_type == HC.SERVER_ADMIN:
            ...
            dictionary[
                'server_bandwidth_tracker'] = HydrusNetworking.BandwidthTracker(
                )
            dictionary[
                'server_bandwidth_rules'] = HydrusNetworking.BandwidthRules()
...
def GenerateServiceFromSerialisableTuple(serialisable_info):
    ...
    (service_key_encoded, service_type, name, port,
     dictionary_string) = serialisable_info
    ...
    try:
        ...
        raise HydrusExceptions.InsufficientCredentialsException(
            'Could not decode that service key!')
...
def GetPossiblePermissions(service_type):
    ...
    permissions.append(
        (HC.CONTENT_TYPE_ACCOUNTS,
         [None, HC.PERMISSION_ACTION_CREATE, HC.PERMISSION_ACTION_OVERRULE]))
    permissions.append(
        (HC.CONTENT_TYPE_ACCOUNT_TYPES, [None, HC.PERMISSION_ACTION_OVERRULE]))

    if service_type == HC.FILE_REPOSITORY:

        permissions.append((HC.CONTENT_TYPE_FILES, [
            None, HC.PERMISSION_ACTION_PETITION, HC.PERMISSION_ACTION_CREATE,
            HC.PERMISSION_ACTION_OVERRULE
        ]))

    elif service_type == HC.TAG_REPOSITORY:

        permissions.append((HC.CONTENT_TYPE_MAPPINGS, [
            None, HC.PERMISSION_ACTION_PETITION, HC.PERMISSION_ACTION_CREATE,
            HC.PERMISSION_ACTION_OVERRULE
        ]))
        permissions.append((HC.CONTENT_TYPE_TAG_PARENTS, [
            None, HC.PERMISSION_ACTION_PETITION, HC.PERMISSION_ACTION_OVERRULE
        ]))
        permissions.append((HC.CONTENT_TYPE_TAG_SIBLINGS, [
            None, HC.PERMISSION_ACTION_PETITION, HC.PERMISSION_ACTION_OVERRULE
        ]))
        ...
        permissions.append(
            (HC.CONTENT_TYPE_SERVICES, [None, HC.PERMISSION_ACTION_OVERRULE]))
...
def DumpHydrusArgsToNetworkBytes(args):
    ...
    if 'account' in args:
        ...
        args['account'] = Account.GenerateSerialisableTupleFromAccount(
            args['account'])

    if 'accounts' in args:

        args['accounts'] = list(
            map(Account.GenerateSerialisableTupleFromAccount,
                args['accounts']))

    if 'account_types' in args:

        args['account_types'] = [
            account_type.ToSerialisableTuple()
            for account_type in args['account_types']
        ]

    if 'registration_keys' in args:

        args['registration_keys'] = [
            registration_key.hex()
            for registration_key in args['registration_keys']
        ]
...
def ParseNetworkBytesToParsedHydrusArgs(network_bytes):
    ...
    if 'account' in args:
        ...
        args['account'] = Account.GenerateAccountFromSerialisableTuple(
            args['account'])
    ...
    if 'accounts' in args:
        ...
        args['accounts'] = list(
            map(Account.GenerateAccountFromSerialisableTuple, account_tuples))
    ...
    if 'account_types' in args:
        ...
        args['account_types'] = list(
            map(AccountType.GenerateAccountTypeFromSerialisableTuple,
                account_type_tuples))

    if 'registration_keys' in args:

        args['registration_keys'] = [
            bytes.fromhex(encoded_registration_key)
            for encoded_registration_key in args['registration_keys']
        ]
    ...
    if 'services' in args:
        ...
        args['services'] = list(
            map(GenerateServiceFromSerialisableTuple, service_tuples))
...
def ParseHydrusNetworkGETArgs(requests_args):
    ...
    args = HydrusNetworking.ParseTwistedRequestGETArgs(requests_args,
                                                       INT_PARAMS, BYTE_PARAMS,
                                                       STRING_PARAMS,
                                                       JSON_PARAMS,
    ...
    if 'subject_account_key' in args:
        ...
        args['subject_identifier'] = HydrusData.AccountIdentifier(
...
class Account(object):
    ...
    def _CheckFunctional(self):
        ...
        if self._account_type.HasPermission(HC.CONTENT_TYPE_SERVICES,
                                            HC.PERMISSION_ACTION_OVERRULE):
        ...
        if self._IsBanned():
            ...
            raise HydrusExceptions.InsufficientCredentialsException(
                'This account is banned: ' + self._GetBannedString())

        if self._IsExpired():

            raise HydrusExceptions.InsufficientCredentialsException(
                'This account is expired: ' + self._GetExpiresString())

        if not self._account_type.BandwidthOK(self._bandwidth_tracker):

            raise HydrusExceptions.BandwidthException(
                'account has exceeded bandwidth')
    ...
    def _GetBannedString(self):
        ...
        if self._banned_info is None:
            ...
            return 'banned ' + HydrusData.TimestampToPrettyTimeDelta(
                created) + ', ' + HydrusData.ConvertTimestampToPrettyExpires(
    ...
    def GetBandwidthStringsAndGaugeTuples(self):
        ...
        with self._lock:
            ...
            return self._account_type.GetBandwidthStringsAndGaugeTuples(
    ...
    def ToString(self):
        ...
        with self._lock:
            ...
            return self._account_type.GetTitle(
            ) + ' -- created ' + HydrusData.TimestampToPrettyTimeDelta(
                self._created)
    ...
    def ToTuple(self):
        ...
        with self._lock:
            ...
            return (self._account_key, self._account_type, self._created,
                    self._expires, self._banned_info, self._bandwidth_tracker)
    ...
    def GenerateAccountFromSerialisableTuple(serialisable_info):
        ...
        (account_key_encoded, account_type_serialisable_tuple, created,
         expires, dictionary_string) = serialisable_info

        account_key = bytes.fromhex(account_key_encoded)
        account_type = AccountType.GenerateAccountTypeFromSerialisableTuple(
            account_type_serialisable_tuple)
        dictionary = HydrusSerialisable.CreateFromString(dictionary_string)

        return Account.GenerateAccountFromTuple(
            (account_key, account_type, created, expires, dictionary))

    @staticmethod
    def GenerateAccountFromTuple(serialisable_info):

        (account_key, account_type, created, expires,
         dictionary) = serialisable_info

        banned_info = dictionary['banned_info']
        bandwidth_tracker = dictionary['bandwidth_tracker']

        return Account(account_key, account_type, created, expires,
                       banned_info, bandwidth_tracker)

    @staticmethod
    def GenerateSerialisableTupleFromAccount(account):

        (account_key, account_type, created, expires,
         dictionary) = Account.GenerateTupleFromAccount(account)
        ...
        return (account_key_encoded, serialisable_account_type, created,
                expires, dictionary_string)

    @staticmethod
    def GenerateTupleFromAccount(account):

        (account_key, account_type, created, expires, banned_info,
         bandwidth_tracker) = account.ToTuple()
...
class AccountType(object):
    ...
    def HasPermission(self, content_type, permission):
        ...
        if permission == HC.PERMISSION_ACTION_OVERRULE:
            ...
            return my_permission in (HC.PERMISSION_ACTION_CREATE,
                                     HC.PERMISSION_ACTION_OVERRULE)

        elif permission == HC.PERMISSION_ACTION_PETITION:

            return my_permission in (HC.PERMISSION_ACTION_PETITION,
    ...
    def GetBandwidthStringsAndGaugeTuples(self, bandwidth_tracker):
        ...
        return self._bandwidth_rules.GetBandwidthStringsAndGaugeTuples(
            bandwidth_tracker)
    ...
    def ToTuple(self):
        ...
        return (self._account_type_key, self._title, self._permissions,
                self._bandwidth_rules)

    @staticmethod
    def GenerateAccountTypeFromParameters(account_type_key, title, permissions,
                                          bandwidth_rules):
    ...
    def GenerateAccountTypeFromSerialisableTuple(serialisable_info):
        ...
        (account_type_key_encoded, title,
         dictionary_string) = serialisable_info
        ...
        try:
            ...
            raise HydrusExceptions.InsufficientCredentialsException(
                'Could not decode that account type key!')
    ...
    def GenerateAdminAccountType(service_type):
        ...
        if service_type in HC.REPOSITORIES:
            ...
            raise NotImplementedError(
                'Do not have a default admin account type set up for this service yet!'
            )

        account_type = AccountType.GenerateNewAccountTypeFromParameters(
            'administrator', permissions, bandwidth_rules)

        return account_type

    @staticmethod
    def GenerateNewAccountTypeFromParameters(title, permissions,
                                             bandwidth_rules):

        account_type_key = HydrusData.GenerateKey()

        return AccountType.GenerateAccountTypeFromParameters(
            account_type_key, title, permissions, bandwidth_rules)
    ...
    def GenerateUnknownAccountType():
        ...
        unknown_account_type = AccountType.GenerateNewAccountTypeFromParameters(
...
class ClientToServerUpdate(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        for (action, contents_and_reasons) in list(
                self._actions_to_contents_and_reasons.items()):

            serialisable_contents_and_reasons = [
                (content.GetSerialisableTuple(), reason)
                for (content, reason) in contents_and_reasons
            ]

            serialisable_info.append(
                (action, serialisable_contents_and_reasons))
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        for (action, serialisable_contents_and_reasons) in serialisable_info:
            ...
            contents_and_reasons = [
                (HydrusSerialisable.CreateFromSerialisableTuple(
                    serialisable_content), reason)
                for (serialisable_content,
                     reason) in serialisable_contents_and_reasons
            ]

            self._actions_to_contents_and_reasons[
                action] = contents_and_reasons
    ...
    def GetClientsideContentUpdates(self):
        ...
        for (action, clientside_action) in ((HC.CONTENT_UPDATE_PEND,
                                             HC.CONTENT_UPDATE_ADD),
                                            (HC.CONTENT_UPDATE_PETITION,
                                             HC.CONTENT_UPDATE_DELETE)):

            for (content,
                 reason) in self._actions_to_contents_and_reasons[action]:

                content_type = content.GetContentType()
                content_data = content.GetContentData()

                content_update = HydrusData.ContentUpdate(
                    content_type, clientside_action, content_data)
    ...
    def GetHashes(self):
        ...
        for contents_and_reasons in list(
                self._actions_to_contents_and_reasons.values()):
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_CLIENT_TO_SERVER_UPDATE] = ClientToServerUpdate
...
class Content(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        elif self._content_type in (HC.CONTENT_TYPE_TAG_PARENTS,
    ...
    def GetVirtualWeight(self):
        ...
        if self._content_type in (HC.CONTENT_TYPE_FILES,
    ...
    def HasHashes(self):
        ...
        return self._content_type in (HC.CONTENT_TYPE_FILES,
                                      HC.CONTENT_TYPE_MAPPING,
                                      HC.CONTENT_TYPE_MAPPINGS)
    ...
    def IterateUploadableChunks(self):
        ...
        if self._content_type == HC.CONTENT_TYPE_FILES:
            ...
            for chunk_of_hashes in HydrusData.SplitListIntoChunks(hashes, 100):
                ...
                content = Content(content_type=self._content_type,
                                  content_data=chunk_of_hashes)
            ...
            for chunk_of_hashes in HydrusData.SplitListIntoChunks(hashes, 100):
                ...
                content = Content(content_type=self._content_type,
                                  content_data=(tag, chunk_of_hashes))
    ...
    def ToString(self):
        ...
        if self._content_type == HC.CONTENT_TYPE_FILES:
            ...
            text = 'MAPPINGS: ' + tag + ' for ' + HydrusData.ToHumanInt(
                len(hashes)) + ' files'
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_CONTENT] = Content
...
class ContentUpdate(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        for (content_type,
             actions_to_datas) in list(self._content_data.items()):

            serialisable_actions_to_datas = list(actions_to_datas.items())

            serialisable_info.append(
                (content_type, serialisable_actions_to_datas))
    ...
    def GetDeletedFiles(self):
        ...
        return self._GetContent(HC.CONTENT_TYPE_FILES,
                                HC.CONTENT_UPDATE_DELETE)

    def GetDeletedMappings(self):

        return self._GetContent(HC.CONTENT_TYPE_MAPPINGS,
                                HC.CONTENT_UPDATE_DELETE)

    def GetDeletedTagParents(self):

        return self._GetContent(HC.CONTENT_TYPE_TAG_PARENTS,
                                HC.CONTENT_UPDATE_DELETE)

    def GetDeletedTagSiblings(self):

        return self._GetContent(HC.CONTENT_TYPE_TAG_SIBLINGS,
                                HC.CONTENT_UPDATE_DELETE)
        ...
        for content_type in self._content_data:
            ...
            for action in self._content_data[content_type]:
                ...
                if content_type == HC.CONTENT_TYPE_MAPPINGS:
                    ...
                    num_rows = sum(
                        (len(hash_ids) for (tag_id, hash_ids) in data))
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_CONTENT_UPDATE] = ContentUpdate
...
class Credentials(HydrusSerialisable.SerialisableBase):
    ...
    def GenerateCredentialsFromConnectionString(connection_string):
        ...
        (host, port,
         access_key) = Credentials.ParseConnectionString(connection_string)
    ...
    def ParseConnectionString(connection_string):
        ...
        if '@' in connection_string:
            ...
            try:
                ...
                raise HydrusExceptions.DataMissing(
                    'Could not parse that access key! It should be a 64 character hexadecimal string!'
                )
        ...
        if ':' in connection_string:
            ...
            try:
                ...
                raise HydrusExceptions.DataMissing(
                    'Could not parse that port! It should be an integer between 0 and 65535!'
                )
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_CREDENTIALS] = Credentials
...
class DefinitionsUpdate(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        if len(self._hash_ids_to_hashes) > 0:
            ...
            serialisable_info.append((HC.DEFINITIONS_TYPE_HASHES, [
                (hash_id, hash.hex())
                for (hash_id, hash) in list(self._hash_ids_to_hashes.items())
            ]))

        if len(self._tag_ids_to_tags) > 0:

            serialisable_info.append((HC.DEFINITIONS_TYPE_TAGS,
                                      list(self._tag_ids_to_tags.items())))
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        for (definition_type, definitions) in serialisable_info:
            ...
            if definition_type == HC.DEFINITIONS_TYPE_HASHES:
                ...
                self._hash_ids_to_hashes = {
                    hash_id: bytes.fromhex(encoded_hash)
                    for (hash_id, encoded_hash) in definitions
                }
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_DEFINITIONS_UPDATE] = DefinitionsUpdate
...
class Metadata(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_metadata = [
            (update_index,
             [update_hash.hex() for update_hash in update_hashes], begin, end)
            for (update_index, (update_hashes, begin,
                                end)) in list(self._metadata.items())
        ]
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        for (update_index, encoded_update_hashes, begin,
             end) in serialisable_metadata:

            update_hashes = [
                bytes.fromhex(encoded_update_hash)
                for encoded_update_hash in encoded_update_hashes
            ]
    ...
    def GetNextUpdateBegin(self):
        ...
        with self._lock:
            ...
            if len(keys) == 0:
                ...
                (update_hashes, begin,
                 end) = self._GetUpdate(largest_update_index)
    ...
    def GetSlice(self, from_update_index):
        ...
        with self._lock:
            ...
            metadata = {
                update_index: row
                for (update_index, row) in list(self._metadata.items())
                if update_index >= from_update_index
            }
    ...
    def GetUpdateHashes(self, update_index=None):
        ...
        with self._lock:
            ...
            if update_index is None:
                ...
                for (update_hashes, begin,
                     end) in list(self._metadata.values()):
    ...
    def GetUpdateInfo(self, from_client=False):
        ...
        with self._lock:
            ...
            if len(self._metadata) == 0:
                ...
                status = 'metadata synchronised up to ' + HydrusData.TimestampToPrettyTimeDelta(
                    biggest_end) + ', next update due ' + s
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_METADATA] = Metadata
...
class Petition(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_petitioner_account = Account.GenerateSerialisableTupleFromAccount(
            self._petitioner_account)
        serialisable_contents = [
            content.GetSerialisableTuple() for content in self._contents
        ]

        return (self._action, serialisable_petitioner_account, self._reason,
                serialisable_contents)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._action, serialisable_petitioner_account, self._reason,
         serialisable_contents) = serialisable_info

        self._petitioner_account = Account.GenerateAccountFromSerialisableTuple(
            serialisable_petitioner_account)
        self._contents = [
            HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_content)
            for serialisable_content in serialisable_contents
        ]
    ...
    def GetApproval(self, contents):
        ...
        for content in contents:
            ...
            content_update = HydrusData.ContentUpdate(content_type,
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_PETITION] = Petition
...
class ServerService(object):
    ...
    def Duplicate(self):
        ...
        with self._lock:
            ...
            duplicate = GenerateService(self._service_key, self._service_type,
                                        self._name, self._port, dictionary)
    ...
    def ToSerialisableTuple(self):
        ...
        with self._lock:
            ...
            return (self._service_key.hex(), self._service_type, self._name,
                    self._port, dictionary_string)
    ...
    def ToTuple(self):
        ...
        with self._lock:
            ...
            return (self._service_key, self._service_type, self._name,
                    self._port, dictionary)
        ...
        if update_due:
            ...
            try:
                ...
                while update_due:
                    ...
                    update_hashes = HG.server_controller.WriteSynchronous(
                        'create_update', service_key, begin, end)

                    next_update_due = end + HC.UPDATE_DURATION + 1

                    with self._lock:

                        self._metadata.AppendUpdate(update_hashes, begin, end,
                                                    next_update_due)
...
class ServerServiceAdmin(ServerServiceRestricted):
    ...
    def ServerBandwidthOK(self):
        ...
        with self._lock:
            ...
            return self._server_bandwidth_rules.CanStartRequest(
                self._server_bandwidth_tracker)
