...
from . import (ClientData, ClientGUICommon, ClientGUIControls,
               ClientGUIFunctions, ClientGUIOptionsPanels,
               ClientGUIScrolledPanels, ClientGUIShortcuts, ClientGUITime,
               ClientRatings, ClientSearch)
...
class InputFileSystemPredicate(ClientGUIScrolledPanels.EditPanel):
    def __init__(self, parent, predicate_type):
        ...
        if predicate_type == HC.PREDICATE_TYPE_SYSTEM_AGE:
            ...
            editable_pred_panel_classes.append(
                PanelPredicateSystemModifiedDelta)
            editable_pred_panel_classes.append(
                PanelPredicateSystemModifiedDate)
            ...
            static_pred_buttons.append(
                StaticSystemPredicateButton(
                    self,
                    (ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_DURATION,
                                            ('>', 0)), )))
            static_pred_buttons.append(
                StaticSystemPredicateButton(
                    self,
            ...
            editable_pred_panel_classes.append(
                PanelPredicateSystemKnownURLsExactURL)
            editable_pred_panel_classes.append(
                PanelPredicateSystemKnownURLsDomain)
            editable_pred_panel_classes.append(
                PanelPredicateSystemKnownURLsRegex)
            editable_pred_panel_classes.append(
                PanelPredicateSystemKnownURLsURLClass)

        elif predicate_type == HC.PREDICATE_TYPE_SYSTEM_HAS_AUDIO:

            static_pred_buttons.append(
                StaticSystemPredicateButton(self, (ClientSearch.Predicate(
                    HC.PREDICATE_TYPE_SYSTEM_HAS_AUDIO, True), )))
            static_pred_buttons.append(
                StaticSystemPredicateButton(self, (ClientSearch.Predicate(
                    HC.PREDICATE_TYPE_SYSTEM_HAS_AUDIO, False), )))
            ...
            static_pred_buttons.append(
                StaticSystemPredicateButton(self, (ClientSearch.Predicate(
                    HC.PREDICATE_TYPE_SYSTEM_LIMIT, 64), )))
            static_pred_buttons.append(
                StaticSystemPredicateButton(self, (ClientSearch.Predicate(
                    HC.PREDICATE_TYPE_SYSTEM_LIMIT, 256), )))
            static_pred_buttons.append(
                StaticSystemPredicateButton(self, (ClientSearch.Predicate(
                    HC.PREDICATE_TYPE_SYSTEM_LIMIT, 1024), )))
            ...
            editable_pred_panel_classes.append(
                PanelPredicateSystemDuplicateKing)
        ...
        if len(static_pred_buttons) > 0 and len(
                editable_pred_panel_classes) == 0:

            QP.CallAfter(static_pred_buttons[0].setFocus,
...
class StaticSystemPredicateButton(QW.QPushButton):
    def __init__(self, parent, predicates):
        ...
        label = ', '.join(
            (predicate.ToString() for predicate in self._predicates))
...
class PanelPredicateSystemAgeDate(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self,
                                                   'system:time imported'),
            CC.FLAGS_VCENTER)
...
class PanelPredicateSystemAgeDelta(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(hbox, ClientGUICommon.BetterStaticText(self, 'years'),
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._months, CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, ClientGUICommon.BetterStaticText(self, 'months'),
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._days, CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, ClientGUICommon.BetterStaticText(self, 'days'),
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._hours, CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, ClientGUICommon.BetterStaticText(self, 'hours'),
                       CC.FLAGS_VCENTER)
    ...
    def GetInfo(self):
        ...
        info = (self._sign.GetStringSelection(), 'delta',
                (self._years.value(), self._months.value(), self._days.value(),
                 self._hours.value()))
...
class PanelPredicateSystemDuplicateKing(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        choices = [
            'is the best quality file of its group',
            'is not the best quality file of its group'
        ]
        ...
        QP.AddToLayout(hbox, ClientGUICommon.BetterStaticText(self, 'system:'),
                       CC.FLAGS_VCENTER)
...
class PanelPredicateSystemDuplicateRelationships(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        choices = [
            ...
            for status in (HC.DUPLICATE_MEMBER, HC.DUPLICATE_ALTERNATE,
                           HC.DUPLICATE_FALSE_POSITIVE, HC.DUPLICATE_POTENTIAL)
        ]

        self._dupe_type = ClientGUICommon.BetterRadioBox(self,
                                                         choices=choices,
                                                         vertical=True)
    ...
    def GetInfo(self):
        ...
        info = (self._sign.GetStringSelection(), self._num.value(),
                self._dupe_type.GetValue())
...
class PanelPredicateSystemDuration(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self, 'system:duration'),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._sign, CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._duration_s, CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, ClientGUICommon.BetterStaticText(self, 's'),
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._duration_ms, CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, ClientGUICommon.BetterStaticText(self, 'ms'),
                       CC.FLAGS_VCENTER)
    ...
    def GetInfo(self):
        ...
        info = (self._sign.GetStringSelection(),
                self._duration_s.value() * 1000 + self._duration_ms.value())
...
class PanelPredicateSystemFileService(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        self._file_service_key = ClientGUICommon.BetterRadioBox(
            self, choices=choices, vertical=True)
    ...
    def GetInfo(self):
        ...
        info = (self._sign.GetValue(), self._current_pending.GetValue(),
                self._file_service_key.GetValue())
...
class PanelPredicateSystemFileViewingStatsViewtime(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        self._time_delta = ClientGUITime.TimeDeltaCtrl(self,
                                                       min=0,
                                                       days=True,
                                                       hours=True,
                                                       minutes=True,
...
class PanelPredicateSystemHash(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        init_size = ClientGUIFunctions.ConvertTextToPixels(
            self._hashes, (66, 10))
        ...
        self._hashes.setPlaceholderText(
            'enter hash (paste newline-separated for multiple hashes)')

        hbox = QP.HBoxLayout()

        QP.AddToLayout(hbox,
                       ClientGUICommon.BetterStaticText(self, 'system:hash='),
                       CC.FLAGS_VCENTER)
    ...
    def GetInfo(self):
        ...
        hex_hashes = [
            HydrusText.HexFilter(hex_hash) for hex_hash in hex_hashes
        ]
...
class PanelPredicateSystemHeight(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(hbox,
                       ClientGUICommon.BetterStaticText(self, 'system:height'),
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._sign, CC.FLAGS_VCENTER)
...
class PanelPredicateSystemKnownURLsExactURL(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self, 'system:known url'),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._operator, CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox,
                       ClientGUICommon.BetterStaticText(self, 'exact url:'),
                       CC.FLAGS_VCENTER)
...
class PanelPredicateSystemKnownURLsDomain(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(hbox, self._operator, CC.FLAGS_VCENTER)
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self, 'a url with domain:'),
            CC.FLAGS_VCENTER)
...
class PanelPredicateSystemLimit(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(hbox,
                       ClientGUICommon.BetterStaticText(self, 'system:limit='),
                       CC.FLAGS_VCENTER)
...
class PanelPredicateSystemMime(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self, 'system:filetype'),
            CC.FLAGS_VCENTER)
...
class PanelPredicateSystemNumPixels(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        self._unit = QP.RadioBox(
            self, choices=['pixels', 'kilopixels', 'megapixels'])
        ...
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self, 'system:num_pixels'),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._sign, CC.FLAGS_VCENTER)
    ...
    def GetInfo(self):
        ...
        info = (self._sign.GetStringSelection(), self._num_pixels.value(),
                HydrusData.ConvertPixelsToInt(self._unit.GetStringSelection()))
...
class PanelPredicateSystemNumTags(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self, 'system:num_tags'),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._sign, CC.FLAGS_VCENTER)
...
class PanelPredicateSystemNumWords(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self, 'system:num_words'),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._sign, CC.FLAGS_VCENTER)
...
class PanelPredicateSystemRating(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        for service in local_like_services:
            ...
            QP.AddToLayout(gridbox,
                           ClientGUICommon.BetterStaticText(self, name),
                           CC.FLAGS_VCENTER)
    ...
    def GetInfo(self):
        ...
        for (checkbox,
             (service_key,
              rating_state)) in list(self._like_checkboxes_to_info.items()):
        ...
        for (checkbox, (service_key, rating_state)) in list(
                self._numerical_checkboxes_to_info.items()):
        ...
        for (ctrl,
             choice) in list(self._numerical_rating_ctrls_to_info.items()):
    ...
    def GetPredicates(self):
        ...
        predicates = [
            ClientSearch.Predicate(self.PREDICATE_TYPE, info) for info in infos
        ]
...
class PanelPredicateSystemRatio(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        self._sign = QP.RadioBox(
            self, choices=['=', 'wider than', 'taller than', '\u2248'])
        ...
        QP.AddToLayout(hbox,
                       ClientGUICommon.BetterStaticText(self, 'system:ratio'),
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._sign, CC.FLAGS_VCENTER)
...
class PanelPredicateSystemSimilarTo(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self, 'system:similar_to'),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._hashes, CC.FLAGS_VCENTER)
...
class PanelPredicateSystemSize(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(self, 'system:filesize'),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._sign, CC.FLAGS_VCENTER)
...
class PanelPredicateSystemTagAsNumber(PanelPredicateSystem):
    ...
    def GetInfo(self):
        ...
        info = (self._namespace.text(), self._sign.GetStringSelection(),
                self._num.value())
...
class PanelPredicateSystemWidth(PanelPredicateSystem):
    ...
    def __init__(self, parent):
        ...
        QP.AddToLayout(hbox,
                       ClientGUICommon.BetterStaticText(self, 'system:width'),
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._sign, CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._width, CC.FLAGS_VCENTER)
