...
from . import (ClientData, ClientDragDrop, ClientGUICommon, ClientGUIFunctions,
               ClientGUIShortcuts, ClientSerialisable, HydrusData,
               HydrusExceptions)
...
class BetterListCtrl(QW.QTreeWidget):
    ...
    def __init__(self,
        ...
        sizing_column_initial_width = self.fontMetrics().boundingRect(
            'x' * sizing_column_initial_width_num_chars).width()
        total_width = self.fontMetrics().boundingRect(
            'x' * sizing_column_initial_width_num_chars).width()
        ...
        for (i, (name, width_num_chars)) in enumerate(columns):
            ...
            if width_num_chars == -1:
                ...
                width = self.fontMetrics().boundingRect(
                    'x' * width_num_chars).width()
        ...
        self.header().setSectionResizeMode(resize_column - 1,
    ...
    def _SortDataInfo(self):
        ...
        def sort_key(data_info):
            ...
            return (sort_tuple[self._sort_column], sort_tuple
    ...
    def GrowShrinkColumnsHeight(self, ideal_rows):
        ...
        (width_gumpf,
         ideal_client_height) = ClientGUIFunctions.ConvertTextToPixels(
    ...
    def UpdateDatas(self, datas=None):
        ...
        if datas is None:
            ...
            indices_and_datas = [(index, data)
                                 for (data,
                                      index) in self._data_to_indices.items()]
        ...
        for data in datas:
            ...
            if data_info != existing_data_info:
                ...
                if not sort_data_has_changed:
                    ...
                    if sort_tuple[self._sort_column] != existing_sort_tuple[
                            self.
                            _sort_column]:  # this does not govern secondary sorts, but let's not spam sorts m8
...
class BetterListCtrlPanel(QW.QWidget):
    ...
    def AddDeleteButton(self, enabled_check_func=None):
        ...
        self.AddButton('delete',
                       self._listctrl.ProcessDeleteAction,
                       enabled_check_func=enabled_check_func,
                       enabled_only_on_selection=enabled_only_on_selection)
    ...
    def AddImportExportButtons(self, permitted_object_types,
        ...
        import_menu_items.append(
            ('normal',
             'from pngs (note you can also drag and drop pngs onto this list)',
             'Load a data from an encoded png.', self._ImportFromPng))

        self.AddMenuButton('export',
                           export_menu_items,
                           enabled_only_on_selection=True)
        self.AddMenuButton('import', import_menu_items)
        self.AddButton('duplicate',
                       self._Duplicate,
                       enabled_only_on_selection=True)
        ...
        self.installEventFilter(
            ClientDragDrop.FileDropTarget(
                self, filenames_callable=self.ImportFromDragDrop))
    ...
    def SetListCtrl(self, listctrl):
        ...
        QP.AddToLayout(self._vbox, self._listctrl,
