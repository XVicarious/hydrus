...
from . import (HydrusImageHandling, HydrusNetworking, HydrusPaths,
               HydrusThreading)
...
regen_file_enum_to_str_lookup[
    REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_PRESENCE] = 'if file is missing, remove record'
regen_file_enum_to_str_lookup[
    REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_PRESENCE_URL] = 'if file is missing and has url, try to redownload'
regen_file_enum_to_str_lookup[
    REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA] = 'if file is missing/incorrect, move file out and remove record'
regen_file_enum_to_str_lookup[
    REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA_URL] = 'if file is missing/incorrect and has url, move file out and try to redownload'
regen_file_enum_to_str_lookup[
...
regen_file_enum_to_job_weight_lookup[
    REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_PRESENCE] = 5
regen_file_enum_to_job_weight_lookup[
    REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_PRESENCE_URL] = 50
regen_file_enum_to_job_weight_lookup[
    REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA] = 100
regen_file_enum_to_job_weight_lookup[
    REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA_URL] = 100
regen_file_enum_to_job_weight_lookup[
...
regen_file_enum_to_overruled_jobs[REGENERATE_FILE_DATA_JOB_FORCE_THUMBNAIL] = [
...
class ClientFilesManager(object):
    ...
    def _AddFile(self, hash, mime, source_path):
        ...
        if HG.file_report_mode or HG.file_import_report_mode:
            ...
            HydrusData.ShowText(
                'Adding file to client file structure: from {} to {}'.format(
        ...
        if not successful:
            ...
            raise Exception('There was a problem copying the file from ' +
    ...
    def _AddThumbnailFromBytes(self, hash, thumbnail_bytes, silent=False):
        ...
        if HG.file_report_mode:
            ...
            HydrusData.ShowText('Adding thumbnail: ' +
                                str((len(thumbnail_bytes), dest_path)))
        ...
        try:
            ...
            if not os.path.exists(thumb_dir):
                ...
                raise HydrusExceptions.DirectoryMissingException(
                    'The directory {} was not found! Reconnect the missing location or shut down the client immediately!'
                    .format(thumb_dir))

            raise HydrusExceptions.FileMissingException(
                'The thumbnail for file "{}" failed to write to path "{}". This event suggests that hydrus does not have permission to write to its thumbnail folder. Please check everything is ok.'
                .format(hash.hex(), dest_path))
    ...
    def _AttemptToHealMissingLocations(self):
        ...
        (locations_to_ideal_weights, thumbnail_override
        ...
        for (missing_location, prefix) in self._missing_locations:
            ...
            if len(potential_correct_locations) == 1:
                ...
                correct_rows.append(
                    (missing_location, prefix, correct_location))
        ...
        if len(correct_rows) > 0 and some_are_unhealable:
            ...
            self._controller.SafeShowCriticalMessage(
                'Multiple file location problems.', message)

        if len(correct_rows) > 0:

            summaries = [
                '{} moved from {} to {}'.format(HydrusData.ToHumanInt(count),
                                                missing_location,
                                                correct_location)
                for ((missing_location, correct_location),
                     count) in fixes_counter.items()
            ]
            ...
            self._controller.SafeShowCriticalMessage(
                'About to auto-heal client file folders.', summary_message)
    ...
    def _ChangeFileExt(self, hash, old_mime, mime):
        ...
        if HG.file_report_mode:
            ...
            HydrusData.ShowText('Changing file ext: ' +
                                str((old_path, new_path)))
    ...
    def _GenerateExpectedFilePath(self, hash, mime):
        ...
        path = os.path.join(location, prefix,
                            hash_encoded + HC.mime_ext_lookup[mime])
    ...
    def _GenerateThumbnailBytes(self, file_path, media):
        ...
        percentage_in = self._controller.new_options.GetInteger(
        ...
        try:
            ...
            raise HydrusExceptions.FileMissingException(
                'The thumbnail for file ' + hash.hex() +
                ' could not be regenerated from the original file for the above reason. This event could indicate hard drive corruption. Please check everything is ok.'
            )
    ...
    def _GetRecoverTuple(self):
        ...
        all_locations = {
            location
            for location in list(self._prefixes_to_locations.values())
        }
        ...
        for possible_location in all_locations:
            ...
            for prefix in all_prefixes:
                ...
                if possible_location != correct_location and os.path.exists(
                        os.path.join(possible_location, prefix)):
    ...
    def _GetRebalanceTuple(self):
        ...
        ideal_locations_to_normalised_weights = {
            location: weight / total_weight
            for (location, weight) in list(locations_to_ideal_weights.items())
        }

        current_locations_to_normalised_weights = collections.defaultdict(
            lambda: 0)

        file_prefixes = [
            prefix for prefix in self._prefixes_to_locations
        ...
        for (location, ideal_weight) in list(
                ideal_locations_to_normalised_weights.items()):

            if location in current_locations_to_normalised_weights:

                current_weight = current_locations_to_normalised_weights[
                    location]
        ...
        if len(underweight_locations) > 0 and len(overweight_locations) > 0:
            ...
            for file_prefix in file_prefixes:
                ...
                if location == overweight_location:
                    ...
                    return (file_prefix, overweight_location,
                            underweight_location)
            ...
            for hex_prefix in HydrusData.IterateHexPrefixes():
                ...
                current_thumbnails_location = self._prefixes_to_locations[
                    thumbnail_prefix]

                if current_thumbnails_location != correct_location:

                    return (thumbnail_prefix, current_thumbnails_location,
                            correct_location)
    ...
    def _LookForFilePath(self, hash):
        ...
        for potential_mime in HC.ALLOWED_MIMES:
            ...
            potential_path = self._GenerateExpectedFilePath(
                hash, potential_mime)
        ...
        raise HydrusExceptions.FileMissingException('File for ' + hash.hex() +
                                                    ' not found!')

    def _Reinit(self):

        self._prefixes_to_locations = self._controller.Read(
        ...
        if HG.client_controller.IsFirstStart():
            ...
            try:
                ...
                for (prefix,
                     location) in list(self._prefixes_to_locations.items()):
                ...
                text = 'Attempting to create the database\'s client_files folder structure in {} failed!'.format(
                    location)

                self._controller.SafeShowCriticalMessage(
                    'unable to create file structure', text)
            ...
            if len(self._missing_locations) > 0:
                ...
                missing_dict = HydrusData.BuildKeyToListDict(
                ...
                for missing_location in missing_locations:
                    ...
                    missing_prefixes_string = '    ' + os.linesep.join(
                        (', '.join(block)
                         for block in HydrusData.SplitListIntoChunks(
                             missing_prefixes, 32)))
    ...
    def AllLocationsAreDefault(self):
        ...
        with self._rwlock.read:
            ...
            return False not in (location.startswith(client_files_default)
    ...
    def LocklessAddFileFromBytes(self, hash, mime, file_bytes):
        ...
        if HG.file_report_mode:
            ...
            HydrusData.ShowText('Adding file from string: ' +
                                str((len(file_bytes), dest_path)))
        ...
        with self._rwlock.write:
            ...
            for (i, path) in enumerate(self._IterateAllFilePaths()):
                ...
                if i % 100 == 0:
                    ...
                    status = 'reviewed ' + HydrusData.ToHumanInt(
                        i) + ' files, found ' + HydrusData.ToHumanInt(
                            len(orphan_paths)) + ' orphans'
                ...
                try:
                    ...
                    is_an_orphan = HG.client_controller.Read(
                        'is_an_orphan', 'file', hash)
                ...
                if is_an_orphan:
                    ...
                    if move_location is not None:
                        ...
                        HydrusData.Print('Moving the orphan ' + path + ' to ' +
                                         dest)
            ...
            for (i, path) in enumerate(self._IterateAllThumbnailPaths()):
                ...
                if i % 100 == 0:
                    ...
                    status = 'reviewed ' + HydrusData.ToHumanInt(
                        i) + ' thumbnails, found ' + HydrusData.ToHumanInt(
                            len(orphan_thumbnails)) + ' orphans'
            ...
            if move_location is None and len(orphan_paths) > 0:
                ...
                status = 'found ' + HydrusData.ToHumanInt(
                    len(orphan_paths)) + ' orphans, now deleting'
                ...
                for path in orphan_paths:
                    ...
                    status = 'deleting orphan files: ' + HydrusData.ConvertValueRangeToPrettyString(
                        i + 1, len(orphan_paths))
            ...
            if len(orphan_thumbnails) > 0:
                ...
                status = 'found ' + HydrusData.ToHumanInt(
                    len(orphan_thumbnails)
                ) + ' orphan thumbnails, now deleting'
                ...
                for (i, path) in enumerate(orphan_thumbnails):
                    ...
                    status = 'deleting orphan thumbnails: ' + HydrusData.ConvertValueRangeToPrettyString(
                        i + 1, len(orphan_thumbnails))
            ...
            if len(orphan_paths) == 0 and len(orphan_thumbnails) == 0:
                ...
                final_text = HydrusData.ToHumanInt(
                    len(orphan_paths)
                ) + ' orphan files and ' + HydrusData.ToHumanInt(
                    len(orphan_thumbnails)) + ' orphan thumbnails cleared!'
    ...
    def DelayedDeleteFiles(self, hashes):
        ...
        if HG.file_report_mode:
            ...
            HydrusData.ShowText('Delayed delete files call: ' +
                                str(len(hashes)))
    ...
    def DelayedDeleteThumbnails(self, hashes):
        ...
        if HG.file_report_mode:
            ...
            HydrusData.ShowText('Delayed delete thumbs call: ' +
                                str(len(hashes)))
    ...
    def GetFilePath(self, hash, mime=None, check_file_exists=True):
        ...
        with self._rwlock.read:
            ...
            return self.LocklessGetFilePath(
                hash, mime=mime, check_file_exists=check_file_exists)
        ...
        try:
            ...
            if self._bad_error_occurred:
                ...
                QW.QMessageBox.warning(
                    None, 'Warning',
                    'A serious file error has previously occurred during this session, so further file moving will not be reattempted. Please restart the client before trying again.'
                )
            ...
            with self._rwlock.write:
                ...
                while rebalance_tuple is not None:
                    ...
                    (prefix, overweight_location,
                     underweight_location) = rebalance_tuple
                    ...
                    self._controller.Write('relocate_client_files', prefix,
                                           overweight_location,
                                           underweight_location)
                ...
                while recover_tuple is not None:
                    ...
                    (prefix, recoverable_location,
                     correct_location) = recover_tuple
                    ...
                    recoverable_path = os.path.join(recoverable_location,
                                                    prefix)
    ...
    def RegenerateThumbnail(self, media):
        ...
        with self._rwlock.read:
            ...
            if not os.path.exists(file_path):
                ...
                raise HydrusExceptions.FileMissingException(
                    'The thumbnail for file ' + hash.hex() +
                    ' could not be regenerated from the original file because the original file is missing! This event could indicate hard drive corruption. Please check everything is ok.'
                )
    ...
    def RegenerateThumbnailIfWrongSize(self, media):
        ...
        try:
            ...
            (current_width, current_height
             ) = HydrusImageHandling.GetResolutionNumPy(numpy_image)

            bounding_dimensions = self._controller.options[
                'thumbnail_dimensions']
...
class FilesMaintenanceManager(object):
    ...
    def _AbleToDoBackgroundMaintenance(self):
        ...
        if HG.client_controller.CurrentlyIdle():
            ...
            if not self._controller.new_options.GetBoolean(
    ...
    def _CheckFileIntegrity(self, media_result, job_type):
        ...
        error_dir = os.path.join(self._controller.GetDBDir(),
                                 'missing_and_invalid_files')
        ...
        try:
            ...
            path = self._controller.client_files_manager.GetFilePath(
                hash, mime)
        ...
        if not file_is_missing and job_type in (
                REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA,
                REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA_URL,
                REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA_SILENT_DELETE):
            ...
            if hash != actual_hash:
                ...
                HydrusData.DebugPrint(
                    'Invalid file: {} actually had hash {}!'.format(
                        hash.hex(), actual_hash.hex()))
        ...
        if file_was_bad:
            ...
            if len(urls) > 0:
                ...
                with open(os.path.join(error_dir, 'all_urls.txt'),
                          'a',
                          encoding='utf-8') as f:
            ...
            for url in urls:
                ...
                if url_class is None:
                    ...
                    if url_class.GetURLType() in (HC.URL_TYPE_FILE,
                                                  HC.URL_TYPE_POST):
            ...
            delete_record = job_type in (
                REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_PRESENCE,
                REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA)
            try_redownload = job_type in (
                REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_PRESENCE_URL,
                REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA_URL
            ) and len(useful_urls) > 0
            do_export = file_is_invalid and (
                job_type in
                (REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA,
                 REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA_SILENT_DELETE) or
                (job_type == REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA_URL
                 and try_redownload))
            ...
            if do_export:
                ...
                if not self._pubbed_message_about_invalid_file_export:
                    ...
                    message = 'During file maintenance, a file was found to be invalid. It and any known URLs have been moved to "{}".'.format(
                        error_dir)
            ...
            if delete_record:
                ...
                content_update = HydrusData.ContentUpdate(
                    ...
                    reason='Record deleted during File Integrity check.')

                for service_key in [
                        CC.LOCAL_FILE_SERVICE_KEY, CC.LOCAL_UPDATE_SERVICE_KEY,
                ...
                if not self._pubbed_message_about_bad_file_record_delete:
                    ...
                    message = 'During file maintenance, a file was found to be missing or invalid. Its record has been removed from the database. Any known URLs for the file have been written to "{}".'.format(
                        error_dir)
                    message += os.linesep * 2
            ...
            if try_redownload:
                ...
                def qt_add_url(url):
                    ...
                    if QP.isValid(HG.client_controller.gui):
                        ...
                        HG.client_controller.gui.ImportURL(
                            url, 'missing files redownloader')
    ...
    def _ClearJobs(self, hashes, job_type):
        ...
        if len(hashes) > 0:
            ...
            self._controller.WriteSynchronous('file_maintenance_clear_jobs',
                                              cleared_jobs)
    ...
    def _RegenFileMetadata(self, media_result):
        ...
        try:
            ...
            (size, mime, width, height, duration, num_frames, has_audio,
             num_words) = HydrusFileHandling.GetFileInfo(
                 path, ok_to_look_for_hydrus_updates=True)

            additional_data = (size, mime, width, height, duration, num_frames,
                               has_audio, num_words)

            if mime != original_mime:

                needed_to_dupe_the_file = self._controller.client_files_manager.ChangeFileExt(
                    hash, original_mime, mime)

                if needed_to_dupe_the_file:

                    self._controller.WriteSynchronous(
                        'file_maintenance_add_jobs_hashes', {hash},
                        REGENERATE_FILE_DATA_JOB_DELETE_NEIGHBOUR_DUPES,
                        HydrusData.GetNow() + (7 * 86400))

            return additional_data

        except HydrusExceptions.MimeException:

            self._CheckFileIntegrity(
                media_result, REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA_URL)
    ...
    def _RegenFileModifiedTimestamp(self, media_result):
        ...
        try:
            ...
            file_modified_timestamp = HydrusFileHandling.GetFileModifiedTimestamp(
                path)
    ...
    def _RegenFileOtherHashes(self, media_result):
        ...
        try:
            ...
            (md5, sha1,
             sha512) = HydrusFileHandling.GetExtraHashesFromPath(path)
    ...
    def _RegenFileThumbnailForce(self, media_result):
        ...
        try:
            ...
            self._controller.client_files_manager.RegenerateThumbnail(
                media_result)
    ...
    def _RegenFileThumbnailRefit(self, media_result):
        ...
        try:
            ...
            was_regenerated = self._controller.client_files_manager.RegenerateThumbnailIfWrongSize(
                media_result)
    ...
    def _ReInitialiseWorkRules(self):
        ...
        file_maintenance_idle_throttle_files = self._controller.new_options.GetInteger(
            'file_maintenance_idle_throttle_files')
        file_maintenance_idle_throttle_time_delta = self._controller.new_options.GetInteger(
            'file_maintenance_idle_throttle_time_delta')

        self._idle_work_rules = HydrusNetworking.BandwidthRules()

        self._idle_work_rules.AddRule(
            HC.BANDWIDTH_TYPE_REQUESTS,
            file_maintenance_idle_throttle_time_delta,
            file_maintenance_idle_throttle_files * NORMALISED_BIG_JOB_WEIGHT)

        file_maintenance_active_throttle_files = self._controller.new_options.GetInteger(
            'file_maintenance_active_throttle_files')
        file_maintenance_active_throttle_time_delta = self._controller.new_options.GetInteger(
            'file_maintenance_active_throttle_time_delta')

        self._active_work_rules = HydrusNetworking.BandwidthRules()

        self._active_work_rules.AddRule(
            HC.BANDWIDTH_TYPE_REQUESTS,
            file_maintenance_active_throttle_time_delta,
            file_maintenance_active_throttle_files * NORMALISED_BIG_JOB_WEIGHT)
    ...
    def _RunJob(self, media_results, job_type, job_key):
        ...
        try:
            ...
            if HG.file_report_mode:
                ...
                HydrusData.ShowText('file maintenance: {} for {} files'.format(
                    regen_file_enum_to_str_lookup[job_type],
                    HydrusData.ToHumanInt(num_to_do)))
            ...
            for (i, media_result) in enumerate(media_results):
                ...
                try:
                    ...
                    if job_type == REGENERATE_FILE_DATA_JOB_FILE_METADATA:
                        ...
                        additional_data = self._RegenFileModifiedTimestamp(
                            media_result)

                    elif job_type == REGENERATE_FILE_DATA_JOB_OTHER_HASHES:

                        additional_data = self._RegenFileOtherHashes(
                            media_result)
                        ...
                        was_regenerated = self._RegenFileThumbnailRefit(
                            media_result)
                        ...
                        job_key.SetVariable(
                            'popup_text_2', 'thumbs needing regen: {}'.format(
                                HydrusData.ToHumanInt(num_thumb_refits)))
                        ...
                        additional_data = self._CheckSimilarFilesMembership(
                            media_result)

                    elif job_type == REGENERATE_FILE_DATA_JOB_SIMILAR_FILES_METADATA:

                        additional_data = self._RegenSimilarFilesMetadata(
                            media_result)
                        ...
                        file_was_bad = self._CheckFileIntegrity(
                            media_result, job_type)
                        ...
                        job_key.SetVariable(
                            'popup_text_2',
                            'missing or invalid files: {}'.format(
                                HydrusData.ToHumanInt(num_bad_files)))

                except Exception as e:

                    HydrusData.PrintException(e)

                    message = 'There was a problem performing maintenance task {} on file {}! The job will not be reattempted. A full traceback of this error should be written to the log.'.format(
                        regen_file_enum_to_str_lookup[job_type], hash.hex())
                    message += os.linesep * 2
                    message += str(e)
                    ...
                    self._work_tracker.ReportRequestUsed(
                        num_requests=regen_file_enum_to_job_weight_lookup[
                            job_type])
                ...
                if len(cleared_jobs) > 100:
                    ...
                    self._controller.WriteSynchronous(
                        'file_maintenance_clear_jobs', cleared_jobs)
            ...
            if len(cleared_jobs) > 0:
                ...
                self._controller.Write('file_maintenance_clear_jobs',
                                       cleared_jobs)
    ...
    def CancelJobs(self, job_type):
        ...
        with self._lock:
            ...
            self._controller.WriteSynchronous('file_maintenance_cancel_jobs',
                                              job_type)
        ...
        with self._maintenance_lock:
            ...
            try:
                ...
                while True:
                    ...
                    media_results = self._controller.Read(
                        'media_results', hashes)
                    ...
                    missing_hashes = [
                        ...
                        if hash not in hashes_to_media_results
                    ]
    ...
    def MainLoopBackgroundWork(self):
        ...
        def wait_on_maintenance():
            ...
            while True:
                ...
                if self._AbleToDoBackgroundMaintenance(
                ) or self._reset_background_event.is_set():
                        ...
                        try:
                            ...
                            for media_result in media_results:
                                ...
                                with self._lock:
                                    ...
                                    self._RunJob((media_result, ), job_type,
                                                 job_key)
    ...
    def ScheduleJob(self, hashes, job_type, time_can_start=0):
        ...
        with self._lock:
            ...
            self._controller.Write('file_maintenance_add_jobs_hashes', hashes,
                                   job_type, time_can_start)
    ...
    def ScheduleJobHashIds(self, hash_ids, job_type, time_can_start=0):
        ...
        with self._lock:
            ...
            self._controller.Write('file_maintenance_add_jobs', hash_ids,
                                   job_type, time_can_start)
