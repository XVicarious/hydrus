...
def AddAudioVolumeMenu(menu, canvas_type):
    ...
    ClientGUIMenus.AppendMenuItem(volume_menu, label, 'Mute/unmute audio.',
    ...
    if mute_volume_type is not None:
        ...
        if HG.client_controller.new_options.GetBoolean(mute_option_name):
            ...
            label = 'unmute {}'.format(
                ClientGUIMediaControls.
                volume_types_str_lookup[mute_volume_type])

        else:

            label = 'mute {}'.format(ClientGUIMediaControls.
                                     volume_types_str_lookup[mute_volume_type])
    ...
    (
        mute_option_name, volume_option_name
    ) = ClientGUIMediaControls.volume_types_to_option_names[volume_volume_type]

    # 0-100 inclusive
    volumes = list(range(0, 110, 10))

    current_volume = HG.client_controller.new_options.GetInteger(
        volume_option_name)
    ...
    for volume in volumes:
        ...
        if volume == current_volume:
            ...
            ClientGUIMenus.AppendMenuCheckItem(
                volume_menu, label, 'Set the volume.', True,
                ClientGUIMediaControls.ChangeVolume, volume_volume_type,
                volume)

        else:

            ClientGUIMenus.AppendMenuItem(volume_menu, label,
                                          'Set the volume.',
                                          ClientGUIMediaControls.ChangeVolume,
                                          volume_volume_type, volume)
...
def CalculateCanvasMediaSize(media, canvas_size, show_action):
    ...
    if ShouldHaveAnimationBar(media, show_action):
        ...
        animated_scanbar_height = HG.client_controller.new_options.GetInteger(
            'animated_scanbar_height')
...
def CalculateCanvasZooms(canvas, media, show_action):
    ...
    if show_action in (
            ...
            CC.MEDIA_VIEWER_ACTION_DO_NOT_SHOW):
    ...
    (canvas_width,
     canvas_height) = CalculateCanvasMediaSize(media, canvas.size(),
                                               show_action)
    ...
    (media_scale_up, media_scale_down, preview_scale_up, preview_scale_down,
     exact_zooms_only, scale_up_quality,
     scale_down_quality) = new_options.GetMediaZoomOptions(mime)
    ...
    if exact_zooms_only:
        ...
        valid_regular_zooms = [
            zoom for zoom in regular_zooms if zoom < canvas_zoom
        ]
...
def CalculateMediaContainerSize(media, zoom, show_action):
    ...
    if show_action in (
        ...
        if media.GetMime() in HC.MIMES_WITH_THUMBNAILS:
            ...
            (thumb_width,
             thumb_height) = HydrusImageHandling.GetThumbnailResolution(
                 media.GetResolution(),
                 HG.client_controller.options['thumbnail_dimensions'])
...
def ShouldHaveAnimationBar(media, show_action):
    ...
    if show_action not in (CC.MEDIA_VIEWER_ACTION_SHOW_WITH_NATIVE,
...
class Animation(QW.QWidget):
    def __init__(self, parent, canvas_type):
        ...
        self._my_shortcut_handler = ClientGUIShortcuts.ShortcutsHandler(
            self, [shortcut_set], catch_mouse=True)
    ...
    def _TryToDrawCanvasBitmap(self):
        ...
        if self._video_container is None:
            ...
            self._video_container = ClientRendering.RasterContainerVideo(
                self._media,
                self.size().toTuple(),
                init_position=self._current_frame_index)
        ...
        if self._canvas_qt_pixmap is None:
            ...
            self._canvas_qt_pixmap = HG.client_controller.bitmap_manager.GetQtPixmap(
                my_width, my_height)

        painter = QG.QPainter(self._canvas_qt_pixmap)

        current_frame = self._video_container.GetFrame(
        ...
        next_frame_time_s = self._video_container.GetDuration(
            self._current_frame_index) / 1000.0
    ...
    def _DrawABlankFrame(self, painter):
        ...
        painter.setBackground(
            QG.QBrush(new_options.GetColour(CC.COLOUR_MEDIA_BACKGROUND)))
    ...
    def GetAnimationBarStatus(self):
        ...
        if self._video_container is None:
            ...
            if self._current_timestamp_ms is None and self._video_container.IsInitialised(
                ...
                self._current_timestamp_ms = self._video_container.GetTimestampMS(
                    self._current_frame_index)

        return (self._current_frame_index, self._current_timestamp_ms,
                self._paused, buffer_indices)
    ...
    def GotoFrame(self, frame_index):
        ...
        if self._video_container is not None and self._video_container.IsInitialised(
            ...
            if frame_index != self._current_frame_index:
                ...
                self._video_container.GetReadyForFrame(
                    self._current_frame_index)
            ...
            if self.size() != event.oldSize():
                ...
                if self._media is not None:
                    ...
                    if self._video_container is not None:
                        ...
                        (renderer_width,
                         renderer_height) = self._video_container.GetSize()
                        ...
                        if we_just_zoomed_in:
                            ...
                            if self._video_container.IsScaled():
                                ...
                                self._video_container = ClientRendering.RasterContainerVideo(
                                    self._media, (target_width, target_height),
                                    init_position=self._current_frame_index)
                            ...
                            if my_width < media_width or my_height < media_height:  # i.e. new zoom is scaled
                                ...
                                self._video_container = ClientRendering.RasterContainerVideo(
                                    self._media, (my_width, my_height),
                                    init_position=self._current_frame_index)
    ...
    def SetMedia(self, media, start_paused=False):
        ...
        self._current_frame_index = int(
            (self._num_frames - 1) * HC.options['animation_start_position'])
        self._current_frame_drawn = False
        self._current_timestamp_ms = None
        self._next_frame_due_at = HydrusData.GetNowPrecise()
        self._slow_frame_score = 1.0
        ...
        try:
            ...
            if self.isVisible():
                ...
                if self._current_frame_drawn:
                    ...
                    if not self._paused and HydrusData.TimeHasPassedPrecise(
                            self._next_frame_due_at):

                        num_frames = self._media.GetNumFrames()

                        self._current_frame_index = (
                            self._current_frame_index + 1) % num_frames
                        ...
                        if self._current_frame_index == 0:
                            ...
                            if self._current_timestamp_ms is not None and self._video_container is not None and self._video_container.IsInitialised(
                                ...
                                duration_ms = self._video_container.GetDuration(
                ...
                if self._video_container is not None:
                    ...
                    if not self._current_frame_drawn:
                        ...
                        if self._video_container.HasFrame(
                                self._current_frame_index):
...
class AnimationBar(QW.QWidget):
    ...
    def _GetXFromTimestamp(self, timestamp_ms, width_offset=0):
        ...
        return int(
            (my_width - width_offset) * timestamp_ms / self._duration_ms)

    def _Redraw(self, painter):

        self._last_drawn_info = self._GetAnimationBarStatus()

        (current_frame_index, current_timestamp_ms, paused,
         buffer_indices) = self._last_drawn_info
        ...
        if paused:
            ...
            background_colour = ClientData.GetLighterDarkerColour(
                background_colour)
        ...
        if buffer_indices is not None:
            ...
            if ClientRendering.FrameIndexOutOfRange(rendered_to_index,
            ...
            if start_x != rendered_to_x:
                ...
                rendered_colour = ClientData.GetDifferentLighterDarkerColour(
                    background_colour)
                ...
                if rendered_to_x > start_x:
                    ...
                    painter.drawRect(start_x, 0, rendered_to_x - start_x,
                                     animated_scanbar_height)

                else:

                    painter.drawRect(start_x, 0, my_width - start_x,
                                     animated_scanbar_height)

                    painter.drawRect(0, 0, rendered_to_x,
                                     animated_scanbar_height)

            if rendered_to_x != end_x:

                to_be_rendered_colour = ClientData.GetDifferentLighterDarkerColour(
                    background_colour, 1)
                ...
                if end_x > rendered_to_x:
                    ...
                    painter.drawRect(rendered_to_x, 0, end_x - rendered_to_x,
                                     animated_scanbar_height)

                else:

                    painter.drawRect(rendered_to_x, 0,
                                     my_width - rendered_to_x,
                                     animated_scanbar_height)
        ...
        animated_scanbar_nub_width = HG.client_controller.new_options.GetInteger(
        ...
        if self._num_frames is not None and current_frame_index is not None:
            ...
            nub_x = self._GetXFromFrameIndex(
                current_frame_index, width_offset=animated_scanbar_nub_width)

        elif self._duration_ms is not None and current_timestamp_ms is not None:

            nub_x = self._GetXFromTimestamp(
                current_timestamp_ms, width_offset=animated_scanbar_nub_width)

        if nub_x is not None:

            painter.drawRect(nub_x, 0, animated_scanbar_nub_width,
                             animated_scanbar_height)
        ...
        if self._num_frames is not None:
            ...
            progress_strings.append(
                HydrusData.ConvertValueRangeToPrettyString(
                    current_frame_index + 1, self._num_frames))

        if current_timestamp_ms is not None:

            progress_strings.append(
                HydrusData.ConvertValueRangeToScanbarTimestampsMS(
                    current_timestamp_ms, self._duration_ms))

        s = ' - '.join(progress_strings)

        if len(s) > 0:

            (x, y) = painter.fontMetrics().size(QC.Qt.TextSingleLine,
                                                s).toTuple()
    ...
    def EventMouse(self, event):
        ...
        if self._media_window is not None:
            ...
            if event.type(
            ) == QC.QEvent.MouseMove and event.buttons() != QC.Qt.NoButton:
            ...
            if a_button_is_down:
                ...
                compensated_x_position = event_pos.x() - (
                    animated_scanbar_nub_width / 2)

                proportion = (compensated_x_position) / (
                    my_width - animated_scanbar_nub_width)
                ...
                if isinstance(self._media_window, Animation):
                    ...
                    current_frame_index = int(proportion *
                                              (self._num_frames - 1) + 0.5)
...
class CanvasFrame(ClientGUITopLevelWindows.FrameThatResizes):
    def __init__(self, parent):
        ...
        ClientGUITopLevelWindows.FrameThatResizes.__init__(
            self, None, 'hydrus client media viewer', 'media_viewer')

        self._canvas_window = None

        self._my_shortcut_handler = ClientGUIShortcuts.ShortcutsHandler(
            self, ['global', 'media_viewer'])

        HG.client_controller.gui.RegisterCanvasFrameReference(self)

        self.destroyed.connect(
            HG.client_controller.gui.MaintainCanvasFrameReferences)
    ...
    def closeEvent(self, event):
        ...
        if self._canvas_window is not None:
            ...
            if can_close:
                ...
                ClientGUITopLevelWindows.FrameThatResizes.closeEvent(
                    self, event)
...
class Canvas(QW.QWidget):
    ...
    def __init__(self, parent):
        ...
        self._custom_shortcut_names = self._new_options.GetStringList(
        ...
        HG.client_controller.sub(self, 'OpenExternally',
                                 'canvas_open_externally')
        HG.client_controller.sub(self, 'ManageTags', 'canvas_manage_tags')
        HG.client_controller.sub(self, 'ProcessApplicationCommand',
                                 'canvas_application_command')
    ...
    def _CanDisplayMedia(self, media):
        ...
        (media_show_action, media_start_paused,
         media_start_with_embed) = self._GetShowAction(media)

        if media_show_action in (
                CC.
    ...
    def _CopyBMPToClipboard(self):
        ...
        if self._current_media is not None:
            ...
            if self._current_media.GetMime() in HC.IMAGES:
                ...
                HG.client_controller.pub('clipboard', 'bmp',
                                         self._current_media)
    ...
    def _CopyFileToClipboard(self):
        ...
        if self._current_media is not None:
            ...
            paths = [
                client_files_manager.GetFilePath(self._current_media.GetHash(),
                                                 self._current_media.GetMime())
            ]
    ...
    def _CopyPathToClipboard(self):
        ...
        if self._current_media is not None:
            ...
            path = client_files_manager.GetFilePath(
                self._current_media.GetHash(), self._current_media.GetMime())
    ...
    def _DoEdgePan(self, pan_type):
        ...
        if pan_type == 'pan_top_edge':
            ...
            delta_y = my_size.height() - (self._media_window_pos.y() +
                                          media_size.height())

        elif pan_type == 'pan_right_edge':

            delta_x = my_size.width() - (self._media_window_pos.x() +
                                         media_size.width())

        elif pan_type == 'pan_vertical_center':

            delta_y = (my_size.height() / 2) - (self._media_window_pos.y() +
                                                (media_size.height() / 2))

        elif pan_type == 'pan_horizontal_center':

            delta_x = (my_size.width() / 2) - (self._media_window_pos.x() +
                                               (media_size.width() / 2))
    ...
    def _GetShowAction(self, media):
        ...
        bad_result = (CC.MEDIA_VIEWER_ACTION_DO_NOT_SHOW, start_paused,
                      start_with_embed)
    ...
    def _GetMediaContainerSize(self):
        ...
        (media_show_action, media_start_paused,
         media_start_with_embed) = self._GetShowAction(self._current_media)

        (media_width,
         media_height) = CalculateMediaContainerSize(self._current_media,
                                                     self._current_zoom,
                                                     media_show_action)
    ...
    def _IsZoomable(self):
        ...
        return media_show_action not in (
            CC.MEDIA_VIEWER_ACTION_SHOW_OPEN_EXTERNALLY_BUTTON,
            CC.MEDIA_VIEWER_ACTION_DO_NOT_SHOW_ON_ACTIVATION_OPEN_EXTERNALLY,
            CC.MEDIA_VIEWER_ACTION_DO_NOT_SHOW)
    ...
    def _IShouldCatchShortcutEvent(self, event=None):
        ...
        return ClientGUIShortcuts.IShouldCatchShortcutEvent(
            self,
            event=event,
            child_tlw_classes_who_can_pass_up=(
                ClientGUIHoverFrames.FullscreenHoverFrame, ))
    ...
    def _MaintainZoom(self, previous_media):
        ...
        if previous_media is None:
            ...
            (gumpf_current_zoom,
             self._canvas_zoom) = CalculateCanvasZooms(self,
                                                       self._current_media,
                                                       media_show_action)
            ...
            (previous_width,
             previous_height) = CalculateMediaSize(previous_media,
                                                   self._current_zoom)

            (current_media_100_width,
             current_media_100_height) = self._current_media.GetResolution()

            self._current_zoom = previous_width / current_media_100_width

            HG.client_controller.pub('canvas_new_zoom', self._canvas_key,
                                     self._current_zoom)
    ...
    def _ManageNotes(self):
        def qt_do_it(media, notes):
            ...
            with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
                ...
                (min_width,
                 min_height) = ClientGUIFunctions.ConvertTextToPixels(
                     control, (80, 14))
    ...
    def _ManageRatings(self):
        ...
        if len(
            ...
            with ClientGUIDialogsManage.DialogManageRatings(
                    self, (self._current_media, )) as dlg:
    ...
    def _ManageTags(self):
        ...
        for child in self.children():
            ...
            if isinstance(
                    child,
                    ClientGUITopLevelWindows.FrameThatTakesScrollablePanel):
        ...
        manage_tags = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
            self, title, frame_key)

        panel = ClientGUITags.ManageTagsPanel(manage_tags,
    ...
    def _ProcessShortcut(self, shortcut):
        ...
        command = HG.client_controller.GetCommandFromShortcut(
    ...
    def _ReinitZoom(self):
        ...
        (self._current_zoom,
         self._canvas_zoom) = CalculateCanvasZooms(self, self._current_media,
                                                   media_show_action)
    ...
    def _SaveCurrentMediaViewTime(self):
        ...
        HG.client_controller.file_viewing_stats_manager.FinishViewing(
            viewtype, hash, viewtime_delta)
    ...
    def _TryToChangeZoom(self, new_zoom):
        ...
        (media_window_width,
         media_window_height) = self._media_container.size().toTuple()

        (new_media_window_width,
         new_media_window_height) = CalculateMediaSize(self._current_media,
                                                       new_zoom)

        my_size = self.size()

        old_size_bigger = my_size.width(
        ) < media_window_width or my_size.height() < media_window_height
        new_size_fits = my_size.width(
        ) >= new_media_window_width and my_size.height(
        ) >= new_media_window_height
    ...
    def _ZoomIn(self):
        ...
        if self._current_media is not None and self._IsZoomable():
            ...
            (media_scale_up, media_scale_down, preview_scale_up,
             preview_scale_down, exact_zooms_only, scale_up_quality,
             scale_down_quality) = self._new_options.GetMediaZoomOptions(
                 self._current_media.GetMime())
            ...
            bigger_zooms = [
                zoom for zoom in possible_zooms if zoom > self._current_zoom
            ]
    ...
    def resizeEvent(self, event):
        ...
        if self._current_media is not None:
            ...
            (media_width,
             media_height) = self._media_container.size().toTuple()
    ...
    def MouseIsOverMedia(self):
        ...
        if self._current_media is None:
            ...
            media_mouse_pos = self._media_container.mapFromGlobal(
                QG.QCursor.pos())
    ...
    def ProcessApplicationCommand(self, command, canvas_key=None):
        ...
        if command_type == CC.APPLICATION_COMMAND_TYPE_SIMPLE:
            ...
            elif action in ('pan_top_edge', 'pan_bottom_edge', 'pan_left_edge',
                            'pan_right_edge', 'pan_vertical_center',
                            'pan_horizontal_center'):
            ...
            command_processed = ClientGUIFunctions.ApplyContentApplicationCommandToMedia(
                self, command, (self._current_media, ))
    ...
    def SetMedia(self, media):
        ...
        if media != self._current_media:
            ...
            if self._current_media is None:
                ...
                if self._current_media.GetLocationsManager().IsLocal(
                ) and initial_width > 0 and initial_height > 0:

                    (media_show_action, media_start_paused,
                     media_start_with_embed) = self._GetShowAction(
                         self._current_media)

                    pos = (self._media_window_pos.x(),
                           self._media_window_pos.y())

                    self._media_container.SetMedia(self._current_media,
                                                   initial_size, pos,
                                                   media_show_action,
                                                   media_start_paused,
            ...
            HG.client_controller.pub('canvas_new_display_media',
                                     self._canvas_key, self._current_media)

            HG.client_controller.pub('canvas_new_index_string',
                                     self._canvas_key, self._GetIndexString())
...
class CanvasPanel(Canvas):
    ...
    def __init__(self, parent, page_key):
        ...
        HG.client_controller.sub(self, 'MediaFocusWentToExternalProgram',
    ...
    def ShowMenu(self):
        ...
        if self._current_media is not None:
            ...
            ClientGUIMedia.AddFileViewingStatsMenu(info_menu,
                                                   self._current_media)
        ...
        if self._current_media is not None:
            ...
            if self._current_media.HasInbox():
                ...
                ClientGUIMenus.AppendMenuItem(menu, 'archive',
                                              'Archive this file.',
                                              self._Archive)
            ...
            if self._current_media.HasArchive():
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'inbox', 'Send this files back to the inbox.',
                    self._Inbox)
            ...
            if CC.LOCAL_FILE_SERVICE_KEY in locations_manager.GetCurrent():
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu,
                    'delete completely',
                    'Physically delete this file from disk.',
                    self._Delete,
                    file_service_key=CC.TRASH_SERVICE_KEY)
                ClientGUIMenus.AppendMenuItem(
                    menu, 'undelete', 'Take this file out of the trash.',
                    self._Undelete)
            ...
            ClientGUIMenus.AppendMenuItem(manage_menu, 'tags',
                                          'Manage this file\'s tags.',
                                          self._ManageTags)

            if i_can_post_ratings:

                ClientGUIMenus.AppendMenuItem(manage_menu, 'ratings',
                                              'Manage this file\'s ratings.',
                                              self._ManageRatings)

            ClientGUIMenus.AppendMenuItem(manage_menu, 'known urls',
                                          'Manage this file\'s known URLs.',
                                          self._ManageURLs)

            ClientGUIMenus.AppendMenuItem(manage_menu, 'notes',
                                          'Manage this file\'s notes.',
                                          self._ManageNotes)

            ClientGUIMenus.AppendMenu(menu, manage_menu, 'manage')

            ClientGUIMedia.AddKnownURLsViewCopyMenu(self, menu,
                                                    self._current_media)
            ...
            ClientGUIMenus.AppendMenuItem(
                ...
                'Open this file in your OS\'s default program.',
                self._OpenExternally)
            ClientGUIMenus.AppendMenuItem(
                open_menu, 'in a new page',
                'Show your current media in a simple new page.',
                self._ShowMediaInNewPage)
            ClientGUIMenus.AppendMenuItem(
            ...
            ClientGUIMenus.AppendMenuItem(copy_menu, 'file',
                                          'Copy this file to your clipboard.',
                                          self._CopyFileToClipboard)

            copy_hash_menu = QW.QMenu(copy_menu)

            ClientGUIMenus.AppendMenuItem(copy_hash_menu,
                                          'sha256 (hydrus default)',
                                          'Open this file\'s SHA256 hash.',
                                          self._CopyHashToClipboard, 'sha256')
            ClientGUIMenus.AppendMenuItem(copy_hash_menu, 'md5',
                                          'Open this file\'s MD5 hash.',
                                          self._CopyHashToClipboard, 'md5')
            ClientGUIMenus.AppendMenuItem(copy_hash_menu, 'sha1',
                                          'Open this file\'s SHA1 hash.',
                                          self._CopyHashToClipboard, 'sha1')
            ClientGUIMenus.AppendMenuItem(copy_hash_menu, 'sha512',
                                          'Open this file\'s SHA512 hash.',
            ...
            if self._current_media.GetMime() in HC.IMAGES:
                ...
                ClientGUIMenus.AppendMenuItem(
                    ...
                    'Copy this file to your clipboard as a bmp.',
                    self._CopyBMPToClipboard)

            ClientGUIMenus.AppendMenuItem(
                copy_menu, 'path', 'Copy this file\'s path to your clipboard.',
                self._CopyPathToClipboard)
    ...
    def ProcessContentUpdates(self, service_keys_to_content_updates):
        ...
        if self._current_media is not None:
            ...
            for (service_key, content_updates) in list(
                ...
                if True in (my_hash in content_update.GetHashes()
...
class CanvasWithDetails(Canvas):
    def __init__(self, parent):
        ...
        HG.client_controller.sub(self, 'RedrawDetails',
    ...
    def _DrawBackgroundDetails(self, painter):
        ...
        if self._current_media is None:
            ...
            (width,
             height) = painter.fontMetrics().size(QC.Qt.TextSingleLine,
                                                  text).toTuple()
            ...
            ClientTags.SortTags(HC.options['default_tag_sort'],
                                tags_i_want_to_display)
            ...
            like_services = services_manager.GetServices(
            ...
            for like_service in like_services:
                ...
                rating_state = ClientRatings.GetLikeStateFromMedia(
                    (self._current_media, ), service_key)

                ClientRatings.DrawLike(painter, like_rating_current_x,
                                       current_y, service_key, rating_state)
            ...
            numerical_services = services_manager.GetServices(
            ...
            for numerical_service in numerical_services:
                ...
                ClientRatings.DrawNumerical(
                    painter, client_width - numerical_width - 2, current_y,
                    service_key, rating_state,
                    rating)  # -2 to line up exactly with the floating panel
            ...
            if CC.TRASH_SERVICE_KEY in self._current_media.GetLocationsManager(
            ).GetCurrent():
            ...
            if len(icons_to_show) > 0:
                ...
                for icon in icons_to_show:
                    ...
                    painter.drawPixmap(client_width + icon_x - 18, current_y,
                                       icon)
            ...
            painter.setPen(
                QG.QPen(self._new_options.GetColour(CC.COLOUR_MEDIA_TEXT)))

            # repo strings

            remote_strings = self._current_media.GetLocationsManager(
            ).GetRemoteLocationStrings()

            for remote_string in remote_strings:

                (text_width, text_height) = painter.fontMetrics().size(
                    QC.Qt.TextSingleLine, remote_string).toTuple()

                QP.DrawText(painter, client_width - text_width - 3, current_y,
                            remote_string)
            ...
            url_tuples = HG.client_controller.network_engine.domain_manager.ConvertURLsToMediaViewerTuples(
                urls)

            for (display_string, url) in url_tuples:

                (text_width, text_height) = painter.fontMetrics().size(
                    QC.Qt.TextSingleLine, display_string).toTuple()
            ...
            if len(title_string) > 0:
                ...
                (x, y) = painter.fontMetrics().size(QC.Qt.TextSingleLine,
                                                    title_string).toTuple()

                QP.DrawText(painter, (client_width - x) // 2, current_y,
                            title_string)
            ...
            if len(index_string) > 0:
                ...
                (x, y) = painter.fontMetrics().size(QC.Qt.TextSingleLine,
                                                    index_string).toTuple()

                QP.DrawText(painter, client_width - x - 3,
                            client_height - y - 3, index_string)
...
class CanvasWithHovers(CanvasWithDetails):
    def __init__(self, parent):
        ...
        ClientGUIHoverFrames.FullscreenHoverFrameTags(self, self,
                                                      self._canvas_key)

        ClientGUIHoverFrames.FullscreenHoverFrameTopRight(
            self, self, self._canvas_key)
        ...
        HG.client_controller.sub(self, 'FullscreenSwitch',
                                 'canvas_fullscreen_switch')
    ...
    def EventMouseMove(self, event):
        ...
        is_dragging = (event.type() == QC.QEvent.MouseMove
                       and event.buttons() == QC.Qt.LeftButton
        ...
        if is_dragging:
            ...
            if approx_distance > 0:
                ...
                touchscreen_canvas_drags_unanchor = HG.client_controller.new_options.GetBoolean(
                ...
                anchor_and_hide_canvas_drags = HG.client_controller.new_options.GetBoolean(
    ...
    def _PutOffCursorHide(self):
        ...
        self._timer_cursor_hide_job = HG.client_controller.CallLaterQtSafe(
            self, 0.8, self._HideCursor)
...
class CanvasFilterDuplicates(CanvasWithHovers):
    def __init__(self, parent, file_search_context, both_files_match):
        ...
        ClientGUIHoverFrames.FullscreenHoverFrameRightDuplicates(
            self, self, self._canvas_key)
    ...
    def TryToDoPreClose(self):
        ...
        if num_committable > 0:
            ...
            label = 'commit ' + HydrusData.ToHumanInt(
                num_committable) + ' decisions?'

            (result,
             cancelled) = ClientGUIDialogsQuick.GetFinishFilteringAnswer(
                 self, label)

            if cancelled:

                close_was_triggered_by_everything_being_processed = len(
                    self._unprocessed_pairs) == 0
    ...
    def _CommitProcessed(self, blocking=True):
        ...
        for (hash_pair, duplicate_type, first_media, second_media,
             service_keys_to_content_updates,
             was_auto_skipped) in self._processed_pairs:
    ...
    def _Delete(self, media=None, reason=None, file_service_key=None):
        ...
        with ClientGUIDialogs.DialogYesYesNo(self,
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                if value == 'current':
                    ...
                    media = [
                        self._current_media,
                        self._media_list.GetNext(self._current_media)
                    ]
        ...
        deleted = CanvasWithHovers._Delete(self,
                                           media=media,
    ...
    def _DoCustomAction(self):
        ...
        duplicate_types = [
            ...
            HC.DUPLICATE_ALTERNATE, HC.DUPLICATE_FALSE_POSITIVE
        ]
        ...
        if duplicate_type in [
                HC.DUPLICATE_BETTER, HC.DUPLICATE_SAME_QUALITY
        ] or (new_options.GetBoolean('advanced_mode')
        ...
        self._ProcessPair(duplicate_type,
    ...
    def _GenerateHoverTopFrame(self):
        ...
        ClientGUIHoverFrames.FullscreenHoverFrameTopDuplicatesFilter(
            self, self, self._canvas_key)
    ...
    def _GetBackgroundColour(self):
        ...
        if self._current_media is None or len(self._media_list) == 0:
            ...
            if self._current_media == self._media_list.GetFirst():
                ...
                duplicate_intensity = new_options.GetNoneableInteger(
                    'duplicate_background_switch_intensity')

                return ClientData.GetLighterDarkerColour(
                    normal_colour, duplicate_intensity)
    ...
    def _GetIndexString(self):
        ...
        if self._current_media is None or len(self._media_list) == 0:
            ...
            progress = len(
                self._processed_pairs
            ) + 1  # +1 here actually counts for the one currently displayed
            total = progress + len(self._unprocessed_pairs)

            index_string = HydrusData.ConvertValueRangeToPrettyString(
                progress, total)
    ...
    def _GetNumCommittableDecisions(self):
        ...
        return len([
            1 for (hash_pair, duplicate_type, first_media, second_media,
                   service_keys_to_content_updates,
                   was_auto_skipped) in self._processed_pairs
            if duplicate_type is not None and not was_auto_skipped
        ])

    def _GoBack(self):

        if len(self._processed_pairs) > 0 and self._GetNumCommittableDecisions(
        ) > 0:

            self._unprocessed_pairs.append(self._current_pair)

            (hash_pair, duplicate_type, first_media, second_media,
             service_keys_to_content_updates,
            ...
            while was_auto_skipped:
                ...
                if len(self._processed_pairs) == 0:
                    ...
                    QW.QMessageBox.critical(
                        self, 'Error',
                        'Due to an unexpected series of events (likely a series of file deletes), the duplicate filter has no valid pair to back up to. It will now close.'
                    )
            ...
            self._hashes_due_to_be_deleted_in_this_batch.difference_update(
                hash_pair)
    ...
    def _ProcessPair(self,
        ...
        if delete_first or delete_second or delete_both:
            ...
            if delete_first or delete_both:
                ...
                self._hashes_due_to_be_deleted_in_this_batch.update(
                    first_media.GetHashes())

            if delete_second or delete_both:

                self._hashes_due_to_be_deleted_in_this_batch.update(
                    second_media.GetHashes())
            ...
            if duplicate_type in (HC.DUPLICATE_BETTER, HC.DUPLICATE_WORSE):
                ...
                file_deletion_reason = HC.duplicate_type_string_lookup[
                    duplicate_type]
            ...
            file_deletion_reason = 'Deleted in Duplicate Filter ({}).'.format(
        ...
        self._processed_pairs.append(
            (self._current_pair, duplicate_type, first_media, second_media,
             service_keys_to_content_updates, was_auto_skipped))
    ...
    def _ShowNewPair(self):
        ...
        for tlw in tlws:
            ...
            if isinstance(tlw, ClientGUITopLevelWindows.
                          DialogCustomButtonQuestion) and tlw.isModal():
        ...
        if len(self._unprocessed_pairs) == 0 and num_committable > 0:
            ...
            label = 'commit ' + HydrusData.ToHumanInt(
                num_committable) + ' decisions and continue?'

            result = ClientGUIDialogsQuick.GetInterstitialFilteringAnswer(
                self, label)
        ...
        if len(self._unprocessed_pairs) == 0:
            ...
            self._hashes_due_to_be_deleted_in_this_batch = set()
            self._processed_pairs = [
            ]  # just in case someone 'skip'ed everything in the last batch, so this never got cleared above

            self.SetMedia(None)

            self._media_list = ClientMedia.ListeningMediaList(
                file_service_key, [])
            ...
            while not pair_is_good(potential_pair):
                ...
                self._processed_pairs.append(
                    (potential_pair, None, None, None, {}, was_auto_skipped))
                ...
                if len(self._unprocessed_pairs) == 0:
                    ...
                    if len(self._processed_pairs) == 0:
                        ...
                        self._ShowNewPair(
                        )  # there are no useful decisions left in the queue, so let's reset
            ...
            (first_media_result,
             second_media_result) = HG.client_controller.Read(
                 'media_results', self._current_pair)

            if not (first_media_result.GetLocationsManager().IsLocal()
                    and second_media_result.GetLocationsManager().IsLocal()):

                QW.QMessageBox.warning(
                    self, 'Warning',
                    'At least one of the potential files in this pair was not in this client. Likely it was very recently deleted through a different process. Your decisions until now will be saved, and then the duplicate filter will close.'
                )
            ...
            score = ClientMedia.GetDuplicateComparisonScore(
                first_media, second_media)

            if score > 0:

                media_results_with_better_first = (first_media_result,
                                                   second_media_result)

            else:

                media_results_with_better_first = (second_media_result,
                                                   first_media_result)

            self._media_list = ClientMedia.ListeningMediaList(
                file_service_key, media_results_with_better_first)
    ...
    def _SkipPair(self):
        ...
        self._processed_pairs.append(
            (self._current_pair, None, None, None, {}, was_auto_skipped))
    ...
    def EventMouse(self, event):
        ...
        if self._IShouldCatchShortcutEvent(event=event):
            ...
            if event.modifiers() & QC.Qt.ShiftModifier:
                ...
                ) == QC.QEvent.MouseButtonRelease and event.button(
                ) == QC.Qt.LeftButton:
    ...
    def ProcessApplicationCommand(self, command, canvas_key=None):
        ...
        if command_type == CC.APPLICATION_COMMAND_TYPE_SIMPLE:
            ...
            elif action in ('view_first', 'view_last', 'view_previous',
                            'view_next'):
        ...
        if not command_processed:
            ...
            command_processed = CanvasWithHovers.ProcessApplicationCommand(
                self, command)
    ...
    def SetMedia(self, media):
        ...
        if media is not None:
            ...
            if shown_media != comparison_media:
                ...
                HG.client_controller.pub('canvas_new_duplicate_pair',
                                         self._canvas_key, shown_media,
                                         comparison_media)
    ...
    def THREADFetchPairs(self, file_search_context, both_files_match):
        ...
        result = HG.client_controller.Read('duplicate_pairs_for_filtering',
                                           file_search_context,
                                           both_files_match)
...
class CanvasMediaList(ClientMedia.ListeningMediaList, CanvasWithHovers):
    def __init__(self, parent, page_key, media_results):
        ...
        CanvasWithHovers.__init__(self, parent)
        ClientMedia.ListeningMediaList.__init__(self,
    ...
    def TryToDoPreClose(self):
        ...
        HG.client_controller.pub('set_focus', self._page_key,
                                 self._current_media)
    ...
    def _GetIndexString(self):
        ...
        if self._current_media is None:
            ...
            index_string = '-/' + HydrusData.ToHumanInt(len(
                self._sorted_media))

        else:

            index_string = HydrusData.ConvertValueRangeToPrettyString(
                self._sorted_media.index(self._current_media) + 1,
            ...
            if media.IsStaticImage():
                ...
                if not image_cache.HasImageRenderer(hash):
                    ...
                    HG.client_controller.CallLaterQtSafe(
                        self, delay, image_cache.GetImageRenderer, media)
    ...
    def _Remove(self):
        ...
        ClientMedia.ListeningMediaList._RemoveMediaDirectly(
...
class CanvasMediaListFilterArchiveDelete(CanvasMediaList):
    def __init__(self, parent, page_key, media_results):
        ...
        QP.CallAfter(self.SetMedia, self._GetFirst()
                     )  # don't set this until we have a size > (20, 20)!
    ...
    def TryToDoPreClose(self):
        ...
        if len(self._kept) > 0 or len(self._deleted) > 0:
            ...
            label = 'keep ' + HydrusData.ToHumanInt(len(
                self._kept)) + ' and delete ' + HydrusData.ToHumanInt(
                    len(self._deleted)) + ' files?'
            ...
            if cancelled:
                ...
                def process_in_thread(service_keys_and_content_updates):
                    ...
                    for (service_key,
                         content_update) in service_keys_and_content_updates:

                        HG.client_controller.WriteSynchronous(
                            'content_updates', {service_key: [content_update]})

                self._deleted_hashes = [
                    media.GetHash() for media in self._deleted
                ]
                ...
                for chunk_of_hashes in HydrusData.SplitListIntoChunks(
                        self._deleted_hashes, 64):

                    service_keys_and_content_updates.append(
                        (CC.LOCAL_FILE_SERVICE_KEY,
                         HydrusData.ContentUpdate(HC.CONTENT_TYPE_FILES,
                ...
                HG.client_controller.CallToThread(
                    process_in_thread, service_keys_and_content_updates)

                self._kept = set()
                self._deleted = set()

                self._current_media = self._GetFirst(
                )  # so the pubsub on close is better
                ...
                if HC.options['remove_filtered_files']:
                    ...
                    HG.client_controller.pub('remove_media', self._page_key,
                                             all_hashes)
    ...
    def _GenerateHoverTopFrame(self):
        ...
        ClientGUIHoverFrames.FullscreenHoverFrameTopArchiveDeleteFilter(
            self, self, self._canvas_key)
    ...
    def ProcessApplicationCommand(self, command, canvas_key=None):
        ...
        if not command_processed:
            ...
            command_processed = CanvasMediaList.ProcessApplicationCommand(
                self, command)
...
class CanvasMediaListNavigable(CanvasMediaList):
    ...
    def _GenerateHoverTopFrame(self):
        ...
        ClientGUIHoverFrames.FullscreenHoverFrameTopNavigableList(
            self, self, self._canvas_key)
...
class CanvasMediaListBrowser(CanvasMediaListNavigable):
    def __init__(self, parent, page_key, media_results, first_hash):
        ...
        CanvasMediaListNavigable.__init__(self, parent, page_key,
                                          media_results)
        ...
        QP.CallAfter(
            self.SetMedia,
            first_media)  # don't set this until we have a size > (20, 20)!
    ...
    def _StartSlideshow(self, interval=None):
        ...
        if interval is None:
            ...
            with ClientGUIDialogs.DialogTextEntry(
                    self, 'Enter the interval, in seconds.',
                    default='15') as dlg:
        ...
        if interval > 0:
            ...
            self._timer_slideshow_job = HG.client_controller.CallLaterQtSafe(
                self, self._timer_slideshow_interval, self.DoSlideshow)
    ...
    def DoSlideshow(self):
        ...
        try:
            ...
            if self._current_media is not None and self._timer_slideshow_job is not None:
                ...
                ) and not HG.client_controller.MenuIsOpen():
    ...
    def ProcessApplicationCommand(self, command, canvas_key=None):
        ...
        if not command_processed:
            ...
            command_processed = CanvasMediaListNavigable.ProcessApplicationCommand(
                self, command)
    ...
    def ShowMenu(self):
        ...
        if self._current_media is not None:
            ...
            if self._IsZoomable():
                ...
                ClientGUIMenus.AppendMenuItem(zoom_menu, 'zoom in',
                                              'Zoom the media in.',
                                              self._ZoomIn)
                ClientGUIMenus.AppendMenuItem(zoom_menu, 'zoom out',
                                              'Zoom the media out.',
                                              self._ZoomOut)

                if self._current_zoom != 1.0:

                    ClientGUIMenus.AppendMenuItem(zoom_menu, 'zoom to 100%',
                                                  'Set the zoom to 100%.',
                                                  self._ZoomSwitch)
                    ...
                    ClientGUIMenus.AppendMenuItem(
                        zoom_menu, 'zoom fit',
                        'Set the zoom so the media fits the canvas.',
                        self._ZoomSwitch)

                ClientGUIMenus.AppendMenu(
                    menu, zoom_menu, 'current zoom: {}'.format(
                        ClientData.ConvertZoomToPercentage(
                            self._current_zoom)))
            ...
            if self.parentWidget().isFullScreen():
                ...
                ClientGUIMenus.AppendMenuItem(
                    ...
                    'Make this media viewer a regular window with borders.',
                    self.parentWidget().FullscreenSwitch)
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'go fullscreen',
                    'Make this media viewer a fullscreen window without borders.',
                    self.parentWidget().FullscreenSwitch)
            ...
            ClientGUIMenus.AppendMenuItem(
                slideshow, '1 second',
                'Start a slideshow with a one second interval.',
                self._StartSlideshow, 1.0)
            ClientGUIMenus.AppendMenuItem(
                slideshow, '5 second',
                'Start a slideshow with a five second interval.',
                self._StartSlideshow, 5.0)
            ClientGUIMenus.AppendMenuItem(
                slideshow, '10 second',
                'Start a slideshow with a ten second interval.',
                self._StartSlideshow, 10.0)
            ClientGUIMenus.AppendMenuItem(
                slideshow, '30 second',
                'Start a slideshow with a thirty second interval.',
                self._StartSlideshow, 30.0)
            ClientGUIMenus.AppendMenuItem(
                slideshow, '60 second',
                'Start a slideshow with a one minute interval.',
                self._StartSlideshow, 60.0)
            ClientGUIMenus.AppendMenuItem(slideshow, 'very fast',
                                          'Start a very fast slideshow.',
                                          self._StartSlideshow, 0.08)
            ClientGUIMenus.AppendMenuItem(
                slideshow, 'custom interval',
                'Start a slideshow with a custom interval.',
            ...
            if self._timer_slideshow_job is not None:
                ...
                ClientGUIMenus.AppendMenuItem(menu, 'stop slideshow',
            ...
            ClientGUIMenus.AppendMenuItem(
                menu, 'remove from view',
                'Remove this file from the list you are viewing.',
                self._Remove)
            ...
            if self._current_media.HasInbox():
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'archive',
                    'Archive this file, taking it out of the inbox.',
                    self._Archive)

            elif self._current_media.HasArchive():

                ClientGUIMenus.AppendMenuItem(
                    menu, 'return to inbox',
                    'Put this file back in the inbox.', self._Inbox)
            ...
            if CC.LOCAL_FILE_SERVICE_KEY in locations_manager.GetCurrent():
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu,
                    'delete from trash now',
                    'Delete this file immediately. This cannot be undone.',
                    self._Delete,
                    file_service_key=CC.TRASH_SERVICE_KEY)
                ClientGUIMenus.AppendMenuItem(
                    menu, 'undelete',
                    'Take this file out of the trash, returning it to its original file service.',
                    self._Undelete)
            ...
            ClientGUIMenus.AppendMenuItem(
                open_menu, 'in external program',
                'Open this file in the default external program.',
                self._OpenExternally)
            ClientGUIMenus.AppendMenuItem(
                open_menu, 'in a new page',
                'Show your current media in a simple new page.',
            ...
            ClientGUIMenus.AppendMenuItem(
                ...
                'Copy this file\'s SHA256 hash to your clipboard.',
                self._CopyHashToClipboard, 'sha256')
            ClientGUIMenus.AppendMenuItem(
                copy_hash_menu, 'md5',
                'Copy this file\'s MD5 hash to your clipboard.',
                self._CopyHashToClipboard, 'md5')
            ClientGUIMenus.AppendMenuItem(
                copy_hash_menu, 'sha1',
                'Copy this file\'s SHA1 hash to your clipboard.',
                self._CopyHashToClipboard, 'sha1')
            ClientGUIMenus.AppendMenuItem(
                copy_hash_menu, 'sha512',
                'Copy this file\'s SHA512 hash to your clipboard.',
                self._CopyHashToClipboard, 'sha512')
            ...
            if self._current_media.GetMime() in HC.IMAGES:
                ...
                ClientGUIMenus.AppendMenuItem(
                    copy_menu, 'image (bitmap)',
                    'Copy this file to your clipboard as a BMP image.',
                    self._CopyBMPToClipboard)
...
class MediaContainerDragClickReportingFilter(QC.QObject):
    ...
    def eventFilter(self, watched, event):
        ...
        if event.type() == QC.QEvent.MouseButtonPress and event.button(
        ) == QC.Qt.LeftButton and event.modifiers() == QC.Qt.NoModifier:

            self.parent().BeginDrag()

        elif event.type() == QC.QEvent.MouseButtonRelease and event.button(
        ) == QC.Qt.LeftButton and event.modifiers() == QC.Qt.NoModifier:
...
class MediaContainer(QW.QWidget):
    def __init__(self, parent, canvas_type):
        ...
        self._embed_button_widget_event_filter = QP.WidgetEventFilter(
            self._embed_button)
        self._embed_button_widget_event_filter.EVT_LEFT_DOWN(
            self.EventEmbedButton)

        self.setMouseTracking(True)

        self._drag_click_reporting_filter = MediaContainerDragClickReportingFilter(
            self.parent())

        self._animation_window = Animation(self, self._canvas_type)
        self._animation_bar = AnimationBar(self)
        self._volume_control = ClientGUIMediaControls.VolumeControl(
            self, self._canvas_type, direction='up')
    ...
    def _MakeMediaWindow(self):
        ...
        if self._show_action in (
                CC.
                MEDIA_VIEWER_ACTION_DO_NOT_SHOW_ON_ACTIVATION_OPEN_EXTERNALLY,
                CC.MEDIA_VIEWER_ACTION_DO_NOT_SHOW):

            raise Exception(
                'This media should not be shown in the media viewer!')
        ...
        if ShouldHaveAnimationBar(self._media, self._show_action):
            ...
            self._animation_bar.SetMediaAndWindow(self._media,
                                                  self._media_window)

            if isinstance(self._media_window,
                          ClientGUIMPV.mpvWidget) and self._media.HasAudio():
        ...
        if old_media_window != self._media_window:
            ...
            self._media_window.installEventFilter(
                self._drag_click_reporting_filter)

            if old_media_window is not None:

                old_media_window.removeEventFilter(
                    self._drag_click_reporting_filter)
    ...
    def _SizeAndPositionChildren(self):
        ...
        if self._media is not None:
            ...
            if self._media_window is None:
                ...
                self._embed_button.setFixedSize(
                    QP.TupleToQSize((my_width, my_height)))
                self._embed_button.move(QP.TupleToQPoint((0, 0)))

            else:

                is_open_externally = isinstance(self._media_window,
                                                OpenExternallyPanel)

                (media_width, media_height) = (my_width, my_height)

                if ShouldHaveAnimationBar(
                        self._media,
                        self._show_action) and not is_open_externally:
                    ...
                    self._animation_bar.setFixedSize(
                        QP.TupleToQSize((my_width - volume_width,
                                         animated_scanbar_height)))
                    self._animation_bar.move(
                        QP.TupleToQPoint(
                            (0, my_height - animated_scanbar_height)))

                    if self._volume_control.isVisibleTo(self):

                        self._volume_control.setFixedSize(
                            QP.TupleToQSize(
                                (volume_width, animated_scanbar_height)))
                        self._volume_control.move(
                            QP.TupleToQPoint(
                                (self._animation_bar.width(),
                                 my_height - animated_scanbar_height)))

                self._media_window.setFixedSize(
                    QP.TupleToQSize((media_width, media_height)))
    ...
    def MouseIsNearAnimationBar(self):
        ...
        if self._media is None:
            ...
            if ShouldHaveAnimationBar(self._media, self._show_action):
                ...
                animation_bar_mouse_pos = self._animation_bar.mapFromGlobal(
                    QG.QCursor.pos())
                ...
                test_rect = animation_bar_rect.adjusted(
    ...
    def paintEvent(self, event):
        ...
        if self._volume_control.isVisible():
            ...
            background_colour = HG.client_controller.new_options.GetColour(
                CC.COLOUR_MEDIA_BACKGROUND)
    ...
    def Pause(self):
        ...
        if self._media is not None:
            ...
            if isinstance(self._media_window,
                          (Animation, ClientGUIMPV.mpvWidget)):
    ...
    def ReadyToSlideshow(self):
        ...
        if self._media is None:
            ...
            if isinstance(self._media_window,
                ...
                if self._media_window.IsPlaying(
                ) and not self._media_window.HasPlayedOnceThrough():
...
class EmbedButton(QW.QWidget):
    ...
    def _Redraw(self, painter):
        ...
        if self._thumbnail_qt_pixmap is not None:
            ...
            (thumb_width,
             thumb_height) = self._thumbnail_qt_pixmap.size().toTuple()
        ...
        points.append(
            QC.QPoint(center_x - third_triangle_width,
                      center_y - half_triangle_side))
    ...
    def SetMedia(self, media):
        ...
        if self._media is None:
            ...
            needs_thumb = self._media.GetLocationsManager().IsLocal(
            ) and self._media.GetMime() in HC.MIMES_WITH_THUMBNAILS

        if needs_thumb:

            mime = self._media.GetMime()

            thumbnail_path = HG.client_controller.client_files_manager.GetThumbnailPath(
                self._media)

            self._thumbnail_qt_pixmap = ClientRendering.GenerateHydrusBitmap(
                thumbnail_path, mime).GetQtPixmap()
...
class OpenExternallyPanel(QW.QWidget):
    def __init__(self, parent, media):
        ...
        if self._media.GetLocationsManager().IsLocal() and self._media.GetMime(
        ) in HC.MIMES_WITH_THUMBNAILS:
            ...
            qt_pixmap = ClientRendering.GenerateHydrusBitmap(
                thumbnail_path, mime).GetQtPixmap()

            thumbnail_window = ClientGUICommon.BufferedWindowIcon(
                self, qt_pixmap)
    ...
    def mousePressEvent(self, event):
        ...
        if not (event.modifiers() &
                (QC.Qt.ShiftModifier | QC.Qt.ControlModifier
                 | QC.Qt.AltModifier)) and event.button() == QC.Qt.LeftButton:
    ...
    def paintEvent(self, event):
        ...
        background_colour = self._new_options.GetColour(
            CC.COLOUR_MEDIA_BACKGROUND)
