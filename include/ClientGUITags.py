...
class EditTagFilterPanel(ClientGUIScrolledPanels.EditPanel):
    ...
    def __init__(self,
        ...
        self._import_favourite = ClientGUICommon.BetterButton(
            self, 'import', self._ImportFavourite)
        self._export_favourite = ClientGUICommon.BetterButton(
            self, 'export', self._ExportFavourite)
        self._load_favourite = ClientGUICommon.BetterButton(
            self, 'load', self._LoadFavourite)
        self._save_favourite = ClientGUICommon.BetterButton(
            self, 'save', self._SaveFavourite)
        self._delete_favourite = ClientGUICommon.BetterButton(
            self, 'delete', self._DeleteFavourite)
        ...
        self._redundant_st = ClientGUICommon.BetterStaticText(
            self, '', ellipsize_end=True)

        self._current_filter_st = ClientGUICommon.BetterStaticText(
            self, 'currently keeping: ', ellipsize_end=True)

        self._test_result_st = ClientGUICommon.BetterStaticText(
            self, self.TEST_RESULT_DEFAULT)
        self._test_result_st.setAlignment(QC.Qt.AlignVCenter
                                          | QC.Qt.AlignRight)
        ...
        QP.AddToLayout(vbox, self._current_filter_st,
                       CC.FLAGS_EXPAND_PERPENDICULAR)

        hbox = QP.HBoxLayout()

        QP.AddToLayout(hbox, self._test_result_st,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        QP.AddToLayout(hbox, self._test_input,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        ...
        self._simple_whitelist_global_checkboxes.clicked.connect(
            self.EventSimpleWhitelistGlobalCheck)
        self._simple_whitelist_namespace_checkboxes.clicked.connect(
            self.EventSimpleWhitelistNamespaceCheck)
        self._simple_blacklist_global_checkboxes.clicked.connect(
            self.EventSimpleBlacklistGlobalCheck)
        self._simple_blacklist_namespace_checkboxes.clicked.connect(
            self.EventSimpleBlacklistNamespaceCheck)
    ...
    def _AdvancedAddBlacklist(self, tag_slice):
        ...
        if tag_slice in self._advanced_blacklist.GetClientData():
            ...
            if self._CurrentlyBlocked(tag_slice):
                ...
                self._ShowRedundantError(
                    ClientTags.ConvertTagSliceToString(tag_slice) +
                    ' is already blocked by a broader rule!')
    ...
    def _AdvancedAddWhitelist(self, tag_slice):
        ...
        if tag_slice in self._advanced_whitelist.GetClientData():
            ...
            if self._CurrentlyBlocked(tag_slice):
                ...
                self._ShowRedundantError(
                    ClientTags.ConvertTagSliceToString(tag_slice) +
                    ' is already permitted by a broader rule!')
    ...
    def _DeleteFavourite(self):
        def do_it(name):
            ...
            if name in names_to_tag_filters:
                ...
                HG.client_controller.new_options.SetFavouriteTagFilters(
        ...
        if len(names_to_tag_filters) == 0:
            ...
            for (name, tag_filter) in names_to_tag_filters.items():
                ...
                ClientGUIMenus.AppendMenuItem(menu, name,
                                              'delete {}'.format(name), do_it,
                                              name)
    ...
    def _ImportFavourite(self):
        ...
        if not isinstance(obj, ClientTags.TagFilter):
            ...
            QW.QMessageBox.critical(
                self, 'Error',
                'That object was not a Tag Filter! It seemed to be a "{}".'.
                format(type(obj)))
        ...
        with ClientGUIDialogs.DialogTextEntry(
                self, 'Enter a name for the favourite.') as dlg:
    ...
    def _InitAdvancedPanel(self):
        ...
        add_blacklist_button = ClientGUICommon.BetterButton(
            blacklist_panel, 'add', self._AdvancedAddBlacklistButton)
        ...
        self._advanced_add_whitelist_button = ClientGUICommon.BetterButton(
            whitelist_panel, 'add', self._AdvancedAddWhitelistButton)
        ...
        QP.AddToLayout(button_hbox, self._advanced_blacklist_input,
        ...
        QP.AddToLayout(button_hbox, blacklist_everything_button,
                       CC.FLAGS_VCENTER)
        ...
        QP.AddToLayout(button_hbox, self._advanced_whitelist_input,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(button_hbox, self._advanced_add_whitelist_button,
                       CC.FLAGS_VCENTER)
    ...
    def _InitBlacklistPanel(self):
        ...
        self._simple_blacklist_error_st = ClientGUICommon.BetterStaticText(
            blacklist_panel)

        self._simple_blacklist_global_checkboxes = QP.CheckListBox(
            blacklist_panel)

        self._simple_blacklist_global_checkboxes.Append(
            'unnamespaced tags', '')
        self._simple_blacklist_global_checkboxes.Append('namespaced tags', ':')

        self._simple_blacklist_namespace_checkboxes = QP.CheckListBox(
            blacklist_panel)
        ...
        for namespace in self._namespaces:
            ...
            self._simple_blacklist_namespace_checkboxes.Append(
                namespace, namespace + ':')

        self._simple_blacklist = ClientGUIListBoxes.ListBoxTagsCensorship(
            blacklist_panel, removed_callable=self._SimpleBlacklistRemoved)

        self._simple_blacklist_input = ClientGUIControls.TextAndPasteCtrl(
            blacklist_panel,
            self._SimpleAddBlacklistMultiple,
            allow_empty_input=True)
        ...
        QP.AddToLayout(left_vbox, self._simple_blacklist_global_checkboxes,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(left_vbox, (20, 20), CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(left_vbox, self._simple_blacklist_namespace_checkboxes,
                       CC.FLAGS_EXPAND_PERPENDICULAR)

        right_vbox = QP.VBoxLayout()

        QP.AddToLayout(right_vbox, self._simple_blacklist,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(right_vbox, self._simple_blacklist_input,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        QP.AddToLayout(vbox, self._simple_blacklist_error_st,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _InitWhitelistPanel(self):
        ...
        self._simple_whitelist_global_checkboxes = QP.CheckListBox(
            whitelist_panel)
        ...
        self._simple_whitelist_namespace_checkboxes = QP.CheckListBox(
            whitelist_panel)
        ...
        QP.AddToLayout(left_vbox, self._simple_whitelist_global_checkboxes,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(left_vbox, (20, 20), CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(left_vbox, self._simple_whitelist_namespace_checkboxes,
                       CC.FLAGS_EXPAND_PERPENDICULAR)

        right_vbox = QP.VBoxLayout()

        QP.AddToLayout(right_vbox, self._simple_whitelist,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(right_vbox, self._simple_whitelist_input,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        QP.AddToLayout(vbox, self._simple_whitelist_error_st,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, main_hbox, CC.FLAGS_EXPAND_BOTH_WAYS)
    ...
    def _ShowRedundantError(self, text):
        ...
        HG.client_controller.CallLaterQtSafe(self._redundant_st, 2,
                                             self._redundant_st.setText, '')
    ...
    def _SimpleAddWhitelistMultiple(self, tag_slices):
        ...
        for tag_slice in tag_slices:
            ...
            if tag_slice in (
                    '', ':'
            ) and tag_slice in self._simple_whitelist.GetClientData():
    ...
    def _UpdateStatus(self):
        ...
        if whitelist_possible:
            ...
            for index in range(
                    self._simple_whitelist_global_checkboxes.count()):

                check = QP.GetClientData(
                    self._simple_whitelist_global_checkboxes,
                    index) in whitelist_tag_slices

                self._simple_whitelist_global_checkboxes.Check(index, check)

            for index in range(
                    self._simple_whitelist_namespace_checkboxes.count()):

                check = QP.GetClientData(
                    self._simple_whitelist_namespace_checkboxes,
                    index) in whitelist_tag_slices
            ...
            self._simple_whitelist_error_st.setText(
                'The filter is currently more complicated than a simple whitelist, so cannot be shown here.'
            )
        ...
        if blacklist_possible:
            ...
            for index in range(
                    self._simple_blacklist_global_checkboxes.count()):

                check = QP.GetClientData(
                    self._simple_blacklist_global_checkboxes,
                    index) in blacklist_tag_slices

                self._simple_blacklist_global_checkboxes.Check(index, check)

            for index in range(
                    self._simple_blacklist_namespace_checkboxes.count()):

                check = QP.GetClientData(
                    self._simple_blacklist_namespace_checkboxes,
                    index) in blacklist_tag_slices
            ...
            self._simple_blacklist_error_st.setText(
                'The filter is currently more complicated than a simple blacklist, so cannot be shown here.'
            )
        ...
        self._current_filter_st.setText('currently keeping: ' +
                                        pretty_tag_filter)
    ...
    def EventSimpleBlacklistNamespaceCheck(self, index):
        ...
        if index != -1:
            ...
            tag_slice = QP.GetClientData(
                self._simple_blacklist_namespace_checkboxes, index)
    ...
    def EventSimpleBlacklistGlobalCheck(self, index):
        ...
        if index != -1:
            ...
            tag_slice = QP.GetClientData(
                self._simple_blacklist_global_checkboxes, index)
    ...
    def EventSimpleWhitelistNamespaceCheck(self, index):
        ...
        if index != -1:
            ...
            tag_slice = QP.GetClientData(
                self._simple_whitelist_namespace_checkboxes, index)
    ...
    def EventSimpleWhitelistGlobalCheck(self, index):
        ...
        if index != -1:
            ...
            tag_slice = QP.GetClientData(
                self._simple_whitelist_global_checkboxes, index)
...
class ManageTagsPanel(ClientGUIScrolledPanels.ManagePanel):
    def __init__(self,
        ...
        for service in services:
            ...
            page = self._Panel(self._tag_repositories,
            ...
            if select:
                self._tag_repositories.setCurrentIndex(
                    self._tag_repositories.count() - 1)
        ...
        if self._canvas_key is not None:
            ...
            HG.client_controller.sub(self, 'CanvasHasNewMedia',
    ...
    def _GetGroupsOfServiceKeysToContentUpdates(self):
        ...
        for page in self._tag_repositories.GetPages():
            ...
            (service_key,
             groups_of_content_updates) = page.GetGroupsOfContentUpdates()
            ...
            for content_updates in groups_of_content_updates:
                ...
                if len(content_updates) > 0:
                    ...
                    groups_of_service_keys_to_content_updates.append(
                        service_keys_to_content_updates)
    ...
    def EventServiceChanged(self, index):
        ...
        if not self or not QP.isValid(
                self
        ):  # actually did get a runtime error here, on some Linux WM dialog shutdown
    ...
    class _Panel(QW.QWidget):
        ...
        def __init__(self,
            ...
            self._i_am_local_tag_service = self._service.GetServiceType(
            ) == HC.LOCAL_TAG

            self._tags_box_sorter = ClientGUICommon.StaticBoxSorterForListBoxTags(
                self, 'tags')

            self._tags_box = ClientGUIListBoxes.ListBoxTagsSelectionTagsDialog(
                self._tags_box_sorter, self.EnterTags, self.RemoveTags)
            ...
            self._remove_tags = ClientGUICommon.BetterButton(
                self._tags_box_sorter, text, self._RemoveTagsButton)

            menu_items = []

            call = HydrusData.Call(self._DoSiblingsAndParents,
                                   self._tag_service_key)

            menu_items.append((
                'normal',
                'Hard-replace all applicable tags with their siblings and add missing parents. (Just this service\'s siblings and parents)',
                'Fix siblings and parents.', call))
            ...
            menu_items.append((
                'normal',
                'Hard-replace all applicable tags with their siblings and add missing parents. (All service siblings and parents)',
                'Fix siblings and parents.', call))

            self._do_siblings_and_parents = ClientGUICommon.MenuBitmapButton(
                self._tags_box_sorter, CC.GlobalPixmaps.family, menu_items)
            self._do_siblings_and_parents.setToolTip(
                'Hard-replace all applicable tags with their siblings and add missing parents.'
            )

            self._copy_button = ClientGUICommon.BetterBitmapButton(
                self._tags_box_sorter, CC.GlobalPixmaps.copy, self._Copy)
            self._copy_button.setToolTip(
                'Copy selected tags to the clipboard. If none are selected, copies all.'
            )

            self._paste_button = ClientGUICommon.BetterBitmapButton(
                self._tags_box_sorter, CC.GlobalPixmaps.paste, self._Paste)
            self._paste_button.setToolTip(
                'Paste newline-separated tags from the clipboard into here.')
            ...
            check_manager = ClientGUICommon.CheckboxManagerOptions(
                'add_parents_on_manage_tags')

            menu_items.append((
                'check', 'auto-add entered tags\' parents on add/pend action',
                'If checked, adding any tag that has parents will also add those parents.',
                check_manager))

            check_manager = ClientGUICommon.CheckboxManagerOptions(
                'replace_siblings_on_manage_tags')

            menu_items.append((
                'check', 'auto-replace entered siblings on add/pend action',
                'If checked, adding any tag that has a sibling will instead add that sibling.',
                check_manager))

            check_manager = ClientGUICommon.CheckboxManagerOptions(
                'allow_remove_on_manage_tags_input')

            menu_items.append((
                'check',
                'allow remove/petition result on tag input for already existing tag',
                'If checked, inputting a tag that already exists will try to remove it.',
                check_manager))

            check_manager = ClientGUICommon.CheckboxManagerOptions(
                'yes_no_on_remove_on_manage_tags')

            menu_items.append((
                'check',
                'confirm remove/petition tags on explicit delete actions',
                'If checked, clicking the remove/petition tags button (or hitting the deleted key on the list) will first confirm the action with a yes/no dialog.',
                check_manager))

            check_manager = ClientGUICommon.CheckboxManagerCalls(
                self._FlipShowDeleted, lambda: self._show_deleted)

            menu_items.append(('check', 'show deleted',
                               'Show deleted tags, if any.', check_manager))

            menu_items.append(('separator', 0, 0, 0))

            menu_items.append((
                'normal', 'migrate tags for these files',
                'Migrate the tags for the files used to launch this manage tags panel.',
                self._MigrateTags))

            if not self._i_am_local_tag_service and self._service.HasPermission(
                    HC.CONTENT_TYPE_ACCOUNTS, HC.PERMISSION_ACTION_OVERRULE):

                menu_items.append(('separator', 0, 0, 0))

                menu_items.append(
                    ('normal', 'modify users who added the selected tags',
                     'Modify the users who added the selected tags.',
                     self._ModifyMappers))

            self._cog_button = ClientGUICommon.MenuBitmapButton(
                self._tags_box_sorter, CC.GlobalPixmaps.cog, menu_items)

            #

            expand_parents = self._new_options.GetBoolean(
                'add_parents_on_manage_tags')

            self._add_tag_box = ClientGUIACDropdown.AutoCompleteDropdownTagsWrite(
                self,
                self.AddTags,
                expand_parents,
                self._file_service_key,
            ...
            self._suggested_tags = ClientGUITagSuggestions.SuggestedTagsPanel(
                self,
                self._tag_service_key,
                self._media,
                self.AddTags,
            ...
            QP.AddToLayout(button_hbox, self._do_siblings_and_parents,
                           CC.FLAGS_VCENTER)
            ...
            QP.AddToLayout(button_hbox, self._cog_button,
                           CC.FLAGS_SIZER_CENTER)

            self._tags_box_sorter.Add(button_hbox, CC.FLAGS_BUTTON_SIZER)

            vbox = QP.VBoxLayout()

            QP.AddToLayout(vbox, self._tags_box_sorter,
                           CC.FLAGS_EXPAND_BOTH_WAYS)
            ...
            QP.AddToLayout(hbox, self._suggested_tags,
                           CC.FLAGS_EXPAND_BOTH_WAYS_POLITE)
            ...
            HG.client_controller.sub(self, 'CheckboxExpandParents',
                                     'checkbox_manager_inverted')
        ...
        def _EnterTags(self,
            ...
            currents = [
                tags_manager.GetCurrent(self._tag_service_key,
                                        ClientTags.TAG_DISPLAY_STORAGE)
                for tags_manager in tags_managers
            ]
            pendings = [
                tags_manager.GetPending(self._tag_service_key,
                                        ClientTags.TAG_DISPLAY_STORAGE)
                for tags_manager in tags_managers
            ]
            petitioneds = [
                tags_manager.GetPetitioned(self._tag_service_key,
                                           ClientTags.TAG_DISPLAY_STORAGE)
                for tags_manager in tags_managers
            ]
            ...
            for tag in tags:
                ...
                num_current = sum(
                    (1 for current in currents if tag in current))
                ...
                if self._i_am_local_tag_service:
                    ...
                    if not only_remove:
                        ...
                        if num_current < num_files:
                            ...
                            choices[HC.CONTENT_UPDATE_ADD].append(
                                (tag, num_non_current))
                    ...
                    if not only_add:
                        ...
                        if num_current > 0:
                            ...
                            choices[HC.CONTENT_UPDATE_DELETE].append(
                                (tag, num_current))

                else:

                    num_pending = sum(
                        (1 for pending in pendings if tag in pending))
                    num_petitioned = sum(
                        (1 for petitioned in petitioneds if tag in petitioned))

                    if not only_remove:

                        if num_current + num_pending < num_files:

                            num_pendable = num_files - (num_current +
                                                        num_pending)

                            choices[HC.CONTENT_UPDATE_PEND].append(
                                (tag, num_pendable))
                    ...
                    if not only_add:
                        ...
                        if num_current > num_petitioned and not only_add:
                            ...
                            choices[HC.CONTENT_UPDATE_PETITION].append(
                                (tag, num_petitionable))

                        if num_pending > 0 and not only_add:

                            choices[HC.CONTENT_UPDATE_RESCIND_PEND].append(
                                (tag, num_pending))
                    ...
                    if not only_remove:
                        ...
                        if num_petitioned > 0:
                            ...
                            choices[HC.CONTENT_UPDATE_RESCIND_PETITION].append(
                                (tag, num_petitioned))
            ...
            if len(choices) == 1:
                ...
                preferred_order = [
                    HC.CONTENT_UPDATE_ADD, HC.CONTENT_UPDATE_DELETE,
                ...
                choice_text_lookup[
                    HC.CONTENT_UPDATE_RESCIND_PEND] = 'rescind pend'
                choice_text_lookup[
                    HC.CONTENT_UPDATE_RESCIND_PETITION] = 'rescind petition'
                ...
                for choice_action in preferred_order:
                    ...
                    if len(tags) == 1:
                        ...
                        text = choice_text_prefix + ' "' + HydrusText.ElideText(
                            tag, 64) + '" for ' + HydrusData.ToHumanInt(
                                count) + ' files'

                    else:

                        text = choice_text_prefix + ' ' + HydrusData.ToHumanInt(
                            len(tags)) + ' tags'
                    ...
                    if len(tag_counts) > 25:
                        ...
                        t_c_lines = [
                            tag + ' - ' + HydrusData.ToHumanInt(count) +
                            ' files' for (tag, count) in t_c
                        ]

                        t_c_lines.append(
                            'and ' +
                            HydrusData.ToHumanInt(len(tag_counts) - 25) +
                            ' others')
                        ...
                        tooltip = os.linesep.join(
                            (tag + ' - ' + HydrusData.ToHumanInt(count) +
                             ' files' for (tag, count) in tag_counts))
                ...
                try:
                    ...
                    (choice_action,
                     tags) = ClientGUIDialogsQuick.SelectFromListButtons(
                         self, 'What would you like to do?', bdc_choices)
            ...
            if choice_action == HC.CONTENT_UPDATE_PETITION:
                ...
                if forced_reason is None:
                    ...
                    if len(tags) == 1:
                        ...
                        tag_text = 'the ' + HydrusData.ToHumanInt(
                            len(tags)) + ' tags'
                    ...
                    with ClientGUIDialogs.DialogTextEntry(
                            self, message, suggestions=suggestions) as dlg:
            ...
            for tag in tags:
                ...
                if choice_action == HC.CONTENT_UPDATE_ADD:
                    media_to_affect = [
                        m for m in self._media
                        if tag not in m.GetTagsManager().GetCurrent(
                            self._tag_service_key,
                            ClientTags.TAG_DISPLAY_STORAGE)
                    ]
                    ...
                    media_to_affect = [
                        m for m in self._media
                        if tag in m.GetTagsManager().GetCurrent(
                            self._tag_service_key,
                            ClientTags.TAG_DISPLAY_STORAGE)
                    ]
                    ...
                    media_to_affect = [
                        ...
                        if tag not in m.GetTagsManager().GetCurrent(
                            self._tag_service_key,
                            ClientTags.TAG_DISPLAY_STORAGE)
                        and tag not in m.GetTagsManager().GetPending(
                            self._tag_service_key,
                            ClientTags.TAG_DISPLAY_STORAGE)
                    ]
                    ...
                    media_to_affect = [
                        ...
                        if tag in m.GetTagsManager().GetCurrent(
                            self._tag_service_key,
                            ClientTags.TAG_DISPLAY_STORAGE)
                        and tag not in m.GetTagsManager().GetPetitioned(
                            self._tag_service_key,
                            ClientTags.TAG_DISPLAY_STORAGE)
                    ]
                    ...
                    media_to_affect = [
                        m for m in self._media
                        if tag in m.GetTagsManager().GetPending(
                            self._tag_service_key,
                            ClientTags.TAG_DISPLAY_STORAGE)
                    ]
                    ...
                    media_to_affect = [
                        m for m in self._media
                        if tag in m.GetTagsManager().GetPetitioned(
                            self._tag_service_key,
                            ClientTags.TAG_DISPLAY_STORAGE)
                    ]
                ...
                hashes = set(
                    itertools.chain.from_iterable(
                        (m.GetHashes() for m in media_to_affect)))

                if len(hashes) > 0:

                    content_updates = []

                    if choice_action in (HC.CONTENT_UPDATE_ADD,
                                         HC.CONTENT_UPDATE_PEND):

                        if self._new_options.GetBoolean(
                                'replace_siblings_on_manage_tags'):

                            siblings_manager = HG.client_controller.tag_siblings_manager

                            tag = siblings_manager.CollapseTag(
                        ...
                        if self._new_options.GetBoolean(
                            ...
                            parents = tag_parents_manager.GetParents(
                                self._tag_service_key, tag)

                            content_updates.extend((HydrusData.ContentUpdate(
                                HC.CONTENT_TYPE_MAPPINGS, choice_action,
                                (parent, hashes)) for parent in parents))

                    content_updates.append(
                        HydrusData.ContentUpdate(HC.CONTENT_TYPE_MAPPINGS,
                                                 choice_action, (tag, hashes),
                                                 reason=reason))
                        ...
                        if not self._immediate_commit:
                            ...
                            for m in media_to_affect:
                                ...
                                for content_update in content_updates:
                                    ...
                                    mt.ProcessContentUpdate(
                                        self._tag_service_key, content_update)

                        content_updates_group.extend(content_updates)

            if len(
                    recent_tags
            ) > 0 and HG.client_controller.new_options.GetNoneableInteger(
                    'num_recent_tags') is not None:

                HG.client_controller.Write('push_recent_tags',
                                           self._tag_service_key, recent_tags)
            ...
            if len(content_updates_group) > 0:
                ...
                if self._immediate_commit:
                    ...
                    service_keys_to_content_updates = {
                        self._tag_service_key: content_updates_group
                    }
                    ...
                    self._groups_of_content_updates.append(
                        content_updates_group)
        ...
        def _MigrateTags(self):
            ...
            def do_it(tag_service_key, hashes):
                ...
                frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
                    HG.client_controller.gui, 'tag migration')

                panel = ClientGUIScrolledPanelsReview.MigrateTagsPanel(
                    frame, self._tag_service_key, hashes)
            ...
            if len(tags) == 0:
                ...
                (current_tags_to_count, deleted_tags_to_count,
                 ...
                 petitioned_tags_to_count) = ClientMedia.GetMediasTagCount(
                     self._media, self._tag_service_key,
                     ClientTags.TAG_DISPLAY_STORAGE)

                tags = set(current_tags_to_count.keys()).union(
                    pending_tags_to_count.keys())
        ...
        def _DoSiblingsAndParents(self, service_key):
            ...
            for m in self._media:
                ...
                tags = m.GetTagsManager().GetCurrentAndPending(
                    self._tag_service_key, ClientTags.TAG_DISPLAY_STORAGE)

                sibling_correct_tags = tag_siblings_manager.CollapseTags(
                    service_key, tags, service_strict=True)

                sibling_and_parent_correct_tags = tag_parents_manager.ExpandTags(
                    service_key, sibling_correct_tags, service_strict=True)
            ...
            if len(removee_tags_to_hashes) == 0 and len(
                    addee_tags_to_hashes) == 0:

                QW.QMessageBox.information(
                    self, 'Information', 'No replacements seem to be needed.')
            ...
            if len(removee_tags_to_hashes) > 0:
                ...
                ClientTags.SortTags(CC.SORT_BY_INCIDENCE_DESC, removee_tags,
                                    removee_tags_to_counts)

                removee_tag_statements = [
                    '{} ({})'.format(tag, removee_tags_to_counts[tag])
                    for tag in removee_tags
                ]
                ...
                summary_message += 'REMOVE: ' + ', '.join(
                    removee_tag_statements)

            if len(addee_tags_to_hashes) > 0:

                addee_tags_to_counts = {
                    tag: len(hashes)
                ...
                ClientTags.SortTags(CC.SORT_BY_INCIDENCE_DESC, addee_tags,
                                    addee_tags_to_counts)

                addee_tag_statements = [
                    '{} ({})'.format(tag, addee_tags_to_counts[tag])
                    for tag in addee_tags
                ]
            ...
            for (tag, hashes) in addee_tags_to_hashes.items():
                ...
                content_updates.append(
                    HydrusData.ContentUpdate(HC.CONTENT_TYPE_MAPPINGS,
                                             addee_action, (tag, hashes),
                                             reason=reason))

            if not self._immediate_commit:

                hashes_to_tag_managers = {
                    m.GetHash(): m.GetTagsManager()
                    for m in self._media
                }
                ...
                for content_update in content_updates:
                    ...
                    for hash in hashes:
                        ...
                        if hash in hashes_to_tag_managers:
                            ...
                            hashes_to_tag_managers[hash].ProcessContentUpdate(
                                self._tag_service_key, content_update)
        ...
        def _ModifyMappers(self):
            ...
            hashes = set(
                itertools.chain.from_iterable(
                    (m.GetHashes() for m in self._media)))

            for tag in tags:

                contents.extend([
                    HydrusNetwork.Content(HC.CONTENT_TYPE_MAPPING, (tag, hash))
                    for hash in hashes
                ])
            ...
            if len(contents) > 0:
                ...
                with ClientGUIDialogs.DialogModifyAccounts(
                        self, self._tag_service_key, subject_accounts) as dlg:
        ...
        def _RemoveTagsButton(self):
            ...
            if len(selected_tags) == 0:
                ...
                tags_to_remove = [
                    tag for tag in selected_tags if tag in removable_tags
                ]
        ...
        def AddTags(self, tags, only_add=False):
            ...
            if not self._new_options.GetBoolean(
                    'allow_remove_on_manage_tags_input'):
        ...
        def CheckboxExpandParents(self):
            ...
            self._add_tag_box.SetExpandParents(
                self._new_options.GetBoolean('add_parents_on_manage_tags'))
            ...
            if command_type == CC.APPLICATION_COMMAND_TYPE_SIMPLE:
                ...
                if action == 'set_search_focus':
                    ...
                    self.SetTagBoxFocus()
                        ...
                        'show_and_focus_manage_tags_favourite_tags',
                        'show_and_focus_manage_tags_related_tags',
                        'show_and_focus_manage_tags_file_lookup_script_tags',
                        'show_and_focus_manage_tags_recent_tags'):
            ...
            for (service_key, content_updates) in list(
                ...
                for content_update in content_updates:
                    ...
                    for m in self._media:
                        ...
                        if len(m.GetHashes().intersection(
                                content_update.GetHashes())) > 0:

                            m.GetMediaResult().ProcessContentUpdate(
                                service_key, content_update)
                ...
                if self._new_options.GetBoolean(
                    ...
                    if len(tags) < 10:
                        ...
                        message += os.linesep.join(
                            (HydrusText.ElideText(tag, 64) for tag in tags))

                    else:

                        message = 'Are you sure you want to remove these ' + HydrusData.ToHumanInt(
                            len(tags)) + ' tags?'
...
class ManageTagParents(ClientGUIScrolledPanels.ManagePanel):
    def __init__(self, parent, tags=None):
        ...
        services = [
            ...
            for service in HG.client_controller.services_manager.GetServices((
                HC.TAG_REPOSITORY, )) if service.HasPermission(
        ...
        if self._tag_repositories.currentWidget().HasUncommittedPair():
            ...
            if result != QW.QDialog.Accepted:
                ...
                raise HydrusExceptions.VetoException(
                    'Cancelled OK due to uncommitted pair.')
    ...
    class _Panel(QW.QWidget):
        def __init__(self, parent, service_key, tags=None):
            ...
            self._tag_parents = ClientGUIListCtrl.BetterListCtrl(
                listctrl_panel,
                'tag_parents',
                8,
                25,
                columns,
                self._ConvertPairToListCtrlTuples,
                delete_key_callback=self._ListCtrlActivated,
                activation_callback=self._ListCtrlActivated)
            ...
            menu_items.append(('normal', 'from clipboard',
                               'Load parents from text in your clipboard.',
                               HydrusData.Call(self._ImportFromClipboard,
                                               False)))
            menu_items.append(
                ('normal', 'from clipboard (only add pairs--no deletions)',
                 'Load parents from text in your clipboard.',
                 HydrusData.Call(self._ImportFromClipboard, True)))
            menu_items.append(
                ('normal', 'from .txt file', 'Load parents from a .txt file.',
                 HydrusData.Call(self._ImportFromTXT, False)))
            menu_items.append(
                ('normal', 'from .txt file (only add pairs--no deletions)',
                 'Load parents from a .txt file.',
                 HydrusData.Call(self._ImportFromTXT, True)))

            listctrl_panel.AddMenuButton('import', menu_items)

            menu_items = []

            menu_items.append(('normal', 'to clipboard',
                               'Save selected parents to your clipboard.',
                               self._ExportToClipboard))
            menu_items.append(
                ('normal', 'to .txt file',
                 'Save selected parents to a .txt file.', self._ExportToTXT))

            listctrl_panel.AddMenuButton('export',
                                         menu_items,
                                         enabled_only_on_selection=True)

            self._children = ClientGUIListBoxes.ListBoxTagsStringsAddRemove(
                self, self._service_key, show_sibling_text=False)
            self._parents = ClientGUIListBoxes.ListBoxTagsStringsAddRemove(
                self, self._service_key, show_sibling_text=False)

            (gumpf, preview_height) = ClientGUIFunctions.ConvertTextToPixels(
                self._children, (12, 6))
            ...
            self._child_input = ClientGUIACDropdown.AutoCompleteDropdownTagsWrite(
                self,
                self.EnterChildren,
                expand_parents,
            ...
            self._parent_input = ClientGUIACDropdown.AutoCompleteDropdownTagsWrite(
                self,
                self.EnterParents,
                expand_parents,
                CC.LOCAL_FILE_SERVICE_KEY,
            ...
            self._status_st = ClientGUICommon.BetterStaticText(
                self, 'initialising\u2026' + os.linesep + '.')
            ...
            QP.AddToLayout(vbox, self._status_st,
                           CC.FLAGS_EXPAND_PERPENDICULAR)
            QP.AddToLayout(vbox, self._count_st, CC.FLAGS_EXPAND_PERPENDICULAR)
            QP.AddToLayout(
                vbox,
                ClientGUICommon.WrapInText(self._show_all, self,
                                           'show all pairs'),
                CC.FLAGS_EXPAND_PERPENDICULAR)
            ...
            self._tag_parents.itemSelectionChanged.connect(
        ...
        def _AddPairs(self, pairs, add_only=False):
            ...
            for pair in pairs:
                ...
                if pair in self._current_statuses_to_pairs[
                ...
                elif pair in self._current_statuses_to_pairs[
                        HC.CONTENT_STATUS_PETITIONED]:

                    petitioned_pairs.append(pair)

                elif pair in self._original_statuses_to_pairs[
            ...
            suggestions.append(
                'character/person/etc... properties (character x is a female)')
            ...
            if len(new_pairs) > 0:
                ...
                if not self._i_am_local_tag_service:
                    ...
                    if self._service.HasPermission(
                ...
                if do_it:
                    ...
                    self._current_statuses_to_pairs[
                        HC.CONTENT_STATUS_PENDING].update(new_pairs)
                ...
                if len(current_pairs) > 0:
                    ...
                    if not self._i_am_local_tag_service:
                        ...
                        if len(current_pairs) > 10:
                            ...
                            pair_strings = os.linesep.join(
                                (child + '->' + parent
                                 for (child, parent) in current_pairs))
                        ...
                        if result == QW.QDialog.Accepted:
                            ...
                            if self._service.HasPermission(
                                ...
                                with ClientGUIDialogs.DialogTextEntry(
                                        self, message,
                                        suggestions=suggestions) as dlg:
                    ...
                    if do_it:
                        ...
                        self._current_statuses_to_pairs[
                            HC.CONTENT_STATUS_PETITIONED].update(current_pairs)
                ...
                if len(pending_pairs) > 0:
                    ...
                    if len(pending_pairs) > 10:
                        ...
                        pair_strings = os.linesep.join(
                            (child + '->' + parent
                             for (child, parent) in pending_pairs))
                    ...
                    result = ClientGUIDialogsQuick.GetYesNo(
                        ...
                        message,
                        title='Choose what to do.',
                        yes_label='rescind the pend',
                        no_label='do nothing')

                    if result == QW.QDialog.Accepted:

                        self._current_statuses_to_pairs[
                            HC.CONTENT_STATUS_PENDING].difference_update(
                                pending_pairs)
                ...
                if len(petitioned_pairs) > 0:
                    ...
                    if len(petitioned_pairs) > 10:
                        ...
                        pair_strings = os.linesep.join(
                            (child + '->' + parent
                             for (child, parent) in petitioned_pairs))
                    ...
                    result = ClientGUIDialogsQuick.GetYesNo(
                        ...
                        message,
                        title='Choose what to do.',
                        yes_label='rescind the petition',
                        no_label='do nothing')

                    if result == QW.QDialog.Accepted:

                        self._current_statuses_to_pairs[
                            HC.CONTENT_STATUS_PETITIONED].difference_update(
                                petitioned_pairs)
            ...
            if len(affected_pairs) > 0:
                ...
                def in_current(pair):
                    ...
                    for status in (HC.CONTENT_STATUS_CURRENT,
                                   HC.CONTENT_STATUS_PENDING,
                                   HC.CONTENT_STATUS_PETITIONED):
                ...
                affected_pairs = [(self._tag_parents.HasData(pair),
                                   in_current(pair), pair)
                                  for pair in affected_pairs]

                to_add = [
                    pair for (exists, current, pair) in affected_pairs
                    if not exists
                ]
                to_update = [
                    pair for (exists, current, pair) in affected_pairs
        ...
        def _CanAdd(self, potential_pair):
            ...
            current_pairs = self._current_statuses_to_pairs[
                HC.CONTENT_STATUS_CURRENT].union(
                    self._current_statuses_to_pairs[
                        HC.CONTENT_STATUS_PENDING]).difference(
                            self._current_statuses_to_pairs[
                                HC.CONTENT_STATUS_PETITIONED])
            ...
            if potential_parent in current_children:
                ...
                simple_children_to_parents = ClientManagers.BuildSimpleChildrenToParents(
                    current_pairs)

                if ClientManagers.LoopInSimpleChildrenToParents(
                        simple_children_to_parents, potential_child,
                        potential_parent):

                    QW.QMessageBox.critical(
                        self, 'Error', 'Adding ' + potential_child + '->' +
                        potential_parent + ' would create a loop!')
        ...
        def _ExportToTXT(self):
            ...
            with QP.FileDialog(self,
                               'Set the export path.',
                               defaultFile='parents.txt',
                               acceptMode=QW.QFileDialog.AcceptSave) as dlg:
        ...
        def _SetButtonStatus(self):
            ...
            if len(self._children.GetTags()) == 0 or len(
                    self._parents.GetTags()) == 0:
        ...
        def _UpdateListCtrlData(self):
            ...
            for (status,
                 pairs) in list(self._current_statuses_to_pairs.items()):
        ...
        def GetContentUpdates(self):
            ...
            if self._i_am_local_tag_service:
                ...
                for pair in self._current_statuses_to_pairs[
                        HC.CONTENT_STATUS_PENDING]:
                    content_updates.append(
                        HydrusData.ContentUpdate(HC.CONTENT_TYPE_TAG_PARENTS,
                                                 HC.CONTENT_UPDATE_ADD, pair))
                for pair in self._current_statuses_to_pairs[
                        HC.CONTENT_STATUS_PETITIONED]:
                    content_updates.append(
                        HydrusData.ContentUpdate(HC.CONTENT_TYPE_TAG_PARENTS,
                ...
                current_pending = self._current_statuses_to_pairs[
                    HC.CONTENT_STATUS_PENDING]
                original_pending = self._original_statuses_to_pairs[
                    HC.CONTENT_STATUS_PENDING]

                current_petitioned = self._current_statuses_to_pairs[
                    HC.CONTENT_STATUS_PETITIONED]
                original_petitioned = self._original_statuses_to_pairs[
                    HC.CONTENT_STATUS_PETITIONED]

                new_pends = current_pending.difference(original_pending)
                rescinded_pends = original_pending.difference(current_pending)

                new_petitions = current_petitioned.difference(
                    original_petitioned)
                rescinded_petitions = original_petitioned.difference(
                    current_petitioned)
                ...
                content_updates.extend((HydrusData.ContentUpdate(
                    ...
                    reason=self._pairs_to_reasons[pair])
                                        for pair in new_pends))
                content_updates.extend(
                    (HydrusData.ContentUpdate(HC.CONTENT_TYPE_TAG_PARENTS,
                                              HC.CONTENT_UPDATE_RESCIND_PEND,
                                              pair)
                     for pair in rescinded_pends))
                content_updates.extend((HydrusData.ContentUpdate(
                    HC.CONTENT_TYPE_TAG_PARENTS,
                    HC.CONTENT_UPDATE_PETITION,
                    pair,
                    reason=self._pairs_to_reasons[pair])
                                        for pair in new_petitions))
                content_updates.extend((HydrusData.ContentUpdate(
                    HC.CONTENT_TYPE_TAG_PARENTS,
                    HC.CONTENT_UPDATE_RESCIND_PETITION, pair)
                                        for pair in rescinded_petitions))

            return (self._service_key, content_updates)

        def HasUncommittedPair(self):

            return len(self._children.GetTags()) > 0 and len(
                self._parents.GetTags()) > 0

        def SetTagBoxFocus(self):

            if len(self._children.GetTags()) == 0:
                self._child_input.setFocus(QC.Qt.OtherFocusReason)
            else:
                self._parent_input.setFocus(QC.Qt.OtherFocusReason)
        ...
        def THREADInitialise(self, tags, service_key):
            def qt_code(original_statuses_to_pairs, current_statuses_to_pairs):
                ...
                self._status_st.setText(
                    'Files with a tag on the left will also be given the tag on the right.'
                    + os.linesep +
                    'As an experiment, this panel will only display the \'current\' pairs for those tags entered below.'
                )
                self._count_st.setText('Starting with ' +
                                       HydrusData.ToHumanInt(
                                           len(original_statuses_to_pairs[
                                               HC.CONTENT_STATUS_CURRENT])) +
                                       ' pairs.')
            ...
            original_statuses_to_pairs = HG.client_controller.Read(
                'tag_parents', service_key)

            current_statuses_to_pairs = collections.defaultdict(set)

            current_statuses_to_pairs.update({
                key: set(value)
                for (key, value) in list(original_statuses_to_pairs.items())
            })

            QP.CallAfter(qt_code, original_statuses_to_pairs,
                         current_statuses_to_pairs)
...
class ManageTagSiblings(ClientGUIScrolledPanels.ManagePanel):
    def __init__(self, parent, tags=None):
        ...
        services = [
            ...
            for service in HG.client_controller.services_manager.GetServices((
                HC.TAG_REPOSITORY, ))
            if service.HasPermission(HC.CONTENT_TYPE_TAG_SIBLINGS,
        ...
        for service in services:
            ...
            if select:
                self._tag_repositories.setCurrentIndex(
                    self._tag_repositories.indexOf(page))
    ...
    class _Panel(QW.QWidget):
        def __init__(self, parent, service_key, tags=None):
            ...
            self._tag_siblings = ClientGUIListCtrl.BetterListCtrl(
                listctrl_panel,
                'tag_siblings',
                8,
                40,
            ...
            menu_items.append(('normal', 'from clipboard',
                               'Load siblings from text in your clipboard.',
                               HydrusData.Call(self._ImportFromClipboard,
                                               False)))
            menu_items.append(
                ('normal', 'from clipboard (only add pairs--no deletions)',
                 ...
                 HydrusData.Call(self._ImportFromClipboard, True)))
            menu_items.append(
                ('normal', 'from .txt file', 'Load siblings from a .txt file.',
                 HydrusData.Call(self._ImportFromTXT, False)))
            menu_items.append(
                ('normal', 'from .txt file (only add pairs--no deletions)',
                 'Load siblings from a .txt file.',
                 HydrusData.Call(self._ImportFromTXT, True)))
            ...
            menu_items.append(('normal', 'to clipboard',
                               'Save selected siblings to your clipboard.',
                               self._ExportToClipboard))
            menu_items.append(
                ('normal', 'to .txt file',
                 'Save selected siblings to a .txt file.', self._ExportToTXT))

            listctrl_panel.AddMenuButton('export',
                                         menu_items,
                                         enabled_only_on_selection=True)

            self._old_siblings = ClientGUIListBoxes.ListBoxTagsStringsAddRemove(
                self, self._service_key, show_sibling_text=False)
            self._new_sibling = ClientGUICommon.BetterStaticText(self)

            (gumpf, preview_height) = ClientGUIFunctions.ConvertTextToPixels(
                self._old_siblings, (12, 6))

            self._old_siblings.setMinimumHeight(preview_height)

            expand_parents = False

            self._old_input = ClientGUIACDropdown.AutoCompleteDropdownTagsWrite(
                self,
                self.EnterOlds,
                expand_parents,
                CC.LOCAL_FILE_SERVICE_KEY,
            ...
            self._new_input = ClientGUIACDropdown.AutoCompleteDropdownTagsWrite(
                self, self.SetNew, expand_parents, CC.LOCAL_FILE_SERVICE_KEY,
                service_key)
            ...
            QP.AddToLayout(new_sibling_box, (10, 10),
                           CC.FLAGS_EXPAND_BOTH_WAYS)
            QP.AddToLayout(new_sibling_box, self._new_sibling,
                           CC.FLAGS_EXPAND_PERPENDICULAR)
            QP.AddToLayout(new_sibling_box, (10, 10),
                           CC.FLAGS_EXPAND_BOTH_WAYS)
            ...
            QP.AddToLayout(text_box, self._old_siblings,
                           CC.FLAGS_EXPAND_BOTH_WAYS)
            QP.AddToLayout(text_box, new_sibling_box,
            ...
            self._tag_siblings.itemSelectionChanged.connect(
                self._SetButtonStatus)
        ...
        def _AddPairs(self,
            ...
            pairs.sort(
                key=lambda c_p1: HydrusTags.ConvertTagToSortable(c_p1[1]))
            ...
            if len(new_pairs) > 0:
                ...
                if not self._i_am_local_tag_service:
                    ...
                    if default_reason is not None:
                    ...
                    elif self._service.HasPermission(
                        ...
                        if len(new_pairs) > 10:
                            ...
                            pair_strings = os.linesep.join(
                                (old + '->' + new for (old, new) in new_pairs))
                ...
                if len(pending_pairs) > 0:
                    ...
                    if len(pending_pairs) > 10:
                        ...
                        pair_strings = os.linesep.join(
                            (old + '->' + new for (old, new) in pending_pairs))
                ...
                if len(petitioned_pairs) > 0:
                    ...
                    if len(petitioned_pairs) > 10:
                        ...
                        pair_strings = ', '.join(
                            (old + '->' + new
                             for (old, new) in petitioned_pairs))
        ...
        def _AutoPetitionConflicts(self, pairs):
            ...
            current_olds = {
                current_old
                for (current_old, current_new) in current_pairs
            }
            ...
            if len(pairs_to_auto_petition) > 0:
                ...
                self._AddPairs(
                    pairs_to_auto_petition,
                    remove_only=True,
                    default_reason='AUTO-PETITION TO REASSIGN TO: ' + new)
        ...
        def _CanAdd(self, potential_pair):
            ...
            if potential_old in current_olds:
                ...
                QW.QMessageBox.critical(
                    self, 'Error',
                    'There already is a relationship set for the tag ' +
                    potential_old + '.')
            ...
            if potential_new in current_olds:
                ...
                while next_new in d:
                    ...
                    if next_new == potential_old:
                        ...
                        QW.QMessageBox.critical(
                            self, 'Error', 'Adding ' + potential_old + '->' +
                            potential_new + ' would create a loop!')
        ...
        def _SetButtonStatus(self):
            ...
            if self._current_new is None or len(
                    self._old_siblings.GetTags()) == 0:
        ...
        def EventAddButton(self):
            ...
            if self._current_new is not None and len(
                    self._old_siblings.GetTags()) > 0:
        ...
        def GetContentUpdates(self):
            ...
            if self._i_am_local_tag_service:
                ...
                for pair in self._current_statuses_to_pairs[
                    ...
                    content_updates.append(
                        HydrusData.ContentUpdate(HC.CONTENT_TYPE_TAG_SIBLINGS,
                                                 HC.CONTENT_UPDATE_ADD, pair))
        ...
        def HasUncommittedPair(self):
            ...
            return len(self._old_siblings.GetTags()
                       ) > 0 and self._current_new is not None
        ...
        def THREADInitialise(self, tags, service_key):
            def qt_code(original_statuses_to_pairs, current_statuses_to_pairs):
                ...
                self._status_st.setText(
                    'Tags on the left will be replaced by those on the right.')
                self._count_st.setText('Starting with ' +
                                       HydrusData.ToHumanInt(
                                           len(original_statuses_to_pairs[
                                               HC.CONTENT_STATUS_CURRENT])) +
            ...
            original_statuses_to_pairs = HG.client_controller.Read(
                'tag_siblings', service_key)

            current_statuses_to_pairs = collections.defaultdict(set)

            current_statuses_to_pairs.update({
                key: set(value)
                for (key, value) in original_statuses_to_pairs.items()
            })
...
class TagFilterButton(ClientGUICommon.BetterButton):
    def __init__(self,
        ...
        ClientGUICommon.BetterButton.__init__(self, parent, 'tag filter',
                                              self._EditTagFilter)
    ...
    def _EditTagFilter(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self,
                                                 'edit tag filter') as dlg:

            namespaces = HG.client_controller.network_engine.domain_manager.GetParserNamespaces(
            )

            panel = EditTagFilterPanel(dlg,
                                       self._tag_filter,
                                       prefer_blacklist=self._is_blacklist,
...
class TagSummaryGenerator(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (list(self._background_colour.toTuple()),
                list(self._text_colour.toTuple()), self._namespace_info,
                self._separator, self._example_tags, self._show)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (background_rgba, text_rgba, self._namespace_info, self._separator,
         self._example_tags, self._show) = serialisable_info
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            new_serialisable_info = (background_rgba, text_rgba,
                                     namespace_info, separator, example_tags,
                                     show)
    ...
    def GenerateSummary(self, tags, max_length=None):
        ...
        for (namespace, unsorted_l) in list(namespaces_to_subtags.items()):
            ...
            sorted_l = HydrusTags.CollapseMultipleSortedNumericTagsToMinMax(
                sorted_l)
        ...
        for (namespace, prefix, separator) in self._namespace_info:
            ...
            if len(subtags) > 0:
                ...
                namespace_text = prefix + separator.join(
                    namespaces_to_subtags[namespace])
    ...
    def ToTuple(self):
        ...
        return (self._background_colour, self._text_colour,
                self._namespace_info, self._separator, self._example_tags,
                self._show)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_TAG_SUMMARY_GENERATOR] = TagSummaryGenerator
