...
def ConvertPortablePathToAbsPath(portable_path, base_dir_override=None):
    ...
    portable_path = os.path.normpath(
        portable_path
...
def CopyFileLikeToFileLike(f_source, f_dest):
    ...
    for block in ReadFileLikeAsBlocks(f_source):
        f_dest.write(block)
        ...
        try:
            ...
            if 'Error 32' in str(e):
                ...
                HydrusData.DebugPrint(
                    ...
                    ' failed because it was in use by another process.')

            else:

                HydrusData.ShowText('Trying to delete ' + path +
...
def GetDevice(path):
    ...
    try:
        ...
        def sort_descending_mountpoint(
            partition_info):  # i.e. put '/home' before '/'
...
def SetEnvTempDir(path):
    ...
    if not DirectoryIsWritable(path):
        ...
        raise Exception(
            'The given temp directory, "{}", does not seem to be writable-to!'.
            format(path))
...
def MergeFile(source, dest):
    ...
    if PathsHaveSameSizeAndDate(source, dest):
        ...
        try:
            ...
            HydrusData.ShowText('Trying to move ' + source + ' to ' + dest +
...
def MirrorFile(source, dest):
    ...
    if not PathsHaveSameSizeAndDate(source, dest):
        ...
        try:
            ...
            HydrusData.ShowText('Trying to copy ' + source + ' to ' + dest +
                                ' caused the following problem:')
...
def PathsHaveSameSizeAndDate(path1, path2):
    ...
    if os.path.exists(path1) and os.path.exists(path2):
        ...
        same_modified_time = int(os.path.getmtime(path1)) == int(
...
def RecyclePath(path):
    ...
    if os.path.exists(path):
        ...
        try:
            ...
            HydrusData.Print('Trying to recycle ' + path +
                             ' created this error:')
