...
pair_types_to_content_types[
    HydrusTagArchive.TAG_PAIR_TYPE_PARENTS] = HC.CONTENT_TYPE_TAG_PARENTS
pair_types_to_content_types[
    HydrusTagArchive.TAG_PAIR_TYPE_SIBLINGS] = HC.CONTENT_TYPE_TAG_SIBLINGS

content_types_to_pair_types = {}

content_types_to_pair_types[
    HC.CONTENT_TYPE_TAG_PARENTS] = HydrusTagArchive.TAG_PAIR_TYPE_PARENTS
content_types_to_pair_types[
    HC.CONTENT_TYPE_TAG_SIBLINGS] = HydrusTagArchive.TAG_PAIR_TYPE_SIBLINGS
...
class MigrationDestinationHTA(MigrationDestination):
    ...
    def Prepare(self):
        ...
        hta_hash_type = HydrusTagArchive.hash_str_to_type_lookup[
            self._desired_hash_type]
...
class MigrationDestinationTagServicePairs(MigrationDestinationTagService):
    def __init__(self, controller, tag_service_key, content_action,
        ...
        MigrationDestinationTagService.__init__(self, controller,
                                                tag_service_key,
                                                content_action)
    ...
    def DoSomeWork(self, source):
        ...
        if self._content_action in (HC.CONTENT_UPDATE_PETITION,
        ...
        content_updates = [
            HydrusData.ContentUpdate(self._content_type,
                                     self._content_action,
                                     tag_pair,
                                     reason=reason) for tag_pair in data
        ]
...
class MigrationSourceHTA(MigrationSource):
    def __init__(self, controller, path, file_service_key, desired_hash_type,
                 hashes, tag_filter):
    ...
    def _ConvertHashes(self, source_hash_type, desired_hash_type, data):
        ...
        if source_hash_type != desired_hash_type:
            ...
            for (hash, tags) in data:
                ...
                result = self._controller.Read('file_hashes', (hash, ),
                                               source_hash_type,
                                               desired_hash_type)
    ...
    def _FilterSHA256Hashes(self, data):
        ...
        if self._hashes is not None:
            ...
            data = [(hash, tags) for (hash, tags) in data
        ...
        if self._file_service_key != CC.COMBINED_FILE_SERVICE_KEY:
            ...
            hashes_to_media_results = {
                ...
                for media_result in self._controller.Read(
                    'media_results', all_hashes)
            }
            ...
            for (hash, tags) in data:
                ...
                if hash in hashes_to_media_results:
                    ...
                    if self._file_service_key not in media_result.GetLocationsManager(
                    ).GetCurrent():
    ...
    def GetSomeData(self):
        ...
        if self._SHA256FilteringNeeded():
            ...
            if self._source_hash_type == 'sha256':
                ...
                data = self._ConvertHashes(self._source_hash_type,
                                           self._desired_hash_type, data)
                ...
                data = self._ConvertHashes(self._source_hash_type, 'sha256',
                                           data)

                data = self._FilterSHA256Hashes(data)

                data = self._ConvertHashes('sha256', self._desired_hash_type,
                                           data)
    ...
    def Prepare(self):
        ...
        self._source_hash_type = HydrusTagArchive.hash_type_to_str_lookup[
            self._hta.GetHashType()]
...
class MigrationSourceHTPA(MigrationSource):
    ...
    def GetSomeData(self):
        ...
        if not (self._left_tag_filter.AllowsEverything()
                and self._right_tag_filter.AllowsEverything()):

            data = [(left_tag, right_tag) for (left_tag, right_tag) in data
                    if self._left_tag_filter.TagOK(left_tag)
                    and self._right_tag_filter.TagOK(right_tag)]
...
class MigrationSourceTagServiceMappings(MigrationSource):
    def __init__(self, controller, tag_service_key, file_service_key,
        ...
        self._database_temp_job_name = 'migrate_{}'.format(
            os.urandom(16).hex())

    def CleanUp(self):

        self._controller.WriteSynchronous('migration_clear_job',
                                          self._database_temp_job_name)

    def GetSomeData(self):

        data = self._controller.Read('migration_get_mappings',
                                     self._database_temp_job_name,
                                     self._file_service_key,
                                     self._tag_service_key,
                                     self._desired_hash_type, self._tag_filter,
                                     self._content_statuses)
    ...
    def Prepare(self):
        ...
        self._controller.WriteSynchronous('migration_start_mappings_job',
                                          self._database_temp_job_name,
                                          self._file_service_key,
                                          self._tag_service_key, self._hashes,
                                          self._content_statuses)


class MigrationSourceTagServicePairs(MigrationSource):
    def __init__(self, controller, tag_service_key, content_type,
                 left_tag_filter, right_tag_filter, content_statuses):
    ...
    def GetSomeData(self):
        ...
        data = self._controller.Read('migration_get_pairs',
                                     self._database_temp_job_name,
                                     self._left_tag_filter,
    ...
    def Prepare(self):
        ...
        self._controller.WriteSynchronous('migration_start_pairs_job',
                                          self._database_temp_job_name,
                                          self._tag_service_key,
                                          self._content_type,
                                          self._content_statuses)
