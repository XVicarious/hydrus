...
from . import (ClientGUIAPI, ClientGUIAsync, ClientGUICommon, ClientGUIDialogs,
        ...
        def qt_clean_up(success):
            ...
            if success:
                ...
                QW.QMessageBox.critical(
                    self, 'Error',
                    'Unfortunately, was unable to set nocopy configuration.')
...
class ReviewServicePanel(QW.QWidget):
    def __init__(self, parent, service):
        ...
        if self._service.GetServiceKey() == CC.COMBINED_LOCAL_FILE_SERVICE_KEY:
            ...
            subpanels.append(
                self._ServiceCombinedLocalFilesPanel(self, service))
    ...
    def EventImmediateSync(self, event):
        def do_it():
            ...
            for (content_updates, weight
                ...
                update_speed_string = ' at ' + HydrusData.ToHumanInt(
                    rows_s) + ' rows/s'
            ...
            job_key.SetVariable(
                'popup_text_1', 'done! ' +
                HydrusData.ToHumanInt(c_u_p_num_rows) + ' rows added.')
    ...
    class _ServicePanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            self._my_updater = ClientGUIAsync.FastThreadToGUIUpdater(
                self, self._Refresh)
    ...
    class _ServiceClientAPIPanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            permissions_list_panel = ClientGUIListCtrl.BetterListCtrlPanel(
                self)

            columns = [('name', -1), ('basic permissions', 36),
                       ('advanced permissions', 36)]

            self._permissions_list = ClientGUIListCtrl.BetterListCtrl(
            ...
            menu_items.append(('normal', 'manually',
                               'Enter the details of the share manually.',
                               self._AddManually))
            menu_items.append((
                'normal', 'from api request',
                'Listen for an access permission request from an external program via the API.',
                self._AddFromAPI))

            permissions_list_panel.AddMenuButton('add', menu_items)
            permissions_list_panel.AddButton('edit',
                                             self._Edit,
                                             enabled_only_on_selection=True)
            permissions_list_panel.AddButton('duplicate',
                                             self._Duplicate,
                                             enabled_only_on_selection=True)
            permissions_list_panel.AddButton('delete',
                                             self._Delete,
                                             enabled_only_on_selection=True)
            permissions_list_panel.AddSeparator()
            permissions_list_panel.AddButton('open client api base url',
                                             self._OpenBaseURL)
            permissions_list_panel.AddButton(
                'copy api access key',
                self._CopyAPIAccessKey,
                enabled_only_on_single_selection=True)
        ...
        def _ConvertDataToListCtrlTuples(self, api_permissions):
            ...
            display_tuple = (pretty_name, basic_permissions_string,
                             advanced_permissions_string)
            sort_tuple = (name, sort_basic_permissions,
                          sort_advanced_permissions)
        ...
        def _AddFromAPI(self):
            ...
            with ClientGUITopLevelWindows.DialogNullipotent(self,
                                                            title) as dlg:

                panel = ClientGUIAPI.CaptureAPIAccessPermissionsRequestPanel(
                    dlg)
        ...
        def _AddManually(self, api_permissions=None):
            ...
            with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
                ...
                panel = ClientGUIAPI.EditAPIPermissionsPanel(
                    dlg, api_permissions)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HG.client_controller.client_api_manager.AddAccess(
                        api_permissions)
        ...
        def _Delete(self):
            ...
            if result == QW.QDialog.Accepted:
                ...
                access_keys = [
                    api_permissions.GetAccessKey()
                    for api_permissions in self._permissions_list.GetData(
                        only_selected=True)
                ]

                HG.client_controller.client_api_manager.DeleteAccess(
                    access_keys)
        ...
        def _Duplicate(self):
            ...
            selected_api_permissions_objects = self._permissions_list.GetData(
                only_selected=True)

            dupes = [
                api_permissions.Duplicate()
            ...
            HG.client_controller.client_api_manager.SetPermissions(
                existing_objects)
            ...
            for api_permissions in selected_api_permissions_objects:
                ...
                with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
                    ...
                    if dlg.exec() == QW.QDialog.Accepted:
                        ...
                        HG.client_controller.client_api_manager.OverwriteAccess(
                            api_permissions)
        ...
        def _OpenBaseURL(self):
            ...
            if port is None:
                ...
                QW.QMessageBox.warning(
                    self, 'Warning',
                    'The service is not running, so you cannot view it in a web browser!'
                )
        ...
        def _Refresh(self):
            ...
            if port is None:
                ...
                status = 'The client api should be running on port {}.'.format(
                    port)
                ...
                if upnp_port is not None:
                    ...
                    status += ' It should be open via UPnP on external port {}.'.format(
                        upnp_port)
    ...
    class _ServiceCombinedLocalFilesPanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            self._clear_deleted_files_record = ClientGUICommon.BetterButton(
                self, 'clear deleted files record',
                self._ClearDeletedFilesRecord)
    ...
    class _ServiceFilePanel(ClientGUICommon.StaticBox):
        ...
        def THREADFetchInfo(self, service):
            ...
            service_info = HG.client_controller.Read('service_info',
                                                     service.GetServiceKey())

            num_files = service_info[HC.SERVICE_INFO_NUM_FILES]
            total_size = service_info[HC.SERVICE_INFO_TOTAL_SIZE]

            text = HydrusData.ToHumanInt(
                num_files) + ' files, totalling ' + HydrusData.ToHumanBytes(
                    total_size)

            if service.GetServiceType() in (HC.COMBINED_LOCAL_FILE,
                                            HC.FILE_REPOSITORY):
    ...
    class _ServiceRemotePanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            self.Add(self._bandwidth_panel,
    ...
    class _ServiceRestrictedPanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            ClientGUICommon.StaticBox.__init__(
                self, parent,
                'hydrus service account - shared by all clients using the same access key'
            )
            ...
            QP.AddToLayout(hbox, self._refresh_account_button,
                           CC.FLAGS_VCENTER)
            QP.AddToLayout(hbox, self._copy_account_key_button,
                           CC.FLAGS_VCENTER)
        ...
        def _Refresh(self):
            ...
            self._title_and_expires_st.setText(title + ' that ' +
                                               expires_status)
            ...
            if self._service.CanSyncAccount(
                    including_external_communication=False):
        ...
        def _RefreshAccount(self):
            ...
            if self._service.GetServiceType(
            ) in HC.REPOSITORIES and self._service.IsPaused():

                QW.QMessageBox.warning(
                    self, 'Warning',
                    'The service is paused! Please unpause it to refresh its account.'
                )
    ...
    class _ServiceRepositoryPanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            self._is_mostly_caught_up_st = ClientGUICommon.BetterStaticText(
                self)

            self._sync_remote_now_button = ClientGUICommon.BetterButton(
                self, 'download now', self._SyncRemoteNow)
            self._sync_processing_now_button = ClientGUICommon.BetterButton(
                self, 'process now', self._SyncProcessingNow)
            ...
            reset_menu_items.append(
                ('normal', 'reprocess definitions',
                 'Reprocess all definitions.', self._ReprocessDefinitions))
            reset_menu_items.append(
                ('normal', 'reprocess content', 'Reprocess all content.',
                 self._ReprocessContent))
            reset_menu_items.append(('separator', None, None, None))
            reset_menu_items.append(('normal', 'complete wipe and reset',
                                     'Reset entire repository.', self._Reset))

            self._reset_button = ClientGUICommon.MenuButton(
                self, 'reset processing', reset_menu_items)
            ...
            QP.AddToLayout(hbox, self._sync_remote_now_button,
                           CC.FLAGS_VCENTER)
            QP.AddToLayout(hbox, self._sync_processing_now_button,
                           CC.FLAGS_VCENTER)
            ...
            self.Add(self._is_mostly_caught_up_st,
            ...
            def do_it(dest_dir, service):
                ...
                try:
                    ...
                    if num_to_do == 0:
                        ...
                        QP.CallAfter(QW.QMessageBox.information, None,
                                     'Information', 'No updates to export!')
                        ...
                        try:
                            ...
                            job_key.SetVariable(
                                'popup_title',
                                'exporting updates for ' + service.GetName())
        ...
        def _ReprocessDefinitions(self):
            def do_it(service, my_updater):
                ...
                HG.client_controller.WriteSynchronous(
                    'reprocess_repository', service_key,
            ...
            message = 'This will command the client to reprocess all definition updates for {}. It will not delete anything.'.format(
                name)
        ...
        def _ReprocessContent(self):
            ...
            message = 'This will command the client to reprocess all content updates for {}. It will not delete anything.'.format(
                name)
            message += os.linesep * 2
            message += 'This is a only useful as a debug tool for filling in \'gaps\'. If you do not understand what this does, turn back now.'
        ...
        def THREADFetchInfo(self, service):
            def qt_code(download_text, download_value, processing_text,
                        processing_value, range, is_mostly_caught_up):

                if not self or not QP.isValid(self):

                    return

                self._download_progress.SetValue(download_text, download_value,
                                                 range)
                self._processing_progress.SetValue(processing_text,
                                                   processing_value, range)
                ...
                metadata_due = self._service.GetMetadata().UpdateDue(
                    from_client=True)
            ...
            (download_value, processing_value,
             range) = HG.client_controller.Read('repository_progress',
                                                service.GetServiceKey())

            is_mostly_caught_up = service.IsMostlyCaughtUp()

            download_text = 'downloaded ' + HydrusData.ConvertValueRangeToPrettyString(
                download_value, range)

            processing_text = 'processed ' + HydrusData.ConvertValueRangeToPrettyString(
                processing_value, range)

            QP.CallAfter(qt_code, download_text, download_value,
                         processing_text, processing_value, range,
                         is_mostly_caught_up)
    ...
    class _ServiceIPFSPanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            interaction_panel = IPFSDaemonStatusAndInteractionPanel(
                self, self.GetService)

            self._ipfs_shares_panel = ClientGUIListCtrl.BetterListCtrlPanel(
                self)

            columns = [('multihash', 34), ('num files', 11),
                       ('total size', 12), ('note', -1)]

            self._ipfs_shares = ClientGUIListCtrl.BetterListCtrl(
                self._ipfs_shares_panel,
                'ipfs_shares',
                12,
                32,
                columns,
                self._ConvertDataToListCtrlTuple,
                delete_key_callback=self._Unpin,
                activation_callback=self._SetNotes)

            self._ipfs_shares_panel.SetListCtrl(self._ipfs_shares)

            self._ipfs_shares_panel.AddButton('copy multihashes',
                                              self._CopyMultihashes,
                                              enabled_only_on_selection=True)
            self._ipfs_shares_panel.AddButton('show selected in main gui',
                                              self._ShowSelectedInNewPages,
                                              enabled_only_on_selection=True)
            self._ipfs_shares_panel.AddButton('set notes',
                                              self._SetNotes,
                                              enabled_only_on_selection=True)
            self._ipfs_shares_panel.AddButton('unpin selected',
                                              self._Unpin,
                                              enabled_only_on_selection=True)
        ...
        def _ConvertDataToListCtrlTuple(self, data):
            ...
            display_tuple = (pretty_multihash, pretty_num_files,
                             pretty_total_size, pretty_note)
        ...
        def _CopyMultihashes(self):
            ...
            multihashes = [
                multihash
                for (multihash, num_files, total_size,
                     note) in self._ipfs_shares.GetData(only_selected=True)
            ]

            if len(multihashes) == 0:

                multihashes = [
                    multihash for (multihash, num_files, total_size,
                                   note) in self._ipfs_shares.GetData()
                ]
            ...
            if len(multihashes) > 0:
                ...
                text = os.linesep.join((multihash_prefix + multihash
        ...
        def _SetNotes(self):
            ...
            if len(datas) > 0:
                ...
                with ClientGUIDialogs.DialogTextEntry(
                        self, 'Set a note for these shares.') as dlg:
        ...
        def _Unpin(self):
            ...
            result = ClientGUIDialogsQuick.GetYesNo(
                self, 'Unpin (remove) all selected?')
        ...
        def THREADFetchInfo(self, service):
            ...
            ipfs_shares = HG.client_controller.Read('service_directories',
                                                    service.GetServiceKey())
    ...
    class _ServiceLocalBooruPanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            columns = [('name', -1), ('info', 36), ('expires', 12),
                       ('files', 12)]

            self._booru_shares = ClientGUIListCtrl.BetterListCtrl(
                booru_share_panel,
                'local_booru_shares',
                10,
                36,
            ...
            booru_share_panel.AddButton('edit',
                                        self._Edit,
                                        enabled_only_on_selection=True)
            booru_share_panel.AddButton('delete',
                                        self._Delete,
                                        enabled_only_on_selection=True)
            booru_share_panel.AddSeparator()
            booru_share_panel.AddButton('open in new page',
                                        self._OpenSearch,
                                        enabled_only_on_selection=True)
            booru_share_panel.AddButton('copy internal share url',
                                        self._CopyInternalShareURL,
                                        enabled_check_func=self._CanCopyURL)
            booru_share_panel.AddButton('copy external share url',
                                        self._CopyExternalShareURL,
                                        enabled_check_func=self._CanCopyURL)
        ...
        def _ConvertDataToListCtrlTuples(self, share_key):
            ...
            pretty_name = name
            pretty_text = text
            pretty_timeout = HydrusData.ConvertTimestampToPrettyExpires(
                timeout)
            pretty_hashes = HydrusData.ToHumanInt(num_hashes)

            sort_timeout = ClientGUIListCtrl.SafeNoneInt(timeout)

            display_tuple = (pretty_name, pretty_text, pretty_timeout,
                             pretty_hashes)
        ...
        def _CopyExternalShareURL(self):
            ...
            if internal_port is None:
                ...
                QW.QMessageBox.warning(
                    self, 'Warning',
                    'The local booru is not currently running!')
            ...
            for share_key in self._booru_shares.GetData(only_selected=True):
                ...
                try:
                    ...
                    QW.QMessageBox.critical(
                        self, 'Error',
                        'Unfortunately, could not generate an external URL: {}'
                        .format(e))
        ...
        def _CopyInternalShareURL(self):
            ...
            for share_key in self._booru_shares.GetData(only_selected=True):
                ...
                url = 'http://' + internal_ip + ':' + str(
                    internal_port) + '/gallery?share_key=' + share_key.hex()
        ...
        def _Delete(self):
            ...
            if result == QW.QDialog.Accepted:
                ...
                for share_key in self._booru_shares.GetData(
                        only_selected=True):

                    HG.client_controller.Write('delete_local_booru_share',
                                               share_key)
        ...
        def _Edit(self):
            ...
            for share_key in self._booru_shares.GetData(only_selected=True):
                ...
                with ClientGUIDialogs.DialogInputLocalBooruShare(
                        ...
                        timeout,
                        hashes,
                        new_share=False) as dlg:

                    if dlg.exec() == QW.QDialog.Accepted:

                        (share_key, name, text, timeout,
                         hashes) = dlg.GetInfo()
                        ...
                        HG.client_controller.Write('local_booru_share',
                                                   share_key, info)
        ...
        def _Refresh(self):
            ...
            if port is None:
                ...
                status = 'The local booru should be running on port {}.'.format(
                    port)
    ...
    class _ServiceRatingPanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            menu_items.append((
                'normal', 'for deleted files',
                'delete all set ratings for files that have since been deleted',
                HydrusData.Call(self._ClearRatings, 'delete_for_deleted_files',
                                'deleted files')))
            menu_items.append((
                'normal', 'for all non-local files',
                'delete all set ratings for files that are not in this client right now',
                HydrusData.Call(self._ClearRatings,
                                'delete_for_non_local_files',
                                'non-local files')))
            menu_items.append(('separator', None, None, None))
            menu_items.append(
                ('normal', 'for all files',
                 'delete all set ratings for all files',
                 HydrusData.Call(self._ClearRatings, 'delete_for_all_files',
                                 'ALL FILES')))

            self._clear_deleted = ClientGUICommon.MenuButton(
                self, 'clear ratings', menu_items)
        ...
        def _ClearRatings(self, advanced_action, action_description):
            ...
            message = 'Delete any ratings on this service for {}? THIS CANNOT BE UNDONE'.format(
                action_description)
            message += os.linesep * 2
            ...
            if result == QW.QDialog.Accepted:
                ...
                content_update = HydrusData.ContentUpdate(
                    HC.CONTENT_TYPE_RATINGS, HC.CONTENT_UPDATE_ADVANCED,
                    advanced_action)

                service_keys_to_content_updates = {
                    self._service.GetServiceKey(): [content_update]
                }
    ...
    class _ServiceTagPanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            self._tag_migration = ClientGUICommon.BetterButton(
                self, 'migrate tags', self._MigrateTags)
        ...
        def _MigrateTags(self):
            ...
            panel = ClientGUIScrolledPanelsReview.MigrateTagsPanel(
                frame, self._service.GetServiceKey())
        ...
        def THREADFetchInfo(self, service):
            ...
            text = HydrusData.ToHumanInt(
                num_mappings
            ) + ' total mappings involving ' + HydrusData.ToHumanInt(
                num_tags) + ' different tags on ' + HydrusData.ToHumanInt(
                    num_files) + ' different files'

            if service.GetServiceType() == HC.TAG_REPOSITORY:

                num_deleted_mappings = service_info[
                    HC.SERVICE_INFO_NUM_DELETED_MAPPINGS]

                text += ' - ' + HydrusData.ToHumanInt(
                    num_deleted_mappings) + ' deleted mappings'
    ...
    class _ServiceTrashPanel(ClientGUICommon.StaticBox):
        def __init__(self, parent, service):
            ...
            self._clear_trash = ClientGUICommon.BetterButton(
                self, 'clear trash', self._ClearTrash)
