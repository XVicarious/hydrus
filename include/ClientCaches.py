...
class DataCache(object):
    ...
    def _RecalcMemoryUsage(self):
        ...
        self._total_estimated_memory_footprint = sum(
            (data.GetEstimatedMemoryFootprint()
             for data in self._keys_to_data.values()))
    ...
    def GetData(self, key):
        ...
        with self._lock:
            ...
            if key not in self._keys_to_data:
                ...
                raise Exception('Cache error! Looking for ' + str(key) +
                                ', but it was missing.')
        ...
        with self._lock:
            ...
            while True:
                ...
                if len(self._keys_fifo) == 0:
                    ...
                    (key,
                     last_access_time) = next(iter(self._keys_fifo.items()))

                    if HydrusData.TimeHasPassed(last_access_time +
                                                self._timeout):
...
class LocalBooruCache(object):
    def __init__(self, controller):
        ...
        self._controller.sub(self, 'RefreshShares',
                             'refresh_local_booru_shares')
        self._controller.sub(self, 'RefreshShares',
                             'restart_client_server_service')
    ...
    def _CheckFileAuthorised(self, share_key, hash):
        ...
        if hash not in info['hashes_set']:
            ...
            raise HydrusExceptions.NotFoundException(
                'That file was not found in that share.')
    ...
    def _RefreshShares(self):
        ...
        self._local_booru_service = self._controller.services_manager.GetService(
...
class MediaResultCache(object):
    def __init__(self):
        ...
        HG.client_controller.sub(self, 'ProcessServiceUpdates',
                                 'service_updates_data')
        HG.client_controller.sub(self, 'NewForceRefreshTags',
                                 'notify_new_force_refresh_tags_data')
        HG.client_controller.sub(self, 'NewTagDisplayRules',
                                 'notify_new_tag_display_rules')
            ...
            for hash_id in hash_ids:
                ...
                if hash_id in self._hash_ids_to_media_results:
                    ...
                    media_results.append(
                        self._hash_ids_to_media_results[hash_id])
                ...
                with self._lock:
                    ...
                    for (hash_id,
                        ...
                        if hash_id in self._hash_ids_to_media_results:
                            ...
                            self._hash_ids_to_media_results[
                                hash_id].SetTagsManager(tags_manager)
                ...
                for content_update in content_updates:
                    ...
                    for hash in hashes:
                        ...
                        if hash in self._hashes_to_media_results:
                            ...
                            self._hashes_to_media_results[
                                hash].ProcessContentUpdate(
    ...
    def ProcessServiceUpdates(self, service_keys_to_service_updates):
        ...
        with self._lock:
            ...
            for (service_key,
                 service_updates) in service_keys_to_service_updates.items():
...
class RenderedImageCache(object):
    def __init__(self, controller):
        ...
        cache_timeout = self._controller.new_options.GetInteger(
            'image_cache_timeout')

        self._data_cache = DataCache(self._controller,
                                     cache_size,
...
class ThumbnailCache(object):
    ...
    def _GetThumbnailHydrusBitmap(self, display_media):
        ...
        try:
            ...
            try:
                ...
                summary = 'The thumbnail for file ' + hash.hex(
                ) + ' was not loadable. An attempt to regenerate it failed.'
            ...
            try:
                ...
                summary = 'The thumbnail for file ' + hash.hex(
                ) + ' was not loadable. It was regenerated, but that file would not render either. Your image libraries or hard drive connection are unreliable. Please inform the hydrus developer what has happened.'
    ...
    def _RecalcQueues(self):
        ...
        def sort_waterfall(item):
            ...
            if display_media is None:
                ...
                magic_score = self._magic_mime_thumbnail_ease_score_lookup[
                    None]
                hash = ''
        ...
        self._delayed_regeneration_queue = list(
            self._delayed_regeneration_queue_quick)
    ...
    def CancelWaterfall(self, page_key, medias):
        ...
        with self._lock:
            ...
            self._waterfall_queue_quick.difference_update(
                ((page_key, media) for media in medias))

            cancelled_media_results = {
                media.GetDisplayMedia().GetMediaResult()
                for media in medias
            }

            outstanding_delayed_hashes = {
                media_result.GetHash()
                for media_result in cancelled_media_results
                if media_result in self._delayed_regeneration_queue_quick
            }

            if len(outstanding_delayed_hashes) > 0:

                self._controller.files_maintenance_manager.ScheduleJob(
                    outstanding_delayed_hashes,
                    ClientFiles.REGENERATE_FILE_DATA_JOB_FORCE_THUMBNAIL)

            self._delayed_regeneration_queue_quick.difference_update(
                cancelled_media_results)
    ...
    def Clear(self):
        ...
        with self._lock:
            ...
            for name in names:
                ...
                numpy_image_resolution = HydrusImageHandling.GetResolutionNumPy(
                    numpy_image)

                target_resolution = HydrusImageHandling.GetThumbnailResolution(
                    numpy_image_resolution, bounding_dimensions)

                numpy_image = HydrusImageHandling.ResizeNumPyImage(
                    numpy_image, target_resolution)
            ...
            if mime in HC.MIMES_WITH_THUMBNAILS:
                ...
                if result is None:
                    ...
                    try:
                        ...
                        hydrus_bitmap = self._GetThumbnailHydrusBitmap(
                            display_media)
                ...
                return self._special_thumbs['audio']
            elif mime in HC.VIDEO:
                return self._special_thumbs['video']
            elif mime == HC.APPLICATION_PDF:
                return self._special_thumbs['pdf']
            elif mime == HC.APPLICATION_PSD:
                return self._special_thumbs['psd']
            elif mime in HC.ARCHIVES:
                return self._special_thumbs['zip']
            else:
                return self._special_thumbs['hydrus']
    ...
    def Waterfall(self, page_key, medias):
        ...
        with self._lock:
            ...
            self._waterfall_queue_quick.update(
                ((page_key, media) for media in medias))
