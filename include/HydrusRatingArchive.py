...
class HydrusRatingArchive(object):
    ...
    def _InitDB(self):
        ...
        self._c.execute(
            'CREATE TABLE number_of_stars ( number_of_stars INTEGER );', )

        self._c.execute(
            'CREATE TABLE ratings ( hash BLOB PRIMARY KEY, rating REAL );')
    ...
    def DeleteRating(self, hash):
        ...
        self._c.execute('DELETE FROM ratings WHERE hash = ?;',
                        (sqlite3.Binary(hash), ))
    ...
    def GetHashType(self):
        ...
        if result is None:
            ...
            if len(hash) == 16: hash_type = HASH_TYPE_MD5
                ...
                raise Exception(
                    'This archive has non-standard hashes. Something is wrong.'
                )
    ...
    def GetNumberOfStars(self):
        ...
        result = self._c.execute(
            'SELECT number_of_stars FROM number_of_stars;').fetchone()
    ...
    def GetRating(self, hash):
        ...
        result = self._c.execute('SELECT rating FROM ratings WHERE hash = ?;',
    ...
    def HasHash(self, hash):
        ...
        result = self._c.execute('SELECT 1 FROM ratings WHERE hash = ?;',
                                 (sqlite3.Binary(hash), )).fetchone()
    ...
    def SetNumberOfStars(self, number_of_stars):
        ...
        self._c.execute(
            'INSERT INTO number_of_stars ( number_of_stars ) VALUES ( ? );',
            (number_of_stars, ))

    def SetRating(self, hash, rating):

        self._c.execute(
            'REPLACE INTO ratings ( hash, rating ) VALUES ( ?, ? );',
            (sqlite3.Binary(hash), rating))
