...
class OptionsPanelMimes(OptionsPanel):
    ...
    def _DoInitialHideShow(self):
        ...
        for (general_mime_type, general_mime_checkbox) in list(
                self._general_mime_types_to_checkboxes.items()):

            mimes_in_type = self._GetMimesForGeneralMimeType(general_mime_type)

            should_show = general_mime_checkbox.checkState(
    ...
    def _GetMimesForGeneralMimeType(self, general_mime_type):
        ...
        mimes_in_type = [
            mime for mime in mimes_in_type if mime in self._selectable_mimes
        ]
    ...
    def _UpdateMimeGroupCheckboxes(self):
        ...
        for (general_mime_type, general_mime_checkbox
             ) in self._general_mime_types_to_checkboxes.items():

            mimes_in_type = self._GetMimesForGeneralMimeType(general_mime_type)

            all_checkbox_values = {
                self._mimes_to_checkboxes[mime].isChecked()
                for mime in mimes_in_type
            }
    ...
    def EventMimeGroupCheckbox(self):
        ...
        for (general_mime_type, general_mime_checkbox) in list(
            ...
            if mime_check_state is not None:
                ...
                mimes_in_type = self._GetMimesForGeneralMimeType(
                    general_mime_type)

                for mime in mimes_in_type:

                    self._mimes_to_checkboxes[mime].setChecked(
                        mime_check_state)
