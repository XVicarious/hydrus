...
from . import (ClientData, ClientDefaults, ClientDownloading, ClientFiles,
               ClientImportFileSeeds, ClientImportGallerySeeds,
               ClientImportOptions, ClientNetworkingContexts,
...
def PublishPresentationHashes(publishing_label, hashes,
                              publish_to_popup_button, publish_files_to_page):
    ...
    if publish_to_popup_button:
        ...
        files_job_key.SetVariable('popup_files',
                                  (list(hashes), publishing_label))

        HG.client_controller.pub('message', files_job_key)

    if publish_files_to_page:

        HG.client_controller.pub('imported_files_to_page', list(hashes),
                                 publishing_label)
        ...
        try:
            ...
            if status in CC.SUCCESSFUL_IMPORT_STATES:
                ...
                if len(presentation_hashes) > 0:
                    ...
                    job_key.SetVariable('popup_files',
                                        (presentation_hashes, 'downloads'))
...
def UpdateFileSeedCacheWithFileSeeds(file_seed_cache,
                                     file_seeds,
