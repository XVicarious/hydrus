...
def CollapseMultipleSortedNumericTagsToMinMax(tags):
    ...
    if len(tags) <= 2:
        ...
        includes_non_numeric_tag = True in (not isinstance(
            ConvertTagToSortable(tag), tuple) for tag in tags)
...
def CleanTag(tag):
    ...
    try:
        ...
        tag = HydrusText.re_leading_single_colon.sub(
            '::', tag
        )  # Convert anything starting with one colon to start with two i.e. :D -> ::D

        if ':' in tag:

            tag = StripTextOfGumpf(
                tag)  # need to repeat here to catch 'system:' stuff
