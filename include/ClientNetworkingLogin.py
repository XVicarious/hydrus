...
from . import (ClientDefaults, ClientNetworkingContexts,
               ClientNetworkingDomain, ClientNetworkingJobs, ClientParsing,
               ClientThreading)
...
login_access_type_default_description_lookup[
    LOGIN_ACCESS_TYPE_EVERYTHING] = 'Login required to access any content.'
login_access_type_default_description_lookup[
    LOGIN_ACCESS_TYPE_NSFW] = 'Login required to access NSFW content.'
login_access_type_default_description_lookup[
    LOGIN_ACCESS_TYPE_SPECIAL] = 'Login required to access special content.'
login_access_type_default_description_lookup[
    LOGIN_ACCESS_TYPE_USER_PREFS_ONLY] = 'Login only required to access user preferences.'

PIXIV_NETWORK_CONTEXT = ClientNetworkingContexts.NetworkContext(
    CC.NETWORK_CONTEXT_DOMAIN, 'pixiv.net')
HENTAI_FOUNDRY_NETWORK_CONTEXT = ClientNetworkingContexts.NetworkContext(
...
class NetworkLoginManager(HydrusSerialisable.SerialisableBase):
    ...
    def _GetBestLoginScript(self, login_domain):
        ...
        self._login_scripts.sort(
            key=lambda ls: len(ls.GetCredentialDefinitions()))
    ...
    def _GetLoginDomainStatus(self, network_context):
        ...
        potential_login_domains = ClientNetworkingDomain.ConvertDomainIntoAllApplicableDomains(
            domain, discard_www=False)
        ...
        for potential_login_domain in potential_login_domains:
            ...
            if potential_login_domain in self._domains_to_login_info:
                ...
                (login_script_key_and_name, credentials, login_access_type,
                 ...
                 ) = self._domains_to_login_info[login_domain]
    ...
    def _GetLoginScriptAndCredentials(self, login_domain):
        ...
        if login_domain in self._domains_to_login_info:
            ...
            (login_script_key_and_name, credentials, login_access_type,
             login_access_text, active, validity, validity_error_text,
             no_work_until,
             no_work_until_reason) = self._domains_to_login_info[login_domain]
            ...
            if login_script_key in self._login_script_keys_to_login_scripts:
                ...
                login_script = self._login_script_keys_to_login_scripts[
                    login_script_key]

            elif login_script_name in self._login_script_names_to_login_scripts:

                login_script = self._login_script_names_to_login_scripts[
                    login_script_name]
            ...
            raise HydrusExceptions.ValidationException(
                'Could not find any login entry for "' + login_domain + '"!')
    ...
    def _GetSerialisableInfo(self):
        ...
        for (login_domain,
             (login_script_key_and_name, credentials, login_access_type,
              login_access_text, active, validity, validity_error_text,
              no_work_until, no_work_until_reason)) in list(
                  self._domains_to_login_info.items()):

            (login_script_key, login_script_name) = login_script_key_and_name

            serialisable_login_script_key_and_name = (login_script_key.hex(),
                                                      login_script_name)

            serialisable_domains_to_login_info[login_domain] = (
                serialisable_login_script_key_and_name, credentials,
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (serialisable_login_scripts,
         serialisable_domains_to_login_info) = serialisable_info

        self._login_scripts = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_login_scripts)

        self._domains_to_login_info = {}

        for (login_domain, (serialisable_login_script_key_and_name,
                            credentials, login_access_type, login_access_text,
                            active, validity, validity_error_text,
                            no_work_until, no_work_until_reason)) in list(
                                serialisable_domains_to_login_info.items()):

            (serialisable_login_script_key,
             login_script_name) = serialisable_login_script_key_and_name

            login_script_key_and_name = (
                bytes.fromhex(serialisable_login_script_key),
                login_script_name)
    ...
    def _RecalcCache(self):
        ...
        self._login_script_keys_to_login_scripts = {
            login_script.GetLoginScriptKey(): login_script
            for login_script in self._login_scripts
        }
        self._login_script_names_to_login_scripts = {
    ...
    def AlreadyHaveExactlyThisLoginScript(self, new_login_script):
        ...
        with self._lock:
            ...
            dupe_login_scripts = [
                login_script.Duplicate()
                for login_script in self._login_scripts
            ]

            for dupe_login_script in dupe_login_scripts:

                dupe_login_script.SetLoginScriptKeyAndName(
                    login_script_key_and_name)

                if dupe_login_script.DumpToString(
                ) == new_login_script.DumpToString():
    ...
    def CheckCanLogin(self, network_context):
        ...
        with self._lock:
            ...
            if network_context.context_type == CC.NETWORK_CONTEXT_DOMAIN:
                ...
                (login_domain, login_expected, login_possible, login_error_text
                 ) = self._GetLoginDomainStatus(network_context)

                if login_domain is None or not login_expected:

                    raise HydrusExceptions.ValidationException(
                        'The domain ' + login_domain +
                        ' has no active login script--has it just been turned off?'
                    )

                elif not login_possible:

                    raise HydrusExceptions.ValidationException(
                        'The domain ' + login_domain + ' cannot log in: ' +
                        login_error_text)
    ...
    def DeleteLoginScripts(self, login_script_names):
        ...
        with self._lock:
            ...
            login_scripts = [
                login_script for login_script in self._login_scripts
        ...
        with self._lock:
            ...
            if network_context.context_type == CC.NETWORK_CONTEXT_DOMAIN:
                ...
                if login_domain is None or not login_expected:
                    ...
                    login_network_context = ClientNetworkingContexts.NetworkContext(
    ...
    def InvalidateLoginScript(self, login_domain, login_script_key, reason):
        ...
        with self._lock:
            ...
            HydrusData.ShowText(
                'The login for "' + login_domain +
                '" failed! It will not be reattempted until the problem is fixed. The failure reason was:'
                + os.linesep * 2 + validity_error_text)
        ...
        with self._lock:
            ...
            if network_context.context_type == CC.NETWORK_CONTEXT_DOMAIN:
                ...
                if login_domain is None or not login_expected:
                    ...
                    return not login_script.IsLoggedIn(self.engine,
                                                       login_network_context)

            elif network_context.context_type == CC.NETWORK_CONTEXT_HYDRUS:

                return not self._hydrus_login_script.IsLoggedIn(
                    self.engine, network_context)
    ...
    def OverwriteDefaultLoginScripts(self, login_script_names):
        ...
        with self._lock:
            ...
            new_login_scripts = [
                login_script for login_script in existing_login_scripts
                if login_script.GetName() not in login_script_names
            ]
            new_login_scripts.extend([
                login_script for login_script in default_login_scripts
                if login_script.GetName() in login_script_names
            ])
    ...
    def SetLoginScripts(self, login_scripts):
        ...
        with self._lock:
            ...
            for login_script in self._login_scripts:
                ...
                for (login_domain, login_access_type,
                     login_access_text) in example_domains_info:
                    ...
                    if '.' in login_domain:
                        ...
                        if login_domain in self._domains_to_login_info:
                            ...
                            (old_login_script_key_and_name, credentials,
                             old_login_access_type, old_login_access_text,
                             active, validity, validity_error_text,
                             no_work_until, no_work_until_reason
                             ) = self._domains_to_login_info[login_domain]

                            if old_login_script_key_and_name[
                                    1] == login_script_key_and_name[1]:

                                self._domains_to_login_info[login_domain] = (
                                    login_script_key_and_name, credentials,
                            ...
                            active = len(
                                login_script.GetCredentialDefinitions()) == 0
                            ...
                            self._domains_to_login_info[login_domain] = (
                                login_script_key_and_name, credentials,
                                login_access_type, login_access_text, active,
                                validity, validity_error_text, no_work_until,
                                no_work_until_reason)
        ...
        with self._lock:
            ...
            for login_domain in login_domains:
                ...
                try:
                    ...
                    (existing_login_script, existing_credentials
                     ) = self._GetLoginScriptAndCredentials(login_domain)
    ...
    def LoginTumblrGDPR(self):
        ...
        formula = ClientParsing.ParseFormulaHTML(
            tag_rules=[
                ClientParsing.ParseRuleHTML(
                    ...
                    tag_name='meta',
                    tag_attributes={'id': 'tumblr_form_key'})
            ],
        ...
        if len(results) != 1:
            ...
            raise HydrusExceptions.ParseException(
                'Could not figure out the tumblr form key for the GDPR click-through.'
            )
        ...
        HydrusData.ShowText(
            'Looks like tumblr GDPR click-through worked! You should be good for a year, at which point we should have an automatic solution for this!'
        )


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_NETWORK_LOGIN_MANAGER] = NetworkLoginManager
...
class LoginCredentialDefinition(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def __init__(self,
                 name='username',
                 credential_type=CREDENTIAL_TYPE_TEXT,
                 string_match=None):
    ...
    def Test(self, text):
        ...
        if self._string_match is not None:
            ...
            try:
                ...
                raise HydrusExceptions.ValidationException(
                    'Could not validate "' + self._name + '" credential: ' +
                    str(e))


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_LOGIN_CREDENTIAL_DEFINITION] = LoginCredentialDefinition
...
class LoginProcessDomain(LoginProcess):
    ...
    def _Start(self):
        ...
        result = self.login_script.Start(self.engine,
                                         self.network_context,
                                         self.credentials,
                                         job_key=job_key)

        HydrusData.Print('Finished login for ' +
                         self.network_context.context_data + '. Result was: ' +
                         result)
...
class LoginScriptHydrus(object):
    ...
    def Start(self, engine, network_context):
        ...
        try:
            ...
            service = engine.controller.services_manager.GetService(
                service_key)
...
class LoginScriptDomain(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def __init__(self,
                 name='login script',
                 login_script_key=None,
                 required_cookies_info=None,
                 credential_definitions=None,
                 login_steps=None,
                 example_domains_info=None):

        if required_cookies_info is None:

            required_cookies_info = {}

        required_cookies_info = HydrusSerialisable.SerialisableDictionary(
            required_cookies_info)
        ...
        credential_definitions = HydrusSerialisable.SerialisableList(
            credential_definitions)
    ...
    def _GetSerialisableInfo(self):
        ...
        return (serialisable_login_script_key, serialisable_required_cookies,
                serialisable_credential_definitions, serialisable_login_steps,
                self._example_domains_info)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_login_script_key, serialisable_required_cookies,
         serialisable_credential_definitions, serialisable_login_steps,
         self._example_domains_info) = serialisable_info

        self._login_script_key = bytes.fromhex(serialisable_login_script_key)
        self._required_cookies_info = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_required_cookies)
        self._credential_definitions = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_credential_definitions)
        self._login_steps = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_login_steps)

        # convert lists to tups for listctrl data hashing
        self._example_domains_info = [
            tuple(l) for l in self._example_domains_info
        ]
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (serialisable_login_script_key, serialisable_required_cookies,
             serialisable_credential_definitions, serialisable_login_steps,
             example_domains_info) = old_serialisable_info

            old_required_cookies_info = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_required_cookies)
            new_required_cookies_info = HydrusSerialisable.SerialisableDictionary(
            )

            for (name, value_string_match) in list(
                    old_required_cookies_info.items()):

                key_string_match = ClientParsing.StringMatch(
                    match_type=ClientParsing.STRING_MATCH_FIXED,
                    match_value=name,
                    example_string=name)

                new_required_cookies_info[
                    key_string_match] = value_string_match

            serialisable_required_cookies = new_required_cookies_info.GetSerialisableTuple(
            )

            new_serialisable_info = (serialisable_login_script_key,
                                     serialisable_required_cookies,
                                     serialisable_credential_definitions,
                                     serialisable_login_steps,
                                     example_domains_info)
    ...
    def _IsLoggedIn(self, engine, network_context, validation_check=False):
        ...
        for (cookie_name_string_match,
             value_string_match) in list(self._required_cookies_info.items()):

            try:

                cookie = ClientNetworkingDomain.GetCookie(
                    cookies, search_domain, cookie_name_string_match)
            ...
            try:
                ...
                if validation_check:
                    ...
                    raise HydrusExceptions.ValidationException(
                        'Cookie "' + cookie_name_string_match.ToString() +
                        '" failed: ' + str(e) + '!')
    ...
    def CheckCanLogin(self, given_credentials):
        ...
        required_cred_names = {
            ...
            for name in itertools.chain.from_iterable((
                step.GetRequiredCredentials() for step in self._login_steps))
        }
        ...
        if len(missing_givens) > 0:
            ...
            raise HydrusExceptions.ValidationException(
                'Missing required credentials: ' + ', '.join(missing_givens))

        #

        cred_names_to_definitions = {
            credential_definition.GetName(): credential_definition
            for credential_definition in self._credential_definitions
        }
    ...
    def CheckIsValid(self):
        ...
        missing_definitions = required_cred_names.difference(
            defined_cred_names)
        ...
        for login_step in self._login_steps:
            ...
            (required_vars,
             set_vars) = login_step.GetRequiredAndSetTempVariables()
            ...
            if len(missing_vars) > 0:
                ...
                raise HydrusExceptions.ValidationException(
                    ...
                    login_step.GetName() + '": ' + ', '.join(missing_vars))
    ...
    def GetExampleDomains(self):
        ...
        return [
            domain for (domain, login_access_type,
                        login_access_text) in self._example_domains_info
        ]
    ...
    def GetLoginExpiry(self, engine, network_context):
        ...
        for cookie_name_string_match in list(
                self._required_cookies_info.keys()):
    ...
    def GetRequiredCredentials(self):
        ...
        for login_step in self._login_steps:
            ...
            required_creds.extend(
                login_step.GetRequiredCredentials())  # name with an order

        return required_creds

    def GetSafeSummary(self):

        return 'Login Script "' + self._name + '" - ' + ', '.join(
            self.GetExampleDomains())
    ...
    def Start(self,
              ...
              network_context,
              given_credentials,
              network_job_presentation_context_factory=None,
              test_result_callable=None,
              job_key=None):
        ...
        for login_step in self._login_steps:
            ...
            if job_key is not None:
                ...
                if job_key.IsCancelled():
                    ...
                    engine.login_manager.DelayLoginScript(
                        login_domain, self._login_script_key, message)
            ...
            try:
                ...
                last_url_used = login_step.Start(
                    engine,
                    login_domain,
                    given_credentials,
                    temp_variables,
                    referral_url=last_url_used,
                    network_job_presentation_context_factory=
                    network_job_presentation_context_factory,
                    test_result_callable=test_result_callable)
                ...
                engine.login_manager.InvalidateLoginScript(
                    login_domain, self._login_script_key, message)
                ...
                engine.login_manager.DelayLoginScript(login_domain,
                                                      self._login_script_key,
                                                      message)
        ...
        try:
            ...
            engine.login_manager.InvalidateLoginScript(login_domain,
                                                       self._login_script_key,
                                                       message)

            return 'Final cookie check failed: ' + message

        engine.login_manager.ValidateLoginScript(login_domain,
                                                 self._login_script_key)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_LOGIN_SCRIPT_DOMAIN] = LoginScriptDomain
...
class LoginStep(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def __init__(self,
                 name='hit home page to establish session',
                 scheme='https',
                 method='GET',
                 subdomain=None,
                 path='/'):
        ...
        self._required_cookies_info = HydrusSerialisable.SerialisableDictionary(
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._scheme, self._method, self._subdomain, self._path,
                self._required_credentials, self._static_args, self._temp_args,
                serialisable_required_cookies, serialisable_content_parsers)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._scheme, self._method, self._subdomain, self._path,
         self._required_credentials, self._static_args, self._temp_args,
         serialisable_required_cookies,
         serialisable_content_parsers) = serialisable_info
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (scheme, method, subdomain, path, required_credentials,
             static_args, temp_args, serialisable_required_cookies,
             serialisable_content_parsers) = old_serialisable_info
            ...
            new_serialisable_info = (scheme, method, subdomain, path,
                                     required_credentials, static_args,
                                     temp_args, serialisable_required_cookies,
                                     serialisable_content_parsers)
    ...
    def GetRequiredCredentials(self):
        ...
        return [
            pretty_name
            for (pretty_name,
                 arg_name) in list(self._required_credentials.items())
        ]
    ...
    def GetRequiredAndSetTempVariables(self):
        ...
        set_temp_variables = {
            additional_info
            for [(name, content_type, additional_info)] in [
                content_parser.GetParsableContent()
    ...
    def SetComplicatedVariables(self, required_credentials, static_args,
    ...
    def Start(self,
        ...
        new_temp_variables = {}
        original_cookie_strings = session_to_cookie_strings(
            engine.session_manager.GetSessionForDomain(domain))
        ...
        try:
            ...
            for (temp_name, arg_name) in list(self._temp_args.items()):
                ...
                if temp_name not in temp_variables:
                    ...
                    raise HydrusExceptions.ValidationException(
                        'The temporary variable \'' + temp_name +
                        '\' was not found!')
            ...
            if self._method == 'GET':
                ...
                query = ClientNetworkingDomain.ConvertQueryDictToText(
                    query_dict)
                body = None
                test_result_body = ''
                ...
                query = ''
                body = query_dict
                test_result_body = ClientNetworkingDomain.ConvertQueryDictToText(
                    query_dict)
            ...
            network_job = ClientNetworkingJobs.NetworkJob(
                self._method, url, body=body, referral_url=referral_url)
            ...
            if self._method == 'POST' and referral_url is not None:
                ...
                network_job.AddAdditionalHeader(
                    'origin', origin
                )  # GET/POST forms are supposed to have this for CSRF. we'll try it just with POST for now
            ...
            if network_job_presentation_context_factory is not None:
                ...
                with network_job_presentation_context_factory(
                        network_job) as njpc:
            ...
            for (cookie_name_string_match,
                 string_match) in list(self._required_cookies_info.items()):

                try:

                    cookie = ClientNetworkingDomain.GetCookie(
                        cookies, domain, cookie_name_string_match)

                except HydrusExceptions.DataMissing as e:

                    raise HydrusExceptions.ValidationException(
                        'Missing cookie "' +
                        cookie_name_string_match.ToString() + '" on step "' +
                        self._name + '"!')
                ...
                try:
                    ...
                    raise HydrusExceptions.ValidationException(
                        'Cookie "' + cookie_name_string_match.ToString() +
                        '" failed on step "' + self._name + '": ' + str(e) +
                        '!')
            ...
            for content_parser in self._content_parsers:
                ...
                try:
                    ...
                    parse_results = content_parser.Parse(
                        parsing_context, downloaded_text)

                except HydrusExceptions.VetoException as e:

                    raise HydrusExceptions.ValidationException(str(e))

                result = ClientParsing.GetVariableFromParseResults(
                    parse_results)
            ...
            if test_result_callable is not None:
                ...
                current_cookie_strings = session_to_cookie_strings(
                    engine.session_manager.GetSessionForDomain(domain))

                new_cookie_strings = tuple(
                    current_cookie_strings.difference(original_cookie_strings))

                new_temp_strings = tuple(
                    (key + ': ' + value
                     for (key, value) in list(new_temp_variables.items())))

                test_result = (self._name, url, test_result_body,
                               downloaded_text, new_temp_strings,
                               new_cookie_strings, test_script_result)
    ...
    def ToTuple(self):
        ...
        return (self._scheme, self._method, self._subdomain, self._path,
                self._required_credentials, self._static_args, self._temp_args,
                self._required_cookies_info, self._content_parsers)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_LOGIN_STEP] = LoginStep
