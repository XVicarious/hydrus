...
IGNORED_TAG_SEARCH_CHARACTERS_UNICODE_TRANSLATE = {
    ord(char): None
    for char in IGNORED_TAG_SEARCH_CHARACTERS
}
...
def FilterPredicatesBySearchText(service_key, search_text, predicates):
    ...
    matching_tags = FilterTagsBySearchText(service_key, search_text,
                                           list(tags_to_predicates.keys()))
...
def FilterTagsBySearchText(service_key,
                           search_text,
                           tags,
...
class FileSearchContext(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_predicates = [
            predicate.GetSerialisableTuple() for predicate in self._predicates
        ]

        return (self._file_service_key.hex(), self._tag_service_key.hex(),
                self._search_type, self._include_current_tags,
                self._include_pending_tags, serialisable_predicates,
                self._search_complete)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (file_service_key, tag_service_key, self._search_type,
         self._include_current_tags, self._include_pending_tags,
         serialisable_predicates, self._search_complete) = serialisable_info
        ...
        self._predicates = [
            HydrusSerialisable.CreateFromSerialisableTuple(pred_tuple)
            for pred_tuple in serialisable_predicates
        ]
    ...
    def _InitialiseTemporaryVariables(self):
        ...
        system_predicates = [
            predicate for predicate in self._predicates
            if predicate.GetType() in HC.SYSTEM_PREDICATES
        ]
        ...
        namespace_predicates = [
            predicate for predicate in self._predicates
            if predicate.GetType() == HC.PREDICATE_TYPE_NAMESPACE
        ]
        ...
        for predicate in namespace_predicates:
            ...
            if predicate.GetInclusive():
                self._namespaces_to_include.append(namespace)
            else:
                self._namespaces_to_exclude.append(namespace)

        wildcard_predicates = [
            predicate for predicate in self._predicates
            if predicate.GetType() == HC.PREDICATE_TYPE_WILDCARD
        ]
        ...
        for predicate in wildcard_predicates:
            ...
            if predicate.GetInclusive():
                self._wildcards_to_include.append(wildcard)
            else:
                self._wildcards_to_exclude.append(wildcard)

        self._or_predicates = [
            predicate for predicate in self._predicates
            if predicate.GetType() == HC.PREDICATE_TYPE_OR_CONTAINER
        ]
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (file_service_key_hex, tag_service_key_hex, include_current_tags,
             include_pending_tags, serialisable_predicates,
             search_complete) = old_serialisable_info

            search_type = SEARCH_TYPE_AND

            new_serialisable_info = (file_service_key_hex, tag_service_key_hex,
                                     search_type, include_current_tags,
                                     include_pending_tags,
                                     serialisable_predicates, search_complete)

            return (2, new_serialisable_info)

        if version == 2:

            (file_service_key_hex, tag_service_key_hex, search_type,
             include_current_tags, include_pending_tags,
             serialisable_predicates, search_complete) = old_serialisable_info
    ...
    def GetNamespacesToExclude(self):
        return self._namespaces_to_exclude

    def GetNamespacesToInclude(self):
        return self._namespaces_to_include

    def GetORPredicates(self):
        return self._or_predicates

    def GetPredicates(self):
        return self._predicates

    def GetSystemPredicates(self):
        return self._system_predicates

    def GetTagServiceKey(self):
        return self._tag_service_key

    def GetTagsToExclude(self):
        return self._tags_to_exclude

    def GetTagsToInclude(self):
        return self._tags_to_include

    def GetWildcardsToExclude(self):
        return self._wildcards_to_exclude

    def GetWildcardsToInclude(self):
        return self._wildcards_to_include

    def HasNoPredicates(self):

        return len(self._predicates) == 0

    def IncludeCurrentTags(self):
        return self._include_current_tags

    def IncludePendingTags(self):
        return self._include_pending_tags
    ...
    def IsJustSystemEverything(self):
        ...
        return len(self._predicates
                   ) == 1 and self._system_predicates.HasSystemEverything()
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_FILE_SEARCH_CONTEXT] = FileSearchContext
...
class FileSystemPredicates(object):
    def __init__(self, system_predicates, apply_implicit_limit=True):
        ...
        for predicate in system_predicates:
            ...
            if predicate_type == HC.PREDICATE_TYPE_SYSTEM_EVERYTHING:
                self._has_system_everything = True
            ...
            if predicate_type == HC.PREDICATE_TYPE_SYSTEM_KNOWN_URLS:
                ...
                self._common_info['known_url_rules'].append(
                    (operator, rule_type, rule))
            ...
            if predicate_type in (HC.PREDICATE_TYPE_SYSTEM_AGE,
                ...
                if age_type == 'delta':
                    ...
                    age = ((((((
                        (years * 12) + months) * 30) + days) * 24) + hours) *
                           3600)
            ...
            if predicate_type == HC.PREDICATE_TYPE_SYSTEM_TAG_AS_NUMBER:
                ...
                if operator == '<':
                    self._common_info['max_tag_as_number'] = (namespace, num)
                elif operator == '>':
                    self._common_info['min_tag_as_number'] = (namespace, num)
                elif operator == '\u2248':

                    self._common_info['min_tag_as_number'] = (namespace,
                                                              int(num * 0.85))
                    self._common_info['max_tag_as_number'] = (namespace,
                                                              int(num * 1.15))
            ...
            if predicate_type == HC.PREDICATE_TYPE_SYSTEM_FILE_RELATIONSHIPS_COUNT:
                ...
                self._duplicate_count_predicates.append(
                    (operator, num_relationships, dupe_type))
            ...
            if predicate_type == HC.PREDICATE_TYPE_SYSTEM_FILE_VIEWING_STATS:
                ...
                self._file_viewing_stats_predicates.append(
                    (view_type, viewing_locations, operator, viewing_value))
    ...
    def GetFileServiceInfo(self):
        ...
        return (self._file_services_to_include_current,
                self._file_services_to_include_pending,
                self._file_services_to_exclude_current,
                self._file_services_to_exclude_pending)
    ...
    def GetLimit(self, apply_implicit_limit=True):
        ...
        if self._limit is None and apply_implicit_limit:
            ...
            forced_search_limit = HG.client_controller.new_options.GetNoneableInteger(
                'forced_search_limit')
    ...
    def MustBeArchive(self):
        return self._archive

    def MustBeInbox(self):
        return self._inbox

    def MustBeLocal(self):
        return self._local

    def MustNotBeLocal(self):
        return self._not_local
...
class Predicate(HydrusSerialisable.SerialisableBase):
    ...
    def __repr__(self):
        ...
        return 'Predicate: ' + str((self._predicate_type, self._value,
                                    self._inclusive, self.GetCount()))

    def _GetSerialisableInfo(self):

        if self._predicate_type in (HC.PREDICATE_TYPE_SYSTEM_RATING,
            ...
            serialisable_value = (operator, rule_type, serialisable_rule,
                                  description)
            ...
            serialisable_value = HydrusSerialisable.SerialisableList(
                or_predicates).GetSerialisableTuple()
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (self._predicate_type, serialisable_value,
         self._inclusive) = serialisable_info
        ...
        if self._predicate_type in (HC.PREDICATE_TYPE_SYSTEM_RATING,
            ...
            self._value = (tuple([
                bytes.fromhex(serialisable_hash)
                for serialisable_hash in serialisable_hashes
            ]), max_hamming)

        elif self._predicate_type == HC.PREDICATE_TYPE_SYSTEM_KNOWN_URLS:

            (operator, rule_type, serialisable_rule,
             description) = serialisable_value

            if rule_type in ('url_match', 'url_class'):

                rule = HydrusSerialisable.CreateFromSerialisableTuple(
                    serialisable_rule)
        ...
        elif self._predicate_type in (HC.PREDICATE_TYPE_SYSTEM_AGE,
                                      HC.PREDICATE_TYPE_SYSTEM_MODIFIED_TIME):
            ...
            (view_type, viewing_locations, operator,
             viewing_value) = serialisable_value

            self._value = (view_type, tuple(viewing_locations), operator,
                           viewing_value)
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (predicate_type, serialisable_value,
             inclusive) = old_serialisable_info

            if predicate_type == HC.PREDICATE_TYPE_SYSTEM_AGE:

                (operator, years, months, days, hours) = serialisable_value

                serialisable_value = (operator, 'delta', (years, months, days,
                                                          hours))
        ...
        if version == 2:
            ...
            if predicate_type in (HC.PREDICATE_TYPE_SYSTEM_HASH,
    ...
    def GetAllCounts(self):
        ...
        return (self._min_current_count, self._max_current_count,
                self._min_pending_count, self._max_pending_count)

    def GetCopy(self):

        return Predicate(self._predicate_type, self._value, self._inclusive,
                         self._min_current_count, self._min_pending_count,
                         self._max_current_count, self._max_pending_count)
    ...
    def GetNamespace(self):
        ...
        elif self._predicate_type in (HC.PREDICATE_TYPE_PARENT,
                                      HC.PREDICATE_TYPE_TAG,
                                      HC.PREDICATE_TYPE_WILDCARD):
    ...
    def GetInverseCopy(self):
        ...
        elif self._predicate_type in (HC.PREDICATE_TYPE_TAG,
                                      HC.PREDICATE_TYPE_NAMESPACE,
                                      HC.PREDICATE_TYPE_WILDCARD):

            return Predicate(self._predicate_type, self._value,
                             not self._inclusive)
    ...
    def GetTextsAndNamespaces(self, or_under_construction=False):
        ...
        if self._predicate_type == HC.PREDICATE_TYPE_OR_CONTAINER:
            ...
            for or_predicate in self._value:
                ...
                texts_and_namespaces.append(
                    (or_predicate.ToString(), or_predicate.GetNamespace()))
    ...
    def IsMutuallyExclusive(self, predicate):
        ...
        if my_type == other_type:
            ...
            if my_type in (HC.PREDICATE_TYPE_SYSTEM_LIMIT,
    ...
    def ToString(self,
        ...
        if with_count:
            ...
            if self._min_current_count > 0:
                ...
                if self._max_current_count is not None:
                    ...
                    number_text += '-' + HydrusData.ToHumanInt(
        ...
        if self._predicate_type in HC.SYSTEM_PREDICATES:
            ...
            if self._predicate_type == HC.PREDICATE_TYPE_SYSTEM_EVERYTHING:
                ...
                base = 'dimensions'
            elif self._predicate_type == HC.PREDICATE_TYPE_SYSTEM_FILE_RELATIONSHIPS:
                base = 'file relationships'
            elif self._predicate_type in (HC.PREDICATE_TYPE_SYSTEM_WIDTH,
                                          HC.PREDICATE_TYPE_SYSTEM_HEIGHT,
                                          HC.PREDICATE_TYPE_SYSTEM_NUM_WORDS):

                if self._predicate_type == HC.PREDICATE_TYPE_SYSTEM_WIDTH:
                    base = 'width'
                ...
                if self._value is not None:
                    ...
                    if operator == '>' and value == 0:
                        ...
                        base += ' ' + operator + ' ' + HydrusData.ConvertMillisecondsToPrettyTime(
                            value)
                ...
                if self._value is not None:
                    ...
                    if operator == '>' and value == 0:
                        ...
                        base += ' ' + operator + ' ' + HydrusData.ToHumanInt(
                            value)
                ...
                if self._value is not None:
                    ...
                    if age_type == 'delta':
                        ...
                        base += ': ' + pretty_operator + HydrusData.TimeDeltaToPrettyTimeDelta(
                            time_delta) + ' ago'
                        ...
                        base += ': ' + pretty_operator + HydrusData.ConvertTimestampToPrettyTime(
                            timestamp, include_24h_time=False)
                ...
                if self._value is not None:
                    ...
                    if len(hashes) == 1:
                        ...
                        base = '{} hash is {}'.format(hash_type,
                                                      hashes[0].hex())

                    else:

                        base = '{} hash is in {} hashes'.format(
                            hash_type, HydrusData.ToHumanInt(len(hashes)))
                ...
                if self._value is not None:
                    ...
                    try:
                        ...
                        base += ' for ' + service.GetName(
                        ) + ' ' + operator + ' ' + pretty_value
                ...
                if self._value is not None:
                    ...
                    base += ' {} files using max hamming of {}'.format(
                        HydrusData.ToHumanInt(len(hashes)), max_hamming)
                ...
                if self._value is not None:
                    ...
                    base += ' - has' + o_text + HydrusData.ToHumanInt(
                        num_relationships
                    ) + ' ' + HC.duplicate_type_string_lookup[dupe_type]
                ...
                if self._value is not None:
                    ...
                    if view_type == 'views':
                        ...
                        value_string = HydrusData.TimeDeltaToPrettyTimeDelta(
                            viewing_value)
            ...
            base += ' OR '.join(
                (or_predicate.ToString(render_for_user=render_for_user)
    ...
    def GetUnnamespacedCopy(self):
        ...
        if self._predicate_type == HC.PREDICATE_TYPE_TAG:
            ...
            return Predicate(self._predicate_type, subtag, self._inclusive,
                             self._min_current_count, self._min_pending_count,
                             self._max_current_count, self._max_pending_count)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_PREDICATE] = Predicate
...
SYSTEM_PREDICATE_NOT_LOCAL = Predicate(HC.PREDICATE_TYPE_SYSTEM_NOT_LOCAL,
                                       None)
