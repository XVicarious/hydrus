...
)  # stopping log-moaning when request sessions have verify = False
...
def LocalPortInUse(port):
    ...
    if HC.PLATFORM_WINDOWS:
        ...
        for sconn in psutil.net_connections():
            ...
            if port == sconn.laddr[1] and sconn.status in (
                    'ESTABLISHED', 'LISTEN'):  # local address: ( ip, port )
...
def ParseTwistedRequestGETArgs(requests_args, int_params, byte_params,
        ...
        if name in int_params:
            ...
            try:
                ...
                raise HydrusExceptions.BadRequestException(
                    'I was expecting to parse \'' + name +
                    '\' as an integer, but it failed.')
            ...
            try:
                ...
                raise HydrusExceptions.BadRequestException(
                    'I was expecting to parse \'' + name +
                    '\' as a percent-encdode string, but it failed.')
            ...
            try:
                ...
                raise HydrusExceptions.BadRequestException(
                    'I was expecting to parse \'' + name +
                    '\' as a json-encoded string, but it failed.')
            ...
            try:
                ...
                raise HydrusExceptions.BadRequestException(
                    'I was expecting to parse \'' + name +
                    '\' as a json-encoded hex strings, but it failed.')
...
class ParsedRequestArguments(dict):
    def __missing__(self, key):
        ...
        raise HydrusExceptions.BadRequestException(
            'It looks like the parameter "{}" was missing!'.format(key))
    ...
    def GetValue(self, key, expected_type, default_value=None):
        ...
        if key in self:
            ...
            if not isinstance(value, expected_type):
                ...
                raise HydrusExceptions.BadRequestException(
                    'The parameter "{}" was not the expected type: {}!'.format(
                        key, type_error_text))
            ...
            if default_value is None:
                ...
                raise HydrusExceptions.BadRequestException(
                    'The required parameter "{}" was missing!'.format(key))
...
class BandwidthRules(HydrusSerialisable.SerialisableBase):
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        self._rules = set(
            (tuple(rule_list) for rule_list in serialisable_info))
    ...
    def CanContinueDownload(self, bandwidth_tracker, threshold=15):
        ...
        with self._lock:
            ...
            for (bandwidth_type, time_delta, max_allowed) in self._rules:
                ...
                if bandwidth_tracker.GetUsage(bandwidth_type,
                                              time_delta) >= max_allowed:
        ...
        with self._lock:
            ...
            for (bandwidth_type, time_delta, max_allowed) in self._rules:
                ...
                if bandwidth_tracker.GetUsage(bandwidth_type,
                    ...
                    estimates.append(
                        bandwidth_tracker.GetWaitingEstimate(
                            bandwidth_type, time_delta, max_allowed))
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_BANDWIDTH_RULES] = BandwidthRules
...
class BandwidthTracker(HydrusSerialisable.SerialisableBase):
    ...
    def __init__(self):
        ...
        self._next_cache_maintenance_timestamp = HydrusData.GetNow(
        ) + self.CACHE_MAINTENANCE_TIME_DELTA
    ...
    def _GetSerialisableInfo(self):
        ...
        for d in (self._months_bytes, self._days_bytes, self._hours_bytes,
                  self._minutes_bytes, self._seconds_bytes,
                  self._months_requests, self._days_requests,
                  self._hours_requests, self._minutes_requests,
                  self._seconds_requests):
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        counters = [
            collections.Counter(dict(flat_dict))
            for flat_dict in serialisable_info
        ]
    ...
    def _GetRawUsage(self, bandwidth_type, time_delta):
        ...
        (window, counter) = self._GetWindowAndCounter(bandwidth_type,
                                                      time_delta)
        ...
        if time_delta == 1:
            ...
            return sum((value for (timestamp, value) in list(counter.items())
                        if timestamp >= since))
    ...
    def _MaintainCache(self):
        ...
        if HydrusData.TimeHasPassed(self._next_cache_maintenance_timestamp):
            ...
            def clear_counter(counter, timestamp):
                ...
                bad_keys = [
                    key for key in list(counter.keys()) if key < timestamp
                ]
    ...
    def ReportDataUsed(self, num_bytes):
        ...
        with self._lock:
            ...
            (month_time, day_time, hour_time, minute_time,
             second_time) = self._GetTimes(dt)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_BANDWIDTH_TRACKER] = BandwidthTracker
