...
class SimpleDownloaderImport(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_pending_jobs = [
            (url, simple_downloader_formula.GetSerialisableTuple())
            for (url, simple_downloader_formula) in self._pending_jobs
        ]
        ...
        return (serialisable_pending_jobs, serialisable_gallery_seed_log,
                serialisable_file_seed_cache, serialisable_file_import_options,
                self._formula_name, self._queue_paused, self._files_paused)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_pending_jobs, serialisable_gallery_seed_log,
         serialisable_file_seed_cache, serialisable_file_import_options,
         self._formula_name, self._queue_paused,
         self._files_paused) = serialisable_info

        self._pending_jobs = [(url,
                               HydrusSerialisable.CreateFromSerialisableTuple(
                                   serialisable_simple_downloader_formula))
                              for (url, serialisable_simple_downloader_formula
                                   ) in serialisable_pending_jobs]
    ...
    def _NetworkJobFactory(self, *args, **kwargs):
        ...
        network_job = ClientNetworkingJobs.NetworkJobDownloader(
            self._downloader_key, *args, **kwargs)
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (pending_page_urls, serialisable_file_seed_cache,
             serialisable_file_import_options, download_image_links,
             download_unlinked_images, paused) = old_serialisable_info

            queue_paused = paused
            files_paused = paused

            new_serialisable_info = (pending_page_urls,
                                     serialisable_file_seed_cache,
                                     serialisable_file_import_options,
                                     download_image_links,
                                     download_unlinked_images, queue_paused,
                                     files_paused)
        ...
        if version == 2:
            ...
            (pending_page_urls, serialisable_file_seed_cache,
             serialisable_file_import_options, download_image_links,
             download_unlinked_images, queue_paused,
             files_paused) = old_serialisable_info

            pending_jobs = []

            new_serialisable_info = (pending_jobs,
                                     serialisable_file_seed_cache,
                                     serialisable_file_import_options,
                                     queue_paused, files_paused)

            return (3, new_serialisable_info)

        if version == 3:

            (pending_jobs, serialisable_file_seed_cache,
             serialisable_file_import_options, queue_paused,
             files_paused) = old_serialisable_info
            ...
            new_serialisable_info = (pending_jobs,
                                     serialisable_file_seed_cache,
                                     serialisable_file_import_options,
                                     formula_name, queue_paused, files_paused)
        ...
        if version == 4:
            ...
            (pending_jobs, serialisable_file_seed_cache,
             serialisable_file_import_options, formula_name, queue_paused,
             files_paused) = old_serialisable_info
    ...
    def CurrentlyWorking(self):
        ...
        with self._lock:
            ...
            finished = not self._file_seed_cache.WorkToDo() or len(
    ...
    def GetStatus(self):
        ...
        with self._lock:
            ...
            return (list(self._pending_jobs), self._parser_status,
                    self._current_action, self._queue_paused,
                    self._files_paused)
    ...
    def Start(self, page_key):
        ...
        self._files_repeating_job = HG.client_controller.CallRepeating(
            ...
            self.REPEATINGWorkOnFiles, page_key)
        self._queue_repeating_job = HG.client_controller.CallRepeating(
            ClientImporting.GetRepeatingJobInitialDelay(),
            ClientImporting.REPEATING_JOB_TYPICAL_PERIOD,
            self.REPEATINGWorkOnQueue, page_key)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_SIMPLE_DOWNLOADER_IMPORT] = SimpleDownloaderImport
...
class URLsImport(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (serialisable_gallery_seed_log, serialisable_file_seed_cache,
                serialisable_file_import_options,
                serialisable_tag_import_options, self._paused)
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (serialisable_gallery_seed_log, serialisable_file_seed_cache,
         ...
         self._paused) = serialisable_info
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            new_serialisable_info = (serialisable_gallery_seed_log,
                                     serialisable_file_seed_cache,
                                     serialisable_file_import_options, paused)
        ...
        if version == 2:
            ...
            (serialisable_gallery_seed_log, serialisable_file_seed_cache,
             serialisable_file_import_options, paused) = old_serialisable_info
            ...
            new_serialisable_info = (serialisable_gallery_seed_log,
                                     serialisable_file_seed_cache,
                                     serialisable_file_import_options,
                                     serialisable_tag_import_options, paused)
    ...
    def Start(self, page_key):
        ...
        self._gallery_repeating_job = HG.client_controller.CallRepeating(
            ClientImporting.GetRepeatingJobInitialDelay(),
            ClientImporting.REPEATING_JOB_TYPICAL_PERIOD,
            self.REPEATINGWorkOnGallery, page_key)
    ...
    def REPEATINGWorkOnFiles(self, page_key):
        ...
        with self._lock:
            ...
            files_paused = self._paused or HG.client_controller.new_options.GetBoolean(
                'pause_all_file_queues')
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_URLS_IMPORT] = URLsImport
