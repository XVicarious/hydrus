...
def ConvertKeyEventToShortcut(event):
    ...
    if shortcut_type != CC.SHORTCUT_TYPE_NOT_ALLOWED:
        ...
        shortcut = Shortcut(shortcut_type, key_ord, shortcut_press_type,
                            modifiers)
...
def ConvertMouseEventToShortcut(event):
    ...
    if key is not None:
        ...
        shortcut = Shortcut(CC.SHORTCUT_TYPE_MOUSE, key, shortcut_press_type,
                            modifiers)
...
class Shortcut(HydrusSerialisable.SerialisableBase):
    ...
    def __init__(self,
        ...
        if shortcut_type == CC.SHORTCUT_TYPE_KEYBOARD_CHARACTER and ClientData.OrdIsAlphaUpper(
                shortcut_key):
    ...
    def __hash__(self):
        ...
        return (self.shortcut_type, self.shortcut_key,
                self.shortcut_press_type, tuple(self.modifiers)).__hash__()
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self.shortcut_type, self.shortcut_key,
                self.shortcut_press_type, self.modifiers)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self.shortcut_type, self.shortcut_key, self.shortcut_press_type,
         self.modifiers) = serialisable_info
    ...
    def ToString(self):
        ...
        if self.shortcut_press_type != CC.SHORTCUT_PRESS_TYPE_PRESS:
            ...
            action_name = '{} '.format(
                CC.shortcut_press_type_str_lookup[self.shortcut_press_type])
        ...
        if self.shortcut_type == CC.SHORTCUT_TYPE_MOUSE and self.shortcut_key in CC.shortcut_mouse_string_lookup:
            ...
            action_name += CC.special_key_shortcut_str_lookup[
                self.shortcut_key]
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_SHORTCUT] = Shortcut
...
class ShortcutSet(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def _GetSerialisableInfo(self):
        ...
        return [(shortcut.GetSerialisableTuple(),
                 command.GetSerialisableTuple())
                for (shortcut,
                     command) in list(self._shortcuts_to_commands.items())]
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            for (modifier, key, (serialisable_service_key,
                ...
                shortcut = Shortcut(CC.SHORTCUT_TYPE_KEYBOARD_CHARACTER, key,
                                    CC.SHORTCUT_PRESS_TYPE_PRESS, modifiers)
    ...
    def GetShortcuts(self, simple_command):
        ...
        for (shortcut, command) in self._shortcuts_to_commands.items():
            ...
            ) == CC.APPLICATION_COMMAND_TYPE_SIMPLE and command.GetData(
            ) == simple_command:
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_SHORTCUT_SET] = ShortcutSet


class ShortcutsHandler(QC.QObject):
    def __init__(self,
                 parent,
                 initial_shortcuts_names=None,
                 catch_mouse=False):
            ...
            if name in self._shortcuts:
                ...
                if command is not None:
                    ...
                    if HG.gui_report_mode:
                        ...
                        HydrusData.ShowText('command matched: ' +
                                            repr(command))
