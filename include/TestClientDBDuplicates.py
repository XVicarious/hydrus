...
class TestClientDBDuplicates(unittest.TestCase):
    ...
    def _read(self, action, *args, **kwargs):
        return TestClientDBDuplicates._db.Read(action, *args, **kwargs)

    def _write(self, action, *args, **kwargs):
        return TestClientDBDuplicates._db.Write(action, True, *args, **kwargs)
    ...
    def _test_initial_state(self):
        ...
        num_potentials = self._read('potential_duplicates_count',
        ...
        result = self._read('random_potential_duplicate_hashes',
                            self._file_search_context, both_files_match)

        self.assertEqual(len(result), len(self._all_hashes))

        self.assertEqual(set(result), self._all_hashes)

        filtering_pairs = self._read('duplicate_pairs_for_filtering',
                                     self._file_search_context,
                                     both_files_match)
        ...
        result = self._read('file_duplicate_info', CC.LOCAL_FILE_SERVICE_KEY,
                            self._dupe_hashes[0])
        ...
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_POTENTIAL],
            self._get_group_potential_count(file_duplicate_types_to_counts))

        result = self._read('file_duplicate_hashes', CC.LOCAL_FILE_SERVICE_KEY,
                            self._dupe_hashes[0], HC.DUPLICATE_POTENTIAL)
    ...
    def _test_initial_better_worse(self):
        ...
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_POTENTIAL],
            self._get_group_potential_count(file_duplicate_types_to_counts))
        self.assertEqual(file_duplicate_types_to_counts[HC.DUPLICATE_MEMBER],
                         len(self._our_main_dupe_group_hashes) - 1)
    ...
    def _test_initial_king_usurp(self):
        ...
        result = self._read('file_duplicate_hashes', CC.LOCAL_FILE_SERVICE_KEY,
                            self._old_king_hash, HC.DUPLICATE_KING)

        self.assertEqual(result, [self._old_king_hash, self._king_hash])

        result = self._read('file_duplicate_hashes', CC.LOCAL_FILE_SERVICE_KEY,
                            self._old_king_hash, HC.DUPLICATE_MEMBER)
    ...
    def _test_initial_same_quality(self):
        ...
        row = (HC.DUPLICATE_SAME_QUALITY, self._king_hash,
               self._dupe_hashes[4], {})
        ...
        row = (HC.DUPLICATE_SAME_QUALITY, self._old_king_hash,
               self._dupe_hashes[5], {})
    ...
    def _test_establish_second_group(self):
        ...
        rows.append((HC.DUPLICATE_BETTER, self._second_group_king_hash,
                     self._second_group_dupe_hashes[1], {}))
        rows.append((HC.DUPLICATE_SAME_QUALITY, self._second_group_king_hash,
                     self._second_group_dupe_hashes[2], {}))
        rows.append((HC.DUPLICATE_BETTER, self._second_group_king_hash,
                     self._second_group_dupe_hashes[3], {}))

        self._write('duplicate_pair_status', rows)

        self._our_second_dupe_group_hashes.add(
            self._second_group_dupe_hashes[1])
        self._our_second_dupe_group_hashes.add(
            self._second_group_dupe_hashes[2])
        self._our_second_dupe_group_hashes.add(
    ...
    def _test_poach_better(self):
        ...
        row = (HC.DUPLICATE_BETTER, self._king_hash,
               self._second_group_dupe_hashes[1], {})

        self._write('duplicate_pair_status', [row])

        self._our_second_dupe_group_hashes.discard(
            self._second_group_dupe_hashes[1])
        ...
        self.assertEqual(
            ...
            self._get_group_potential_count(file_duplicate_types_to_counts))
        self.assertEqual(file_duplicate_types_to_counts[HC.DUPLICATE_MEMBER],
                         len(self._our_second_dupe_group_hashes) - 1)
        ...
        result = self._read('file_duplicate_hashes', CC.LOCAL_FILE_SERVICE_KEY,
                            self._second_group_king_hash, HC.DUPLICATE_KING)

        self.assertEqual(result, [self._second_group_king_hash])

        result = self._read('file_duplicate_hashes', CC.LOCAL_FILE_SERVICE_KEY,
                            self._second_group_king_hash, HC.DUPLICATE_MEMBER)
    ...
    def _test_group_merge(self):
        ...
        rows.append((HC.DUPLICATE_BETTER, self._dupe_hashes[6],
                     self._dupe_hashes[7], {}))
        rows.append((HC.DUPLICATE_BETTER, self._dupe_hashes[8],
                     self._dupe_hashes[9], {}))
        rows.append((HC.DUPLICATE_BETTER, self._dupe_hashes[10],
                     self._dupe_hashes[11], {}))
        rows.append((HC.DUPLICATE_BETTER, self._dupe_hashes[12],
                     self._dupe_hashes[13], {}))

        self._write('duplicate_pair_status', rows)

        rows = []

        rows.append((HC.DUPLICATE_SAME_QUALITY, self._old_king_hash,
                     self._dupe_hashes[6], {}))
        rows.append((HC.DUPLICATE_SAME_QUALITY, self._king_hash,
                     self._dupe_hashes[8], {}))
        rows.append((HC.DUPLICATE_BETTER, self._old_king_hash,
                     self._dupe_hashes[10], {}))
        rows.append(
            (HC.DUPLICATE_BETTER, self._king_hash, self._dupe_hashes[12], {}))
        ...
        self._our_main_dupe_group_hashes.update(
            (self._dupe_hashes[i] for i in range(6, 14)))
    ...
    def _test_establish_false_positive_group(self):
        ...
        rows.append((HC.DUPLICATE_BETTER, self._false_positive_king_hash,
                     self._similar_looking_false_positive_hashes[1], {}))
        rows.append((HC.DUPLICATE_SAME_QUALITY, self._false_positive_king_hash,
                     self._similar_looking_false_positive_hashes[2], {}))
        ...
        self._our_fp_dupe_group_hashes.add(
            self._similar_looking_false_positive_hashes[1])
        self._our_fp_dupe_group_hashes.add(
            self._similar_looking_false_positive_hashes[2])
    ...
    def _test_false_positive(self):
        ...
        row = (HC.DUPLICATE_FALSE_POSITIVE, self._king_hash,
               self._false_positive_king_hash, {})
        ...
        self.assertEqual(file_duplicate_types_to_counts[HC.DUPLICATE_MEMBER],
                         len(self._our_main_dupe_group_hashes) - 1)
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_FALSE_POSITIVE], 1)
        ...
        self.assertEqual(
            ...
            self._get_group_potential_count(file_duplicate_types_to_counts))
        self.assertEqual(file_duplicate_types_to_counts[HC.DUPLICATE_MEMBER],
                         len(self._our_fp_dupe_group_hashes) - 1)
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_FALSE_POSITIVE], 1)

        result = self._read('file_duplicate_hashes', CC.LOCAL_FILE_SERVICE_KEY,
                            self._king_hash, HC.DUPLICATE_FALSE_POSITIVE)

        self.assertEqual(result,
                         [self._king_hash, self._false_positive_king_hash])
        ...
        self.assertEqual(result,
                         [self._false_positive_king_hash, self._king_hash])

    def _test_establish_alt_group(self):

        rows = []

        rows.append((HC.DUPLICATE_BETTER, self._alternate_king_hash,
                     self._similar_looking_alternate_hashes[1], {}))
        rows.append((HC.DUPLICATE_SAME_QUALITY, self._alternate_king_hash,
                     self._similar_looking_alternate_hashes[2], {}))
        ...
        self._our_alt_dupe_group_hashes.add(
            self._similar_looking_alternate_hashes[1])
        self._our_alt_dupe_group_hashes.add(
            self._similar_looking_alternate_hashes[2])
    ...
    def _test_alt(self):
        ...
        row = (HC.DUPLICATE_ALTERNATE, self._king_hash,
               self._alternate_king_hash, {})
        ...
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_FALSE_POSITIVE], 1)
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_ALTERNATE], 1)
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_CONFIRMED_ALTERNATE],
            1)

        result = self._read('file_duplicate_hashes', CC.LOCAL_FILE_SERVICE_KEY,
                            self._alternate_king_hash, HC.DUPLICATE_POTENTIAL)
        ...
        self.assertEqual(
            ...
            self._get_group_potential_count(file_duplicate_types_to_counts))
        self.assertEqual(file_duplicate_types_to_counts[HC.DUPLICATE_MEMBER],
                         len(self._our_alt_dupe_group_hashes) - 1)
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_FALSE_POSITIVE], 1)
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_ALTERNATE], 1)
        ...
        result = self._read('file_duplicate_hashes', CC.LOCAL_FILE_SERVICE_KEY,
                            self._king_hash, HC.DUPLICATE_ALTERNATE)

        self.assertEqual(result, [self._king_hash, self._alternate_king_hash])

        result = self._read('file_duplicate_hashes', CC.LOCAL_FILE_SERVICE_KEY,
                            self._alternate_king_hash, HC.DUPLICATE_ALTERNATE)
    ...
    def _test_expand_false_positive(self):
        ...
        rows.append((HC.DUPLICATE_BETTER, self._false_positive_king_hash,
                     self._similar_looking_false_positive_hashes[3], {}))
        rows.append((HC.DUPLICATE_SAME_QUALITY, self._false_positive_king_hash,
                     self._similar_looking_false_positive_hashes[4], {}))
        ...
        self.assertEqual(file_duplicate_types_to_counts[HC.DUPLICATE_MEMBER],
                         len(self._our_fp_dupe_group_hashes) - 1)
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_FALSE_POSITIVE], 2)
    ...
    def _test_expand_alt(self):
        ...
        rows.append((HC.DUPLICATE_BETTER, self._alternate_king_hash,
                     self._similar_looking_alternate_hashes[3], {}))
        rows.append((HC.DUPLICATE_SAME_QUALITY, self._alternate_king_hash,
                     self._similar_looking_alternate_hashes[4], {}))
    ...
    def _test_dissolve(self):
        ...
        rows.append((HC.DUPLICATE_ALTERNATE, self._king_hash,
                     self._false_positive_king_hash, {}))
        ...
        self.assertEqual(file_duplicate_types_to_counts[HC.DUPLICATE_MEMBER],
                         len(self._our_main_dupe_group_hashes) - 1)
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_ALTERNATE], 2)
        self.assertEqual(
            file_duplicate_types_to_counts[HC.DUPLICATE_CONFIRMED_ALTERNATE],
            2)

        self._write('remove_alternates_member',
                    (self._false_positive_king_hash, ))
        ...
        rows.append(
            (HC.DUPLICATE_BETTER, self._king_hash, self._dupe_hashes[1], {}))
    ...
    def test_duplicates(self):
        ...
        self._file_search_context = ClientSearch.FileSearchContext(
            file_service_key=CC.LOCAL_FILE_SERVICE_KEY, predicates=[size_pred])
