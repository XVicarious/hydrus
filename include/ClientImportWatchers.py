...
class MultipleWatcherImport(HydrusSerialisable.SerialisableBase):
    ...
    def _CleanAddedTimestamps(self):
        ...
        for key in keys:
            ...
            if HydrusData.TimeHasPassed(
                    self._watcher_keys_to_added_timestamps[key] +
                    self.ADDED_TIMESTAMP_DURATION):
        ...
        for key in keys:
            ...
            if HydrusData.TimeHasPassed(
                    self._watcher_keys_to_already_in_timestamps[key] +
                    self.ADDED_TIMESTAMP_DURATION):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (serialisable_watchers, self._highlighted_watcher_url,
                serialisable_checker_options, serialisable_file_import_options,
                serialisable_tag_import_options)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_watchers, self._highlighted_watcher_url,
         serialisable_checker_options, serialisable_file_import_options,
         serialisable_tag_import_options) = serialisable_info

        self._watchers = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_watchers)

        self._watcher_keys_to_watchers = {
            watcher.GetWatcherKey(): watcher
    ...
    def _RegenerateStatus(self):
        ...
        file_seed_caches = [
            watcher.GetFileSeedCache() for watcher in self._watchers
        ]
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            new_serialisable_info = (serialisable_watchers,
                                     serialisable_highlighted_watcher_key,
                                     serialisable_checker_options,
                                     serialisable_file_import_options,
                                     serialisable_tag_import_options)
    ...
    def GetAPIInfoDict(self, simple):
        ...
        with self._lock:
            ...
            d['watcher_imports'] = [
                watcher_import.GetAPIInfoDict(simple)
                for watcher_import in self._watchers
            ]
    ...
    def GetNumDead(self):
        ...
        with self._lock:
            ...
            return len(
                [watcher for watcher in self._watchers if watcher.IsDead()])
    ...
    def GetOptions(self):
        ...
        with self._lock:
            ...
            return (self._checker_options, self._file_import_options,
    ...
    def GetWatcherSimpleStatus(self, watcher):
        ...
        with self._lock:
            ...
            if watcher_key in self._watcher_keys_to_added_timestamps:
                ...
                added_timestamp = self._watcher_keys_to_added_timestamps[
                    watcher_key]

                if HydrusData.TimeHasPassed(added_timestamp +
                                            self.ADDED_TIMESTAMP_DURATION):
            ...
            if watcher_key in self._watcher_keys_to_already_in_timestamps:
                ...
                already_in_timestamp = self._watcher_keys_to_already_in_timestamps[
                    watcher_key]

                if HydrusData.TimeHasPassed(already_in_timestamp +
                                            self.ADDED_TIMESTAMP_DURATION):
    ...
    def SetOptions(self, checker_options, file_import_options,
                   tag_import_options):
    ...
    def Start(self, page_key):
        ...
        self._watchers_repeating_job = HG.client_controller.CallRepeating(
            ClientImporting.GetRepeatingJobInitialDelay(), 2.0,
            self.REPEATINGWorkOnWatchers)

        for watcher in self._watchers:

            publish_to_page = False

            if self._highlighted_watcher_url is not None and watcher.GetURL(
            ) == self._highlighted_watcher_url:
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_MULTIPLE_WATCHER_IMPORT] = MultipleWatcherImport
...
class WatcherImport(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._url, serialisable_gallery_seed_log,
                ...
                serialisable_checker_options, serialisable_file_import_options,
                serialisable_tag_import_options, self._last_check_time,
                self._files_paused, self._checking_paused,
                self._checking_status, self._subject, self._no_work_until,
                self._no_work_until_reason, self._creation_time)
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (self._url, serialisable_gallery_seed_log,
         serialisable_file_seed_cache, serialisable_fixed_service_keys_to_tags,
         serialisable_checker_options, serialisable_file_import_options,
         serialisable_tag_import_options, self._last_check_time,
         self._files_paused, self._checking_paused, self._checking_status,
         self._subject, self._no_work_until, self._no_work_until_reason,
         self._creation_time) = serialisable_info
    ...
    def _UpdateFileVelocityStatus(self):
        ...
        self._file_velocity_status = self._checker_options.GetPrettyCurrentVelocity(
        ...
        if self._check_now:
            ...
            if not HydrusData.TimeHasPassed(self._no_work_until):
                ...
                if self._checking_status == ClientImporting.CHECKER_STATUS_OK:
                    ...
                    if self._checker_options.IsDead(self._file_seed_cache,
                ...
                self._next_check_time = self._checker_options.GetNextCheckTime(
                    self._file_seed_cache, self._last_check_time,
    ...
    def GetOptions(self):
        ...
        with self._lock:
            ...
            return (self._url, self._file_import_options,
                    self._tag_import_options)
    ...
    def _IsDead(self):
        ...
        return self._checking_status in (ClientImporting.CHECKER_STATUS_404,
                                         ClientImporting.CHECKER_STATUS_DEAD)
    ...
    def Start(self, page_key, publish_to_page):
        ...
        self._checker_repeating_job = HG.client_controller.CallRepeating(
            ClientImporting.GetRepeatingJobInitialDelay(),
            ClientImporting.REPEATING_JOB_TYPICAL_PERIOD,
            self.REPEATINGWorkOnChecker)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_WATCHER_IMPORT] = WatcherImport
