...
def GetDuplicateComparisonScore(shown_media, comparison_media):
    ...
    statements_and_scores = GetDuplicateComparisonStatements(
        shown_media, comparison_media)

    total_score = sum(
        (score for (statement, score) in statements_and_scores.values()))
...
def GetDuplicateComparisonStatements(shown_media, comparison_media):
    ...
    duplicate_comparison_score_higher_jpeg_quality = new_options.GetInteger(
        'duplicate_comparison_score_higher_jpeg_quality')
    duplicate_comparison_score_much_higher_jpeg_quality = new_options.GetInteger(
        'duplicate_comparison_score_much_higher_jpeg_quality')
    duplicate_comparison_score_higher_filesize = new_options.GetInteger(
        'duplicate_comparison_score_higher_filesize')
    duplicate_comparison_score_much_higher_filesize = new_options.GetInteger(
        'duplicate_comparison_score_much_higher_filesize')
    duplicate_comparison_score_higher_resolution = new_options.GetInteger(
        'duplicate_comparison_score_higher_resolution')
    duplicate_comparison_score_much_higher_resolution = new_options.GetInteger(
        'duplicate_comparison_score_much_higher_resolution')
    duplicate_comparison_score_more_tags = new_options.GetInteger(
        'duplicate_comparison_score_more_tags')
    duplicate_comparison_score_older = new_options.GetInteger(
    ...
    if s_size != c_size:
        ...
        statement = '{} {} {}'.format(HydrusData.ToHumanBytes(s_size),
                                      operator,
    ...
    if s_mime != c_mime:
        ...
        statement = '{} vs {}'.format(HC.mime_string_lookup[s_mime],
                                      HC.mime_string_lookup[c_mime])
        score = 0
    ...
    if s_num_tags != c_num_tags:
        ...
        statement = '{} tags {} {} tags'.format(
            HydrusData.ToHumanInt(s_num_tags), operator,
            HydrusData.ToHumanInt(c_num_tags))

        statements_and_scores['num_tags'] = (statement, score)

    # older

    s_ts = shown_media.GetLocationsManager().GetTimestamp(
        CC.COMBINED_LOCAL_FILE_SERVICE_KEY)
    c_ts = comparison_media.GetLocationsManager().GetTimestamp(
        CC.COMBINED_LOCAL_FILE_SERVICE_KEY)
    ...
    if s_ts is not None and c_ts is not None and abs(s_ts - c_ts) > one_month:
        ...
        statement = '{} {} {}'.format(
            HydrusData.TimestampToPrettyTimeDelta(s_ts), operator,
            HydrusData.TimestampToPrettyTimeDelta(c_ts))
    ...
    if s_mime == HC.IMAGE_JPEG and c_mime == HC.IMAGE_JPEG:
        ...
        if s_hash not in hashes_to_jpeg_quality:
            ...
            hashes_to_jpeg_quality[
                s_hash] = HydrusImageHandling.GetJPEGQuantizationQualityEstimate(
                    path)
        ...
        if c_hash not in hashes_to_jpeg_quality:
            ...
            hashes_to_jpeg_quality[
                c_hash] = HydrusImageHandling.GetJPEGQuantizationQualityEstimate(
                    path)
    ...
    if shown_media.IsStaticImage() and comparison_media.IsStaticImage(
    ) and shown_media.GetResolution() == comparison_media.GetResolution():
        ...
        if s_hash not in hashes_to_pixel_hashes:
            ...
            hashes_to_pixel_hashes[
                s_hash] = HydrusImageHandling.GetImagePixelHash(path, s_mime)
        ...
        if c_hash not in hashes_to_pixel_hashes:
            ...
            hashes_to_pixel_hashes[
                c_hash] = HydrusImageHandling.GetImagePixelHash(path, c_mime)
...
def GetMediasTagCount(pool, tag_service_key, tag_display_type):
    ...
    for tags_manager in tags_managers:
        ...
        statuses_to_tags = tags_manager.GetStatusesToTags(
            tag_service_key, tag_display_type)

        current_tags_to_count.update(
            statuses_to_tags[HC.CONTENT_STATUS_CURRENT])
        deleted_tags_to_count.update(
            statuses_to_tags[HC.CONTENT_STATUS_DELETED])
        pending_tags_to_count.update(
            statuses_to_tags[HC.CONTENT_STATUS_PENDING])
        petitioned_tags_to_count.update(
            statuses_to_tags[HC.CONTENT_STATUS_PETITIONED])

    return (current_tags_to_count, deleted_tags_to_count,
            pending_tags_to_count, petitioned_tags_to_count)


class MediaResult(object):
    def __init__(self, file_info_manager, tags_manager, locations_manager,
                 ratings_manager, file_viewing_stats_manager):
    ...
    def Duplicate(self):
        ...
        return MediaResult(file_info_manager, tags_manager, locations_manager,
                           ratings_manager, file_viewing_stats_manager)
    ...
    def IsStaticImage(self):
        ...
        static_animation = self._file_info_manager.mime in HC.ANIMATIONS and self._file_info_manager.duration in (
            0, None)
    ...
    def ProcessContentUpdate(self, service_key, content_update):
        ...
        if service_type in HC.TAG_SERVICES:
            ...
            self._tags_manager.ProcessContentUpdate(service_key,
                                                    content_update)

        elif service_type in HC.FILE_SERVICES:

            if content_update.GetDataType(
            ) == HC.CONTENT_TYPE_FILE_VIEWING_STATS:

                self._file_viewing_stats_manager.ProcessContentUpdate(
                    content_update)

            else:

                self._locations_manager.ProcessContentUpdate(
                    service_key, content_update)

        elif service_type in HC.RATINGS_SERVICES:

            self._ratings_manager.ProcessContentUpdate(service_key,
                                                       content_update)
    ...
    def ToTuple(self):
        ...
        return (self._file_info_manager, self._tags_manager,
                self._locations_manager, self._ratings_manager)
...
class DuplicatesManager(object):
    ...
    def Duplicate(self):
        ...
        service_keys_to_dupe_statuses_to_counts = collections.defaultdict(
            collections.Counter)
...
class FileInfoManager(object):
    ...
    def Duplicate(self):
        ...
        return FileInfoManager(self.hash_id, self.hash, self.size, self.mime,
                               self.width, self.height, self.duration,
                               self.num_frames, self.has_audio, self.num_words)

    def ToTuple(self):

        return (self.hash_id, self.hash, self.size, self.mime, self.width,
                self.height, self.duration, self.num_frames, self.has_audio,
                self.num_words)


class FileViewingStatsManager(object):
    def __init__(self, preview_views, preview_viewtime, media_views,
                 media_viewtime):
    ...
    def Duplicate(self):
        ...
        return FileViewingStatsManager(self.preview_views,
                                       self.preview_viewtime, self.media_views,
                                       self.media_viewtime)
    ...
    def ProcessContentUpdate(self, content_update):
        ...
        if action == HC.CONTENT_UPDATE_ADD:
            ...
            (hash, preview_views_delta, preview_viewtime_delta,
             media_views_delta, media_viewtime_delta) = row
...
class LocationsManager(object):
    ...
    def __init__(self,
                 ...
                 inbox=False,
                 urls=None,
                 service_keys_to_filenames=None,
                 current_to_timestamps=None,
    ...
    def Duplicate(self):
        ...
        return LocationsManager(current, deleted, pending, petitioned,
                                self.inbox, urls, service_keys_to_filenames,
                                current_to_timestamps,
                                self._file_modified_timestamp)
    ...
    def GetRemoteLocationStrings(self):
        ...
        remote_services = HG.client_controller.services_manager.GetServices(
            (HC.FILE_REPOSITORY, HC.IPFS))
            ...
            if action == HC.CONTENT_UPDATE_ARCHIVE:
                ...
                if service_key == CC.LOCAL_FILE_SERVICE_KEY:
                    ...
                    if CC.COMBINED_LOCAL_FILE_SERVICE_KEY not in self._current:
                        ...
                        self._current_to_timestamps[
                            CC.
                            COMBINED_LOCAL_FILE_SERVICE_KEY] = HydrusData.GetNow(
                            )
                ...
                if service_key == CC.LOCAL_FILE_SERVICE_KEY:
                    ...
                    self._current_to_timestamps[
                        CC.TRASH_SERVICE_KEY] = HydrusData.GetNow()
...
class MediaCollect(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_rating_service_keys = [
            key.hex() for key in self.rating_service_keys
        ]

        return (self.namespaces, serialisable_rating_service_keys,
                self.collect_unmatched)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self.namespaces, serialisable_rating_service_keys,
         self.collect_unmatched) = serialisable_info

        self.rating_service_keys = [
            bytes.fromhex(serialisable_key)
            for serialisable_key in serialisable_rating_service_keys
        ]
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_MEDIA_COLLECT] = MediaCollect
...
class MediaList(object):
    def __init__(self, file_service_key, media_results):
        ...
        self._media_sort = MediaSort(('system', CC.SORT_FILES_BY_FILESIZE),
                                     CC.SORT_ASC)
    ...
    def __len__(self):
        ...
        return len(self._singleton_media) + sum(map(len,
    ...
    def _CalculateCollectionKeysToMedias(self, media_collect, medias):
        ...
        for media in medias:
            ...
            if len(namespaces_to_collect_by) > 0:
                ...
                namespace_key = media.GetTagsManager().GetNamespaceSlice(
                    namespaces_to_collect_by,
                    ClientTags.TAG_DISPLAY_SIBLINGS_AND_PARENTS)
            ...
            if len(ratings_to_collect_by) > 0:
                ...
                rating_key = media.GetRatingsManager().GetRatingSlice(
                    ratings_to_collect_by)
    ...
    def _GetMedia(self, hashes, discriminator=None):
        ...
        if discriminator is None or discriminator == 'singletons':
            ...
            medias.extend((self._hashes_to_singleton_media[hash]
                           for hash in hashes
                           if hash in self._hashes_to_singleton_media))

        if discriminator is None or discriminator == 'collections':

            medias.extend({
                self._hashes_to_collected_media[hash]
                for hash in hashes if hash in self._hashes_to_collected_media
            })
    ...
    def _RemoveMediaByHashes(self, hashes):
        ...
        affected_singleton_media = self._GetMedia(hashes,
                                                  discriminator='singletons')

        for media in self._collected_media:

            media._RemoveMediaByHashes(hashes)

        affected_collected_media = [
            media for media in self._collected_media if media.HasNoMedia()
        ]
    ...
    def Collect(self, media_collect=None):
        ...
        if self._media_collect.DoesACollect():
            ...
            )  #{ medias[0] for ( key, medias ) in keys_to_medias.items() if len( medias ) == 1 }
    ...
    def GetFilteredFileCount(self, file_filter):
        ...
        if file_filter.filter_type == FILE_FILTER_ALL:
            ...
            return self.GetNumFiles() - sum(
                (m.GetNumFiles() for m in self._selected_media))
            ...
            return self.GetNumFiles() - sum(
                (m.GetNumInbox() for m in self._selected_media))
            ...
            if file_filter.filter_type == FILE_FILTER_FILE_SERVICE:
                ...
                return sum((1 for m in flat_media if file_service_key in
                            m.GetLocationsManager().GetCurrent()))

            elif file_filter.filter_type == FILE_FILTER_LOCAL:

                return sum((1 for m in flat_media
                            if m.GetLocationsManager().IsLocal()))

            elif file_filter.filter_type == FILE_FILTER_REMOTE:

                return sum((1 for m in flat_media
                            if m.GetLocationsManager().IsRemote()))
                ...
                if and_or_or == 'AND':
                    ...
                    return sum(
                        (1 for m in flat_media
                         if len(m.GetTagsManager().GetCurrentAndPending(
                             CC.COMBINED_TAG_SERVICE_KEY, ClientTags.
                             TAG_DISPLAY_SIBLINGS_AND_PARENTS).intersection(
                                 select_tags)) == len(select_tags)))
    ...
    def GetFilteredHashes(self, file_filter):
        ...
        if file_filter.filter_type == FILE_FILTER_ALL:
            ...
            if file_filter.filter_type == FILE_FILTER_INBOX:
                ...
                filtered_media = [
                    m for m in flat_media if file_service_key in
                    m.GetLocationsManager().GetCurrent()
                ]
                ...
                filtered_media = [
                    m for m in flat_media if m.GetLocationsManager().IsLocal()
                ]

            elif file_filter.filter_type == FILE_FILTER_REMOTE:

                filtered_media = [
                    m for m in flat_media
                    if m.GetLocationsManager().IsRemote()
                ]
    ...
    def GetFilteredMedia(self, file_filter):
        ...
        if file_filter.filter_type == FILE_FILTER_ALL:
            ...
            return {
                m
                for m in self._sorted_media if m not in self._selected_media
            }
            ...
            if file_filter.filter_type == FILE_FILTER_INBOX:
                ...
                filtered_media = {
                    m
                    for m in self._sorted_media if m.HasInbox()
                }
                ...
                filtered_media = {
                    m
                    for m in self._sorted_media if not m.HasInbox()
                }
                ...
                filtered_media = {
                    m
                    for m in self._sorted_media if file_service_key in
                    m.GetLocationsManager().GetCurrent()
                }
    ...
    def GenerateMediaResults(self,
                             has_location=None,
                             discriminant=None,
                             selected_media=None,
                             unrated=None,
                             for_media_viewer=False):
        ...
        for media in self._sorted_media:
            ...
            if media.IsCollection():
                ...
                media_results.extend(
                    media.GenerateMediaResults(has_location=has_location,
                ...
                if discriminant is not None:
                    ...
                    if discriminant == CC.DISCRIMINANT_INBOX:
                        ...
                        p = locations_manager.IsLocal(
                        ) and not locations_manager.IsTrashed()
    ...
    def GetNumArchive(self):
        ...
        num_archive = sum(
            (1 for m in self._singleton_media if not m.HasInbox())) + sum(
                (m.GetNumArchive() for m in self._collected_media))
    ...
    def GetNumInbox(self):
        ...
        num_inbox = sum(
            (1 for m in self._singleton_media if m.HasInbox())) + sum(
                (m.GetNumInbox() for m in self._collected_media))
            ...
            for content_update in content_updates:
                ...
                if data_type == HC.CONTENT_TYPE_FILES:
                    ...
                    if action == HC.CONTENT_UPDATE_DELETE:
                        ...
                        local_file_domains = HG.client_controller.services_manager.GetServiceKeys(
                            (HC.LOCAL_FILE_DOMAIN, ))
                        non_trash_local_file_services = list(
                            local_file_domains) + [
                                CC.COMBINED_LOCAL_FILE_SERVICE_KEY
                            ]
                        all_local_file_services = list(
                            non_trash_local_file_services) + [
                                CC.TRASH_SERVICE_KEY
                            ]

                        #

                        physically_deleted = service_key in (
                            CC.TRASH_SERVICE_KEY,
                            CC.COMBINED_LOCAL_FILE_SERVICE_KEY)
                        ...
                        user_says_remove_and_trashed_from_non_trash_local_view = HC.options[
                            'remove_trashed_files'] and trashed and deleted_from_our_domain
    ...
    def ResetService(self, service_key):
        ...
        if service_key == self._file_service_key:
            ...
            self._RemoveMediaDirectly(self._singleton_media,
                                      self._collected_media)

        else:

            for media in self._collected_media:
                media.ResetService(service_key)
    ...
    def Sort(self, media_sort=None):
        ...
        (sort_key, reverse) = media_sort_fallback.GetSortKeyAndReverse(
        ...
        (sort_key, reverse) = self._media_sort.GetSortKeyAndReverse(
            self._file_service_key)
...
class FileFilter(object):
    ...
    def PopulateFilterCounts(self, media_list: MediaList, filter_counts: dict):
        ...
        if self not in filter_counts:
            ...
            quick_inverse_lookups[FileFilter(FILE_FILTER_INBOX)] = FileFilter(
                FILE_FILTER_ARCHIVE)
            quick_inverse_lookups[FileFilter(
                FILE_FILTER_ARCHIVE)] = FileFilter(FILE_FILTER_INBOX)
            quick_inverse_lookups[FileFilter(
                FILE_FILTER_SELECTED)] = FileFilter(FILE_FILTER_NOT_SELECTED)
            quick_inverse_lookups[FileFilter(
                FILE_FILTER_NOT_SELECTED)] = FileFilter(FILE_FILTER_SELECTED)
            quick_inverse_lookups[FileFilter(FILE_FILTER_LOCAL)] = FileFilter(
                FILE_FILTER_REMOTE)
            quick_inverse_lookups[FileFilter(FILE_FILTER_REMOTE)] = FileFilter(
                FILE_FILTER_LOCAL)
            ...
            if self in quick_inverse_lookups:
                ...
                if all_filter in filter_counts and inverse in filter_counts:
                    ...
                    filter_counts[self] = filter_counts[
                        all_filter] - filter_counts[inverse]
...
class ListeningMediaList(MediaList):
    def __init__(self, file_service_key, media_results):
        ...
        HG.client_controller.sub(self, 'ProcessContentUpdates',
                                 'content_updates_gui')
        HG.client_controller.sub(self, 'ProcessServiceUpdates',
                                 'service_updates_gui')
...
class MediaCollection(MediaList, Media):
    ...
    def _RecalcInternals(self):
        ...
        self._archive = True in (media.HasArchive()
                                 for media in self._sorted_media)
        self._inbox = True in (media.HasInbox()
                               for media in self._sorted_media)
        ...
        duration_sum = sum([
            media.GetDuration() for media in self._sorted_media
        ...
        self._has_audio = True in (media.HasAudio()
                                   for media in self._sorted_media)
        ...
        current = HydrusData.MassUnion([
            locations_manager.GetCurrent()
            for locations_manager in all_locations_managers
        ])
        deleted = HydrusData.MassUnion([
            locations_manager.GetDeleted()
            for locations_manager in all_locations_managers
        ])
        pending = HydrusData.MassUnion([
            locations_manager.GetPending()
            for locations_manager in all_locations_managers
        ])
        petitioned = HydrusData.MassUnion([
            locations_manager.GetPetitioned()
            for locations_manager in all_locations_managers
        ])

        self._locations_manager = LocationsManager(current, deleted, pending,
                                                   petitioned)
    ...
    def _RecalcFileViewingStats(self):
        ...
        self._file_viewing_stats_manager = FileViewingStatsManager(
            preview_views, preview_viewtime, media_views, media_viewtime)
    ...
    def GetPrettyInfoLines(self):
        ...
        info_string += ' (' + HydrusData.ToHumanInt(
    ...
    def ProcessContentUpdates(self, service_keys_to_content_updates):
        ...
        for (service_key,
             content_updates) in service_keys_to_content_updates.items():
...
class MediaSingleton(Media):
    ...
    def GetHashes(self,
        ...
        if self.MatchesDiscriminant(has_location=has_location,
                                    discriminant=discriminant,
    ...
    def GetNumFrames(self):
        return self._media_result.GetNumFrames()
    ...
    def GetNumWords(self):
        return self._media_result.GetNumWords()

    def GetTimestamp(self, service_key):

        return self._media_result.GetLocationsManager().GetTimestamp(
            service_key)
    ...
    def GetPrettyInfoLines(self):
        ...
        (hash_id, hash, size, mime, width, height, duration, num_frames,
         has_audio, num_words) = file_info_manager.ToTuple()

        info_string = HydrusData.ToHumanBytes(
            size) + ' ' + HC.mime_string_lookup[mime]

        if width is not None and height is not None:
            info_string += ' (' + HydrusData.ToHumanInt(
                width) + 'x' + HydrusData.ToHumanInt(height) + ')'

        if duration is not None:
            info_string += ', ' + HydrusData.ConvertMillisecondsToPrettyTime(
                duration)
        ...
        if has_audio:
            ...
            info_string += ', {}'.format(
                HG.client_controller.new_options.GetString('has_audio_label'))

        if num_words is not None:
            info_string += ' (' + HydrusData.ToHumanInt(num_words) + ' words)'
        ...
        if CC.COMBINED_LOCAL_FILE_SERVICE_KEY in current_service_keys:
            ...
            lines.append('imported ' +
                         HydrusData.TimestampToPrettyTimeDelta(timestamp))
        ...
        if file_modified_timestamp is not None:
            ...
            lines.append('file modified: {}'.format(
                HydrusData.TimestampToPrettyTimeDelta(
                    file_modified_timestamp)))

        for service_key in current_service_keys:

            if service_key in (CC.COMBINED_LOCAL_FILE_SERVICE_KEY,
            ...
            lines.append(status + 'to ' + service.GetName() + ' ' +
                         HydrusData.TimestampToPrettyTimeDelta(timestamp))

        return lines

    def GetRatingsManager(self):
        return self._media_result.GetRatingsManager()
    ...
    def GetTitleString(self):
        ...
        tag_summary_generator = new_options.GetTagSummaryGenerator(
            'media_viewer_top')
...
class MediaSort(HydrusSerialisable.SerialisableBase):
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (sort_metatype, serialisable_sort_data,
         self.sort_asc) = serialisable_info
        ...
        if sort_metadata == 'system':
            ...
            if sort_data == CC.SORT_FILES_BY_RANDOM:
                ...
                def sort_key(x):
                    ...
                    return deal_with_none(
                        x.GetLocationsManager().GetFileModifiedTimestamp())
                ...
                def sort_key(x):
                    ...
                    return len(
                        tags_manager.GetCurrentAndPending(
                            CC.COMBINED_TAG_SERVICE_KEY,
            ...
            def sort_key(x):
                ...
                return [
                    x_tags_manager.GetComparableNamespaceSlice(
                        (namespace, ), ClientTags.TAG_DISPLAY_SINGLE_MEDIA)
            ...
            def sort_key(x):
                ...
                rating = deal_with_none(
                    x_ratings_manager.GetRating(service_key))
    ...
    def GetSortTypeString(self):
        ...
        if sort_metatype == 'system':
            ...
            sort_string_lookup[
                CC.SORT_FILES_BY_DURATION] = 'dimensions: duration'
            sort_string_lookup[CC.SORT_FILES_BY_HEIGHT] = 'dimensions: height'
            sort_string_lookup[
                CC.SORT_FILES_BY_NUM_PIXELS] = 'dimensions: number of pixels'
            sort_string_lookup[
                CC.SORT_FILES_BY_RATIO] = 'dimensions: resolution ratio'
            sort_string_lookup[CC.SORT_FILES_BY_WIDTH] = 'dimensions: width'
            sort_string_lookup[
                CC.SORT_FILES_BY_APPROX_BITRATE] = 'file: approximate bitrate'
            ...
            sort_string_lookup[
                CC.SORT_FILES_BY_IMPORT_TIME] = 'file: time imported'
            sort_string_lookup[
                CC.
                SORT_FILES_BY_FILE_MODIFIED_TIMESTAMP] = 'file: modified time'
            sort_string_lookup[CC.SORT_FILES_BY_RANDOM] = 'random'
            sort_string_lookup[
                CC.SORT_FILES_BY_NUM_TAGS] = 'tags: number of tags'
            sort_string_lookup[
                CC.SORT_FILES_BY_MEDIA_VIEWS] = 'views: media views'
            sort_string_lookup[
                CC.SORT_FILES_BY_MEDIA_VIEWTIME] = 'views: media viewtime'
    ...
    def GetSortAscStrings(self):
        ...
        if sort_metatype == 'system':
            ...
            sort_string_lookup[CC.SORT_FILES_BY_APPROX_BITRATE] = (
                'smallest first', 'largest first', CC.SORT_DESC)
            sort_string_lookup[CC.SORT_FILES_BY_FILESIZE] = ('smallest first',
                                                             'largest first',
                                                             CC.SORT_DESC)
            sort_string_lookup[CC.SORT_FILES_BY_DURATION] = ('shortest first',
                                                             'longest first',
                                                             CC.SORT_DESC)
            sort_string_lookup[CC.SORT_FILES_BY_HAS_AUDIO] = ('audio first',
                                                              'silent first',
                                                              CC.SORT_ASC)
            sort_string_lookup[CC.SORT_FILES_BY_IMPORT_TIME] = ('oldest first',
                                                                'newest first',
                                                                CC.SORT_DESC)
            sort_string_lookup[CC.SORT_FILES_BY_FILE_MODIFIED_TIMESTAMP] = (
                'oldest first', 'newest first', CC.SORT_DESC)
            sort_string_lookup[CC.SORT_FILES_BY_MIME] = ('mime', 'mime',
                                                         CC.SORT_ASC)
            sort_string_lookup[CC.SORT_FILES_BY_RANDOM] = ('random', 'random',
                                                           CC.SORT_ASC)
            sort_string_lookup[CC.SORT_FILES_BY_WIDTH] = ('slimmest first',
                                                          'widest first',
                                                          CC.SORT_ASC)
            sort_string_lookup[CC.SORT_FILES_BY_HEIGHT] = ('shortest first',
                                                           'tallest first',
                                                           CC.SORT_ASC)
            sort_string_lookup[CC.SORT_FILES_BY_RATIO] = ('tallest first',
                                                          'widest first',
                                                          CC.SORT_ASC)
            sort_string_lookup[CC.SORT_FILES_BY_NUM_PIXELS] = ('ascending',
                                                               'descending',
                                                               CC.SORT_DESC)
            sort_string_lookup[CC.SORT_FILES_BY_NUM_TAGS] = ('ascending',
                                                             'descending',
                                                             CC.SORT_ASC)
            sort_string_lookup[CC.SORT_FILES_BY_MEDIA_VIEWS] = ('ascending',
                                                                'descending',
                                                                CC.SORT_DESC)
            sort_string_lookup[CC.SORT_FILES_BY_MEDIA_VIEWTIME] = (
                'ascending', 'descending', CC.SORT_DESC)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_MEDIA_SORT] = MediaSort
...
class SortedList(object):
    ...
    def _RecalcIndices(self):
        ...
        self._items_to_indices = {
            item: index
            for (index, item) in enumerate(self._sorted_list)
        }
...
class TagsManager(object):
    def __init__(self, service_keys_to_statuses_to_tags):
        ...
        self._tag_display_types_to_service_keys_to_statuses_to_tags = {
            ClientTags.TAG_DISPLAY_STORAGE: service_keys_to_statuses_to_tags
        }
    ...
    def _GetServiceKeysToStatusesToTags(self, tag_display_type):
        ...
        return self._tag_display_types_to_service_keys_to_statuses_to_tags[
            tag_display_type]
    ...
    def _RecalcCaches(self):
        ...
        if self._cache_is_dirty:
            ...
            source_service_keys_to_statuses_to_tags = self._tag_display_types_to_service_keys_to_statuses_to_tags[
                ClientTags.TAG_DISPLAY_STORAGE]

            destination_service_keys_to_statuses_to_tags = collections.defaultdict(
                HydrusData.default_dict_set)

            for (service_key, source_statuses_to_tags
                 ) in source_service_keys_to_statuses_to_tags.items():

                destination_statuses_to_tags = tag_siblings_manager.CollapseStatusesToTags(
                    service_key, source_statuses_to_tags)

                destination_service_keys_to_statuses_to_tags[
                    service_key] = destination_statuses_to_tags

            self._tag_display_types_to_service_keys_to_statuses_to_tags[
                ClientTags.
                TAG_DISPLAY_SIBLINGS_AND_PARENTS] = destination_service_keys_to_statuses_to_tags
            ...
            cache_jobs.append((ClientTags.TAG_DISPLAY_SIBLINGS_AND_PARENTS,
                               ClientTags.TAG_DISPLAY_SINGLE_MEDIA))
            cache_jobs.append((ClientTags.TAG_DISPLAY_SIBLINGS_AND_PARENTS,
            ...
            for (source_tag_display_type, dest_tag_display_type) in cache_jobs:
                ...
                for (service_key, source_statuses_to_tags
                    ...
                    if tag_display_manager.FiltersTags(dest_tag_display_type,
                                                       service_key):
                        ...
                        for (status,
                             source_tags) in source_statuses_to_tags.items():

                            dest_tags = tag_display_manager.FilterTags(
                                dest_tag_display_type, service_key,
                                source_tags)

                            if len(source_tags) != len(dest_tags):

                                destination_statuses_to_tags[
                                    status] = dest_tags
                ...
                self._tag_display_types_to_service_keys_to_statuses_to_tags[
                    dest_tag_display_type] = destination_service_keys_to_statuses_to_tags

            # combined service merge calculation
            # would be great if this also could be db pre-computed

            for (
                    tag_display_type, service_keys_to_statuses_to_tags
            ) in self._tag_display_types_to_service_keys_to_statuses_to_tags.items(
            ):

                combined_statuses_to_tags = HydrusData.default_dict_set()

                for (service_key, statuses_to_tags
                     ) in service_keys_to_statuses_to_tags.items():
                ...
                service_keys_to_statuses_to_tags[
                    CC.COMBINED_TAG_SERVICE_KEY] = combined_statuses_to_tags
    ...
    def MergeTagsManagers(tags_managers):
        ...
        s_k_s_t_t_tupled = (CurrentAndPendingFilter(
            tags_manager.GetServiceKeysToStatusesToTags(
                ClientTags.TAG_DISPLAY_STORAGE).items())
                            for tags_manager in tags_managers)
    ...
    def DeletePending(self, service_key):
        ...
        if len(statuses_to_tags[HC.CONTENT_STATUS_PENDING]) + len(
                statuses_to_tags[HC.CONTENT_STATUS_PETITIONED]) > 0:
    ...
    def Duplicate(self):
        ...
        for (
            ...
            dupe_service_keys_to_statuses_to_tags = collections.defaultdict(
                HydrusData.default_dict_set)

            for (service_key,
                 statuses_to_tags) in service_keys_to_statuses_to_tags.items():
                ...
                dupe_service_keys_to_statuses_to_tags[
                    service_key] = dupe_statuses_to_tags

            dupe_tag_display_types_to_service_keys_to_statuses_to_tags[
                tag_display_type] = dupe_service_keys_to_statuses_to_tags
    ...
    def GetCurrentAndPending(self, service_key, tag_display_type):
        ...
        return statuses_to_tags[HC.CONTENT_STATUS_CURRENT].union(
            statuses_to_tags[HC.CONTENT_STATUS_PENDING])
    ...
    def GetNamespaceSlice(self, namespaces, tag_display_type):
        ...
        slice = {
            tag
            for tag in combined if True in (tag.startswith(namespace + ':')
                                            for namespace in namespaces)
        }
    ...
    def GetNumTags(self,
        ...
        if include_current_tags:
            num_tags += len(statuses_to_tags[HC.CONTENT_STATUS_CURRENT])
        if include_pending_tags:
            num_tags += len(statuses_to_tags[HC.CONTENT_STATUS_PENDING])
    ...
    def HasTag(self, tag, tag_display_type):
        ...
        return tag in combined_statuses_to_tags[
            HC.CONTENT_STATUS_CURRENT] or tag in combined_statuses_to_tags[
                HC.CONTENT_STATUS_PENDING]
