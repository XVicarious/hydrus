...
from . import (ClientData, ClientGUICommon, ClientGUIControls,
               ...
               ClientGUIScrolledPanels, ClientGUITopLevelWindows,
               ClientThreading)
...
class PopupMessage(PopupWindow):
    ...
    def __init__(self, parent, manager, job_key):
        ...
        self._pause_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.pause, self.PausePlay)
    ...
    def PausePlay(self):
        ...
        if self._job_key.IsPaused():
            ...
            ClientGUIFunctions.SetBitmapButtonBitmap(self._pause_button,
    ...
    def ShowFiles(self):
        ...
        if result is not None:
            ...
            HG.client_controller.pub('new_page_query',
                                     ...
                                     initial_hashes=popup_files,
                                     page_name=popup_files_name)
    ...
    def UpdateMessage(self):
        ...
        popup_yes_no_question = self._job_key.GetIfHasVariable(
        ...
        if self._job_key.HasVariable('popup_network_job'):
            ...
            popup_network_job = self._job_key.GetIfHasVariable(
                'popup_network_job')
        ...
        if result is not None:
            ...
            text = popup_files_name + ' - show ' + HydrusData.ToHumanInt(
                len(hashes)) + ' files'
...
class PopupMessageManager(QW.QWidget):
    def __init__(self, parent):
        ...
        self._message_panel.setSizePolicy(QW.QSizePolicy.MinimumExpanding,
                                          QW.QSizePolicy.Preferred)

        self._summary_bar = PopupMessageSummaryBar(self, self)
        self._summary_bar.setSizePolicy(QW.QSizePolicy.MinimumExpanding,
                                        QW.QSizePolicy.Preferred)
        ...
        job_key.SetVariable('popup_text_1',
                            'initialising popup message manager\u2026')

        self._update_job = HG.client_controller.CallRepeatingQtSafe(
            self, 0.25, 0.5, self.REPEATINGUpdate)

        HG.client_controller.CallLaterQtSafe(self, 0.5, self.AddMessage,
                                             job_key)
    ...
    def _CheckPending(self):
        ...
        self._pending_job_keys = [
            job_key for job_key in self._pending_job_keys
            if not job_key.IsDeleted()
        ]

        while len(self._pending_job_keys) > 0 and self._message_vbox.count(
        ) < self._max_messages_to_display:
        ...
        total_messages = len(
            self._pending_job_keys) + self._message_vbox.count()
    ...
    def _DoDebugHide(self):
        ...
        possibly_on_hidden_virtual_desktop = not ClientGUITopLevelWindows.MouseIsOnMyDisplay(
            parent)
    ...
    def _OKToAlterUI(self):
        ...
        not_on_hidden_or_virtual_display = ClientGUITopLevelWindows.MouseIsOnMyDisplay(
            main_gui) or ClientGUITopLevelWindows.MouseIsOnMyDisplay(self)
    ...
    def DismissAll(self):
        ...
        self._pending_job_keys = [
            job_key for job_key in self._pending_job_keys
            if job_key.IsPausable() or job_key.IsCancellable()
        ]
        ...
        try:
            ...
            if self._job_key.IsDone():
                ...
                if not self._yesno_open:  # don't close while a child dialog open m8
                    ...
                    if parent.isModal(
                    ):  # event sometimes fires after modal done
...
class PopupMessageSummaryBar(PopupWindow):
    ...
    def SetNumMessages(self, num_messages_pending):
        ...
        if num_messages_pending == 1:
            ...
            self._text.setText(
                HydrusData.ToHumanInt(num_messages_pending) + ' messages')
