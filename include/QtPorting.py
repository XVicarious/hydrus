...
def MonkeyPatchMissingMethods():
    ...
    if qtpy.PYQT5:
        ...
        QW.QFileDialog.getSaveFileName = MonkeyPatchGetSaveFileName(
        ...
        if self._save_mode:
            ...
            if self._wildcard:
                ...
                path = QW.QFileDialog.getSaveFileName(
                    ...
                    '',
                    existing_path,
                    filter=self._wildcard,
                    selectedFilter=self._wildcard)[0]

            else:

                path = QW.QFileDialog.getSaveFileName(self, '',
                                                      existing_path)[0]
            ...
            if self._wildcard:
                ...
                path = QW.QFileDialog.getOpenFileName(self, '',
                                                      existing_path)[0]
...
class TabWidgetWithDnD(QW.QTabWidget):
    ...
    def LayoutPages(self):
        ...
        highest_ancestor_of_same_type._LayoutPagesHelper(
        )  # This does the actual recursive descent and making pages visible
    ...
    def mouseMoveEvent(self, e):
        ...
        if self.currentWidget() and self.currentWidget().rect().contains(
                self.currentWidget().mapFromGlobal(self.mapToGlobal(e.pos()))):
        ...
        (clicked_tab_index,
         clicked_global_pos) = self._tab_bar.lastClickedTabInfo()
    ...
    def addTab(self, widget, *args, **kwargs):
        ...
        if isinstance(widget, TabWidgetWithDnD):
            ...
            widget.AddSupplementaryTabBarDropTarget(
                self._supplementary_drop_target)
    ...
    def dropEvent(self, e):
        ...
        if 'application/hydrus-tab' not in e.mimeData().formats(
        ):  #Page dnd has no associated mime data
        ...
        (source_page_index,
         source_page_click_global_pos) = source_tab_bar.lastClickedTabInfo()
        ...
        if dropped_on_tab_index != -1:
            ...
            right_edge_rect = QC.QRect(
                tab_rect.topRight() - QC.QPoint(EDGE_PADDING, 0), edge_size)
        ...
        if counter == 0:
            ...
            if follow_dropped_page:
                ...
                if source_page_index > 1:
                    ...
                    neighbour_page = source_notebook.widget(source_page_index -
                                                            1)
...
def AddToLayout(layout, item, flag=None, alignment=None, sizePolicy=None):
    ...
    if isinstance(layout, GridLayout):
        ...
        if isinstance(item, QW.QLayout):
            ...
            spacer = QW.QPushButton(
            )  #QW.QSpacerItem( 0, 0, QW.QSizePolicy.Expanding, QW.QSizePolicy.Fixed )
    ...
    expand_both_ways = flag in (CC.FLAGS_EXPAND_BOTH_WAYS,
                                CC.FLAGS_EXPAND_SIZER_BOTH_WAYS,
                                CC.FLAGS_EXPAND_BOTH_WAYS_POLITE,
                                CC.FLAGS_EXPAND_BOTH_WAYS_SHY)
    zero_border = False
    ...
    if flag is None or flag == CC.FLAGS_NONE:
        ...
        if isinstance(layout, QW.QVBoxLayout) or isinstance(
    ...
    if expand_both_ways:
        ...
        if isinstance(item, QW.QWidget):
            ...
            item.setSizePolicy(QW.QSizePolicy.Expanding,
                               QW.QSizePolicy.Expanding)

        if isinstance(layout, QW.QVBoxLayout) or isinstance(
                layout, QW.QHBoxLayout):

            if flag in (CC.FLAGS_EXPAND_BOTH_WAYS,
                        CC.FLAGS_EXPAND_SIZER_BOTH_WAYS):
...
def AdjustOpacity(image, opacity_factor):
    ...
    new_image = QG.QImage(image.width(), image.height(),
...
def DrawText(painter, x, y, text):
    ...
    painter.drawText(
        QC.QRectF(x, y, boundingRect.width(), boundingRect.height()), text)
...
def ToKeySequence(modifiers, key):
    ...
    if isinstance(modifiers, QC.Qt.KeyboardModifiers):
        ...
        for modifier in [
                QC.Qt.ShiftModifier, QC.Qt.ControlModifier, QC.Qt.AltModifier,
                QC.Qt.MetaModifier, QC.Qt.KeypadModifier,
                QC.Qt.GroupSwitchModifier
        ]:

            if modifiers & modifier:
                seq_str += QG.QKeySequence(modifier).toString()
...
def AdjustColour(colour, percent):
    ...
    return QG.QColor(colour.red() + colour.red() * percent,
                     colour.green() + colour.green() * percent,
                     colour.blue() + colour.blue() * percent, colour.alpha())
...
class CallAfterEventFilter(QC.QObject):
    ...
    def eventFilter(self, watched, event):
        ...
        if event.type() == CallAfterEventType and isinstance(
                event, CallAfterEvent):
...
def CallAfter(fn, *args, **kwargs):
    ...
    QW.QApplication.instance().postEvent(
        QW.QApplication.instance().call_after_catcher,
        CallAfterEvent(fn, *args, **kwargs))
...
def CenterOnWindow(parent, window):
    ...
    window.move(
        parent_window.mapToGlobal(parent_window.rect().center()) -
        window.rect().center())


def CenterOnScreen(window):

    window.move(QW.QApplication.desktop().availableGeometry().center() -
                window.rect().center())
...
def SetBackgroundColour(widget, colour):
    ...
    if isinstance(colour, QG.QColor):
        ...
        widget.setStyleSheet('#{} {{ background-color: {} }}'.format(
            object_name, colour.name()))

    elif isinstance(colour, tuple):

        widget.setStyleSheet('#{} {{ background-color: {} }}'.format(
            object_name,
            TupleToQColor(colour).name()))

    else:

        widget.setStyleSheet('#{} {{ background-color: {} }}'.format(
            object_name,
            QG.QColor(colour).name()))
...
def SetForegroundColour(widget, colour):
    ...
    if isinstance(colour, QG.QColor):
        ...
        widget.setStyleSheet('#{} {{ color: {} }}'.format(
            object_name, colour.name()))
...
class UIActionSimulator:
    ...
    def Char(self, key, text=None):
        ...
        ev1 = QG.QKeyEvent(QC.QEvent.KeyPress,
                           key,
                           QC.Qt.NoModifier,
                           text=text)
        ev2 = QG.QKeyEvent(QC.QEvent.KeyRelease,
                           key,
                           QC.Qt.NoModifier,
                           text=text)
...
class AboutBox(QW.QDialog):
    def __init__(self, parent, about_info):
        ...
        url_label = QW.QLabel(
            '<a href="{0}">{0}</a>'.format(about_info.website), self)
...
class CheckListBox(QW.QListWidget):
    ...
    def GetChecked(self):
        ...
        result = [
            GetClientData(self, index) for index in self.GetCheckedItems()
        ]
...
class EllipsizedLabel(QW.QLabel):
    ...
    def paintEvent(self, event):
        ...
        for text_line in text_lines:
            ...
            elided_line = fontMetrics.elidedText(text_line, QC.Qt.ElideRight,
                                                 my_width)
...
class FileDialog(QW.QFileDialog):
    def __init__(self,
                 ...
                 fileMode=QW.QFileDialog.ExistingFile,
                 defaultFile=None,
                 wildcard=None):
...
class ColourPickerCtrl(QW.QPushButton):
    ...
    def _UpdatePixmap(self):
        ...
        px = QG.QPixmap(self.contentsRect().height(),
...
def ListsToTuples(
    l
):  # Since lists are not hashable, we need to (recursively) convert lists to tuples in data that is to be added to BetterListCtrl
...
class WidgetEventFilter(QC.QObject):
    ...
    def eventFilter(self, watched, event):
        ...
        if not hasattr(self, '_parent_widget') or not isValid(
                self._parent_widget):
            return False
        ...
        if type == QC.QEvent.KeyPress:
            ...
            event_killed = event_killed or self._ExecuteCallbacks(
                'EVT_KEY_DOWN', event)
            ...
            if isValid(self._parent_widget):
                ...
                if self._parent_widget.isMinimized() or (
                        event.oldState() & QC.Qt.WindowMinimized):
                    event_killed = event_killed or self._ExecuteCallbacks(
                        'EVT_ICONIZE', event)

                if self._parent_widget.isMaximized() or (
                        event.oldState() & QC.Qt.WindowMaximized):
                    event_killed = event_killed or self._ExecuteCallbacks(
                        'EVT_MAXIMIZE', event)
            ...
            if event.button() == QC.Qt.LeftButton:
                ...
                event_killed = event_killed or self._ExecuteCallbacks(
                    'EVT_LEFT_DCLICK', event)

            elif event.button() == QC.Qt.RightButton:

                event_killed = event_killed or self._ExecuteCallbacks(
                    'EVT_RIGHT_DCLICK', event)
            ...
            if event.buttons() & QC.Qt.MiddleButton:
                event_killed = event_killed or self._ExecuteCallbacks(
                    'EVT_MIDDLE_DOWN', event)

            if event.buttons() & QC.Qt.RightButton:
                event_killed = event_killed or self._ExecuteCallbacks(
                    'EVT_RIGHT_DOWN', event)
            ...
            if isValid(
                    self._parent_widget) and self._parent_widget.isVisible():
...
class CollectComboCtrl(QW.QComboBox):
    ...
    def __init__(self, parent, media_collect):
        ...
        text_and_data_tuples = list([(namespace, ('namespace', namespace))
                                     for namespace in text_and_data_tuples])

        text_and_data_tuples.sort()

        ratings_services = HG.client_controller.services_manager.GetServices(
            (HC.LOCAL_RATING_LIKE, HC.LOCAL_RATING_NUMERICAL))

        for ratings_service in ratings_services:

            text_and_data_tuples.append(
                (ratings_service.GetName(), ('rating',
                                             ratings_service.GetServiceKey())))
