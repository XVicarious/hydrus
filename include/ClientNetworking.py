...
class NetworkEngine(object):
    def __init__(self, controller, bandwidth_manager, session_manager,
        ...
        self._pause_all_new_network_traffic = self.controller.new_options.GetBoolean(
    ...
    def AddJob(self, job):
        ...
        if HG.network_report_mode:
            ...
            HydrusData.ShowText('Network Job Added: ' + job._method + ' ' +
                                job._url)
    ...
    def GetJobsSnapshot(self):
        ...
        with self._lock:
            ...
            jobs.extend(((JOB_STATUS_AWAITING_VALIDITY, j)
                         for j in self._jobs_awaiting_validity))
            jobs.extend(((JOB_STATUS_AWAITING_BANDWIDTH, j)
                         for j in self._jobs_awaiting_bandwidth))
            jobs.extend(((JOB_STATUS_AWAITING_LOGIN, j)
                         for j in self._jobs_awaiting_login))
            jobs.extend(((JOB_STATUS_AWAITING_SLOT, j)
                         for j in self._jobs_awaiting_slot))
    ...
    def IsBusy(self):
        ...
        with self._lock:
            ...
            return len(self._jobs_awaiting_validity) + len(
                self._jobs_awaiting_bandwidth) + len(
                    self._jobs_awaiting_login) + len(
                        self._jobs_awaiting_slot) + len(
                            self._jobs_running) > 50
    ...
    def MainLoop(self):
        def ProcessValidationJob(job):
            ...
            if job.IsDone():
                ...
                if job.CanValidateInPopup():
                    ...
                    job.SetError(
                        HydrusExceptions.ValidationException(error_text),
                        error_text)
        ...
        def ProcessForceLogins():
            ...
            if len(self._domains_to_login
                   ) > 0 and self._current_login_process is None:

                try:

                    login_domain = self._domains_to_login.pop(0)

                    login_process = self.login_manager.GenerateLoginProcessForDomain(
                        login_domain)
                ...
                try:
                    ...
                    if job.WillingToWaitOnInvalidLogin():
                        ...
                        if job.IsHydrusJob():
                            ...
                            message = 'This hydrus service (' + job.GetLoginNetworkContext(
                            ).ToString(
                            ) + ') could not do work because: {}'.format(
                                str(e))

                        else:

                            message = 'This job\'s network context (' + job.GetLoginNetworkContext(
                            ).ToString(
                            ) + ') seems to have an invalid login. The error was: {}'.format(
                                str(e))
        ...
        def ProcessReadyJob(job):
            ...
            if job.IsDone():
                ...
                if self._pause_all_new_network_traffic:
                ...
                elif self._active_domains_counter[job.GetSecondLevelDomain(
                )] >= self.MAX_JOBS_PER_DOMAIN:
                    ...
                    if HG.network_report_mode:
                        ...
                        HydrusData.ShowText('Network Job Starting: ' +
                                            job._method + ' ' + job._url)

                    self._active_domains_counter[
                        job.GetSecondLevelDomain()] += 1
        ...
        def ProcessRunningJob(job):
            ...
            if job.IsDone():
                ...
                if HG.network_report_mode:
                    ...
                    HydrusData.ShowText('Network Job Done: ' + job._method +
                                        ' ' + job._url)
        ...
        while not (self._local_shutdown or HG.model_shutdown):
            ...
            with self._lock:
                ...
                self._jobs_awaiting_validity = list(
                    filter(ProcessValidationJob, self._jobs_awaiting_validity))

                ProcessCurrentValidationJob()

                self._jobs_awaiting_bandwidth = list(
                    filter(ProcessBandwidthJob, self._jobs_awaiting_bandwidth))

                ProcessForceLogins()

                self._jobs_awaiting_login = list(
                    filter(ProcessLoginJob, self._jobs_awaiting_login))

                ProcessCurrentLoginJob()

                self._jobs_awaiting_slot = list(
                    filter(ProcessReadyJob, self._jobs_awaiting_slot))

                self._jobs_running = list(
                    filter(ProcessRunningJob, self._jobs_running))
    ...
    def PausePlayNewJobs(self):
        ...
        self.controller.new_options.SetBoolean(
            'pause_all_new_network_traffic',
            self._pause_all_new_network_traffic)

    def RefreshOptions(self):

        with self._lock:

            self.MAX_JOBS = self.controller.new_options.GetInteger(
                'max_network_jobs')
            self.MAX_JOBS_PER_DOMAIN = self.controller.new_options.GetInteger(
