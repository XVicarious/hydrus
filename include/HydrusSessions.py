...
class HydrusSessionManagerServer(object):
    def __init__(self):
        ...
        HG.controller.sub(self, 'RefreshAllAccounts',
                          'update_all_session_accounts')
        ...
        if account_key not in account_keys_to_accounts:
            ...
            if HG.server_busy.locked():
                ...
                raise HydrusExceptions.ServerBusyException(
                    'Sorry, server is busy and cannot fetch account data right now!'
                )
    ...
    def _GetAccountKeyFromAccessKey(self, service_key, access_key):
        ...
        if hashed_access_key not in self._service_keys_to_hashed_access_keys_to_account_keys[
            ...
            if HG.server_busy.locked():
                ...
                raise HydrusExceptions.ServerBusyException(
                    'Sorry, server is busy and cannot fetch account key data right now!'
                )

            account_key = HG.controller.Read('account_key_from_access_key',
                                             service_key, access_key)

            self._service_keys_to_hashed_access_keys_to_account_keys[
                service_key][hashed_access_key] = account_key

        account_key = self._service_keys_to_hashed_access_keys_to_account_keys[
            service_key][hashed_access_key]
    ...
    def AddSession(self, service_key, access_key):
        ...
        with self._lock:
            ...
            account_key = self._GetAccountKeyFromAccessKey(
                service_key, access_key)
    ...
    def GetDirtyAccounts(self):
        ...
        with self._lock:
            ...
            for (service_key, account_keys_to_accounts
                 ) in self._service_keys_to_account_keys_to_accounts.items():

                dirty_accounts = [
                    account_key
                    for account_key in account_keys_to_accounts.values()
                    if account_key.IsDirty()
                ]

                if len(dirty_accounts) > 0:

                    service_keys_to_dirty_accounts[
                        service_key] = dirty_accounts
    ...
    def RefreshAllAccounts(self, service_key=None):
        ...
        with self._lock:
            ...
            if service_key is None:
                ...
                self._service_keys_to_session_keys_to_sessions = collections.defaultdict(
                    dict)

                self._service_keys_to_account_keys_to_accounts = collections.defaultdict(
                    dict)

                self._service_keys_to_hashed_access_keys_to_account_keys = collections.defaultdict(
                    dict)
                ...
                del self._service_keys_to_hashed_access_keys_to_account_keys[
                    service_key]

                existing_sessions = HG.controller.Read('sessions', service_key)

            for (session_key, service_key, account, hashed_access_key,
                 expires) in existing_sessions:
                ...
                if account_key not in self._service_keys_to_account_keys_to_accounts:
                    ...
                    self._service_keys_to_account_keys_to_accounts[
                        service_key][account_key] = account
