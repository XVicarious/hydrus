...
def GenerateHydrusBitmapFromPILImage(pil_image, compressed=True):
    ...
    return HydrusBitmap(pil_image.tobytes(),
                        pil_image.size,
                        depth,
                        compressed=compressed)
...
class ImageRenderer(object):
    ...
    def _GetNumPyImage(self, target_resolution=None):
        ...
        if target_resolution is None:
            ...
            numpy_image = ClientImageHandling.ResizeNumPyImageForMediaViewer(
                self._mime, self._numpy_image, target_resolution.toTuple())
...
class RasterContainerVideo(RasterContainer):
    def __init__(self, media, target_resolution=None, init_position=0):
        ...
        if duration is None or duration == 0:
            ...
            message = 'The file with hash ' + media.GetHash().hex(
            ) + ', had an invalid duration.'
        ...
        if num_frames_in_video is None or num_frames_in_video == 0:
            ...
            message = 'The file with hash ' + media.GetHash().hex(
            ) + ', had an invalid number of frames.'
            message += os.linesep * 2
            message += 'You may wish to try regenerating its metadata through the advanced mode right-click menu.'
        ...
        frame_buffer_length = (video_buffer_size_mb * 1024 * 1024) // (x * y *
                                                                       3)

        # if we can't buffer the whole vid, then don't have a clunky massive buffer

        max_streaming_buffer_size = max(
            48, int(num_frames_in_video / (duration / 3.0)))  # 48 or 3 seconds
    ...
    def _MaintainBuffer(self):
        ...
        deletees = [
            index
            for index in list(self._frames.keys()) if FrameIndexOutOfRange(
                index, self._buffer_start_index, self._buffer_end_index)
        ]
    ...
    def THREADRender(self):
        ...
        if self._media.GetMime() == HC.IMAGE_GIF:
            ...
            self._durations = HydrusImageHandling.GetGIFFrameDurations(
                self._path)

            self._renderer = ClientVideoHandling.GIFRenderer(
                self._path, num_frames_in_video, self._target_resolution)

        else:

            self._renderer = HydrusVideoHandling.VideoRendererFFMPEG(
                self._path, mime, duration, num_frames_in_video,
        ...
        while True:
            ...
            with self._lock:
                ...
                next_render_is_out_of_buffer = FrameIndexOutOfRange(
                    self._next_render_index, self._buffer_start_index,
                    self._buffer_end_index)
                ...
                will_render_ideal_frame_soon = self._IndexInRange(
                    self._next_render_index, self._buffer_start_index,
                    self._ideal_next_frame)

                need_ideal_next_frame = not self._HasFrame(
                    self._ideal_next_frame)
            ...
            if need_to_render:
                ...
                if should_save_frame:
                    ...
                    frame = GenerateHydrusBitmapFromNumPyImage(
                        numpy_image, compressed=False)
                ...
                if work_still_to_do:
                    ...
                    half_a_frame = (self._average_frame_duration /
                                    1000.0) * 0.5

                    sleep_duration = min(
                        0.1, half_a_frame)  # for 10s-long 3-frame gifs, wew
    ...
    def GetBufferIndices(self):
        ...
        if self._last_index_rendered == -1:
            ...
            return (self._buffer_start_index, self._last_index_rendered,
                    self._buffer_end_index)
    ...
    def GetReadyForFrame(self, next_index_to_expect):
        ...
        frame_request_is_impossible = FrameIndexOutOfRange(
            next_index_to_expect, 0, num_frames_in_video - 1)
        ...
        with self._lock:
            ...
            if video_is_bigger_than_buffer:
                ...
                current_ideal_is_out_of_buffer = self._buffer_start_index == -1 or FrameIndexOutOfRange(
                    self._ideal_next_frame, self._buffer_start_index,
                    self._buffer_end_index)

                ideal_buffer_start_index = max(
                    0, self._ideal_next_frame - self._num_frames_backwards)

                ideal_buffer_end_index = (
                    self._ideal_next_frame +
                    self._num_frames_forwards) % num_frames_in_video
                ...
                if current_ideal_is_out_of_buffer:
                    ...
                    next_ideal_start_would_shunt_right = self._IndexInRange(
                        ideal_buffer_start_index, self._buffer_start_index,
                        self._ideal_next_frame)
                    have_next_ideal_start = self._HasFrame(
                        ideal_buffer_start_index)

                    if next_ideal_start_would_shunt_right and have_next_ideal_start:

                        self._buffer_start_index = ideal_buffer_start_index

                    next_ideal_end_would_shunt_right = self._IndexInRange(
                        ideal_buffer_end_index, self._buffer_end_index,
...
class HydrusBitmap(object):
    ...
    def GetQtImage(self):
        ...
        return HG.client_controller.bitmap_manager.GetQtImageFromBuffer(
            width, height, self._depth * 8, self._GetData())
