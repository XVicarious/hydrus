...
def catch_all(url, request):
    ...
    raise Exception('An unexpected request for ' + url +
                    ' came through in testing.')
...
def catch_hydrus_error(url, request):
    ...
    return response(
        500, BAD_RESPONSE, {
            'Server':
            HC.service_string_lookup[HC.TAG_REPOSITORY] + '/' +
    ...
    def test_bandwidth_reported(self):
        ...
        with HTTMock(catch_all):
            ...
            with HTTMock(catch_wew_ok):
                ...
                tracker = bm.GetTracker(
                    ClientNetworkingContexts.GLOBAL_NETWORK_CONTEXT)

                self.assertTrue(
                    tracker.GetUsage(HC.BANDWIDTH_TYPE_REQUESTS, None), 1)
                self.assertTrue(tracker.GetUsage(HC.BANDWIDTH_TYPE_DATA, None),
                                256)
    ...
    def test_error(self):
        ...
        with HTTMock(catch_all):
            ...
            with HTTMock(catch_wew_error):
                ...
                self.assertEqual(type(job.GetErrorException()),
                                 HydrusExceptions.ServerException)

                self.assertTrue(job.GetErrorText(), BAD_RESPONSE)

                self.assertEqual(job.GetStatus(),
                                 ('500 - Internal Server Error', 18, 18, None))
