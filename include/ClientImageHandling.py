...
def GenerateNumPyImage(path, mime):
    ...
    force_pil = HG.client_controller.new_options.GetBoolean(
        'load_images_with_pil')

    return HydrusImageHandling.GenerateNumPyImage(path,
                                                  mime,
                                                  force_pil=force_pil)
...
def GenerateShapePerceptualHashes(path, mime):
    ...
    if HG.phash_generation_report_mode:
        ...
        HydrusData.ShowText('phash generation: image shape: {}'.format(
            numpy_image.shape))
    ...
    if depth == 4:
        ...
        numpy_image_gray_bare = cv2.cvtColor(numpy_image_bgr,
                                             cv2.COLOR_RGB2GRAY)
        ...
        numpy_image_gray = numpy.uint8(
            (numpy_image_gray_bare * numpy_alpha_float) +
            (white * (numpy.ones((y, x)) - numpy_alpha_float)))
    ...
    if HG.phash_generation_report_mode:
        ...
        HydrusData.ShowText('phash generation: grey image shape: {}'.format(
            numpy_image_gray.shape))

    numpy_image_tiny = cv2.resize(numpy_image_gray, (32, 32),
                                  interpolation=cv2.INTER_AREA)

    if HG.phash_generation_report_mode:

        HydrusData.ShowText('phash generation: tiny image shape: {}'.format(
            numpy_image_tiny.shape))
    ...
    if HG.phash_generation_report_mode:
        ...
        HydrusData.ShowText(
            'phash generation: tiny float image shape: {}'.format(
                numpy_image_tiny_float.shape))
    ...
    if HG.phash_generation_report_mode:
        ...
        HydrusData.ShowText('phash generation: final phashes: {}'.format(
            len(phashes)))
...
def ResizeNumPyImageForMediaViewer(mime, numpy_image, target_resolution):
    ...
    (scale_up_quality,
     scale_down_quality) = new_options.GetMediaZoomQuality(mime)
