...
class HydrusResourceBusyCheck(HydrusServerResources.Resource):
    def __init__(self):
        ...
        self._server_version_string = HC.service_string_lookup[
            HC.SERVER_ADMIN] + '/' + str(HC.NETWORK_VERSION)
...
class HydrusResourceHydrusNetwork(HydrusServerResources.HydrusResource):
    ...
    def _callbackParseGETArgs(self, request):
        ...
        parsed_request_args = HydrusNetwork.ParseHydrusNetworkGETArgs(
            request.args)
    ...
    def _callbackParsePOSTArgs(self, request):
        ...
        if not request.requestHeaders.hasHeader('Content-Type'):
            ...
            if mime == HC.APPLICATION_JSON:
                ...
                parsed_request_args = HydrusServerResources.ParseFileArguments(
...
class HydrusResourceAccessKey(HydrusResourceHydrusNetwork):
    def _threadDoGETJob(self, request):
        ...
        access_key = HG.server_controller.Read('access_key', self._service_key,
                                               registration_key)
...
class HydrusResourceAccessKeyVerification(HydrusResourceHydrusNetwork):
    def _threadDoGETJob(self, request):
        ...
        verified = HG.server_controller.Read('verify_access_key',
...
class HydrusResourceSessionKey(HydrusResourceHydrusNetwork):
    def _threadDoGETJob(self, request):
        ...
        (session_key,
         expires) = HG.server_controller.server_session_manager.AddSession(
             self._service_key, access_key)
...
class HydrusResourceRestricted(HydrusResourceHydrusNetwork):
    def _callbackCheckAccountRestrictions(self, request):
        ...
        HydrusResourceHydrusNetwork._callbackCheckAccountRestrictions(
            self, request)
    ...
    def _checkBandwidth(self, request):
        ...
        if not HG.server_controller.ServerBandwidthOK():
            ...
            raise HydrusExceptions.BandwidthException(
                'This server has run out of bandwidth. Please try again later.'
            )
...
class HydrusResourceRestrictedAccountModification(HydrusResourceRestricted):
    def _threadDoPOSTJob(self, request):
        ...
        with HG.dirty_object_lock:
            ...
            HG.server_controller.WriteSynchronous('account_modification',
                                                  self._service_key,
                                                  request.hydrus_account,
                                                  action, subject_accounts,
                                                  **kwargs)

            HG.server_controller.server_session_manager.UpdateAccounts(
                self._service_key, subject_accounts)
...
class HydrusResourceRestrictedAccountTypes(HydrusResourceRestricted):
    def _threadDoGETJob(self, request):
        ...
        account_types = HG.server_controller.Read('account_types',
                                                  self._service_key,
                                                  request.hydrus_account)
    ...
    def _threadDoPOSTJob(self, request):
        ...
        deletee_account_type_keys_to_new_account_type_keys = request.parsed_request_args[
            'deletee_account_type_keys_to_new_account_type_keys']

        HG.server_controller.WriteSynchronous(
            'account_types', self._service_key, request.hydrus_account,
            account_types, deletee_account_type_keys_to_new_account_type_keys)

        HG.server_controller.server_session_manager.RefreshAccounts(
            self._service_key)
...
class HydrusResourceRestrictedBackup(HydrusResourceRestricted):
    def _threadDoPOSTJob(self, request):
        ...
        request.hydrus_account.CheckPermission(HC.CONTENT_TYPE_SERVICES,
...
class HydrusResourceRestrictedIP(HydrusResourceRestricted):
    def _threadDoGETJob(self, request):
        ...
        (ip, timestamp) = HG.server_controller.Read('ip', self._service_key,
                                                    request.hydrus_account,
                                                    hash)
...
class HydrusResourceRestrictedNumPetitions(HydrusResourceRestricted):
    def _threadDoGETJob(self, request):
        ...
        petition_count_info = HG.server_controller.Read(
            'num_petitions', self._service_key, request.hydrus_account)

        body = HydrusNetwork.DumpHydrusArgsToNetworkBytes(
            {'num_petitions': petition_count_info})
...
class HydrusResourceRestrictedPetition(HydrusResourceRestricted):
    def _threadDoGETJob(self, request):
        ...
        petition = HG.server_controller.Read('petition', self._service_key,
                                             request.hydrus_account,
                                             content_type, status)
...
class HydrusResourceRestrictedRepositoryFile(HydrusResourceRestricted):
    def _DecompressionBombsOK(self, request):
        ...
        if request.hydrus_account is None:
            ...
            return request.hydrus_account.HasPermission(
    ...
    def _threadDoGETJob(self, request):
        ...
        if not valid:
            ...
            raise HydrusExceptions.NotFoundException(
                'File not found on this service!')
    ...
    def _threadDoPOSTJob(self, request):
        ...
        HG.server_controller.WriteSynchronous('file', self._service,
                                              request.hydrus_account,
                                              file_dict, timestamp)
...
class HydrusResourceRestrictedServices(HydrusResourceRestricted):
    def _threadDoGETJob(self, request):
        ...
        services = HG.server_controller.Read('services_from_account',
                                             request.hydrus_account)
...
class HydrusResourceRestrictedUpdate(HydrusResourceRestricted):
    def _threadDoGETJob(self, request):
        ...
        if not self._service.HasUpdateHash(update_hash):
            ...
            raise HydrusExceptions.NotFoundException(
                'This update hash does not exist on this service!')
    ...
    def _threadDoPOSTJob(self, request):
        ...
        client_to_server_update = request.parsed_request_args[
            'client_to_server_update']

        timestamp = self._service.GetMetadata().GetNextUpdateBegin() + 1

        HG.server_controller.WriteSynchronous('update', self._service_key,
                                              request.hydrus_account,
                                              client_to_server_update,
                                              timestamp)
