...
from . import (ClientImporting, ClientNetworkingDomain, ClientParsing,
               ClientTags)
...
class GallerySeed(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self.url, self._can_generate_more_pages,
                serialisable_fixed_service_keys_to_tags, self.created,
                self.modified, self.status, self.note, self._referral_url)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self.url, self._can_generate_more_pages,
         serialisable_fixed_service_keys_to_tags, self.created, self.modified,
         self.status, self.note, self._referral_url) = serialisable_info
    ...
    def GenerateRestartedDuplicate(self, can_generate_more_pages):
        ...
        gallery_seed = GallerySeed(
            url=self.url, can_generate_more_pages=can_generate_more_pages)
        ...
        try:
            ...
            if url_type not in (HC.URL_TYPE_GALLERY, HC.URL_TYPE_WATCHABLE):
                ...
                raise HydrusExceptions.VetoException(
                    'Did not recognise this as a gallery or watchable URL!')
            ...
            title = ClientParsing.GetTitleFromAllParseResults(
                all_parse_results)
            ...
            file_seeds = ClientImporting.ConvertAllParseResultsToFileSeeds(
                all_parse_results, self.url, file_import_options)
            ...
            (num_urls_added, num_urls_already_in_file_seed_cache,
             ...
             stop_reason) = file_seeds_callable(file_seeds)
            ...
            if parser.CanOnlyGenerateGalleryURLs(
            ) or self._force_next_page_url_generation:
            ...
            if self._can_generate_more_pages and can_add_more_gallery_urls:
                ...
                flattened_results = list(
                    itertools.chain.from_iterable(all_parse_results))

                next_page_urls = ClientParsing.GetURLsFromParseResults(
                    flattened_results, (HC.URL_TYPE_NEXT, ),
                ...
                if len(next_page_urls) > 0:
                    ...
                    if url_class is not None and url_class.CanGenerateNextGalleryPage(
                        ...
                        try:
                            ...
                            next_page_url = url_class.GetNextGalleryPage(
                                url_to_check)
                ...
                if len(next_page_urls) > 0:
                    ...
                    new_next_page_urls = [
                        next_page_url for next_page_url in next_page_urls
                        if next_page_url not in gallery_urls_seen_before
                    ]

                    duplicate_next_page_urls = gallery_urls_seen_before.intersection(
                        new_next_page_urls)
                    ...
                    if num_new_next_page_urls > 0:
                        ...
                        next_gallery_seeds = [
                            GallerySeed(next_page_url)
                            for next_page_url in new_next_page_urls
                        ]

                        for next_gallery_seed in next_gallery_seeds:

                            next_gallery_seed.SetFixedServiceKeysToTags(
                                self._fixed_service_keys_to_tags)
                        ...
                        if num_dupe_next_page_urls == 0:
                            ...
                            note += ' - ' + HydrusData.ToHumanInt(
                                num_new_next_page_urls
                            ) + next_page_generation_phrase

                        else:

                            note += ' - ' + HydrusData.ToHumanInt(
                                num_new_next_page_urls
                            ) + next_page_generation_phrase + ', but ' + HydrusData.ToHumanInt(
                                num_dupe_next_page_urls
                            ) + ' had already been visited this run and were not added'

                    else:

                        note += ' - ' + HydrusData.ToHumanInt(
                            num_dupe_next_page_urls
                        ) + next_page_generation_phrase + ', but they had already been visited this run and were not added'
            ...
            if isinstance(
                    e, HydrusExceptions.NetworkException
            ):  # so the larger queue can set a delaywork or whatever
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_GALLERY_SEED] = GallerySeed
...
class GallerySeedLog(HydrusSerialisable.SerialisableBase):
    ...
    def _GetGallerySeeds(self, status=None):
        ...
        if status is None:
            ...
            return [
                gallery_seed for gallery_seed in self._gallery_seeds
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        with self._lock:
            ...
            self._gallery_seeds = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_info)

            self._gallery_seeds_to_indices = {
                gallery_seed: index
                for (index, gallery_seed) in enumerate(self._gallery_seeds)
            }
    ...
    def AddGallerySeeds(self, gallery_seeds):
        ...
        with self._lock:
            ...
            for gallery_seed in gallery_seeds:
                ...
                self._gallery_seeds_to_indices[gallery_seed] = len(
                    self._gallery_seeds) - 1
    ...
    def Compact(self, compact_before_this_source_time):
        ...
        with self._lock:
            ...
            new_gallery_seeds.extend(
                self._gallery_seeds[-self.COMPACT_NUMBER:])
    ...
    def GetAPIInfoDict(self, simple):
        ...
        with self._lock:
            ...
            if not simple:
                ...
                d['log_items'] = [
                    gallery_seed.GetAPIInfoDict(simple)
                    for gallery_seed in self._gallery_seeds
                ]
    ...
    def HasGalleryURL(self, url):
        ...
        return search_url in (gallery_seed.url
                              for gallery_seed in self._gallery_seeds)
    ...
    def NotifyGallerySeedsUpdated(self, gallery_seeds):
        ...
        HG.client_controller.pub('gallery_seed_log_gallery_seeds_updated',
                                 self._gallery_seed_log_key, gallery_seeds)
    ...
    def RemoveGallerySeeds(self, gallery_seeds):
        ...
        with self._lock:
            ...
            self._gallery_seeds = HydrusSerialisable.SerialisableList([
                gallery_seed for gallery_seed in self._gallery_seeds
                if gallery_seed not in gallery_seeds_to_delete
            ])
    ...
    def RemoveGallerySeedsByStatus(self, statuses_to_remove):
        ...
        with self._lock:
            ...
            gallery_seeds_to_delete = [
                gallery_seed for gallery_seed in self._gallery_seeds
                if gallery_seed.status in statuses_to_remove
            ]
    ...
    def RestartFailedSearch(self):
        ...
        with self._lock:
            ...
            new_gallery_seeds = (last_gallery_seed.GenerateRestartedDuplicate(
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_GALLERY_SEED_LOG] = GallerySeedLog
