...
def CreateManagementController(page_name,
                               management_type,
    ...
    management_controller.SetVariable('media_sort',
                                      new_options.GetDefaultSort())
    management_controller.SetVariable('media_collect',
                                      new_options.GetDefaultCollect())
...
def CreateManagementControllerDuplicateFilter():
    ...
    management_controller = CreateManagementController(
        'duplicates', MANAGEMENT_TYPE_DUPLICATE_FILTER)

    file_search_context = ClientSearch.FileSearchContext(
        file_service_key=CC.LOCAL_FILE_SERVICE_KEY,
        predicates=[
            ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_EVERYTHING)
        ])

    management_controller.SetVariable('file_search_context',
                                      file_search_context)
...
def CreateManagementControllerImportGallery():
    ...
    management_controller = CreateManagementController(
        page_name, MANAGEMENT_TYPE_IMPORT_MULTIPLE_GALLERY)
    ...
    multiple_gallery_import = ClientImportGallery.MultipleGalleryImport(
        gug_key_and_name=gug_key_and_name)

    management_controller.SetVariable('multiple_gallery_import',
...
def CreateManagementControllerImportSimpleDownloader():
    ...
    management_controller = CreateManagementController(
        'simple downloader', MANAGEMENT_TYPE_IMPORT_SIMPLE_DOWNLOADER)

    simple_downloader_import = ClientImportSimpleURLs.SimpleDownloaderImport()

    formula_name = HG.client_controller.new_options.GetString(
        'favourite_simple_downloader_formula')

    simple_downloader_import.SetFormulaName(formula_name)

    management_controller.SetVariable('simple_downloader_import',
                                      simple_downloader_import)

    return management_controller


def CreateManagementControllerImportHDD(paths, file_import_options,
                                        paths_to_service_keys_to_tags,
                                        delete_after_success):

    management_controller = CreateManagementController(
        'import', MANAGEMENT_TYPE_IMPORT_HDD)

    hdd_import = ClientImportLocal.HDDImport(
        paths=paths,
        file_import_options=file_import_options,
        paths_to_service_keys_to_tags=paths_to_service_keys_to_tags,
...
def CreateManagementControllerImportMultipleWatcher(page_name=None, url=None):
    ...
    management_controller = CreateManagementController(
        page_name, MANAGEMENT_TYPE_IMPORT_MULTIPLE_WATCHER)

    multiple_watcher_import = ClientImportWatchers.MultipleWatcherImport(
        url=url)

    management_controller.SetVariable('multiple_watcher_import',
...
def CreateManagementControllerImportURLs(page_name=None):
    ...
    management_controller = CreateManagementController(
        page_name, MANAGEMENT_TYPE_IMPORT_URLS)
...
def CreateManagementControllerPetitions(petition_service_key):
    ...
    petition_service = HG.client_controller.services_manager.GetService(
        petition_service_key)
...
def CreateManagementControllerQuery(page_name, file_service_key,
                                    file_search_context, search_enabled):

    management_controller = CreateManagementController(
        page_name, MANAGEMENT_TYPE_QUERY, file_service_key=file_service_key)
...
def CreateManagementPanel(parent, page, controller, management_controller):
    ...
    management_panel = management_class(parent, page, controller,
...
class ManagementController(HydrusSerialisable.SerialisableBase):
    ...
    def __repr__(self):
        ...
        return 'Management Controller: {} - {}'.format(self._management_type,
                                                       self._page_name)
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_keys = {
            name: value.hex()
            for (name, value) in list(self._keys.items())
        }

        serialisable_simples = dict(self._simples)

        serialisable_serialisables = {
            name: value.GetSerialisableTuple()
            for (name, value) in list(self._serialisables.items())
        }

        return (self._page_name, self._management_type, serialisable_keys,
                serialisable_simples, serialisable_serialisables)

    def _InitialiseDefaults(self):

        self._serialisables['media_sort'] = ClientMedia.MediaSort(
            ('system', CC.SORT_FILES_BY_FILESIZE), CC.SORT_ASC)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._page_name, self._management_type, serialisable_keys,
         serialisable_simples, serialisable_serialisables) = serialisable_info

        self._InitialiseDefaults()

        self._keys.update({
            name: bytes.fromhex(key)
            for (name, key) in list(serialisable_keys.items())
        })

        if 'file_service' in self._keys:

            if not HG.client_controller.services_manager.ServiceExists(
                    self._keys['file_service']):

                self._keys['file_service'] = CC.COMBINED_LOCAL_FILE_SERVICE_KEY

        self._simples.update(dict(serialisable_simples))

        self._serialisables.update({
            name: HydrusSerialisable.CreateFromSerialisableTuple(value)
            for (name, value) in list(serialisable_serialisables.items())
        })
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (management_type, serialisable_keys, serialisable_simples,
             serialisable_serialisables) = old_serialisable_info

            if management_type == MANAGEMENT_TYPE_IMPORT_HDD:

                advanced_import_options = serialisable_simples[
                    'advanced_import_options']
                paths_info = serialisable_simples['paths_info']
                paths_to_tags = serialisable_simples['paths_to_tags']
                delete_after_success = serialisable_simples[
                    'delete_after_success']

                paths = [
                    path_info for (path_type, path_info) in paths_info
                    if path_type != 'zip'
                ]
                ...
                automatic_archive = advanced_import_options[
                    'automatic_archive']
                ...
                file_import_options.SetPreImportOptions(
                    ...
                    allow_decompression_bombs, min_size, max_size,
                    max_gif_size, min_resolution, max_resolution)
                file_import_options.SetPostImportOptions(
                    automatic_archive, associate_source_urls)

                paths_to_tags = {
                    path: {
                        bytes.fromhex(service_key): tags
                        for (service_key, tags) in service_keys_to_tags
                    }
                    for (path,
                         service_keys_to_tags) in list(paths_to_tags.items())
                }

                hdd_import = ClientImportLocal.HDDImport(
                    paths=paths,
                    file_import_options=file_import_options,
                    paths_to_service_keys_to_tags=paths_to_tags,
                    delete_after_success=delete_after_success)

                serialisable_serialisables[
                    'hdd_import'] = hdd_import.GetSerialisableTuple()
            ...
            new_serialisable_info = (management_type, serialisable_keys,
                                     serialisable_simples,
                                     serialisable_serialisables)
        ...
        if version == 2:
            ...
            new_serialisable_info = (page_name, management_type,
                                     serialisable_keys, serialisable_simples,
                                     serialisable_serialisables)
        ...
        if version == 3:
            ...
            (page_name, management_type, serialisable_keys,
             serialisable_simples,
             serialisable_serialisables) = old_serialisable_info
            ...
            if 'page_of_images_import' in serialisable_serialisables:
                ...
                serialisable_serialisables[
                    'simple_downloader_import'] = serialisable_serialisables[
        ...
        if version == 4:
            ...
            if 'thread_watcher_import' in serialisable_serialisables:
                ...
                serialisable_serialisables[
                    'watcher_import'] = serialisable_serialisables[
                        'thread_watcher_import']
        ...
        if version == 5:
            ...
            if 'gallery_import' in serialisable_serialisables:
                ...
                serialisable_serialisables[
                    'multiple_gallery_import'] = serialisable_serialisables[
                        'gallery_import']
        ...
        if version == 6:
            ...
            if 'watcher_import' in serialisable_serialisables:
                ...
                watcher = HydrusSerialisable.CreateFromSerialisableTuple(
                    serialisable_serialisables['watcher_import'])
                ...
                serialisable_serialisables[
                    'multiple_watcher_import'] = serialisable_multiple_watcher_import
        ...
        if version == 8:
            ...
            if management_type == MANAGEMENT_TYPE_DUPLICATE_FILTER:
                ...
                serialisable_serialisables[
                    'file_search_context'] = file_search_context.GetSerialisableTuple(
                    )
        ...
        if version == 9:
            ...
            if 'media_collect' in serialisable_simples:
                ...
                try:
                    ...
                    namespaces = [
                        n for (t, n) in old_collect if t == 'namespace'
                    ]
                    rating_service_keys = [
                        bytes.fromhex(r) for (t, r) in old_collect
                        if t == 'rating'
                    ]
                ...
                serialisable_serialisables[
                    'media_collect'] = media_collect.GetSerialisableTuple()
    ...
    def GetAPIInfoDict(self, simple):
        ...
        if self._management_type == MANAGEMENT_TYPE_IMPORT_HDD:
            ...
            simple_downloader_import = self._serialisables[
                'simple_downloader_import']

            d['simple_downloader_import'] = simple_downloader_import.GetAPIInfoDict(
                simple)

        elif self._management_type == MANAGEMENT_TYPE_IMPORT_MULTIPLE_GALLERY:

            multiple_gallery_import = self._serialisables[
                'multiple_gallery_import']

            d['multiple_gallery_import'] = multiple_gallery_import.GetAPIInfoDict(
                simple)

        elif self._management_type == MANAGEMENT_TYPE_IMPORT_MULTIPLE_WATCHER:

            multiple_watcher_import = self._serialisables[
                'multiple_watcher_import']

            d['multiple_watcher_import'] = multiple_watcher_import.GetAPIInfoDict(
                simple)
    ...
    def IsImporter(self):
        ...
        return self._management_type in (
            MANAGEMENT_TYPE_IMPORT_HDD,
            MANAGEMENT_TYPE_IMPORT_SIMPLE_DOWNLOADER,
            MANAGEMENT_TYPE_IMPORT_MULTIPLE_GALLERY,
            MANAGEMENT_TYPE_IMPORT_MULTIPLE_WATCHER,
            MANAGEMENT_TYPE_IMPORT_URLS)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_MANAGEMENT_CONTROLLER] = ManagementController
...
class ManagementPanel(QW.QScrollArea):
    ...
    def __init__(self, parent, page, controller, management_controller):
        ...
        self.verticalScrollBar().valueChanged.connect(
            managementScrollbarValueChanged)
        ...
        self._media_sort = ClientGUICommon.ChoiceSort(
            self, management_controller=self._management_controller)

        silent_collect = not self.SHOW_COLLECT

        self._media_collect = ClientGUICommon.CheckboxCollect(
            self,
            management_controller=self._management_controller,
            silent=silent_collect)
    ...
    def _MakeCurrentSelectionTagsBox(self, sizer):
        ...
        tags_box = ClientGUICommon.StaticBoxSorterForListBoxTags(
            self, 'selection tags')

        t = ClientGUIListBoxes.ListBoxTagsSelectionManagementPanel(
            tags_box, self._page_key, self.TAG_DISPLAY_TYPE)
...
class ManagementPanelDuplicateFilter(ManagementPanel):
    ...
    def __init__(self, parent, page, controller, management_controller):
        ...
        ManagementPanel.__init__(self, parent, page, controller,
                                 management_controller)
        ...
        self._refresh_maintenance_status = ClientGUICommon.BetterStaticText(
            self._main_left_panel)
        self._refresh_maintenance_button = ClientGUICommon.BetterBitmapButton(
            self._main_left_panel, CC.GlobalPixmaps.refresh,
            self.RefreshMaintenanceNumbers)

        menu_items = []

        menu_items.append((
            'normal', 'reset potential duplicates',
            'This will delete all the potential duplicate pairs found so far and reset their files\' search status.',
            self._ResetUnknown))
        ...
        menu_items.append((
            'check',
            'search for duplicate pairs at the current distance during normal db maintenance',
            'Tell the client to find duplicate pairs in its normal db maintenance cycles, whether you have that set to idle or shutdown time.',
            check_manager))

        self._cog_button = ClientGUICommon.MenuBitmapButton(
            self._main_left_panel, CC.GlobalPixmaps.cog, menu_items)

        menu_items = []

        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR, 'duplicates.html'))

        menu_items.append((
            'normal', 'open the html duplicates help',
            'Open the help page for duplicates processing in your web browser.',
            page_func))

        self._help_button = ClientGUICommon.MenuBitmapButton(
            self._main_left_panel, CC.GlobalPixmaps.help, menu_items)

        #

        self._searching_panel = ClientGUICommon.StaticBox(
            self._main_left_panel, 'finding potential duplicates')

        self._eligible_files = ClientGUICommon.BetterStaticText(
            self._searching_panel)

        menu_items = []

        menu_items.append(
            ('normal', 'exact match', 'Search for exact matches.',
             HydrusData.Call(self._SetSearchDistance, HC.HAMMING_EXACT_MATCH)))
        menu_items.append(
            ('normal', 'very similar', 'Search for very similar files.',
             HydrusData.Call(self._SetSearchDistance,
                             HC.HAMMING_VERY_SIMILAR)))
        menu_items.append(('normal', 'similar', 'Search for similar files.',
                           HydrusData.Call(self._SetSearchDistance,
                                           HC.HAMMING_SIMILAR)))
        menu_items.append(('normal', 'speculative',
                           'Search for files that are probably similar.',
                           HydrusData.Call(self._SetSearchDistance,
                                           HC.HAMMING_SPECULATIVE)))

        self._search_distance_button = ClientGUICommon.MenuButton(
            self._searching_panel, 'similarity', menu_items)

        self._search_distance_spinctrl = QP.MakeQSpinBox(self._searching_panel,
                                                         min=0,
                                                         max=64,
                                                         width=50)
        self._search_distance_spinctrl.valueChanged.connect(
            self.EventSearchDistanceChanged)

        self._num_searched = ClientGUICommon.TextAndGauge(
            self._searching_panel)

        self._search_button = ClientGUICommon.BetterBitmapButton(
            self._searching_panel, CC.GlobalPixmaps.play,
        ...
        menu_items.append(
            ('normal',
             'edit duplicate metadata merge options for \'this is better\'',
             'edit what content is merged when you filter files',
             HydrusData.Call(self._EditMergeOptions, HC.DUPLICATE_BETTER)))
        menu_items.append(
            ('normal',
             'edit duplicate metadata merge options for \'same quality\'',
             'edit what content is merged when you filter files',
        ...
        if new_options.GetBoolean('advanced_mode'):
            ...
            menu_items.append((
                'normal',
                'edit duplicate metadata merge options for \'alternates\' (advanced!)',
                'edit what content is merged when you filter files',
                HydrusData.Call(self._EditMergeOptions,
                                HC.DUPLICATE_ALTERNATE)))

        self._edit_merge_options = ClientGUICommon.MenuButton(
            self._main_right_panel,
            'edit default duplicate metadata merge options', menu_items)

        #

        self._filtering_panel = ClientGUICommon.StaticBox(
            self._main_right_panel, 'duplicate filter')

        file_search_context = management_controller.GetVariable(
            'file_search_context')

        predicates = file_search_context.GetPredicates()

        self._active_predicates_box = ClientGUIListBoxes.ListBoxTagsActiveSearchPredicates(
            self._filtering_panel, self._page_key, predicates)

        self._ac_read = ClientGUIACDropdown.AutoCompleteDropdownTagsRead(
            self._filtering_panel,
            self._page_key,
            file_search_context,
        ...
        self._num_potential_duplicates = ClientGUICommon.BetterStaticText(
            self._filtering_panel)
        self._refresh_dupe_counts_button = ClientGUICommon.BetterBitmapButton(
            self._filtering_panel, CC.GlobalPixmaps.refresh,
            self.RefreshDuplicateNumbers)

        self._launch_filter = ClientGUICommon.BetterButton(
            self._filtering_panel, 'launch the filter', self._LaunchFilter)

        #

        random_filtering_panel = ClientGUICommon.StaticBox(
            self._main_right_panel, 'quick and dirty processing')

        self._show_some_dupes = ClientGUICommon.BetterButton(
            random_filtering_panel, 'show some random potential pairs',
            self._ShowRandomPotentialDupes)

        self._set_random_as_same_quality_button = ClientGUICommon.BetterButton(
            random_filtering_panel,
            'set current media as duplicates of the same quality',
            self._SetCurrentMediaAs, HC.DUPLICATE_SAME_QUALITY)
        self._set_random_as_alternates_button = ClientGUICommon.BetterButton(
            random_filtering_panel,
            'set current media as all related alternates',
            self._SetCurrentMediaAs, HC.DUPLICATE_ALTERNATE)
        self._set_random_as_false_positives_button = ClientGUICommon.BetterButton(
            random_filtering_panel,
            'set current media as not related/false positive',
            self._SetCurrentMediaAs, HC.DUPLICATE_FALSE_POSITIVE)
        ...
        self._search_distance_spinctrl.setValue(
            new_options.GetInteger(
                'similar_files_duplicate_pairs_search_distance'))

        self._both_files_match.setChecked(
            management_controller.GetVariable('both_files_match'))
        ...
        QP.AddToLayout(
            distance_hbox,
            ClientGUICommon.BetterStaticText(self._searching_panel,
                                             label='search distance: '),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(distance_hbox, self._search_distance_button,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(distance_hbox, self._search_distance_spinctrl,
                       CC.FLAGS_VCENTER)
        ...
        QP.AddToLayout(gridbox_2, self._num_searched,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(gridbox_2, self._search_button, CC.FLAGS_VCENTER)

        self._searching_panel.Add(self._eligible_files,
                                  CC.FLAGS_EXPAND_PERPENDICULAR)
        self._searching_panel.Add(distance_hbox,
                                  CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        self._searching_panel.Add(gridbox_2,
                                  CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        ...
        QP.AddToLayout(hbox, self._refresh_maintenance_status,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        QP.AddToLayout(hbox, self._refresh_maintenance_button,
                       CC.FLAGS_VCENTER)
        ...
        QP.AddToLayout(vbox, self._searching_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, QW.QWidget(self._main_left_panel),
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(text_and_button_hbox, self._num_potential_duplicates,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        QP.AddToLayout(text_and_button_hbox, self._refresh_dupe_counts_button,
                       CC.FLAGS_VCENTER)

        rows = []

        rows.append(
            ('both files of pair match in search: ', self._both_files_match))

        gridbox = ClientGUICommon.WrapInGrid(self._filtering_panel, rows)

        self._filtering_panel.Add(self._active_predicates_box,
                                  CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        self._filtering_panel.Add(text_and_button_hbox,
                                  CC.FLAGS_EXPAND_PERPENDICULAR)
        self._filtering_panel.Add(self._launch_filter,
                                  CC.FLAGS_EXPAND_PERPENDICULAR)

        random_filtering_panel.Add(self._show_some_dupes,
                                   CC.FLAGS_EXPAND_PERPENDICULAR)
        random_filtering_panel.Add(self._set_random_as_same_quality_button,
                                   CC.FLAGS_EXPAND_PERPENDICULAR)
        random_filtering_panel.Add(self._set_random_as_alternates_button,
                                   CC.FLAGS_EXPAND_PERPENDICULAR)
        random_filtering_panel.Add(self._set_random_as_false_positives_button,
                                   CC.FLAGS_EXPAND_PERPENDICULAR)

        vbox = QP.VBoxLayout()

        QP.AddToLayout(vbox, self._edit_merge_options,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._filtering_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, random_filtering_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, QW.QWidget(self._main_right_panel),
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        self._controller.sub(self, 'RefreshAllNumbers',
                             'refresh_dupe_page_numbers')
        self._controller.sub(self, 'RefreshQuery', 'refresh_query')
        self._controller.sub(self, 'SearchImmediately',
                             'notify_search_immediately')
    ...
    def _EditMergeOptions(self, duplicate_type):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
            ...
            panel = ClientGUIScrolledPanelsEdit.EditDuplicateActionOptionsPanel(
                dlg, duplicate_type, duplicate_action_options)
    ...
    def _LaunchFilter(self):
        ...
        (file_search_context,
         both_files_match) = self._GetFileSearchContextAndBothFilesMatch()

        canvas_frame = ClientGUICanvas.CanvasFrame(self.window())

        canvas_window = ClientGUICanvas.CanvasFilterDuplicates(
            canvas_frame, file_search_context, both_files_match)
    ...
    def _RefreshDuplicateCounts(self):
        ...
        def thread_do_it(file_search_context, both_files_match):
            ...
            potential_duplicates_count = HG.client_controller.Read(
                'potential_duplicates_count', file_search_context,
                both_files_match)
    ...
    def _SearchDomainUpdated(self):
        ...
        self._management_controller.SetVariable('file_search_context',
                                                file_search_context)
        self._management_controller.SetVariable('both_files_match',
                                                both_files_match)
    ...
    def _ShowRandomPotentialDupes(self):
        ...
        hashes = self._controller.Read('random_potential_duplicate_hashes',
                                       file_search_context, both_files_match)

        if len(hashes) == 0:

            QW.QMessageBox.critical(
                self, 'Error',
                'No files were found. Try refreshing the count, and if this keeps happening, please let hydrus_dev know.'
            )

            return

        media_results = self._controller.Read('media_results',
                                              hashes,
                                              sorted=True)
        ...
        if len(media_results) == 0:
            ...
            QW.QMessageBox.information(
                self, 'Information',
                'Files were found, but no media results could be loaded for them. Try refreshing the count, and if this keeps happening, please let hydrus_dev know.'
            )

            return

        panel = ClientGUIResults.MediaPanelThumbnails(self._page,
                                                      self._page_key,
                                                      file_service_key,
                                                      media_results)
    ...
    def _UpdateMaintenanceStatus(self):
        ...
        self._eligible_files.setText('{} eligible files in the system.'.format(
            HydrusData.ToHumanInt(total_num_files)))
        ...
        new_options.SetInteger('similar_files_duplicate_pairs_search_distance',
                               search_distance)
        ...
        num_searched = sum(
            (count for (value, count) in searched_distances_to_count.items()
             if value is not None and value >= search_distance))

        if num_searched == total_num_files:

            self._num_searched.SetValue(
            ...
            if num_searched == 0:
                ...
                self._num_searched.SetValue(
                    'Have not yet searched at this distance.', 0,
                    total_num_files)

            else:

                self._num_searched.SetValue(
                    'Searched ' + HydrusData.ConvertValueRangeToPrettyString(
                        num_searched, total_num_files) +
                    ' files at this distance.', num_searched, total_num_files)
    ...
    def _UpdateBothFilesMatchButton(self):
        ...
        if file_search_context.IsJustSystemEverything(
        ) or file_search_context.HasNoPredicates():
    ...
    def _UpdatePotentialDuplicatesCount(self, potential_duplicates_count):
        ...
        self._num_potential_duplicates.setText(
            HydrusData.ToHumanInt(potential_duplicates_count) +
            ' potential pairs.')
...
management_panel_types_to_classes[
    MANAGEMENT_TYPE_DUPLICATE_FILTER] = ManagementPanelDuplicateFilter
...
class ManagementPanelImporterHDD(ManagementPanelImporter):
    def __init__(self, parent, page, controller, management_controller):
        ...
        ManagementPanelImporter.__init__(self, parent, page, controller,
                                         management_controller)

        self._import_queue_panel = ClientGUICommon.StaticBox(
            self, 'import summary')

        self._current_action = ClientGUICommon.BetterStaticText(
            self._import_queue_panel)
        self._file_seed_cache_control = ClientGUIFileSeedCache.FileSeedCacheStatusControl(
            self._import_queue_panel, self._controller, self._page_key)

        self._pause_button = ClientGUICommon.BetterBitmapButton(
            self._import_queue_panel, CC.GlobalPixmaps.file_pause, self.Pause)
        self._pause_button.setToolTip('pause/play imports')

        self._hdd_import = self._management_controller.GetVariable(
            'hdd_import')
        ...
        self._file_import_options = ClientGUIImport.FileImportOptionsButton(
            self._import_queue_panel, file_import_options,
            show_downloader_options, self._hdd_import.SetFileImportOptions)
        ...
        self._import_queue_panel.Add(self._current_action,
                                     CC.FLAGS_EXPAND_PERPENDICULAR)
        self._import_queue_panel.Add(self._file_seed_cache_control,
                                     CC.FLAGS_EXPAND_PERPENDICULAR)
        self._import_queue_panel.Add(self._pause_button, CC.FLAGS_LONE_BUTTON)
        self._import_queue_panel.Add(self._file_import_options,
                                     CC.FLAGS_EXPAND_PERPENDICULAR)

        QP.AddToLayout(vbox, self._import_queue_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _UpdateImportStatus(self):
        ...
        if paused:
            ...
            ClientGUIFunctions.SetBitmapButtonBitmap(
                self._pause_button, CC.GlobalPixmaps.file_play)

        else:

            ClientGUIFunctions.SetBitmapButtonBitmap(
                self._pause_button, CC.GlobalPixmaps.file_pause)
...
management_panel_types_to_classes[
    MANAGEMENT_TYPE_IMPORT_HDD] = ManagementPanelImporterHDD
...
class ManagementPanelImporterMultipleGallery(ManagementPanelImporter):
    def __init__(self, parent, page, controller, management_controller):
        ...
        self._multiple_gallery_import = self._management_controller.GetVariable(
            'multiple_gallery_import')
        ...
        self._gallery_downloader_panel = ClientGUICommon.StaticBox(
            self, 'gallery downloader')

        #

        self._gallery_importers_status_st_top = ClientGUICommon.BetterStaticText(
            self._gallery_downloader_panel, ellipsize_end=True)
        self._gallery_importers_status_st_bottom = ClientGUICommon.BetterStaticText(
            self._gallery_downloader_panel, ellipsize_end=True)

        self._gallery_importers_listctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self._gallery_downloader_panel)

        columns = [('query', -1), ('source', 11), ('f', 3), ('s', 3),
                   ('status', 8), ('items', 9), ('added', 8)]

        self._gallery_importers_listctrl = ClientGUIListCtrl.BetterListCtrl(
            self._gallery_importers_listctrl_panel,
            'gallery_importers',
            4,
            8,
            columns,
            self._ConvertDataToListCtrlTuples,
            delete_key_callback=self._RemoveGalleryImports,
            activation_callback=self._HighlightSelectedGalleryImport)

        self._gallery_importers_listctrl_panel.SetListCtrl(
            self._gallery_importers_listctrl)

        self._gallery_importers_listctrl_panel.AddBitmapButton(
            CC.GlobalPixmaps.highlight,
            self._HighlightSelectedGalleryImport,
            tooltip='highlight',
            enabled_check_func=self._CanHighlight)
        self._gallery_importers_listctrl_panel.AddBitmapButton(
            CC.GlobalPixmaps.clear_highlight,
            self._ClearExistingHighlightAndPanel,
            tooltip='clear highlight',
            enabled_check_func=self._CanClearHighlight)
        self._gallery_importers_listctrl_panel.AddBitmapButton(
            CC.GlobalPixmaps.file_pause,
            self._PausePlayFiles,
            tooltip='pause/play files',
            enabled_only_on_selection=True)
        self._gallery_importers_listctrl_panel.AddBitmapButton(
            CC.GlobalPixmaps.gallery_pause,
            self._PausePlayGallery,
            tooltip='pause/play search',
            enabled_only_on_selection=True)
        self._gallery_importers_listctrl_panel.AddBitmapButton(
            CC.GlobalPixmaps.trash,
            self._RemoveGalleryImports,
            tooltip='remove selected',
            enabled_only_on_selection=True)

        self._gallery_importers_listctrl_panel.NewButtonRow()

        self._gallery_importers_listctrl_panel.AddButton(
            'retry failed',
            self._RetryFailed,
            enabled_check_func=self._CanRetryFailed)
        self._gallery_importers_listctrl_panel.AddButton(
            'retry ignored',
            self._RetryIgnored,
        ...
        self._gallery_importers_listctrl_panel.AddButton(
            'set options to queries',
            self._SetOptionsToGalleryImports,
            enabled_only_on_selection=True)
        ...
        self._query_input = ClientGUIControls.TextAndPasteCtrl(
            self._gallery_downloader_panel, self._PendQueries)

        self._cog_button = ClientGUICommon.BetterBitmapButton(
            self._gallery_downloader_panel, CC.GlobalPixmaps.cog,
            self._ShowCogMenu)

        self._gug_key_and_name = ClientGUIImport.GUGKeyAndNameSelector(
            self._gallery_downloader_panel,
            self._multiple_gallery_import.GetGUGKeyAndName(),
            update_callable=self._SetGUGKeyAndName)

        self._file_limit = ClientGUICommon.NoneableSpinCtrl(
            self._gallery_downloader_panel,
            'stop after this many files',
            min=1,
            none_phrase='no limit')
        self._file_limit.valueChanged.connect(self.EventFileLimit)
        self._file_limit.setToolTip(
            'per query, stop searching the gallery once this many files has been reached'
        )
        ...
        self._file_import_options = ClientGUIImport.FileImportOptionsButton(
            self._gallery_downloader_panel, file_import_options,
            show_downloader_options,
            self._multiple_gallery_import.SetFileImportOptions)
        self._tag_import_options = ClientGUIImport.TagImportOptionsButton(
            self._gallery_downloader_panel,
            tag_import_options,
            show_downloader_options,
            update_callable=self._multiple_gallery_import.SetTagImportOptions,
        ...
        QP.AddToLayout(input_hbox, self._query_input,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(input_hbox, self._cog_button, CC.FLAGS_VCENTER)

        self._gallery_downloader_panel.Add(
            self._gallery_importers_status_st_top,
            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._gallery_downloader_panel.Add(
            self._gallery_importers_status_st_bottom,
            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._gallery_downloader_panel.Add(
            self._gallery_importers_listctrl_panel, CC.FLAGS_EXPAND_BOTH_WAYS)
        self._gallery_downloader_panel.Add(input_hbox,
                                           CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        self._gallery_downloader_panel.Add(self._gug_key_and_name,
                                           CC.FLAGS_EXPAND_PERPENDICULAR)
        self._gallery_downloader_panel.Add(self._file_limit,
                                           CC.FLAGS_EXPAND_PERPENDICULAR)
        self._gallery_downloader_panel.Add(self._file_import_options,
                                           CC.FLAGS_EXPAND_PERPENDICULAR)
        self._gallery_downloader_panel.Add(self._tag_import_options,
                                           CC.FLAGS_EXPAND_PERPENDICULAR)

        #

        self._highlighted_gallery_import_panel = ClientGUIImport.GalleryImportPanel(
            self, self._page_key, name='highlighted query')

        self._highlighted_gallery_import_panel.SetGalleryImport(
            self._highlighted_gallery_import)

        #

        vbox = QP.VBoxLayout()

        QP.AddToLayout(vbox, self._media_sort,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)

        QP.AddToLayout(vbox, self._gallery_downloader_panel,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(vbox, self._highlighted_gallery_import_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _CanRetryFailed(self):
        ...
        for gallery_import in self._gallery_importers_listctrl.GetData(
                only_selected=True):
    ...
    def _ClearExistingHighlight(self):
        ...
        if self._highlighted_gallery_import is not None:
            ...
            self._multiple_gallery_import.SetHighlightedGalleryImport(
                self._highlighted_gallery_import)
    ...
    def _ClearExistingHighlightAndPanel(self):
        ...
        panel = ClientGUIResults.MediaPanelThumbnails(
            self._page, self._page_key, CC.LOCAL_FILE_SERVICE_KEY,
            media_results)
    ...
    def _ConvertDataToListCtrlTuples(self, gallery_import):
        ...
        if files_paused:
            ...
            pretty_files_paused = HG.client_controller.new_options.GetString(
                'pause_character')
        ...
        if gallery_finished:
            ...
            pretty_gallery_paused = HG.client_controller.new_options.GetString(
                'stop_character')
        ...
        (file_seed_cache_status, file_seed_cache_simple_status,
         (num_done,
          num_total)) = gallery_import.GetFileSeedCache().GetStatus()
        ...
        pretty_added = HydrusData.TimestampToPrettyTimeDelta(
            added, show_seconds=False)

        display_tuple = (pretty_query_text, pretty_source, pretty_files_paused,
                         pretty_gallery_paused, pretty_status, pretty_progress,
                         pretty_added)
        sort_tuple = (query_text, pretty_source, files_paused, gallery_paused,
    ...
    def _CopySelectedQueries(self):
        ...
        gallery_importers = self._gallery_importers_listctrl.GetData(
            only_selected=True)

        if len(gallery_importers) > 0:

            text = os.linesep.join((gallery_importer.GetQueryText()
                                    for gallery_importer in gallery_importers))

            HG.client_controller.pub('clipboard', 'text', text)

    def _GetListCtrlMenu(self):

        selected_watchers = self._gallery_importers_listctrl.GetData(
            only_selected=True)
        ...
        ClientGUIMenus.AppendMenuItem(
            menu, 'copy queries',
            'Copy all the selected downloaders\' queries to clipboard.',
            self._CopySelectedQueries)

        ClientGUIMenus.AppendSeparator(menu)

        ClientGUIMenus.AppendMenuItem(
            menu,
            'show all importers\' presented files',
            'Gather the presented files for the selected importers and show them in a new page.',
            self._ShowSelectedImportersFiles,
            show='presented')
        ClientGUIMenus.AppendMenuItem(
            menu,
            'show all importers\' new files',
            'Gather the presented files for the selected importers and show them in a new page.',
            self._ShowSelectedImportersFiles,
            show='new')
        ClientGUIMenus.AppendMenuItem(
            menu,
            'show all importers\' files',
            'Gather the presented files for the selected importers and show them in a new page.',
            self._ShowSelectedImportersFiles,
            show='all')
        ClientGUIMenus.AppendMenuItem(
            menu,
            'show all importers\' files (including trash)',
            'Gather the presented files (including trash) for the selected importers and show them in a new page.',
            self._ShowSelectedImportersFiles,
            show='all_and_trash')
        ...
        if self._CanRetryFailed() or self._CanRetryIgnored():
            ...
            if self._CanRetryFailed():
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'retry failed', 'Retry all the failed downloads.',
                    self._RetryFailed)

            if self._CanRetryIgnored():

                ClientGUIMenus.AppendMenuItem(
                    menu, 'retry ignored', 'Retry all the failed downloads.',
                    self._RetryIgnored)

        ClientGUIMenus.AppendSeparator(menu)

        ClientGUIMenus.AppendMenuItem(menu, 'remove',
                                      'Remove the selected queries.',
                                      self._RemoveGalleryImports)

        ClientGUIMenus.AppendSeparator(menu)

        ClientGUIMenus.AppendMenuItem(
            menu, 'pause/play files',
            'Pause/play all the selected downloaders\' file queues.',
            self._PausePlayFiles)
        ClientGUIMenus.AppendMenuItem(
            menu, 'pause/play search',
            'Pause/play all the selected downloaders\' gallery searches.',
            self._PausePlayGallery)
    ...
    def _HighlightGalleryImport(self, new_highlight):
        ...
        if new_highlight == self._highlighted_gallery_import:
            ...
            hashes = HG.client_controller.Read('filter_hashes', hashes,
            ...
            sorted_media_results = [
                hashes_to_media_results[hash] for hash in hashes
            ]
    ...
    def _PendQueries(self, queries):
        ...
        if len(
                results
        ) > 0 and self._highlighted_gallery_import is None and HG.client_controller.new_options.GetBoolean(
                'highlight_new_query'):
    ...
    def _RemoveGalleryImports(self):
        ...
        removees = list(
            self._gallery_importers_listctrl.GetData(only_selected=True))
        ...
        message = 'Remove the ' + HydrusData.ToHumanInt(
            len(removees)) + ' selected queries?'

        if num_working > 0:

            message += os.linesep * 2
            message += HydrusData.ToHumanInt(
                num_working) + ' are still working.'
        ...
        if result == QW.QDialog.Accepted:
            ...
            for gallery_import in removees:
                ...
                self._multiple_gallery_import.RemoveGalleryImport(
                    gallery_import.GetGalleryImportKey())
    ...
    def _SetGUGKeyAndName(self, gug_key_and_name):
        ...
        should_initialise_new_text = current_input_value in (
            current_initial_search_text, '')
    ...
    def _SetOptionsToGalleryImports(self):
        ...
        gallery_imports = self._gallery_importers_listctrl.GetData(
            only_selected=True)
    ...
    def _ShowCogMenu(self):
        ...
        (start_file_queues_paused, start_gallery_queues_paused,
         merge_simultaneous_pends_to_one_importer
         ) = self._multiple_gallery_import.GetQueueStartSettings()

        ClientGUIMenus.AppendMenuCheckItem(
            menu, 'start new importers\' files paused',
            'Start any new importers in a file import-paused state.',
            start_file_queues_paused,
            self._multiple_gallery_import.SetQueueStartSettings,
            not start_file_queues_paused, start_gallery_queues_paused,
            merge_simultaneous_pends_to_one_importer)
        ClientGUIMenus.AppendMenuCheckItem(
            menu, 'start new importers\' search paused',
            'Start any new importers in a gallery search-paused state.',
            start_gallery_queues_paused,
            self._multiple_gallery_import.SetQueueStartSettings,
            start_file_queues_paused, not start_gallery_queues_paused,
            merge_simultaneous_pends_to_one_importer)
        ClientGUIMenus.AppendSeparator(menu)
        ClientGUIMenus.AppendMenuCheckItem(
            menu,
            'bundle multiple pasted queries into one importer (advanced)',
            'If you are pasting many small queries at once (such as md5 lookups), check this to smooth out the workflow.',
            merge_simultaneous_pends_to_one_importer,
            self._multiple_gallery_import.SetQueueStartSettings,
            start_file_queues_paused, start_gallery_queues_paused,
            not merge_simultaneous_pends_to_one_importer)
    ...
    def _ShowSelectedImportersFiles(self, show='presented'):
        ...
        for gallery_import in gallery_imports:
            ...
            new_hashes = [
                hash for hash in gallery_hashes if hash not in seen_hashes
            ]
        ...
        if len(hashes) > 0:
            ...
            QW.QMessageBox.critical(self, 'Error',
                                    'No presented hashes for that selection!')
        ...
        if HydrusData.TimeHasPassed(self._next_update_time):
            ...
            if num_gallery_imports == 0:
                ...
                (status, simple_status,
                 (value,
                  range)) = self._multiple_gallery_import.GetTotalStatus()

                text_top = HydrusData.ToHumanInt(
                    num_gallery_imports
                ) + ' queries - ' + HydrusData.ConvertValueRangeToPrettyString(
                    value, range)
                text_bottom = status
            ...
            if self._last_time_imports_changed != last_time_imports_changed:
                ...
                self._gallery_importers_listctrl.GrowShrinkColumnsHeight(
                    ideal_rows)
    ...
    def CheckAbleToClose(self):
        ...
        if num_working > 0:
            ...
            raise HydrusExceptions.VetoException(
                HydrusData.ToHumanInt(num_working) +
...
management_panel_types_to_classes[
    MANAGEMENT_TYPE_IMPORT_MULTIPLE_GALLERY] = ManagementPanelImporterMultipleGallery
...
class ManagementPanelImporterMultipleWatcher(ManagementPanelImporter):
    def __init__(self, parent, page, controller, management_controller):
        ...
        self._multiple_watcher_import = self._management_controller.GetVariable(
            'multiple_watcher_import')

        self._highlighted_watcher = self._multiple_watcher_import.GetHighlightedWatcher(
        )

        (checker_options, file_import_options,
         tag_import_options) = self._multiple_watcher_import.GetOptions()

        #

        self._watchers_panel = ClientGUICommon.StaticBox(self, 'watchers')

        self._watchers_status_st_top = ClientGUICommon.BetterStaticText(
            self._watchers_panel, ellipsize_end=True)
        self._watchers_status_st_bottom = ClientGUICommon.BetterStaticText(
            self._watchers_panel, ellipsize_end=True)

        self._watchers_listctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self._watchers_panel)

        columns = [('subject', -1), ('f', 3), ('c', 3), ('status', 8),
                   ('items', 9), ('added', 8)]

        self._watchers_listctrl = ClientGUIListCtrl.BetterListCtrl(
            self._watchers_listctrl_panel,
            'watchers',
            4,
            8,
            columns,
            self._ConvertDataToListCtrlTuples,
            delete_key_callback=self._RemoveWatchers,
            activation_callback=self._HighlightSelectedWatcher)

        self._watchers_listctrl_panel.SetListCtrl(self._watchers_listctrl)

        self._watchers_listctrl_panel.AddBitmapButton(
            CC.GlobalPixmaps.highlight,
            self._HighlightSelectedWatcher,
            tooltip='highlight',
            enabled_check_func=self._CanHighlight)
        self._watchers_listctrl_panel.AddBitmapButton(
            CC.GlobalPixmaps.clear_highlight,
        ...
        self._watchers_listctrl_panel.AddBitmapButton(
            CC.GlobalPixmaps.gallery_pause,
            self._PausePlayChecking,
            tooltip='pause/play checking',
            enabled_only_on_selection=True)
        self._watchers_listctrl_panel.AddBitmapButton(
            CC.GlobalPixmaps.trash,
            ...
            tooltip='remove selected',
            enabled_only_on_selection=True)
        self._watchers_listctrl_panel.AddButton('check now',
                                                self._CheckNow,
                                                enabled_only_on_selection=True)

        self._watchers_listctrl_panel.NewButtonRow()

        self._watchers_listctrl_panel.AddButton(
            'retry failed',
            self._RetryFailed,
            enabled_check_func=self._CanRetryFailed)
        self._watchers_listctrl_panel.AddButton(
        ...
        self._watchers_listctrl_panel.AddButton('set options to watchers',
                                                self._SetOptionsToWatchers,
                                                enabled_only_on_selection=True)

        self._watchers_listctrl.Sort(3)

        self._watcher_url_input = ClientGUIControls.TextAndPasteCtrl(
            self._watchers_panel, self._AddURLs)
        ...
        self._checker_options = ClientGUIImport.CheckerOptionsButton(
            self._watchers_panel, checker_options, self._OptionsUpdated)
        self._file_import_options = ClientGUIImport.FileImportOptionsButton(
            self._watchers_panel, file_import_options, show_downloader_options,
            self._OptionsUpdated)
        self._tag_import_options = ClientGUIImport.TagImportOptionsButton(
            self._watchers_panel,
            tag_import_options,
            show_downloader_options,
            update_callable=self._OptionsUpdated,
            allow_default_selection=True)
        ...
        self._highlighted_watcher_panel = ClientGUIImport.WatcherReviewPanel(
            self, self._page_key, name='highlighted watcher')

        self._highlighted_watcher_panel.SetWatcher(self._highlighted_watcher)

        #

        self._watchers_panel.Add(self._watchers_status_st_top,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        self._watchers_panel.Add(self._watchers_status_st_bottom,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        self._watchers_panel.Add(self._watchers_listctrl_panel,
                                 CC.FLAGS_EXPAND_BOTH_WAYS)
        self._watchers_panel.Add(self._watcher_url_input,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        self._watchers_panel.Add(self._checker_options,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        self._watchers_panel.Add(self._file_import_options,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        self._watchers_panel.Add(self._tag_import_options,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        QP.AddToLayout(vbox, self._highlighted_watcher_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        HG.client_controller.sub(self, '_ClearExistingHighlightAndPanel',
    ...
    def _AddURLs(self, urls, service_keys_to_tags=None):
        ...
        for url in urls:
            ...
            result = self._multiple_watcher_import.AddURL(
                url, service_keys_to_tags)

            if result is not None and first_result is None:

                first_result = result

        if first_result is not None and self._highlighted_watcher is None and HG.client_controller.new_options.GetBoolean(
    ...
    def _ClearExistingHighlight(self):
        ...
        if self._highlighted_watcher is not None:
            ...
            self._multiple_watcher_import.SetHighlightedWatcher(
                self._highlighted_watcher)
    ...
    def _ConvertDataToListCtrlTuples(self, watcher):
        ...
        if checking_dead:
            ...
            pretty_checking_paused = HG.client_controller.new_options.GetString(
                'stop_character')
        ...
        (status, simple_status,
         (num_done, num_total)) = watcher.GetFileSeedCache().GetStatus()
        ...
        watcher_status = self._multiple_watcher_import.GetWatcherSimpleStatus(
            watcher)
        ...
        display_tuple = (pretty_subject, pretty_files_paused,
                         pretty_checking_paused, pretty_watcher_status,
                         pretty_progress, pretty_added)
        sort_tuple = (subject, files_paused, checking_paused, watcher_status,
                      progress, added)
    ...
    def _GetListCtrlMenu(self):
        ...
        ClientGUIMenus.AppendMenuItem(
            menu, 'copy urls',
            'Copy all the selected watchers\' urls to clipboard.',
            self._CopySelectedURLs)
        ClientGUIMenus.AppendMenuItem(
            menu, 'open urls',
            'Open all the selected watchers\' urls in your browser.',
            self._OpenSelectedURLs)

        ClientGUIMenus.AppendSeparator(menu)

        ClientGUIMenus.AppendMenuItem(
            menu,
            'show all watchers\' presented files',
            'Gather the presented files for the selected watchers and show them in a new page.',
            self._ShowSelectedImportersFiles,
            show='presented')
        ClientGUIMenus.AppendMenuItem(
            menu,
            'show all watchers\' new files',
            'Gather the presented files for the selected watchers and show them in a new page.',
            self._ShowSelectedImportersFiles,
            show='new')
        ClientGUIMenus.AppendMenuItem(
            menu,
            'show all watchers\' files',
            'Gather the presented files for the selected watchers and show them in a new page.',
            self._ShowSelectedImportersFiles,
            show='all')
        ClientGUIMenus.AppendMenuItem(
            menu,
            'show all watchers\' files (including trash)',
            'Gather the presented files (including trash) for the selected watchers and show them in a new page.',
            self._ShowSelectedImportersFiles,
            show='all_and_trash')
        ...
        if self._CanRetryFailed() or self._CanRetryIgnored():
            ...
            if self._CanRetryIgnored():
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'retry ignored', 'Retry all the ignored downloads.',
                    self._RetryIgnored)

        ClientGUIMenus.AppendSeparator(menu)

        ClientGUIMenus.AppendMenuItem(menu, 'remove selected',
                                      'Remove the selected watchers.',
                                      self._RemoveWatchers)
        ...
        ClientGUIMenus.AppendMenuItem(
            menu, 'pause/play files',
            'Pause/play all the selected watchers\' file queues.',
            self._PausePlayFiles)
        ClientGUIMenus.AppendMenuItem(
            menu, 'pause/play checking',
            'Pause/play all the selected watchers\' checking routines.',
            self._PausePlayChecking)
    ...
    def _HighlightWatcher(self, new_highlight):
        ...
        if new_highlight == self._highlighted_watcher:
            ...
            self._highlighted_watcher_panel.SetWatcher(
                self._highlighted_watcher)
    ...
    def _OptionsUpdated(self, *args, **kwargs):
        ...
        self._multiple_watcher_import.SetOptions(
            self._checker_options.GetValue(),
            self._file_import_options.GetValue(),
            self._tag_import_options.GetValue())
    ...
    def _RemoveWatchers(self):
        ...
        message = 'Remove the ' + HydrusData.ToHumanInt(
            len(removees)) + ' selected watchers?'
        ...
        if result == QW.QDialog.Accepted:
            ...
            for watcher in removees:
                ...
                self._multiple_watcher_import.RemoveWatcher(
                    watcher.GetWatcherKey())
    ...
    def _ShowSelectedImportersFiles(self, show='presented'):
        ...
        for watcher in watchers:
            ...
            new_hashes = [
                hash for hash in watcher_hashes if hash not in seen_hashes
            ]
        ...
        if HydrusData.TimeHasPassed(self._next_update_time):
            ...
            if num_watchers == 0:
                ...
                if num_dead == 0:
                    ...
                    num_dead_text = HydrusData.ToHumanInt(
                        num_dead) + ' DEAD - '

                (status, simple_status,
                 (value,
                  range)) = self._multiple_watcher_import.GetTotalStatus()

                text_top = HydrusData.ToHumanInt(
                    num_watchers
                ) + ' watchers - ' + num_dead_text + HydrusData.ConvertValueRangeToPrettyString(
                    value, range)
                text_bottom = status
...
management_panel_types_to_classes[
    MANAGEMENT_TYPE_IMPORT_MULTIPLE_WATCHER] = ManagementPanelImporterMultipleWatcher
...
class ManagementPanelImporterSimpleDownloader(ManagementPanelImporter):
    def __init__(self, parent, page, controller, management_controller):
        ...
        self._simple_downloader_import = self._management_controller.GetVariable(
            'simple_downloader_import')

        #

        self._simple_downloader_panel = ClientGUICommon.StaticBox(
            self, 'simple downloader')

        #

        self._import_queue_panel = ClientGUICommon.StaticBox(
            self._simple_downloader_panel, 'imports')

        self._pause_files_button = ClientGUICommon.BetterBitmapButton(
            self._import_queue_panel, CC.GlobalPixmaps.file_pause,
            self.PauseFiles)
        self._pause_files_button.setToolTip('pause/play files')

        self._current_action = ClientGUICommon.BetterStaticText(
            self._import_queue_panel, ellipsize_end=True)
        self._file_seed_cache_control = ClientGUIFileSeedCache.FileSeedCacheStatusControl(
            self._import_queue_panel, self._controller, self._page_key)
        self._file_download_control = ClientGUIControls.NetworkJobControl(
            self._import_queue_panel)
        ...
        self._simple_parsing_jobs_panel = ClientGUICommon.StaticBox(
            self._simple_downloader_panel, 'simple parsing urls')

        self._pause_queue_button = ClientGUICommon.BetterBitmapButton(
            self._simple_parsing_jobs_panel, CC.GlobalPixmaps.gallery_pause,
            self.PauseQueue)
        self._pause_queue_button.setToolTip('pause/play queue')

        self._parser_status = ClientGUICommon.BetterStaticText(
            self._simple_parsing_jobs_panel, ellipsize_end=True)

        self._gallery_seed_log_control = ClientGUIGallerySeedLog.GallerySeedLogStatusControl(
            self._simple_parsing_jobs_panel, self._controller, True, False,
            self._page_key)

        self._page_download_control = ClientGUIControls.NetworkJobControl(
            self._simple_parsing_jobs_panel)

        self._pending_jobs_listbox = QW.QListWidget(
            self._simple_parsing_jobs_panel)

        self._advance_button = QW.QPushButton('\u2191',
                                              self._simple_parsing_jobs_panel)
        self._advance_button.clicked.connect(self.EventAdvance)

        self._delete_button = QW.QPushButton('X',
                                             self._simple_parsing_jobs_panel)
        self._delete_button.clicked.connect(self.EventDelete)

        self._delay_button = QW.QPushButton('\u2193',
                                            self._simple_parsing_jobs_panel)
        self._delay_button.clicked.connect(self.EventDelay)

        self._page_url_input = ClientGUIControls.TextAndPasteCtrl(
            self._simple_parsing_jobs_panel, self._PendPageURLs)

        self._page_url_input.setPlaceholderText(
            'url to be parsed by the selected formula')

        self._formulae = ClientGUICommon.BetterChoice(
            self._simple_parsing_jobs_panel)

        menu_items = []

        menu_items.append(('normal', 'edit formulae',
                           'Edit these parsing formulae.', self._EditFormulae))

        self._formula_cog = ClientGUICommon.MenuBitmapButton(
            self._simple_parsing_jobs_panel, CC.GlobalPixmaps.cog, menu_items)
        ...
        self._file_import_options = ClientGUIImport.FileImportOptionsButton(
            self._simple_downloader_panel, file_import_options,
            show_downloader_options,
            self._simple_downloader_import.SetFileImportOptions)

        #

        hbox = QP.HBoxLayout()

        QP.AddToLayout(hbox, self._current_action,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        ...
        self._import_queue_panel.Add(self._file_seed_cache_control,
                                     CC.FLAGS_EXPAND_PERPENDICULAR)
        self._import_queue_panel.Add(self._file_download_control,
                                     CC.FLAGS_EXPAND_PERPENDICULAR)

        queue_buttons_vbox = QP.VBoxLayout()

        QP.AddToLayout(queue_buttons_vbox, self._advance_button,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(queue_buttons_vbox, self._delete_button,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(queue_buttons_vbox, self._delay_button,
                       CC.FLAGS_VCENTER)

        queue_hbox = QP.HBoxLayout()

        QP.AddToLayout(queue_hbox, self._pending_jobs_listbox,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(queue_hbox, queue_buttons_vbox, CC.FLAGS_VCENTER)

        formulae_hbox = QP.HBoxLayout()

        QP.AddToLayout(formulae_hbox, self._formulae,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(formulae_hbox, self._formula_cog, CC.FLAGS_VCENTER)

        hbox = QP.HBoxLayout()

        QP.AddToLayout(hbox, self._parser_status,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        QP.AddToLayout(hbox, self._pause_queue_button, CC.FLAGS_VCENTER)

        self._simple_parsing_jobs_panel.Add(hbox,
                                            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._simple_parsing_jobs_panel.Add(self._gallery_seed_log_control,
                                            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._simple_parsing_jobs_panel.Add(self._page_download_control,
                                            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._simple_parsing_jobs_panel.Add(queue_hbox,
                                            CC.FLAGS_EXPAND_SIZER_BOTH_WAYS)
        self._simple_parsing_jobs_panel.Add(self._page_url_input,
                                            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._simple_parsing_jobs_panel.Add(formulae_hbox,
                                            CC.FLAGS_EXPAND_PERPENDICULAR)

        #

        self._simple_downloader_panel.Add(self._import_queue_panel,
                                          CC.FLAGS_EXPAND_PERPENDICULAR)
        self._simple_downloader_panel.Add(self._simple_parsing_jobs_panel,
                                          CC.FLAGS_EXPAND_PERPENDICULAR)
        self._simple_downloader_panel.Add(self._file_import_options,
                                          CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        QP.AddToLayout(vbox, self._simple_downloader_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _EditFormulae(self):
        ...
        def edit_callable(data):
            ...
            with ClientGUIDialogs.DialogTextEntry(dlg,
                                                  'edit name',
                                                  default=name) as dlg_2:
            ...
            with ClientGUITopLevelWindows.DialogEdit(dlg,
                                                     'edit formula') as dlg_3:

                panel = ClientGUIScrolledPanels.EditSingleCtrlPanel(dlg_3)

                formula = simple_downloader_formula.GetFormula()

                control = ClientGUIParsing.EditFormulaPanel(
                    panel, formula, lambda: ({}, ''))
                ...
                if dlg_3.exec() == QW.QDialog.Accepted:
                    ...
                    simple_downloader_formula = ClientParsing.SimpleDownloaderParsingFormula(
        ...
        formulae = list(
            self._controller.new_options.GetSimpleDownloaderFormulae())

        formulae.sort(key=lambda o: o.GetName())

        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit simple downloader formulae') as dlg:

            panel = ClientGUIScrolledPanels.EditSingleCtrlPanel(dlg)

            height_num_chars = 20

            control = ClientGUIListBoxes.AddEditDeleteListBoxUniqueNamedObjects(
                panel, height_num_chars, data_to_pretty_callable, add_callable,
                edit_callable)

            control.AddSeparator()
            control.AddImportExportButtons(
                (ClientParsing.SimpleDownloaderParsingFormula, ))
            control.AddSeparator()
            control.AddDefaultsButton(
                ClientDefaults.GetDefaultSimpleDownloaderFormulae)
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                self._controller.new_options.SetSimpleDownloaderFormulae(
                    formulae)
    ...
    def _RefreshFormulae(self):
        ...
        simple_downloader_formulae = list(
            self._controller.new_options.GetSimpleDownloaderFormulae())

        simple_downloader_formulae.sort(key=lambda o: o.GetName())

        for (i, simple_downloader_formula
             ) in enumerate(simple_downloader_formulae):
    ...
    def _UpdateImportStatus(self):
        ...
        (pending_jobs, parser_status, current_action, queue_paused,
         files_paused) = self._simple_downloader_import.GetStatus()

        current_pending_jobs = [
            QP.GetClientData(self._pending_jobs_listbox, i)
            for i in range(self._pending_jobs_listbox.count())
        ]

        if current_pending_jobs != pending_jobs:

            selected_string = QP.ListWidgetGetStringSelection(
            ...
            selection_index = QP.ListWidgetIndexForString(
                self._pending_jobs_listbox, selected_string)

            if selection_index != -1:

                QP.ListWidgetSetSelection(self._pending_jobs_listbox,
                                          selection_index)
        ...
        if files_paused:
            ...
            ClientGUIFunctions.SetBitmapButtonBitmap(
                self._pause_files_button, CC.GlobalPixmaps.file_play)

        else:

            ClientGUIFunctions.SetBitmapButtonBitmap(
                self._pause_files_button, CC.GlobalPixmaps.file_pause)

        if queue_paused:

            ClientGUIFunctions.SetBitmapButtonBitmap(
                self._pause_queue_button, CC.GlobalPixmaps.gallery_play)

        else:

            ClientGUIFunctions.SetBitmapButtonBitmap(
                self._pause_queue_button, CC.GlobalPixmaps.gallery_pause)

        (file_network_job,
         page_network_job) = self._simple_downloader_import.GetNetworkJobs()
    ...
    def EventFormulaChanged(self):
        ...
        self._controller.new_options.SetString(
            'favourite_simple_downloader_formula', formula_name)
...
management_panel_types_to_classes[
    MANAGEMENT_TYPE_IMPORT_SIMPLE_DOWNLOADER] = ManagementPanelImporterSimpleDownloader
...
class ManagementPanelImporterURLs(ManagementPanelImporter):
    def __init__(self, parent, page, controller, management_controller):
        ...
        self._pause_button = ClientGUICommon.BetterBitmapButton(
            self._url_panel, CC.GlobalPixmaps.file_pause, self.Pause)
        self._pause_button.setToolTip('pause/play files')

        self._file_download_control = ClientGUIControls.NetworkJobControl(
            self._url_panel)

        self._urls_import = self._management_controller.GetVariable(
            'urls_import')

        self._file_seed_cache_control = ClientGUIFileSeedCache.FileSeedCacheStatusControl(
            self._url_panel, self._controller, page_key=self._page_key)

        self._gallery_download_control = ClientGUIControls.NetworkJobControl(
            self._url_panel)
        ...
        self._url_input = ClientGUIControls.TextAndPasteCtrl(
            self._url_panel, self._PendURLs)

        self._url_input.setPlaceholderText(
            'any url hydrus recognises, or a raw file url')

        (file_import_options,
         tag_import_options) = self._urls_import.GetOptions()

        show_downloader_options = True

        self._file_import_options = ClientGUIImport.FileImportOptionsButton(
            self._url_panel, file_import_options, show_downloader_options,
            self._urls_import.SetFileImportOptions)
        self._tag_import_options = ClientGUIImport.TagImportOptionsButton(
            self._url_panel,
            tag_import_options,
            show_downloader_options,
            update_callable=self._urls_import.SetTagImportOptions,
            allow_default_selection=True)

        #

        self._url_panel.Add(self._pause_button, CC.FLAGS_LONE_BUTTON)
        self._url_panel.Add(self._file_seed_cache_control,
                            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._url_panel.Add(self._file_download_control,
                            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._url_panel.Add(self._gallery_seed_log_control,
                            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._url_panel.Add(self._gallery_download_control,
                            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._url_panel.Add(self._url_input, CC.FLAGS_EXPAND_PERPENDICULAR)
        self._url_panel.Add(self._file_import_options,
                            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._url_panel.Add(self._tag_import_options,
                            CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _UpdateImportStatus(self):
        ...
        (file_network_job,
         gallery_network_job) = self._urls_import.GetNetworkJobs()
...
management_panel_types_to_classes[
    MANAGEMENT_TYPE_IMPORT_URLS] = ManagementPanelImporterURLs
...
class ManagementPanelPetitions(ManagementPanel):
    ...
    def __init__(self, parent, page, controller, management_controller):
        ...
        self._petition_service_key = management_controller.GetKey(
            'petition_service')

        ManagementPanel.__init__(self, parent, page, controller,
                                 management_controller)

        self._service = self._controller.services_manager.GetService(
            self._petition_service_key)
        self._can_ban = self._service.HasPermission(
        ...
        self._petitions_info_panel = ClientGUICommon.StaticBox(
            self, 'petitions info')

        self._refresh_num_petitions_button = ClientGUICommon.BetterButton(
            self._petitions_info_panel, 'refresh counts',
        ...
        for (content_type, status) in petition_types:
            ...
            button = ClientGUICommon.BetterButton(
                self._petitions_info_panel,
                'fetch ' + HC.content_status_string_lookup[status] + ' ' +
                HC.content_type_string_lookup[content_type] + ' petition',
                func)

            button.setEnabled(False)

            self._petition_types_to_controls[(content_type, status)] = (st,
                                                                        button)
        ...
        self._num_files_to_show = ClientGUICommon.NoneableSpinCtrl(
            self._petition_panel, message='number of files to show', min=1)

        self._num_files_to_show.SetValue(256)

        self._action_text = ClientGUICommon.BetterStaticText(
            self._petition_panel, label='')
        ...
        check_all = ClientGUICommon.BetterButton(self._petition_panel,
                                                 'check all', self._CheckAll)
        flip_selected = ClientGUICommon.BetterButton(self._petition_panel,
                                                     'flip selected',
                                                     self._FlipSelected)
        check_none = ClientGUICommon.BetterButton(self._petition_panel,
                                                  'check none',
                                                  self._CheckNone)

        self._sort_by_left = ClientGUICommon.BetterButton(
            self._petition_panel, 'sort by left', self._SortBy, 'left')
        self._sort_by_right = ClientGUICommon.BetterButton(
            self._petition_panel, 'sort by right', self._SortBy, 'right')
        ...
        self._modify_petitioner = QW.QPushButton('modify petitioner',
                                                 self._petition_panel)
        ...
        self._petitions_info_panel.Add(self._refresh_num_petitions_button,
                                       CC.FLAGS_EXPAND_PERPENDICULAR)

        for hbox in content_type_hboxes:

            self._petitions_info_panel.Add(hbox,
                                           CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)

        check_hbox = QP.HBoxLayout()

        QP.AddToLayout(check_hbox, check_all,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        QP.AddToLayout(check_hbox, flip_selected,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        QP.AddToLayout(check_hbox, check_none,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)

        sort_hbox = QP.HBoxLayout()

        QP.AddToLayout(sort_hbox, self._sort_by_left,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        QP.AddToLayout(sort_hbox, self._sort_by_right,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)

        self._petition_panel.Add(
            ClientGUICommon.BetterStaticText(
                self._petition_panel,
                label=
                'Double-click a petition to see its files, if it has them.'),
            CC.FLAGS_EXPAND_PERPENDICULAR)
        self._petition_panel.Add(self._num_files_to_show,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        self._petition_panel.Add(self._action_text,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        self._petition_panel.Add(self._reason_text,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)
        self._petition_panel.Add(check_hbox,
                                 CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        self._petition_panel.Add(sort_hbox,
                                 CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        ...
        self._petition_panel.Add(self._modify_petitioner,
                                 CC.FLAGS_EXPAND_PERPENDICULAR)

        vbox = QP.VBoxLayout()

        QP.AddToLayout(vbox, self._media_sort, CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._media_collect,
                       CC.FLAGS_EXPAND_PERPENDICULAR)

        QP.AddToLayout(vbox, self._petitions_info_panel,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
    ...
    def _DrawCurrentPetition(self):
        ...
        if self._current_petition is None:
            ...
            QP.SetBackgroundColour(self._reason_text,
            ...
            (action_text,
             action_colour) = self._current_petition.GetActionTextAndColour()
            ...
            if self._last_petition_type_fetched[0] in (
                    HC.CONTENT_TYPE_TAG_SIBLINGS, HC.CONTENT_TYPE_TAG_PARENTS):
    ...
    def _FetchPetition(self, content_type, status):
        ...
        def qt_done():
            ...
            button.setEnabled(True)
            button.setText('fetch ' + HC.content_status_string_lookup[status] +
                           ' ' + HC.content_type_string_lookup[content_type] +
                           ' petition')
    ...
    def _SetContentsAndChecks(self, contents_and_checks, sort_type):
        def key(c_and_s):
            ...
            if c.GetContentType() in (HC.CONTENT_TYPE_TAG_SIBLINGS,
                ...
                if sort_type == 'left':
                    ...
                    (part_one, part_two) = (HydrusTags.SplitTag(left),
                                            HydrusTags.SplitTag(right))

                elif sort_type == 'right':

                    (part_one, part_two) = (HydrusTags.SplitTag(right),
                                            HydrusTags.SplitTag(left))
    ...
    def EventContentDoubleClick(self, item):
        ...
        if len(selections) > 0:
            ...
            if content.HasHashes():
                ...
                if num_files_to_show is not None and len(
                        hashes) > num_files_to_show:
    ...
    def EventProcess(self):
        def break_approved_contents_into_chunks(approved_contents):
            ...
            for content in approved_contents:
                ...
                for content_chunk in content.IterateUploadableChunks(
                ):  # break 20K-strong mappings petitions into smaller bits to POST back
                    ...
                    if weight > 50:
                        ...
                        chunks_of_approved_contents.append(
                            chunk_of_approved_contents)
        ...
        def do_it(controller, service, petition_service_key, approved_contents,
                  denied_contents, petition):
            ...
            try:
                ...
                chunks_of_approved_contents = break_approved_contents_into_chunks(
                    approved_contents)
                ...
                for chunk_of_approved_contents in chunks_of_approved_contents:
                    ...
                    if job_key is not None:
                        ...
                        job_key.SetVariable('popup_gauge_1',
                                            (num_done, num_approved_to_do))

                    (update, content_updates
                     ) = petition.GetApproval(chunk_of_approved_contents)

                    service.Request(HC.POST, 'update',
                                    {'client_to_server_update': update})

                    controller.WriteSynchronous(
                        'content_updates',
                        {petition_service_key: content_updates})
        ...
        HG.client_controller.CallToThread(do_it, self._controller,
                                          self._service,
                                          self._petition_service_key,
                                          approved_contents, denied_contents,
                                          self._current_petition)
    ...
    def EventModifyPetitioner(self):
        ...
        QW.QMessageBox.warning(self, 'Warning',
                               'modify users does not work yet!')

        with ClientGUIDialogs.DialogModifyAccounts(
                self, self._petition_service_key,
            (self._current_petition.GetPetitionerAccount(), )) as dlg:
    ...
    def EventRowRightClick(self):
        ...
        for content in selected_contents:
            ...
            elif content_type in (HC.CONTENT_TYPE_TAG_SIBLINGS,
                                  HC.CONTENT_TYPE_TAG_PARENTS):
            ...
            for copyable_items in [copyable_items_a, copyable_items_b]:
                ...
                if len(copyable_items) > 0:
                    ...
                    if len(copyable_items) == 1:
                        ...
                        ClientGUIMenus.AppendMenuItem(
                            menu, 'copy {} tags'.format(
                                HydrusData.ToHumanInt(len(copyable_items))),
                            'Copy this tag.', HG.client_controller.pub,
                            'clipboard', 'text', text)
...
management_panel_types_to_classes[
    MANAGEMENT_TYPE_PETITIONS] = ManagementPanelPetitions
...
class ManagementPanelQuery(ManagementPanel):
    def __init__(self, parent, page, controller, management_controller):
        ...
        file_search_context = self._management_controller.GetVariable(
            'file_search_context')

        self._search_enabled = self._management_controller.GetVariable(
            'search_enabled')
        ...
        if self._search_enabled:
            ...
            self._current_predicates_box = ClientGUIListBoxes.ListBoxTagsActiveSearchPredicates(
                self._search_panel, self._page_key, initial_predicates)

            synchronised = self._management_controller.GetVariable(
                'synchronised')
            ...
            self._searchbox = ClientGUIACDropdown.AutoCompleteDropdownTagsRead(
                ...
                self._page_key,
                file_search_context,
                media_callable=self._page.GetMedia,
                synchronised=synchronised)

            self._cancel_search_button = ClientGUICommon.BetterBitmapButton(
                self._search_panel, CC.GlobalPixmaps.stop, self._CancelSearch)
            ...
            self._search_panel.Add(self._current_predicates_box,
                                   CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        if self._search_enabled:
            ...
            QP.AddToLayout(vbox, self._search_panel,
                           CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        self._controller.sub(self, 'SearchImmediately',
                             'notify_search_immediately')
        self._controller.sub(self, 'RefreshQuery', 'refresh_query')
        self._controller.sub(self, 'ChangeFileServicePubsub',
                             'change_file_service')
    ...
    def _MakeCurrentSelectionTagsBox(self, sizer):
        ...
        if self._search_enabled:
            ...
            t = ClientGUIListBoxes.ListBoxTagsSelectionManagementPanel(
                ...
                predicates_callable=self._current_predicates_box.GetPredicates)
    ...
    def _RefreshQuery(self):
        ...
        if self._search_enabled:
            ...
            if self._management_controller.GetVariable('synchronised'):
                ...
                self._management_controller.SetVariable(
                ...
                if len(current_predicates) > 0:
                    ...
                    self._controller.CallToThread(self.THREADDoQuery,
                                                  self._controller,
                                                  self._page_key,
                                                  self._query_job_key,
                                                  file_search_context, sort_by)

                    panel = ClientGUIResults.MediaPanelLoading(
                        self._page, self._page_key, file_service_key)
    ...
    def _UpdateCancelButton(self):
        ...
        if self._search_enabled:
            ...
            if self._query_job_key.IsDone():
                ...
                can_show = HydrusData.TimeHasPassedFloat(
                    self._query_job_key.GetCreationTime() + WAIT_PERIOD)
    ...
    def ShowFinishedQuery(self, query_job_key, media_results):
        ...
        if query_job_key == self._query_job_key:
            ...
            file_service_key = self._management_controller.GetKey(
                'file_service')
    ...
    def THREADDoQuery(self, controller, page_key, query_job_key,
                      search_context, sort_by):
        def qt_code():
        ...
        query_hash_ids = controller.Read('file_query_ids',
                                         search_context,
                                         job_key=query_job_key,
                                         limit_sort_by=sort_by)
        ...
        for sub_query_hash_ids in HydrusData.SplitListIntoChunks(
                query_hash_ids, QUERY_CHUNK_SIZE):

            if query_job_key.IsCancelled():

                return

            more_media_results = controller.Read('media_results_from_ids',
                                                 sub_query_hash_ids)

            media_results.extend(more_media_results)

            controller.pub('set_num_query_results', page_key,
                           len(media_results), len(query_hash_ids))
