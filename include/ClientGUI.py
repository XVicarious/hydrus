...
from . import (
    ClientData, ClientDownloading, ClientDragDrop, ClientExporting,
    ClientGUIAsync, ClientGUICommon, ClientGUIDialogs, ClientGUIDialogsManage,
    ClientGUIDialogsQuick, ClientGUIExport, ClientGUIFrames,
    ClientGUIFunctions, ClientGUIImport, ClientGUILogin, ClientGUIManagement,
    ClientGUIMediaControls, ClientGUIMenus, ClientGUIMPV, ClientGUIPages,
    ClientGUIParsing, ClientGUIPopupMessages, ClientGUIPredicates,
    ClientGUIScrolledPanels, ClientGUIScrolledPanelsEdit,
    ClientGUIScrolledPanelsManagement, ClientGUIScrolledPanelsReview,
    ClientGUIShortcutControls, ClientGUIShortcuts, ClientGUIStyle,
    ClientGUITags, ClientGUITopLevelWindows, ClientMedia,
    ClientNetworkingContexts, ClientNetworkingJobs, ClientParsing, ClientPaths,
    ClientRendering, ClientSearch, ClientServices, ClientTags, ClientThreading)
from . import HydrusConstants as HC
from . import HydrusData, HydrusExceptions
from . import HydrusGlobals as HG
from . import (HydrusImageHandling, HydrusNetwork, HydrusNetworking,
               HydrusPaths, HydrusSerialisable, HydrusText,
               HydrusVideoHandling)
from . import QtPorting as QP

MENU_ORDER = [
    'file', 'undo', 'pages', 'database', 'pending', 'network', 'services',
    'help'
]
...
def THREADUploadPending(service_key):
    ...
    try:
        ...
        job_key.SetVariable('popup_title',
                            'uploading pending to ' + service_name)
        ...
        while result is not None:
            ...
            job_key.SetVariable(
                'popup_text_1', 'uploading to ' + service_name + ': ' +
                HydrusData.ConvertValueRangeToPrettyString(
                    done_num_pending, num_to_do))
            ...
            try:
                ...
                if service_type in HC.REPOSITORIES:
                    ...
                    if isinstance(result, ClientMedia.MediaResult):
                        ...
                        service.Request(HC.POST, 'update', {
                    ...
                    if isinstance(result, ClientMedia.MediaResult):
                        ...
                        try:
                            ...
                            HydrusData.ShowText(
                                'File {} could not be pinned!'.format(
                                    hash.hexh()))
                ...
                job_key.SetVariable(
                    'popup_text_1',
                    service.GetName() +
                    ' was busy. please try again in a few minutes')
        ...
        if r is not None:
            ...
            HydrusData.ShowText(
                'Found a possible hash in that error message--trying to show it in a new page.'
            )

            HG.client_controller.pub('imported_files_to_page', [possible_hash],
                                     'files that did not upload right')
...
class BonedUpdater(ClientGUIAsync.AsyncQtUpdater):
    ...
    def _publishResult(self, result):
        ...
        frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
            self._win, 'review your fate')

        panel = ClientGUIScrolledPanelsReview.ReviewHowBonedAmI(
            frame, boned_stats)
...
class MenuUpdaterFile(ClientGUIAsync.AsyncQtUpdater):
    def _getResult(self):
        ...
        import_folder_names = HG.client_controller.Read(
            'serialisable_names',
            HydrusSerialisable.SERIALISABLE_TYPE_IMPORT_FOLDER)
        export_folder_names = HG.client_controller.Read(
            'serialisable_names',
    ...
    def _publishResult(self, result):
        ...
        (menu, label) = self._win.GenerateMenuInfoFile(import_folder_names,
                                                       export_folder_names)
...
class MenuUpdaterPages(ClientGUIAsync.AsyncQtUpdater):
    def _getResult(self):
        ...
        if len(gui_session_names) > 0:
            ...
            gui_session_names_to_backup_timestamps = HG.client_controller.Read(
                'serialisable_names_to_backup_timestamps',
                HydrusSerialisable.SERIALISABLE_TYPE_GUI_SESSION)
    ...
    def _publishResult(self, result):
        ...
        (menu, label) = self._win.GenerateMenuInfoPages(
            gui_session_names, gui_session_names_to_backup_timestamps)
...
class FrameGUI(ClientGUITopLevelWindows.MainFrameThatResizes):
    def __init__(self, controller):
        ...
        ClientGUITopLevelWindows.MainFrameThatResizes.__init__(
            self, None, title, 'main_gui')
        ...
        hydrus_busy_width = ClientGUIFunctions.ConvertTextToPixelWidth(
            self, 11)
        system_busy_width = ClientGUIFunctions.ConvertTextToPixelWidth(
            self, 13)
        db_width = ClientGUIFunctions.ConvertTextToPixelWidth(self, 14)

        self._statusbar = QP.StatusBar([
            -1, bandwidth_width, idle_width, hydrus_busy_width,
            system_busy_width, db_width
        ])
        self._statusbar.setSizeGripEnabled(True)
        self.setStatusBar(self._statusbar)

        self._statusbar_thread_updater = ClientGUIAsync.FastThreadToGUIUpdater(
            self._statusbar, self.RefreshStatusBar)

        self._canvas_frames = [
        ]  # Keep references to canvas frames so they won't get garbage collected (canvas frames don't have a parent)
        ...
        self._notebook = ClientGUIPages.PagesNotebook(self, self._controller,
                                                      'top page notebook')
        ...
        drop_target = ClientDragDrop.FileDropTarget(
            self, self.ImportFiles, self.ImportURLFromDragAndDrop,
            self._notebook.MediaDragAndDropDropped)
        self.installEventFilter(
            ClientDragDrop.FileDropTarget(
                self, self.ImportFiles, self.ImportURLFromDragAndDrop,
                self._notebook.MediaDragAndDropDropped))
        self._notebook.AddSupplementaryTabBarDropTarget(
            drop_target
        )  # ugly hack to make the case of files/media dropped onto a tab work

        self._message_manager = ClientGUIPopupMessages.PopupMessageManager(
            self)
        ...
        self._controller.sub(self, 'DeleteOldClosedPages',
                             'delete_old_closed_pages')
        ...
        self._controller.sub(self, 'NotifyNewExportFolders',
                             'notify_new_export_folders')
        self._controller.sub(self, 'NotifyNewImportFolders',
        ...
        self._controller.sub(self, 'NotifyNewPermissions',
                             'notify_new_permissions')
        self._controller.sub(self, 'NotifyNewServices',
        ...
        self._controller.sub(self, 'PresentImportedFilesToPage',
                             'imported_files_to_page')
        ...
        self._bandwidth_repeating_job = self._controller.CallRepeatingQtSafe(
            self, 1.0, 1.0, self.REPEATINGBandwidth)

        self._page_update_repeating_job = self._controller.CallRepeatingQtSafe(
            self, 0.25, 0.25, self.REPEATINGPageUpdate)
        ...
        self._animation_update_timer.timeout.connect(
            self.TIMEREventAnimationUpdate)

        self._animation_update_windows = set()

        self._my_shortcut_handler = ClientGUIShortcuts.ShortcutsHandler(
            self, ['global', 'main_gui'])

        self._controller.CallLaterQtSafe(
            self, 0.5, self._InitialiseSession
        )  # do this in callafter as some pages want to talk to controller.gui, which doesn't exist yet!
    ...
    def _AboutWindow(self):
        ...
        aboutinfo.SetVersion(
            str(HC.SOFTWARE_VERSION) + ', using network version ' +
            str(HC.NETWORK_VERSION))

        library_versions = []

        library_versions.append(
            ('FFMPEG', HydrusVideoHandling.GetFFMPEGVersion()))
        ...
        if ClientGUIMPV.MPV_IS_AVAILABLE:
            ...
            library_versions.append(('mpv api version: ',
                                     ClientGUIMPV.GetClientAPIVersionString()))
        ...
        library_versions.append(
            ('html5lib present: ', str(ClientParsing.HTML5LIB_IS_OK)))
        library_versions.append(
            ('lxml present: ', str(ClientParsing.LXML_IS_OK)))
        ...
        library_versions.append(('locale strings', str(
            (l_string, qtl_string))))

        description = 'This client is the media management application of the hydrus software suite.'

        description += os.linesep * 2 + os.linesep.join(
            (lib + ': ' + version for (lib, version) in library_versions))
    ...
    def _AccountInfo(self, service_key):
        ...
        with ClientGUIDialogs.DialogTextEntry(
                self, 'Enter the account\'s account key.') as dlg:
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                response = service.Request(
                    HC.GET, 'account_info',
                    {'subject_account_key': subject_account_key})
    ...
    def _AnalyzeDatabase(self):
        ...
        (result, was_cancelled) = ClientGUIDialogsQuick.GetYesNo(
            self,
            message,
            title='Choose how thorough your analyze will be.',
            yes_label='soft',
            no_label='full',
        ...
        if result == QW.QDialog.Accepted:
            ...
            self._controller.Write('analyze',
    ...
    def _AutoRepoSetup(self):
        ...
        ptr_credentials = HydrusNetwork.Credentials(host=host,
                                                    port=port,
        ...
        def do_it():
            ...
            all_services = list(
                self._controller.services_manager.GetServices())

            all_names = [s.GetName() for s in all_services]

            name = HydrusSerialisable.GetNonDupeName('public tag repository',
                                                     all_names)

            service_key = HydrusData.GenerateKey()
            service_type = HC.TAG_REPOSITORY

            public_tag_repo = ClientServices.GenerateService(
                service_key, service_type, name)
        ...
        for service in services:
            ...
            if credentials.GetSerialisableTuple(
            ) == ptr_credentials.GetSerialisableTuple():
    ...
    def _AutoServerSetup(self):
        def do_it():
            ...
            if HydrusNetworking.LocalPortInUse(port):
                ...
                HydrusData.ShowText(
                    'The server appears to be already running. Either that, or something else is using port '
                    + str(HC.DEFAULT_SERVER_ADMIN_PORT) + '.')
                ...
                try:
                    ...
                    if HC.PLATFORM_WINDOWS:
                        ...
                        server_frozen_path = os.path.join(
                            HC.BASE_DIR, 'server.exe')
                    ...
                    if os.path.exists(server_frozen_path):
                        ...
                        if python_executable.endswith(
                            ...
                            raise Exception(
                                'Could not automatically set up the server--could not find server executable or python executable.'
                            )
                    ...
                    while not HydrusNetworking.LocalPortInUse(port):
                        ...
                        if time_waited > 30:
                            ...
                            raise Exception(
                                'The server\'s port did not appear!')

                except:

                    HydrusData.ShowText(
                        'I tried to start the server, but something failed!' +
            ...
            admin_service = ClientServices.GenerateService(
                admin_service_key, service_type, name)
            ...
            admin_service = self._controller.services_manager.GetService(
                admin_service_key)  # let's refresh it
            ...
            response = admin_service.Request(HC.GET, 'access_key',
            ...
            tag_service = HydrusNetwork.GenerateService(
                service_key, HC.TAG_REPOSITORY, 'tag service',
            ...
            file_service = HydrusNetwork.GenerateService(
                service_key, HC.FILE_REPOSITORY, 'file service',
                HC.DEFAULT_SERVICE_PORT + 1)

            serverside_services.append(file_service)

            response = admin_service.Request(HC.POST, 'services',
                                             {'services': serverside_services})

            service_keys_to_access_keys = response[
            ...
            with HG.dirty_object_lock:
                ...
                self._controller.WriteSynchronous('update_server_services',
                                                  admin_service_key,
                                                  serverside_services,
                                                  service_keys_to_access_keys,
                                                  deletee_service_keys)
            ...
            HydrusData.ShowText(
                'Done! Check services->review services to see your new server and its services.'
            )
    ...
    def _BackupDatabase(self):
        ...
        if not os.path.exists(path):
            ...
            QW.QMessageBox.information(
                self, 'Information',
                'The backup path does not exist--creating it now.')
    ...
    def _BackupServer(self, service_key):
        def do_it(service):
            ...
            HydrusData.ShowText(
                'Server backup done in ' +
                HydrusData.TimeDeltaToPrettyTimeDelta(it_took) + '!')
    ...
    def _CheckImportFolder(self, name=None):
        ...
        if self._controller.options['pause_import_folders_sync']:
            ...
            HydrusData.ShowText(
                'Import folders are currently paused under the \'file\' menu. Please unpause them and try this again.'
            )

        if name is None:

            import_folders = self._controller.Read(
                'serialisable_named',
                HydrusSerialisable.SERIALISABLE_TYPE_IMPORT_FOLDER)

        else:

            import_folder = self._controller.Read(
                'serialisable_named',
    ...
    def _ClearFileViewingStats(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            content_update = HydrusData.ContentUpdate(
                HC.CONTENT_TYPE_FILE_VIEWING_STATS, HC.CONTENT_UPDATE_ADVANCED,
                'clear')
            ...
            QW.QMessageBox.information(
                self, 'Information',
                'Delete done! Please restart the client to see the changes in the UI.'
            )
    ...
    def _ClearOrphanFiles(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            (result, was_cancelled) = ClientGUIDialogsQuick.GetYesNo(
                self,
                text,
                title='Choose what do to with the orphans.',
                yes_label='move them somewhere',
                no_label='delete them',
                check_for_cancelled=True)
            ...
            if result == QW.QDialog.Accepted:
                ...
                with QP.DirDialog(self, 'Select location.') as dlg_3:
                    ...
                    if dlg_3.exec() == QW.QDialog.Accepted:
                        ...
                        self._controller.CallToThread(
                            client_files_manager.ClearOrphans, path)

            elif result == QW.QDialog.Rejected:

                self._controller.CallToThread(
                    client_files_manager.ClearOrphans)
    ...
    def _CullFileViewingStats(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            QW.QMessageBox.information(
                self, 'Information',
                'Cull done! Please restart the client to see the changes in the UI.'
            )
    ...
    def _DebugFetchAURL(self):
        def qt_code(network_job):
            ...
            text = 'Request complete. Length of response is ' + HydrusData.ToHumanBytes(
                len(content)) + '.'
            ...
            with ClientGUIDialogs.DialogYesYesNo(self,
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    if value == 'file':
                        ...
                        with QP.FileDialog(
                                self,
                                'select where to save content',
                                defaultFile='result.html',
                                acceptMode=QW.QFileDialog.AcceptSave) as f_dlg:
    ...
    def _DebugMakeDelayedModalPopup(self):
        def do_it(controller):
            ...
            for i in range(5):
                ...
                job_key.SetVariable(
                    'popup_text_1', 'Will auto-dismiss in ' +
                    HydrusData.TimeDeltaToPrettyTimeDelta(5 - i) + '.')
    ...
    def _DebugLongTextPopup(self):
        ...
        for i in range(2, 64):
            ...
            self._controller.CallLater(t, job_key.SetVariable, 'popup_text_1',
                                       text)
        ...
        for i in range(2, 64):
            ...
            self._controller.CallLater(t, job_key.SetVariable, 'popup_title',
                                       text)
    ...
    def _DebugMakeSomePopups(self):
        ...
        job_key.SetVariable(
            'popup_title',
            'This popup has a very long title -- it is a subscription that is running with a long "artist sub 123456" kind of name'
        )
        ...
        job_key.SetVariable('popup_text_1',
                            'Currently processing test job 5/8')
        job_key.SetVariable('popup_gauge_1', (5, 8))

        self._controller.pub('message', job_key)

        self._controller.CallLater(2.0, job_key.SetVariable, 'popup_text_2',
                                   'Pulsing subjob')

        self._controller.CallLater(2.0, job_key.SetVariable, 'popup_gauge_2',
                                   (0, None))
        ...
        for i in range(1, 4):
            ...
            self._controller.CallLater(
                0.5 * i, HydrusData.ShowText,
                'This is a delayed popup message -- ' + str(i))
    ...
    def _DebugShowGarbageDifferences(self):
        ...
        for (t, count) in to_print:
            ...
            text += os.linesep + '{}: {}'.format(t,
                                                 HydrusData.ToHumanInt(count))
    ...
    def _DeletePending(self, service_key):
        ...
        message = 'Are you sure you want to delete the pending data for {}?'.format(
            service_name)
    ...
    def _EnableLoadTruncatedImages(self):
        ...
        if not result:
            ...
            QW.QMessageBox.critical(
                self, 'Error',
                'Could not turn on--perhaps your version of PIL does not support it?'
            )

    def _ExportDownloader(self):

        with ClientGUITopLevelWindows.DialogNullipotent(
                self, 'export downloaders') as dlg:

            panel = ClientGUIParsing.DownloaderExportPanel(
                dlg, self._controller.network_engine)
    ...
    def _FetchIP(self, service_key):
        ...
        with ClientGUIDialogs.DialogTextEntry(
                self, 'Enter the file\'s hash.') as dlg:
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                with QP.BusyCursor():
                    response = service.Request(HC.GET, 'ip', {'hash': hash})

                ip = response['ip']
                timestamp = response['timestamp']

                gmt_time = HydrusData.ConvertTimestampToPrettyTime(timestamp,
                                                                   in_gmt=True)
                ...
                QW.QMessageBox.information(
                    self, 'Information', text + os.linesep * 2 +
                    'This has been written to the log.')

    def _FindMenuBarIndex(self, name):

        for index in range(len(self._menubar.actions())):

            if self._menubar.actions()[index].property(
                    'hydrus_menubar_name') == name:
    ...
    def _FlipClipboardWatcher(self, option_name):
        ...
        if self._clipboard_watcher_repeating_job is None:
            ...
            self._clipboard_watcher_repeating_job = self._controller.CallRepeatingQtSafe(
                self, 1.0, 1.0, self.REPEATINGClipboardWatcher)

    def _GenerateNewAccounts(self, service_key):

        with ClientGUIDialogs.DialogGenerateNewAccounts(self,
                                                        service_key) as dlg:
            dlg.exec()
    ...
    def _ImportDownloaders(self):
        ...
        frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
            self, 'import downloaders')

        panel = ClientGUIScrolledPanelsReview.ReviewDownloaderImport(
            frame, self._controller.network_engine)
    ...
    def _ImportFiles(self, paths=None):
        ...
        frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
            self, 'importing files')

        panel = ClientGUIScrolledPanelsReview.ReviewLocalFileImports(
            frame, paths)
    ...
    def _ImportUpdateFiles(self):
        def do_it(external_update_dir):
            ...
            update_paths = [
                os.path.join(external_update_dir, filename)
            ...
            if num_to_do == 0:
                ...
                QP.CallAfter(QW.QMessageBox.warning, self, 'Warning',
            ...
            try:
                ...
                for (i, update_path) in enumerate(update_paths):
                    ...
                    try:
                        ...
                        update_network_string_hash = hashlib.sha256(
                            update_network_bytes).digest()
                        ...
                        self._controller.WriteSynchronous(
                            'import_update', update_network_bytes,
                            update_network_string_hash, mime)
                ...
                if num_errors == 0:
                    ...
                    job_key.SetVariable(
                        'popup_text_1',
                        'Done with ' + HydrusData.ToHumanInt(num_errors) +
    ...
    def _ImportURL(self,
        ...
        (
            url_type, match_name, can_parse
        ) = self._controller.network_engine.domain_manager.GetURLParseCapability(
            url)

        if url_type in (HC.URL_TYPE_GALLERY, HC.URL_TYPE_POST,
                        HC.URL_TYPE_WATCHABLE) and not can_parse:

            message = 'This URL was recognised as a "{}" but this URL class does not yet have a parsing script linked to it!'.format(
                match_name)
            message += os.linesep * 2
        ...
        if (url_type == HC.URL_TYPE_UNKNOWN and allow_unrecognised_urls) or (
                url_type in (HC.URL_TYPE_FILE, HC.URL_TYPE_POST,
                             HC.URL_TYPE_GALLERY)
                and allow_other_recognised_urls):
            ...
            if not self._notebook.HasURLImportPage() and self.isMinimized():
                ...
                self._controller.CallLaterQtSafe(
                    ...
                    destination_page_key=destination_page_key,
                    show_destination_page=show_destination_page,
                    allow_watchers=allow_watchers,
                    allow_other_recognised_urls=allow_other_recognised_urls,
                    allow_unrecognised_urls=allow_unrecognised_urls)

                return (
                    url,
                    '"{}" URL was accepted, but it needed a new page and the client is currently minimized. It is queued to be added once the client is restored.'
                )

            page = self._notebook.GetOrMakeURLImportPage(
                desired_page_name=destination_page_name,
                desired_page_key=destination_page_key,
                select_page=show_destination_page)
            ...
            if page is not None:
                ...
                management_panel.PendURL(
                    url, service_keys_to_tags=service_keys_to_tags)
            ...
            if not self._notebook.HasMultipleWatcherPage(
                ...
                return (
                    url,
                    '"{}" URL was accepted, but it needed a new page and the client is current minimized. It is queued to be added once the client is restored.'
                )
    ...
    def _InitialiseMenubar(self):
        ...
        self._menu_updater = ClientGUIAsync.FastThreadToGUIUpdater(
            self._menubar, self.RefreshMenu)
    ...
    def _InitialiseSession(self):
        ...
        existing_session_names = self._controller.Read(
            'serialisable_names',
            HydrusSerialisable.SERIALISABLE_TYPE_GUI_SESSION)

        cannot_load_from_db = default_gui_session not in existing_session_names

        load_a_blank_page = HC.options[
            'default_gui_session'] == 'just a blank page' or cannot_load_from_db
        ...
        if not load_a_blank_page:
            ...
            if self._controller.LastShutdownWasBad():
                ...
                result = ClientGUIDialogsQuick.GetYesNo(
                    self,
                    message,
                    title='Previous shutdown was bad',
                    yes_label='try to load "' + default_gui_session + '"',
                    no_label='just load a blank page',
                    auto_yes_time=15)
        ...
        def do_it(default_gui_session, load_a_blank_page):
            ...
            try:
                ...
                last_session_save_period_minutes = self._controller.new_options.GetInteger(
                ...
                self._controller.CallLaterQtSafe(
                    self, last_session_save_period_minutes * 60,
        ...
        self._controller.CallLaterQtSafe(self, 0.25, do_it,
                                         default_gui_session,
                                         load_a_blank_page)
    ...
    def _ManageAccountTypes(self, service_key):
        ...
        with ClientGUITopLevelWindows.DialogManage(self, title) as dlg:
            ...
            panel = ClientGUIScrolledPanelsManagement.ManageAccountTypesPanel(
                dlg, service_key)
    ...
    def _ManageDefaultTagImportOptions(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
            ...
            panel = ClientGUIScrolledPanelsEdit.EditDefaultTagImportOptionsPanel(
                dlg, url_classes, parsers, url_class_keys_to_parser_keys,
                file_post_default_tag_import_options,
                watchable_default_tag_import_options,
                url_class_keys_to_tag_import_options)
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                (file_post_default_tag_import_options,
                 watchable_default_tag_import_options,
                 url_class_keys_to_tag_import_options) = panel.GetValue()

                domain_manager.SetDefaultTagImportOptions(
                    file_post_default_tag_import_options,
                    watchable_default_tag_import_options,
                    url_class_keys_to_tag_import_options)
    ...
    def _ManageDownloaderDisplay(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
            ...
            panel = ClientGUIScrolledPanelsEdit.EditDownloaderDisplayPanel(
                dlg, self._controller.network_engine, gugs,
                gug_keys_to_display, url_classes, url_class_keys_to_display)
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                (gug_keys_to_display,
                 url_class_keys_to_display) = panel.GetValue()

                domain_manager.SetGUGKeysToDisplay(gug_keys_to_display)
                domain_manager.SetURLClassKeysToDisplay(
                    url_class_keys_to_display)

    def _ManageExportFolders(self):
        def qt_do_it():

            export_folders = self._controller.Read(
                'serialisable_named',
                HydrusSerialisable.SERIALISABLE_TYPE_EXPORT_FOLDER)

            with ClientGUITopLevelWindows.DialogEdit(
                    self, 'edit export folders') as dlg:

                panel = ClientGUIExport.EditExportFoldersPanel(
                    dlg, export_folders)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    existing_db_names = set(
                        self._controller.Read(
                            ...
                            SERIALISABLE_TYPE_EXPORT_FOLDER))
        ...
        def THREAD_do_it(controller):
            ...
            with self._delayed_dialog_lock:
                ...
                original_pause_status = controller.options[
                ...
                try:
                    ...
                    if HG.export_folders_running:
                        ...
                        try:
                            ...
                            job_key.SetVariable(
                                'popup_text_1',
                                'Waiting for import folders to finish.')
                    ...
                    controller.options[
                        'pause_export_folders_sync'] = original_pause_status
    ...
    def _ManageImportFolders(self):
        def qt_do_it():
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                    self, 'edit import folders') as dlg:

                panel = ClientGUIImport.EditImportFoldersPanel(
                    dlg, import_folders)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    existing_db_names = set(
                        self._controller.Read(
                            'serialisable_names', HydrusSerialisable.
                            SERIALISABLE_TYPE_IMPORT_FOLDER))
        ...
        def THREAD_do_it(controller):
            ...
            with self._delayed_dialog_lock:
                ...
                try:
                    ...
                    controller.options[
                        'pause_import_folders_sync'] = original_pause_status
    ...
    def _ManageLogins(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
            ...
            panel = ClientGUILogin.EditLoginsPanel(
                dlg, self._controller.network_engine, login_scripts,
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                if len(domains_to_login) > 0:
                    ...
                    self._controller.network_engine.ForceLogins(
                        domains_to_login)
    ...
    def _ManageNetworkHeaders(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
            ...
            panel = ClientGUIScrolledPanelsEdit.EditNetworkContextCustomHeadersPanel(
                dlg, network_contexts_to_custom_header_dicts)
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                domain_manager.SetNetworkContextsToCustomHeaderDicts(
    ...
    def _ManageOptions(self):
        ...
        with ClientGUITopLevelWindows.DialogManage(self, title,
                                                   frame_key) as dlg:
        ...
        qt_style_name = self._controller.new_options.GetNoneableString(
            'qt_style_name')
        qt_stylesheet_name = self._controller.new_options.GetNoneableString(
            'qt_stylesheet_name')
    ...
    def _ManageServer(self, service_key):
        ...
        with ClientGUITopLevelWindows.DialogManage(self, title) as dlg:
            ...
            panel = ClientGUIScrolledPanelsManagement.ManageServerServicesPanel(
                dlg, service_key)
    ...
    def _ManageServices(self):
        ...
        try:
            ...
            with ClientGUITopLevelWindows.DialogManage(self, title) as dlg:
                ...
                panel = ClientGUIScrolledPanelsManagement.ManageClientServicesPanel(
                    dlg)
    ...
    def _ManageShortcuts(self):
        ...
        with ClientGUITopLevelWindows.DialogManage(self,
    ...
    def _ManageSubscriptions(self):
        def qt_do_it(subscriptions, original_pause_status):
            ...
            with ClientGUITopLevelWindows.DialogEdit(self, title,
                                                     frame_key) as dlg:

                panel = ClientGUIScrolledPanelsEdit.EditSubscriptionsPanel(
                    dlg, subscriptions, original_pause_status)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HG.client_controller.subscriptions_manager.NewSubscriptions(
                        subscriptions)
        ...
        def THREAD_do_it(controller):
            ...
            with self._delayed_dialog_lock:
                ...
                try:
                    ...
                    controller.CallLater(1.0, controller.pub, 'message',
                                         job_key)

                    subscription_names = HG.client_controller.Read(
                        'serialisable_names',
                        HydrusSerialisable.SERIALISABLE_TYPE_SUBSCRIPTION)
                    ...
                    for (i, name) in enumerate(subscription_names):
                        ...
                        subscription = HG.client_controller.Read(
                            'serialisable_named',
                            HydrusSerialisable.SERIALISABLE_TYPE_SUBSCRIPTION,
                            name)
                    ...
                    controller.options[
                        'pause_subs_sync'] = original_pause_status
    ...
    def _ManageTagDisplay(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
            ...
            panel = ClientGUIScrolledPanelsEdit.EditTagDisplayManagerPanel(
                dlg, self._controller.tag_display_manager)
    ...
    def _ManageTagParents(self):
        ...
        with ClientGUITopLevelWindows.DialogManage(
                self, 'manage tag parents') as dlg:
    ...
    def _ManageTagSiblings(self):
        ...
        with ClientGUITopLevelWindows.DialogManage(
                self, 'manage tag siblings') as dlg:
    ...
    def _ManageURLClasses(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
            ...
            panel = ClientGUIScrolledPanelsEdit.EditURLClassesPanel(
                dlg, url_classes)
    ...
    def _ManageURLClassLinks(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, title) as dlg:
            ...
            panel = ClientGUIScrolledPanelsEdit.EditURLClassLinksPanel(
                dlg, self._controller.network_engine, url_classes, parsers,
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                domain_manager.SetURLClassKeysToParserKeys(
                    url_class_keys_to_parser_keys)

    def _ManageUPnP(self):

        with ClientGUIDialogsManage.DialogManageUPnP(self) as dlg:
            dlg.exec()

    def _MigrateDatabase(self):

        with ClientGUITopLevelWindows.DialogNullipotent(
                self, 'migrate database') as dlg:

            panel = ClientGUIScrolledPanelsReview.MigrateDatabasePanel(
                dlg, self._controller)
    ...
    def _MigrateTags(self):
        ...
        frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
            HG.client_controller.gui, 'migrate tags')

        panel = ClientGUIScrolledPanelsReview.MigrateTagsPanel(
            frame, default_tag_repository_key)
    ...
    def _ModifyAccount(self, service_key):
        ...
        with ClientGUIDialogs.DialogTextEntry(
                self, 'Enter the account key for the account to be modified.'
        ) as dlg:
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                try:
                    ...
                    QW.QMessageBox.critical(
                        self, 'Error', 'Could not parse that account key')

                    return

                subject_account = 'blah'  # fetch account from service

                with ClientGUIDialogs.DialogModifyAccounts(
                        self, service_key, [subject_account]) as dlg2:
                    dlg2.exec()
    ...
    def _PauseSync(self, sync_type):
        ...
        if sync_type == 'repo':
            ...
            HC.options['pause_export_folders_sync'] = not HC.options[
                'pause_export_folders_sync']
            ...
            HC.options['pause_import_folders_sync'] = not HC.options[
                'pause_import_folders_sync']
    ...
    def _RefreshStatusBar(self):
        ...
        if not QP.isValid(
                self
        ) or not self._notebook or not self._statusbar or self.isMinimized():
    ...
    def _RestoreSplitterPositions(self):
        ...
        self._controller.pub('set_splitter_positions', HC.options['hpos'],
                             HC.options['vpos'])

    def _ReviewBandwidth(self):

        frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
            self, 'review bandwidth')

        panel = ClientGUIScrolledPanelsReview.ReviewAllBandwidthPanel(
            frame, self._controller)
    ...
    def _ReviewFileMaintenance(self):
        ...
        panel = ClientGUIScrolledPanelsReview.ReviewFileMaintenance(
            frame, self._controller)
    ...
    def _ReviewNetworkJobs(self):
        ...
        frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
            self, 'review network jobs')

        panel = ClientGUIScrolledPanelsReview.ReviewNetworkJobs(
            frame, self._controller)
    ...
    def _ReviewNetworkSessions(self):
        ...
        frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
            self, 'review session cookies')

        panel = ClientGUIScrolledPanelsReview.ReviewNetworkSessionsPanel(
            frame, self._controller.network_engine.session_manager)
    ...
    def _ReviewServices(self):
        ...
        frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
            self, self._controller.PrepStringForDisplay('Review Services'),
            'review_services')

        panel = ClientGUIScrolledPanelsReview.ReviewServicesPanel(
            frame, self._controller)
    ...
    def _ReviewThreads(self):
        ...
        panel = ClientGUIScrolledPanelsReview.ReviewThreads(
            frame, self._controller)
    ...
    def _RunExportFolder(self, name=None):
        ...
        if self._controller.options['pause_export_folders_sync']:
            ...
            HydrusData.ShowText(
                'Export folders are currently paused under the \'file\' menu. Please unpause them and try this again.'
            )
        ...
        if name is None:
            ...
            export_folder = self._controller.Read(
                'serialisable_named',
    ...
    def _RunUITest(self):
        def qt_open_pages():
            ...
            page_of_pages = self._notebook.NewPagesNotebook(
                on_deepest_notebook=False, select_page=True)

            t = 0.25

            HG.client_controller.CallLaterQtSafe(self,
                                                 t,
            ...
            HG.client_controller.CallLaterQtSafe(
                self, t, self.ProcessApplicationCommand,
                ClientData.ApplicationCommand(
                    CC.APPLICATION_COMMAND_TYPE_SIMPLE, 'new_page_of_pages'))

            t += 0.25

            HG.client_controller.CallLaterQtSafe(self,
                                                 t,
                                                 page_of_pages.NewPageQuery,
                                                 CC.LOCAL_FILE_SERVICE_KEY,
                                                 page_name='test',
        ...
        def qt_close_pages(page_of_pages):
            ...
            for i in indices:
                ...
                HG.client_controller.CallLaterQtSafe(self, t,
                                                     page_of_pages._ClosePage,
                                                     i)
            ...
            HG.client_controller.CallLaterQtSafe(
                self, t, self._notebook.CloseCurrentPage)
        ...
        def qt_test_ac():
            ...
            page = self._notebook.NewPageQuery(CC.LOCAL_FILE_SERVICE_KEY,
                                               page_name='test',
                                               select_page=True)
            ...
            HG.client_controller.CallLaterQtSafe(
                self, t, self.ProcessApplicationCommand,
                ClientData.ApplicationCommand(
                    CC.APPLICATION_COMMAND_TYPE_SIMPLE, 'set_media_focus'))
            ...
            HG.client_controller.CallLaterQtSafe(
                self, t, self.ProcessApplicationCommand,
                ClientData.ApplicationCommand(
                    CC.APPLICATION_COMMAND_TYPE_SIMPLE, 'set_search_focus'))
            ...
            HG.client_controller.CallLaterQtSafe(self, t, uias.Char,
                                                 QC.Qt.Key_Return)
            ...
            for i in range(16):
                ...
                for j in range(i + 1):
                    ...
                    HG.client_controller.CallLaterQtSafe(
                        self, t, uias.Char, QC.Qt.Key_Down)
        ...
        def do_it():
            ...
            page_of_pages = HG.client_controller.CallBlockingToQt(
                self, qt_open_pages)

            time.sleep(4)

            HG.client_controller.CallBlockingToQt(self,
                                                  qt_close_unclose_one_page)

            time.sleep(1.5)

            HG.client_controller.CallBlockingToQt(self, qt_close_pages,
                                                  page_of_pages)
...
The password is cleartext here but obscured in the entry dialog. Enter a blank password to remove.'''
        ...
        with ClientGUIDialogs.DialogTextEntry(self, message,
        ...
        with QP.DirDialog(self, 'Select backup location.') as dlg:
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                if path == self._controller.GetDBDir():
                    ...
                    QW.QMessageBox.critical(
                        self, 'Error',
                        'That directory is your current database directory! You cannot backup to the same location you are backing up from!'
                    )
    ...
    def _StartIPFSDownload(self):
        ...
        ipfs_services = self._controller.services_manager.GetServices(
            (HC.IPFS, ))
        ...
        if len(ipfs_services) > 0:
            ...
            if len(ipfs_services) == 1:
                ...
                choice_tuples = [(service.GetName(), service)
                                 for service in ipfs_services]

                try:

                    service = ClientGUIDialogsQuick.SelectFromList(
                        self, 'Select which IPFS Daemon', choice_tuples)
            ...
            with ClientGUIDialogs.DialogTextEntry(
                    self, 'Enter multihash to download.') as dlg:
    ...
    def _TestServerBusy(self, service_key):
        def do_it(service):
            ...
            if result_bytes == b'1':
                ...
                HydrusData.ShowText(
                    'server responded in a way I do not understand')
        ...
        try:
            ...
            if isinstance(service, ClientServices.ServiceRepository):
                ...
                if not service.IsMostlyCaughtUp():
                    ...
                    raise Exception(
                        'Repository processing is not caught up--please process more before you upload new content.'
                    )
            ...
            QW.QMessageBox.critical(
                self, 'Error',
                'Unfortunately, there is a problem with starting the upload: '
                + str(e))
    ...
    def _VacuumDatabase(self):
        ...
        (result, was_cancelled) = ClientGUIDialogsQuick.GetYesNo(
            self,
            text,
            title='Choose how thorough your vacuum will be.',
            yes_label='soft',
            no_label='full',
            check_for_cancelled=True)
    ...
    def AddModalMessage(self, job_key):
        ...
        if self.isMinimized(
        ) or dialog_open or not ClientGUIFunctions.TLWOrChildIsActive(self):

            self._controller.CallLaterQtSafe(self, 0.5, self.AddModalMessage,
                                             job_key)
            ...
            with ClientGUITopLevelWindows.DialogNullipotent(
                    self,
                    title,
                    hide_buttons=hide_close_button,
                    do_not_activate=True) as dlg:

                panel = ClientGUIPopupMessages.PopupMessageDialogPanel(
                    dlg, job_key)
    ...
    def AutoSaveLastSession(self):
        ...
        only_save_last_session_during_idle = self._controller.new_options.GetBoolean(
            'only_save_last_session_during_idle')

        if only_save_last_session_during_idle and not self._controller.CurrentlyIdle(
        ):

            self._controller.CallLaterQtSafe(self, 60,
                                             self.AutoSaveLastSession)
            ...
            if HC.options['default_gui_session'] == 'last session':
                ...
                self._controller.CallToThread(do_it, self._controller, session,
                                              self, next_call_delay, callable)
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() != QC.Qt.RightButton:
            ...
            ClientGUITopLevelWindows.MainFrameThatResizes.mouseReleaseEvent(
                self, event)
    ...
    def FleshOutPredicates(self, predicates):
        ...
        for predicate in predicates:
            ...
            if value is None and predicate_type in [
                    HC.PREDICATE_TYPE_SYSTEM_NUM_TAGS,
                    HC.PREDICATE_TYPE_SYSTEM_LIMIT,
                    HC.PREDICATE_TYPE_SYSTEM_SIZE,
                    HC.PREDICATE_TYPE_SYSTEM_DIMENSIONS,
                    HC.PREDICATE_TYPE_SYSTEM_AGE,
                    HC.PREDICATE_TYPE_SYSTEM_MODIFIED_TIME,
                    HC.PREDICATE_TYPE_SYSTEM_KNOWN_URLS,
                    HC.PREDICATE_TYPE_SYSTEM_HASH,
                    HC.PREDICATE_TYPE_SYSTEM_DURATION,
                    HC.PREDICATE_TYPE_SYSTEM_HAS_AUDIO,
                    HC.PREDICATE_TYPE_SYSTEM_NUM_WORDS,
                    HC.PREDICATE_TYPE_SYSTEM_MIME,
                    HC.PREDICATE_TYPE_SYSTEM_RATING,
                    HC.PREDICATE_TYPE_SYSTEM_SIMILAR_TO,
                    HC.PREDICATE_TYPE_SYSTEM_FILE_SERVICE,
                    HC.PREDICATE_TYPE_SYSTEM_TAG_AS_NUMBER,
                    HC.PREDICATE_TYPE_SYSTEM_FILE_RELATIONSHIPS,
                    HC.PREDICATE_TYPE_SYSTEM_FILE_VIEWING_STATS
            ]:

                with ClientGUITopLevelWindows.DialogEdit(
                        self, 'input predicate', hide_buttons=True) as dlg:

                    panel = ClientGUIPredicates.InputFileSystemPredicate(
                        dlg, predicate_type)
                ...
                good_predicates.append(
                    ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_NUM_TAGS,
                                           ('=', 0)))
    ...
    def GenerateMenuInfo(self, name):
        def undo():
            ...
            if have_closed_pages or have_undo_stuff:
                ...
                if undo_string is not None:
                    ClientGUIMenus.AppendMenuItem(menu, undo_string,
                                                  'Undo last operation.',
                                                  self._controller.pub, 'undo')

                if redo_string is not None:
                    ClientGUIMenus.AppendMenuItem(menu, redo_string,
                                                  'Redo last operation.',
                ...
                if have_closed_pages:
                    ...
                    ClientGUIMenus.AppendMenuItem(
                        undo_pages, 'clear all',
                        'Remove all closed pages from memory.',
                        self.DeleteAllClosedPages)
                    ...
                    for (i, (time_closed,
                             page)) in enumerate(self._closed_pages):
                    ...
                    for (index, name) in args:
                        ...
                        ClientGUIMenus.AppendMenuItem(undo_pages, name,
                                                      'Restore this page.',
                                                      self._UnclosePage, index)
        ...
        def database():
            ...
            ClientGUIMenus.AppendMenuItem(
                menu, 'set a password',
                'Set a simple password for the database so only you can open it in the client.',
                self._SetPassword)
            ...
            if HG.client_controller.client_files_manager.AllLocationsAreDefault(
                ...
                backup_path = self._new_options.GetNoneableString(
                    'backup_path')

                if backup_path is None:

                    ClientGUIMenus.AppendMenuItem(
                        menu, 'set up a database backup location',
                        'Choose a path to back the database up to.',
                        self._SetupBackupPath)

                else:

                    ClientGUIMenus.AppendMenuItem(
                        menu, 'update database backup',
                        'Back the database up to an external location.',
                        self._BackupDatabase)
                    ClientGUIMenus.AppendMenuItem(
                        menu, 'change database backup location',
                        'Choose a path to back the database up to.',
                        self._SetupBackupPath)
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'restore from a database backup',
                    'Restore the database from an external location.',
                    self._controller.RestoreDatabase)

                ClientGUIMenus.AppendSeparator(menu)

            ClientGUIMenus.AppendMenuItem(
                menu, 'migrate database',
                'Review and manage the locations your database is stored.',
            ...
            ClientGUIMenus.AppendMenuItem(
                file_maintenance_menu, 'review scheduled jobs',
                'Review outstanding jobs, and schedule new ones.',
                self._ReviewFileMaintenance)
            ClientGUIMenus.AppendSeparator(file_maintenance_menu)

            check_manager = ClientGUICommon.CheckboxManagerOptions(
            ...
            ClientGUIMenus.AppendMenuCheckItem(
                file_maintenance_menu, 'work during idle time',
                'Control whether file maintenance can work during idle time.',
                current_value, func)
            ...
            ClientGUIMenus.AppendMenuCheckItem(
                file_maintenance_menu, 'work during normal time',
                'Control whether file maintenance can work during normal time.',
                current_value, func)

            ClientGUIMenus.AppendMenu(submenu, file_maintenance_menu,
                                      'file maintenance')

            ClientGUIMenus.AppendMenuItem(
                submenu, 'vacuum',
                'Defrag the database by completely rebuilding it.',
                self._VacuumDatabase)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'analyze',
                'Optimise slow queries by running statistical analyses on the database.',
                self._AnalyzeDatabase)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'clear orphan files',
                'Clear out surplus files that have found their way into the file structure.',
                self._ClearOrphanFiles)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'clear orphan file records',
                'Clear out surplus file records that have not been deleted correctly.',
                self._ClearOrphanFileRecords)

            if self._controller.new_options.GetBoolean('advanced_mode'):

                ClientGUIMenus.AppendMenuItem(
                    submenu, 'clear orphan tables',
                    'Clear out surplus db tables that have not been deleted correctly.',
                    self._ClearOrphanTables)
            ...
            ClientGUIMenus.AppendMenuItem(
                submenu, 'database integrity',
                'Have the database examine all its records for internal consistency.',
                self._CheckDBIntegrity)
            ...
            ClientGUIMenus.AppendMenuItem(
                submenu, 'autocomplete cache',
                'Delete and recreate the tag autocomplete cache, fixing any miscounts.',
                self._RegenerateACCache)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'similar files search tree',
                'Delete and recreate the similar files search tree.',
                self._RegenerateSimilarFilesTree)
            ...
            ClientGUIMenus.AppendMenuItem(
                submenu, 'clear all file viewing statistics',
                'Delete all file viewing records from the database.',
                self._ClearFileViewingStats)
            ClientGUIMenus.AppendMenuItem(
                submenu,
                'cull file viewing statistics based on current min/max values',
                'Cull your file viewing statistics based on minimum and maximum permitted time deltas.',
                self._CullFileViewingStats)
        ...
        def network():
            ...
            pause_all_new_network_traffic = self._controller.new_options.GetBoolean(
                'pause_all_new_network_traffic')

            ClientGUIMenus.AppendMenuCheckItem(
                submenu, 'subscriptions',
                'Pause the client\'s synchronisation with website subscriptions.',
                HC.options['pause_subs_sync'], self._PauseSync, 'subs')
            ClientGUIMenus.AppendSeparator(submenu)
            ClientGUIMenus.AppendMenuCheckItem(
                submenu, 'all new network traffic',
                'Stop any new network jobs from sending data.',
                pause_all_new_network_traffic,
                self._controller.network_engine.PausePlayNewJobs)
            ClientGUIMenus.AppendMenuCheckItem(
                submenu, 'paged file import queues',
                'Pause all file import queues.',
                self._controller.new_options.GetBoolean(
                    'pause_all_file_queues'),
                self._controller.new_options.FlipBoolean,
                'pause_all_file_queues')
            ClientGUIMenus.AppendMenuCheckItem(
                submenu, 'gallery searches',
                'Pause all gallery imports\' searching.',
                self._controller.new_options.GetBoolean(
                    'pause_all_gallery_searches'),
                self._controller.new_options.FlipBoolean,
                'pause_all_gallery_searches')
            ClientGUIMenus.AppendMenuCheckItem(
                submenu, 'watcher checkers', 'Pause all watchers\' checking.',
                self._controller.new_options.GetBoolean(
            ...
            ClientGUIMenus.AppendMenuItem(submenu, 'review bandwidth usage',
                                          'See where you are consuming data.',
                                          self._ReviewBandwidth)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'review current network jobs',
                'Review the jobs currently running in the network engine.',
                self._ReviewNetworkJobs)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'review session cookies',
                'Review and edit which cookies you have for which network contexts.',
                self._ReviewNetworkSessions)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage http headers',
                'Configure how the client talks to the network.',
                self._ManageNetworkHeaders)

            ClientGUIMenus.AppendSeparator(submenu)

            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage upnp',
                'If your router supports it, see and edit your current UPnP NAT traversal mappings.',
                self._ManageUPnP)
            ...
            if not ClientParsing.HTML5LIB_IS_OK:
                ...
                ClientGUIMenus.AppendMenuItem(
                    submenu, '*** html5lib not found! ***',
                    'Your client does not have an important library.',
            ...
            ClientGUIMenus.AppendMenuItem(
                ...
                'Change the queries you want the client to regularly import from.',
                self._ManageSubscriptions)

            if self._controller.new_options.GetBoolean('advanced_mode'):

                ClientGUIMenus.AppendMenuItem(
                    submenu, 'nudge subscriptions awake',
                    'Tell the subs daemon to wake up, just in case any subs are due.',
                    self._controller.subscriptions_manager.ClearCacheAndWake)
            ...
            ClientGUIMenus.AppendMenuCheckItem(
                clipboard_menu, 'watcher urls',
                'Automatically import watcher URLs that enter the clipboard just as if you drag-and-dropped them onto the ui.',
                self._controller.new_options.GetBoolean(
                    'watch_clipboard_for_watcher_urls'),
                self._FlipClipboardWatcher, 'watch_clipboard_for_watcher_urls')
            ClientGUIMenus.AppendMenuCheckItem(
                clipboard_menu, 'other recognised urls',
                'Automatically import recognised URLs that enter the clipboard just as if you drag-and-dropped them onto the ui.',
                self._controller.new_options.GetBoolean(
                    'watch_clipboard_for_other_recognised_urls'),
                self._FlipClipboardWatcher,
                'watch_clipboard_for_other_recognised_urls')

            ClientGUIMenus.AppendMenu(submenu, clipboard_menu,
                                      'watch clipboard for urls')

            ClientGUIMenus.AppendSeparator(submenu)

            ClientGUIMenus.AppendMenuItem(
                submenu, 'import downloaders',
                'Import new download capability through encoded pngs from other users.',
                self._ImportDownloaders)

            ClientGUIMenus.AppendSeparator(submenu)

            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage default tag import options',
                'Change the default tag import options for each of your linked url matches.',
                self._ManageDefaultTagImportOptions)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage downloader and url display',
                'Configure how downloader objects present across the client.',
                self._ManageDownloaderDisplay)

            ClientGUIMenus.AppendSeparator(submenu)

            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage logins',
                'Edit which domains you wish to log in to.',
                self._ManageLogins)

            debug_menu = QW.QMenu(submenu)

            ClientGUIMenus.AppendMenuItem(
                debug_menu, 'do tumblr GDPR click-through',
                'Do a manual click-through for the tumblr GDPR page.',
                self._controller.CallLater, 0.0,
                self._controller.network_engine.login_manager.LoginTumblrGDPR)
            ...
            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage gallery url generators',
                'Manage the client\'s GUGs, which convert search terms into URLs.',
                self._ManageGUGs)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage url classes',
                'Configure which URLs the client can recognise.',
                self._ManageURLClasses)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage parsers',
                'Manage the client\'s parsers, which convert URL content into hydrus metadata.',
                self._ManageParsers)

            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage login scripts',
                'Manage the client\'s login scripts, which define how to log in to different sites.',
                self._ManageLoginScripts)

            ClientGUIMenus.AppendSeparator(submenu)

            ClientGUIMenus.AppendMenuItem(
                submenu, 'manage url class links',
                'Configure how URLs present across the client.',
                self._ManageURLClassLinks)
            ClientGUIMenus.AppendMenuItem(
                submenu, 'export downloaders',
                'Export downloader components to easy-import pngs.',
                self._ExportDownloader)

            ClientGUIMenus.AppendSeparator(submenu)

            ClientGUIMenus.AppendMenuItem(
                submenu, 'SEMI-LEGACY: manage file lookup scripts',
                'Manage how the client parses different types of web content.',
                self._ManageParsingScripts)
        ...
        def services():
            ...
            tag_services = self._controller.services_manager.GetServices(
                (HC.TAG_REPOSITORY, ))
            file_services = self._controller.services_manager.GetServices(
                (HC.FILE_REPOSITORY, ))

            submenu = QW.QMenu(menu)

            ClientGUIMenus.AppendMenuCheckItem(
                submenu, 'repositories synchronisation',
                'Pause the client\'s synchronisation with hydrus repositories.',
                HC.options['pause_repo_sync'], self._PauseSync, 'repo')
            ...
            ClientGUIMenus.AppendMenuItem(
                ...
                'Look at the services your client connects to.',
                self._ReviewServices)
            ClientGUIMenus.AppendMenuItem(
                menu, 'manage services',
                'Edit the services your client connects to.',
                self._ManageServices)

            repository_admin_permissions = [
                (HC.CONTENT_TYPE_ACCOUNTS, HC.PERMISSION_ACTION_CREATE),
                (HC.CONTENT_TYPE_ACCOUNTS, HC.PERMISSION_ACTION_OVERRULE),
                (HC.CONTENT_TYPE_ACCOUNT_TYPES, HC.PERMISSION_ACTION_OVERRULE)
            ]

            repositories = self._controller.services_manager.GetServices(
                HC.REPOSITORIES)
            admin_repositories = [
                service for service in repositories
                if True in (service.HasPermission(content_type, action)
                            for (content_type,
                                 action) in repository_admin_permissions)
            ]

            servers_admin = self._controller.services_manager.GetServices(
                (HC.SERVER_ADMIN, ))
            server_admins = [
                service for service in servers_admin if service.HasPermission(
                    HC.CONTENT_TYPE_SERVICES, HC.PERMISSION_ACTION_OVERRULE)
            ]
            ...
            if len(admin_repositories) > 0 or len(server_admins) > 0:
                ...
                for service in admin_repositories:
                    ...
                    can_create_accounts = service.HasPermission(
                        HC.CONTENT_TYPE_ACCOUNTS, HC.PERMISSION_ACTION_CREATE)
                    can_overrule_accounts = service.HasPermission(
                        HC.CONTENT_TYPE_ACCOUNTS,
                        HC.PERMISSION_ACTION_OVERRULE)
                    can_overrule_account_types = service.HasPermission(
                        HC.CONTENT_TYPE_ACCOUNT_TYPES,
                        HC.PERMISSION_ACTION_OVERRULE)

                    if can_create_accounts:
                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'create new accounts',
                            'Create new account keys for this service.',
                            self._GenerateNewAccounts, service_key)

                    if can_overrule_accounts:

                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'modify an account',
                            'Modify a specific account\'s type and expiration.',
                            self._ModifyAccount, service_key)
                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'get an account\'s info',
                            'Fetch information about an account from the service.',
                            self._AccountInfo, service_key)

                    if can_overrule_accounts and service.GetServiceType(
                    ) == HC.FILE_REPOSITORY:
                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'get an uploader\'s ip address',
                            'Fetch the ip address that uploaded a specific file, if the service knows it.',
                            self._FetchIP, service_key)
                    ...
                    if can_overrule_account_types:
                        ...
                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'manage account types',
                            'Add, edit and delete account types for this service.',
                            self._ManageAccountTypes, service_key)

                    ClientGUIMenus.AppendMenu(admin_menu, submenu,
                                              service.GetName())
                ...
                for service in server_admins:
                    ...
                    can_overrule_services = service.HasPermission(
                        HC.CONTENT_TYPE_SERVICES,
                        HC.PERMISSION_ACTION_OVERRULE)
                    ...
                    if can_overrule_services:
                        ...
                        ClientGUIMenus.AppendMenuItem(
                            ...
                            'Add, edit, and delete this server\'s services.',
                            self._ManageServer, service_key)
                        ClientGUIMenus.AppendSeparator(submenu)
                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'backup server',
                            'Command the server to temporarily pause and back up its database.',
                            self._BackupServer, service_key)
                        ClientGUIMenus.AppendSeparator(submenu)
                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'vacuum server',
                            'Command the server to temporarily pause and vacuum its database.',
                            self._VacuumServer, service_key)
                        ClientGUIMenus.AppendSeparator(submenu)
                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'server/db lock: on',
                            'Command the server to lock itself and disconnect its db.',
                            self._LockServer, service_key, True)
                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'server/db lock: test',
                            'See if the server is currently busy.',
                            self._TestServerBusy, service_key)
                        ClientGUIMenus.AppendMenuItem(
                            submenu, 'server/db lock: off',
                            'Command the server to unlock itself and resume its db.',
                            self._LockServer, service_key, False)

                    ClientGUIMenus.AppendMenu(admin_menu, submenu,
                                              service.GetName())

                ClientGUIMenus.AppendMenu(menu, admin_menu,
                                          'administrate services')

            ClientGUIMenus.AppendSeparator(menu)

            ClientGUIMenus.AppendMenuItem(
                menu, 'import repository update files',
                'Add repository update files to the database.',
                self._ImportUpdateFiles)

            ClientGUIMenus.AppendSeparator(menu)

            ClientGUIMenus.AppendMenuItem(
                menu, 'tag migration',
                'Migrate tags from one place to another.', self._MigrateTags)

            ClientGUIMenus.AppendSeparator(menu)

            ClientGUIMenus.AppendMenuItem(
                menu, 'manage tag display',
                'Set which tags you want to see from which services.',
                self._ManageTagDisplay)
            ClientGUIMenus.AppendMenuItem(
                menu, 'manage tag siblings',
                'Set certain tags to be automatically replaced with other tags.',
                self._ManageTagSiblings)
            ClientGUIMenus.AppendMenuItem(
                menu, 'manage tag parents',
                'Set certain tags to be automatically added with other tags.',
                self._ManageTagParents)
        ...
        def help():
            ...
            ClientGUIMenus.AppendMenuItem(
                menu, 'help and getting started guide',
                'Open hydrus\'s local help in your web browser.',
                ClientPaths.LaunchPathInWebBrowser,
                os.path.join(HC.HELP_DIR, 'index.html'))

            links = QW.QMenu(menu)

            site = ClientGUIMenus.AppendMenuBitmapItem(
                links, 'site',
                'Open hydrus\'s website, which is mostly a mirror of the local help.',
                CC.GlobalPixmaps.file_repository,
                ClientPaths.LaunchURLInWebBrowser,
                'https://hydrusnetwork.github.io/hydrus/')
            site = ClientGUIMenus.AppendMenuBitmapItem(
                links, '8kun board',
                'Open hydrus dev\'s 8kun board, where he makes release posts and other status updates.',
                CC.GlobalPixmaps.eight_kun, ClientPaths.LaunchURLInWebBrowser,
                'https://8kun.top/hydrus/index.html')
            site = ClientGUIMenus.AppendMenuItem(
                links, 'Endchan board bunker',
                'Open hydrus dev\'s Endchan board, the bunker for when 8kun is unavailable.',
                ClientPaths.LaunchURLInWebBrowser,
                'https://endchan.net/hydrus/index.html')
            site = ClientGUIMenus.AppendMenuBitmapItem(
                links, 'twitter',
                'Open hydrus dev\'s twitter, where he makes general progress updates and emergency notifications.',
                CC.GlobalPixmaps.twitter, ClientPaths.LaunchURLInWebBrowser,
                'https://twitter.com/hydrusnetwork')
            site = ClientGUIMenus.AppendMenuBitmapItem(
                links, 'tumblr',
                'Open hydrus dev\'s tumblr, where he makes release posts and other status updates.',
                CC.GlobalPixmaps.tumblr, ClientPaths.LaunchURLInWebBrowser,
                'http://hydrus.tumblr.com/')
            site = ClientGUIMenus.AppendMenuBitmapItem(
                links, 'discord',
                'Open a discord channel where many hydrus users congregate. Hydrus dev visits regularly.',
                CC.GlobalPixmaps.discord, ClientPaths.LaunchURLInWebBrowser,
                'https://discord.gg/vy8CUB4')
            site = ClientGUIMenus.AppendMenuBitmapItem(
                links, 'patreon',
                'Open hydrus dev\'s patreon, which lets you support development.',
                CC.GlobalPixmaps.patreon, ClientPaths.LaunchURLInWebBrowser,
            ...
            ClientGUIMenus.AppendMenuItem(
                menu, 'changelog',
                'Open hydrus\'s local changelog in your web browser.',
                ClientPaths.LaunchPathInWebBrowser,
                os.path.join(HC.HELP_DIR, 'changelog.html'))

            ClientGUIMenus.AppendSeparator(menu)

            ClientGUIMenus.AppendMenuItem(
                menu, 'add the public tag repository',
                'This will add the public tag repository to your client.',
                self._AutoRepoSetup)
            ...
            ClientGUIMenus.AppendMenuItem(
                menu, 'how boned am I?',
                'Check for a summary of your ride so far.', self._HowBonedAmI)

            ClientGUIMenus.AppendSeparator(menu)

            currently_darkmode = self._new_options.GetString(
                'current_colourset') == 'darkmode'

            ClientGUIMenus.AppendMenuCheckItem(
                menu, 'darkmode', 'Set the \'darkmode\' colourset on and off.',
                currently_darkmode, self.FlipDarkmode)
            ...
            ClientGUIMenus.AppendMenuCheckItem(
                menu, 'advanced mode',
                'Turn on advanced menu options and buttons.', current_value,
                func)
            ...
            ClientGUIMenus.AppendMenuCheckItem(
                debug_modes, 'force idle mode',
                'Make the client consider itself idle and fire all maintenance routines right now. This may hang the gui for a while.',
                HG.force_idle_mode, self._SwitchBoolean, 'force_idle_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                debug_modes, 'no page limit mode',
                'Let the user create as many pages as they want with no warnings or prohibitions.',
                HG.no_page_limit_mode, self._SwitchBoolean,
                'no_page_limit_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                debug_modes, 'thumbnail debug mode',
                'Show some thumbnail debug info.', HG.thumbnail_debug_mode,
                self._SwitchBoolean, 'thumbnail_debug_mode')
            ClientGUIMenus.AppendMenuItem(
                debug_modes, 'simulate a wake from sleep',
                'Tell the controller to pretend that it just woke up from sleep.',
            ...
            ClientGUIMenus.AppendMenuCheckItem(
                profile_modes, 'db profile mode',
                'Run detailed \'profiles\' on every database query and dump this information to the log (this is very useful for hydrus dev to have, if something is running slow for you!).',
                HG.db_profile_mode, self._SwitchBoolean, 'db_profile_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                profile_modes, 'menu profile mode',
                'Run detailed \'profiles\' on menu actions.',
                HG.menu_profile_mode, self._SwitchBoolean, 'menu_profile_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                profile_modes, 'pubsub profile mode',
                'Run detailed \'profiles\' on every internal publisher/subscriber message and dump this information to the log. This can hammer your log with dozens of large dumps every second. Don\'t run it unless you know you need to.',
                HG.pubsub_profile_mode, self._SwitchBoolean,
                'pubsub_profile_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                profile_modes, 'ui timer profile mode',
                'Run detailed \'profiles\' on every ui timer update. This will likely spam you!',
                HG.ui_timer_profile_mode, self._SwitchBoolean,
                'ui_timer_profile_mode')
            ...
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'callto report mode',
                'Report whenever the thread pool is given a task.',
                HG.callto_report_mode, self._SwitchBoolean,
                'callto_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'daemon report mode',
                'Have the daemons report whenever they fire their jobs.',
                HG.daemon_report_mode, self._SwitchBoolean,
                'daemon_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'db report mode',
                'Have the db report query information, where supported.',
                HG.db_report_mode, self._SwitchBoolean, 'db_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'file import report mode',
                'Have the db and file manager report file import progress.',
                HG.file_import_report_mode, self._SwitchBoolean,
                'file_import_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'file report mode',
                'Have the file manager report file request information, where supported.',
                HG.file_report_mode, self._SwitchBoolean, 'file_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'gui report mode',
                'Have the gui report inside information, where supported.',
                HG.gui_report_mode, self._SwitchBoolean, 'gui_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'hover window report mode',
                'Have the hover windows report their show/hide logic.',
                HG.hover_window_report_mode, self._SwitchBoolean,
                'hover_window_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'media load report mode',
                'Have the client report media load information, where supported.',
                HG.media_load_report_mode, self._SwitchBoolean,
                'media_load_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'network report mode',
                'Have the network engine report new jobs.',
                HG.network_report_mode, self._SwitchBoolean,
                'network_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'pubsub report mode',
                'Report info about every pubsub processed.',
                HG.pubsub_report_mode, self._SwitchBoolean,
                'pubsub_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'similar files metadata generation report mode',
                'Have the phash generation routine report its progress.',
                HG.phash_generation_report_mode, self._SwitchBoolean,
                'phash_generation_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'shortcut report mode',
                'Have the new shortcut system report what shortcuts it catches and whether it matches an action.',
                HG.shortcut_report_mode, self._SwitchBoolean,
                'shortcut_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'subprocess report mode',
                'Report whenever an external process is called.',
                HG.subprocess_report_mode, self._SwitchBoolean,
                'subprocess_report_mode')
            ClientGUIMenus.AppendMenuCheckItem(
                report_modes, 'subscription report mode',
                'Have the subscription system report what it is doing.',
                HG.subscription_report_mode, self._SwitchBoolean,
                'subscription_report_mode')
            ...
            ClientGUIMenus.AppendMenuItem(
                gui_actions, 'make some popups',
                'Throw some varied popups at the message manager, just to check it is working.',
                self._DebugMakeSomePopups)
            ClientGUIMenus.AppendMenuItem(
                gui_actions, 'make a long text popup',
                'Make a popup with text that will grow in size.',
                self._DebugLongTextPopup)
            ClientGUIMenus.AppendMenuItem(
                gui_actions, 'make a popup in five seconds',
                'Throw a delayed popup at the message manager, giving you time to minimise or otherwise alter the client before it arrives.',
                self._controller.CallLater, 5, HydrusData.ShowText,
                'This is a delayed popup message.')
            ClientGUIMenus.AppendMenuItem(
                gui_actions, 'make a modal popup in five seconds',
                'Throw up a delayed modal popup to test with. It will stay alive for five seconds.',
                self._DebugMakeDelayedModalPopup)
            ClientGUIMenus.AppendMenuItem(
                gui_actions, 'make a new page in five seconds',
                'Throw a delayed page at the main notebook, giving you time to minimise or otherwise alter the client before it arrives.',
                self._controller.CallLater, 5, self._controller.pub,
                'new_page_query', CC.LOCAL_FILE_SERVICE_KEY)
            ClientGUIMenus.AppendMenuItem(
                gui_actions, 'make a parentless text ctrl dialog',
                'Make a parentless text control in a dialog to test some character event catching.',
                self._DebugMakeParentlessTextCtrl)
            ClientGUIMenus.AppendMenuItem(
                gui_actions, 'force a main gui layout now',
                'Tell the gui to relayout--useful to test some gui bootup layout issues.',
                self.adjustSize)
            ClientGUIMenus.AppendMenuItem(
                gui_actions, 'save \'last session\' gui session',
                'Make an immediate save of the \'last session\' gui session. Mostly for testing crashes, where last session is not saved correctly.',
                self.ProposeSaveGUISession, 'last session')
            ClientGUIMenus.AppendMenuItem(
                gui_actions, 'run the ui test',
                'Run hydrus_dev\'s weekly UI Test. Guaranteed to work and not mess up your session, ha ha.',
                self._RunUITest)
            ...
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'run fast memory maintenance',
                'Tell all the fast caches to maintain themselves.',
                self._controller.MaintainMemoryFast)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'run slow memory maintenance',
                'Tell all the slow caches to maintain themselves.',
                self._controller.MaintainMemorySlow)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'review threads',
                'Show current threads and what they are doing.',
                self._ReviewThreads)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'show scheduled jobs',
                'Print some information about the currently scheduled jobs log.',
                self._DebugShowScheduledJobs)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'subscription manager snapshot',
                'Have the subscription system show what it is doing.',
                self._controller.subscriptions_manager.ShowSnapshot)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'flush log',
                'Command the log to write any buffered contents to hard drive.',
                HydrusData.DebugPrint, 'Flushing log')
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'print garbage',
                'Print some information about the python garbage to the log.',
                self._DebugPrintGarbage)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'take garbage snapshot',
                'Capture current garbage object counts.',
                self._DebugTakeGarbageSnapshot)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'show garbage snapshot changes',
                'Show object count differences from the last snapshot.',
                self._DebugShowGarbageDifferences)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'enable truncated image loading',
                'Enable the truncated image loading to test out broken jpegs.',
                self._EnableLoadTruncatedImages)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'clear image rendering cache',
                'Tell the image rendering system to forget all current images. This will often free up a bunch of memory immediately.',
                self._controller.ClearCaches)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'clear thumbnail cache',
                'Tell the thumbnail cache to forget everything and redraw all current thumbs.',
                self._controller.pub, 'reset_thumbnail_cache')
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'clear db service info cache',
                'Delete all cached service info like total number of mappings or files, in case it has become desynchronised. Some parts of the gui may be laggy immediately after this as these numbers are recalculated.',
                self._DeleteServiceInfo)
            ClientGUIMenus.AppendMenuItem(
                data_actions, 'load whole db in disk cache',
                'Contiguously read as much of the db as will fit into memory. This will massively speed up any subsequent big job.',
                self._controller.CallToThread, self._controller.Read,
                'load_into_disk_cache')
            ...
            ClientGUIMenus.AppendMenuItem(
                network_actions, 'fetch a url',
                'Fetch a URL using the network engine as per normal.',
                self._DebugFetchAURL)

            ClientGUIMenus.AppendMenu(debug, network_actions,
                                      'network actions')

            ClientGUIMenus.AppendMenuItem(
                debug, 'run and initialise server for testing',
                'This will try to boot the server in your install folder and initialise it. This is mostly here for testing purposes.',
                self._AutoServerSetup)
            ...
            ClientGUIMenus.AppendMenuItem(
                menu, 'about Qt', 'See information about the Qt framework.',
                QW.QMessageBox.aboutQt, self)
            ClientGUIMenus.AppendMenuItem(
                menu, 'about',
                'See this client\'s version and other information.',
                self._AboutWindow)
    ...
    def GenerateMenuInfoFile(self, import_folder_names, export_folder_names):
        ...
        ClientGUIMenus.AppendMenuItem(menu, 'import files',
        ...
        ClientGUIMenus.AppendMenuCheckItem(
            submenu, 'import folders', 'Pause the client\'s import folders.',
            HC.options['pause_import_folders_sync'], self._PauseSync,
            'import_folders')
        ClientGUIMenus.AppendMenuCheckItem(
            submenu, 'export folders', 'Pause the client\'s export folders.',
            HC.options['pause_export_folders_sync'], self._PauseSync,
            'export_folders')
        ...
        if len(import_folder_names) > 0:
            ...
            if len(import_folder_names) > 1:
                ...
                ClientGUIMenus.AppendMenuItem(submenu, 'check all',
            ...
            for name in import_folder_names:
                ...
                ClientGUIMenus.AppendMenuItem(submenu, name,
                                              'Check this import folder now.',
                                              self._CheckImportFolder, name)

            ClientGUIMenus.AppendMenu(i_and_e_submenu, submenu,
        ...
        if len(export_folder_names) > 0:
            ...
            if len(export_folder_names) > 1:
                ...
                ClientGUIMenus.AppendMenuItem(submenu, 'run all',
                                              'Run all export folders.',
            ...
            for name in export_folder_names:
                ClientGUIMenus.AppendMenuItem(submenu, name,
                                              'Check this export folder now.',
                                              self._RunExportFolder, name)

            ClientGUIMenus.AppendMenu(i_and_e_submenu, submenu,
                                      'check export folder now')

        ClientGUIMenus.AppendSeparator(i_and_e_submenu)

        ClientGUIMenus.AppendMenuItem(
            i_and_e_submenu, 'manage import folders',
            'Manage folders from which the client can automatically import.',
            self._ManageImportFolders)
        ClientGUIMenus.AppendMenuItem(
            i_and_e_submenu, 'manage export folders',
            'Manage folders to which the client can automatically export.',
            self._ManageExportFolders)

        ClientGUIMenus.AppendMenu(menu, i_and_e_submenu,
        ...
        ClientGUIMenus.AppendMenuItem(
            ...
            'Open the installation directory for this client.',
            self._OpenInstallFolder)
        ClientGUIMenus.AppendMenuItem(
            open, 'database directory',
            'Open the database directory for this instance of the client.',
            self._OpenDBFolder)
        ClientGUIMenus.AppendMenuItem(
            open, 'quick export directory',
            'Open the export directory so you can easily access the files you have exported.',
            self._OpenExportFolder)

        ClientGUIMenus.AppendMenu(menu, open, 'open')

        ClientGUIMenus.AppendSeparator(menu)

        ClientGUIMenus.AppendMenuItem(menu, 'options',
                                      'Change how the client operates.',
                                      self._ManageOptions)
        ClientGUIMenus.AppendMenuItem(
            menu, 'shortcuts', 'Edit the shortcuts your client responds to.',
        ...
        if not we_borked_linux_pyinstaller:
            ...
            ClientGUIMenus.AppendMenuItem(
                menu,
                'restart',
                'Shut the client down and then start it up again.',
        ...
        ClientGUIMenus.AppendMenuItem(
            ...
            'Shut the client down and force any outstanding shutdown maintenance to run.',
            self.TryToSaveAndClose,
            force_shutdown_maintenance=True)

        ClientGUIMenus.AppendMenuItem(menu, 'exit', 'Shut the client down.',
                                      self.TryToSaveAndClose)

        return (menu, '&file')

    def GenerateMenuInfoPages(self, gui_session_names,
                              gui_session_names_to_backup_timestamps):

        menu = QW.QMenu(self)

        (total_active_page_count, total_closed_page_count, total_active_weight,
         total_closed_weight) = self.GetTotalPageCounts()

        self._last_total_page_weight = total_active_weight + total_closed_weight

        ClientGUIMenus.AppendMenuLabel(
            menu, '{} pages open'.format(
                HydrusData.ToHumanInt(total_active_page_count)),
            'You have this many pages open.')
        ClientGUIMenus.AppendMenuLabel(
            menu, 'total session weight: {}'.format(
                HydrusData.ToHumanInt(self._last_total_page_weight)),
            'Your session is this heavy.')

        ClientGUIMenus.AppendSeparator(menu)

        ClientGUIMenus.AppendMenuItem(
            menu, 'refresh', 'If the current page has a search, refresh it.',
            self._Refresh)

        splitter_menu = QW.QMenu(menu)

        ClientGUIMenus.AppendMenuItem(splitter_menu, 'show/hide',
        ...
        ClientGUIMenus.AppendMenuCheckItem(
            ...
            'Set whether sash position should be saved over on client exit.',
            self._new_options.GetBoolean('saving_sash_positions_on_exit'),
            self._new_options.FlipBoolean, 'saving_sash_positions_on_exit')

        ClientGUIMenus.AppendSeparator(splitter_menu)

        ClientGUIMenus.AppendMenuItem(
            splitter_menu, 'save current page\'s sash positions now',
            'Save the current page\'s sash positions.',
            self._SaveSplitterPositions)

        ClientGUIMenus.AppendSeparator(splitter_menu)

        ClientGUIMenus.AppendMenuItem(
            splitter_menu, 'restore all pages\' sash positions to saved value',
            'Restore the current sash positions for all pages to the values that are saved.',
            self._RestoreSplitterPositions)

        ClientGUIMenus.AppendMenu(menu, splitter_menu,
        ...
        if len(gui_session_names) > 0:
            ...
            for name in gui_session_names:
                ...
                ClientGUIMenus.AppendMenuItem(
                    load, name, 'Close all other pages and load this session.',
            ...
            for name in gui_session_names:
                ...
                ClientGUIMenus.AppendMenuItem(
                    append, name,
                    'Append this session to whatever pages are already open.',
                    self._notebook.AppendGUISession, name)
                ...
                for (name, timestamps) in rows:
                    ...
                    for timestamp in timestamps:
                        ...
                        ClientGUIMenus.AppendMenuItem(
                            ...
                            'Append this backup session to whatever pages are already open.',
                            self._notebook.AppendGUISessionBackup, name,
                            timestamp)

                    ClientGUIMenus.AppendMenu(append_backup, submenu, name)

                ClientGUIMenus.AppendMenu(sessions, append_backup,
        ...
        for name in gui_session_names:
            ...
            ClientGUIMenus.AppendMenuItem(
                save, name, 'Save the existing open pages as a session.',
                self.ProposeSaveGUISession, name)

        ClientGUIMenus.AppendMenuItem(
            save, 'as new session',
            'Save the existing open pages as a session.',
            self.ProposeSaveGUISession)

        ClientGUIMenus.AppendMenu(sessions, save, 'save')

        if len(
                set(gui_session_names).difference(
                    ClientGUIPages.RESERVED_SESSION_NAMES)) > 0:
            ...
            for name in gui_session_names:
                ...
                ClientGUIMenus.AppendMenuItem(delete, name,
                                              'Delete this session.',
                                              self._DeleteGUISession, name)
        ...
        ClientGUIMenus.AppendMenuItem(
            menu, 'pick a new page', 'Choose a new page to open.',
            self.ProcessApplicationCommand,
        ...
        petition_permissions = [(content_type, HC.PERMISSION_ACTION_OVERRULE)
                                for content_type in HC.REPOSITORY_CONTENT_TYPES
                                ]

        repositories = [
            service for service in services
            if service.GetServiceType() in HC.REPOSITORIES
        ]
        ...
        petition_resolvable_repositories = [
            repository for repository in repositories
            if True in (repository.HasPermission(content_type, action)
                        for (content_type, action) in petition_permissions)
        ]

        ClientGUIMenus.AppendMenuItem(search_menu,
                                      'my files',
                                      'Open a new search tab for your files.',
                                      self._notebook.NewPageQuery,
                                      CC.LOCAL_FILE_SERVICE_KEY,
                                      on_deepest_notebook=True)
        ClientGUIMenus.AppendMenuItem(
            search_menu,
            'trash',
            'Open a new search tab for your recently deleted files.',
            self._notebook.NewPageQuery,
        ...
        if len(petition_resolvable_repositories) > 0:
            ...
            for service in petition_resolvable_repositories:
                ...
                ClientGUIMenus.AppendMenuItem(petition_menu,
                                              service.GetName(),
                                              'Open a new petition page for ' +
                                              service.GetName() + '.',
                                              self._notebook.NewPagePetitions,
                                              service.GetServiceKey(),
                                              on_deepest_notebook=True)
        ...
        ClientGUIMenus.AppendMenuItem(
            download_menu, 'url download',
            'Open a new tab to download some separate urls.',
            self.ProcessApplicationCommand,
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'new_url_downloader_page'))
        ClientGUIMenus.AppendMenuItem(
            download_menu, 'watcher',
            'Open a new tab to watch threads or other updating locations.',
            self.ProcessApplicationCommand,
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'new_watcher_downloader_page'))
        ClientGUIMenus.AppendMenuItem(
            download_menu, 'gallery',
            'Open a new tab to download from gallery sites.',
            self.ProcessApplicationCommand,
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'new_gallery_downloader_page'))
        ClientGUIMenus.AppendMenuItem(
            download_menu, 'simple downloader',
            'Open a new tab to download files from generic galleries or threads.',
            self.ProcessApplicationCommand,
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'new_simple_downloader_page'))
        ...
        if has_ipfs:
            ...
            ClientGUIMenus.AppendMenuItem(
                download_popup_menu, 'an ipfs multihash',
                'Enter an IPFS multihash and attempt to import whatever is returned.',
                self._StartIPFSDownload)

            ClientGUIMenus.AppendMenu(menu, download_popup_menu,
                                      'new download popup')
        ...
        ClientGUIMenus.AppendMenuItem(
            special_menu, 'page of pages',
            'Open a new tab that can hold more tabs.',
            self.ProcessApplicationCommand,
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'new_page_of_pages'))
        ClientGUIMenus.AppendMenuItem(
            special_menu, 'duplicates processing',
            'Open a new tab to discover and filter duplicate files.',
            self.ProcessApplicationCommand,
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'new_duplicate_filter_page'))
        ...
        ClientGUIMenus.AppendMenuItem(
            special_command_menu, 'clear all multiwatcher highlights',
            'Command all multiwatcher pages to clear their highlighted watchers.',
            HG.client_controller.pub, 'clear_multiwatcher_highlights')

        ClientGUIMenus.AppendMenu(menu, special_command_menu,
                                  'special commands')
    ...
    def GenerateMenuInfoPending(self, nums_pending):
        ...
        for (service_key, info) in nums_pending.items():
            ...
            if service_type == HC.TAG_REPOSITORY:
                ...
                num_pending = info[
                    HC.SERVICE_INFO_NUM_PENDING_MAPPINGS] + info[
                        HC.SERVICE_INFO_NUM_PENDING_TAG_SIBLINGS] + info[
                            HC.SERVICE_INFO_NUM_PENDING_TAG_PARENTS]
                num_petitioned = info[
                    HC.SERVICE_INFO_NUM_PETITIONED_MAPPINGS] + info[
                        HC.SERVICE_INFO_NUM_PETITIONED_TAG_SIBLINGS] + info[
            ...
            if can_do_a_menu and num_pending + num_petitioned > 0:
                ...
                ClientGUIMenus.AppendMenuItem(
                    submenu, 'commit',
                    'Upload ' + name + '\'s pending content.',
                    self._UploadPending, service_key)
                ClientGUIMenus.AppendMenuItem(
                    submenu, 'forget',
                    'Clear ' + name + '\'s pending content.',
                    self._DeletePending, service_key)
                ...
                if num_pending > 0:
                    ...
                    submessages.append(
                        HydrusData.ToHumanInt(num_pending) + ' ' +
                        pending_phrase)

                if num_petitioned > 0:

                    submessages.append(
                        HydrusData.ToHumanInt(num_petitioned) + ' ' +
                        petitioned_phrase)
        ...
        return (menu, '&pending ({})'.format(
            HydrusData.ToHumanInt(total_num_pending)))
    ...
    def IShouldRegularlyUpdate(self, window):
        ...
        if current_page is not None:
            ...
            in_current_page = ClientGUIFunctions.IsQtAncestor(
                window, current_page)
    ...
    def ImportURLFromAPI(self, url, service_keys_to_tags,
                         destination_page_name, destination_page_key,
                         show_destination_page):

        try:

            (normalised_url, result_text) = self._ImportURL(
                url,
                service_keys_to_tags=service_keys_to_tags,
                destination_page_name=destination_page_name,
                destination_page_key=destination_page_key,
    ...
    def MaintainCanvasFrameReferences(self):
        ...
        self._canvas_frames = [
            frame for frame in self._canvas_frames if QP.isValid(frame)
        ]

    def NewPageImportHDD(self, paths, file_import_options,
                         paths_to_service_keys_to_tags, delete_after_success):

        management_controller = ClientGUIManagement.CreateManagementControllerImportHDD(
            paths, file_import_options, paths_to_service_keys_to_tags,
            delete_after_success)

        self._notebook.NewPage(management_controller, on_deepest_notebook=True)

    def NewPageQuery(self,
                     service_key,
                     initial_hashes=None,
        ...
        self._notebook.NewPageQuery(service_key,
    ...
    def PresentImportedFilesToPage(self, hashes, page_name):
        ...
        if tlw.isMinimized(
        ) and not self._notebook.HasMediaPageName(page_name):

            self._controller.CallLaterQtSafe(self, 10.0,
                                             self.PresentImportedFilesToPage,
                                             hashes, page_name)
    ...
    def ProcessApplicationCommand(self, command):
        ...
        if command_type == CC.APPLICATION_COMMAND_TYPE_SIMPLE:
            ...
            if action == 'exit_application':
                ...
                self._notebook.NewPageImportSimpleDownloader(
                    on_deepest_notebook=True)
                ...
                self._notebook.NewPageImportMultipleWatcher(
                    on_deepest_notebook=True)
    ...
    def ProposeSaveGUISession(self,
                              name=None,
                              suggested_name='',
                              notebook=None):
                ...
                with ClientGUIDialogs.DialogTextEntry(
                    ...
                    if dlg.exec() == QW.QDialog.Accepted:
                        ...
                        if name in ClientGUIPages.RESERVED_SESSION_NAMES:
                            ...
                            QW.QMessageBox.critical(
                                self, 'Error',
                                'Sorry, you cannot have that name! Try another.'
                            )
                            ...
                            existing_session_names = self._controller.Read(
                                'serialisable_names', HydrusSerialisable.
                                SERIALISABLE_TYPE_GUI_SESSION)

                            if name in existing_session_names:

                                message = 'Session "{}" already exists! Do you want to overwrite it?'.format(
                                    name)

                                (result, closed_by_user
                                 ) = ClientGUIDialogsQuick.GetYesNo(
                                     self,
                                     message,
                                     title='Overwrite existing session?',
                                     yes_label='yes, overwrite',
                                     no_label='no, choose another name',
                                     check_for_cancelled=True)
    ...
    def RegisterCanvasFrameReference(self, frame):
        ...
        self._canvas_frames = [
            fr for fr in self._canvas_frames if QP.isValid(fr)
        ]
    ...
    def RegisterUIUpdateWindow(self, window):
        ...
        if self._ui_update_repeating_job is None:
            ...
            self._ui_update_repeating_job = self._controller.CallRepeatingQtSafe(
                self, 0.0, 0.1, self.REPEATINGUIUpdate)
    ...
    def REPEATINGBandwidth(self):
        ...
        global_tracker = self._controller.network_engine.bandwidth_manager.GetTracker(
        ...
        current_usage = global_tracker.GetUsage(HC.BANDWIDTH_TYPE_DATA,
                                                1,
                                                for_user=True)

        if current_usage > 0:

            bandwidth_status += ' (' + HydrusData.ToHumanBytes(
                current_usage) + '/s)'
    ...
    def REPEATINGClipboardWatcher(self):
        ...
        allow_watchers = self._controller.new_options.GetBoolean(
            'watch_clipboard_for_watcher_urls')
        allow_other_recognised_urls = self._controller.new_options.GetBoolean(
            'watch_clipboard_for_other_recognised_urls')
        ...
        if text != self._last_clipboard_watched_text:
            ...
            for possible_url in HydrusText.DeserialiseNewlinedTexts(text):
                ...
                try:
                    ...
                    HydrusData.ShowText(
                        'Could not find a new page to place the clipboard URL. Perhaps the client is at its page limit.'
                    )
    ...
    def SaveAndClose(self):
        ...
        try:
            ...
            ClientGUITopLevelWindows.SaveTLWSizeAndPosition(
                self, self._frame_key)
            ...
            self._controller.WriteSynchronous('serialisable',
                                              self._new_options)
    ...
    def TryToSaveAndClose(self,
                          restart=False,
                          force_shutdown_maintenance=False):
        ...
        if not HG.emergency_exit:
            ...
            if HC.options[
                    'confirm_client_exit'] or able_to_close_statement is not None:
...
class FrameSplashPanel(QW.QWidget):
    ...
    def EventDrag(self, event):
        ...
        if event.type() == QC.QEvent.MouseMove and (
                event.buttons()
