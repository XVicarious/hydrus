...
class HydrusController(object):
    ...
    def _MaintainCallToThreads(self):
        ...
        with self._call_to_thread_lock:
            ...
            self._call_to_threads = list(
                filter(filter_call_to_threads, self._call_to_threads))

            self._long_running_call_to_threads = list(
                filter(filter_call_to_threads,
                       self._long_running_call_to_threads))
    ...
    def AcquireThreadSlot(self, thread_type):
        ...
        with self._thread_slot_lock:
            ...
            if current_threads < max_threads:
                ...
                self._thread_slots[thread_type] = (current_threads + 1,
                                                   max_threads)
    ...
    def CallLater(self, initial_delay, func, *args, **kwargs):
        ...
        job = HydrusThreading.SchedulableJob(self, job_scheduler,
                                             initial_delay, call)
    ...
    def CallRepeating(self, initial_delay, period, func, *args, **kwargs):
        ...
        job = HydrusThreading.RepeatingJob(self, job_scheduler, initial_delay,
                                           period, call)
    ...
    def GetThreadPoolBusyStatus(self):
        ...
        if HydrusData.TimeHasPassed(
                self._thread_pool_busy_status_text_new_check_time):

            with self._call_to_thread_lock:

                num_threads = sum(
                    (1 for t in self._call_to_threads if t.CurrentlyWorking()))
            ...
            self._thread_pool_busy_status_text_new_check_time = HydrusData.GetNow(
            ) + 10
    ...
    def GoodTimeToStartBackgroundWork(self):
        ...
        return self.CurrentlyIdle() and not (self.JustWokeFromSleep()
                                             or self.SystemBusy())
    ...
    def InitView(self):
        ...
        self.services_upnp_manager = HydrusNATPunch.ServicesUPnPManager(
            upnp_services)

        job = self.CallRepeating(10.0, 43200.0,
                                 self.services_upnp_manager.RefreshUPnP)
    ...
    def PrintProfile(self, summary, profile_text):
        ...
        boot_pretty_timestamp = time.strftime(
            '%Y-%m-%d %H-%M-%S', time.localtime(self._timestamps['boot']))
    ...
    def ShutdownFromServer(self):
        ...
        raise Exception(
            'This hydrus application cannot be shut down from the server!')
        ...
        with self._sleep_lock:
            ...
            if HydrusData.TimeHasPassed(self._timestamps['now_awake']):
                ...
                if HydrusData.TimeHasPassed(
                    ...
                    ) + 15  # enough time for ethernet to get back online and all that
