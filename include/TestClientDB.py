...
from . import (HydrusNetwork, HydrusSerialisable, HydrusVideoHandling,
               ServerDB, TestController)
...
class TestClientDB(unittest.TestCase):
    ...
    def _clear_db(cls):
        ...
        # class variable
        cls._db = ClientDB.DB(HG.test_controller, TestController.DB_DIR,
                              'client')
    ...
    def _read(self, action, *args, **kwargs):
        return TestClientDB._db.Read(action, *args, **kwargs)

    def _write(self, action, *args, **kwargs):
        return TestClientDB._db.Write(action, True, *args, **kwargs)

    def test_autocomplete(self):

        TestClientDB._clear_db()

        result = self._read('autocomplete_predicates',
        ...
        for p in result:
            self.assertEqual(p.GetCount(HC.CONTENT_STATUS_CURRENT), 1)
        ...
        result = self._read('autocomplete_predicates',
                            tag_service_key=CC.DEFAULT_LOCAL_TAG_SERVICE_KEY,
                            search_text='series:c*')

        pred = ClientSearch.Predicate(HC.PREDICATE_TYPE_TAG,
                                      'series:cars',
                                      min_current_count=1)
    ...
    def test_export_folders(self):
        ...
        file_search_context = ClientSearch.FileSearchContext(
            file_service_key=HydrusData.GenerateKey(),
            tag_service_key=HydrusData.GenerateKey(),
            predicates=[ClientSearch.Predicate(HC.PREDICATE_TYPE_TAG, 'test')])
    ...
    def test_file_query_ids(self):
        ...
        def run_namespace_predicate_tests(tests):
            ...
            for (inclusive, namespace, result) in tests:
                ...
                search_context = ClientSearch.FileSearchContext(
        ...
        (written_status, written_note) = self._write('import_file',
                                                     file_import_job)
        ...
        tests.append((HC.PREDICATE_TYPE_SYSTEM_AGE, ('<', 'delta', (
            1,
            1,
            1,
            1,
        ...
        tests.append((HC.PREDICATE_TYPE_SYSTEM_DURATION, (
            '<',
            100,
        ), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_DURATION, (
        ...
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_FILE_SERVICE,
             (False, HC.CONTENT_STATUS_CURRENT, CC.LOCAL_FILE_SERVICE_KEY), 0))
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_FILE_SERVICE,
             (False, HC.CONTENT_STATUS_PENDING, CC.LOCAL_FILE_SERVICE_KEY), 1))
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_FILE_SERVICE,
             (True, HC.CONTENT_STATUS_CURRENT, CC.LOCAL_FILE_SERVICE_KEY), 1))
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_FILE_SERVICE,
             (True, HC.CONTENT_STATUS_PENDING, CC.LOCAL_FILE_SERVICE_KEY), 0))
        ...
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_HASH,
             ((bytes.fromhex('0123456789abcdef' * 4), ), 'sha256'), 0))
        ...
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIMILAR_TO,
                      ((bytes.fromhex('0123456789abcdef' * 4), ), 5), 0))

        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('<', 0, HydrusData.ConvertUnitToInt('B')), 0))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('<', 5270, HydrusData.ConvertUnitToInt('B')), 0))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('<', 5271, HydrusData.ConvertUnitToInt('B')), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('=', 5270, HydrusData.ConvertUnitToInt('B')), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('=', 0, HydrusData.ConvertUnitToInt('B')), 0))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('\u2248', 5270, HydrusData.ConvertUnitToInt('B')), 1))
        ...
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('>', 5270, HydrusData.ConvertUnitToInt('B')), 0))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('>', 5269, HydrusData.ConvertUnitToInt('B')), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('>', 0, HydrusData.ConvertUnitToInt('B')), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('>', 0, HydrusData.ConvertUnitToInt('KB')), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('>', 0, HydrusData.ConvertUnitToInt('MB')), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_SIZE,
                      ('>', 0, HydrusData.ConvertUnitToInt('GB')), 1))
        ...
        service_keys_to_content_updates[CC.COMBINED_LOCAL_FILE_SERVICE_KEY] = (
            HydrusData.ContentUpdate(HC.CONTENT_TYPE_FILES,
                                     HC.CONTENT_UPDATE_ARCHIVE, (hash, )), )
        service_keys_to_content_updates[CC.DEFAULT_LOCAL_TAG_SERVICE_KEY] = (
            HydrusData.ContentUpdate(HC.CONTENT_TYPE_MAPPINGS,
                                     HC.CONTENT_UPDATE_ADD,
                                     ('car', (hash, ))), )
        ...
        or_pred_1 = ClientSearch.Predicate(HC.PREDICATE_TYPE_OR_CONTAINER,
                                           preds)

        preds = []

        preds.append(
            ClientSearch.Predicate(HC.PREDICATE_TYPE_TAG, 'maker:toyota'))
        preds.append(
            ClientSearch.Predicate(HC.PREDICATE_TYPE_TAG, 'maker:ford'))

        or_pred_2 = ClientSearch.Predicate(HC.PREDICATE_TYPE_OR_CONTAINER,
                                           preds)
        ...
        services.append(
            ClientServices.GenerateService(
                TestController.LOCAL_RATING_LIKE_SERVICE_KEY,
                HC.LOCAL_RATING_LIKE, 'test like rating service'))
        services.append(
            ClientServices.GenerateService(
                TestController.LOCAL_RATING_NUMERICAL_SERVICE_KEY,
                HC.LOCAL_RATING_NUMERICAL, 'test numerical rating service'))
        ...
        content_updates.append(
            HydrusData.ContentUpdate(HC.CONTENT_TYPE_RATINGS,
                                     HC.CONTENT_UPDATE_ADD, (1.0, (hash, ))))

        service_keys_to_content_updates[
            TestController.LOCAL_RATING_LIKE_SERVICE_KEY] = content_updates
        ...
        content_updates.append(
            HydrusData.ContentUpdate(HC.CONTENT_TYPE_RATINGS,
                                     HC.CONTENT_UPDATE_ADD, (0.6, (hash, ))))

        service_keys_to_content_updates[
            TestController.
            LOCAL_RATING_NUMERICAL_SERVICE_KEY] = content_updates
        ...
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_RATING,
             ('=', 1.0, TestController.LOCAL_RATING_LIKE_SERVICE_KEY), 1))
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_RATING,
             ('=', 0.0, TestController.LOCAL_RATING_LIKE_SERVICE_KEY), 0))
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_RATING,
             ('=', 'rated', TestController.LOCAL_RATING_LIKE_SERVICE_KEY), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_RATING,
                      ('=', 'not rated',
                       TestController.LOCAL_RATING_LIKE_SERVICE_KEY), 0))

        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_RATING,
             ('=', 0.6, TestController.LOCAL_RATING_NUMERICAL_SERVICE_KEY), 1))
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_RATING,
             ('=', 1.0, TestController.LOCAL_RATING_NUMERICAL_SERVICE_KEY), 0))
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_RATING,
             ('>', 0.6, TestController.LOCAL_RATING_NUMERICAL_SERVICE_KEY), 0))
        tests.append(
            (HC.PREDICATE_TYPE_SYSTEM_RATING,
             ('>', 0.4, TestController.LOCAL_RATING_NUMERICAL_SERVICE_KEY), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_RATING,
                      ('=', 'rated',
                       TestController.LOCAL_RATING_NUMERICAL_SERVICE_KEY), 1))
        tests.append((HC.PREDICATE_TYPE_SYSTEM_RATING,
                      ('=', 'not rated',
                       TestController.LOCAL_RATING_NUMERICAL_SERVICE_KEY), 0))
    ...
    def test_file_system_predicates(self):
        ...
        predicates.append(
            ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_EVERYTHING,
                                   min_current_count=1))
        predicates.append(
            ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_INBOX,
                                   min_current_count=1))
        predicates.append(
            ClientSearch.Predicate(HC.PREDICATE_TYPE_SYSTEM_ARCHIVE,
                                   min_current_count=0))
        predicates.extend([
            ClientSearch.Predicate(predicate_type) for predicate_type in [
                HC.PREDICATE_TYPE_SYSTEM_NUM_TAGS,
                HC.PREDICATE_TYPE_SYSTEM_LIMIT, HC.PREDICATE_TYPE_SYSTEM_SIZE,
                HC.PREDICATE_TYPE_SYSTEM_AGE,
                HC.PREDICATE_TYPE_SYSTEM_MODIFIED_TIME,
                HC.PREDICATE_TYPE_SYSTEM_KNOWN_URLS, HC.
                PREDICATE_TYPE_SYSTEM_HAS_AUDIO, HC.PREDICATE_TYPE_SYSTEM_HASH,
                HC.PREDICATE_TYPE_SYSTEM_DIMENSIONS,
                HC.PREDICATE_TYPE_SYSTEM_DURATION, HC.
                PREDICATE_TYPE_SYSTEM_NUM_WORDS, HC.PREDICATE_TYPE_SYSTEM_MIME,
    ...
    def test_gui_sessions(self):
        def qt_code():
            ...
            try:
                ...
                page = ClientGUIPages.Page(test_frame, HG.test_controller,
                ...
                service_keys_to_tags = ClientTags.ServiceKeysToTags(
                    {HydrusData.GenerateKey(): ['some', 'tags']})

                management_controller = ClientGUIManagement.CreateManagementControllerImportHDD(
                    ['some', 'paths'], ClientImportOptions.FileImportOptions(),
                    {'paths': service_keys_to_tags}, True)

                management_controller.GetVariable('hdd_import').PausePlay(
                )  # to stop trying to import 'some' 'paths'
                ...
                fsc = ClientSearch.FileSearchContext(
                    file_service_key=CC.LOCAL_FILE_SERVICE_KEY, predicates=[])

                management_controller = ClientGUIManagement.CreateManagementControllerQuery(
                    'search', CC.LOCAL_FILE_SERVICE_KEY, fsc, True)
                ...
                management_controller = ClientGUIManagement.CreateManagementControllerQuery(
                    'search', CC.LOCAL_FILE_SERVICE_KEY, fsc, False)

                page = ClientGUIPages.Page(
                    test_frame, HG.test_controller, management_controller,
                    [HydrusData.GenerateKey() for i in range(200)])
                ...
                fsc = ClientSearch.FileSearchContext(
                    file_service_key=CC.LOCAL_FILE_SERVICE_KEY,
                    predicates=[ClientSearch.SYSTEM_PREDICATE_ARCHIVE])

                management_controller = ClientGUIManagement.CreateManagementControllerQuery(
                    'files', CC.LOCAL_FILE_SERVICE_KEY, fsc, True)
                ...
                management_controller = ClientGUIManagement.CreateManagementControllerQuery(
                    'wew lad', CC.LOCAL_FILE_SERVICE_KEY, fsc, True)
                ...
                fsc = ClientSearch.FileSearchContext(
                    ...
                    predicates=[
                        ...
                        ClientSearch.Predicate(
                            HC.PREDICATE_TYPE_SYSTEM_FILE_SERVICE,
                            (True, HC.CONTENT_STATUS_CURRENT,
                             CC.LOCAL_FILE_SERVICE_KEY))
                    ])
                ...
                self.assertEqual(page_names, [
                    'gallery', 'watcher', 'import', 'simple downloader',
                    'example tag repo petitions', 'search', 'search', 'files',
                    'wew lad', 'files'
                ])
    ...
    def test_import(self):
        ...
        test_files.append((
            'muh_swf.swf',
            'edfef9905fdecde38e0752a5b6ab7b6df887c3968d4246adc9cffc997e168cdf',
            456774, HC.APPLICATION_FLASH, 400, 400, {33}, {1}, True, None))
        test_files.append((
            'muh_mp4.mp4',
            '2fa293907144a046d043d74e9570b1c792cbfd77ee3f5c93b2b1a1cb3e4c7383',
            570534, HC.VIDEO_MP4, 480, 480, {6266, 6290}, {151}, True, None))
        test_files.append((
            'muh_mpeg.mpeg',
            'aebb10aaf3b27a5878fd2732ea28aaef7bbecef7449eaa759421c4ba4efff494',
            772096, HC.VIDEO_MPEG, 720, 480, {3500}, {105}, False, None))
        test_files.append((
            'muh_webm.webm',
            '55b6ce9d067326bf4b2fbe66b8f51f366bc6e5f776ba691b0351364383c43fcb',
            84069, HC.VIDEO_WEBM, 640, 360, {4010}, {120}, True, None))
        test_files.append((
            'muh_jpg.jpg',
            '5d884d84813beeebd59a35e474fa3e4742d0f2b6679faa7609b245ddbbd05444',
            42296, HC.IMAGE_JPEG, 392, 498, {None}, {None}, False, None))
        test_files.append((
            'muh_png.png',
            'cdc67d3b377e6e1397ffa55edc5b50f6bdf4482c7a6102c6f27fa351429d6f49',
            31452, HC.IMAGE_PNG, 191, 196, {None}, {None}, False, None))
        test_files.append((
            'muh_apng.png',
            '9e7b8b5abc7cb11da32db05671ce926a2a2b701415d1b2cb77a28deea51010c3',
            616956, HC.IMAGE_APNG, 500, 500, {3133, 1880, 1125,
                                              1800}, {27, 47}, False, None))
        test_files.append((
            'muh_gif.gif',
            '00dd9e9611ebc929bfc78fde99a0c92800bbb09b9d18e0946cea94c099b211c2',
            15660, HC.IMAGE_GIF, 329, 302, {600}, {5}, False, None))

        for (filename, hex_hash, size, mime, width, height, durations,
             num_frames, has_audio, num_words) in test_files:
            ...
            (written_status,
             written_note) = self._write('import_file', file_import_job)
            ...
            (mr_file_info_manager, mr_tags_manager, mr_locations_manager,
             mr_ratings_manager) = media_result.ToTuple()

            (mr_hash_id, mr_hash, mr_size, mr_mime, mr_width, mr_height,
             mr_duration, mr_num_frames, mr_has_audio,
             mr_num_words) = mr_file_info_manager.ToTuple()
    ...
    def test_import_folders(self):
        ...
        import_folder_1 = ClientImportLocal.ImportFolder(
            'imp 1',
            path=TestController.DB_DIR,
            mimes=HC.VIDEO,
            publish_files_to_popup_button=False)
        import_folder_2 = ClientImportLocal.ImportFolder(
            'imp 2',
            path=TestController.DB_DIR,
            mimes=HC.IMAGES,
            period=1200,
        ...
        result = self._read('serialisable_named',
        ...
        self._write('delete_serialisable_named',
                    HydrusSerialisable.SERIALISABLE_TYPE_IMPORT_FOLDER,
                    'imp 2')
    ...
    def test_init(self):
        ...
        self.assertTrue(
            os.path.exists(os.path.join(TestController.DB_DIR, 'client.db')))
    ...
    def test_hash_status(self):
        ...
        self.assertIn(status,
                      (CC.STATUS_UNKNOWN, CC.STATUS_SUCCESSFUL_BUT_REDUNDANT))
    ...
    def test_pending(self):
        ...
        services.append(
            ClientServices.GenerateService(service_key, HC.TAG_REPOSITORY,
                                           'new tag repo'))
        ...
        content_updates = [
            HydrusData.ContentUpdate(HC.CONTENT_TYPE_MAPPINGS,
                                     HC.CONTENT_UPDATE_PEND, (tag, hashes))
            for tag in tags
        ]
    ...
    def test_pixiv_account(self):
        ...
        self._write('serialisable_simple', 'pixiv_account',
                    (pixiv_id, password))
    ...
    def test_services(self):
        ...
        result = self._read('services',
                            (HC.LOCAL_FILE_DOMAIN, HC.LOCAL_FILE_TRASH_DOMAIN,
                             HC.COMBINED_LOCAL_FILE, HC.LOCAL_TAG))

        result_service_keys = {service.GetServiceKey() for service in result}

        self.assertEqual(
            {
                CC.TRASH_SERVICE_KEY, CC.LOCAL_FILE_SERVICE_KEY,
        ...
        services.append(
            ClientServices.GenerateService(HydrusData.GenerateKey(),
                                           HC.TAG_REPOSITORY, 'new service'))
    ...
    def test_shortcuts(self):
        ...
        for (i, shortcuts) in enumerate(ClientDefaults.GetDefaultShortcuts()):
            ...
            for (shortcut, command) in shortcuts:
                ...
                self.assertEqual(
                    result.GetCommand(shortcut).GetData(), command.GetData())
