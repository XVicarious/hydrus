...
from . import (ClientDownloading, ClientImportFileSeeds,
               ...
               ClientNetworkingJobs, ClientPaths)
...
class GalleryImport(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (serialisable_gallery_import_key, self._creation_time,
                self._query, self._source_name, self._current_page_index,
                self._num_urls_found, self._num_new_urls_found,
                self._file_limit, self._gallery_paused, self._files_paused,
                serialisable_file_import_options,
                serialisable_tag_import_options, serialisable_gallery_seed_log,
                serialisable_file_seed_cache, self._no_work_until,
                self._no_work_until_reason)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (serialisable_gallery_import_key, self._creation_time, self._query,
         self._source_name, self._current_page_index, self._num_urls_found,
         self._num_new_urls_found, self._file_limit, self._gallery_paused,
         self._files_paused, serialisable_file_import_options,
         serialisable_tag_import_options, serialisable_gallery_seed_log,
         serialisable_file_seed_cache, self._no_work_until,
         self._no_work_until_reason) = serialisable_info

        self._gallery_import_key = bytes.fromhex(
    ...
    def _NetworkJobFactory(self, *args, **kwargs):
        ...
        network_job = ClientNetworkingJobs.NetworkJobDownloader(
            self._gallery_import_key, *args, **kwargs)
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (serialisable_gallery_import_key, self._creation_time, self._query,
             serialisable_gallery_identifier, self._current_page_index,
             self._num_urls_found, self._num_new_urls_found, self._file_limit,
             self._gallery_paused, self._files_paused,
             ...
             self._no_work_until,
             self._no_work_until_reason) = old_serialisable_info

            gallery_identifier = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_gallery_identifier)

            source_name = ClientDownloading.ConvertGalleryIdentifierToGUGName(
                gallery_identifier)

            new_serialisable_info = (
                serialisable_gallery_import_key, self._creation_time,
                self._query, source_name, self._current_page_index,
                self._num_urls_found, self._num_new_urls_found,
                self._file_limit, self._gallery_paused, self._files_paused,
                serialisable_file_import_options,
                serialisable_tag_import_options, serialisable_gallery_seed_log,
    ...
    def _WorkOnFiles(self):
        ...
        try:
            ...
            with self._lock:
                ...
                should_present = self._publish_to_page and file_seed.ShouldPresent(
    ...
    def GetCurrentAction(self):
        ...
        with self._lock:
            ...
            elif not self._gallery_seed_log.WorkToDo(
            ) and not self._file_seed_cache.WorkToDo():
    ...
    def GetStatus(self):
        ...
        with self._lock:
            ...
            return (gallery_status, file_status, self._files_paused,
    ...
    def Start(self, page_key, publish_to_page):
        ...
        self._files_repeating_job = HG.client_controller.CallRepeating(
            ClientImporting.GetRepeatingJobInitialDelay(),
            ClientImporting.REPEATING_JOB_TYPICAL_PERIOD,
            self.REPEATINGWorkOnFiles)
        self._gallery_repeating_job = HG.client_controller.CallRepeating(
            ClientImporting.GetRepeatingJobInitialDelay(),
            ClientImporting.REPEATING_JOB_TYPICAL_PERIOD,
            self.REPEATINGWorkOnGallery)
    ...
    def REPEATINGWorkOnGallery(self):
        ...
        with self._lock:
            ...
            gallery_paused = self._gallery_paused or HG.client_controller.new_options.GetBoolean(
                'pause_all_gallery_searches')

            work_pending = self._gallery_seed_log.WorkToDo(
            ) and not gallery_paused
            no_delays = HydrusData.TimeHasPassed(self._no_work_until)
            page_shown = not HG.client_controller.PageClosedButNotDestroyed(
                self._page_key)
            network_engine_good = not HG.client_controller.network_engine.IsBusy(
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_GALLERY_IMPORT] = GalleryImport
...
class MultipleGalleryImport(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        pend_options = (self._start_file_queues_paused,
                        self._start_gallery_queues_paused,
                        self._merge_simultaneous_pends_to_one_importer)
        ...
        return (serialisable_gug_key_and_name,
                serialisable_highlighted_gallery_import_key, self._file_limit,
                pend_options, serialisable_file_import_options,
                serialisable_tag_import_options, serialisable_gallery_imports)
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (serialisable_gug_key_and_name,
         ...
         pend_options, serialisable_file_import_options,
         serialisable_tag_import_options,
         serialisable_gallery_imports) = serialisable_info
        ...
        if serialisable_highlighted_gallery_import_key is None:
            ...
            self._highlighted_gallery_import_key = bytes.fromhex(
                serialisable_highlighted_gallery_import_key)

        (self._start_file_queues_paused, self._start_gallery_queues_paused,
         self._merge_simultaneous_pends_to_one_importer) = pend_options
        ...
        self._gallery_imports = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_gallery_imports)

        self._gallery_import_keys_to_gallery_imports = {
            gallery_import.GetGalleryImportKey(): gallery_import
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (serialisable_gallery_identifier,
             ...
             serialisable_current_query_stuff, pending_queries,
             get_tags_if_url_recognised_and_file_redundant, file_limit,
             gallery_paused, files_paused, serialisable_file_import_options,
             serialisable_tag_import_options,
             serialisable_file_seed_cache) = old_serialisable_info

            new_serialisable_info = (serialisable_gallery_identifier,
                                     serialisable_gallery_stream_identifiers,
                                     serialisable_current_query_stuff,
                                     pending_queries, file_limit,
                                     gallery_paused, files_paused,
                                     serialisable_file_import_options,
                                     serialisable_tag_import_options,
                                     serialisable_file_seed_cache)
        ...
        if version == 2:
            ...
            (serialisable_gallery_identifier,
             serialisable_gallery_stream_identifiers,
             serialisable_current_query_stuff, pending_queries, file_limit,
             gallery_paused, files_paused, serialisable_file_import_options,
             serialisable_tag_import_options,
             serialisable_file_seed_cache) = old_serialisable_info
        ...
        if version == 3:
            ...
            (current_query, current_query_num_new_urls,
             serialisable_current_gallery_stream_identifier,
             current_gallery_stream_identifier_page_index,
             serialisable_current_gallery_stream_identifier_found_urls,
             serialisable_pending_gallery_stream_identifiers
             ) = serialisable_current_query_stuff
            ...
            file_import_options = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_file_import_options)
            tag_import_options = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_tag_import_options)

            gallery_seed_log = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_gallery_seed_log)
            ...
            if len(file_seed_cache) > 0:
                ...
                gallery_import = GalleryImport(
                    query=current_query,
                    source_name='updated from old system',
                    initial_search_urls=[])
            ...
            new_serialisable_info = (
                serialisable_gallery_identifier,
                serialisable_highlighted_gallery_import_key, file_limit,
                serialisable_file_import_options,
                serialisable_tag_import_options, serialisable_gallery_imports)
        ...
        if version == 4:
            ...
            (serialisable_gallery_identifier,
             serialisable_highlighted_gallery_import_key, file_limit,
             serialisable_file_import_options, serialisable_tag_import_options,
             serialisable_gallery_imports) = old_serialisable_info
            ...
            serialisable_gug_key_and_name = (HydrusData.GenerateKey().hex(),
                                             gug_name)
        ...
        if version == 6:
            ...
            pend_options = (start_file_queues_paused,
    ...
    def CurrentlyWorking(self):
        ...
        with self._lock:
            ...
            return True in (gallery_import.CurrentlyWorking()
                            for gallery_import in self._gallery_imports)
    ...
    def GetAPIInfoDict(self, simple):
        ...
        with self._lock:
            ...
            d['gallery_imports'] = [
                gallery_import.GetAPIInfoDict(simple)
                for gallery_import in self._gallery_imports
            ]
        ...
        with self._lock:
            ...
            if self._highlighted_gallery_import_key is not None:
                ...
                if self._highlighted_gallery_import_key in self._gallery_import_keys_to_gallery_imports:
                    ...
                    return self._gallery_import_keys_to_gallery_imports[
                        self._highlighted_gallery_import_key]
    ...
    def GetInitialSearchText(self):
        ...
        return HG.client_controller.network_engine.domain_manager.GetInitialSearchText(
    ...
    def GetNumSeeds(self):
        ...
        with self._lock:
            ...
            return sum((gallery_import.GetNumSeeds()
                        for gallery_import in self._gallery_imports))
        ...
        with self._lock:
            ...
            for query_text in query_texts:
                ...
                if len(initial_search_urls) == 0:
                    ...
                    HydrusData.ShowText('The Gallery URL Generator "' +
                                        self._gug_key_and_name[1] +
                                        '" did not produce any URLs!')

                    return created_importers

                groups_of_query_data.append((query_text, initial_search_urls))

            if self._merge_simultaneous_pends_to_one_importer and len(
                    groups_of_query_data) > 1:
                ...
                query_text = HydrusData.ToHumanInt(
                    len(groups_of_query_data)) + ' queries'
            ...
            for (query_text, initial_search_urls) in groups_of_query_data:
                ...
                gallery_import = GalleryImport(
                    ...
                    initial_search_urls=initial_search_urls,
                    start_file_queue_paused=self._start_file_queues_paused,
                    start_gallery_queue_paused=self.
                    _start_gallery_queues_paused)
    ...
    def SetQueueStartSettings(self, start_file_queues_paused,
                              start_gallery_queues_paused,
                              merge_simultaneous_pends_to_one_importer):
    ...
    def Start(self, page_key):
        ...
        self._importers_repeating_job = HG.client_controller.CallRepeating(
            ClientImporting.GetRepeatingJobInitialDelay(), 2.0,
            self.REPEATINGWorkOnImporters)

        for gallery_import in self._gallery_imports:

            publish_to_page = gallery_import.GetGalleryImportKey(
            ) == self._highlighted_gallery_import_key
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_MULTIPLE_GALLERY_IMPORT] = MultipleGalleryImport
