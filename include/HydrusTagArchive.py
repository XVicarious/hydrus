...
class HydrusTagArchive(object):
    def __init__(self, path):
        ...
        self._namespaces = {
            namespace
            for (namespace,
                 ) in self._c.execute('SELECT namespace FROM namespaces;')
        }
    ...
    def GetTags(self, hash):
        ...
        result = {
            tag
            for (tag, ) in self._c.execute(
                'SELECT DISTINCT tag FROM mappings NATURAL JOIN tags WHERE hash_id = ?;',
                (hash_id, ))
        }
    ...
    def IterateMappings(self):
        ...
        hash_ids = [
            hash_id
            for (hash_id, ) in self._c.execute('SELECT hash_id FROM hashes;')
        ]
...
class HydrusTagPairArchive(object):
    ...
    def _InitDB(self):
        ...
        self._c.execute(
            'CREATE TABLE pairs ( tag_id_1 INTEGER, tag_id_2 INTEGER, PRIMARY KEY ( tag_id_1, tag_id_2 ) );'
        )
    ...
    def AddPairs(self, pairs):
        ...
        if self._pair_type is None:
            ...
            raise Exception(
                'Please set the pair type first before you start populating the database!'
            )

        pair_id_inserts = [(self._GetTagId(tag_1), self._GetTagId(tag_2))
                           for (tag_1, tag_2) in pairs]

        self._c.executemany(
            'INSERT OR IGNORE INTO pairs ( tag_id_1, tag_id_2 ) VALUES ( ?, ? );',
            pair_id_inserts)
    ...
    def DeletePairs(self, pairs):
        ...
        pair_id_deletees = [(self._GetTagId(tag_1), self._GetTagId(tag_2))
                            for (tag_1, tag_2) in pairs]

        self._c.executemany(
            'DELETE FROM pairs WHERE tag_id_1 = ? AND tag_id_2 = ?;',
            pair_id_deletees)
    ...
    def HasPair(self, tag_1, tag_2):
        ...
        result = self._c.execute(
            'SELECT 1 FROM pairs WHERE tag_id_1 = ? AND tag_id_2 = ?;',
            (tag_id_1, tag_id_2)).fetchone()
    ...
    def SetPairType(self, pair_type):
        ...
        self._c.execute('INSERT INTO pair_type ( pair_type ) VALUES ( ? );',
                        (pair_type, ))
