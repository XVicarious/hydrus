...
def GenerateStatusesToCountsStatus(statuses_to_counts):
    ...
    num_successful_but_redundant = statuses_to_counts[
        ...
        if num_successful_and_new > 0:
            ...
            if num_successful_but_redundant > 0:
                ...
                s += ' (' + HydrusData.ToHumanInt(
                    num_successful_but_redundant) + ' already in db)'
    ...
    if num_deleted > 0:
        ...
        status_strings.append(
            HydrusData.ToHumanInt(num_deleted) + ' previously deleted')
    ...
    if total > 0:
        ...
        if num_unknown > 0:
            ...
            simple_status += HydrusData.ConvertValueRangeToPrettyString(
        ...
        show_new_on_file_seed_short_summary = HG.client_controller.new_options.GetBoolean(
            'show_new_on_file_seed_short_summary')

        if show_new_on_file_seed_short_summary and num_successful_and_new:

            simple_status += ' - ' + HydrusData.ToHumanInt(
                num_successful_and_new) + 'N'

        simple_status_strings = []

        if num_ignored > 0:

            simple_status_strings.append(
                HydrusData.ToHumanInt(num_ignored) + 'Ig')

        show_deleted_on_file_seed_short_summary = HG.client_controller.new_options.GetBoolean(
            'show_deleted_on_file_seed_short_summary')

        if show_deleted_on_file_seed_short_summary and num_deleted > 0:

            simple_status_strings.append(
                HydrusData.ToHumanInt(num_deleted) + 'D')

        if num_failed > 0:

            simple_status_strings.append(
                HydrusData.ToHumanInt(num_failed) + 'F')

        if num_skipped > 0:

            simple_status_strings.append(
                HydrusData.ToHumanInt(num_skipped) + 'S')
...
class FileImportJob(object):
    def __init__(self, temp_path, file_import_options=None):
        ...
        if HG.file_import_report_mode:
            ...
            HydrusData.ShowText(
                'File import job created for path {}.'.format(temp_path))
    ...
    def CheckIsGoodToImport(self):
        ...
        if HG.file_import_report_mode:
            ...
            HydrusData.ShowText(
                'File import job testing if good to import for file import options'
            )
    ...
    def DoWork(self):
        ...
        if self.IsNewToDB():
            ...
            (import_status, note) = HG.client_controller.WriteSynchronous(
                'import_file', self)
    ...
    def GenerateHashAndStatus(self):
        ...
        if HG.file_import_report_mode:
            ...
            HydrusData.ShowText('File import job hash: {}'.format(
                self._hash.hex()))

        (self._pre_import_status, hash,
         note) = HG.client_controller.Read('hash_status',
                                           'sha256',
                                           self._hash,
                                           prefix='file recognised')

        if HG.file_import_report_mode:

            HydrusData.ShowText(
                'File import job pre-import status: {}, {}'.format(
                    CC.status_string_lookup[self._pre_import_status], note))
    ...
    def GenerateInfo(self):
        ...
        if mime in HC.MIMES_WE_CAN_PHASH:
            ...
            self._phashes = ClientImageHandling.GenerateShapePerceptualHashes(
                self._temp_path, mime)

            if HG.file_import_report_mode:

                HydrusData.ShowText(
                    'File import job generated {} phashes: {}'.format(
                        len(self._phashes),
                        [phash.hex() for phash in self._phashes]))
...
class FileSeed(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_hashes = [(hash_type, hash.hex())
                               for (hash_type,
                                    hash) in list(self._hashes.items())
                               if hash is not None]

        return (self.file_seed_type, self.file_seed_data, self.created,
                self.modified, self.source_time, self.status, self.note,
                self._referral_url, serialisable_fixed_service_keys_to_tags,
                serialisable_urls, serialisable_tags, serialisable_hashes)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self.file_seed_type, self.file_seed_data, self.created, self.modified,
         self.source_time, self.status, self.note, self._referral_url,
         serialisable_fixed_service_keys_to_tags, serialisable_urls,
         serialisable_tags, serialisable_hashes) = serialisable_info

        self._fixed_service_keys_to_tags = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_fixed_service_keys_to_tags)

        self._urls = set(serialisable_urls)
        self._tags = set(serialisable_tags)
        self._hashes = {
            hash_type: bytes.fromhex(encoded_hash)
            for (hash_type, encoded_hash) in serialisable_hashes
    ...
    def _NormaliseAndFilterAssociableURLs(self, urls):
        ...
        associable_urls = {
            url
            for url in normalised_urls if HG.client_controller.network_engine.
            domain_manager.ShouldAssociateURLWithFiles(url)
        }
    ...
    def _SetupTagImportOptions(self, given_tag_import_options):
        ...
        if given_tag_import_options.IsDefault():
            ...
            tag_import_options = HG.client_controller.network_engine.domain_manager.GetDefaultTagImportOptionsForURL(
                tio_lookup_url)
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (file_seed_type, file_seed_data, created, modified, source_time,
             status, note, serialisable_urls, serialisable_tags,
             serialisable_hashes) = old_serialisable_info

            referral_url = None

            new_serialisable_info = (file_seed_type, file_seed_data, created,
                                     modified, source_time, status, note,
                                     referral_url, serialisable_urls,
                                     serialisable_tags, serialisable_hashes)
        ...
        if version == 2:
            ...
            (file_seed_type, file_seed_data, created, modified, source_time,
             status, note, referral_url, serialisable_urls, serialisable_tags,
             serialisable_hashes) = old_serialisable_info
            ...
            new_serialisable_info = (file_seed_type, file_seed_data, created,
                                     ...
                                     referral_url,
                                     serialisable_fixed_service_keys_to_tags,
                                     serialisable_urls, serialisable_tags,
                                     serialisable_hashes)
    ...
    def AddParseResults(self, parse_results, file_import_options):
        ...
        for (hash_type,
             hash) in ClientParsing.GetHashesFromParseResults(parse_results):
        ...
        if file_import_options.ShouldAssociateSourceURLs():
            ...
            source_urls = ClientParsing.GetURLsFromParseResults(
                parse_results, (HC.URL_TYPE_SOURCE, ))

            associable_urls = self._NormaliseAndFilterAssociableURLs(
                source_urls)
        ...
        source_timestamp = ClientParsing.GetTimestampFromParseResults(
            parse_results, HC.TIMESTAMP_TYPE_SOURCE)
    ...
    def DownloadAndImportRawFile(self,
                                 file_url,
                                 file_import_options,
                                 network_job_factory,
        ...
        try:
            ...
            network_job = network_job_factory('GET',
                                              file_url,
                                              temp_path=temp_path,
    ...
    def GetExampleNetworkJob(self, network_job_factory):
        ...
        if self.IsAPostURL():
            ...
            (
                url_to_check, parser
            ) = HG.client_controller.network_engine.domain_manager.GetURLToFetchAndParser(
                post_url)
    ...
    def GetPreImportStatusPredictionURL(self,
                                        file_import_options,
                                        file_url=None):
        ...
        for url in urls:
            ...
            if HG.client_controller.network_engine.domain_manager.URLCanReferToMultipleFiles(
                    url):
            ...
            if HG.client_controller.network_engine.domain_manager.URLDefinitelyRefersToOneFile(
                    url) or url in (self.file_seed_data, file_url):

                results = HG.client_controller.Read('url_statuses', url)

                if len(results
                       ) == 0:  # if no match found, no useful data discovered

                    continue

                elif len(
                        results
                ) > 1:  # if more than one file claims this url, it cannot be relied on to guess the file
                    ...
                    if status != CC.STATUS_UNKNOWN:
                        ...
                        if self.file_seed_type == FILE_SEED_TYPE_URL:
                            ...
                            if url != my_url:
                                ...
                                my_url_class = HG.client_controller.network_engine.domain_manager.GetURLClass(
                                    my_url)

                                (media_result, ) = HG.client_controller.Read(
                                    'media_results', (hash, ))

                                this_files_urls = media_result.GetLocationsManager(
                                ).GetURLs()
                                ...
                                for this_files_url in this_files_urls:
                                    ...
                                    if this_files_url != my_url:
                                        ...
                                        this_url_class = HG.client_controller.network_engine.domain_manager.GetURLClass(
                                            this_files_url)
    ...
    def GetSearchFileSeeds(self):
        ...
        if self.file_seed_type == FILE_SEED_TYPE_URL:
            ...
            search_urls = ClientNetworkingDomain.GetSearchURLs(
                self.file_seed_data)

            search_file_seeds = [
                FileSeed(FILE_SEED_TYPE_URL, search_url)
        ...
        try:
            ...
            if self.file_seed_type != FILE_SEED_TYPE_HDD:
                ...
                raise HydrusExceptions.VetoException(
                    'Attempted to import as a path, but I do not think I am a path!'
                )
            ...
            try:
                ...
                if not copied:
                    ...
                    raise Exception(
                        'File failed to copy to temp path--see log for error.')
    ...
    def Normalise(self):
        ...
        if self.file_seed_type == FILE_SEED_TYPE_URL:
            ...
            self.file_seed_data = HG.client_controller.network_engine.domain_manager.NormaliseURL(
                self.file_seed_data)

    def PredictPreImportStatus(self,
                               file_import_options,
                               tag_import_options,
                               file_url=None):

        (url_status, url_hash,
         url_note) = self.GetPreImportStatusPredictionURL(file_import_options,
                                                          file_url=file_url)
        (hash_status, hash_hash, hash_note
         ) = self.GetPreImportStatusPredictionHash(file_import_options)
    ...
    def PresentToPage(self, page_key):
        ...
        if hash is not None:
            ...
            HG.client_controller.pub('add_media_results', page_key,
                                     (media_result, ))
    ...
    def ShouldPresent(self, file_import_options, in_inbox=None):
        ...
        if hash is not None and self.status in CC.SUCCESSFUL_IMPORT_STATES:
            ...
            if in_inbox is None:
                ...
                if file_import_options.ShouldPresentIgnorantOfInbox(
                        self.status):

                    return True

                if file_import_options.ShouldNotPresentIgnorantOfInbox(
                        self.status):
    ...
    def WorkOnURL(self, file_seed_cache, status_hook, network_job_factory,
                  network_job_presentation_context_factory,
        ...
        try:
            ...
            if url_type not in (HC.URL_TYPE_POST, HC.URL_TYPE_FILE,
                                HC.URL_TYPE_UNKNOWN):
            ...
            if url_type == HC.URL_TYPE_POST and not can_parse:
                ...
                raise HydrusExceptions.VetoException(
                    'Did not have a parser for this URL!')

            tag_import_options = self._SetupTagImportOptions(
                tag_import_options)

            status_hook('checking url status')

            (should_download_metadata,
             should_download_file) = self.PredictPreImportStatus(
                 file_import_options, tag_import_options)
            ...
            if self.IsAPostURL():
                ...
                if should_download_metadata:
                    ...
                    network_job = network_job_factory(
                        'GET', url_to_check, referral_url=referral_url)
                    ...
                    all_parse_results = parser.Parse(parsing_context,
                                                     parsing_text)

                    if len(all_parse_results) == 0:

                        raise HydrusExceptions.VetoException(
                            'The parser found nothing in the document!')

                    elif len(all_parse_results) > 1:

                        # multiple child urls generated by a subsidiary page parser

                        file_seeds = ClientImporting.ConvertAllParseResultsToFileSeeds(
                            all_parse_results, self.file_seed_data,
                            file_import_options)
                        ...
                        num_urls_added = file_seed_cache.InsertFileSeeds(
                            insertion_index, file_seeds)

                        status = CC.STATUS_SUCCESSFUL_AND_NEW
                        note = 'Found ' + HydrusData.ToHumanInt(
                            num_urls_added) + ' new URLs.'
                        ...
                        desired_urls = ClientParsing.GetURLsFromParseResults(
                            parse_results, (HC.URL_TYPE_DESIRED, ),
                            only_get_top_priority=True)
                        ...
                        if len(desired_urls) == 0:
                            ...
                            if url_type in (HC.URL_TYPE_FILE,
                                            HC.URL_TYPE_UNKNOWN):

                                file_url = desired_url

                                (should_download_metadata, should_download_file
                                 ) = self.PredictPreImportStatus(
                                     file_import_options, tag_import_options,
                                     file_url)

                                if should_download_file:

                                    self.DownloadAndImportRawFile(
                                        file_url,
                                        file_import_options,
                                        network_job_factory,
                                        network_job_presentation_context_factory,
                                        status_hook,
                                        override_bandwidth=True)
                                ...
                                raise HydrusExceptions.VetoException(
                                    'Found a URL--' + desired_url +
                                    '--but could not understand/parse it!')
                        ...
                        if len(child_urls) > 0:
                            ...
                            for child_url in child_urls:
                                ...
                                duplicate_file_seed = self.Duplicate(
                                )  # inherits all urls and tags from here

                                duplicate_file_seed.file_seed_data = child_url

                                duplicate_file_seed.SetReferralURL(
                                    self.file_seed_data)

                                if self._referral_url is not None:

                                    duplicate_file_seed.AddURL(
                                        self._referral_url)
                            ...
                            try:
                                ...
                                my_index = file_seed_cache.GetFileSeedIndex(
                                    self)
                            ...
                            num_urls_added = file_seed_cache.InsertFileSeeds(
                                insertion_index, child_file_seeds)
                ...
                if should_download_file:
                    ...
                    self.DownloadAndImportRawFile(
                        file_url, file_import_options, network_job_factory,
                        network_job_presentation_context_factory, status_hook)

            did_substantial_work |= self.WriteContentUpdates(
                tag_import_options)
    ...
    def WriteContentUpdates(self, tag_import_options=None):
        ...
        if tag_import_options is not None:
            ...
            for (service_key, content_updates
                 ) in tag_import_options.GetServiceKeysToContentUpdates(
                     self.status, in_inbox, hash, set(self._tags)).items():
        ...
        ) in ClientData.ConvertServiceKeysToTagsToServiceKeysToContentUpdates(
            (hash, ), self._fixed_service_keys_to_tags).items():
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_FILE_SEED] = FileSeed
...
class FileSeedCache(HydrusSerialisable.SerialisableBase):
    ...
    def _GenerateStatus(self):
        ...
        self._status_cache = GenerateStatusesToCountsStatus(
            self._GetStatusesToCounts())
    ...
    def _GetFileSeeds(self, status=None):
        ...
        if status is None:
            ...
            return [
                file_seed for file_seed in self._file_seeds
    ...
    def _HasFileSeed(self, file_seed):
        ...
        has_file_seed = True in (
            search_file_seed in self._file_seeds_to_indices
            for search_file_seed in search_file_seeds)
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        with self._lock:
            ...
            self._file_seeds = HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_info)

            self._file_seeds_to_indices = {
                file_seed: index
                for (index, file_seed) in enumerate(self._file_seeds)
            }
            ...
            for (file_seed, file_seed_info) in old_serialisable_info:
                ...
                try:
                    ...
                    if 'media.tumblr.com' in parse.netloc:
                        ...
                        file_seed = ClientNetworkingDomain.ConvertHTTPToHTTPS(
                            file_seed)
            ...
            for (file_seed, file_seed_info) in old_serialisable_info:
                ...
                try:
                    ...
                    if magic_phrase in file_seed:
                        ...
                        file_seed = file_seed.replace(magic_phrase,
                                                      replacement)
    ...
    def AddFileSeeds(self, file_seeds):
        ...
        with self._lock:
            ...
            for file_seed in file_seeds:
                ...
                self._file_seeds_to_indices[file_seed] = len(
                    self._file_seeds) - 1
    ...
    def CanCompact(self, compact_before_this_source_time):
        ...
        with self._lock:
            ...
            for file_seed in self._file_seeds[:-self.COMPACT_NUMBER]:
                ...
                if self._GetSourceTimestamp(
                        file_seed) < compact_before_this_source_time:
    ...
    def Compact(self, compact_before_this_source_time):
        ...
        with self._lock:
            ...
            for file_seed in self._file_seeds[:-self.COMPACT_NUMBER]:
                ...
                still_relevant = self._GetSourceTimestamp(
                    file_seed) > compact_before_this_source_time
    ...
    def GetAPIInfoDict(self, simple):
        ...
        with self._lock:
            ...
            (status, simple_status, (total_processed,
                                     total)) = self._status_cache
            ...
            if not simple:
                ...
                d['import_items'] = [
                    file_seed.GetAPIInfoDict(simple)
                    for file_seed in self._file_seeds
                ]
    ...
    def GetEarliestSourceTime(self):
        ...
        with self._lock:
            ...
            earliest_timestamp = min((self._GetSourceTimestamp(file_seed)
                                      for file_seed in self._file_seeds))
    ...
    def GetHashes(self):
        ...
        with self._lock:
            ...
            hashes = [
                file_seed.GetHash() for file_seed in self._file_seeds
    ...
    def GetLatestAddedTime(self):
        ...
        with self._lock:
            ...
            latest_timestamp = max(
                (file_seed.created for file_seed in self._file_seeds))
    ...
    def GetLatestSourceTime(self):
        ...
        with self._lock:
            ...
            latest_timestamp = max((self._GetSourceTimestamp(file_seed)
                                    for file_seed in self._file_seeds))
    ...
    def GetPresentedHashes(self, file_import_options):
        ...
        with self._lock:
            ...
            file_seed_hashes = [
                file_seed.GetHash() for file_seed in eligible_file_seeds
            ]

            inbox_hashes = HG.client_controller.Read('in_inbox',
                                                     file_seed_hashes)
            ...
            for file_seed in eligible_file_seeds:
                ...
                if file_seed.ShouldPresent(file_import_options,
                                           in_inbox=in_inbox):
    ...
    def NotifyFileSeedsUpdated(self, file_seeds):
        ...
        HG.client_controller.pub('file_seed_cache_file_seeds_updated',
                                 self._file_seed_cache_key, file_seeds)
    ...
    def RemoveFileSeeds(self, file_seeds):
        ...
        with self._lock:
            ...
            self._file_seeds = HydrusSerialisable.SerialisableList([
                file_seed for file_seed in self._file_seeds
                if file_seed not in file_seeds_to_delete
            ])
    ...
    def RemoveFileSeedsByStatus(self, statuses_to_remove):
        ...
        with self._lock:
            ...
            file_seeds_to_delete = [
                file_seed for file_seed in self._file_seeds
                if file_seed.status in statuses_to_remove
            ]
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_FILE_SEED_CACHE] = FileSeedCache
