...
class TestServerDB(unittest.TestCase):
    def _read(self, action, *args, **kwargs):
        return TestServerDB._db.Read(action, *args, **kwargs)

    def _write(self, action, *args, **kwargs):
        return TestServerDB._db.Write(action, True, *args, **kwargs)

    @classmethod
    def setUpClass(cls):

        cls._db = ServerDB.DB(HG.test_controller, TestController.DB_DIR,
                              'server')
    ...
    def _test_account_creation(self):
        ...
        user_at = HydrusData.AccountType('user', [HC.GET_DATA, HC.POST_DATA],
                                         (50000, 500))
        ...
        user_at_diff = HydrusData.AccountType('user different', [HC.GET_DATA],
                                              (40000, None))
        ...
        self.assertRaises(HydrusExceptions.InsufficientCredentialsException,
                          self._read, 'account_key_from_access_key',
                          self._tag_service_key, access_key)

        account_key = self._read('account_key_from_access_key',
                                 self._tag_service_key, access_key_2)

        self.assertRaises(HydrusExceptions.InsufficientCredentialsException,
                          self._read, 'access_key', r_key)
    ...
    def _test_init_server_admin(self):
        ...
        result = self._read('account_key_from_access_key', HC.SERVER_ADMIN_KEY,
                            self._admin_access_key)
    ...
    def _test_service_creation(self):
        ...
        t_options = {
            ...
            'message': 'tag repo message',
            'port': 100,
            'upnp': None
        }
        f_options = {
            'max_monthly_data': None,
            'message': 'file repo message',
            'port': 101,
            'upnp': None
        }

        edit_log.append(
            (HC.ADD, (self._tag_service_key, HC.TAG_REPOSITORY, t_options)))
        edit_log.append(
            (HC.ADD, (self._file_service_key, HC.FILE_REPOSITORY, f_options)))
