...
ALLOWED_PERMISSIONS = (CLIENT_API_PERMISSION_ADD_FILES,
                       ...
                       CLIENT_API_PERMISSION_MANAGE_COOKIES)

basic_permission_to_str_lookup = {}

basic_permission_to_str_lookup[
    CLIENT_API_PERMISSION_ADD_URLS] = 'add urls for processing'
basic_permission_to_str_lookup[
    CLIENT_API_PERMISSION_ADD_FILES] = 'import files'
basic_permission_to_str_lookup[
    CLIENT_API_PERMISSION_ADD_TAGS] = 'add tags to files'
basic_permission_to_str_lookup[
    CLIENT_API_PERMISSION_SEARCH_FILES] = 'search for files'
basic_permission_to_str_lookup[
    CLIENT_API_PERMISSION_MANAGE_PAGES] = 'manage pages'
basic_permission_to_str_lookup[
    CLIENT_API_PERMISSION_MANAGE_COOKIES] = 'manage cookies'
...
class APIManager(HydrusSerialisable.SerialisableBase):
    ...
    def __init__(self):
        ...
        HG.client_controller.sub(self, 'MaintainMemory',
                                 'memory_maintenance_pulse')

    def _GetSerialisableInfo(self):

        serialisable_api_permissions_objects = [
            api_permissions.GetSerialisableTuple()
    ...
    def GenerateSessionKey(self, access_key):
        ...
        with self._lock:
            ...
            self._session_keys_to_access_keys_and_expirys[session_key] = (
                access_key, HydrusData.GetNow() + SESSION_EXPIRY)
    ...
    def GetAccessKey(self, session_key):
        ...
        with self._lock:
            ...
            (access_key, session_expiry
             ) = self._session_keys_to_access_keys_and_expirys[session_key]

            if HydrusData.TimeHasPassed(session_expiry):

                del self._session_keys_to_access_keys_and_expirys[
                    session_expiry]

                raise HydrusExceptions.SessionException(
                    'That session key has expired!')
    ...
    def GetPermissions(self, access_key):
        ...
        with self._lock:
            ...
            if access_key not in self._access_keys_to_permissions:
                ...
                raise HydrusExceptions.DataMissing(
                    'Did not find an entry for that access key!')
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_CLIENT_API_MANAGER] = APIManager
...
class APIPermissions(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (serialisable_access_key, serialisable_basic_permissions,
                serialisable_search_tag_filter)
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (serialisable_access_key, serialisable_basic_permissions,
         serialisable_search_tag_filter) = serialisable_info

        self._access_key = bytes.fromhex(serialisable_access_key)

        self._basic_permissions = set(serialisable_basic_permissions)
        self._search_tag_filter = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_search_tag_filter)
    ...
    def CheckCanSeeAllFiles(self):
        ...
        with self._lock:
            ...
            if not (self._HasPermission(CLIENT_API_PERMISSION_SEARCH_FILES)
                    and self._search_tag_filter.AllowsEverything()):
    ...
    def CheckPermissionToSeeFiles(self, hash_ids):
        ...
        with self._lock:
            ...
            num_files_allowed_to_see = len(
                self._last_search_results.intersection(hash_ids))
            ...
            if num_files_allowed_to_see != num_files_asked_for:
                ...
                error_text = error_text.format(
                    HydrusData.ToHumanInt(num_files_asked_for),
                    HydrusData.ToHumanInt(num_files_allowed_to_see))

                raise HydrusExceptions.InsufficientCredentialsException(
                    error_text)

            self._search_results_timeout = HydrusData.GetNow(
            ) + SEARCH_RESULTS_CACHE_TIMEOUT
    ...
    def GetAdvancedPermissionsString(self):
        ...
        with self._lock:
            ...
            if self._HasPermission(CLIENT_API_PERMISSION_SEARCH_FILES):
                ...
                p_strings.append('Can search: ' +
                                 self._search_tag_filter.ToPermittedString())
    ...
    def GetBasicPermissionsString(self):
        ...
        with self._lock:
            ...
            l = [
                basic_permission_to_str_lookup[p]
                for p in self._basic_permissions
            ]
    ...
    def MaintainMemory(self):
        ...
        with self._lock:
            ...
            if self._last_search_results is not None and HydrusData.TimeHasPassed(
                    self._search_results_timeout):
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_CLIENT_API_PERMISSIONS] = APIPermissions
