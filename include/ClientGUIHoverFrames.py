...
class FullscreenHoverFrame(QW.QFrame):
    ...
    def _SizeAndPosition(self, force=False):
        ...
        if self.parentWidget().isVisible() or force:
            ...
            (should_resize, my_ideal_size,
             my_ideal_position) = self._GetIdealSizeAndPosition()
    ...
    def TIMERUIUpdate(self):
        ...
        focus_is_on_descendant = ClientGUIFunctions.IsQtAncestor(
            current_focus_tlw, self._my_canvas.window(), through_tlws=True)
        focus_has_right_window_type = isinstance(
            current_focus_tlw,
            (ClientGUICanvas.CanvasFrame, FullscreenHoverFrame))
        ...
        for tlw in QW.QApplication.topLevelWidgets():
            ...
            if tlw == self or ClientGUIFunctions.IsQtAncestor(
                    tlw, self, through_tlws=True):
        ...
        if self._current_media is None or not self._my_canvas.isVisible():
            ...
            for tlw in tlws:
                ...
                if isinstance(tlw, QW.QDialog) and not isinstance(
                        tlw, ClientGUICanvas.CanvasFrame) and tlw.isModal():
            ...
            ready_to_hide = not menu_open and not mouse_is_over_self_or_child and (
                not in_position or dialog_open or not hide_focus_is_good)
            ...
            def get_logic_report_string():
                ...
                tuples.append(
                    ('ideal winsize: ', (my_ideal_width, my_ideal_height)))
...
class FullscreenHoverFrameRightDuplicates(FullscreenHoverFrame):
    def __init__(self, parent, my_canvas, canvas_key):
        ...
        self._trash_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.delete, HG.client_controller.pub,
        ...
        close_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.stop, HG.client_controller.pub,
            'canvas_close', self._canvas_key)
        close_button.setToolTip('close filter')

        self._back_a_pair = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.first, HG.client_controller.pub,
            'canvas_application_command',
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'duplicate_filter_back'),
            self._canvas_key)
        self._back_a_pair.SetToolTipWithShortcuts('go back a pair',
                                                  'duplicate_filter_back')

        self._previous_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.previous, HG.client_controller.pub,
            'canvas_application_command',
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'view_previous'), self._canvas_key)
        self._previous_button.SetToolTipWithShortcuts('previous',
                                                      'view_previous')

        self._index_text = ClientGUICommon.BetterStaticText(self, 'index')

        self._next_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.next_bmp, HG.client_controller.pub,
            'canvas_application_command',
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'view_next'), self._canvas_key)
        self._next_button.SetToolTipWithShortcuts('next', 'view next')

        self._skip_a_pair = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.last, HG.client_controller.pub,
            'canvas_application_command',
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'duplicate_filter_skip'),
            self._canvas_key)
        self._skip_a_pair.SetToolTipWithShortcuts('show a different pair',
                                                  'duplicate_filter_skip')
        ...
        dupe_commands.append((
            'this is better, and delete the other',
            'Set that the current file you are looking at is better than the other in the pair, and set the other file to be deleted.',
            ClientData.ApplicationCommand(
                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                'duplicate_filter_this_is_better_and_delete_other')))
        dupe_commands.append((
            'this is better, but keep both',
            'Set that the current file you are looking at is better than the other in the pair, but keep both files.',
            ClientData.ApplicationCommand(
                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
        ...
        dupe_commands.append((
            ...
            'Set that the files are not duplicates or otherwise related--that this potential pair is a false positive match.',
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'duplicate_filter_false_positive')))
        dupe_commands.append((
            'custom action',
        ...
        for (panel_name, dupe_commands) in dupe_boxes:
            ...
            for (label, tooltip, command) in dupe_commands:
                ...
                command_button = ClientGUICommon.BetterButton(
                    button_panel, label, HG.client_controller.pub,
                    'canvas_application_command', command, self._canvas_key)

                command_button.SetToolTipWithShortcuts(tooltip,
                                                       command.GetData())

                button_panel.Add(command_button, CC.FLAGS_EXPAND_PERPENDICULAR)

            QP.AddToLayout(command_button_vbox, button_panel,
                           CC.FLAGS_EXPAND_PERPENDICULAR)

        self._comparison_statements_vbox = QP.VBoxLayout()

        self._comparison_statement_names = [
            'filesize', 'resolution', 'ratio', 'mime', 'num_tags',
            'time_imported', 'jpeg_quality', 'pixel_duplicates'
        ]
        ...
        for name in self._comparison_statement_names:
            ...
            QP.AddToLayout(self._comparison_statements_vbox, panel,
        ...
        QP.AddToLayout(navigation_button_hbox, self._back_a_pair,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(navigation_button_hbox, self._previous_button,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(navigation_button_hbox, (20, 20),
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(navigation_button_hbox, self._index_text,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(navigation_button_hbox, (20, 20),
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(navigation_button_hbox, self._next_button,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(navigation_button_hbox, self._skip_a_pair,
                       CC.FLAGS_VCENTER)

        vbox = QP.VBoxLayout()

        QP.AddToLayout(vbox, navigation_button_hbox,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, top_button_hbox, CC.FLAGS_BUTTON_SIZER)
        QP.AddToLayout(vbox, command_button_vbox,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, self._comparison_statements_vbox,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)

        self.setLayout(vbox)

        HG.client_controller.sub(self, 'SetDuplicatePair',
                                 'canvas_new_duplicate_pair')
    ...
    def _EditBackgroundSwitchIntensity(self):
        ...
        value = new_options.GetNoneableInteger(
            'duplicate_background_switch_intensity')

        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit lighten/darken intensity') as dlg:

            panel = ClientGUIScrolledPanelsEdit.EditNoneableIntegerPanel(
                dlg,
                value,
                message='intensity: ',
                none_phrase='do not change',
                min=1,
                max=9)
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                new_options.SetNoneableInteger(
                    'duplicate_background_switch_intensity', new_value)
    ...
    def _GetIdealSizeAndPosition(self):
        ...
        ideal_position = ClientGUIFunctions.ClientToScreen(
            parent_window,
            (int(parent_width - my_ideal_width), int(parent_height * 0.3)))

        return (should_resize, ideal_size, ideal_position.toTuple())

    def _ResetComparisonStatements(self):

        statements_and_scores = ClientMedia.GetDuplicateComparisonStatements(
            self._current_media, self._comparison_media)
...
class FullscreenHoverFrameTop(FullscreenHoverFrame):
    def __init__(self, parent, my_canvas, canvas_key):
        ...
        self._title_text = ClientGUICommon.BetterStaticText(self,
                                                            'title',
                                                            ellipsize_end=True)
        self._info_text = ClientGUICommon.BetterStaticText(self,
                                                           'info',
                                                           ellipsize_end=True)
        ...
        QP.AddToLayout(vbox, self._top_hbox,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
    ...
    def _Archive(self):
        ...
        if self._current_media.HasInbox():
            ...
            command = ClientData.ApplicationCommand(
                CC.APPLICATION_COMMAND_TYPE_SIMPLE, 'archive_file')

        else:

            command = ClientData.ApplicationCommand(
                CC.APPLICATION_COMMAND_TYPE_SIMPLE, 'inbox_file')

        HG.client_controller.pub('canvas_application_command', command,
                                 self._canvas_key)
    ...
    def _GetIdealSizeAndPosition(self):
        ...
        ideal_size = (my_ideal_width, my_ideal_height)
        ideal_position = ClientGUIFunctions.ClientToScreen(
            parent_window, (int(parent_width * 0.2), 0))
    ...
    def _PopulateCenterButtons(self):
        ...
        self._archive_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.archive, self._Archive)
        ...
        self._delete_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.trash_delete, HG.client_controller.pub,
            'canvas_delete', self._canvas_key)
        self._delete_button.setToolTip('delete completely')

        self._undelete_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.undelete, HG.client_controller.pub,
    ...
    def _PopulateRightButtons(self):
        ...
        zoom_in = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.zoom_in, HG.client_controller.pub,
            'canvas_application_command',
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'zoom_in'), self._canvas_key)
        zoom_in.SetToolTipWithShortcuts('zoom in', 'zoom_in')

        zoom_out = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.zoom_out, HG.client_controller.pub,
            'canvas_application_command',
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'zoom_out'), self._canvas_key)
        zoom_out.SetToolTipWithShortcuts('zoom out', 'zoom_out')

        zoom_switch = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.zoom_switch, HG.client_controller.pub,
            'canvas_application_command',
            ClientData.ApplicationCommand(
                CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                'switch_between_100_percent_and_canvas_zoom'),
            self._canvas_key)
        zoom_switch.SetToolTipWithShortcuts(
            'zoom switch', 'switch_between_100_percent_and_canvas_zoom')

        self._volume_control = ClientGUIMediaControls.VolumeControl(
            self, ClientGUICommon.CANVAS_MEDIA_VIEWER)

        if not ClientGUIMPV.MPV_IS_AVAILABLE:

            self._volume_control.hide()

        shortcuts = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.keyboard, self._ShowShortcutMenu)
        shortcuts.setToolTip('shortcuts')

        fullscreen_switch = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.fullscreen_switch, HG.client_controller.pub,
        ...
        open_externally = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.open_externally, HG.client_controller.pub,
            'canvas_application_command',
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'open_file_in_external_program'),
            self._canvas_key)
        open_externally.SetToolTipWithShortcuts(
            'open externally', 'open_file_in_external_program')
        ...
        close = ClientGUICommon.BetterBitmapButton(self, CC.GlobalPixmaps.stop,
    ...
    def _ResetArchiveButton(self):
        ...
        if self._current_media.HasInbox():
            ...
            ClientGUIFunctions.SetBitmapButtonBitmap(self._archive_button,
                                                     CC.GlobalPixmaps.archive)
            self._archive_button.setToolTip('archive')

        else:

            ClientGUIFunctions.SetBitmapButtonBitmap(self._archive_button,
                                                     CC.GlobalPixmaps.to_inbox)
    ...
    def _ResetButtons(self):
        ...
        if self._current_media is not None:
            ...
            current_locations = self._current_media.GetLocationsManager(
            ).GetCurrent()
    ...
    def _FlipActiveDefaultCustomShortcut(self, name):
        ...
        default_media_viewer_custom_shortcuts = list(
            new_options.GetStringList('default_media_viewer_custom_shortcuts'))
        ...
        new_options.SetStringList('default_media_viewer_custom_shortcuts',
                                  default_media_viewer_custom_shortcuts)

    def _ShowShortcutMenu(self):

        all_shortcut_names = HG.client_controller.Read(
            'serialisable_names',
            HydrusSerialisable.SERIALISABLE_TYPE_SHORTCUT_SET)

        custom_shortcuts_names = [
            name for name in all_shortcut_names
            if name not in CC.SHORTCUTS_RESERVED_NAMES
        ]
        ...
        if len(custom_shortcuts_names) > 0:
            ...
            default_media_viewer_custom_shortcuts = HG.client_controller.new_options.GetStringList(
                'default_media_viewer_custom_shortcuts')
            ...
            for name in custom_shortcuts_names:
                ...
                ClientGUIMenus.AppendMenuCheckItem(
                    current_menu, name, 'turn this shortcut set on/off',
                    name in my_canvas_active_custom_shortcuts,
                    self._my_canvas.FlipActiveCustomShortcutName, name)

            ClientGUIMenus.AppendMenu(menu, current_menu,
            ...
            for name in custom_shortcuts_names:
                ...
                ClientGUIMenus.AppendMenuCheckItem(
                    defaults_menu, name,
                    'turn this shortcut set on/off by default',
                    name in default_media_viewer_custom_shortcuts,
                    self._FlipActiveDefaultCustomShortcut, name)

            ClientGUIMenus.AppendMenu(menu, defaults_menu,
    ...
    def EventDragButton(self):
        ...
        result = ClientDragDrop.DoFileExportDragDrop(self, page_key, media,
                                                     alt_down)
        ...
        if result != QC.Qt.IgnoreAction:
            ...
            HG.client_controller.pub(
                'canvas_application_command',
                ClientData.ApplicationCommand(
                    CC.APPLICATION_COMMAND_TYPE_SIMPLE, 'pause_media'),
                self._canvas_key)
...
class FullscreenHoverFrameTopArchiveDeleteFilter(FullscreenHoverFrameTop):
    ...
    def _PopulateLeftButtons(self):
        ...
        self._back_button = ClientGUICommon.BetterBitmapButton(
            ...
            self._canvas_key)
        self._back_button.SetToolTipWithShortcuts(
            'back', 'archive_delete_filter_back')
        ...
        self._skip_button = ClientGUICommon.BetterBitmapButton(
            ...
            self._canvas_key)
        self._skip_button.SetToolTipWithShortcuts(
            'skip', 'archive_delete_filter_skip')
    ...
    def _PopulateLeftButtons(self):
        ...
        self._first_button = ClientGUICommon.BetterBitmapButton(
            ...
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'duplicate_filter_back'),
            self._canvas_key)
        self._first_button.SetToolTipWithShortcuts('go back a pair',
                                                   'duplicate_filter_back')
        ...
        self._last_button = ClientGUICommon.BetterBitmapButton(
            ...
            ClientData.ApplicationCommand(CC.APPLICATION_COMMAND_TYPE_SIMPLE,
                                          'duplicate_filter_skip'),
            self._canvas_key)
        self._last_button.SetToolTipWithShortcuts('show a different pair',
                                                  'duplicate_filter_skip')
...
class FullscreenHoverFrameTopRight(FullscreenHoverFrame):
    def __init__(self, parent, my_canvas, canvas_key):
        ...
        self._trash_icon = ClientGUICommon.BufferedWindowIcon(
            self._icon_panel, CC.GlobalPixmaps.trash)
        self._inbox_icon = ClientGUICommon.BufferedWindowIcon(
            self._icon_panel, CC.GlobalPixmaps.inbox)
        ...
        self._file_repos = QP.MakeQLabelWithAlignment(
            '', self, QC.Qt.AlignRight | QC.Qt.AlignVCenter)
        ...
        QP.AddToLayout(vbox, self._icon_panel,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        QP.AddToLayout(vbox, self._file_repos, CC.FLAGS_EXPAND_PERPENDICULAR)
        QP.AddToLayout(vbox, self._urls_vbox,
                       CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
    ...
    def _GetIdealSizeAndPosition(self):
        ...
        ideal_size = (my_ideal_width, my_ideal_height)
        ideal_position = ClientGUIFunctions.ClientToScreen(
            parent_window, (int(parent_width - my_ideal_width), 0))
    ...
    def _ResetData(self):
        ...
        if self._current_media is not None:
            ...
            has_trash = CC.TRASH_SERVICE_KEY in self._current_media.GetLocationsManager(
            ).GetCurrent()
            ...
            if urls != self._last_seen_urls:
                ...
                for (display_string, url) in url_tuples:
                    ...
                    link = ClientGUICommon.BetterHyperLink(
                        self, display_string, url)

                    QP.AddToLayout(self._urls_vbox, link,
                                   CC.FLAGS_EXPAND_PERPENDICULAR)
...
class FullscreenHoverFrameTags(FullscreenHoverFrame):
    def __init__(self, parent, my_canvas, canvas_key):
        ...
        self._tags = ClientGUIListBoxes.ListBoxTagsSelectionHoverFrame(
            self, self._canvas_key)
