...
def NewInboxArchiveMatchIgnorantOfInbox(new_files, inbox_files, archive_files,
                                        status):
...
def NewInboxArchiveNonMatchIgnorantOfInbox(new_files, inbox_files,
                                           archive_files, status):

    if status == CC.STATUS_SUCCESSFUL_AND_NEW and not new_files:

        return True

    elif status == CC.STATUS_SUCCESSFUL_BUT_REDUNDANT and not (archive_files
                                                               or inbox_files):
...
class CheckerOptions(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._intended_files_per_check, self._never_faster_than,
                self._never_slower_than, self._death_file_velocity)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (self._intended_files_per_check, self._never_faster_than,
         self._never_slower_than,
         self._death_file_velocity) = serialisable_info
    ...
    def GetNextCheckTime(self, file_seed_cache, last_check_time,
        ...
        if len(file_seed_cache) == 0:
            ...
            if current_files_found == 0:
                ...
                time_since_latest_file = max(
                    last_check_time - latest_source_time, 30)

                never_faster_than = max(self._never_faster_than,
                                        time_since_latest_file)

                check_period = min(max(never_faster_than, ideal_check_period),
                                   self._never_slower_than)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_CHECKER_OPTIONS] = CheckerOptions
...
class FilenameTaggingOptions(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (list(self._tags_for_all),
                self._load_from_neighbouring_txt_files, self._add_filename,
                serialisable_directories_dict, self._quick_namespaces,
                self._regexes)

    def _InitialiseFromSerialisableInfo(self, serialisable_info):

        (tags_for_all_list, self._load_from_neighbouring_txt_files,
         self._add_filename, serialisable_directories_dict,
         self._quick_namespaces, self._regexes) = serialisable_info
        ...
        self._quick_namespaces = [
            tuple(item) for item in self._quick_namespaces
        ]
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (tags_for_all_list, load_from_neighbouring_txt_files, add_filename,
             add_first_directory, add_second_directory, add_third_directory,
             quick_namespaces, regexes) = old_serialisable_info
            ...
            new_serialisable_info = (tags_for_all_list,
                                     load_from_neighbouring_txt_files,
                                     add_filename,
                                     serialisable_directories_dict,
                                     quick_namespaces, regexes)
    ...
    def SimpleSetTuple(self, tags_for_all, load_from_neighbouring_txt_files,
                       add_filename, directories_dict):
    ...
    def SimpleToTuple(self):
        ...
        return (self._tags_for_all, self._load_from_neighbouring_txt_files,
                self._add_filename, self._directories_dict)


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_FILENAME_TAGGING_OPTIONS] = FilenameTaggingOptions
...
class FileImportOptions(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        pre_import_options = (self._exclude_deleted,
                              self._do_not_check_known_urls_before_importing,
                              self._do_not_check_hashes_before_importing,
                              self._allow_decompression_bombs, self._min_size,
                              self._max_size, self._max_gif_size,
                              self._min_resolution, self._max_resolution)
        post_import_options = (self._automatic_archive,
                               self._associate_source_urls)
        presentation_options = (self._present_new_files,
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (pre_import_options, post_import_options,
         presentation_options) = serialisable_info

        (self._exclude_deleted, self._do_not_check_known_urls_before_importing,
         self._do_not_check_hashes_before_importing,
         self._allow_decompression_bombs, self._min_size, self._max_size,
         self._max_gif_size, self._min_resolution,
         self._max_resolution) = pre_import_options
        (self._automatic_archive,
         self._associate_source_urls) = post_import_options
        (self._present_new_files, self._present_already_in_inbox_files,
         self._present_already_in_archive_files) = presentation_options

    def _UpdateSerialisableInfo(self, version, old_serialisable_info):

        if version == 1:

            (automatic_archive, exclude_deleted, min_size,
             min_resolution) = old_serialisable_info

            present_new_files = True
            present_already_in_inbox_files = False
            present_already_in_archive_files = False

            new_serialisable_info = (automatic_archive, exclude_deleted,
                                     present_new_files,
                                     present_already_in_inbox_files,
                                     present_already_in_archive_files,
                                     min_size, min_resolution)

            return (2, new_serialisable_info)

        if version == 2:

            (automatic_archive, exclude_deleted, present_new_files,
             present_already_in_inbox_files, present_already_in_archive_files,
             min_size, min_resolution) = old_serialisable_info
            ...
            pre_import_options = (exclude_deleted, allow_decompression_bombs,
                                  min_size, max_size, max_gif_size,
                                  min_resolution, max_resolution)
            post_import_options = automatic_archive
            presentation_options = (present_new_files,
                                    present_already_in_inbox_files,
                                    present_already_in_archive_files)

            new_serialisable_info = (pre_import_options, post_import_options,
        ...
        if version == 3:
            ...
            (pre_import_options, post_import_options,
             presentation_options) = old_serialisable_info

            (exclude_deleted, allow_decompression_bombs, min_size, max_size,
             max_gif_size, min_resolution, max_resolution) = pre_import_options
            ...
            pre_import_options = (exclude_deleted,
    ...
    def CheckFileIsValid(self, size, mime, width, height):
        ...
        if self._min_size is not None and size < self._min_size:
            ...
            raise HydrusExceptions.SizeException(
    ...
    def SetPresentationOptions(self, present_new_files,
                               present_already_in_inbox_files,
                               present_already_in_archive_files):
    ...
    def SetPreImportOptions(self, exclude_deleted,
                            do_not_check_known_urls_before_importing,
                            do_not_check_hashes_before_importing,
    ...
    def ShouldNotPresentIgnorantOfInbox(self, status):
        ...
        return NewInboxArchiveNonMatchIgnorantOfInbox(
            self._present_new_files, self._present_already_in_inbox_files,
            self._present_already_in_archive_files, status)

    def ShouldPresent(self, status, inbox):

        return NewInboxArchiveMatch(self._present_new_files,
                                    self._present_already_in_inbox_files,
                                    self._present_already_in_archive_files,
                                    status, inbox)
    ...
    def ShouldPresentIgnorantOfInbox(self, status):
        ...
        return NewInboxArchiveMatchIgnorantOfInbox(
            self._present_new_files, self._present_already_in_inbox_files,
            self._present_already_in_archive_files, status)
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_FILE_IMPORT_OPTIONS] = FileImportOptions
...
class TagImportOptions(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        serialisable_service_keys_to_service_tag_import_options = [
            (service_key.hex(),
             service_tag_import_options.GetSerialisableTuple())
            for (service_key, service_tag_import_options) in list(
                self._service_keys_to_service_tag_import_options.items())
            if test_func(service_key)
        ]
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (self._fetch_tags_even_if_url_recognised_and_file_already_in_db,
         ...
         serialisable_tag_blacklist,
         serialisable_service_keys_to_service_tag_import_options,
         self._is_default) = serialisable_info

        self._tag_blacklist = HydrusSerialisable.CreateFromSerialisableTuple(
            serialisable_tag_blacklist)

        self._service_keys_to_service_tag_import_options = {
            bytes.fromhex(encoded_service_key):
            HydrusSerialisable.CreateFromSerialisableTuple(
                serialisable_service_tag_import_options)
            for (encoded_service_key, serialisable_service_tag_import_options
                 ) in serialisable_service_keys_to_service_tag_import_options
        }
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            new_serialisable_info = (safe_service_keys_to_namespaces,
                                     safe_service_keys_to_additional_tags)
        ...
        if version == 2:
            ...
            (safe_service_keys_to_namespaces,
             safe_service_keys_to_additional_tags) = old_serialisable_info
        ...
        if version == 3:
            ...
            new_serialisable_info = (
                fetch_tags_even_if_url_recognised_and_file_already_in_db,
                serialisable_tag_blacklist, safe_service_keys_to_namespaces,
                safe_service_keys_to_additional_tags)
        ...
        if version == 4:
            ...
            new_serialisable_info = (
                fetch_tags_even_if_url_recognised_and_file_already_in_db,
                serialisable_tag_blacklist, serialisable_get_all_service_keys,
                safe_service_keys_to_namespaces,
                safe_service_keys_to_additional_tags)
        ...
        if version == 5:
            ...
            get_all_service_keys = {
                bytes.fromhex(encoded_service_key)
                for encoded_service_key in serialisable_get_all_service_keys
            }
            service_keys_to_namespaces = {
                bytes.fromhex(service_key): set(namespaces)
                for (service_key, namespaces
                     ) in list(safe_service_keys_to_namespaces.items())
            }
            service_keys_to_additional_tags = {
                bytes.fromhex(service_key): set(tags)
                for (
                    service_key,
                    tags) in list(safe_service_keys_to_additional_tags.items())
            }
            ...
            for service_key in service_keys:
                ...
                if service_key in service_keys_to_additional_tags:
                    ...
                    additional_tags = service_keys_to_additional_tags[
                        service_key]

                (to_new_files, to_already_in_inbox, to_already_in_archive,
                 only_add_existing_tags) = (True, True, True, False)

                service_tag_import_options = ServiceTagImportOptions(
                    get_tags=get_tags,
                    additional_tags=additional_tags,
                    to_new_files=to_new_files,
                    to_already_in_inbox=to_already_in_inbox,
                    to_already_in_archive=to_already_in_archive,
                    only_add_existing_tags=only_add_existing_tags)

                service_keys_to_service_tag_import_options[
                    service_key] = service_tag_import_options

            serialisable_service_keys_to_service_tag_import_options = [
                (service_key.hex(),
                 service_tag_import_options.GetSerialisableTuple())
                for (service_key, service_tag_import_options) in list(
                    service_keys_to_service_tag_import_options.items())
            ]
    ...
    def CheckBlacklist(self, tags):
        ...
        sibling_tags = HG.client_controller.tag_siblings_manager.CollapseTags(
            CC.COMBINED_TAG_SERVICE_KEY, tags)
    ...
    def GetServiceKeysToContentUpdates(self, status, in_inbox, hash,
                                       parsed_tags):
        ...
        for (service_key, service_tag_import_options
             ) in self._service_keys_to_service_tag_import_options.items():

            service_parsed_tags = siblings_manager.CollapseTags(
                service_key, parsed_tags)
            service_parsed_tags = parents_manager.ExpandTags(
                service_key, service_parsed_tags)

            service_tags = service_tag_import_options.GetTags(
                service_key, status, in_inbox, hash, service_parsed_tags)

            if len(service_tags) > 0:

                service_tags = siblings_manager.CollapseTags(
                    service_key, service_tags)
                service_tags = parents_manager.ExpandTags(
                    service_key, service_tags)
    ...
    def GetServiceTagImportOptions(self, service_key):
        ...
        if service_key not in self._service_keys_to_service_tag_import_options:
            ...
            self._service_keys_to_service_tag_import_options[
                service_key] = ServiceTagImportOptions()
        ...
        for (service_key, service_tag_import_options) in list(
            ...
            if len(sub_statements) > 0:
                ...
                try:
                    ...
                    name = HG.client_controller.services_manager.GetName(
                        service_key)
                ...
                service_statement = name + ':' + os.linesep * 2 + os.linesep.join(
                    sub_statements)
    ...
    def HasAdditionalTags(self):
        ...
        return True in (
            service_tag_import_options.HasAdditionalTags()
            for service_tag_import_options in list(
                self._service_keys_to_service_tag_import_options.values()))
    ...
    def WorthFetchingTags(self):
        ...
        return True in (
            service_tag_import_options.WorthFetchingTags()
            for service_tag_import_options in list(
                self._service_keys_to_service_tag_import_options.values()))


HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_TAG_IMPORT_OPTIONS] = TagImportOptions
...
class ServiceTagImportOptions(HydrusSerialisable.SerialisableBase):
    ...
    def _GetSerialisableInfo(self):
        ...
        return (self._get_tags, serialisable_get_tags_filter,
                list(self._additional_tags), self._to_new_files,
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        ...
        (self._get_tags, serialisable_get_tags_filter, self._additional_tags,
         ...
         serialisable_only_add_existing_tags_filter) = serialisable_info
    ...
    def _UpdateSerialisableInfo(self, version, old_serialisable_info):
        ...
        if version == 1:
            ...
            (get_tags, namespaces, additional_tags, to_new_files,
             to_already_in_inbox, to_already_in_archive,
             only_add_existing_tags) = old_serialisable_info
            ...
            new_serialisable_info = (
                get_tags, serialisable_get_tags_filter, namespaces,
                additional_tags, to_new_files, to_already_in_inbox,
                to_already_in_archive, only_add_existing_tags,
                serialisable_only_add_existing_tags_filter)
        ...
        if version == 2:
            ...
            if not get_tags and len(namespaces) > 0:
                ...
                for namespace in namespaces:
                    ...
                    get_tags_filter.SetRule(namespace + ':',
                                            CC.FILTER_WHITELIST)
            ...
            new_serialisable_info = (
                get_tags, serialisable_get_tags_filter, additional_tags,
                to_new_files, to_already_in_inbox, to_already_in_archive,
    ...
    def GetSummaryStatements(self):
        ...
        if len(self._additional_tags) > 0:
            ...
            statements.append('additional tags: ' +
                              ', '.join(pretty_additional_tags))
    ...
    def GetTags(self, service_key, status, in_inbox, hash, parsed_tags):
        ...
        if NewInboxArchiveMatch(self._to_new_files, self._to_already_in_inbox,
            ...
            if self._only_add_existing_tags:
                ...
                existing_applicable_tags = HG.client_controller.Read(
                    'filter_existing_tags', service_key, tags)
    ...
    def ToTuple(self):
        ...
        return (self._get_tags, self._get_tags_filter, self._additional_tags,
                self._to_new_files, self._to_already_in_inbox,
                self._to_already_in_archive, self._only_add_existing_tags,
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.
    SERIALISABLE_TYPE_SERVICE_TAG_IMPORT_OPTIONS] = ServiceTagImportOptions
