...
current_tag_pool = [
    'blonde hair', 'blue eyes', 'bodysuit', 'character:samus aran',
    'series:metroid', 'studio:nintendo'
]
...
current_parents_pool.append(
    ('character:princess peach', 'series:super mario bros'))
...
pending_parents_pool.append(
    ('character:princess daisy', 'series:super mario bros'))
...
current_siblings_pool.append(
    ('series:tomb raider (series)', 'series:tomb raider'))
...
pair_types_to_pools[HC.CONTENT_TYPE_TAG_PARENTS] = (current_parents_pool,
                                                    pending_parents_pool,
                                                    to_be_pended_parents_pool,
                                                    deleted_parents_pool)
pair_types_to_pools[HC.CONTENT_TYPE_TAG_SIBLINGS] = (
    current_siblings_pool, pending_siblings_pool, to_be_pended_siblings_pool,
    deleted_siblings_pool)
...
class TestMigration(unittest.TestCase):
    ...
    def Read(self, action, *args, **kwargs):
        return TestMigration._db.Read(action, *args, **kwargs)

    def WriteSynchronous(self, action, *args, **kwargs):
        return TestMigration._db.Write(action, True, *args, **kwargs)
    ...
    def _add_pairs_to_services(self, content_type):
        ...
        for pair in current:
            ...
            content_updates.append(
                HydrusData.ContentUpdate(content_type, HC.CONTENT_UPDATE_ADD,
                                         pair))

        for pair in deleted:

            content_updates.append(
                HydrusData.ContentUpdate(content_type,
                                         HC.CONTENT_UPDATE_DELETE, pair))
        ...
        for pair in pending:
            ...
            content_updates.append(
                HydrusData.ContentUpdate(content_type, HC.CONTENT_UPDATE_PEND,
                                         pair))
    ...
    def _test_pairs_service_to_list(self, content_type):
        ...
        content_source_tests.append(
            (CC.DEFAULT_LOCAL_TAG_SERVICE_KEY, (current, ),
             (HC.CONTENT_STATUS_CURRENT, )))
        content_source_tests.append(
            (CC.DEFAULT_LOCAL_TAG_SERVICE_KEY, (deleted, ),
             (HC.CONTENT_STATUS_DELETED, )))
        content_source_tests.append(
            (tag_repo_service_key, (current, ), (HC.CONTENT_STATUS_CURRENT, )))
        content_source_tests.append(
            (tag_repo_service_key, (current, pending),
             (HC.CONTENT_STATUS_CURRENT, HC.CONTENT_STATUS_PENDING)))
        content_source_tests.append(
            (tag_repo_service_key, (deleted, ), (HC.CONTENT_STATUS_DELETED, )))
        ...
        for (left_tag_filter, right_tag_filter) in test_filters:
            ...
            for (service_key, content_lists,
                 content_statuses) in content_source_tests:

                source = ClientMigration.MigrationSourceTagServicePairs(
                    self, service_key, content_type, left_tag_filter,
                    right_tag_filter, content_statuses)
                ...
                for content_list in content_lists:
                    ...
                    expected_data.update(
                        ((left_tag, right_tag)
                         for (left_tag, right_tag) in content_list
                         if left_tag_filter.TagOK(left_tag)
                         and right_tag_filter.TagOK(right_tag)))
    ...
    def test_migration(self):
        ...
        for content_type in (HC.CONTENT_TYPE_TAG_PARENTS,
