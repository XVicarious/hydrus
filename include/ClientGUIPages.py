...
from . import (ClientData, ClientDownloading, ClientGUICanvas,
               ...
               ClientGUIManagement, ClientGUIMedia, ClientGUIMenus,
               ClientGUIResults, ClientGUIShortcuts, ClientSearch,
               ClientThreading)
...
RESERVED_SESSION_NAMES = {
    '', 'just a blank page', 'last session', 'exit session'
}
...
class DialogPageChooser(ClientGUIDialogs.Dialog):
    def __init__(self, parent, controller):
        ...
        ClientGUIDialogs.Dialog.__init__(self,
                                         parent,
                                         'new page',
                                         position='center')
        ...
        (width,
         height) = ClientGUIFunctions.ConvertTextToPixels(self, (64, 14))
        ...
        repository_petition_permissions = [
            (content_type, HC.PERMISSION_ACTION_OVERRULE)
            for content_type in HC.REPOSITORY_CONTENT_TYPES
        ]

        self._petition_service_keys = [
            service.GetServiceKey() for service in self._services
            if service.GetServiceType() in HC.REPOSITORIES and True in (
                service.HasPermission(content_type, action)
                for (content_type, action) in repository_petition_permissions)
        ]
    ...
    def _AddEntry(self, button, entry):
        ...
        if entry_type == 'menu':
            ...
            name = HG.client_controller.services_manager.GetService(
                obj).GetName()
        ...
        if id in self._command_dict:
            ...
            if entry_type == 'menu':
                ...
                if entry_type == 'page_query':
                    ...
                    tag_service_key = new_options.GetKey(
                        'default_tag_service_search_page')

                    if not self._controller.services_manager.ServiceExists(
                            tag_service_key):
                    ...
                    self._result = (
                        'page',
                        ClientGUIManagement.CreateManagementControllerQuery(
                            page_name, file_service_key, file_search_context,
                            search_enabled))
                    ...
                    self._result = (
                        'page',
                        ClientGUIManagement.
                        CreateManagementControllerDuplicateFilter())
                    ...
                    self._result = ('page',
                                    ClientGUIManagement.
                                    CreateManagementControllerImportGallery())
                    ...
                    self._result = (
                        'page',
                        ClientGUIManagement.
                        CreateManagementControllerImportSimpleDownloader())
                    ...
                    self._result = (
                        'page',
                        ClientGUIManagement.
                        CreateManagementControllerImportMultipleWatcher())

                elif entry_type == 'page_import_urls':

                    self._result = ('page',
                                    ClientGUIManagement.
                                    CreateManagementControllerImportURLs())
                    ...
                    self._result = ('page',
                                    ClientGUIManagement.
                                    CreateManagementControllerPetitions(
    ...
    def _InitButtons(self, menu_keyword):
        ...
        if menu_keyword == 'home':
            ...
            if HG.client_controller.new_options.GetBoolean('advanced_mode'):
                ...
                entries.append(
                    ('page_query', CC.COMBINED_LOCAL_FILE_SERVICE_KEY))
            ...
            entries = [('page_petitions', service_key)
                       for service_key in self._petition_service_keys]
        ...
        if len(entries) <= 4:
            ...
            potential_buttons = [
                self._button_8, self._button_4, self._button_6, self._button_2
            ]

        elif len(entries) <= 9:

            potential_buttons = [
                self._button_7, self._button_8, self._button_9, self._button_4,
                self._button_5, self._button_6, self._button_1, self._button_2,
                self._button_3
            ]
    ...
    def event(self, event):
        ...
        if event.type(
        ) == QC.QEvent.WindowDeactivate and not self._action_picked:
...
class Page(QW.QSplitter):
    def __init__(self, parent, controller, management_controller,
        ...
        self._preview_canvas = ClientGUICanvas.CanvasPanel(
            self._preview_panel, self._page_key)

        self._media_panel = ClientGUIResults.MediaPanelThumbnails(
            self, self._page_key, file_service_key, [])

        vbox = QP.VBoxLayout(margin=0)

        QP.AddToLayout(vbox, self._preview_canvas,
        ...
        self._search_preview_split._handle_event_filter = QP.WidgetEventFilter(
            self._search_preview_split.handle(1))
        self._search_preview_split._handle_event_filter.EVT_LEFT_DCLICK(
            self.EventPreviewUnsplit)

        if HC.options['hide_preview']:

            QP.CallAfter(QP.Unsplit, self._search_preview_split,
                         self._preview_panel)

        self._controller.sub(self, 'SetPrettyStatus', 'new_page_status')
        self._controller.sub(self, 'SetSplitterPositions',
    ...
    def IsMultipleWatcherPage(self):
        ...
        return self._management_controller.GetType(
        ) == ClientGUIManagement.MANAGEMENT_TYPE_IMPORT_MULTIPLE_WATCHER
    ...
    def IsURLImportPage(self):
        ...
        return self._management_controller.GetType(
        ) == ClientGUIManagement.MANAGEMENT_TYPE_IMPORT_URLS
    ...
    def SetupSplits(self):
        ...
        QP.SplitVertically(self, self._search_preview_split, self._media_panel,
                           HC.options['hpos'])

        QP.SplitHorizontally(self._search_preview_split,
                             self._management_panel, self._preview_panel,
                             HC.options['vpos'])
    ...
    def SetMediaResults(self, media_results):
        ...
        media_panel = ClientGUIResults.MediaPanelThumbnails(
            self, self._page_key, file_service_key, media_results)
        ...
        )  # importand this is callafter, so it happens after a heavy session load is done
    ...
    def SetSplitterPositions(self, hpos, vpos):
        ...
        QP.SplitHorizontally(self._search_preview_split,
                             self._management_panel, self._preview_panel, vpos)
    ...
    def THREADLoadInitialMediaResults(self, controller, initial_hashes):
        ...
        for group_of_initial_hashes in HydrusData.SplitListIntoChunks(
                initial_hashes, 256):

            more_media_results = controller.Read('media_results',
                                                 group_of_initial_hashes)

            initial_media_results.extend(more_media_results)

            status = 'Loading initial files\u2026 ' + HydrusData.ConvertValueRangeToPrettyString(
                len(initial_media_results), len(initial_hashes))

            QP.CallAfter(qt_code_status, status)

        hashes_to_media_results = {
            media_result.GetHash(): media_result
            for media_result in initial_media_results
        }

        sorted_initial_media_results = [
            hashes_to_media_results[hash] for hash in initial_hashes
...
class PagesNotebook(QP.TabWidgetWithDnD):
    def __init__(self, parent, controller, name):
        ...
        self._controller.sub(self, 'NotifyPageUnclosed',
                             'notify_page_unclosed')
    ...
    def _RefreshPageNamesAfterDnD(self, page_widget, source_widget):
        ...
        if hasattr(source_widget.parentWidget(), 'GetPageKey'):
            ...
            self._controller.pub('refresh_page_name',
                                 source_widget.parentWidget().GetPageKey())
    ...
    def _CloseLeftPages(self, from_index):
        ...
        if result == QW.QDialog.Accepted:
            ...
            closees = [
                index for index in range(self.count()) if index < from_index
            ]
    ...
    def _CloseOtherPages(self, except_index):
        ...
        if result == QW.QDialog.Accepted:
            ...
            closees = [
                index for index in range(self.count()) if index != except_index
            ]
    ...
    def _GetIndex(self, page_key):
        ...
        for (page, index) in ((self.widget(index), index)
    ...
    def _GetNotebookFromScreenPosition(self, screen_position):
        ...
        if current_page is None or not isinstance(current_page, PagesNotebook):
            ...
            tab_index = ClientGUIFunctions.NotebookScreenToHitTest(
            ...
            if on_child_notebook_somewhere:
                ...
                return current_page._GetNotebookFromScreenPosition(
                    screen_position)
    ...
    def _MovePage(self,
                  ...
                  dest_notebook,
                  insertion_tab_index,
                  follow_dropped_page=False):

        source_notebook = page.parentWidget().parentWidget(
        )  # page.parentWidget() is a QStackedWidget
        ...
        if follow_dropped_page:
            dest_notebook.setCurrentIndex(insertion_tab_index)
    ...
    def _RefreshPageName(self, index):
        ...
        page_file_count_display = new_options.GetInteger(
            'page_file_count_display')

        import_page_progress_display = new_options.GetBoolean(
        ...
        if page_file_count_display == CC.PAGE_FILE_COUNT_DISPLAY_ALL or (
                page_file_count_display == CC.
                PAGE_FILE_COUNT_DISPLAY_ONLY_IMPORTERS and page.IsImporter()):
        ...
        if import_page_progress_display:
            ...
            if num_range > 0 and num_value != num_range:
                ...
                num_string += HydrusData.ConvertValueRangeToPrettyString(
                    num_value, num_range)
    ...
    def _SendPageToNewNotebook(self, index):
        ...
        if 0 <= index and index <= self.count() - 1:
            ...
            dest_notebook = self.NewPagesNotebook(forced_insertion_index=index,
                                                  give_it_a_blank_page=False)
    ...
    def _SendRightPagesToNewNotebook(self, from_index):
        ...
        if result == QW.QDialog.Accepted:
            ...
            dest_notebook = self.NewPagesNotebook(
                forced_insertion_index=pages_index, give_it_a_blank_page=False)
    ...
    def _ShowMenu(self, screen_position):
        ...
        if click_over_tab:
            ...
            ClientGUIMenus.AppendMenuItem(menu, 'close page',
                                          'Close this page.', self._ClosePage,
                                          tab_index)

            if num_pages > 1:

                ClientGUIMenus.AppendMenuItem(menu, 'close other pages',
                                              'Close all pages but this one.',
                                              self._CloseOtherPages, tab_index)

                if can_go_left:
                    ClientGUIMenus.AppendMenuItem(
                        menu, 'close pages to the left',
                        'Close all pages to the left of this one.',
                        self._CloseLeftPages, tab_index)

                if can_go_right:
                    ClientGUIMenus.AppendMenuItem(
                        menu, 'close pages to the right',
                        'Close all pages to the right of this one.',
                        self._CloseRightPages, tab_index)

            ClientGUIMenus.AppendSeparator(menu)

            ClientGUIMenus.AppendMenuItem(menu, 'rename page',
                                          'Rename this page.',
                                          self._RenamePage, tab_index)

        ClientGUIMenus.AppendMenuItem(menu, 'new page', 'Choose a new page.',
                                      self._ChooseNewPage)

        if click_over_tab:

            ClientGUIMenus.AppendMenuItem(menu, 'new page here',
                                          'Choose a new page.',
                                          self._ChooseNewPage, tab_index)
            ...
            if more_than_one_tab:
                ...
                if can_home:
                    ClientGUIMenus.AppendMenuItem(
                        menu,
                        'move to left end',
                        'Move this page all the way to the left.',
                        self._ShiftPage,
                        tab_index,
                        new_index=0)
                ...
                if can_move_left:
                    ClientGUIMenus.AppendMenuItem(
                        menu,
                        'move left',
                        'Move this page one to the left.',
                        self._ShiftPage,
                        tab_index,
                        delta=-1)
                ...
                if can_move_right:
                    ClientGUIMenus.AppendMenuItem(
                        menu, 'move right', 'Move this page one to the right.',
                        self._ShiftPage, tab_index, 1)

                if can_end:
                    ClientGUIMenus.AppendMenuItem(
                        menu,
                        'move to right end',
                        'Move this page all the way to the right.',
                        self._ShiftPage,
                        tab_index,
                        new_index=end_index)
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'sort pages by most files first',
                    'Sort these pages according to how many files they appear to have.',
                    self._SortPagesByFileCount, 'desc')
                ClientGUIMenus.AppendMenuItem(
                    menu, 'sort pages by fewest files first',
                    'Sort these pages according to how few files they appear to have.',
                    self._SortPagesByFileCount, 'asc')

            ClientGUIMenus.AppendSeparator(menu)

            ClientGUIMenus.AppendMenuItem(
                menu, 'send this page down to a new page of pages',
                'Make a new page of pages and put this page in it.',
                self._SendPageToNewNotebook, tab_index)

            if can_go_right:

                ClientGUIMenus.AppendMenuItem(
                    menu, 'send pages to the right to a new page of pages',
                    'Make a new page of pages and put all the pages to the right into it.',
                    self._SendRightPagesToNewNotebook, tab_index)
            ...
            if click_over_page_of_pages and page.count() > 0:
                ...
                ClientGUIMenus.AppendMenuItem(
                    menu, 'refresh all this page\'s pages',
                    'Command every page below this one to refresh.',
        ...
        if click_over_page_of_pages:
            ...
            for name in existing_session_names:
                ...
                ClientGUIMenus.AppendMenuItem(
                    submenu,
                    name,
                    'Save this page of pages to the session.',
                    self._controller.gui.ProposeSaveGUISession,
                    notebook=page,
                    name=name)
            ...
            ClientGUIMenus.AppendMenuItem(
                ...
                self._controller.gui.ProposeSaveGUISession,
                notebook=page,
                suggested_name=page.GetName())

            ClientGUIMenus.AppendMenu(menu, submenu,
                                      'save this page of pages to a session')
    ...
    def _SortPagesByFileCount(self, order):
        ...
        def key(page):
            ...
            (total_num_files, (total_num_value,
                               total_num_range)) = page.GetNumFileSummary()
    ...
    def AppendGUISession(self, name, load_in_a_page_of_pages=True):
        ...
        try:
            ...
            session = self._controller.Read(
                'serialisable_named',
                HydrusSerialisable.SERIALISABLE_TYPE_GUI_SESSION, name)

        except Exception as e:

            HydrusData.ShowText('While trying to load session ' + name +
                                ', this error happened:')
        ...
        if load_in_a_page_of_pages:
            ...
            destination = self.NewPagesNotebook(name=name,
                                                give_it_a_blank_page=False)
    ...
    def AppendGUISessionBackup(self,
                               name,
                               timestamp,
                               load_in_a_page_of_pages=True):
    ...
    def GetOrMakeMultipleWatcherPage(self,
                                     desired_page_name=None,
                                     desired_page_key=None,
                                     select_page=True):

        potential_watcher_pages = [
            page for page in self._GetMediaPages(False)
            if page.IsMultipleWatcherPage()
        ]

        if desired_page_key is not None and desired_page_key in (
                page.GetPageKey() for page in potential_watcher_pages):

            potential_watcher_pages = [
                page for page in potential_watcher_pages
                if page.GetPageKey() == desired_page_key
            ]
            ...
            potential_watcher_pages = [
                page for page in potential_watcher_pages
                if page.GetName() == desired_page_name
            ]
        ...
        if len(potential_watcher_pages) > 0:
            ...
            return self.NewPageImportMultipleWatcher(
                page_name=desired_page_name,
    ...
    def GetOrMakeURLImportPage(self,
        ...
        potential_url_import_pages = [
            page for page in self._GetMediaPages(False)
            if page.IsURLImportPage()
        ]

        if desired_page_key is not None and desired_page_key in (
                page.GetPageKey() for page in potential_url_import_pages):

            potential_url_import_pages = [
                page for page in potential_url_import_pages
                if page.GetPageKey() == desired_page_key
            ]
        ...
        if len(potential_url_import_pages) > 0:
            ...
            return self.NewPageImportURLs(page_name=desired_page_name,
                                          on_deepest_notebook=True,
                                          select_page=select_page)
    ...
    def GetSessionAPIInfoDict(self, is_selected=True):
        ...
        for page in self._GetPages():
            ...
            page_info_dict = page.GetSessionAPIInfoDict(
    ...
    def GetPrettyStatus(self):
        ...
        return HydrusData.ToHumanInt(
            self.count()) + ' pages, ' + num_string + ' files'
    ...
    def GetTotalWeight(self):
        ...
        total_weight = sum(
            (page.GetTotalWeight() for page in self._GetPages()))
    ...
    def LoadGUISession(self, name):
        ...
        if self.count() > 0:
            ...
            message = 'Close the current pages and load session "{}"?'.format(
                name)

            result = ClientGUIDialogsQuick.GetYesNo(
                self, message, title='Clear and load session?')
            ...
            self._controller.CallLaterQtSafe(self,
                                             1.0,
                                             self.AppendGUISession,
                                             name,
                                             load_in_a_page_of_pages=False)
    ...
    def MediaDragAndDropDropped(self, source_page_key, hashes):
        ...
        if tab_index == -1 and dest_notebook.currentWidget(
        ) and dest_notebook.currentWidget().rect().contains(
                dest_notebook.currentWidget().mapFromGlobal(screen_position)):

            dest_page = dest_notebook.currentWidget()

        elif tab_index == -1:

            dest_page = dest_notebook.NewPageQuery(CC.LOCAL_FILE_SERVICE_KEY,
            ...
            if isinstance(dest_page, PagesNotebook):
                ...
                if result is None:
                    ...
                    dest_page = dest_page.NewPageQuery(
                        CC.LOCAL_FILE_SERVICE_KEY, initial_hashes=hashes)
        ...
        if do_add:
            ...
            dest_page.GetMediaPanel().AddMediaResults(dest_page.GetPageKey(),
                                                      media_results)
        ...
        ctrl_down = QW.QApplication.queryKeyboardModifiers(
        ) & QC.Qt.ControlModifier

        if not ctrl_down:

            source_page.GetMediaPanel().RemoveMedia(source_page.GetPageKey(),
                                                    hashes)
    ...
    def NewPage(self,
        ...
        WARNING_TOTAL_PAGES = self._controller.new_options.GetInteger(
            'total_pages_warning')
        MAX_TOTAL_PAGES = 200

        (total_active_page_count, total_closed_page_count, total_active_weight,
         total_closed_weight) = self._controller.gui.GetTotalPageCounts()
        ...
        if not HG.no_page_limit_mode:
            ...
            if total_active_page_count >= MAX_TOTAL_PAGES:
                ...
                message = 'The client should not have more than ' + str(
                    MAX_TOTAL_PAGES
                ) + ' pages open, as it leads to program instability! Are you sure you want to open more pages?'

                result = ClientGUIDialogsQuick.GetYesNo(
                    self,
                    message,
                    title='Too many pages!',
                    yes_label='yes, and do not tell me again',
                    no_label='no')
            ...
            if total_active_page_count == WARNING_TOTAL_PAGES:
                ...
                HydrusData.ShowText(
                    'You have ' + str(total_active_page_count) +
                    ' pages open! You can only open a few more before program stability is affected! Please close some now!'
                )
        ...
        page = Page(self, self._controller, management_controller,
                    initial_hashes)
    ...
    def NewPageImportMultipleWatcher(self,
                                     page_name=None,
                                     url=None,
                                     on_deepest_notebook=False,
                                     select_page=True):
    ...
    def NewPagePetitions(self, service_key, on_deepest_notebook=False):
        ...
        management_controller = ClientGUIManagement.CreateManagementControllerPetitions(
            service_key)
    ...
    def NewPageQuery(self,
        ...
        management_controller = ClientGUIManagement.CreateManagementControllerQuery(
            page_name, file_service_key, file_search_context, search_enabled)

        page = self.NewPage(management_controller,
                            initial_hashes=initial_hashes,
                            on_deepest_notebook=on_deepest_notebook,
                            select_page=select_page)
    ...
    def NewPagesNotebook(self,
                         name='pages',
                         forced_insertion_index=None,
                         on_deepest_notebook=False,
                         give_it_a_blank_page=True,
                         select_page=True):
        ...
        if on_deepest_notebook and isinstance(current_page, PagesNotebook):
            ...
            return current_page.NewPagesNotebook(
                name=name,
                forced_insertion_index=forced_insertion_index,
                on_deepest_notebook=on_deepest_notebook,
                give_it_a_blank_page=give_it_a_blank_page)
    ...
    def PresentImportedFilesToPage(self, hashes, page_name):
        ...
        if page is None:
            ...
            page = self.NewPageQuery(CC.LOCAL_FILE_SERVICE_KEY,
                                     initial_hashes=hashes,
                                     page_name=page_name,
                                     on_deepest_notebook=True,
                                     select_page=False)
            ...
            def qt_finish(page, media_results):
                ...
                page.GetMediaPanel().AddMediaResults(page.GetPageKey(),
                                                     media_results)
    ...
    def RefreshPageName(self, page_key=None):
        ...
        if page_key is None:
            ...
            for (index, page) in enumerate(self._GetPages()):
                ...
                elif isinstance(page,
                                PagesNotebook) and page.HasPageKey(page_key):
...
class GUISession(HydrusSerialisable.SerialisableBaseNamed):
    ...
    def _GetPageTuple(self, page):
        ...
        if isinstance(page, PagesNotebook):
            ...
            page_tuples = [
                self._GetPageTuple(subpage) for subpage in page.GetPages()
            ]
    ...
    def _GetSerialisableInfo(self):
        def handle_e(e):
            ...
            HydrusData.ShowText(
                'Attempting to save a page to the session failed! Its data tuple and error follows! Please close it or see if you can clear any potentially invalid data from it!'
            )
            ...
            if page_type == 'pages':
                ...
                for pt in page_tuples:
                    ...
                    try:
                        ...
                        serialisable_page_tuples.append(
                            GetSerialisablePageTuple(pt))
                ...
                serialisable_page_data = (serialisable_management_controller,
                                          serialisable_hashes)
    ...
    def _InitialiseFromSerialisableInfo(self, serialisable_info):
        def handle_e(e):
            ...
            HydrusData.ShowText(
                'A page failed to load! Its serialised data and error follows!'
            )
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[
    HydrusSerialisable.SERIALISABLE_TYPE_GUI_SESSION] = GUISession
