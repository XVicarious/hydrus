...
from . import (ClientLocalServer, ClientMedia, ClientRatings, ClientServices,
               ClientTags)
...
class TestServer(unittest.TestCase):
    ...
    def _test_file_repo(self, service):
        ...
        self.assertEqual(
            ...
            b'\xadm5\x99\xa6\xc4\x89\xa5u\xeb\x19\xc0&\xfa\xce\x97\xa9\xcdey\xe7G(\xb0\xce\x94\xa6\x01\xd22\xf3\xc3'
        )
    ...
    def _test_local_booru(self, host, port):
        ...
        media_results = [
            ClientMedia.MediaResult(
                ...
                ClientRatings.RatingsManager({}), file_viewing_stats_manager)
            for hash in hashes
        ]
    ...
    def _test_local_booru_requests(self, connection, share_key, hash,
                                   expected_result):

        requests = []

        requests.append('/gallery?share_key=' + share_key.hex())
        requests.append('/page?share_key=' + share_key.hex() + '&hash=' +
                        hash.hex())
        requests.append('/file?share_key=' + share_key.hex() + '&hash=' +
                        hash.hex())
        requests.append('/thumbnail?share_key=' + share_key.hex() + '&hash=' +
                        hash.hex())
    ...
    def _test_restricted(self, service):
        ...
        HG.test_controller.SetRead('account_key_from_identifier',
                                   HydrusData.GenerateKey())

        response = service.Request(
            HC.GET, 'account_info',
            {'subject_account_key': HydrusData.GenerateKey()})
        ...
        self.assertEqual(written_account_types[0].GetAccountTypeKey(),
                         account_types[0].GetAccountTypeKey())
        self.assertEqual(
