...
class EditFileSeedCachePanel(ClientGUIScrolledPanels.EditPanel):
    ...
    def _CopySelectedFileSeedData(self):
        ...
        if len(file_seeds) > 0:
            ...
            text = separator.join(
                (file_seed.file_seed_data for file_seed in file_seeds))
    ...
    def _UpdateText(self):
        ...
        (status, simple_status, (total_processed,
                                 total)) = self._file_seed_cache.GetStatus()
...
class FileSeedCacheButton(ClientGUICommon.BetterBitmapButton):
    def __init__(self,
                 parent,
                 controller,
                 file_seed_cache_get_callable,
                 file_seed_cache_set_callable=None):

        ClientGUICommon.BetterBitmapButton.__init__(
            self, parent, CC.GlobalPixmaps.listctrl,
            self._ShowFileSeedCacheFrame)
        ...
        self.setToolTip(
            'open detailed file import status--right-click for quick actions, if applicable'
        )
    ...
    def _ClearFileSeeds(self, statuses_to_remove):
        ...
        ) + ' file import items? This is useful for cleaning up and de-laggifying a very large list, but be careful you aren\'t removing something you would want to revisit or what watcher/subscription may be using for future check time calculations.'
...
class FileSeedCacheStatusControl(QW.QFrame):
    ...
    def _Update(self):
        ...
        if self._file_seed_cache is None:
            ...
            (import_summary, simple_status,
             (num_done, num_to_do)) = self._file_seed_cache.GetStatus()
    ...
    def TIMERUIUpdate(self):
        ...
        if self._file_seed_cache is not None:
            ...
            (old_num_done,
             old_num_to_do) = self._progress_gauge.GetValueRange()
            ...
            if old_num_done != num_done or old_num_to_do != num_to_do:
                ...
                if self._page_key is not None:
                    ...
                    HG.client_controller.pub('refresh_page_name',
                                             self._page_key)
