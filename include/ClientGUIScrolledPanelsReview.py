...
from . import (
    ClientData, ClientDefaults, ClientDragDrop, ClientExporting, ClientFiles,
    ClientGUIACDropdown, ClientGUIAsync, ClientGUICharts, ClientGUICommon,
    ClientGUIControls, ClientGUIDialogs, ClientGUIDialogsQuick,
    ClientGUIFrames, ClientGUIFunctions, ClientGUIImport, ClientGUIListBoxes,
    ClientGUIListCtrl, ClientGUIPanels, ClientGUIPopupMessages,
    ClientGUIScrolledPanels, ClientGUIScrolledPanelsEdit, ClientGUIShortcuts,
    ClientGUITags, ClientGUITime, ClientGUITopLevelWindows, ClientMigration,
    ClientNetworking, ClientNetworkingContexts, ClientNetworkingDomain,
    ClientNetworkingLogin, ClientParsing, ClientPaths, ClientRendering,
    ClientSearch, ClientSerialisable, ClientTags, ClientThreading)
...
class MigrateDatabasePanel(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, controller):
        ...
        self._prefixes_to_locations = HG.client_controller.Read(
            'client_files_locations')

        (self._locations_to_ideal_weights,
         self._ideal_thumbnails_location_override
         ) = self._controller.Read('ideal_client_files_locations')

        service_info = HG.client_controller.Read(
            'service_info', CC.COMBINED_LOCAL_FILE_SERVICE_KEY)

        self._all_local_files_total_size = service_info[
            HC.SERVICE_INFO_TOTAL_SIZE]
        self._all_local_files_total_num = service_info[
            HC.SERVICE_INFO_NUM_FILES]

        menu_items = []

        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR, 'database_migration.html'))

        menu_items.append(
            ('normal', 'open the html migration help',
             'Open the help page for database migration in your web browser.',
             page_func))
        ...
        self._current_install_path_st = ClientGUICommon.BetterStaticText(
            info_panel)
        self._current_db_path_st = ClientGUICommon.BetterStaticText(info_panel)
        self._current_media_paths_st = ClientGUICommon.BetterStaticText(
            info_panel)

        current_media_locations_listctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            info_panel)

        columns = [('location', -1), ('portable?', 11), ('free space', 12),
                   ('file weight', 10), ('current usage', 24),
                   ('ideal usage', 24)]

        self._current_media_locations_listctrl = ClientGUIListCtrl.BetterListCtrl(
            current_media_locations_listctrl_panel, 'db_migration_locations',
            8, 36, columns, self._ConvertLocationToListCtrlTuples)
        self._current_media_locations_listctrl.setSelectionMode(
            QW.QAbstractItemView.SingleSelection)

        self._current_media_locations_listctrl.Sort()

        current_media_locations_listctrl_panel.SetListCtrl(
            self._current_media_locations_listctrl)

        current_media_locations_listctrl_panel.AddButton(
            'add new location for files', self._SelectPathToAdd)
        current_media_locations_listctrl_panel.AddButton(
            'increase file weight',
            self._IncreaseWeight,
            enabled_check_func=self._CanIncreaseWeight)
        current_media_locations_listctrl_panel.AddButton(
            'decrease file weight',
            self._DecreaseWeight,
            enabled_check_func=self._CanDecreaseWeight)

        self._thumbnails_location = QW.QLineEdit(info_panel)
        self._thumbnails_location.setEnabled(False)

        self._thumbnails_location_set = ClientGUICommon.BetterButton(
            info_panel, 'set', self._SetThumbnailLocation)

        self._thumbnails_location_clear = ClientGUICommon.BetterButton(
            info_panel, 'clear', self._ClearThumbnailLocation)

        self._rebalance_status_st = ClientGUICommon.BetterStaticText(
            info_panel)
        self._rebalance_status_st.setAlignment(QC.Qt.AlignRight
                                               | QC.Qt.AlignVCenter)

        self._rebalance_button = ClientGUICommon.BetterButton(
            info_panel, 'move files now', self._Rebalance)

        #

        migration_panel = ClientGUICommon.StaticBox(self,
                                                    'migrate entire database')

        self._migrate_db_button = ClientGUICommon.BetterButton(
            migration_panel, 'move entire database and all portable paths',
        ...
        QP.AddToLayout(
            t_hbox,
            ClientGUICommon.BetterStaticText(info_panel,
                                             'thumbnail location override'),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(t_hbox, self._thumbnails_location,
                       CC.FLAGS_VCENTER_EXPAND_DEPTH_ONLY)
        QP.AddToLayout(t_hbox, self._thumbnails_location_set, CC.FLAGS_VCENTER)
        QP.AddToLayout(t_hbox, self._thumbnails_location_clear,
                       CC.FLAGS_VCENTER)

        rebalance_hbox = QP.HBoxLayout()

        QP.AddToLayout(rebalance_hbox, self._rebalance_status_st,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(rebalance_hbox, self._rebalance_button,
                       CC.FLAGS_VCENTER)

        info_panel.Add(self._current_install_path_st,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        info_panel.Add(self._current_db_path_st, CC.FLAGS_EXPAND_PERPENDICULAR)
        info_panel.Add(self._current_media_paths_st,
                       CC.FLAGS_EXPAND_PERPENDICULAR)
        info_panel.Add(current_media_locations_listctrl_panel,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
    ...
    def _AddPath(self, path, starting_weight=1):
        ...
        if path in self._locations_to_ideal_weights:
            ...
            QW.QMessageBox.warning(self, 'Warning',
                                   'You already have that location entered!')
        ...
        self._controller.Write('ideal_client_files_locations',
                               self._locations_to_ideal_weights,
                               self._ideal_thumbnails_location_override)
    ...
    def _AdjustWeight(self, amount):
        ...
        locations = self._current_media_locations_listctrl.GetData(
            only_selected=True)
    ...
    def _CanDecreaseWeight(self):
        ...
        if len(locations) > 0:
            ...
            if location in self._locations_to_ideal_weights:
                ...
                others_can_take_slack = len(
                    self._locations_to_ideal_weights) > 1
    ...
    def _CanIncreaseWeight(self):
        ...
        (locations_to_file_weights,
         locations_to_thumb_weights) = self._GetLocationsToCurrentWeights()
    ...
    def _ConvertLocationToListCtrlTuples(self, location):
        ...
        if len(usages) > 0:
            ...
            pretty_current_usage = HydrusData.ToHumanBytes(
                current_bytes) + ' - ' + ','.join(usages)
        ...
        if location in self._locations_to_ideal_weights:
            ...
            ideal_fp = self._locations_to_ideal_weights[
                location] / total_ideal_weight
        ...
        if len(usages) > 0:
            ...
            pretty_ideal_usage = HydrusData.ToHumanBytes(
                ideal_bytes) + ' - ' + ','.join(usages)

        else:

            pretty_ideal_usage = 'nothing'

        display_tuple = (pretty_location, pretty_portable, pretty_free_space,
                         pretty_ideal_weight, pretty_current_usage,
                         pretty_ideal_usage)
        sort_tuple = (location, portable, free_space, ideal_weight,
                      current_usage, ideal_usage)
    ...
    def _GetThumbnailSizeEstimate(self):
        ...
        (t_width,
         t_height) = HG.client_controller.options['thumbnail_dimensions']

        typical_thumb_num_pixels = 320 * 240
        typical_thumb_size = 36 * 1024

        our_thumb_num_pixels = t_width * t_height
        our_thumb_size_estimate = typical_thumb_size * (
            our_thumb_num_pixels / typical_thumb_num_pixels)
    ...
    def _MigrateDatabase(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            with QP.DirDialog(self,
                              message='Choose new database location.') as dlg:
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    if len(os.listdir(dest)) > 0:
                        ...
                        message = '"{}" is not empty! Please select an empty destination--if your situation is more complicated, please do this move manually! Feel free to ask hydrus dev for help.'.format(
                            dest)
                    ...
                    with ClientGUIDialogs.DialogTextEntry(
                            self, message, default=shortcut) as dlg_3:
                        ...
                        if dlg_3.exec() == QW.QDialog.Accepted:
                            ...
                            for location in set(
                                    self._prefixes_to_locations.values()):
                                ...
                                if portable:
                                    ...
                                    portable_locations.append(
                                        portable_location)

                            HG.client_controller.CallToThreadLongRunning(
                                THREADMigrateDatabase, self._controller,
                                source, portable_locations, dest)
    ...
    def _Rebalance(self):
        ...
        self._controller.CallToThread(
            self._controller.client_files_manager.Rebalance, job_key)

        with ClientGUITopLevelWindows.DialogNullipotent(
                self, 'migrating files') as dlg:
    ...
    def _RemovePath(self, location):
        ...
        if location not in self._locations_to_ideal_weights:
            ...
            QW.QMessageBox.warning(self, 'Warning',
                                   'Please select a location with weight.')
        ...
        if len(self._locations_to_ideal_weights) == 1:
            ...
            QW.QMessageBox.warning(
                self, 'Warning',
                'You cannot empty every single current file location--please add a new place for the files to be moved to and then try again.'
            )
    ...
    def _Update(self):
        ...
        self._current_db_path_st.setText(
            'database (about ' +
            HydrusData.ToHumanBytes(approx_total_db_size) + '): ' +
            self._controller.GetDBDir())
        ...
        label = 'media is ' + HydrusData.ToHumanBytes(
            approx_total_client_files
        ) + ', thumbnails are estimated at ' + HydrusData.ToHumanBytes(
            approx_total_thumbnails) + ':'
        ...
        if self._ideal_thumbnails_location_override is None:
            ...
            self._thumbnails_location.setText(
                self._ideal_thumbnails_location_override)
        ...
        if self._controller.client_files_manager.RebalanceWorkToDo():
            ...
            self._rebalance_status_st.setText(
...
def THREADMigrateDatabase(controller, source, portable_locations, dest):
    ...
    time.sleep(
        2
    )  # important to have this, so the migrate dialog can close itself and clean its event loop, wew

    def qt_code(job_key):

        HG.client_controller.CallLaterQtSafe(controller.gui, 3.0,
                                             controller.gui.SaveAndClose)

        # no parent because this has to outlive the gui, obvs

        with ClientGUITopLevelWindows.DialogNullipotent(
                None, 'migrating files') as dlg:
    ...
    try:
        ...
        for portable_location in portable_locations:
            ...
            HydrusPaths.MergeTree(source_path,
                                  dest_path,
        ...
        QP.CallAfter(QW.QMessageBox.critical, None, 'Error',
...
class MigrateTagsPanel(ClientGUIScrolledPanels.ReviewPanel):
    ...
    def __init__(self, parent, service_key, hashes=None):
        ...
        self._migration_content_type = ClientGUICommon.BetterChoice(
            self._migration_panel)

        for content_type in (HC.CONTENT_TYPE_MAPPINGS,
                             HC.CONTENT_TYPE_TAG_PARENTS,
                             HC.CONTENT_TYPE_TAG_SIBLINGS):

            self._migration_content_type.addItem(
                HC.content_type_string_lookup[content_type], content_type)

        self._migration_content_type.setToolTip('Sets what will be migrated.')

        self._migration_source = ClientGUICommon.BetterChoice(
            self._migration_panel)

        self._migration_source_archive_path_button = ClientGUICommon.BetterButton(
            self._migration_panel, 'no path set', self._SetSourceArchivePath)

        self._migration_source_hash_type_st = ClientGUICommon.BetterStaticText(
            self._migration_panel, 'hash type: unknown')

        self._migration_source_hash_type_st.setToolTip(
            'If this is something other than sha256, this will only work for files the client has ever previously imported.'
        )

        self._migration_source_content_status_filter = ClientGUICommon.BetterChoice(
            self._migration_panel)

        self._migration_source_content_status_filter.setToolTip(
            'This filters which status of tags will be migrated.')

        self._migration_source_file_filter = ClientGUICommon.BetterChoice(
            self._migration_panel)

        for service_key in (CC.LOCAL_FILE_SERVICE_KEY,
                            CC.COMBINED_FILE_SERVICE_KEY):

            service = HG.client_controller.services_manager.GetService(
                service_key)

            self._migration_source_file_filter.addItem(service.GetName(),
                                                       service_key)

        if self._hashes is not None:

            self._migration_source_file_filter.addItem(
                '{} files'.format(HydrusData.ToHumanInt(len(self._hashes))),
                self.HASHES_SERVICE_KEY)

            self._migration_source_file_filter.SetValue(
                self.HASHES_SERVICE_KEY)

        self._migration_source_file_filter.setToolTip(
            'Tags that pass this filter will be applied to the destination with the chosen action.'
        )
        ...
        self._migration_source_tag_filter = ClientGUITags.TagFilterButton(
            self._migration_panel,
            message,
            tag_filter,
            label_prefix='tags taken: ')
        ...
        self._migration_source_left_tag_pair_filter = ClientGUITags.TagFilterButton(
            self._migration_panel, message, tag_filter, label_prefix='left: ')
        ...
        self._migration_source_right_tag_pair_filter = ClientGUITags.TagFilterButton(
            self._migration_panel, message, tag_filter, label_prefix='right: ')

        self._migration_destination = ClientGUICommon.BetterChoice(
            self._migration_panel)

        self._migration_destination_archive_path_button = ClientGUICommon.BetterButton(
            self._migration_panel, 'no path set',
            self._SetDestinationArchivePath)

        self._migration_destination_hash_type_choice_st = ClientGUICommon.BetterStaticText(
            self._migration_panel, 'hash type: ')

        self._migration_destination_hash_type_choice = ClientGUICommon.BetterChoice(
            self._migration_panel)

        for hash_type in ('sha256', 'md5', 'sha1', 'sha512'):

            self._migration_destination_hash_type_choice.addItem(
                hash_type, hash_type)

        self._migration_destination_hash_type_choice.setToolTip(
            'If you set something other than sha256, this will only work for files the client has ever previously imported.'
        )

        self._migration_action = ClientGUICommon.BetterChoice(
            self._migration_panel)

        self._migration_go = ClientGUICommon.BetterButton(
            self._migration_panel, 'Go!', self._MigrationGo)

        #

        file_left_vbox = QP.VBoxLayout()

        QP.AddToLayout(file_left_vbox, self._migration_source_file_filter,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(file_left_vbox,
                       self._migration_source_left_tag_pair_filter,
                       CC.FLAGS_EXPAND_BOTH_WAYS)

        tag_right_vbox = QP.VBoxLayout()

        QP.AddToLayout(tag_right_vbox, self._migration_source_tag_filter,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(tag_right_vbox,
                       self._migration_source_right_tag_pair_filter,
                       CC.FLAGS_EXPAND_BOTH_WAYS)

        dest_hash_type_hbox = QP.HBoxLayout()

        QP.AddToLayout(dest_hash_type_hbox,
                       self._migration_destination_hash_type_choice_st,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(dest_hash_type_hbox,
                       self._migration_destination_hash_type_choice,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(
            gridbox,
            ClientGUICommon.BetterStaticText(self._migration_panel, 'content'),
            CC.FLAGS_CENTER)
        QP.AddToLayout(
            gridbox,
            ClientGUICommon.BetterStaticText(self._migration_panel, 'source'),
            CC.FLAGS_CENTER)
        QP.AddToLayout(
            gridbox,
            ClientGUICommon.BetterStaticText(self._migration_panel, 'filter'),
            CC.FLAGS_CENTER)
        QP.AddToLayout(
            gridbox,
            ClientGUICommon.BetterStaticText(self._migration_panel, 'action'),
            CC.FLAGS_CENTER)
        QP.AddToLayout(
            gridbox,
            ClientGUICommon.BetterStaticText(self._migration_panel,
                                             'destination'), CC.FLAGS_CENTER)
        QP.AddToLayout(gridbox, (20, 20), CC.FLAGS_CENTER)

        QP.AddToLayout(gridbox, self._migration_content_type,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(gridbox, self._migration_source,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(gridbox, self._migration_source_content_status_filter,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(gridbox, self._migration_action,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(gridbox, self._migration_destination,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(gridbox, self._migration_go, CC.FLAGS_EXPAND_BOTH_WAYS)

        QP.AddToLayout(gridbox, (20, 20), CC.FLAGS_CENTER)
        QP.AddToLayout(gridbox, self._migration_source_archive_path_button,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(gridbox, file_left_vbox,
                       CC.FLAGS_EXPAND_SIZER_BOTH_WAYS)
        QP.AddToLayout(gridbox, (20, 20), CC.FLAGS_CENTER)
        QP.AddToLayout(gridbox,
                       self._migration_destination_archive_path_button,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(gridbox, (20, 20), CC.FLAGS_CENTER)

        QP.AddToLayout(gridbox, (20, 20), CC.FLAGS_CENTER)
        QP.AddToLayout(gridbox, self._migration_source_hash_type_st,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(gridbox, tag_right_vbox,
                       CC.FLAGS_EXPAND_SIZER_BOTH_WAYS)
        QP.AddToLayout(gridbox, (20, 20), CC.FLAGS_CENTER)
        QP.AddToLayout(gridbox, dest_hash_type_hbox,
                       CC.FLAGS_EXPAND_SIZER_BOTH_WAYS)
        QP.AddToLayout(gridbox, (20, 20), CC.FLAGS_CENTER)
        ...
        self._migration_content_type.activated.connect(
            self._UpdateMigrationControlsNewType)
        self._migration_source.activated.connect(
            self._UpdateMigrationControlsNewSource)
        self._migration_destination.activated.connect(
            self._UpdateMigrationControlsNewDestination)
        self._migration_source_content_status_filter.activated.connect(
    ...
    def _MigrationGo(self):
        ...
        source_content_statuses_strings[(
            ...
            HC.CONTENT_STATUS_PENDING)] = 'current and pending'
        source_content_statuses_strings[(
        ...
        destination_action_strings[
            HC.
            CONTENT_UPDATE_CLEAR_DELETE_RECORD] = 'clearing their deletion record from'
        destination_action_strings[HC.CONTENT_UPDATE_PEND] = 'pending them to'
        destination_action_strings[
            HC.CONTENT_UPDATE_PETITION] = 'petitioning them from'
        ...
        if content_type == HC.CONTENT_TYPE_MAPPINGS:
            ...
            if destination_service_key == self.HTA_SERVICE_KEY:
                ...
                if self._dest_archive_path is None:
                    ...
                    QW.QMessageBox.warning(
                        self, 'Warning',
                        'Please set a path for the destination Hydrus Tag Archive.'
                    )
                ...
                destination = ClientMigration.MigrationDestinationHTA(
                    HG.client_controller, self._dest_archive_path,
                    desired_hash_type)
                ...
                destination = ClientMigration.MigrationDestinationTagServiceMappings(
                    HG.client_controller, destination_service_key,
                    content_action)
            ...
            if file_service_key == self.HASHES_SERVICE_KEY:
                ...
                if file_service_key == CC.COMBINED_FILE_SERVICE_KEY:
                    ...
                    file_service_name = HG.client_controller.services_manager.GetName(
                        file_service_key)
            ...
            extra_info += ' and for tags "{}"'.format(
                HydrusText.ElideText(tag_filter.ToPermittedString(), 96))
            ...
            if source_service_key == self.HTA_SERVICE_KEY:
                ...
                if self._source_archive_path is None:
                    ...
                    QW.QMessageBox.warning(
                        self, 'Warning',
                        'Please set a path for the source Hydrus Tag Archive.')

                    return

                source = ClientMigration.MigrationSourceHTA(
                    HG.client_controller, self._source_archive_path,
                    file_service_key, desired_hash_type, hashes, tag_filter)

            else:

                source = ClientMigration.MigrationSourceTagServiceMappings(
                    HG.client_controller, source_service_key, file_service_key,
                    desired_hash_type, hashes, tag_filter, content_statuses)
            ...
            if destination_service_key == self.HTPA_SERVICE_KEY:
                ...
                if self._dest_archive_path is None:
                    ...
                    QW.QMessageBox.warning(
                        self, 'Warning',
                        'Please set a path for the destination Hydrus Tag Pair Archive.'
                    )
            ...
            if left_s == right_s:
                ...
                extra_info = ' for "{}" on the left and "{}" on the right'.format(
                    left_s, right_s)
            ...
            if source_service_key == self.HTPA_SERVICE_KEY:
                ...
                source = ClientMigration.MigrationSourceHTPA(
                    HG.client_controller, self._source_archive_path,
                    left_tag_pair_filter, right_tag_pair_filter)

            else:

                source = ClientMigration.MigrationSourceTagServicePairs(
                    HG.client_controller, source_service_key, content_type,
                    left_tag_pair_filter, right_tag_pair_filter,
                    content_statuses)

        title = 'taking {} {}{} from "{}" and {} "{}"'.format(
            source_content_statuses_strings[content_statuses],
            HC.content_type_string_lookup[content_type], extra_info,
            source.GetName(), destination_action_strings[content_action],
            destination.GetName())
        ...
        if result == QW.QDialog.Accepted:
            ...
            if do_it:
                ...
                migration_job = ClientMigration.MigrationJob(
                    HG.client_controller, title, source, destination)
    ...
    def _SetDestinationArchivePath(self):
        ...
        with QP.FileDialog(self,
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    if content_type == HC.CONTENT_TYPE_MAPPINGS:
                        ...
                        try:
                            ...
                            QW.QMessageBox.critical(
                                self, 'Error',
                                'Could not load that path as an Archive! {}'.
                                format(str(e)))
                        ...
                        try:
                            ...
                            self._migration_destination_hash_type_choice.SetValue(
                                hash_type)

                            self._migration_destination_hash_type_choice.setEnabled(
                                False)
                        ...
                        try:
                            ...
                            QW.QMessageBox.warning(
                                self, 'Warning',
                                'Could not load that file as a Hydrus Tag Pair Archive! {}'
                                .format(str(e)))
                        ...
                        if content_type == HC.CONTENT_TYPE_TAG_PARENTS and pair_type != HydrusTagArchive.TAG_PAIR_TYPE_PARENTS:
                            ...
                            QW.QMessageBox.warning(
                                self, 'Warning',
                                'This Hydrus Tag Pair Archive is not a tag siblings archive!'
                            )
                ...
                self._migration_destination_archive_path_button.setText(
                    filename)
    ...
    def _SetSourceArchivePath(self):
        ...
        with QP.FileDialog(self,
                           message=message,
                           acceptMode=QW.QFileDialog.AcceptOpen) as dlg:
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                if content_type == HC.CONTENT_TYPE_MAPPINGS:
                    ...
                    try:
                        ...
                        QW.QMessageBox.warning(
                            self, 'Warning',
                            'Could not load that file as a Hydrus Tag Archive! {}'
                            .format(str(e)))

                        return

                    hash_type_str = HydrusTagArchive.hash_type_to_str_lookup[
                        hash_type]

                    self._migration_source_hash_type_st.setText(
                        'hash type: {}'.format(hash_type_str))
                    ...
                    if pair_type is None:
                        ...
                        QW.QMessageBox.warning(
                            self, 'Warning',
                            'This Hydrus Tag Pair Archive does not have a pair type set!'
                        )
    ...
    def _UpdateMigrationControlsActions(self):
        ...
        if destination in (self.HTA_SERVICE_KEY, self.HTPA_SERVICE_KEY):
            ...
            destination_service = HG.client_controller.services_manager.GetService(
                destination)
        ...
        for action in actions:
            ...
            self._migration_action.addItem(
                HC.content_update_string_lookup[action], action)
    ...
    def _UpdateMigrationControlsNewSource(self):
        ...
        if source in (self.HTA_SERVICE_KEY, self.HTPA_SERVICE_KEY):
            ...
            self._migration_source_content_status_filter.addItem(
                'current', (HC.CONTENT_STATUS_CURRENT, ))
            ...
            source_service = HG.client_controller.services_manager.GetService(
                source)
            ...
            self._migration_source_content_status_filter.addItem(
                'deleted', (HC.CONTENT_STATUS_DELETED, ))
    ...
    def _UpdateMigrationControlsNewType(self):
        ...
        for service in HG.client_controller.services_manager.GetServices(
            ...
            self._migration_source.addItem(service.GetName(),
                                           service.GetServiceKey())
            self._migration_destination.addItem(service.GetName(),
                                                service.GetServiceKey())
        ...
        if content_type == HC.CONTENT_TYPE_MAPPINGS:
            ...
            self._migration_source.addItem('Hydrus Tag Archive',
                                           self.HTA_SERVICE_KEY)
            self._migration_destination.addItem('Hydrus Tag Archive',
                                                self.HTA_SERVICE_KEY)
            ...
            self._migration_source.addItem('Hydrus Tag Pair Archive',
                                           self.HTPA_SERVICE_KEY)
            self._migration_destination.addItem('Hydrus Tag Pair Archive',
                                                self.HTPA_SERVICE_KEY)
...
class ReviewAllBandwidthPanel(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, controller):
        ...
        self._history_time_delta_threshold = ClientGUITime.TimeDeltaButton(
            self, days=True, hours=True, minutes=True, seconds=True)
        self._history_time_delta_threshold.timeDeltaChanged.connect(
            self.EventTimeDeltaChanged)

        self._history_time_delta_none = QW.QCheckBox('show all', self)
        self._history_time_delta_none.clicked.connect(
            self.EventTimeDeltaChanged)

        columns = [('name', -1), ('type', 14), ('current usage', 14),
                   ('past 24 hours', 15), ('search distance', 17),
                   ('this month', 12), ('uses non-default rules', 24),
                   ('blocked?', 10)]

        self._bandwidths = ClientGUIListCtrl.BetterListCtrl(
            self,
            'bandwidth review',
            20,
            30,
            columns,
            self._ConvertNetworkContextsToListCtrlTuples,
            activation_callback=self.ShowNetworkContext)

        self._edit_default_bandwidth_rules_button = ClientGUICommon.BetterButton(
            self, 'edit default bandwidth rules',
            self._EditDefaultBandwidthRules)

        self._reset_default_bandwidth_rules_button = ClientGUICommon.BetterButton(
            self, 'reset default bandwidth rules',
            self._ResetDefaultBandwidthRules)

        default_rules_help_button = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.help, self._ShowDefaultRulesHelp)
        default_rules_help_button.setToolTip(
            'Show help regarding default bandwidth rules.')

        self._delete_record_button = ClientGUICommon.BetterButton(
            self, 'delete selected history', self._DeleteNetworkContexts)

        #

        last_review_bandwidth_search_distance = self._controller.new_options.GetNoneableInteger(
            'last_review_bandwidth_search_distance')
        ...
        if last_review_bandwidth_search_distance is None:
            ...
            self._history_time_delta_threshold.SetValue(
                last_review_bandwidth_search_distance)

        self._bandwidths.Sort(0)

        self._update_job = HG.client_controller.CallRepeatingQtSafe(
            self, 0.5, 5.0, self._Update)
        ...
        QP.AddToLayout(
            ...
            ClientGUICommon.BetterStaticText(
                self, 'Show network contexts with usage in the past: '),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._history_time_delta_threshold,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(button_hbox, self._edit_default_bandwidth_rules_button,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(button_hbox, self._reset_default_bandwidth_rules_button,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(button_hbox, default_rules_help_button,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(button_hbox, self._delete_record_button,
                       CC.FLAGS_VCENTER)
    ...
    def _ConvertNetworkContextsToListCtrlTuples(self, network_context):
        ...
        bandwidth_tracker = self._controller.network_engine.bandwidth_manager.GetTracker(
            network_context)

        has_rules = not self._controller.network_engine.bandwidth_manager.UsesDefaultRules(
            network_context)

        sortable_network_context = (network_context.context_type,
                                    network_context.context_data)
        sortable_context_type = CC.network_context_type_string_lookup[
            network_context.context_type]
        current_usage = bandwidth_tracker.GetUsage(HC.BANDWIDTH_TYPE_DATA,
                                                   1,
                                                   for_user=True)

        day_usage_requests = bandwidth_tracker.GetUsage(
            HC.BANDWIDTH_TYPE_REQUESTS, 86400)
        day_usage_data = bandwidth_tracker.GetUsage(HC.BANDWIDTH_TYPE_DATA,
                                                    86400)

        day_usage = (day_usage_data, day_usage_requests)

        month_usage_requests = bandwidth_tracker.GetUsage(
            HC.BANDWIDTH_TYPE_REQUESTS, None)
        month_usage_data = bandwidth_tracker.GetUsage(HC.BANDWIDTH_TYPE_DATA,
                                                      None)
        ...
        if self._history_time_delta_none.isChecked():
            ...
            search_usage_requests = bandwidth_tracker.GetUsage(
                HC.BANDWIDTH_TYPE_REQUESTS, search_delta)
            search_usage_data = bandwidth_tracker.GetUsage(
                HC.BANDWIDTH_TYPE_DATA, search_delta)

            search_usage = (search_usage_data, search_usage_requests)

            pretty_search_usage = HydrusData.ToHumanBytes(
                search_usage_data) + ' in ' + HydrusData.ToHumanInt(
                    search_usage_requests) + ' requests'

        pretty_network_context = network_context.ToString()
        pretty_context_type = CC.network_context_type_string_lookup[
            network_context.context_type]
        ...
        pretty_day_usage = HydrusData.ToHumanBytes(
            day_usage_data) + ' in ' + HydrusData.ToHumanInt(
                day_usage_requests) + ' requests'
        pretty_month_usage = HydrusData.ToHumanBytes(
            month_usage_data) + ' in ' + HydrusData.ToHumanInt(
                month_usage_requests) + ' requests'
        ...
        (
            waiting_estimate, network_context_gumpf
        ) = self._controller.network_engine.bandwidth_manager.GetWaitingEstimateAndContext(
            [network_context])

        if waiting_estimate > 0:

            pretty_blocked = HydrusData.TimeDeltaToPrettyTimeDelta(
                waiting_estimate)
        ...
        display_tuple = (pretty_network_context, pretty_context_type,
                         pretty_current_usage, pretty_day_usage,
                         pretty_search_usage, pretty_month_usage,
                         pretty_has_rules, pretty_blocked)
        sort_tuple = (sortable_network_context, sortable_context_type,
                      current_usage, day_usage, search_usage, month_usage,
                      has_rules, waiting_estimate)

        return (display_tuple, sort_tuple)

    def _DeleteNetworkContexts(self):

        selected_network_contexts = self._bandwidths.GetData(
            only_selected=True)
        ...
        if len(selected_network_contexts) > 0:
            ...
            if result == QW.QDialog.Accepted:
                ...
                self._controller.network_engine.bandwidth_manager.DeleteHistory(
                    selected_network_contexts)
    ...
    def _EditDefaultBandwidthRules(self):
        ...
        choice_tuples = [
            (network_context.ToString() + ' (' +
             str(len(bandwidth_rules.GetRules())) + ' rules)',
             (network_context, bandwidth_rules))
            for (network_context,
                 bandwidth_rules) in network_contexts_and_bandwidth_rules
        ]

        try:

            (network_context,
             bandwidth_rules) = ClientGUIDialogsQuick.SelectFromList(
                 self, 'select network context', choice_tuples)
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit bandwidth rules for ' +
                network_context.ToString()) as dlg_2:

            summary = network_context.GetSummary()

            panel = ClientGUIScrolledPanelsEdit.EditBandwidthRulesPanel(
                dlg_2, bandwidth_rules, summary)
            ...
            if dlg_2.exec() == QW.QDialog.Accepted:
                ...
                self._controller.network_engine.bandwidth_manager.SetRules(
                    network_context, bandwidth_rules)
    ...
    def _ResetDefaultBandwidthRules(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            ClientDefaults.SetDefaultBandwidthManagerRules(
                self._controller.network_engine.bandwidth_manager)
    ...
    def _Update(self):
        ...
        network_contexts = self._controller.network_engine.bandwidth_manager.GetNetworkContextsForUser(
            history_time_delta_threshold)
    ...
    def EventTimeDeltaChanged(self):
        ...
        self._controller.new_options.SetNoneableInteger(
            'last_review_bandwidth_search_distance',
            last_review_bandwidth_search_distance)
    ...
    def ShowNetworkContext(self):
        ...
        for network_context in self._bandwidths.GetData(only_selected=True):
            ...
            frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
                parent, 'review bandwidth for ' + network_context.ToString())

            panel = ReviewNetworkContextBandwidthPanel(frame, self._controller,
                                                       network_context)
...
class ReviewDownloaderImport(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, network_engine):
        ...
        page_func = HydrusData.Call(
            ClientPaths.LaunchPathInWebBrowser,
            os.path.join(HC.HELP_DIR, 'adding_new_downloaders.html'))

        menu_items.append((
            'normal', 'open the easy downloader import help',
            'Open the help page for easily importing downloaders in your web browser.',
            page_func))
        ...
        st = ClientGUICommon.BetterStaticText(
            self, label='Drop downloader-encoded pngs onto Lain to import.')

        lain_path = os.path.join(HC.STATIC_DIR, 'lain.jpg')

        lain_qt_pixmap = ClientRendering.GenerateHydrusBitmap(
            lain_path, HC.IMAGE_JPEG).GetQtPixmap()
        ...
        QP.AddToLayout(
            vbox,
            ClientGUICommon.WrapInText(self._select_from_list, self,
        ...
        win.installEventFilter(
            ClientDragDrop.FileDropTarget(
                win, filenames_callable=self.ImportFromDragDrop))
    ...
    def _ImportPaths(self, paths):
        ...
        for path in paths:
            ...
            try:
                ...
                QW.QMessageBox.critical(
                    self, 'Error',
                    'I could not understand what was encoded in the file ' +
                    path + '!')
            ...
            if isinstance(
                    obj_list,
                (ClientNetworkingDomain.GalleryURLGenerator,
                 ClientNetworkingDomain.NestedGalleryURLGenerator,
                 ClientNetworkingDomain.URLClass, ClientParsing.PageParser,
                 ClientNetworkingDomain.DomainMetadataPackage,
                 ClientNetworkingLogin.LoginScriptDomain)):
            ...
            if not isinstance(obj_list, HydrusSerialisable.SerialisableList):
                ...
                QW.QMessageBox.warning(
                    self, 'Warning', 'Unfortunately, ' + path +
                    ' did not look like a package of download data! Instead, it looked like: '
                    + obj_list.SERIALISABLE_NAME)
        ...
        if len(obj_list) - num_misc_objects == 0:
            ...
            if num_misc_objects > 0:
                ...
                QW.QMessageBox.warning(
                    self, 'Warning',
                    'I found ' + HydrusData.ToHumanInt(num_misc_objects) +
                    ' misc objects in that png, but nothing downloader related.'
                )
        ...
        for domain_metadata in domain_metadatas:
            ...
            nc = ClientNetworkingContexts.NetworkContext(
            ...
            if domain_metadata.HasHeaders():
                ...
                if domain_manager.AlreadyHaveExactlyTheseHeaders(
                        nc, headers_list):
            ...
            if domain_metadata.HasBandwidthRules():
                ...
                if bandwidth_manager.AlreadyHaveExactlyTheseBandwidthRules(
                        nc, bandwidth_rules):
            ...
            if headers_list is None and bandwidth_rules is None:
                ...
                new_dm = ClientNetworkingDomain.DomainMetadataPackage(
                    domain=domain,
        ...
        if len(new_gugs) + len(new_url_classes) + len(new_parsers) + len(
                new_domain_metadatas) + len(new_login_scripts) == 0:

            QW.QMessageBox.information(
                self, 'Information',
                'All ' + HydrusData.ToHumanInt(total_num_dupes) +
                ' downloader objects in that package appeared to already be in the client, so nothing need be added.'
            )
        ...
        if self._select_from_list.isChecked():
            ...
            choice_tuples = []
            choice_tuples.extend([('GUG: ' + gug.GetName(), gug, True)
                                  for gug in new_gugs])
            choice_tuples.extend([('URL Class: ' + url_class.GetName(),
                                   url_class, True)
                                  for url_class in new_url_classes])
            choice_tuples.extend([('Parser: ' + parser.GetName(), parser, True)
                                  for parser in new_parsers])
            choice_tuples.extend([('Login Script: ' + login_script.GetName(),
                                   login_script, True)
                                  for login_script in new_login_scripts])
            choice_tuples.extend([
                ('Domain Metadata: ' + domain_metadata.GetDomain(),
                 domain_metadata, True)
                for domain_metadata in new_domain_metadatas
            ])

            with ClientGUITopLevelWindows.DialogEdit(
                    self, 'select objects to add') as dlg:
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    new_url_classes = [
                        obj for obj in new_objects
                        if isinstance(obj, ClientNetworkingDomain.URLClass)
                    ]
                    new_parsers = [
                        obj for obj in new_objects
                    ...
                    new_login_scripts = [
                        obj for obj in new_objects if isinstance(
                            obj, ClientNetworkingLogin.LoginScriptDomain)
                    ]
                    new_domain_metadatas = [
                        obj for obj in new_objects if isinstance(
                            obj, ClientNetworkingDomain.DomainMetadataPackage)
                    ]
        ...
        if len(new_domain_metadatas) > 0:
            ...
            if len(new_domain_metadatas) > TOO_MANY_DM:
                ...
                message += os.linesep * 2
                message += 'There are more than ' + HydrusData.ToHumanInt(
                    TOO_MANY_DM
                ) + ' domain metadata objects. So I do not give you dozens of preview windows, I will only show you these first ' + HydrusData.ToHumanInt(
                    TOO_MANY_DM) + '.'
        ...
        message += os.linesep.join(
            (obj.GetSafeSummary() for obj in all_to_add[:20]))

        if len(all_to_add) > 20:

            message += os.linesep
            message += '(and ' + HydrusData.ToHumanInt(len(all_to_add) -
                                                       20) + ' others)'
        ...
        domain_manager.AutoAddURLClassesAndParsers(new_url_classes,
                                                   dupe_url_classes,
                                                   new_parsers)

        bandwidth_manager.AutoAddDomainMetadatas(new_domain_metadatas)
        domain_manager.AutoAddDomainMetadatas(new_domain_metadatas,
                                              approved=True)
        login_manager.AutoAddLoginScripts(new_login_scripts)

        num_new_gugs = len(new_gugs)
        num_aux = len(new_url_classes) + len(new_parsers) + len(
            new_login_scripts) + len(new_domain_metadatas)

        final_message = 'Successfully added ' + HydrusData.ToHumanInt(
            num_new_gugs) + ' new downloaders and ' + HydrusData.ToHumanInt(
                num_aux) + ' auxiliary objects.'

        if total_num_dupes > 0:

            final_message += ' ' + HydrusData.ToHumanInt(
                total_num_dupes
            ) + ' duplicate objects were not added (but some additional metadata may have been merged).'
    ...
    def EventLainClick(self, event):
        ...
        with QP.FileDialog(self,
                           'Select the pngs to add.',
                           acceptMode=QW.QFileDialog.AcceptOpen,
                           fileMode=QW.QFileDialog.ExistingFiles) as dlg:
...
class ReviewFileMaintenance(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, stats):
        ...
        jobs_listctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(
            self._current_work_panel)

        columns = [('job type', -1), ('scheduled jobs', 16)]

        self._jobs_listctrl = ClientGUIListCtrl.BetterListCtrl(
            jobs_listctrl_panel, 'file maintenance jobs', 8, 48, columns,
            self._ConvertJobTypeToListCtrlTuples)

        jobs_listctrl_panel.SetListCtrl(self._jobs_listctrl)

        jobs_listctrl_panel.AddButton('clear',
                                      self._DeleteWork,
                                      enabled_only_on_selection=True)
        jobs_listctrl_panel.AddButton('do work',
                                      self._DoWork,
                                      enabled_only_on_selection=True)
        jobs_listctrl_panel.AddButton('do all work',
                                      self._DoAllWork,
                                      enabled_check_func=self._WorkToDo)
        ...
        self._search_panel = ClientGUICommon.StaticBox(
            self._new_work_panel, 'select files by search')

        page_key = HydrusData.GenerateKey()

        self._current_predicates_box = ClientGUIListBoxes.ListBoxTagsActiveSearchPredicates(
            self._search_panel, page_key, [])

        file_search_context = ClientSearch.FileSearchContext(
            file_service_key=CC.LOCAL_FILE_SERVICE_KEY)

        self._tag_ac_input = ClientGUIACDropdown.AutoCompleteDropdownTagsRead(
            self._search_panel, page_key, file_search_context)

        self._run_search_st = ClientGUICommon.BetterStaticText(
            self._search_panel, label='no results yet')

        self._run_search = ClientGUICommon.BetterButton(
            self._search_panel, 'run this search', self._RunSearch)
        ...
        self._search_panel.Add(self._current_predicates_box,
                               CC.FLAGS_EXPAND_BOTH_WAYS)
        self._search_panel.Add(self._tag_ac_input,
                               CC.FLAGS_EXPAND_PERPENDICULAR)
        self._search_panel.Add(hbox, CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)

        #

        self._button_panel = ClientGUICommon.StaticBox(self._new_work_panel,
                                                       'select special files')

        self._select_repo_files = ClientGUICommon.BetterButton(
            self._button_panel, 'all repository update files',
            self._SelectRepoUpdateFiles)

        self._button_panel.Add(self._select_repo_files, CC.FLAGS_LONE_BUTTON)

        #

        self._action_panel = ClientGUICommon.StaticBox(self._new_work_panel,
                                                       'add job')

        self._selected_files_st = ClientGUICommon.BetterStaticText(
            self._action_panel, label='no files selected yet')

        self._action_selector = ClientGUICommon.BetterChoice(
            self._action_panel)

        for job_type in ClientFiles.ALL_REGEN_JOBS_IN_PREFERRED_ORDER:

            self._action_selector.addItem(
                ClientFiles.regen_file_enum_to_str_lookup[job_type], job_type)

        self._description_button = ClientGUICommon.BetterButton(
            self._action_panel, 'see description', self._SeeDescription)

        self._add_new_job = ClientGUICommon.BetterButton(
            self._action_panel, 'add job', self._AddJob)

        self._add_new_job.setEnabled(False)

        hbox = QP.HBoxLayout()

        QP.AddToLayout(hbox, self._selected_files_st,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(
            vbox,
            ClientGUICommon.BetterStaticText(self._new_work_panel,
                                             label=label),
            CC.FLAGS_EXPAND_PERPENDICULAR)
        ...
        HG.client_controller.sub(self, '_RefreshWorkDue',
                                 'notify_files_maintenance_done')
    ...
    def _AddJob(self):
        ...
        def do_it(hash_ids, job_type):
            ...
            HG.client_controller.files_maintenance_manager.ScheduleJobHashIds(
                hash_ids, job_type)
        ...
        if len(hash_ids) > 1000:
            ...
            message = 'Are you sure you want to schedule "{}" on {} files?'.format(
                ClientFiles.regen_file_enum_to_str_lookup[job_type],
                HydrusData.ToHumanInt(len(hash_ids)))
    ...
    def _DeleteWork(self):
        ...
        def do_it(job_types):
            ...
            for job_type in job_types:
                ...
                HG.client_controller.files_maintenance_manager.CancelJobs(
                    job_type)
    ...
    def _DoAllWork(self):
        ...
        HG.client_controller.CallToThread(
            HG.client_controller.files_maintenance_manager.ForceMaintenance)
    ...
    def _RefreshWorkDue(self):
        ...
        def do_it():
            ...
            job_types_to_counts = HG.client_controller.Read(
                'file_maintenance_get_job_counts')
    ...
    def _RunSearch(self):
        def qt_done(hash_ids):
            ...
            self._run_search_st.setText('{} files found'.format(
                HydrusData.ToHumanInt(len(hash_ids))))
    ...
    def _SeeDescription(self):
        ...
        message += os.linesep * 2
        message += 'This job has weight {}, where a normalised unit of file work has value {}.'.format(
            HydrusData.ToHumanInt(
                ClientFiles.regen_file_enum_to_job_weight_lookup[job_type]),
            HydrusData.ToHumanInt(ClientFiles.NORMALISED_BIG_JOB_WEIGHT))
    ...
    def _SelectRepoUpdateFiles(self):
        ...
        file_search_context = ClientSearch.FileSearchContext(
            file_service_key=CC.LOCAL_UPDATE_SERVICE_KEY)
    ...
    def _SetHashIds(self, hash_ids):
        ...
        self._selected_files_st.setText('{} files selected'.format(
            HydrusData.ToHumanInt(len(hash_ids))))
...
class ReviewHowBonedAmI(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, boned_stats):
        ...
        if num_total < 1000:
            ...
            get_more = ClientGUICommon.BetterStaticText(
                self,
                label=
                'I hope you enjoy my software. You might like to check out the downloaders! :^)'
            )
            ...
            hooray = ClientGUICommon.BetterStaticText(
                self,
                label=
            ...
            boned_qt_pixmap = ClientRendering.GenerateHydrusBitmap(
                boned_path, HC.IMAGE_JPEG).GetQtPixmap()
        ...
        if num_total == 0:
            ...
            nothing_st = ClientGUICommon.BetterStaticText(self,
                                                          label=nothing_label)
            ...
            summary_label = 'Total: {} files, totalling {}, averaging {}'.format(
                HydrusData.ToHumanInt(num_total),
                HydrusData.ToHumanBytes(size_total),
                HydrusData.ToHumanBytes(average_filesize))

            summary_st = ClientGUICommon.BetterStaticText(self,
                                                          label=summary_label)
            ...
            archive_label = 'Archive: ' + HydrusData.ToHumanInt(
                num_archive) + ' files (' + ClientData.ConvertZoomToPercentage(
                    num_archive_percent
                ) + '), totalling ' + HydrusData.ToHumanBytes(
                    size_archive) + '(' + ClientData.ConvertZoomToPercentage(
                        size_archive_percent) + ')'

            archive_st = ClientGUICommon.BetterStaticText(self,
                                                          label=archive_label)

            inbox_label = 'Inbox: ' + HydrusData.ToHumanInt(
                num_inbox) + ' files (' + ClientData.ConvertZoomToPercentage(
                    num_inbox_percent
                ) + '), totalling ' + HydrusData.ToHumanBytes(
                    size_inbox) + '(' + ClientData.ConvertZoomToPercentage(
                        size_inbox_percent) + ')'

            inbox_st = ClientGUICommon.BetterStaticText(self,
                                                        label=inbox_label)

            QP.AddToLayout(vbox, archive_st, CC.FLAGS_CENTER)
            QP.AddToLayout(vbox, inbox_st, CC.FLAGS_CENTER)

            (media_views, media_viewtime, preview_views,
             preview_viewtime) = total_viewtime

            media_label = 'Total media views: ' + HydrusData.ToHumanInt(
                media_views
            ) + ', totalling ' + HydrusData.TimeDeltaToPrettyTimeDelta(
                media_viewtime)

            media_st = ClientGUICommon.BetterStaticText(self,
                                                        label=media_label)

            preview_label = 'Total preview views: ' + HydrusData.ToHumanInt(
                preview_views
            ) + ', totalling ' + HydrusData.TimeDeltaToPrettyTimeDelta(
                preview_viewtime)

            preview_st = ClientGUICommon.BetterStaticText(self,
                                                          label=preview_label)

            QP.AddToLayout(vbox, media_st, CC.FLAGS_CENTER)
            QP.AddToLayout(vbox, preview_st, CC.FLAGS_CENTER)

            potentials_label = 'Total duplicate potential pairs: {}'.format(
                HydrusData.ToHumanInt(total_potential_pairs))
            duplicates_label = 'Total files set duplicate: {}'.format(
                HydrusData.ToHumanInt(total_duplicate_files))
            alternates_label = 'Total duplicate file groups set alternate: {}'.format(
                HydrusData.ToHumanInt(total_alternate_files))

            potentials_st = ClientGUICommon.BetterStaticText(
                self, label=potentials_label)
            duplicates_st = ClientGUICommon.BetterStaticText(
                self, label=duplicates_label)
            alternates_st = ClientGUICommon.BetterStaticText(
                self, label=alternates_label)
...
class ReviewLocalFileImports(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, paths=None):
        ...
        self.widget().installEventFilter(
            ClientDragDrop.FileDropTarget(
                self.widget(), filenames_callable=self._AddPathsToList))

        listctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(self)

        columns = [('#', 4), ('path', -1), ('guessed mime', 16), ('size', 10)]

        self._paths_list = ClientGUIListCtrl.BetterListCtrl(
            listctrl_panel,
            'input_local_files',
            12,
            98,
            columns,
            self._ConvertListCtrlDataToTuple,
            delete_key_callback=self.RemovePaths)
        ...
        listctrl_panel.AddButton('remove files',
                                 self.RemovePaths,
                                 enabled_only_on_selection=True)

        self._progress = ClientGUICommon.TextAndGauge(self)

        self._progress_pause = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.pause, self.PauseProgress)
        self._progress_pause.setEnabled(False)

        self._progress_cancel = ClientGUICommon.BetterBitmapButton(
            self, CC.GlobalPixmaps.stop, self.StopProgress)
        ...
        check_manager = ClientGUICommon.CheckboxManagerOptions(
            'do_human_sort_on_hdd_file_import_paths')

        menu_items.append((
            'check', 'sort paths as they are added',
            'If checked, paths will be sorted in a numerically human-friendly (e.g. "page 9.jpg" comes before "page 10.jpg") way.',
            check_manager))

        self._cog_button = ClientGUICommon.MenuBitmapButton(
            self, CC.GlobalPixmaps.cog, menu_items)

        self._delete_after_success_st = ClientGUICommon.BetterStaticText(self)
        self._delete_after_success_st.setAlignment(QC.Qt.AlignRight
                                                   | QC.Qt.AlignVCenter)

        QP.SetForegroundColour(self._delete_after_success_st, (127, 0, 0))

        self._delete_after_success = QW.QCheckBox(
            'delete original files after successful import', self)
        self._delete_after_success.clicked.connect(
            self.EventDeleteAfterSuccessCheck)

        self._add_button = ClientGUICommon.BetterButton(
            self, 'import now', self._DoImport)
        QP.SetForegroundColour(self._add_button, (0, 128, 0))

        self._tag_button = ClientGUICommon.BetterButton(
            self, 'import with tags', self._AddTags)
        QP.SetForegroundColour(self._tag_button, (0, 128, 0))

        gauge_sizer = QP.HBoxLayout()

        QP.AddToLayout(gauge_sizer, self._progress,
                       CC.FLAGS_EXPAND_SIZER_BOTH_WAYS)
        ...
        QP.AddToLayout(delete_hbox, self._delete_after_success_st,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        QP.AddToLayout(delete_hbox, self._delete_after_success,
                       CC.FLAGS_VCENTER)

        import_options_buttons = QP.HBoxLayout()

        QP.AddToLayout(import_options_buttons, self._file_import_options,
                       CC.FLAGS_SIZER_VCENTER)
        QP.AddToLayout(import_options_buttons, self._cog_button,
                       CC.FLAGS_SIZER_VCENTER)
        ...
        self._progress_updater = ClientGUIAsync.FastThreadToGUIUpdater(
            self._progress, self._progress.SetValue)
        ...
        HG.client_controller.CallToThreadLongRunning(
            self.THREADParseImportablePaths, self._unparsed_paths_queue,
            self._currently_parsing, self._work_to_do, self._parsed_path_queue,
            self._progress_updater, self._pause_event, self._cancel_event)
    ...
    def _AddTags(self):
        ...
        if len(paths) > 0:
            ...
            with ClientGUITopLevelWindows.DialogEdit(
                    self,
                    'filename tagging',
                    frame_key='local_import_filename_tagging') as dlg:

                panel = ClientGUIImport.EditLocalImportFilenameTaggingPanel(
                    dlg, paths)
                ...
                if dlg.exec() == QW.QDialog.Accepted:
                    ...
                    HG.client_controller.pub('new_hdd_import', paths,
                                             file_import_options,
    ...
    def _DoImport(self):
        ...
        if len(paths) > 0:
            ...
            paths_to_service_keys_to_tags = collections.defaultdict(
    ...
    def RemovePaths(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            flat_path_data = [(index, path, mime, size)
                              for (path,
                                   (index, mime,
                                    size)) in self._current_path_data.items()]
    ...
    def THREADParseImportablePaths(self, unparsed_paths_queue,
                                   currently_parsing, work_to_do,
                                   parsed_path_queue, progress_updater,
        ...
        while not HG.view_shutdown:
            ...
            if num_good_files == 0:
                ...
                if num_files_done == 0:
                    ...
                    message = 'none of the ' + HydrusData.ToHumanInt(
                        total_paths) + ' files parsed successfully'

            else:

                message = HydrusData.ConvertValueRangeToPrettyString(
                    num_good_files, total_paths) + ' files parsed successfully'
            ...
            if num_empty_files > 0 or num_unimportable_mime_files > 0 or num_occupied_files > 0:
                ...
                if num_empty_files > 0:
                    ...
                    bad_comments.append(
                        HydrusData.ToHumanInt(num_empty_files) + ' were empty')

                if num_unimportable_mime_files > 0:

                    bad_comments.append(
                        HydrusData.ToHumanInt(num_unimportable_mime_files) +
                        ' had unsupported file types')

                if num_occupied_files > 0:

                    bad_comments.append(
                        HydrusData.ToHumanInt(num_occupied_files) +
            ...
            do_human_sort = HG.client_controller.new_options.GetBoolean(
                'do_human_sort_on_hdd_file_import_paths')
            ...
            while not unparsed_paths_queue.empty():
                ...
                try:
                    ...
                    paths = ClientFiles.GetAllFilePaths(
                        raw_paths, do_human_sort=do_human_sort
                    )  # convert any dirs to subpaths
            ...
            if path.endswith(os.path.sep +
                             'Thumbs.db') or path.endswith(os.path.sep +
                                                           'thumbs.db'):
    ...
    def TIMERUIUpdate(self):
        ...
        can_import = files_in_list and (no_work_in_queue or
                                        paused) and not working_on_a_file_now
        ...
        if paused:
            ...
            ClientGUIFunctions.SetBitmapButtonBitmap(self._progress_pause,
                                                     CC.GlobalPixmaps.play)

        else:

            ClientGUIFunctions.SetBitmapButtonBitmap(self._progress_pause,
                                                     CC.GlobalPixmaps.pause)
...
class ReviewNetworkContextBandwidthPanel(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, controller, network_context):
        ...
        self._bandwidth_rules = self._controller.network_engine.bandwidth_manager.GetRules(
            self._network_context)
        self._bandwidth_tracker = self._controller.network_engine.bandwidth_manager.GetTracker(
            self._network_context)
        ...
        description = CC.network_context_type_description_lookup[
            self._network_context.context_type]

        self._name = ClientGUICommon.BetterStaticText(
            info_panel, label=self._network_context.ToString())
        self._description = ClientGUICommon.BetterStaticText(info_panel,
                                                             label=description)
        ...
        self._time_delta_usage_bandwidth_type = ClientGUICommon.BetterChoice(
            usage_panel)
        self._time_delta_usage_time_delta = ClientGUITime.TimeDeltaButton(
            usage_panel, days=True, hours=True, minutes=True, seconds=True)
        self._time_delta_usage_st = ClientGUICommon.BetterStaticText(
            usage_panel)
        ...
        self._uses_default_rules_st = ClientGUICommon.BetterStaticText(
            rules_panel)
        self._uses_default_rules_st.setAlignment(QC.Qt.AlignVCenter
                                                 | QC.Qt.AlignHCenter)
        ...
        self._use_default_rules_button = ClientGUICommon.BetterButton(
            rules_panel, 'use default rules', self._UseDefaultRules)
        self._edit_rules_button = ClientGUICommon.BetterButton(
            rules_panel, 'edit rules', self._EditRules)

        #

        self._time_delta_usage_time_delta.SetValue(86400)

        for bandwidth_type in (HC.BANDWIDTH_TYPE_DATA,
                               HC.BANDWIDTH_TYPE_REQUESTS):

            self._time_delta_usage_bandwidth_type.addItem(
                HC.bandwidth_type_string_lookup[bandwidth_type],
                bandwidth_type)
        ...
        if len(monthly_usage) > 0:
            ...
            if ClientGUICharts.QT_CHARTS_OK:
                ...
                self._barchart_canvas = ClientGUICharts.BarChartBandwidthHistory(
                    usage_panel, monthly_usage)

                self._barchart_canvas.setMinimumSize(640, 480)

            else:

                self._barchart_canvas = ClientGUICommon.BetterStaticText(
                    usage_panel,
                    'QtCharts not available, so no bandwidth chart here.')

        else:

            self._barchart_canvas = ClientGUICommon.BetterStaticText(
                usage_panel, 'No usage yet, so no usage history to show.')
        ...
        QP.AddToLayout(hbox, self._time_delta_usage_bandwidth_type,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(
            hbox, ClientGUICommon.BetterStaticText(usage_panel,
                                                   ' in the past '),
            CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._time_delta_usage_time_delta,
                       CC.FLAGS_VCENTER)
        QP.AddToLayout(hbox, self._time_delta_usage_st,
                       CC.FLAGS_EXPAND_BOTH_WAYS)
        ...
        QP.AddToLayout(hbox, self._use_default_rules_button,
                       CC.FLAGS_SIZER_VCENTER)

        rules_panel.Add(self._uses_default_rules_st,
                        CC.FLAGS_EXPAND_PERPENDICULAR)
        rules_panel.Add(self._rules_rows_panel,
                        CC.FLAGS_EXPAND_SIZER_PERPENDICULAR)
        ...
        self._rules_job = HG.client_controller.CallRepeatingQtSafe(
            self, 0.5, 5.0, self._UpdateRules)

        self._update_job = HG.client_controller.CallRepeatingQtSafe(
            self, 0.5, 1.0, self._Update)

    def _EditRules(self):

        with ClientGUITopLevelWindows.DialogEdit(
                self, 'edit bandwidth rules for ' +
                self._network_context.ToString()) as dlg:

            summary = self._network_context.GetSummary()

            panel = ClientGUIScrolledPanelsEdit.EditBandwidthRulesPanel(
                dlg, self._bandwidth_rules, summary)
            ...
            if dlg.exec() == QW.QDialog.Accepted:
                ...
                self._controller.network_engine.bandwidth_manager.SetRules(
                    self._network_context, self._bandwidth_rules)

                self._UpdateRules()

    def _Update(self):

        current_usage = self._bandwidth_tracker.GetUsage(
            HC.BANDWIDTH_TYPE_DATA, 1, for_user=True)

        pretty_current_usage = 'current usage: ' + HydrusData.ToHumanBytes(
            current_usage) + '/s'
        ...
        time_delta_usage = self._bandwidth_tracker.GetUsage(
            bandwidth_type, time_delta)
    ...
    def _UpdateRules(self):
        ...
        if self._network_context.IsDefault(
        ) or self._network_context == ClientNetworkingContexts.GLOBAL_NETWORK_CONTEXT:
            ...
            if self._controller.network_engine.bandwidth_manager.UsesDefaultRules(
                    self._network_context):
        ...
        rule_rows = self._bandwidth_rules.GetBandwidthStringsAndGaugeTuples(
            self._bandwidth_tracker, threshold=0)
    ...
    def _UseDefaultRules(self):
        ...
        if result == QW.QDialog.Accepted:
            ...
            self._controller.network_engine.bandwidth_manager.DeleteRules(
                self._network_context)
...
class ReviewNetworkJobs(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, controller):
        ...
        columns = [('position', 20), ('url', -1), ('status', 40),
                   ('current speed', 8), ('progress', 12)]

        self._list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._list_ctrl_panel, 'network jobs review', 20, 30, columns,
        ...
        self._list_ctrl_panel.AddButton('refresh snapshot',
    ...
    def _ConvertDataToListCtrlTuples(self, job_row):
        ...
        url = job.GetURL()
        (status, current_speed, num_bytes_read,
         num_bytes_to_read) = job.GetStatus()
        ...
        pretty_progress = HydrusData.ConvertValueRangeToBytes(
            num_bytes_read, num_bytes_to_read)

        display_tuple = (pretty_position, pretty_url, pretty_status,
                         pretty_current_speed, pretty_progress)
...
class ReviewNetworkSessionsPanel(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, session_manager):
        ...
        self._listctrl = ClientGUIListCtrl.BetterListCtrl(
            listctrl_panel,
            'review_network_sessions',
            32,
            34,
            columns,
            self._ConvertNetworkContextToListCtrlTuples,
            delete_key_callback=self._Clear,
            activation_callback=self._Review)
        ...
        listctrl_panel.AddButton(
            'import cookies.txt (drag and drop also works!)',
            self._ImportCookiesTXT)
        listctrl_panel.AddButton('review',
                                 self._Review,
                                 enabled_only_on_selection=True)
        listctrl_panel.AddButton('clear',
                                 self._Clear,
                                 enabled_only_on_selection=True)
        listctrl_panel.AddSeparator()
        listctrl_panel.AddButton('refresh', self._Update)

        listctrl_panel.installEventFilter(
            ClientDragDrop.FileDropTarget(
                listctrl_panel,
                filenames_callable=self._ImportCookiesTXTPaths))
    ...
    def _Add(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(
                self, 'enter new network context') as dlg:

            network_context = ClientNetworkingContexts.NetworkContext(
                CC.NETWORK_CONTEXT_DOMAIN, 'example.com')

            panel = ClientGUIScrolledPanelsEdit.EditNetworkContextPanel(
                dlg,
                network_context,
                limited_types=(CC.NETWORK_CONTEXT_DOMAIN,
    ...
    def _Clear(self):
        ...
        result = ClientGUIDialogsQuick.GetYesNo(
            self, 'Clear these sessions? This will delete them completely.')
    ...
    def _ConvertNetworkContextToListCtrlTuples(self, network_context):
        ...
        expires_numbers = [
            c.expires for c in session.cookies if c.expires is not None
        ]
        ...
        if len(expires_numbers) == 0:
            ...
            try:
                ...
                pretty_expiry = HydrusData.ConvertTimestampToPrettyExpires(
                    expiry)
        ...
        display_tuple = (pretty_network_context, pretty_number_of_cookies,
                         pretty_expiry)
    ...
    def _ImportCookiesTXT(self):
        ...
        with QP.FileDialog(self,
                           'select cookies.txt',
                           acceptMode=QW.QFileDialog.AcceptOpen) as f_dlg:
    ...
    def _ImportCookiesTXTPaths(self, paths):
        ...
        for path in paths:
            ...
            try:
                ...
                QW.QMessageBox.critical(
                    self, 'Error',
                    'It looks like that cookies.txt failed to load. Unfortunately, not all formats are supported (for now!).'
                )
            ...
            for cookie in cj:
                ...
                session = self._session_manager.GetSessionForDomain(
                    cookie.domain)
        ...
        QW.QMessageBox.information(
            self, 'Information',
            'Added ' + HydrusData.ToHumanInt(num_added) + ' cookies!')
    ...
    def _Review(self):
        ...
        for network_context in self._listctrl.GetData(only_selected=True):
            ...
            frame = ClientGUITopLevelWindows.FrameThatTakesScrollablePanel(
                parent, 'review session for ' + network_context.ToString())

            panel = ReviewNetworkSessionPanel(frame, self._session_manager,
                                              network_context)
    ...
    def _Update(self):
        ...
        network_contexts = [
            network_context
            for network_context in self._session_manager.GetNetworkContexts()
            if network_context.context_type in (CC.NETWORK_CONTEXT_DOMAIN,
                                                CC.NETWORK_CONTEXT_HYDRUS)
        ]
...
class ReviewNetworkSessionPanel(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, session_manager, network_context):
        ...
        self._description = ClientGUICommon.BetterStaticText(
            self, network_context.ToString())

        listctrl_panel = ClientGUIListCtrl.BetterListCtrlPanel(self)

        columns = [('name', -1), ('value', 32), ('domain', 20), ('path', 8),
                   ('expires', 28)]

        self._listctrl = ClientGUIListCtrl.BetterListCtrl(
            listctrl_panel,
            'review_network_session',
            8,
            18,
            columns,
            self._ConvertCookieToListCtrlTuples,
    ...
    def _Add(self):
        ...
        with ClientGUITopLevelWindows.DialogEdit(self, 'edit cookie') as dlg:
            ...
            panel = ClientGUIScrolledPanelsEdit.EditCookiePanel(
                dlg, name, value, domain, path, expires)
    ...
    def _ConvertCookieToListCtrlTuples(self, cookie):
        ...
        display_tuple = (pretty_name, pretty_value, pretty_domain, pretty_path,
                         pretty_expiry)
    ...
    def _Delete(self):
        ...
        result = ClientGUIDialogsQuick.GetYesNo(
            self, 'Delete all selected cookies?')
    ...
    def _InitialiseServices(self):
        ...
        for service in services:
            ...
            if service_type in HC.LOCAL_SERVICES:
                parent_notebook = self._local_notebook
            else:
                parent_notebook = self._remote_notebook
            ...
            if service_type_name not in notebook_dict:
                ...
                services_notebook = ClientGUICommon.BetterNotebook(
                    parent_notebook)
            ...
            page = ClientGUIPanels.ReviewServicePanel(services_notebook,
                                                      service)
            ...
            if select:
                services_notebook.setCurrentIndex(services_notebook.count() -
                                                  1)
...
class ReviewThreads(ClientGUIScrolledPanels.ReviewPanel):
    def __init__(self, parent, controller):
        ...
        self._list_ctrl = ClientGUIListCtrl.BetterListCtrl(
            self._list_ctrl_panel, 'threads review', 20, 30, columns,
            self._ConvertDataToListCtrlTuples)
