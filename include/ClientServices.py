...
from . import (ClientDownloading, ClientFiles, ClientImporting,
               ClientNetworkingContexts, ClientNetworkingJobs, ClientRatings,
               ClientThreading)
from . import HydrusConstants as HC
from . import HydrusData, HydrusExceptions
from . import HydrusGlobals as HG
from . import (HydrusNATPunch, HydrusNetwork, HydrusNetworking, HydrusPaths,
               HydrusSerialisable)
...
def GenerateDefaultServiceDictionary(service_type):
    ...
    if service_type in HC.REMOTE_SERVICES:
        ...
        dictionary['credentials'] = HydrusNetwork.Credentials(
            'hostname', 45871)
        ...
        if service_type in HC.RESTRICTED_SERVICES:
            ...
            dictionary[
                'account'] = HydrusNetwork.Account.GenerateSerialisableTupleFromAccount(
                    HydrusNetwork.Account.GenerateUnknownAccount())
    ...
    if service_type in HC.RATINGS_SERVICES:
        ...
        if service_type == HC.LOCAL_RATING_LIKE:
            ...
            dictionary['colours'] = list(
                ClientRatings.default_like_colours.items())

        elif service_type == HC.LOCAL_RATING_NUMERICAL:

            dictionary['colours'] = list(
                ClientRatings.default_numerical_colours.items())
...
class Service(object):
    ...
    def _LoadFromDictionary(self, dictionary):
        ...
        default_dictionary = GenerateDefaultServiceDictionary(
            self._service_type)
...
class ServiceLocalServerService(Service):
    ...
    def _GetSerialisableDictionary(self):
        ...
        dictionary[
            'allow_non_local_connections'] = self._allow_non_local_connections
    ...
    def _LoadFromDictionary(self, dictionary):
        ...
        self._allow_non_local_connections = dictionary[
    ...
    def BandwidthOK(self):
        ...
        with self._lock:
            ...
            return self._bandwidth_rules.CanStartRequest(
                self._bandwidth_tracker)
...
class ServiceLocalBooru(ServiceLocalServerService):
    def GetExternalShareURL(self, share_key):
        ...
        url = '{}://{}{}/gallery?share_key={}'.format(scheme, host, port,
                                                      share_key.hex())
...
class ServiceRemote(Service):
    def __init__(self, service_key, service_type, name, dictionary=None):
        ...
        self.network_context = ClientNetworkingContexts.NetworkContext(
    ...
    def _CheckFunctional(self,
                         including_external_communication=True,
                         including_bandwidth=True):

        if including_external_communication:

            self._CheckCanCommunicateExternally(
                including_bandwidth=including_bandwidth)
    ...
    def _CheckCanCommunicateExternally(self, including_bandwidth=True):
        ...
        if not HydrusData.TimeHasPassed(self._no_requests_until):
            ...
            raise HydrusExceptions.InsufficientCredentialsException(
                self._no_requests_reason + ' - next request ' +
                HydrusData.TimestampToPrettyTimeDelta(self._no_requests_until))

        if including_bandwidth:

            example_nj = ClientNetworkingJobs.NetworkJobHydrus(
                self._service_key, 'GET', self._GetBaseURL())

            can_start = HG.client_controller.network_engine.bandwidth_manager.CanDoWork(
                example_nj.GetNetworkContexts(), threshold=60)
    ...
    def GetBandwidthCurrentMonthSummary(self):
        ...
        with self._lock:
            ...
            return HG.client_controller.network_engine.bandwidth_manager.GetCurrentMonthSummary(
    ...
    def GetBandwidthStringsAndGaugeTuples(self):
        ...
        with self._lock:
            ...
            return HG.client_controller.network_engine.bandwidth_manager.GetBandwidthStringsAndGaugeTuples(
                self.network_context)
...
class ServiceRestricted(ServiceRemote):
    ...
    def _GetErrorWaitPeriod(self):
        ...
        if self._account.HasPermission(HC.CONTENT_TYPE_ACCOUNTS,
    ...
    def _CanSyncAccount(self, including_external_communication=True):
        ...
        try:
            ...
            self._CheckFunctional(including_external_communication=
                                  including_external_communication,
                                  including_account=False)
    ...
    def _CheckFunctional(self,
                         including_external_communication=True,
                         including_bandwidth=True,
                         including_account=True):

        if including_account:

            self._account.CheckFunctional()

        ServiceRemote._CheckFunctional(
            self,
            including_external_communication=including_external_communication,
            including_bandwidth=including_bandwidth)
    ...
    def _CheckCanCommunicateExternally(self, including_bandwidth=True):
        ...
        if not self._credentials.HasAccessKey():
            ...
            raise HydrusExceptions.MissingCredentialsException(
                'this service has no access key set')

        ServiceRemote._CheckCanCommunicateExternally(
            self, including_bandwidth=including_bandwidth)
    ...
    def _LoadFromDictionary(self, dictionary):
        ...
        self._account = HydrusNetwork.Account.GenerateAccountFromSerialisableTuple(
            dictionary['account'])
    ...
    def CanSyncAccount(self, including_external_communication=True):
        ...
        with self._lock:
            ...
            return self._CanSyncAccount(including_external_communication=
                                        including_external_communication)
    ...
    def CheckFunctional(self,
        ...
        with self._lock:
            ...
            self._CheckFunctional(including_external_communication=
                                  including_external_communication,
                                  including_bandwidth=including_bandwidth,
                                  including_account=including_account)
    ...
    def Request(self,
        ...
        try:
            ...
            network_job = ClientNetworkingJobs.NetworkJobHydrus(
                self._service_key, method, url, body=body, temp_path=temp_path)
            ...
            if command in ('', 'access_key', 'access_key_verification'):
                ...
                if command == 'access_key_verification':
                    ...
                    network_job.AddAdditionalHeader(
                        'Hydrus-Key',
                        self._credentials.GetAccessKey().hex())

            if content_type is not None:

                network_job.AddAdditionalHeader(
                    'Content-Type',
                    HC.mime_mimetype_string_lookup[content_type])
            ...
            if content_type == 'application/json':
                ...
                parsed_args = HydrusNetwork.ParseNetworkBytesToParsedHydrusArgs(
                    network_bytes)
            ...
            with self._lock:
                ...
                if isinstance(e, HydrusExceptions.ServerBusyException):
                    ...
                    HG.client_controller.network_engine.session_manager.ClearSession(
                        ...
                        e,
                    (HydrusExceptions.MissingCredentialsException,
                    ...
                    self._DelayFutureRequests('got an unexpected 404',
                                              HC.UPDATE_DURATION)

                elif isinstance(e, HydrusExceptions.BandwidthException):

                    self._DelayFutureRequests('service has exceeded bandwidth',
                                              HC.UPDATE_DURATION * 5)
        ...
        if do_it:
            ...
            try:
                ...
                HG.client_controller.pub('notify_new_permissions')
                    ...
                    HydrusExceptions.NetworkException) as e:
...
class ServiceRepository(ServiceRestricted):
    def __init__(self, service_key, service_type, name, dictionary=None):
        ...
        ServiceRestricted.__init__(self,
                                   service_key,
                                   service_type,
                                   name,
                                   dictionary=dictionary)
    ...
    def _CanSyncProcess(self):
        ...
        try:
            ...
            self._CheckFunctional(including_external_communication=False,
                                  including_bandwidth=False,
                                  including_account=False)
    ...
    def _CheckFunctional(self,
        ...
        ServiceRestricted._CheckFunctional(
            self,
            including_external_communication=including_external_communication,
            including_bandwidth=including_bandwidth,
            including_account=including_account)
    ...
    def _LogFinalRowSpeed(self, precise_timestamp, total_rows, row_name):
        ...
        summary = '{} processed {} {} at {} rows/s'.format(
            self._name, HydrusData.ToHumanInt(total_rows), row_name, rows_s)

        HydrusData.Print(summary)

    def _ReportOngoingRowSpeed(self, job_key, rows_done, total_rows,
                               precise_timestamp, rows_done_in_last_packet,
                               row_name):
        ...
        popup_message = '{} {}: processing at {} rows/s'.format(
            row_name,
            HydrusData.ConvertValueRangeToPrettyString(rows_done, total_rows),
            rows_s)

        HG.client_controller.pub('splash_set_status_text',
                                 popup_message,
                                 print_to_log=False)
    ...
    def _SyncDownloadMetadata(self):
        ...
        if do_it:
            ...
            try:
                ...
                response = self.Request(HC.GET, 'metadata',
                                        {'since': next_update_index})
                ...
                HydrusData.Print('Attempting to download metadata for ' +
                                 name + ' resulted in a network error:')
            ...
            HG.client_controller.WriteSynchronous(
                'associate_repository_update_hashes', service_key,
                metadata_slice)
    ...
    def _SyncDownloadUpdates(self, stop_time):
        ...
        update_hashes = HG.client_controller.Read(
            'missing_repository_update_hashes', service_key)
        ...
        if len(update_hashes) > 0:
            ...
            try:
                ...
                job_key.SetVariable('popup_title',
                                    name + ' sync: downloading updates')

                HG.client_controller.pub('message', job_key)

                for (i, update_hash) in enumerate(update_hashes):

                    status = 'update ' + HydrusData.ConvertValueRangeToPrettyString(
                        i + 1, len(update_hashes))
                    ...
                    if should_quit:
                        ...
                        with self._lock:
                            ...
                            self._DelayFutureRequests(
                                'download was recently cancelled', 3 * 60)

                        return

                    try:

                        update_network_string = self.Request(
                            HC.GET, 'update', {'update_hash': update_hash})
                        ...
                        HydrusData.Print(
                            'Attempting to download an update for ' + name +
                    ...
                    if update_network_string_hash != update_hash:
                        ...
                        with self._lock:
                            ...
                            self._DelayFutureRequests(
                                'had an unusual update response')
                    ...
                    try:
                        ...
                        message = 'Update ' + update_hash.hex(
                        ) + ' downloaded from the ' + self._name + ' repository failed to load! This is a serious error!'
                    ...
                    if isinstance(update, HydrusNetwork.DefinitionsUpdate):
                        ...
                        message = 'Update ' + update_hash.hex(
                        ) + ' downloaded from the ' + self._name + ' was not a valid update--it was a ' + repr(
                            update) + '! This is a serious error!'
                        message += os.linesep * 2
                        message += 'The repository has been paused for now. Please look into what could be wrong and report this to the hydrus dev.'
                    ...
                    try:
                        ...
                        HG.client_controller.WriteSynchronous(
                            'import_update', update_network_string,
                            update_hash, mime)
    ...
    def _SyncProcessUpdates(self,
        ...
        if HG.client_controller.ShouldStopThisWork(maintenance_mode,
                                                   stop_time=stop_time):
        ...
        try:
            ...
            (this_is_first_definitions_work, definition_hashes,
             this_is_first_content_work,
             content_hashes) = HG.client_controller.Read(
                 'repository_update_hashes_to_process', self._service_key)
            ...
            HG.client_controller.pub('message', job_key)
            HG.client_controller.pub('splash_set_title_text',
                                     title,
                                     print_to_log=False)
            ...
            try:
                ...
                for definition_hash in definition_hashes:
                    ...
                    progress_string = HydrusData.ConvertValueRangeToPrettyString(
                        num_updates_done + 1, num_updates_to_do)

                    splash_title = '{} sync: processing updates {}'.format(
                        self._name, progress_string)

                    HG.client_controller.pub('splash_set_title_text',
                                             splash_title,
                                             clear_undertexts=False,
                                             print_to_log=False)
                    ...
                    job_key.SetVariable('popup_text_1', status)
                    job_key.SetVariable('popup_gauge_1',
                                        (num_updates_done, num_updates_to_do))

                    try:

                        update_path = HG.client_controller.client_files_manager.GetFilePath(
                            definition_hash,
                            HC.APPLICATION_HYDRUS_UPDATE_DEFINITIONS)

                    except HydrusExceptions.FileMissingException:

                        HG.client_controller.WriteSynchronous(
                            'schedule_repository_update_file_maintenance',
                            self._service_key, ClientFiles.
                            REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_PRESENCE)

                        raise Exception(
                            'An unusual error has occured during repository processing: an update file was missing. Your repository should be paused, and all update files have been scheduled for a presence check. Please permit file maintenance to check them, or tell it to do so manually, before unpausing your repository.'
                        )
                    ...
                    try:
                        ...
                        definition_update = HydrusSerialisable.CreateFromNetworkBytes(
                        ...
                        HG.client_controller.WriteSynchronous(
                            'schedule_repository_update_file_maintenance',
                            self._service_key, ClientFiles.
                            REGENERATE_FILE_DATA_JOB_FILE_INTEGRITY_DATA)

                        raise Exception(
                            'An unusual error has occured during repository processing: an update file was invalid. Your repository should be paused, and all update files have been scheduled for an integrity check. Please permit file maintenance to check them, or tell it to do so manually, before unpausing your repository.'
                        )

                    if not isinstance(definition_update,
                                      HydrusNetwork.DefinitionsUpdate):
                        ...
                        raise Exception(
                            'An unusual error has occured during repository processing: an update file has incorrect metadata. Your repository should be paused, and all update files have been scheduled for a metadata rescan. Please permit file maintenance to fix them, or tell it to do so manually, before unpausing your repository.'
                        )
                    ...
                    iterator_dict['service_hash_ids_to_hashes'] = iter(
                        definition_update.GetHashIdsToHashes().items())
                    iterator_dict['service_tag_ids_to_tags'] = iter(
                        definition_update.GetTagIdsToTags().items())
                    ...
                    while len(iterator_dict) > 0:
                        ...
                        num_rows_done = HG.client_controller.WriteSynchronous(
                            'process_repository_definitions',
                            self._service_key, definition_hash, iterator_dict,
                            job_key, work_time)
                        ...
                        if HG.client_controller.ShouldStopThisWork(
                                maintenance_mode,
                                stop_time=stop_time) or job_key.IsCancelled():

                            return

                        if HydrusData.TimeHasPassedPrecise(
                                this_work_start_time + work_time):

                            time.sleep(break_time)

                            HG.client_controller.WaitUntilViewFree()

                        self._ReportOngoingRowSpeed(job_key,
                                                    rows_done_in_this_update,
                                                    rows_in_this_update,
                                                    this_work_start_time,
                                                    num_rows_done,
                                                    'definitions')
                ...
                self._LogFinalRowSpeed(definition_start_time,
                                       total_definition_rows_completed,
                                       'definitions')
            ...
            try:
                ...
                for content_hash in content_hashes:
                    ...
                    try:
                        ...
                        update_path = HG.client_controller.client_files_manager.GetFilePath(
                            content_hash, HC.APPLICATION_HYDRUS_UPDATE_CONTENT)
                    ...
                    try:
                        ...
                        content_update = HydrusSerialisable.CreateFromNetworkBytes(
                            update_network_bytes)
                    ...
                    if not isinstance(content_update,
                                      HydrusNetwork.ContentUpdate):
                    ...
                    iterator_dict['new_files'] = iter(
                        content_update.GetNewFiles())
                    iterator_dict['deleted_files'] = iter(
                        content_update.GetDeletedFiles())
                    iterator_dict[
                        'new_mappings'] = HydrusData.SmoothOutMappingIterator(
                            content_update.GetNewMappings(), 50)
                    iterator_dict[
                        'deleted_mappings'] = HydrusData.SmoothOutMappingIterator(
                            content_update.GetDeletedMappings(), 50)
                    iterator_dict['new_parents'] = iter(
                        content_update.GetNewTagParents())
                    iterator_dict['deleted_parents'] = iter(
                        content_update.GetDeletedTagParents())
                    iterator_dict['new_siblings'] = iter(
                        content_update.GetNewTagSiblings())
                    iterator_dict['deleted_siblings'] = iter(
                        content_update.GetDeletedTagSiblings())
                    ...
                    while len(iterator_dict) > 0:
                        ...
                        num_rows_done = HG.client_controller.WriteSynchronous(
                            'process_repository_content', self._service_key,
                            content_hash, iterator_dict, job_key, work_time)
                ...
                self._LogFinalRowSpeed(content_start_time,
                                       total_content_rows_completed,
                                       'content rows')
    ...
    def CanDoIdleShutdownWork(self):
        ...
        (download_value, processing_value,
         range) = HG.client_controller.Read('repository_progress', service_key)
    ...
    def IsMostlyCaughtUp(self):
        ...
        unprocessed_update_hashes = HG.client_controller.Read(
            'repository_unprocessed_hashes', service_key)
        ...
        with self._lock:
            ...
            if len(unprocessed_update_hashes) == 0:
                ...
                earliest_unsorted_update_timestamp = self._metadata.GetEarliestTimestampForTheseHashes(
                    unprocessed_update_hashes)
    ...
    def SyncRemote(self, stop_time=None):
        ...
        with self._sync_remote_lock:
            ...
            try:
                ...
                HydrusData.ShowText(
                    'The service "{}" encountered an error while trying to sync! The error was "{}". It will not do any work for a little while. If the fix is not obvious, please elevate this to hydrus dev.'
    ...
    def SyncProcessUpdates(self,
                           maintenance_mode=HC.MAINTENANCE_IDLE,
                           stop_time=None):

        with self._sync_processing_lock:

            self._SyncProcessUpdates(maintenance_mode=maintenance_mode,
                                     stop_time=stop_time)
    ...
    def SyncThumbnails(self, stop_time):
        ...
        thumbnail_hashes = HG.client_controller.Read(
            'missing_thumbnail_hashes', service_key)
        ...
        if num_to_do > 0:
            ...
            try:
                ...
                job_key.SetVariable('popup_title',
                                    name + ' sync: downloading thumbnails')

                HG.client_controller.pub('message', job_key)

                for (i, thumbnail_hash) in enumerate(thumbnail_hashes):

                    status = 'thumbnail ' + HydrusData.ConvertValueRangeToPrettyString(
                        i + 1, num_to_do)
                    ...
                    try:
                        ...
                        thumbnail_bytes = self.Request(
                            HC.GET, 'thumbnail', {'hash': thumbnail_hash})
                        ...
                        HydrusData.Print(
                            'Attempting to download a thumbnail for ' + name +
                            ' resulted in a network error:')
                    ...
                    client_files_manager.AddThumbnailFromBytes(
                        thumbnail_hash, thumbnail_bytes)
...
class ServiceIPFS(ServiceRemote):
    def _GetSerialisableDictionary(self):
        ...
        dictionary[
            'nocopy_abs_path_translations'] = self._nocopy_abs_path_translations
    ...
    def _LoadFromDictionary(self, dictionary):
        ...
        self._nocopy_abs_path_translations = dictionary[
            'nocopy_abs_path_translations']
    ...
    def ConvertMultihashToURLTree(self, name, size, multihash, job_key=None):
            ...
            if job_key is not None:
                ...
                if job_key.IsCancelled():
                    ...
                    raise HydrusExceptions.CancelledException(
                        'Multihash parsing cancelled by user.')
        ...
        if is_directory:
            ...
            for link in links_json['Links']:
                ...
                children.append(
                    self.ConvertMultihashToURLTree(subname,
                                                   subsize,
                                                   submultihash,
                                                   job_key=job_key))
            ...
            if size is None:
                ...
                size = sum((subsize for (type_gumpf, subname, subsize,
                                         submultihash) in children))
    ...
    def EnableNoCopy(self, value):
        ...
        url = api_base_url + 'config?arg=Experimental.FilestoreEnabled&arg={}&bool=true'.format(
            arg_value)
    ...
    def ImportFile(self, multihash, silent=False):
        def on_qt_select_tree(job_key, url_tree):
            ...
            try:
                ...
                with ClientGUIDialogs.DialogSelectFromURLTree(
                        HG.client_controller.gui, url_tree) as dlg:
                    ...
                    if dlg.exec() == QW.QDialog.Accepted:
                        ...
                        if len(urls) > 0:
                            ...
                            HG.client_controller.CallToThread(
                                ClientImporting.THREADDownloadURLs, job_key,
                                urls, multihash)
        ...
        def off_qt():
            ...
            job_key.SetVariable('popup_text_1',
                                'Looking up multihash information')
            ...
            try:
                ...
                try:
                    ...
                    url_tree = self.ConvertMultihashToURLTree(multihash,
                                                              None,
                                                              multihash,
                                                              job_key=job_key)
                    ...
                    job_key.SetVariable(
                        'popup_text_1',
                        'Failed to find multihash information for "{}"!'.
                        format(multihash))
                ...
                if url_tree[0] == 'file':
                    ...
                    HG.client_controller.CallToThread(
                        ClientImporting.THREADDownloadURL, job_key, url,
                        multihash)
    ...
    def PinDirectory(self, hashes, note):
        ...
        job_key.SetVariable('popup_title',
                            'creating ipfs directory on ' + self._name)
        ...
        try:
            ...
            for (i, hash) in enumerate(hashes):
                ...
                job_key.SetVariable(
                    'popup_text_1', 'ensuring files are pinned: ' +
                ...
                result = HG.client_controller.Read('service_filenames',
                ...
                if len(result) == 0:
                    ...
                    try:
                        ...
                        HydrusData.ShowText(
                            'File {} could not be pinned!'.format(hash.hexh()))
            ...
            for (i, (hash, mime, multihash)) in enumerate(file_info):
                ...
                job_key.SetVariable(
                    'popup_text_1', 'creating directory: ' +
                    HydrusData.ConvertValueRangeToPrettyString(
                        i + 1, len(file_info)))
            ...
            content_updates = [
                HydrusData.ContentUpdate(HC.CONTENT_TYPE_DIRECTORIES,
            ...
            job_key.SetVariable('popup_clipboard',
                                ('copy multihash to clipboard', text))
    ...
    def PinFile(self, hash, mime):
        ...
        with open(path, 'rb') as f:
            ...
            if self._use_nocopy:
                ...
                for (hydrus_portable_path, ipfs_translation
                     ) in self._nocopy_abs_path_translations.items():

                    hydrus_path = HydrusPaths.ConvertPortablePathToAbsPath(
                        hydrus_portable_path)
                    ...
                    if path.startswith(hydrus_path):
                        ...
                        if ipfs_translation == '':
                            ...
                            raise Exception(
                                'The path {} does not have an IPFS translation set! Please check your IPFS path mappings under manage services!'
                                .format(hydrus_path))

                        ipfs_abspath = path.replace(hydrus_path,
                                                    ipfs_translation)
                ...
                if ipfs_abspath is None:
                    ...
                    raise Exception(
                        'Could not figure out an ipfs absolute path for {}! Have new paths been added due to a database migration? Please check your IPFS path mappings under manage services!'
                        .format(path))

                files = {
                    'path': (hash.hex(), f, mime_string, {
                        'Abspath': ipfs_abspath
                    })
                }
        ...
        content_updates = [
            HydrusData.ContentUpdate(HC.CONTENT_TYPE_FILES,
                                     HC.CONTENT_UPDATE_ADD, content_update_row)
        ]
    ...
    def UnpinDirectory(self, multihash):
        ...
        content_updates = [
            HydrusData.ContentUpdate(HC.CONTENT_TYPE_DIRECTORIES,
                                     HC.CONTENT_UPDATE_DELETE, multihash)
        ]
